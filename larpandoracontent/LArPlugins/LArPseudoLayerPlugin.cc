...
namespace lar_content {
  ...
  LArPseudoLayerPlugin::LArPseudoLayerPlugin()
    : m_zPitch(std::numeric_limits<float>::max()), m_zOffset(0.01f), m_zerothLayer(5000)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  unsigned int LArPseudoLayerPlugin::GetPseudoLayer(
    const pandora::CartesianVector& positionVector) const
  {
    const float zLayer((positionVector.GetZ() + m_zOffset) / m_zPitch +
                       static_cast<float>(m_zerothLayer));
  ...
  StatusCode LArPseudoLayerPlugin::Initialize()
  {
    ...
    if (larTPCMap.empty()) {
      std::cout << "LArPseudoLayerPlugin::Initialize - LArTPC description not registered with "
                   "Pandora as required "
                << std::endl;
    ...
    for (const LArTPCMap::value_type& mapEntry : larTPCMap) {
      ...
      if (std::fabs(m_zPitch - pLArTPC->GetWirePitchW()) > std::numeric_limits<float>::epsilon()) {
        std::cout << "LArPseudoLayerPlugin::Initialize - Plugin does not support provided LArTPC "
                     "configurations "
                  << std::endl;
