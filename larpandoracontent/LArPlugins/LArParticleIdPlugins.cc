...
namespace lar_content {
  ...
  {
    if (LArClusterHelper::GetLayerOccupancy(pCluster) < m_minLayerOccupancy) return false;

    const float slidingFitPitch(LArGeometryHelper::GetWireZPitch(this->GetPandora()));
    const TwoDSlidingFitResult twoDSlidingFitResult(
      pCluster, m_layerFitHalfWindow, slidingFitPitch);

    if (this->GetMuonTrackWidth(twoDSlidingFitResult) > m_maxTrackWidth) return false;
  ...
  bool LArParticleIdPlugins::LArMuonId::IsMatch(const ParticleFlowObject* const pPfo) const
  {
    ...
    if (clusterList.empty()) return false;

    unsigned int nClustersPassing(0);

    for (const Cluster* const pCluster : clusterList) {
      if (this->IsMatch(pCluster)) ++nClustersPassing;
    }

    if (nClustersPassing < m_minClustersPassingId) return false;
  ...
  float LArParticleIdPlugins::LArMuonId::GetMuonTrackWidth(
    const TwoDSlidingFitResult& twoDSlidingFitResult) const
  {
    FloatVector residuals;
    const OrderedCaloHitList& orderedCaloHitList(
      twoDSlidingFitResult.GetCluster()->GetOrderedCaloHitList());
    ...
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin();
      ...
      for (CaloHitList::const_iterator hitIter = iter->second->begin(),
        ...
        if (layerFitResultMap.end() == fitResultIter) continue;
        ...
        const double residualSquared(
          (fitT - rT) * (fitT - rT) /
          (1. + gradient * gradient)); // angular correction (note: this is cheating!)
        residuals.push_back(residualSquared);
      }
    }

    if (residuals.empty()) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
  ...
  StatusCode LArParticleIdPlugins::LArMuonId::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "LayerFitHalfWindow", m_layerFitHalfWindow));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinLayerOccupancy", m_minLayerOccupancy));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxTrackWidth", m_maxTrackWidth));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "TrackResidualQuantile", m_trackResidualQuantile));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinClustersPassingId", m_minClustersPassingId));
