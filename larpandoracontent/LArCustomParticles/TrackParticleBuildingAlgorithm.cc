...
namespace lar_content {
  ...
  void TrackParticleBuildingAlgorithm::CreatePfo(const ParticleFlowObject* const pInputPfo,
                                                 const ParticleFlowObject*& pOutputPfo) const
  {
    try {
      ...
      if (LArPfoHelper::IsNeutrinoFinalState(pInputPfo)) {
        if (!LArPfoHelper::IsTrack(pInputPfo)) return;
      }
      else {
        if (!LArPfoHelper::IsFinalState(pInputPfo)) return;

        if (LArPfoHelper::IsNeutrino(pInputPfo)) return;
      }
      ...
      LArPfoHelper::GetSlidingFitTrajectory(
        pInputPfo, pInputVertex, m_slidingFitHalfWindow, layerPitch, trackStateVector);

      if (trackStateVector.empty()) return;
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ParticleFlowObject::AlterMetadata(*this, pOutputPfo, pfodata));
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::Vertex::Create(*this, vtxParameters, pOutputVertex));
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToPfo(*this, pOutputPfo, pOutputVertex));
    }
