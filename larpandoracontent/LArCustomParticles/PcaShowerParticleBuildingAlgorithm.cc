...
namespace lar_content {
  ...
  void PcaShowerParticleBuildingAlgorithm::CreatePfo(const ParticleFlowObject* const pInputPfo,
                                                     const ParticleFlowObject*& pOutputPfo) const
  {
    try {
      ...
      if (LArPfoHelper::IsNeutrinoFinalState(pInputPfo)) {
        if (!LArPfoHelper::IsShower(pInputPfo)) return;
      }
      else {
      ...
      pfoParameters.m_particleId =
        (LArPfoHelper::IsShower(pInputPfo) ? pInputPfo->GetParticleId() : E_MINUS);
      ...
      pfoParameters.m_showerOpeningAngle =
        (showerPCA.GetPrimaryLength() > 0.f ?
           std::atan(showerPCA.GetSecondaryLength() / showerPCA.GetPrimaryLength()) :
           0.f);
      ...
      if (!pLArPfo) throw StatusCodeException(STATUS_CODE_FAILURE);
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::AddToPfo<Vertex>(*this, pOutputPfo, pOutputVertex));
    }
