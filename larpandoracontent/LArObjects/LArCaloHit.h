 ...
 */
  class LArCaloHitFactory : public pandora::ObjectFactory<object_creation::CaloHit::Parameters,
  ...
  inline LArCaloHit::LArCaloHit(const LArCaloHitParameters& parameters)
    ...
    , m_daughterVolumeId(
        parameters.m_daughterVolumeId.IsInitialized() ? parameters.m_daughterVolumeId.Get() : 0)
  {}
  ...
  inline pandora::StatusCode LArCaloHitFactory::Create(const Parameters& parameters,
                                                       const Object*& pObject) const
  {
    const LArCaloHitParameters& larCaloHitParameters(
      dynamic_cast<const LArCaloHitParameters&>(parameters));
  ...
  inline pandora::StatusCode LArCaloHitFactory::Read(Parameters& parameters,
                                                     pandora::FileReader& fileReader) const
  {
    ...
    if (pandora::BINARY == fileReader.GetFileType()) {
      ...
      PANDORA_RETURN_RESULT_IF(
        pandora::STATUS_CODE_SUCCESS, !=, binaryFileReader.ReadVariable(larTPCVolumeId));
      if (m_version > 1)
        PANDORA_RETURN_RESULT_IF(
          pandora::STATUS_CODE_SUCCESS, !=, binaryFileReader.ReadVariable(daughterVolumeId));
    }
      ...
      PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS,
                               !=,
                               xmlFileReader.ReadVariable("LArTPCVolumeId", larTPCVolumeId));
      if (m_version > 1)
        PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS,
                                 !=,
                                 xmlFileReader.ReadVariable("DaughterVolumeId", daughterVolumeId));
    }
    else {
  ...
  inline pandora::StatusCode LArCaloHitFactory::Write(const Object* const pObject,
                                                      pandora::FileWriter& fileWriter) const
  {
    ...
    if (!pLArCaloHit) return pandora::STATUS_CODE_INVALID_PARAMETER;
    ...
    if (pandora::BINARY == fileWriter.GetFileType()) {
      ...
      PANDORA_RETURN_RESULT_IF(pandora::STATUS_CODE_SUCCESS,
                               !=,
                               binaryFileWriter.WriteVariable(pLArCaloHit->GetLArTPCVolumeId()));
      if (m_version > 1)
        PANDORA_RETURN_RESULT_IF(
          pandora::STATUS_CODE_SUCCESS,
          !=,
          binaryFileWriter.WriteVariable(pLArCaloHit->GetDaughterVolumeId()));
    }
      ...
      PANDORA_RETURN_RESULT_IF(
        pandora::STATUS_CODE_SUCCESS,
        !=,
        xmlFileWriter.WriteVariable("LArTPCVolumeId", pLArCaloHit->GetLArTPCVolumeId()));
      if (m_version > 1)
        PANDORA_RETURN_RESULT_IF(
          pandora::STATUS_CODE_SUCCESS,
          !=,
          xmlFileWriter.WriteVariable("DaughterVolumeId", pLArCaloHit->GetDaughterVolumeId()));
    }
    else {
      return pandora::STATUS_CODE_INVALID_PARAMETER;
    }
