...
namespace lar_content {
  ...
  TwoViewDeltaRayOverlapResult::TwoViewDeltaRayOverlapResult(
    const TwoViewXOverlap& xOverlap,
  ...
  TwoViewDeltaRayOverlapResult::TwoViewDeltaRayOverlapResult(
    const TwoViewDeltaRayOverlapResult& rhs)
  ...
  const Cluster* TwoViewDeltaRayOverlapResult::GetBestMatchedAvailableCluster() const
  {
    ...
    for (const Cluster* const pMatchedCluster : m_matchedClusterList) {
      if (!pMatchedCluster->IsAvailable()) continue;
  ...
  TwoViewDeltaRayOverlapResult& TwoViewDeltaRayOverlapResult::operator=(
    const TwoViewDeltaRayOverlapResult& rhs)
  {
    m_isInitialized = rhs.m_isInitialized;
    m_xOverlap = rhs.GetXOverlap();
  ...
  bool TwoViewDeltaRayOverlapResult::operator<(const TwoViewDeltaRayOverlapResult& rhs) const
  {
    ...
    if (!rhs.m_isInitialized) return false;
  ...
  TrackTwoViewOverlapResult::TrackTwoViewOverlapResult()
    : m_isInitialized(false), m_matchingScore(0)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  TrackTwoViewOverlapResult::TrackTwoViewOverlapResult(const float matchingScore)
    : m_isInitialized(true), m_matchingScore(matchingScore)
  {}
  ...
  TrackTwoViewOverlapResult& TrackTwoViewOverlapResult::operator=(
    const TrackTwoViewOverlapResult& rhs)
  {
  ...
  TwoViewTransverseOverlapResult::TwoViewTransverseOverlapResult(
    const TwoViewTransverseOverlapResult& rhs)
    ...
    , m_twoViewXOverlap(rhs.m_isInitialized ? rhs.m_twoViewXOverlap :
  ...
  bool TwoViewTransverseOverlapResult::operator<(const TwoViewTransverseOverlapResult& rhs) const
  {
    ...
    if (std::fabs(m_correlationCoefficient - rhs.m_correlationCoefficient) >
    ...
    if (std::fabs(this->GetLocallyMatchedFraction() - rhs.GetLocallyMatchedFraction()) >
        std::numeric_limits<float>::epsilon())
      return (this->GetLocallyMatchedFraction() < rhs.GetLocallyMatchedFraction());

    if (std::fabs(m_twoViewXOverlap.GetTwoViewXOverlapSpan() -
                  rhs.m_twoViewXOverlap.GetTwoViewXOverlapSpan()) >
        std::numeric_limits<float>::epsilon())
      return (m_twoViewXOverlap.GetTwoViewXOverlapSpan() <
              rhs.m_twoViewXOverlap.GetTwoViewXOverlapSpan());

    if (std::fabs(m_twoViewXOverlap.GetXSpan0() - rhs.m_twoViewXOverlap.GetXSpan0()) >
        std::numeric_limits<float>::epsilon())
  ...
  TwoViewTransverseOverlapResult& TwoViewTransverseOverlapResult::operator=(
    const TwoViewTransverseOverlapResult& rhs)
  {
