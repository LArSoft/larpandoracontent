...
namespace lar_content {
  ...
  template <>
  TwoDSlidingFitResult::TwoDSlidingFitResult(const Cluster* const pCluster,
    ...
    , m_orthoDirection(0.f, 0.f, 0.f)
  {
    ...
    if (std::fabs(cosOpeningAngle) < axisDeviationLimitForHitDivision) {
      ...
      LArHitWidthHelper::ConstituentHitVector constituentHitVector(
        LArHitWidthHelper::GetConstituentHits(pCluster, 0.5f, 1.f, true));
      const CartesianPointVector constituentHitPointVector(
        LArHitWidthHelper::GetConstituentHitPositionVector(constituentHitVector));
  ...
  template <>
  TwoDSlidingFitResult::TwoDSlidingFitResult(const CartesianPointVector* const pPointVector,
                                             const unsigned int layerFitHalfWindow,
                                             const float layerPitch,
                                             const float /*axisDeviationLimitForHitDivision*/)
    : m_pCluster(nullptr)
  ...
  TwoDSlidingFitResult::TwoDSlidingFitResult(const unsigned int layerFitHalfWindow,
                                             const float layerPitch,
                                             const CartesianVector& axisIntercept,
                                             const CartesianVector& axisDirection,
                                             const CartesianVector& orthoDirection,
  ...
  {
    if (!m_pCluster) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
  ...
  {
    if (m_layerFitResultMap.empty()) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
  ...
  void TwoDSlidingFitResult::GetLocalPosition(const CartesianVector& position,
                                              float& rL,
                                              float& rT) const
  {
  ...
  void TwoDSlidingFitResult::GetGlobalPosition(const float rL,
                                               const float rT,
  ...
  {
    if (m_layerFitResultMap.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
  ...
  float TwoDSlidingFitResult::GetCosScatteringAngle(const float rL) const
  {
    ...
    const StatusCode secondStatusCode(
      this->GetGlobalFitDirection(rL + halfWindowLength, secondDirection));

    if (STATUS_CODE_SUCCESS != firstStatusCode) throw StatusCodeException(firstStatusCode);

    if (STATUS_CODE_SUCCESS != secondStatusCode) throw StatusCodeException(secondStatusCode);
  ...
  StatusCode TwoDSlidingFitResult::GetGlobalFitPosition(const float rL,
                                                        CartesianVector& position) const
  {
    LayerInterpolation layerInterpolation;
    const StatusCode statusCode(this->LongitudinalInterpolation(rL, layerInterpolation));

    if (STATUS_CODE_SUCCESS != statusCode) return statusCode;
  ...
  StatusCode TwoDSlidingFitResult::GetGlobalFitDirection(const float rL,
  ...
  StatusCode TwoDSlidingFitResult::GetGlobalFitPositionAtX(const float x,
                                                           CartesianVector& position) const
  {
    ...
    if (layerInterpolationList.size() != 1) return STATUS_CODE_NOT_FOUND;
  ...
  StatusCode TwoDSlidingFitResult::GetGlobalFitDirectionAtX(const float x,
                                                            CartesianVector& direction) const
  {
    LayerInterpolationList layerInterpolationList;
    const StatusCode statusCode(this->TransverseInterpolation(x, layerInterpolationList));
  ...
  StatusCode TwoDSlidingFitResult::GetGlobalFitProjection(const CartesianVector& inputPosition,
                                                          CartesianVector& projectedPosition) const
  {
    float rL(0.f), rT(0.f);
  ...
  StatusCode TwoDSlidingFitResult::GetGlobalFitPositionListAtX(
    const float x,
    CartesianPointVector& positionList) const
  {
    LayerInterpolationList layerInterpolationList;
    const StatusCode statusCode(this->TransverseInterpolation(x, layerInterpolationList));
    ...
    for (LayerInterpolationList::const_iterator iter = layerInterpolationList.begin(),
                                                iterEnd = layerInterpolationList.end();
         iter != iterEnd;
         ++iter) {
  ...
  StatusCode TwoDSlidingFitResult::GetTransverseProjection(const float x,
  ...
  StatusCode TwoDSlidingFitResult::GetExtrapolatedPosition(const float rL,
                                                           CartesianVector& position) const
  {
    const StatusCode statusCode(this->GetGlobalFitPosition(rL, position));

    if (STATUS_CODE_NOT_FOUND != statusCode) return statusCode;
    ...
    if (thisLayer <= minLayer) {
      position = (this->GetGlobalMinLayerPosition() +
                  this->GetGlobalMinLayerDirection() * (rL - this->GetL(minLayer)));
    }
    else if (thisLayer >= maxLayer) {
      position = (this->GetGlobalMaxLayerPosition() +
                  this->GetGlobalMaxLayerDirection() * (rL - this->GetL(maxLayer)));
    }
    else {
  ...
  StatusCode TwoDSlidingFitResult::GetExtrapolatedDirection(const float rL,
                                                            CartesianVector& direction) const
  {
    ...
    if (thisLayer <= minLayer) { direction = this->GetGlobalMinLayerDirection(); }
    else if (thisLayer >= maxLayer) {
  ...
  StatusCode TwoDSlidingFitResult::GetExtrapolatedPositionAtX(const float x,
                                                              CartesianVector& position) const
  {
    ...
    const CartesianVector innerVertex((innerLayer == minLayer) ? this->GetGlobalMinLayerPosition() :
                                                                 this->GetGlobalMaxLayerPosition());
    const CartesianVector innerDirection((innerLayer == minLayer) ?
                                           this->GetGlobalMinLayerDirection() * -1.f :
                                           this->GetGlobalMaxLayerDirection());

    const CartesianVector outerVertex((outerLayer == minLayer) ? this->GetGlobalMinLayerPosition() :
                                                                 this->GetGlobalMaxLayerPosition());
    const CartesianVector outerDirection((outerLayer == minLayer) ?
                                           this->GetGlobalMinLayerDirection() * -1.f :
                                           this->GetGlobalMaxLayerDirection());

    if (innerDirection.GetX() > -std::numeric_limits<float>::epsilon() ||
        outerDirection.GetX() < +std::numeric_limits<float>::epsilon() ||
  ...
  const FitSegment& TwoDSlidingFitResult::GetFitSegment(const float rL) const
  {
    ...
    for (FitSegmentList::const_iterator iter = m_fitSegmentList.begin(),
                                        iterEnd = m_fitSegmentList.end();
         iter != iterEnd;
         ++iter) {
  ...
  void TwoDSlidingFitResult::CalculateAxes(const CartesianPointVector& coordinateVector,
                                           const float layerPitch)
  {
    if (coordinateVector.size() < 2) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    ...
    if (fitDirection.GetZ() < 0.f) fitDirection *= -1.f;
  ...
  void TwoDSlidingFitResult::FillLayerFitContributionMap(
    const CartesianPointVector& coordinateVector)
  {
    if (m_layerPitch < std::numeric_limits<float>::epsilon())
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    ...
    if (!m_layerFitContributionMap.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);

    for (CartesianPointVector::const_iterator iter = coordinateVector.begin(),
                                              iterEnd = coordinateVector.end();
         iter != iterEnd;
         ++iter) {
      float rL(0.f), rT(0.f);
  ...
  void TwoDSlidingFitResult::PerformSlidingLinearFit()
  {
    ...
    for (int iLayer = innerLayer; iLayer <= outerLayer; ++iLayer) {
      ...
      if (slidingNPoints <= 2) continue;

      // only fill the result map if there is an entry in the contribution map
      if (layerFitContributionMap.end() == layerFitContributionMap.find(iLayer)) continue;

      const double denominator(slidingSumLL -
                               slidingSumL * slidingSumL / static_cast<double>(slidingNPoints));

      if (std::fabs(denominator) < std::numeric_limits<float>::epsilon()) continue;

      const double gradient(
        (slidingSumLT - slidingSumL * slidingSumT / static_cast<double>(slidingNPoints)) /
        denominator);
      const double intercept((slidingSumLL * slidingSumT / static_cast<double>(slidingNPoints) -
                              slidingSumL * slidingSumLT / static_cast<double>(slidingNPoints)) /
                             denominator);
      double variance((slidingSumTT - 2. * intercept * slidingSumT - 2. * gradient * slidingSumLT +
                       intercept * intercept * static_cast<double>(slidingNPoints) +
                       2. * gradient * intercept * slidingSumL +
                       gradient * gradient * slidingSumLL) /
                      (1. + gradient * gradient));

      if (variance < -std::numeric_limits<float>::epsilon()) continue;

      if (variance < std::numeric_limits<float>::epsilon()) variance = 0.;
  ...
  {
    if (!m_fitSegmentList.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    for (LayerFitResultMap::const_iterator iter = layerFitResultMap.begin(),
                                           iterEnd = layerFitResultMap.end();
         iter != iterEnd;
         ++iter) {
      CartesianVector position(0.f, 0.f, 0.f);
      this->GetGlobalPosition(iter->second.GetL(), iter->second.GetFitT(), position);
      ...
      const TransverseDirection currentDirection(
        ((position - previousPosition).GetX() > 0.f) ? POSITIVE_IN_X : NEGATIVE_IN_X);
      ...
      if (previousDirection == currentDirection) {
        ...
        if ((POSITIVE_IN_X == sustainedDirection) || (NEGATIVE_IN_X == sustainedDirection))
          m_fitSegmentList.push_back(FitSegment(sustainedDirectionStartIter->first,
                                                sustainedDirectionEndIter->first,
                                                sustainedDirectionStartX,
                                                sustainedDirectionEndX));
  ...
  void TwoDSlidingFitResult::GetMinAndMaxCoordinate(const bool isX, float& min, float& max) const
  {
    ...
    for (LayerFitResultMap::const_iterator iter = m_layerFitResultMap.begin(),
                                           iterEnd = m_layerFitResultMap.end();
         iter != iterEnd;
         ++iter) {
  ...
  CartesianVector TwoDSlidingFitResult::GetGlobalFitPosition(
    const LayerInterpolation& layerInterpolation) const
  {
    ...
    this->GetGlobalPosition(
      firstLayerIter->second.GetL(), firstLayerIter->second.GetFitT(), firstLayerPosition);

    if (firstLayerIter == secondLayerIter) return firstLayerPosition;

    CartesianVector secondLayerPosition(0.f, 0.f, 0.f);
    this->GetGlobalPosition(
      secondLayerIter->second.GetL(), secondLayerIter->second.GetFitT(), secondLayerPosition);

    if (firstWeight + secondWeight < std::numeric_limits<float>::epsilon())
      throw StatusCodeException(STATUS_CODE_FAILURE);

    return ((firstLayerPosition * firstWeight + secondLayerPosition * secondWeight) *
            (1.f / (firstWeight + secondWeight)));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  CartesianVector TwoDSlidingFitResult::GetGlobalFitDirection(
    const LayerInterpolation& layerInterpolation) const
  {
    const LayerFitResultMap::const_iterator firstLayerIter(layerInterpolation.GetStartLayerIter());
    const LayerFitResultMap::const_iterator secondLayerIter(layerInterpolation.GetEndLayerIter());
    ...
    if (firstLayerIter == secondLayerIter) return firstLayerDirection;
    ...
    return (
      (firstLayerDirection * firstWeight + secondLayerDirection * secondWeight).GetUnitVector());
  }
  ...
  float TwoDSlidingFitResult::GetFitRms(const LayerInterpolation& layerInterpolation) const
  {
    ...
    if (firstLayerIter == secondLayerIter) return firstLayerRms;
    ...
    return ((firstLayerRms * firstWeight + secondLayerRms * secondWeight) /
            (firstWeight + secondWeight));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  StatusCode TwoDSlidingFitResult::LongitudinalInterpolation(
    const float rL,
    LayerInterpolation& layerInterpolation) const
  {
    ...
    this->GetLongitudinalInterpolationWeights(
      rL, firstLayerIter, secondLayerIter, firstWeight, secondWeight);
    layerInterpolation =
  ...
  StatusCode TwoDSlidingFitResult::TransverseInterpolation(
    const float x,
    const FitSegment& fitSegment,
    LayerInterpolation& layerInterpolation) const
  {
    ...
    const StatusCode statusCode(this->GetTransverseSurroundingLayers(
      x, fitSegment.GetStartLayer(), fitSegment.GetEndLayer(), firstLayerIter, secondLayerIter));

    if (STATUS_CODE_SUCCESS != statusCode) return statusCode;

    this->GetTransverseInterpolationWeights(
      x, firstLayerIter, secondLayerIter, firstWeight, secondWeight);
    layerInterpolation =
      LayerInterpolation(firstLayerIter, secondLayerIter, firstWeight, secondWeight);
  ...
  StatusCode TwoDSlidingFitResult::TransverseInterpolation(
    const float x,
    LayerInterpolationList& layerInterpolationList) const
  {
    for (FitSegmentList::const_iterator iter = m_fitSegmentList.begin(),
                                        iterEnd = m_fitSegmentList.end();
         iter != iterEnd;
    ...
    LayerFitResultMap::const_iterator& secondLayerIter) const
  {
    ...
    const int minLayer(m_layerFitResultMap.begin()->first),
      maxLayer(m_layerFitResultMap.rbegin()->first);
    const int thisLayer(this->GetLayer(rL));
    ...
    if ((startLayer < minLayer) || (startLayer >= maxLayer)) return STATUS_CODE_NOT_FOUND;
    ...
    for (int iLayer = startLayer; iLayer >= minLayer; --iLayer) {
      ...
      if (m_layerFitResultMap.end() != firstLayerIter) break;
    }

    if (m_layerFitResultMap.end() == firstLayerIter) return STATUS_CODE_NOT_FOUND;
    ...
    for (int iLayer = startLayer + 1; iLayer <= maxLayer; ++iLayer) {
      ...
      if (m_layerFitResultMap.end() != secondLayerIter) break;
    }

    if (m_layerFitResultMap.end() == secondLayerIter) return STATUS_CODE_NOT_FOUND;
  ...
  StatusCode TwoDSlidingFitResult::GetTransverseSurroundingLayers(
    const float x,
    const int minLayer,
    const int maxLayer,
    LayerFitResultMap::const_iterator& firstLayerIter,
    LayerFitResultMap::const_iterator& secondLayerIter) const
  {
    ...
    if (m_layerFitResultMap.end() == minLayerIter) throw StatusCodeException(STATUS_CODE_FAILURE);

    LayerFitResultMap::const_iterator maxLayerIter = m_layerFitResultMap.find(maxLayer);
    if (m_layerFitResultMap.end() == maxLayerIter) throw StatusCodeException(STATUS_CODE_FAILURE);

    CartesianVector minPosition(0.f, 0.f, 0.f), maxPosition(0.f, 0.f, 0.f);
    this->GetGlobalPosition(
      minLayerIter->second.GetL(), minLayerIter->second.GetFitT(), minPosition);
    this->GetGlobalPosition(
      maxLayerIter->second.GetL(), maxLayerIter->second.GetFitT(), maxPosition);

    if ((std::fabs(maxPosition.GetX() - minPosition.GetX()) <
    ...
    const float startL(minL + (maxL - minL) * (x - minPosition.GetX()) /
    ...
    for (int iLayer = startLayer; iLayer <= maxLayer; ++iLayer) {
      ...
      if (m_layerFitResultMap.end() != startLayerIter) break;
    }

    if (m_layerFitResultMap.end() == startLayerIter) return STATUS_CODE_NOT_FOUND;

    this->GetGlobalPosition(
      startLayerIter->second.GetL(), startLayerIter->second.GetFitT(), startLayerPosition);

    const bool startIsAhead((startLayerPosition.GetX() - x) >
                            std::numeric_limits<float>::epsilon());
    ...
    for (int iLayer = startLayerIter->first; (iLayer >= minLayer) && (iLayer <= maxLayer);
         iLayer += increment) {
      LayerFitResultMap::const_iterator tempIter = m_layerFitResultMap.find(iLayer);
      if (m_layerFitResultMap.end() == tempIter) continue;
  ...
  void TwoDSlidingFitResult::GetLongitudinalInterpolationWeights(
    const float rL,
  ...
  void TwoDSlidingFitResult::GetTransverseInterpolationWeights(
    const float x,
    const LayerFitResultMap::const_iterator& firstLayerIter,
    const LayerFitResultMap::const_iterator& secondLayerIter,
    double& firstWeight,
