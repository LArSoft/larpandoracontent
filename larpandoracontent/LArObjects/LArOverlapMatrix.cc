...
namespace lar_content {
  ...
  template <typename T>
  void OverlapMatrix<T>::GetUnambiguousElements(const bool ignoreUnavailable,
                                                ElementList& elementList) const
  {
    for (typename TheMatrix::const_iterator iter1 = this->begin(), iter1End = this->end();
         iter1 != iter1End;
         ++iter1) {
      ElementList tempElementList;
      ClusterList clusterList1, clusterList2;
      this->GetConnectedElements(
        iter1->first, ignoreUnavailable, tempElementList, clusterList1, clusterList2);
      ...
      if (iter1->first != pCluster1) continue;

      if (!pCluster1 || !pCluster2) continue;

      typename OverlapList::const_iterator iter2 = iter1->second.find(pCluster2);
      if (iter1->second.end() == iter2) throw StatusCodeException(STATUS_CODE_FAILURE);
  ...
  template <typename T>
  bool OverlapMatrix<T>::DefaultAmbiguityFunction(const ClusterList& clusterList1,
                                                  const ClusterList& clusterList2,
                                                  const Cluster*& pCluster1,
                                                  const Cluster*& pCluster2) const
  {
    if ((1 != clusterList1.size()) || (1 != clusterList2.size())) return false;
  ...
  template <typename T>
  void OverlapMatrix<T>::GetConnectedElements(const pandora::Cluster* const pCluster,
                                              ...
                                              unsigned int& n2) const
  {
    ClusterList clusterList1, clusterList2;
    this->GetConnectedElements(
      pCluster, ignoreUnavailable, elementList, clusterList1, clusterList2);
  ...
  template <typename T>
  void OverlapMatrix<T>::SetOverlapResult(const pandora::Cluster* const pCluster1,
  ...
  template <typename T>
  void OverlapMatrix<T>::ReplaceOverlapResult(const pandora::Cluster* const pCluster1,
                                              const pandora::Cluster* const pCluster2,
                                              const OverlapResult& overlapResult)
  {
  ...
  void OverlapMatrix<T>::RemoveCluster(const pandora::Cluster* const pCluster)
  {
    ...
    if (m_clusterNavigationMap12.erase(pCluster) > 0) {
      ...
      if (m_overlapMatrix.end() != iter) m_overlapMatrix.erase(iter);

      for (ClusterNavigationMap::iterator navIter = m_clusterNavigationMap21.begin();
           navIter != m_clusterNavigationMap21.end();) {
    ...
    if (m_clusterNavigationMap21.erase(pCluster) > 0) {
      for (typename TheMatrix::iterator iter1 = m_overlapMatrix.begin(),
                                        iter1End = m_overlapMatrix.end();
           iter1 != iter1End;
           ++iter1) {
        typename OverlapList::iterator iter = iter1->second.find(pCluster);

        if (iter1->second.end() != iter) iter1->second.erase(iter);
      }

      for (ClusterNavigationMap::iterator navIter = m_clusterNavigationMap12.begin();
           navIter != m_clusterNavigationMap12.end();) {
        ClusterNavigationMap::iterator thisIter = navIter++;
        ClusterList::iterator listIter =
          std::find(thisIter->second.begin(), thisIter->second.end(), pCluster);
  ...
  template <typename T>
  void OverlapMatrix<T>::GetConnectedElements(const Cluster* const pCluster,
                                              const bool ignoreUnavailable,
                                              ElementList& elementList,
                                              ...
                                              ClusterList& clusterList2) const
  {
    ...
    for (typename TheMatrix::const_iterator iter1 = this->begin(), iter1End = this->end();
         iter1 != iter1End;
         ++iter1) {
      if (localClusterList1.end() ==
          std::find(localClusterList1.begin(), localClusterList1.end(), iter1->first))
        continue;

      for (typename OverlapList::const_iterator iter2 = iter1->second.begin(),
                                                iter2End = iter1->second.end();
           iter2 != iter2End;
           ++iter2) {
  ...
  template <typename T>
  void OverlapMatrix<T>::ExploreConnections(const Cluster* const pCluster,
                                            const bool ignoreUnavailable,
                                            ClusterList& clusterList1,
                                            ClusterList& clusterList2) const
  {
    ...
    const bool clusterFromView1(
      !m_clusterNavigationMap12.empty() &&
      (LArClusterHelper::GetClusterHitType(m_clusterNavigationMap12.begin()->first) == hitType));
    const bool clusterFromView2(
      !m_clusterNavigationMap21.empty() &&
      (LArClusterHelper::GetClusterHitType(m_clusterNavigationMap21.begin()->first) == hitType));

    if (clusterFromView1 == clusterFromView2) throw StatusCodeException(STATUS_CODE_FAILURE);

    ClusterList& clusterList(clusterFromView1 ? clusterList1 : clusterList2);
    const ClusterNavigationMap& navigationMap(clusterFromView1 ? m_clusterNavigationMap12 :
