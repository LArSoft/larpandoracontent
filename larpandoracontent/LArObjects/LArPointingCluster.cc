...
namespace lar_content {
  ...
  LArPointingCluster::LArPointingCluster(const Cluster* const pCluster,
  ...
  void LArPointingCluster::BuildPointingCluster(const TwoDSlidingFitResult& slidingFitResult)
  {
    ...
    const bool isInner((slidingFitResult.GetGlobalMinLayerPosition().GetZ() <
                        slidingFitResult.GetGlobalMaxLayerPosition().GetZ()));

    m_pCluster = slidingFitResult.GetCluster();

    const Vertex minVertex(m_pCluster,
                           slidingFitResult.GetGlobalMinLayerPosition(),
                           slidingFitResult.GetGlobalMinLayerDirection(),
                           slidingFitResult.GetMinLayerRms(),
                           isInner);
    const Vertex maxVertex(m_pCluster,
                           slidingFitResult.GetGlobalMaxLayerPosition(),
                           slidingFitResult.GetGlobalMaxLayerDirection() * -1.f,
                           slidingFitResult.GetMaxLayerRms(),
                           !isInner);
  ...
  LArPointingCluster::Vertex::Vertex(const Cluster* const pCluster,
