...
namespace lar_content {
  ...
  template <typename T>
  ThreeDSlidingFitResult::ThreeDSlidingFitResult(const T* const pT,
    ...
    , m_firstOrthoDirection(ThreeDSlidingFitResult::GetSeedDirection(m_axisDirection)
                              .GetCrossProduct(m_axisDirection)
                              .GetUnitVector())
    , m_secondOrthoDirection(m_axisDirection.GetCrossProduct(m_firstOrthoDirection).GetUnitVector())
    , m_firstFitResult(TwoDSlidingFitResult(pT,
                                            layerWindow,
                                            layerPitch,
                                            m_axisIntercept,
                                            m_axisDirection,
                                            m_firstOrthoDirection))
    , m_secondFitResult(TwoDSlidingFitResult(pT,
                                             layerWindow,
                                             layerPitch,
                                             m_axisIntercept,
                                             m_axisDirection,
  ...
  {
    if (m_minLayer > m_maxLayer) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);

    const float minL(m_firstFitResult.GetL(m_minLayer));
    const float maxL(m_firstFitResult.GetL(m_maxLayer));

    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, this->GetGlobalFitPosition(minL, m_minLayerPosition));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, this->GetGlobalFitPosition(maxL, m_maxLayerPosition));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, this->GetGlobalFitDirection(minL, m_minLayerDirection));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, this->GetGlobalFitDirection(maxL, m_maxLayerDirection));
  }
  ...
  StatusCode ThreeDSlidingFitResult::GetGlobalFitPosition(const float rL,
  ...
  StatusCode ThreeDSlidingFitResult::GetGlobalFitDirection(const float rL,
  ...
  void ThreeDSlidingFitResult::GetGlobalPosition(const float rL,
                                                 const float rT1,
                                                 const float rT2,
                                                 CartesianVector& position) const
  {
    position = m_axisIntercept + m_axisDirection * rL + m_firstOrthoDirection * rT1 +
               m_secondOrthoDirection * rT2;
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void ThreeDSlidingFitResult::GetGlobalDirection(const float dTdL1,
                                                  const float dTdL2,
                                                  CartesianVector& direction) const
  {
  ...
  TrackState ThreeDSlidingFitResult::GetPrimaryAxis(const CartesianPointVector* const pPointVector,
                                                    const float layerPitch)
  {
    if (pPointVector->size() < 2) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  template ThreeDSlidingFitResult::ThreeDSlidingFitResult(const pandora::Cluster* const,
                                                          const unsigned int,
                                                          const float);
  template ThreeDSlidingFitResult::ThreeDSlidingFitResult(
