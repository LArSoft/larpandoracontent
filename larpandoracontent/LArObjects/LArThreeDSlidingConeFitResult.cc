...
namespace lar_content {
  ...
  float SimpleCone::GetBoundedHitFraction(const Cluster* const pCluster,
                                          const float coneLength,
                                          const float coneTanHalfAngle) const
  {
    CartesianPointVector hitPositionVector;
    LArClusterHelper::GetCoordinateVector(pCluster, hitPositionVector);
    ...
    for (const CartesianVector& hitPosition : hitPositionVector) {
      ...
      if ((rL < 0.f) || (rL > coneLength)) continue;

      const float rT(displacement.GetCrossProduct(this->GetConeDirection()).GetMagnitude());

      if (rL * coneTanHalfAngle > rT) ++nMatchedHits;
    }

    return ((nClusterHits > 0) ?
              static_cast<float>(nMatchedHits) / static_cast<float>(nClusterHits) :
              0.f);
  }
  ...
  template <typename T>
  ThreeDSlidingConeFitResult::ThreeDSlidingConeFitResult(const T* const pT,
    ...
    : m_slidingFitResult(ThreeDSlidingFitResult(pT, slidingFitWindow, slidingFitLayerPitch))
  {
    ...
    const int nSteps(static_cast<int>((maxLayerPosition3D - minLayerPosition3D).GetMagnitude() /
    ...
    for (int iStep = 0; iStep <= nSteps; ++iStep) {
      try {
        ...
        if (!contributionMap1.count(fitResult1.GetLayer(rL)) &&
            !contributionMap2.count(fitResult2.GetLayer(rL)))
          continue;

        (void)m_trackStateMap.insert(
          TrackStateMap::value_type(iStep, TrackState(fitPosition3D, fitDirection3D)));
      }
  ...
  void ThreeDSlidingConeFitResult::GetSimpleConeList(const unsigned int nLayersForConeFit,
                                                     ...
                                                     SimpleConeList& simpleConeList) const
  {
    ...
    const unsigned int coneInterval((nCones > 1) ? (nLayers - nLayersForConeFit) / (nCones - 1) :
                                                   1);
    const ThreeDSlidingFitResult& slidingFitResult(this->GetSlidingFitResult());
    const CartesianVector coneDisplacement(slidingFitResult.GetGlobalMaxLayerPosition() -
                                           slidingFitResult.GetGlobalMinLayerPosition());
    ...
    const float clusterLength(
      (trackStateMap.begin()->second.GetPosition() - trackStateMap.rbegin()->second.GetPosition())
        .GetMagnitude());

    unsigned int nConeSamplingSteps(0);

    for (TrackStateMap::const_iterator iter = trackStateMap.begin(), iterEnd = trackStateMap.end();
         iter != iterEnd;
         ++iter) {
      if (nConeSamplingSteps >= nCones) return;

      trackStateList.push_back(iter->second);
      directionSum += iter->second.GetMomentum();

      const unsigned int beginDistance(
        static_cast<unsigned int>(std::distance(trackStateMap.begin(), iter)));

      if (beginDistance + 1 < nLayersForConeFit) continue;

      const TrackState& maxLayerTrackState(trackStateList.back());
      const TrackState& minLayerTrackState(trackStateList.front());

      if ((beginDistance + 1 >= nLayersForConeFit + coneOffset) &&
          (beginDistance + 1 - nLayersForConeFit - coneOffset) % coneInterval == 0) {
  ...
  template ThreeDSlidingConeFitResult::ThreeDSlidingConeFitResult(const pandora::Cluster* const,
                                                                  const unsigned int,
                                                                  const float);
  template ThreeDSlidingConeFitResult::ThreeDSlidingConeFitResult(
    const pandora::CartesianPointVector* const,
    const unsigned int,
    const float);
