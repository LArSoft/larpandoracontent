...
namespace lar_content {
  ...
  ShowerOverlapResult::ShowerOverlapResult(const unsigned int nMatchedSamplingPoints,
  ...
  bool ShowerOverlapResult::operator<(const ShowerOverlapResult& rhs) const
  {
    ...
    if (std::fabs(m_xOverlap.GetXOverlapSpan() - rhs.m_xOverlap.GetXOverlapSpan()) >
        std::numeric_limits<float>::epsilon())
      return (m_xOverlap.GetXOverlapSpan() < rhs.m_xOverlap.GetXOverlapSpan());

    if (std::fabs(m_matchedFraction - rhs.m_matchedFraction) >
        std::numeric_limits<float>::epsilon())
      return (m_matchedFraction < rhs.m_matchedFraction);

    if (std::fabs(m_xOverlap.GetXSpanW() - rhs.m_xOverlap.GetXSpanW()) >
        std::numeric_limits<float>::epsilon())
      return (m_xOverlap.GetXSpanW() < rhs.m_xOverlap.GetXSpanW());

    if (std::fabs(m_xOverlap.GetXSpanU() - rhs.m_xOverlap.GetXSpanU()) >
        std::numeric_limits<float>::epsilon())
