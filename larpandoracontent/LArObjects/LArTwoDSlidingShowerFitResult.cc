...
namespace lar_content {
  ...
  template <typename T>
  TwoDSlidingShowerFitResult::TwoDSlidingShowerFitResult(const T* const pT,
    ...
    , m_negativeEdgeFitResult(
        TwoDSlidingShowerFitResult::LArTwoDShowerEdgeFit(pT,
                                                         m_showerFitResult,
  ...
  void TwoDSlidingShowerFitResult::GetShowerEdges(const float x,
                                                  const bool widenIfAmbiguity,
                                                  FloatVector& edgePositions) const
  {
    edgePositions.clear();
    ...
    PANDORA_THROW_RESULT_IF_AND_IF(
      ...
      !=,
      this->GetNegativeEdgeFitResult().GetGlobalFitPositionListAtX(x, fitPositionVector));
    PANDORA_THROW_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      this->GetPositiveEdgeFitResult().GetGlobalFitPositionListAtX(x, fitPositionVector));
    ...
    if (fitPositionVector.size() < 2) {
      ...
      if (!widenIfAmbiguity) { return; }
        ...
        const float secondEdge(
          (std::fabs(existingEdge - minZ) < std::fabs(existingEdge - maxZ)) ? minZ : maxZ);
    ...
    if (localEdgePositions.size() < 2) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    const float showerEdgeMultiplier)
  {
    ...
    return TwoDSlidingShowerFitResult::LArTwoDShowerEdgeFit(
      &pointVector, fullShowerFit, showerEdge, showerEdgeMultiplier);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  TwoDSlidingFitResult TwoDSlidingShowerFitResult::LArTwoDShowerEdgeFit(
    const CartesianPointVector* const pPointVector,
    const TwoDSlidingFitResult& fullShowerFit,
    const ShowerEdge showerEdge,
    const float showerEdgeMultiplier)
  {
    ...
    for (const CartesianVector& hitPosition : *pPointVector) {
      ...
      if (((POSITIVE_SHOWER_EDGE == showerEdge) && (rTDiff < 0.f)) ||
          ((NEGATIVE_SHOWER_EDGE == showerEdge) && (rTDiff > 0.f)))
        rT = rTFit;
    ...
    for (const FitCoordinateMap::value_type& mapEntry : fitCoordinateMap) {
      ...
      for (const FitCoordinate& fitCoordinate : mapEntry.second) {
        if (((POSITIVE_SHOWER_EDGE == showerEdge) &&
             (fitCoordinate.second > bestFitCoordinate.second)) ||
            ((NEGATIVE_SHOWER_EDGE == showerEdge) &&
             (fitCoordinate.second < bestFitCoordinate.second))) {
      ...
      if (bestFitCoordinateFound)
        layerFitContributionMap[mapEntry.first].AddPoint(bestFitCoordinate.first,
                                                         bestFitCoordinate.second);
    }

    return TwoDSlidingFitResult(fullShowerFit.GetLayerFitHalfWindow(),
                                fullShowerFit.GetLayerPitch(),
                                fullShowerFit.GetAxisIntercept(),
                                fullShowerFit.GetAxisDirection(),
                                fullShowerFit.GetOrthoDirection(),
                                layerFitContributionMap);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------------------------------------------------------------

  template TwoDSlidingShowerFitResult::TwoDSlidingShowerFitResult(const pandora::Cluster* const,
                                                                  const unsigned int,
                                                                  const float,
                                                                  const float);
  template TwoDSlidingShowerFitResult::TwoDSlidingShowerFitResult(
