...
namespace lar_content {
  ...
  StatusCode SupportVectorMachine::Initialize(const std::string& parameterLocation,
                                              const std::string& svmName)
  {
    ...
    if (m_standardizeFeatures) {
      for (const FeatureInfo& featureInfo : m_featureInfoList) {
        if (featureInfo.m_sigmaValue < std::numeric_limits<double>::epsilon()) {
          std::cout << "SupportVectorMachine: could not standardize parameters because sigma value "
                       "was too small"
                    << std::endl;
    ...
    for (const SupportVectorInfo& svInfo : m_svInfoList) {
      if (svInfo.m_supportVector.size() != m_nFeatures) {
        std::cout << "SupportVectorMachine: the number of features in the xml file was inconsistent"
                  << std::endl;
        throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
      }
    }
    ...
    if (m_scaleFactor < std::numeric_limits<double>::epsilon()) {
      std::cout
        << "SupportVectorMachine: could not evaluate kernel because scale factor was too small"
        << std::endl;
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    }
  ...
  void SupportVectorMachine::ReadXmlFile(const std::string& svmFileName, const std::string& svmName)
  {
    ...
    while (pContainerXmlNode) {
      ...
      if (currentName.empty() || (currentName.size() > 1000)) {
        std::cout << "SupportVectorMachine::Initialize - Implausible svm name extracted from xml."
                  << std::endl;
        throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
      }

      if (currentName == svmName) break;

      pContainerXmlNode = pContainerXmlNode->NextSibling();
    }

    if (!pContainerXmlNode) {
      std::cout << "SupportVectorMachine: Could not find an svm by the name " << svmName
                << std::endl;
  ...
  StatusCode SupportVectorMachine::ReadComponent(TiXmlElement* pCurrentXmlElement)
  {
    ...
    if (std::string("Machine") == componentName) return this->ReadMachine(currentHandle);

    if (std::string("Features") == componentName) return this->ReadFeatures(currentHandle);
  ...
  StatusCode SupportVectorMachine::ReadMachine(const TiXmlHandle& currentHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(currentHandle, "KernelType", kernelType));

    double bias(0.);
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(currentHandle, "Bias", bias));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(currentHandle, "ScaleFactor", scaleFactor));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(currentHandle, "Standardize", standardize));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(currentHandle, "EnableProbability", enableProbability));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(currentHandle, "ProbAParameter", probAParameter));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(currentHandle, "ProbBParameter", probBParameter));
    ...
    if (
      kernelType !=
      USER_DEFINED) // if user-defined, leave it so it alone can be set before/after initialization
  ...
  StatusCode SupportVectorMachine::ReadFeatures(const TiXmlHandle& currentHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadVectorOfValues(currentHandle, "MuValues", muValues));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadVectorOfValues(currentHandle, "SigmaValues", sigmaValues));

    if (muValues.size() != sigmaValues.size()) {
      std::cout << "SupportVectorMachine: could not add feature info because the size of mu ("
                << muValues.size()
                << ") did not match "
                   "the size of sigma ("
  ...
  StatusCode SupportVectorMachine::ReadSupportVector(const TiXmlHandle& currentHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(currentHandle, "AlphaY", yAlpha));

    std::vector<double> values;
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadVectorOfValues(currentHandle, "Values", values));
  ...
  double SupportVectorMachine::CalculateClassificationScoreImpl(
    const LArMvaHelper::MvaFeatureVector& features) const
  {
    if (!m_isInitialized) {
      std::cout << "SupportVectorMachine: could not perform classification because the svm was "
                   "uninitialized"
                << std::endl;
    ...
    if (m_svInfoList.empty()) {
      std::cout << "SupportVectorMachine: could not perform classification because the initialized "
                   "svm had no support vectors in the model"
                << std::endl;
      throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
    }
    ...
    if (m_standardizeFeatures) {
      for (std::size_t i = 0; i < m_nFeatures; ++i)
        standardizedFeatures.push_back(
    ...
    for (const SupportVectorInfo& supportVectorInfo : m_svInfoList) {
      classScore += supportVectorInfo.m_yAlpha *
                    m_kernelFunction(supportVectorInfo.m_supportVector,
                                     (m_standardizeFeatures ? standardizedFeatures : features),
                                     m_scaleFactor);
    }
