...
namespace lar_content {
  ...
  DiscreteProbabilityVector::DiscreteProbabilityVector(const InputData<TX, TY>& inputData,
  ...
  DiscreteProbabilityVector::DiscreteProbabilityVector(
    const DiscreteProbabilityVector& discreteProbabilityVector,
    ...
    , m_discreteProbabilityData(
        this->RandomiseDiscreteProbabilityData(discreteProbabilityVector, randomNumberGenerator))
  {
    this->VerifyCompleteData();
  }
  ...
  DiscreteProbabilityVector::DiscreteProbabilityVector(
    ...
    , m_useWidths(discreteProbabilityVector.m_useWidths)
    , m_discreteProbabilityData(
        this->ResampleDiscreteProbabilityData(discreteProbabilityVector, resamplingPoints))
  {
    this->VerifyCompleteData();
  }
  ...
  template <typename TX, typename TY>
  DiscreteProbabilityVector::DiscreteProbabilityData
  DiscreteProbabilityVector::InitialiseDiscreteProbabilityData(InputData<TX, TY> inputData) const
  {
    if (2 > inputData.size())
      throw pandora::StatusCodeException(pandora::STATUS_CODE_INVALID_PARAMETER);

    std::sort(
      inputData.begin(), inputData.end(), DiscreteProbabilityVector::SortInputDataByX<TX, TY>);
    ...
    for (unsigned int iDatum = 0; iDatum < inputData.size() - 1; ++iDatum) {
      ...
      data.emplace_back(DiscreteProbabilityVector::DiscreteProbabilityDatum(
        x, densityDatum, accumulationDatum, deltaX));
    }
  ...
  DiscreteProbabilityVector::DiscreteProbabilityData
  DiscreteProbabilityVector::ResampleDiscreteProbabilityData(
    const DiscreteProbabilityVector& discreteProbabilityVector,
    const ResamplingPoints& resamplingPoints) const
  {
    ...
    for (unsigned int iSample = 0; iSample < resamplingPoints.size() - 1; ++iSample) {
      ...
      const float cumulativeDatumResampled(
        discreteProbabilityVector.EvaluateCumulativeProbability(xResampled));
      const float densityDatumResampled((cumulativeDatumResampled - prevCumulativeData) /
                                        (m_useWidths ? deltaX : 1.f));
      resampledProbabilityData.emplace_back(DiscreteProbabilityVector::DiscreteProbabilityDatum(
        xResampled, densityDatumResampled, cumulativeDatumResampled, deltaX));
  ...
  DiscreteProbabilityVector::DiscreteProbabilityData
  DiscreteProbabilityVector::RandomiseDiscreteProbabilityData(
    const DiscreteProbabilityVector& discreteProbabilityVector,
    std::mt19937& randomNumberGenerator) const
  {
    ...
    std::shuffle(
      std::begin(randomisedElements), std::end(randomisedElements), randomNumberGenerator);
    ...
    for (unsigned int iElement = 0; iElement < discreteProbabilityVector.GetSize(); ++iElement) {
      ...
      const float probabilityDensity(
        discreteProbabilityVector.GetProbabilityDensity(randomElementIndex));
      cumulativeProbability += probabilityDensity * (m_useWidths ? deltaX : 1.f);
      randomisedProbabilityData.emplace_back(DiscreteProbabilityVector::DiscreteProbabilityDatum(
        xPos, probabilityDensity, cumulativeProbability, deltaX));
      xPos += deltaX;
    }
  ...
  template <typename TX, typename TY>
  bool DiscreteProbabilityVector::SortInputDataByX(const InputDatum<TX, TY>& lhs,
                                                   const InputDatum<TX, TY>& rhs)
  {
    const float deltaX(static_cast<float>(rhs.first) - static_cast<float>(lhs.first));
    if (std::fabs(deltaX) < std::numeric_limits<float>::epsilon()) return (lhs.second < rhs.second);
  ...
  float DiscreteProbabilityVector::CalculateNormalisation(const InputData<TX, TY>& inputData) const
  {
    ...
    for (unsigned int iDatum = 0; iDatum < inputData.size() - 1; ++iDatum) {
      ...
      normalisation += y * (m_useWidths ? static_cast<float>(inputData.at(iDatum + 1).first) -
                                            static_cast<float>(inputData.at(iDatum).first) :
                                          1.f);
    }
    const float y(static_cast<float>(inputData.back().second));
    normalisation +=
      y * (m_useWidths ? m_xUpperBound - static_cast<float>(inputData.back().first) : 1.f);
  ...
  template DiscreteProbabilityVector::DiscreteProbabilityVector(const InputData<int, float>&,
                                                                int const,
                                                                bool const);
  template DiscreteProbabilityVector::DiscreteProbabilityVector(const InputData<float, int>&,
                                                                float const,
                                                                bool const);
  template DiscreteProbabilityVector::DiscreteProbabilityVector(const AllFloatInputData&,
                                                                float const,
                                                                bool const);
  template DiscreteProbabilityVector::DiscreteProbabilityVector(const InputData<int, int>&,
                                                                int const,
                                                                bool const);

  template DiscreteProbabilityVector::DiscreteProbabilityData
  DiscreteProbabilityVector::InitialiseDiscreteProbabilityData(InputData<int, float>) const;
  template DiscreteProbabilityVector::DiscreteProbabilityData
  DiscreteProbabilityVector::InitialiseDiscreteProbabilityData(InputData<float, int>) const;
  template DiscreteProbabilityVector::DiscreteProbabilityData
    DiscreteProbabilityVector::InitialiseDiscreteProbabilityData(AllFloatInputData) const;
  template DiscreteProbabilityVector::DiscreteProbabilityData
  DiscreteProbabilityVector::InitialiseDiscreteProbabilityData(InputData<int, int>) const;

  template bool DiscreteProbabilityVector::SortInputDataByX(const InputDatum<int, float>&,
                                                            const InputDatum<int, float>&);
  template bool DiscreteProbabilityVector::SortInputDataByX(const InputDatum<float, int>&,
                                                            const InputDatum<float, int>&);
  template bool DiscreteProbabilityVector::SortInputDataByX(const InputDatum<float, float>&,
                                                            const InputDatum<float, float>&);
  template bool DiscreteProbabilityVector::SortInputDataByX(const InputDatum<int, int>&,
                                                            const InputDatum<int, int>&);

  template float DiscreteProbabilityVector::CalculateNormalisation(
    const InputData<int, float>&) const;
  template float DiscreteProbabilityVector::CalculateNormalisation(
    const InputData<float, int>&) const;
  template float DiscreteProbabilityVector::CalculateNormalisation(const AllFloatInputData&) const;
  template float DiscreteProbabilityVector::CalculateNormalisation(
    const InputData<int, int>&) const;
