...
namespace lar_content {
  ...
  {
    if (this != &rhs) m_pStrongClassifier = new StrongClassifier(*(rhs.m_pStrongClassifier));
  ...
  AdaBoostDecisionTree::~AdaBoostDecisionTree() { delete m_pStrongClassifier; }

  //------------------------------------------------------------------------------------------------------------------------------------------

  StatusCode AdaBoostDecisionTree::Initialize(const std::string& bdtXmlFileName,
                                              const std::string& bdtName)
  {
    if (m_pStrongClassifier) {
      std::cout << "AdaBoostDecisionTree: AdaBoostDecisionTree was already initialized"
                << std::endl;
    ...
    while (pContainerXmlNode) {
      if (pContainerXmlNode->ValueStr() != "AdaBoostDecisionTree") return STATUS_CODE_FAILURE;

      const TiXmlHandle currentHandle(pContainerXmlNode);

      std::string currentName;
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(currentHandle, "Name", currentName));

      if (currentName.empty() || (currentName.size() > 1000)) {
        std::cout << "AdaBoostDecisionTree::Initialize - Implausible AdaBoostDecisionTree name "
                     "extracted from xml."
                  << std::endl;
        return STATUS_CODE_INVALID_PARAMETER;
      }

      if (currentName == bdtName) break;

      pContainerXmlNode = pContainerXmlNode->NextSibling();
    }

    if (!pContainerXmlNode) {
      std::cout << "AdaBoostDecisionTree: Could not find an AdaBoostDecisionTree of name "
                << bdtName << std::endl;
    ...
    try {
      ...
      if (STATUS_CODE_INVALID_PARAMETER == statusCodeException.GetStatusCode())
        std::cout << "AdaBoostDecisionTree: Initialization failure, unknown component in xml file."
                  << std::endl;

      if (STATUS_CODE_FAILURE == statusCodeException.GetStatusCode())
        std::cout << "AdaBoostDecisionTree: Node definition does not contain expected leaf or "
                     "branch variables."
                  << std::endl;
  ...
  double AdaBoostDecisionTree::CalculateClassificationScore(
  ...
  double AdaBoostDecisionTree::CalculateProbability(
    const LArMvaHelper::MvaFeatureVector& features) const
  {
  ...
  double AdaBoostDecisionTree::CalculateScore(const LArMvaHelper::MvaFeatureVector& features) const
  {
    ...
    try {
      ...
      if (STATUS_CODE_NOT_FOUND == statusCodeException.GetStatusCode()) {
        std::cout << "AdaBoostDecisionTree: Caught exception thrown when trying to cut on an "
                     "unknown variable."
                  << std::endl;
      }
      else if (STATUS_CODE_INVALID_PARAMETER == statusCodeException.GetStatusCode()) {
        std::cout << "AdaBoostDecisionTree: Caught exception thrown when classifier weights sum to "
                     "zero indicating defunct classifier."
                  << std::endl;
      }
      else if (STATUS_CODE_OUT_OF_RANGE == statusCodeException.GetStatusCode()) {
        std::cout << "AdaBoostDecisionTree: Caught exception thrown when heirarchy in decision "
                     "tree is incomplete."
                  << std::endl;
      }
      else {
  ...
  {
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(*pXmlHandle, "NodeId", m_nodeId));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(*pXmlHandle, "ParentNodeId", m_parentNodeId));

    const StatusCode leftChildNodeIdStatusCode(
      XmlHelper::ReadValue(*pXmlHandle, "LeftChildNodeId", m_leftChildNodeId));
    const StatusCode rightChildNodeIdStatusCode(
      XmlHelper::ReadValue(*pXmlHandle, "RightChildNodeId", m_rightChildNodeId));
    const StatusCode thresholdStatusCode(
      XmlHelper::ReadValue(*pXmlHandle, "Threshold", m_threshold));
    const StatusCode variableIdStatusCode(
      XmlHelper::ReadValue(*pXmlHandle, "VariableId", m_variableId));
    const StatusCode outcomeStatusCode(XmlHelper::ReadValue(*pXmlHandle, "Outcome", m_outcome));

    if (STATUS_CODE_SUCCESS == leftChildNodeIdStatusCode ||
        STATUS_CODE_SUCCESS == rightChildNodeIdStatusCode ||
  ...
  AdaBoostDecisionTree::WeakClassifier::WeakClassifier(const TiXmlHandle* const pXmlHandle)
    : m_weight(0.), m_treeId(0)
  {
    for (TiXmlElement* pHeadTiXmlElement = pXmlHandle->FirstChildElement().ToElement();
         pHeadTiXmlElement != NULL;
         pHeadTiXmlElement = pHeadTiXmlElement->NextSiblingElement()) {
      if ("TreeIndex" == pHeadTiXmlElement->ValueStr()) {
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(*pXmlHandle, "TreeIndex", m_treeId));
      }
      else if ("TreeWeight" == pHeadTiXmlElement->ValueStr()) {
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(*pXmlHandle, "TreeWeight", m_weight));
      }
  ...
  AdaBoostDecisionTree::WeakClassifier::WeakClassifier(const WeakClassifier& rhs)
    : m_weight(rhs.m_weight), m_treeId(rhs.m_treeId)
  {
  ...
  AdaBoostDecisionTree::WeakClassifier& AdaBoostDecisionTree::WeakClassifier::operator=(
    const WeakClassifier& rhs)
  {
    if (this != &rhs) {
      for (const auto& mapEntry : rhs.m_idToNodeMap) {
        const Node* pNode = new Node(*(mapEntry.second));
  ...
  bool AdaBoostDecisionTree::WeakClassifier::Predict(
    const LArMvaHelper::MvaFeatureVector& features) const
  {
  ...
  bool AdaBoostDecisionTree::WeakClassifier::EvaluateNode(
    const int nodeId,
    const LArMvaHelper::MvaFeatureVector& features) const
  {
    ...
    if (pActiveNode->IsLeaf()) return pActiveNode->GetOutcome();
  ...
  AdaBoostDecisionTree::StrongClassifier& AdaBoostDecisionTree::StrongClassifier::operator=(
    const StrongClassifier& rhs)
  {
    if (this != &rhs) {
      for (const WeakClassifier* const pWeakClassifier : rhs.m_weakClassifiers)
        m_weakClassifiers.emplace_back(new WeakClassifier(*pWeakClassifier));
  ...
  double AdaBoostDecisionTree::StrongClassifier::Predict(
    const LArMvaHelper::MvaFeatureVector& features) const
  {
    ...
    for (const WeakClassifier* const pWeakClassifier : m_weakClassifiers) {
      ...
      if (pWeakClassifier->Predict(features)) { score += pWeakClassifier->GetWeight(); }
      else {
        score -= pWeakClassifier->GetWeight();
      }
    }

    if (weights > std::numeric_limits<double>::epsilon()) { score /= weights; }
    else {
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    }
