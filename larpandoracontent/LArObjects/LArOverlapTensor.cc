...
namespace lar_content {
  ...
  const Cluster* OverlapTensor<T>::Element::GetCluster(const HitType hitType) const
  {
    ...
    return (hitType == TPC_VIEW_U) ? m_pClusterU :
                                     (hitType == TPC_VIEW_V) ? m_pClusterV : m_pClusterW;
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  template <typename T>
  void OverlapTensor<T>::GetUnambiguousElements(const bool ignoreUnavailable,
                                                ElementList& elementList) const
  {
    for (typename TheTensor::const_iterator iterU = this->begin(), iterUEnd = this->end();
         iterU != iterUEnd;
         ++iterU) {
      ElementList tempElementList;
      ClusterList clusterListU, clusterListV, clusterListW;
      this->GetConnectedElements(
        iterU->first, ignoreUnavailable, tempElementList, clusterListU, clusterListV, clusterListW);

      const Cluster *pClusterU(nullptr), *pClusterV(nullptr), *pClusterW(nullptr);
      if (!this->DefaultAmbiguityFunction(
            clusterListU, clusterListV, clusterListW, pClusterU, pClusterV, pClusterW))
        continue;

      // ATTN With HIT_CUSTOM definitions, it is possible to navigate from different U clusters to same combination
      if (iterU->first != pClusterU) continue;

      if (!pClusterU || !pClusterV || !pClusterW) continue;

      typename OverlapMatrix::const_iterator iterV = iterU->second.find(pClusterV);
      if (iterU->second.end() == iterV) throw StatusCodeException(STATUS_CODE_FAILURE);

      typename OverlapList::const_iterator iterW = iterV->second.find(pClusterW);
      if (iterV->second.end() == iterW) throw StatusCodeException(STATUS_CODE_FAILURE);
  ...
  template <typename T>
  void OverlapTensor<T>::GetConnectedElements(const pandora::Cluster* const pCluster,
                                              ...
                                              unsigned int& nW) const
  {
    ClusterList clusterListU, clusterListV, clusterListW;
    this->GetConnectedElements(
      pCluster, ignoreUnavailable, elementList, clusterListU, clusterListV, clusterListW);
  ...
  template <typename T>
  void OverlapTensor<T>::SetOverlapResult(const pandora::Cluster* const pClusterU,
  ...
  template <typename T>
  void OverlapTensor<T>::ReplaceOverlapResult(const pandora::Cluster* const pClusterU,
                                              const pandora::Cluster* const pClusterV,
                                              const pandora::Cluster* const pClusterW,
                                              const OverlapResult& overlapResult)
  {
  ...
  void OverlapTensor<T>::RemoveCluster(const pandora::Cluster* const pCluster)
  {
    ...
    if (m_clusterNavigationMapUV.erase(pCluster) > 0) {
      ...
      if (m_overlapTensor.end() != iter) m_overlapTensor.erase(iter);

      for (ClusterNavigationMap::iterator navIter = m_clusterNavigationMapWU.begin();
           navIter != m_clusterNavigationMapWU.end();) {
        ClusterNavigationMap::iterator thisIter = navIter++;
        ClusterList::iterator listIter =
          std::find(thisIter->second.begin(), thisIter->second.end(), pCluster);

        if (thisIter->second.end() != listIter) thisIter->second.erase(listIter);

        if (thisIter->second.empty()) additionalRemovals.push_back(thisIter->first);
      }
    }

    if (m_clusterNavigationMapVW.erase(pCluster) > 0) {
      for (typename TheTensor::iterator iterU = m_overlapTensor.begin(),
                                        iterUEnd = m_overlapTensor.end();
           iterU != iterUEnd;
           ++iterU) {
        typename OverlapMatrix::iterator iter = iterU->second.find(pCluster);

        if (iterU->second.end() != iter) iterU->second.erase(iter);
      }

      for (ClusterNavigationMap::iterator navIter = m_clusterNavigationMapUV.begin();
           navIter != m_clusterNavigationMapUV.end();) {
        ClusterNavigationMap::iterator thisIter = navIter++;
        ClusterList::iterator listIter =
          std::find(thisIter->second.begin(), thisIter->second.end(), pCluster);
    ...
    if (m_clusterNavigationMapWU.erase(pCluster) > 0) {
      for (typename TheTensor::iterator iterU = m_overlapTensor.begin(),
           ...
           iterU != iterUEnd;
           ++iterU) {
        for (typename OverlapMatrix::iterator iterV = iterU->second.begin(),
                                              iterVEnd = iterU->second.end();
             iterV != iterVEnd;
             ++iterV) {
          typename OverlapList::iterator iter = iterV->second.find(pCluster);

          if (iterV->second.end() != iter) iterV->second.erase(iter);
        }
      }

      for (ClusterNavigationMap::iterator navIter = m_clusterNavigationMapVW.begin();
           navIter != m_clusterNavigationMapVW.end();) {
        ClusterNavigationMap::iterator thisIter = navIter++;
        ClusterList::iterator listIter =
          std::find(thisIter->second.begin(), thisIter->second.end(), pCluster);
    ...
    for (ClusterList::const_iterator iter = additionalRemovals.begin(),
                                     iterEnd = additionalRemovals.end();
         iter != iterEnd;
         ++iter)
  ...
  template <typename T>
  void OverlapTensor<T>::GetConnectedElements(const Cluster* const pCluster,
                                              const bool ignoreUnavailable,
                                              ElementList& elementList,
                                              ...
                                              ClusterList& clusterListW) const
  {
    ...
    this->ExploreConnections(
      pCluster, ignoreUnavailable, localClusterListU, localClusterListV, localClusterListW);
    ...
    for (typename TheTensor::const_iterator iterU = this->begin(), iterUEnd = this->end();
         iterU != iterUEnd;
         ++iterU) {
      if (localClusterListU.end() ==
          std::find(localClusterListU.begin(), localClusterListU.end(), iterU->first))
        continue;

      for (typename OverlapMatrix::const_iterator iterV = iterU->second.begin(),
                                                  iterVEnd = iterU->second.end();
           iterV != iterVEnd;
           ++iterV) {
        for (typename OverlapList::const_iterator iterW = iterV->second.begin(),
                                                  iterWEnd = iterV->second.end();
             iterW != iterWEnd;
             ++iterW) {
          if (ignoreUnavailable && (!iterU->first->IsAvailable() || !iterV->first->IsAvailable() ||
                                    !iterW->first->IsAvailable()))
            continue;
          ...
          if (clusterListU.end() ==
              std::find(clusterListU.begin(), clusterListU.end(), iterU->first))
            clusterListU.push_back(iterU->first);
          if (clusterListV.end() ==
              std::find(clusterListV.begin(), clusterListV.end(), iterV->first))
            clusterListV.push_back(iterV->first);
          if (clusterListW.end() ==
              std::find(clusterListW.begin(), clusterListW.end(), iterW->first))
  ...
  template <typename T>
  void OverlapTensor<T>::ExploreConnections(const Cluster* const pCluster,
                                            const bool ignoreUnavailable,
                                            ClusterList& clusterListU,
                                            ClusterList& clusterListV,
                                            ClusterList& clusterListW) const
  {
    ...
    if (navigationMap.end() == iter) throw StatusCodeException(STATUS_CODE_FAILURE);
