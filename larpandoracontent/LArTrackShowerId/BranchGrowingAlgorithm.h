 ...
 */
  ...
  protected:
     ...
     *  @return the association type
     */
    virtual AssociationType AreClustersAssociated(const pandora::Cluster* const pClusterSeed,
                                                  const pandora::Cluster* const pCluster) const = 0;
     ...
     */
    void FindAssociatedClusters(const pandora::Cluster* const pParticleSeed,
     ...
     */
    void IdentifyClusterMerges(const pandora::ClusterVector& particleSeedVector,
  ...
  inline BranchGrowingAlgorithm::Association::Association()
    : m_order(std::numeric_limits<unsigned int>::max()), m_type(NONE)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  inline BranchGrowingAlgorithm::Association::Association(const unsigned int order,
  ...
  inline unsigned int BranchGrowingAlgorithm::Association::GetOrder() const { return m_order; }

  //------------------------------------------------------------------------------------------------------------------------------------------

  inline BranchGrowingAlgorithm::AssociationType BranchGrowingAlgorithm::Association::GetType()
    const
  {
    return m_type;
  }
