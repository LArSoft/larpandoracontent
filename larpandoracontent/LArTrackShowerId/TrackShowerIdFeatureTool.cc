...
namespace lar_content {
  ...
  TwoDShowerFitFeatureTool::TwoDShowerFitFeatureTool()
    : m_slidingShowerFitWindow(3), m_slidingLinearFitWindow(10000)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  void TwoDShowerFitFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                     const Algorithm* const pAlgorithm,
                                     const pandora::Cluster* const pCluster)
  {
    ...
    try {
      const TwoDSlidingFitResult slidingFitResultLarge(
        pCluster, m_slidingLinearFitWindow, LArGeometryHelper::GetWireZPitch(this->GetPandora()));
      const float straightLineLength = (slidingFitResultLarge.GetGlobalMaxLayerPosition() -
                                        slidingFitResultLarge.GetGlobalMinLayerPosition())
                                         .GetMagnitude();
      if (straightLineLength > std::numeric_limits<float>::epsilon())
        ratio = (CutClusterCharacterisationAlgorithm::GetShowerFitWidth(
                  pAlgorithm, pCluster, m_slidingShowerFitWindow)) /
                straightLineLength;
    }
  ...
  void TwoDShowerFitFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
  ...
  StatusCode TwoDShowerFitFeatureTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "SlidingShowerFitWindow", m_slidingShowerFitWindow));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SlidingLinearFitWindow", m_slidingLinearFitWindow));
  ...
  void TwoDLinearFitFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                     const Algorithm* const pAlgorithm,
                                     const pandora::Cluster* const pCluster)
  {
    if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
    ...
    float dTdLWidth(-1.f), straightLineLengthLarge(-1.f), diffWithStraightLineMean(-1.f),
      diffWithStraightLineSigma(-1.f), maxFitGapLength(-1.f), rmsSlidingLinearFit(-1.f);
    this->CalculateVariablesSlidingLinearFit(pCluster,
                                             straightLineLengthLarge,
                                             diffWithStraightLineMean,
                                             diffWithStraightLineSigma,
                                             dTdLWidth,
                                             maxFitGapLength,
                                             rmsSlidingLinearFit);
  ...
  void TwoDLinearFitFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                     StringVector& featureOrder,
                                     const std::string& featureToolName,
                                     const Algorithm* const pAlgorithm,
                                     const pandora::Cluster* const pCluster)
  ...
  void TwoDLinearFitFeatureTool::CalculateVariablesSlidingLinearFit(
    const pandora::Cluster* const pCluster,
    float& straightLineLengthLarge,
    float& diffWithStraightLineMean,
    float& diffWithStraightLineSigma,
    float& dTdLWidth,
    float& maxFitGapLength,
    float& rmsSlidingLinearFit) const
  {
    try {
      ...
      const TwoDSlidingFitResult slidingFitResultLarge(
        pCluster,
        m_slidingLinearFitWindowLarge,
        LArGeometryHelper::GetWireZPitch(this->GetPandora()));

      if (slidingFitResult.GetLayerFitResultMap().empty())
        throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);

      straightLineLengthLarge = (slidingFitResultLarge.GetGlobalMaxLayerPosition() -
                                 slidingFitResultLarge.GetGlobalMinLayerPosition())
                                  .GetMagnitude();
      ...
      float dTdLMin(+std::numeric_limits<float>::max()),
        dTdLMax(-std::numeric_limits<float>::max());
      ...
      for (const auto& mapEntry : slidingFitResult.GetLayerFitResultMap()) {
        ...
        slidingFitResult.GetGlobalPosition(
          layerFitResult.GetL(), layerFitResult.GetFitT(), thisFitPosition);

        LayerFitResultMap::const_iterator iterLarge =
          slidingFitResultLarge.GetLayerFitResultMap().find(
            slidingFitResultLarge.GetLayer(layerFitResult.GetL()));

        if (slidingFitResultLarge.GetLayerFitResultMap().end() == iterLarge)
          throw StatusCodeException(STATUS_CODE_FAILURE);

        diffWithStraightLineVector.push_back(
          static_cast<float>(std::fabs(layerFitResult.GetFitT() - iterLarge->second.GetFitT())));
        ...
        if ((maxZ - minZ) > std::numeric_limits<float>::epsilon()) {
          const float gapZ(
            LArGeometryHelper::CalculateGapDeltaZ(this->GetPandora(), minZ, maxZ, hitType));
          const float correctedGapLength(thisGapLength * (1.f - gapZ / (maxZ - minZ)));

          if (correctedGapLength > maxFitGapLength) maxFitGapLength = correctedGapLength;
        }
      ...
      if (diffWithStraightLineVector.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
      ...
      for (const float diffWithStraightLine : diffWithStraightLineVector)
        diffWithStraightLineSigma += (diffWithStraightLine - diffWithStraightLineMean) *
                                     (diffWithStraightLine - diffWithStraightLineMean);

      if (diffWithStraightLineSigma < 0.f) throw StatusCodeException(STATUS_CODE_FAILURE);

      diffWithStraightLineSigma = std::sqrt(diffWithStraightLineSigma /
  ...
  void TwoDVertexDistanceFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                          const Algorithm* const pAlgorithm,
                                          const pandora::Cluster* const pCluster)
  {
    if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
    ...
    try {
      const TwoDSlidingFitResult slidingFitResultLarge(
        pCluster, m_slidingLinearFitWindow, LArGeometryHelper::GetWireZPitch(this->GetPandora()));
      straightLineLength = (slidingFitResultLarge.GetGlobalMaxLayerPosition() -
                            slidingFitResultLarge.GetGlobalMinLayerPosition())
                             .GetMagnitude();
      if (straightLineLength > std::numeric_limits<float>::epsilon())
        ratio = (CutClusterCharacterisationAlgorithm::GetVertexDistance(pAlgorithm, pCluster)) /
                straightLineLength;
    }
    catch (const StatusCodeException&) {
      ratio = -1.f;
  ...
  void TwoDVertexDistanceFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                          StringVector& featureOrder,
                                          const std::string& featureToolName,
                                          const Algorithm* const pAlgorithm,
                                          const pandora::Cluster* const pCluster)
  ...
  void PfoHierarchyFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                    const Algorithm* const pAlgorithm,
                                    const pandora::ParticleFlowObject* const pInputPfo)
  {
    ...
    const unsigned int nDaughterPfos(allDaughtersPfoList.empty() ? 0 :
                                                                   allDaughtersPfoList.size() - 1);
    ...
    const LArMvaHelper::MvaFeature daughterParentNHitsRatio(
      (nParentHits3D > 0) ?
        static_cast<double>(nDaughterHits3DTotal) / static_cast<double>(nParentHits3D) :
        0.);
  ...
  void PfoHierarchyFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                    StringVector& featureOrder,
                                    const std::string& featureToolName,
  ...
  void ConeChargeFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                  const Algorithm* const pAlgorithm,
                                  const pandora::ParticleFlowObject* const pInputPfo)
  {
    if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
    ...
    if (!clusterListW.empty()) {
      ...
      this->CalculateChargeDistribution(
        clusterCaloHitList, pfoStart, eigenVecs[0], chargeCore, chargeHalo, chargeCon);
      haloTotalRatio = (chargeCore + chargeHalo > std::numeric_limits<float>::epsilon()) ?
                         chargeHalo / (chargeCore + chargeHalo) :
                         -1.f;
      concentration = (chargeCore + chargeHalo > std::numeric_limits<float>::epsilon()) ?
                        chargeCon / (chargeCore + chargeHalo) :
                        -1.f;
      const float pfoLength(std::sqrt(LArPfoHelper::GetThreeDLengthSquared(pInputPfo)));
      conicalness =
        (pfoLength > std::numeric_limits<float>::epsilon()) ?
  ...
  void ConeChargeFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                  StringVector& featureOrder,
                                  const std::string& featureToolName,
                                  const pandora::Algorithm* const pAlgorithm,
                                  const pandora::ParticleFlowObject* const pInputPfo)
  ...
  void ConeChargeFeatureTool::CalculateChargeDistribution(const CaloHitList& caloHitList,
                                                          ...
                                                          float& chargeCon)
  {
    for (const CaloHit* const pCaloHit : caloHitList) {
      const float distFromTrackFit(
        ((pCaloHit->GetPositionVector() - pfoStart).GetCrossProduct(pfoDir)).GetMagnitude());
      ...
      chargeCon +=
        pCaloHit->GetInputEnergy() /
        std::max(
          1.E-2f,
          distFromTrackFit); /* Set 1.E-2f to prevent division by 0 and to set max histogram bin as 100 */
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  float ConeChargeFeatureTool::CalculateConicalness(const CaloHitList& caloHitList,
                                                    const CartesianVector& pfoStart,
                                                    const CartesianVector& pfoDir,
                                                    const float pfoLength)
  {
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      const float distFromTrackFit(
        ((pCaloHit->GetPositionVector() - pfoStart).GetCrossProduct(pfoDir)).GetMagnitude());
      const float hitLength(
        std::fabs((pCaloHit->GetPositionVector() - pfoStart).GetDotProduct(pfoDir)));
    ...
    if (nHitsConStart >= m_conMinHits && nHitsConEnd >= m_conMinHits &&
        totalChargeEnd > m_minCharge && std::sqrt(chargeConStart) > m_minCharge &&
        totalChargeStart > m_minCharge)
      conicalness =
        (std::sqrt(chargeConEnd / chargeConStart)) / (totalChargeEnd / totalChargeStart);
  ...
  StatusCode ConeChargeFeatureTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "ConMinHits", m_conMinHits));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MinCharge", m_minCharge));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConFracRange", m_conFracRange));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MoliereRadius", m_MoliereRadius));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MoliereRadiusFrac", m_MoliereRadiusFrac));
  ...
  void ThreeDLinearFitFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                       const Algorithm* const pAlgorithm,
                                       const pandora::ParticleFlowObject* const pInputPfo)
  {
    if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
    ...
    for (const Cluster* const pCluster : clusterList) {
      float straightLineLengthLargeCluster(-1.f), diffWithStraightLineMeanCluster(-1.f),
        maxFitGapLengthCluster(-1.f), rmsSlidingLinearFitCluster(-1.f);

      this->CalculateVariablesSlidingLinearFit(pCluster,
                                               straightLineLengthLargeCluster,
                                               diffWithStraightLineMeanCluster,
                                               maxFitGapLengthCluster,
                                               rmsSlidingLinearFitCluster);
  ...
  void ThreeDLinearFitFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                       StringVector& featureOrder,
                                       const std::string& featureToolName,
                                       const pandora::Algorithm* const pAlgorithm,
                                       const pandora::ParticleFlowObject* const pInputPfo)
  ...
  void ThreeDLinearFitFeatureTool::CalculateVariablesSlidingLinearFit(
    const pandora::Cluster* const pCluster,
    float& straightLineLengthLarge,
    float& diffWithStraightLineMean,
    float& maxFitGapLength,
  ...
  void ThreeDVertexDistanceFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                            const Algorithm* const pAlgorithm,
                                            const pandora::ParticleFlowObject* const pInputPfo)
  {
    if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
    ...
    for (const Vertex* pVertex : *pVertexList) {
      if ((pVertex->GetVertexLabel() == VERTEX_INTERACTION) &&
          (pVertex->GetVertexType() == VERTEX_3D)) {
    ...
    if (pInteractionVertex && (1 == nInteractionVertices)) {
      try {
        vertexDistance =
          (pInteractionVertex->GetPosition() - LArPfoHelper::GetVertex(pInputPfo)->GetPosition())
            .GetMagnitude();
      }
      catch (const StatusCodeException&) {
        ...
        if (!threeDCaloHitList.empty())
          vertexDistance =
            (pInteractionVertex->GetPosition() - (threeDCaloHitList.front())->GetPositionVector())
              .GetMagnitude();
      }
    }
  ...
  void ThreeDVertexDistanceFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                            StringVector& featureOrder,
                                            const std::string& featureToolName,
                                            const pandora::Algorithm* const pAlgorithm,
                                            const pandora::ParticleFlowObject* const pInputPfo)
  ...
  ThreeDOpeningAngleFeatureTool::ThreeDOpeningAngleFeatureTool()
    : m_hitFraction(0.5f), m_defaultValue(0.1f)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  void ThreeDOpeningAngleFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                          const Algorithm* const pAlgorithm,
                                          const pandora::ParticleFlowObject* const pInputPfo)
  {
    if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
      ...
      if ((pointVectorStart.size() > 1) && (pointVectorEnd.size() > 1)) {
        try {
          ...
          const float openingAngle(
            this->OpeningAngle(eigenVecsStart.at(0), eigenVecsStart.at(1), eigenValuesStart));
          const float closingAngle(
            this->OpeningAngle(eigenVecsEnd.at(0), eigenVecsEnd.at(1), eigenValuesEnd));
  ...
  void ThreeDOpeningAngleFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                          StringVector& featureOrder,
                                          const std::string& featureToolName,
                                          const pandora::Algorithm* const pAlgorithm,
                                          const pandora::ParticleFlowObject* const pInputPfo)
  ...
  void ThreeDOpeningAngleFeatureTool::Divide3DCaloHitList(const Algorithm* const pAlgorithm,
                                                          const CaloHitList& threeDCaloHitList,
                                                          CartesianPointVector& pointVectorStart,
                                                          CartesianPointVector& pointVectorEnd)
  {
    const VertexList* pVertexList(nullptr);
    (void)PandoraContentApi::GetCurrentList(*pAlgorithm, pVertexList);

    if (threeDCaloHitList.empty() || !pVertexList || pVertexList->empty()) return;
    ...
    if (pInteractionVertex && (1 == nInteractionVertices)) {
      ...
      std::sort(threeDCaloHitVector.begin(),
                threeDCaloHitVector.end(),
  ...
  float ThreeDOpeningAngleFeatureTool::OpeningAngle(const CartesianVector& principal,
                                                    const CartesianVector& secondary,
                                                    const CartesianVector& eigenValues) const
  {
    ...
    if (std::fabs(principalMagnitude) < std::numeric_limits<float>::epsilon()) {
      std::cout << "ThreeDOpeningAngleFeatureTool::OpeningAngle - The principal eigenvector is 0."
                << std::endl;
    ...
    const float cosTheta(principal.GetDotProduct(secondary) /
                         (principalMagnitude * secondaryMagnitude));

    if (cosTheta > 1.f) {
      std::cout << "PcaShowerParticleBuildingAlgorithm::OpeningAngle - cos(theta) reportedly "
                   "greater than 1."
                << std::endl;
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    }
    ...
    if (eigenValues.GetX() < std::numeric_limits<float>::epsilon()) {
      std::cout << "PcaShowerParticleBuildingAlgorithm::OpeningAngle - principal eigenvalue less "
                   "than or equal to 0."
                << std::endl;
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    }
  ...
  StatusCode ThreeDOpeningAngleFeatureTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "HitFraction", m_hitFraction));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "DefaultValue", m_defaultValue));
  ...
  void ThreeDPCAFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                 const Algorithm* const pAlgorithm,
                                 const pandora::ParticleFlowObject* const pInputPfo)
  {
    if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
    ...
    if (!threeDCaloHitList.empty()) {
      try {
        ...
        const float principalEigenvalue(eigenValues.GetX()),
          secondaryEigenvalue(eigenValues.GetY()), tertiaryEigenvalue(eigenValues.GetZ());
  ...
  void ThreeDPCAFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                 StringVector& featureOrder,
                                 const std::string& featureToolName,
                                 const pandora::Algorithm* const pAlgorithm,
                                 const pandora::ParticleFlowObject* const pInputPfo)
  ...
  void ThreeDChargeFeatureTool::Run(LArMvaHelper::MvaFeatureVector& featureVector,
                                    const Algorithm* const pAlgorithm,
                                    const pandora::ParticleFlowObject* const pInputPfo)
  {
    if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
    ...
    if (!clusterListW.empty())
      this->CalculateChargeVariables(
        pAlgorithm, clusterListW.front(), totalCharge, chargeSigma, chargeMean, endCharge);

    if (chargeMean > std::numeric_limits<float>::epsilon()) charge1 = chargeSigma / chargeMean;

    if (totalCharge > std::numeric_limits<float>::epsilon()) charge2 = endCharge / totalCharge;
  ...
  void ThreeDChargeFeatureTool::Run(LArMvaHelper::MvaFeatureMap& featureMap,
                                    StringVector& featureOrder,
                                    const std::string& featureToolName,
                                    const pandora::Algorithm* const pAlgorithm,
                                    const pandora::ParticleFlowObject* const pInputPfo)
  ...
  void ThreeDChargeFeatureTool::CalculateChargeVariables(const Algorithm* const pAlgorithm,
                                                         const pandora::Cluster* const pCluster,
                                                         float& totalCharge,
                                                         float& chargeSigma,
                                                         float& chargeMean,
    ...
    CaloHitList& caloHitList)
  {
    ...
    if (!pVertexList || pVertexList->empty()) return;
    ...
    if (pInteractionVertex && (1 == nInteractionVertices)) {
      const HitType hitType(LArClusterHelper::GetClusterHitType(pCluster));
      const CartesianVector vertexPosition2D(LArGeometryHelper::ProjectPosition(
        pAlgorithm->GetPandora(), pInteractionVertex->GetPosition(), hitType));
  ...
  StatusCode ThreeDChargeFeatureTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "EndChargeFraction", m_endChargeFraction));
  ...
  ThreeDChargeFeatureTool::VertexComparator::VertexComparator(
  ...
  bool ThreeDChargeFeatureTool::VertexComparator::operator()(const CaloHit* const left,
                                                             const CaloHit* const right) const
  {
