...
namespace lar_content {
  ...
  void BranchGrowingAlgorithm::FindAssociatedClusters(const Cluster* const pParticleSeed,
                                                      ClusterVector& candidateClusters,
                                                      ClusterUsageMap& forwardUsageMap,
                                                      ClusterUsageMap& backwardUsageMap) const
  {
    ...
    while (!currentSeedAssociations.empty()) {
      for (ClusterVector::iterator iterI = candidateClusters.begin(),
                                   iterIEnd = candidateClusters.end();
           iterI != iterIEnd;
           ++iterI) {
        const Cluster* const pCandidateCluster = *iterI;

        if (NULL == pCandidateCluster) continue;

        for (ClusterVector::iterator iterJ = currentSeedAssociations.begin(),
                                     iterJEnd = currentSeedAssociations.end();
             iterJ != iterJEnd;
             ++iterJ) {
          const Cluster* const pAssociatedCluster = *iterJ;

          const AssociationType associationType(
            this->AreClustersAssociated(pAssociatedCluster, pCandidateCluster));

          if (NONE == associationType) continue;
          ...
          const Association& existingAssociation =
            forwardUsageMap[pParticleSeed][pCandidateCluster];
          ...
          if (association.GetType() > existingAssociation.GetType()) {
            ...
            if (pParticleSeed != pAssociatedCluster)
              association.SetType(
                std::min(association.GetType(),
                         backwardUsageMap[pAssociatedCluster][pParticleSeed].GetType()));
  ...
  void BranchGrowingAlgorithm::IdentifyClusterMerges(const ClusterVector& particleSeedVector,
                                                     ...
                                                     SeedAssociationList& seedAssociationList) const
  {
    ...
    for (const Cluster* const pCluster : sortedCandidates) {
      ...
      if (particleSeedUsageMap.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
      ...
      for (const Cluster* const pParticleSeed : sortedSeeds) {
        ...
        if ((association.GetType() > bestType) ||
            ((association.GetType() == bestType) && (association.GetOrder() < bestOrder))) {
          // Break-out condition for single order associations
          if ((SINGLE_ORDER == association.GetType()) && (association.GetOrder() > 1)) continue;
      ...
      if (NULL == pBestParticleSeed) continue;

      seedAssociationList[pBestParticleSeed].push_back(pCluster);
    }

    // Now deal with seeds that have no associations
    for (ClusterVector::const_iterator iter = particleSeedVector.begin(),
                                       iterEnd = particleSeedVector.end();
         iter != iterEnd;
         ++iter) {
