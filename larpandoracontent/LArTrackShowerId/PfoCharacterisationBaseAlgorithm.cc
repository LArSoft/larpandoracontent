...
namespace lar_content {
    ...
    for (const std::string& pfoListName : m_inputPfoListNames) {
      ...
      if (!pPfoList || pPfoList->empty()) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "PfoCharacterisationBaseAlgorithm: unable to find pfo list " << pfoListName
                    << std::endl;
      ...
      for (const ParticleFlowObject* const pPfo : *pPfoList) {
        ...
        const bool isTrackLike(m_useThreeDInformation ? this->IsClearTrack(pPfo) :
                                                        this->IsClearTrack3x2D(pPfo));

        if (isTrackLike) {
          pfoMetadata.m_particleId = MU_MINUS;

          if (m_showerPfoListName == pfoListName) showersToTracks.push_back(pPfo);
        }
        else {
          pfoMetadata.m_particleId = E_MINUS;

          if (m_trackPfoListName == pfoListName) tracksToShowers.push_back(pPfo);
        }
        ...
        if (pPfo->GetParticleId() != pfoMetadata.m_particleId.Get())
          PANDORA_RETURN_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::ParticleFlowObject::AlterMetadata(*this, pPfo, pfoMetadata));

        if (!m_updateClusterIds) continue;
        ...
        for (const Cluster* const pCluster : twoDClusterList) {
          if (pCluster->GetParticleId() == pfoMetadata.m_particleId.Get()) continue;
          ...
          PANDORA_RETURN_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::Cluster::AlterMetadata(*this, pCluster, clusterMetadata));
        }
      }
    }
    ...
    if (!tracksToShowers.empty())
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                               ...
                               PandoraContentApi::SaveList(
                                 *this, m_trackPfoListName, m_showerPfoListName, tracksToShowers));

    if (!showersToTracks.empty())
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                               !=,
                               PandoraContentApi::SaveList(
                                 *this, m_showerPfoListName, m_trackPfoListName, showersToTracks));
  ...
  bool PfoCharacterisationBaseAlgorithm::IsClearTrack3x2D(
    const ParticleFlowObject* const pPfo) const
  {
    ...
    for (const Cluster* const pCluster : twoDClusterList) {
      ...
      if (!hitTypeSet.insert(hitType).second) continue;

      if (this->IsClearTrack(pCluster)) ++nTrackLikeViews;

      if (nTrackLikeViews >= m_minTrackLikeViews) return true;
    }
  ...
  StatusCode PfoCharacterisationBaseAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "UpdateClusterIds", m_updateClusterIds));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinTrackLikeViews", m_minTrackLikeViews));
