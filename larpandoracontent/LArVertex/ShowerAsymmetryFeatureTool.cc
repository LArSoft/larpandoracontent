...
namespace lar_content {
  ...
  ShowerAsymmetryFeatureTool::ShowerAsymmetryFeatureTool()
    : AsymmetryFeatureBaseTool(), m_vertexClusterDistance(4.f)
  {}
    ...
    const VertexSelectionBaseAlgorithm::ShowerClusterList& showerClusterList) const
  {
    ...
    for (const VertexSelectionBaseAlgorithm::ShowerCluster& showerCluster : showerClusterList) {
      if (this->ShouldUseShowerCluster(vertexPosition2D, showerCluster)) {
        ...
        if (STATUS_CODE_SUCCESS != showerFit.GetGlobalFitDirection(rL, showerDirection)) continue;

        ClusterVector asymmetryClusters;
        std::copy(showerCluster.GetClusters().begin(),
                  showerCluster.GetClusters().end(),
                  std::back_inserter(asymmetryClusters));

        showerAsymmetry =
          this->CalculateAsymmetry(true, vertexPosition2D, asymmetryClusters, showerDirection);
  ...
  StatusCode ShowerAsymmetryFeatureTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexClusterDistance", m_vertexClusterDistance));
