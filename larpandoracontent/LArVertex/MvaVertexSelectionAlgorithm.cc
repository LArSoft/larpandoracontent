...
namespace lar_content {
  ...
  template <typename T>
  void MvaVertexSelectionAlgorithm<T>::GetVertexScoreList(const VertexVector& vertexVector,
                                                          ...
                                                          VertexScoreList& vertexScoreList) const
  {
    ...
    this->CalculateClusterSlidingFits(
      clustersU, m_minClusterCaloHits, m_slidingFitWindow, slidingFitDataListU);
    this->CalculateClusterSlidingFits(
      clustersV, m_minClusterCaloHits, m_slidingFitWindow, slidingFitDataListV);
    this->CalculateClusterSlidingFits(
      clustersW, m_minClusterCaloHits, m_slidingFitWindow, slidingFitDataListW);
    ...
    const ClusterListMap clusterListMap{
      {TPC_VIEW_U, clustersU}, {TPC_VIEW_V, clustersV}, {TPC_VIEW_W, clustersW}};
    ...
    EventFeatureInfo eventFeatureInfo(
      this->CalculateEventFeatures(clustersU, clustersV, clustersW, vertexVector));
    ...
    for (const Vertex* const pVertex : vertexVector) {
      this->PopulateVertexFeatureInfoMap(beamConstants,
                                         clusterListMap,
    ...
    if (m_trainingSetMode)
      this->ProduceTrainingSets(
        vertexVector, bestRegionVertices, vertexFeatureInfoMap, eventFeatureList, kdTreeMap);

    if ((!m_trainingSetMode || m_allowClassifyDuringTraining) && !bestRegionVertices.empty()) {
      // Use mva to choose the region.
      const Vertex* const pBestRegionVertex(this->CompareVertices(bestRegionVertices,
                                                                  vertexFeatureInfoMap,
                                                                  eventFeatureList,
                                                                  kdTreeMap,
                                                                  m_mvaRegion,
      ...
      if (!regionalVertices.empty()) {
        ...
        const Vertex* const pBestVertex(this->CompareVertices(
          regionalVertices, vertexFeatureInfoMap, eventFeatureList, kdTreeMap, m_mvaVertex, true));
        this->PopulateFinalVertexScoreList(
          vertexFeatureInfoMap, pBestVertex, vertexVector, vertexScoreList);
      }
    }
  }
  ...
  template <typename T>
  const pandora::Vertex* MvaVertexSelectionAlgorithm<T>::CompareVertices(
    ...
    const bool useRPhi) const
  {
    ...
    for (const Vertex* const pVertex : vertexVector) {
      ...
      if (!m_legacyVariables) {
        ...
        if (LArMvaHelper::Classify(
              t,
              LArMvaHelper::ConcatenateFeatureLists(
                eventFeatureList, featureList, chosenFeatureList, sharedFeatureList))) {
          pBestVertex = pVertex;
          chosenFeatureList = featureList;
        }
      }
      else {
        if (LArMvaHelper::Classify(t,
                                   LArMvaHelper::ConcatenateFeatureLists(
                                     eventFeatureList, featureList, chosenFeatureList))) {
          pBestVertex = pVertex;
          chosenFeatureList = featureList;
        }
      }
  ...
  StatusCode MvaVertexSelectionAlgorithm<T>::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "RegionMvaName", m_regionMvaName));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexMvaName", m_vertexMvaName));

    // ATTN : Need access to base class member variables at this point, so call read settings prior to end of this function
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, TrainedVertexSelectionAlgorithm::ReadSettings(xmlHandle));

    if ((!m_trainingSetMode || m_allowClassifyDuringTraining)) {
      if (m_mvaFileName.empty() || m_regionMvaName.empty() || m_vertexMvaName.empty()) {
        std::cout << "MvaVertexSelectionAlgorithm: MvaFileName, RegionMvaName and VertexMvaName "
                     "must be set if training set mode is"
