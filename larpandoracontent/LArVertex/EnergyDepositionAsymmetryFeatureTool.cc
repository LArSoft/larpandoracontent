...
namespace lar_content {
  ...
  float EnergyDepositionAsymmetryFeatureTool::CalculateAsymmetry(
    const bool useEnergyMetrics,
    const CartesianVector& vertexPosition2D,
    const ClusterVector& clusterVector,
    const CartesianVector& localWeightedDirectionSum) const
  {
    ...
    for (const Cluster* const pCluster : clusterVector) {
      ...
      for (const CaloHit* const pCaloHit : caloHitVector) {
        if (pCaloHit->GetPositionVector().GetDotProduct(localWeightedDirection) <
            evtProjectedVtxPos) {
          minBeforeProjectedPos =
            std::min(minBeforeProjectedPos,
                     pCaloHit->GetPositionVector().GetDotProduct(localWeightedDirection));
          maxBeforeProjectedPos =
            std::max(maxBeforeProjectedPos,
                     pCaloHit->GetPositionVector().GetDotProduct(localWeightedDirection));
          ...
          minAfterProjectedPos =
            std::min(minAfterProjectedPos,
                     pCaloHit->GetPositionVector().GetDotProduct(localWeightedDirection));
          maxAfterProjectedPos =
            std::max(maxAfterProjectedPos,
                     pCaloHit->GetPositionVector().GetDotProduct(localWeightedDirection));
    ...
    const float beforeVtxEnergyDeposition(
      beforeLength < std::numeric_limits<float>::epsilon() ? 0 : beforeVtxEnergy / beforeLength);

    const float afterVtxEnergyDeposition(
      afterLength < std::numeric_limits<float>::epsilon() ? 0 : afterVtxEnergy / afterLength);
    ...
    if (useEnergyMetrics && totalEnergyDeposition > std::numeric_limits<float>::epsilon())
      return std::fabs((afterVtxEnergyDeposition - beforeVtxEnergyDeposition)) /
             totalEnergyDeposition;

    if (0 == totalHits) throw StatusCodeException(STATUS_CODE_FAILURE);

    const float beforeVtxHitDeposition(
      beforeLength < std::numeric_limits<float>::epsilon() ? 0 : beforeVtxHits / beforeLength);

    const float afterVtxHitDeposition(
      afterLength < std::numeric_limits<float>::epsilon() ? 0 : afterVtxHits / afterLength);
