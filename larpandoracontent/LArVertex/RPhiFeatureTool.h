 ...
 */
  ...
  private:
    ...
    public:
      ...
      typedef std::multimap<float, float>
        ContributionList; ///< Map from x coord to weight, ATTN avoid map.find, etc. with float key
     ...
     */
    float GetFastScore(const KernelEstimate& kernelEstimateU,
     ...
     */
    float GetMidwayScore(const KernelEstimate& kernelEstimateU,
                         const KernelEstimate& kernelEstimateV,
                         const KernelEstimate& kernelEstimateW) const;
     ...
     */
    float GetFullScore(const KernelEstimate& kernelEstimateU,
                       const KernelEstimate& kernelEstimateV,
                       const KernelEstimate& kernelEstimateW) const;
     ...
     */
    void FillKernelEstimate(const pandora::Vertex* const pVertex,
    ...
    bool
      m_fastScoreCheck; ///< Whether to use the fast histogram based score to selectively avoid calling full or midway scores
    bool m_fastScoreOnly; ///< Whether to use the fast histogram based score only
    bool
      m_fullScore; ///< Whether to use the full kernel density estimation score, as opposed to the midway score

    float m_kernelEstimateSigma; ///< The Gaussian width to use for kernel estimation
    float
      m_kappa; ///< Hit-deweighting offset, of form: weight = 1 / sqrt(distance + kappa), units cm
    float
      m_maxHitVertexDisplacement1D; ///< Max hit-vertex displacement in *any one dimension* for contribution to kernel estimation

    float
      m_minFastScoreFraction; ///< Fast score must be at least this fraction of best fast score to calculate full score
    ...
    bool
      m_enableFolding; ///< Whether to enable folding of -pi -> +pi phi distribution into 0 -> +pi region only
  };
  ...
  inline const RPhiFeatureTool::KernelEstimate::ContributionList&
  RPhiFeatureTool::KernelEstimate::GetContributionList() const
  {
  ...
  inline float RPhiFeatureTool::KernelEstimate::GetSigma() const { return m_sigma; }
