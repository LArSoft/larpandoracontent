 ...
 */
  ...
  private:
     ...
     */
    void SelectClusters(pandora::ClusterVector& clusterVectorU,
     ...
     */
    void CreateEndpointCandidates(const pandora::ClusterVector& clusterVector1,
                                  const pandora::ClusterVector& clusterVector2) const;
     ...
     */
    void CreateEndpointVertex(const pandora::CartesianVector& position1,
     ...
     */
    void CreateCrossingCandidates(const pandora::ClusterVector& clusterVectorU,
     ...
     */
    void FindCrossingPoints(const pandora::ClusterVector& clusterVector,
     ...
     */
    void GetSpacepoints(const pandora::Cluster* const pCluster,
                        pandora::CartesianPointVector& spacePoints) const;
     ...
     */
    void FindCrossingPoints(const pandora::CartesianPointVector& spacepoints1,
     ...
     */
    void CreateCrossingVertices(const pandora::CartesianPointVector& crossingPoints1,
    ...
    float
      m_chiSquaredCut; ///< The chi squared cut (accept only 3D vertex positions with values below cut)
    ...
    unsigned int
      m_extrapolationNSteps; ///< Number of extrapolation steps, at each end of cluster, of specified size
    float m_extrapolationStepSize; ///< The extrapolation step size in cm
    float
      m_maxCrossingSeparationSquared; ///< The separation (squared) between spacepoints below which a crossing can be identified
    float
      m_minNearbyCrossingDistanceSquared; ///< The minimum allowed distance between identified crossing positions
    ...
    float
      m_nClustersPassingMaxCutsPar; ///< Parameter for number of clusters passing the max base cluster selection cuts
  };
