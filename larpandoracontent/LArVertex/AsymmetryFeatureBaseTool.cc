...
namespace lar_content {
  ...
  void AsymmetryFeatureBaseTool::Run(
    ...
    float&)
  {
    ...
    asymmetry += this->GetAsymmetryForView(
      ...
      showerClusterListMap.empty() ? VertexSelectionBaseAlgorithm::ShowerClusterList() :
  ...
  float AsymmetryFeatureBaseTool::CalculateAsymmetry(
    const bool useEnergyMetrics,
    const CartesianVector& vertexPosition2D,
    const ClusterVector& asymmetryClusters,
    const CartesianVector& localWeightedDirectionSum) const
  {
    ...
    if (0 == totHitCount) throw StatusCodeException(STATUS_CODE_FAILURE);

    return std::fabs(
             (static_cast<float>(afterVtxHitCount) - static_cast<float>(beforeVtxHitCount))) /
  ...
  StatusCode AsymmetryFeatureBaseTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxAsymmetryDistance", m_maxAsymmetryDistance));
