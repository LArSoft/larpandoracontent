...
namespace lar_content {
  ...
  void VertexRefinementAlgorithm::GetClusterLists(const StringVector& inputClusterListNames,
                                                  ...
                                                  ClusterList& clusterListW) const
  {
    for (const std::string& clusterListName : inputClusterListNames) {
      ...
      if (!pClusterList || pClusterList->empty()) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "VertexRefinementAlgorithm: unable to find cluster list " << clusterListName
                    << std::endl;
      ...
      ClusterList& outputClusterList((TPC_VIEW_U == hitType) ?
                                       clusterListU :
  ...
  void VertexRefinementAlgorithm::RefineVertices(const VertexList* const pVertexList,
                                                 ...
                                                 const ClusterList& clusterListW) const
  {
    for (const Vertex* const pVertex : *pVertexList) {
      ...
      const CartesianVector vtxU(this->RefineVertexTwoD(
        clusterListU,
        LArGeometryHelper::ProjectPosition(this->GetPandora(), originalPosition, TPC_VIEW_U)));
      const CartesianVector vtxV(this->RefineVertexTwoD(
        clusterListV,
        LArGeometryHelper::ProjectPosition(this->GetPandora(), originalPosition, TPC_VIEW_V)));
      const CartesianVector vtxW(this->RefineVertexTwoD(
        clusterListW,
        LArGeometryHelper::ProjectPosition(this->GetPandora(), originalPosition, TPC_VIEW_W)));

      CartesianVector vtxUV(0.f, 0.f, 0.f), vtxUW(0.f, 0.f, 0.f), vtxVW(0.f, 0.f, 0.f),
        vtx3D(0.f, 0.f, 0.f), position3D(0.f, 0.f, 0.f);
      float chi2UV(0.f), chi2UW(0.f), chi2VW(0.f), chi23D(0.f), chi2(0.f);

      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_U, TPC_VIEW_V, vtxU, vtxV, vtxUV, chi2UV);
      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_U, TPC_VIEW_W, vtxU, vtxW, vtxUW, chi2UW);
      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_V, TPC_VIEW_W, vtxV, vtxW, vtxVW, chi2VW);
      LArGeometryHelper::MergeThreePositions3D(
        this->GetPandora(), TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W, vtxU, vtxV, vtxW, vtx3D, chi23D);
      ...
      if (chi2 > m_chiSquaredCut) position3D = originalPosition;
  ...
  CartesianVector VertexRefinementAlgorithm::RefineVertexTwoD(
    const ClusterList& clusterList,
    const CartesianVector& originalVtxPos) const
  {
    CartesianPointVector intercepts, directions;
    FloatVector weights;

    for (const Cluster* const pCluster : clusterList) {
      if (LArClusterHelper::GetClosestDistance(originalVtxPos, pCluster) > 10) continue;

      if (pCluster->GetNCaloHits() < m_minimumHitsCut) continue;
      ...
      weights.push_back(
        1.f / ((LArClusterHelper::GetClosestPosition(originalVtxPos, pCluster) - originalVtxPos)
                 .GetMagnitudeSquared() +
               1));
    }

    CartesianVector newVtxPos(originalVtxPos);
    if (intercepts.size() > 1) GetBestFitPoint(intercepts, directions, weights, newVtxPos);

    if ((newVtxPos - originalVtxPos).GetMagnitude() > m_twoDDistanceCut) return originalVtxPos;
  ...
  void VertexRefinementAlgorithm::GetBestFitPoint(const CartesianPointVector& intercepts,
                                                  const CartesianPointVector& directions,
                                                  const FloatVector& weights,
                                                  CartesianVector& bestFitPoint) const
  {
    ...
    if ((G.transpose() * G).determinant() < std::numeric_limits<float>::epsilon()) {
      bestFitPoint = CartesianVector(std::numeric_limits<float>::max(),
  ...
  StatusCode VertexRefinementAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "DistanceCut", m_distanceCut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinimumHitsCut", m_minimumHitsCut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "TwoDDistanceCut", m_twoDDistanceCut));
