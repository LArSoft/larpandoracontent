 ...
 */
  ...
  public:
    ...
    public:
      /**
         ...
         */
      SlidingFitData(const pandora::Cluster* const pCluster,
    ...
    public:
      ...
      /**
         ...
         */
      pandora::CartesianPointVector GetClusterListCoordinateVector(
    ...
    typedef std::map<pandora::HitType, const pandora::ClusterList&>
      ClusterListMap; ///< Map array of cluster lists for passing to tools
    typedef std::map<pandora::HitType, const SlidingFitDataList>
      SlidingFitDataListMap; ///< Map of sliding fit data lists for passing to tools
    typedef std::map<pandora::HitType, const ShowerClusterList>
      ShowerClusterListMap; ///< Map of shower cluster lists for passing to tools
    typedef std::map<pandora::HitType, const std::reference_wrapper<HitKDTree2D>>
      KDTreeMap; ///< Map array of hit kd trees for passing to tools

    typedef MvaFeatureTool<const VertexSelectionBaseAlgorithm* const,
                           const pandora::Vertex* const,
                           const SlidingFitDataListMap&,
                           const ClusterListMap&,
                           const KDTreeMap&,
  ...
  protected:
     ...
     */
    virtual void FilterVertexList(const pandora::VertexList* const pInputVertexList,
                                  HitKDTree2D& kdTreeU,
                                  HitKDTree2D& kdTreeV,
                                  HitKDTree2D& kdTreeW,
                                  pandora::VertexVector& filteredVertices) const;
     ...
     */
    virtual void GetBeamConstants(const pandora::VertexVector& vertexVector,
     ...
     */
    virtual void GetVertexScoreList(const pandora::VertexVector& vertexVector,
                                    const BeamConstants& beamConstants,
                                    HitKDTree2D& kdTreeU,
                                    HitKDTree2D& kdTreeV,
                                    HitKDTree2D& kdTreeW,
                                    VertexScoreList& vertexScoreList) const = 0;
     ...
     */
    void CalculateClusterSlidingFits(const pandora::ClusterList& inputClusterList,
     ...
     *  @return the score
     */
    float GetBeamDeweightingScore(const BeamConstants& beamConstants,
     ...
     */
    float VertexHitEnergy(const pandora::Vertex* const pVertex,
  ...
  private:
     ...
     *  @return boolean
     */
    bool IsVertexOnHit(const pandora::Vertex* const pVertex,
                       const pandora::HitType hitType,
                       HitKDTree2D& kdTree) const;
     ...
     */
    void SelectTopScoreVertices(VertexScoreList& vertexScoreList,
                                pandora::VertexList& selectedVertexList) const;
     ...
     *  @return boolean
     */
    bool AcceptVertexLocation(const pandora::Vertex* const pVertex,
                              const pandora::VertexList& selectedVertexList) const;
     ...
     */
    static bool SortByVertexZPosition(const pandora::Vertex* const pLhs,
  ...
  private:
    ...
    bool
      m_replaceCurrentVertexList; ///< Whether to replace the current vertex list with the output list

    bool
      m_beamMode; ///< Whether to run in beam mode, assuming neutrinos travel in positive z-direction
    float
      m_nDecayLengthsInZSpan; ///< The number of score decay lengths to use over the course of the vertex z-span

    bool
      m_selectSingleVertex; ///< Whether to make a final decision and select just one vertex candidate
    unsigned int
      m_maxTopScoreSelections; ///< Max number of top-scoring vertex candidate to select for output

    float
      m_maxOnHitDisplacement; ///< Max hit-vertex displacement for declaring vertex to lie on a hit in each view

    float
      m_minCandidateDisplacement; ///< Ignore other top-scoring candidates located in close proximity to original
    float
      m_minCandidateScoreFraction; ///< Ignore other top-scoring candidates with score less than a fraction of original
    ...
    bool
      m_isEmptyViewAcceptable; ///< Whether views entirely empty of hits are classed as 'acceptable' for candidate filtration
    unsigned int
      m_minVertexAcceptableViews; ///< The minimum number of views in which a candidate must sit on/near a hit or in a gap (or view can be empty)
  };

  //------------------------------------------------------------------------------------------------------------------------------------------

  inline float VertexSelectionBaseAlgorithm::GetBeamDeweightingScore(
    const BeamConstants& beamConstants,
    const pandora::Vertex* const pVertex) const
  {
    const float vertexMinZ(
      std::max(pVertex->GetPosition().GetZ(), beamConstants.GetMinZCoordinate()));
    return (beamConstants.GetMinZCoordinate() - vertexMinZ) * beamConstants.GetDecayConstant();
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  inline bool VertexSelectionBaseAlgorithm::IsBeamModeOn() const { return m_beamMode; }

  //------------------------------------------------------------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------------------------------------------------------------

  inline VertexSelectionBaseAlgorithm::VertexScore::VertexScore(
  ...
  inline float VertexSelectionBaseAlgorithm::VertexScore::GetScore() const { return m_score; }
  ...
  inline void VertexSelectionBaseAlgorithm::BeamConstants::SetConstants(const float minZCoordinate,
  ...
  VertexSelectionBaseAlgorithm::SlidingFitData::GetMinLayerDirection() const
  {
  ...
  inline const pandora::CartesianVector&
  VertexSelectionBaseAlgorithm::SlidingFitData::GetMaxLayerDirection() const
  {
  ...
  inline const pandora::CartesianVector&
  VertexSelectionBaseAlgorithm::SlidingFitData::GetMinLayerPosition() const
  {
  ...
  inline const pandora::CartesianVector&
  VertexSelectionBaseAlgorithm::SlidingFitData::GetMaxLayerPosition() const
  {
  ...
  inline const pandora::ClusterList& VertexSelectionBaseAlgorithm::ShowerCluster::GetClusters()
    const
  {
