...
namespace lar_content {
  ...
  void EnergyKickVertexSelectionAlgorithm::GetVertexScoreList(
    ...
    VertexScoreList& vertexScoreList) const
  {
    ...
    float bestFastScore(
      0.f); // not actually used - artefact of toolizing RPhi score and still using performance trick
    for (const Vertex* const pVertex : vertexVector) {
      const float beamDeweightingScore(
        this->IsBeamModeOn() ? this->GetBeamDeweightingScore(beamConstants, pVertex) : 0.f);
      ...
      const float energyKick(
        LArMvaHelper::CalculateFeaturesOfType<EnergyKickFeatureTool>(m_featureToolVector,
                                                                     ...
                                                                     beamDeweightingScore,
                                                                     bestFastScore)
          .at(0)
          .Get());

      const float energyAsymmetry(
  ...
  StatusCode EnergyKickVertexSelectionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "Epsilon", m_epsilon));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "AsymmetryConstant", m_asymmetryConstant));

    if ((m_epsilon < std::numeric_limits<float>::epsilon()) ||
        (m_asymmetryConstant < std::numeric_limits<float>::epsilon())) {
      std::cout << "EnergyKickVertexSelection: Invalid parameter(s), Epsilon " << m_epsilon
                << ", AsymmetryConstant " << m_asymmetryConstant << std::endl;
