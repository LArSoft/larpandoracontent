...
namespace lar_content {
  ...
  StatusCode CandidateVertexCreationAlgorithm::Run()
  {
    try {
      ...
      if (!m_inputVertexListName.empty()) this->AddInputVertices();

      if (!pVertexList->empty()) {
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::SaveList<Vertex>(*this, m_outputVertexListName));
  ...
  void CandidateVertexCreationAlgorithm::SelectClusters(ClusterVector& clusterVectorU,
                                                        ClusterVector& clusterVectorV,
                                                        ClusterVector& clusterVectorW)
  {
    for (const std::string& clusterListName : m_inputClusterListNames) {
      ...
      if (!pClusterList || pClusterList->empty()) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "CandidateVertexCreationAlgorithm: unable to find cluster list "
      ...
      ClusterVector& selectedClusterVector(
        (TPC_VIEW_U == hitType) ? clusterVectorU :
                                  (TPC_VIEW_V == hitType) ? clusterVectorV : clusterVectorW);

      if (!selectedClusterVector.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
      ...
      if (m_reducedCandidates) {
        for (const Cluster* const pCluster : sortedClusters) {
          ...
          if (pCluster->GetParticleId() == E_MINUS) selectionCutFactor = m_selectionCutFactorMax;

          if (pCluster->GetNCaloHits() < m_minClusterCaloHits * selectionCutFactor) continue;

          if (LArClusterHelper::GetLengthSquared(pCluster) <
              m_minClusterLengthSquared * selectionCutFactor * selectionCutFactor)
            continue;
  ...
  void CandidateVertexCreationAlgorithm::CreateEndpointCandidates(
    const ClusterVector& clusterVector1,
    const ClusterVector& clusterVector2) const
  {
    ...
    const TwoDSlidingFitResult& fitResult2) const
  {
    ...
    if ((((position1.GetX() < minLayerPosition2.GetX()) &&
          (position1.GetX() < maxLayerPosition2.GetX())) ||
         ((position1.GetX() > minLayerPosition2.GetX()) &&
          (position1.GetX() > maxLayerPosition2.GetX()))) &&
    ...
    LArGeometryHelper::MergeTwoPositions3D(
      this->GetPandora(), hitType1, hitType2, position1, position2, position3D, chiSquared);

    if (chiSquared > m_chiSquaredCut) return;
    ...
    const ClusterVector& clusterVectorW) const
  {
    ...
    this->CreateCrossingVertices(
      crossingsU, crossingsV, TPC_VIEW_U, TPC_VIEW_V, nCrossingCandidates);
    this->CreateCrossingVertices(
      crossingsU, crossingsW, TPC_VIEW_U, TPC_VIEW_W, nCrossingCandidates);
    this->CreateCrossingVertices(
      crossingsV, crossingsW, TPC_VIEW_V, TPC_VIEW_W, nCrossingCandidates);
  }
    ...
    CartesianPointVector& crossingPoints) const
  {
    ...
    for (const Cluster* const pCluster : clusterVector) {
      ClusterToSpacepointsMap::iterator mapIter(
        clusterToSpacepointsMap.emplace(pCluster, CartesianPointVector()).first);
    ...
    for (const Cluster* const pCluster1 : clusterVector) {
      for (const Cluster* const pCluster2 : clusterVector) {
        ...
        this->FindCrossingPoints(clusterToSpacepointsMap.at(pCluster1),
                                 clusterToSpacepointsMap.at(pCluster2),
                                 crossingPoints);
      }
    }
  }
  ...
  void CandidateVertexCreationAlgorithm::GetSpacepoints(const Cluster* const pCluster,
  ...
  void CandidateVertexCreationAlgorithm::FindCrossingPoints(
    const CartesianPointVector& spacepoints1,
    const CartesianPointVector& spacepoints2,
    CartesianPointVector& crossingPoints) const
  {
    ...
    if (bestCrossingFound) {
      ...
      for (const CartesianVector& existingPosition : crossingPoints) {
        if (((existingPosition - bestPosition1).GetMagnitudeSquared() <
             m_minNearbyCrossingDistanceSquared) ||
            ((existingPosition - bestPosition2).GetMagnitudeSquared() <
             m_minNearbyCrossingDistanceSquared)) {
  ...
  void CandidateVertexCreationAlgorithm::CreateCrossingVertices(
    const CartesianPointVector& crossingPoints1,
    const CartesianPointVector& crossingPoints2,
    const HitType hitType1,
    const HitType hitType2,
    unsigned int& nCrossingCandidates) const
  {

    for (const CartesianVector& position1 : crossingPoints1) {
      for (const CartesianVector& position2 : crossingPoints2) {
        if (nCrossingCandidates > m_nMaxCrossingCandidates) return;

        if (std::fabs(position1.GetX() - position2.GetX()) > m_maxCrossingXDiscrepancy) continue;
        ...
        if (chiSquared > m_chiSquaredCut) continue;
  ...
  void CandidateVertexCreationAlgorithm::AddInputVertices() const
  {
    ...
    try { // ATTN - No guarantee the list has been initialised, but silent failure here is ok
      PandoraContentApi::GetList(*this, m_inputVertexListName, pInputVertexList);
      if (!pInputVertexList) return;
  ...
  void CandidateVertexCreationAlgorithm::AddToSlidingFitCache(const Cluster* const pCluster)
  {
    ...
    if (!m_slidingFitResultMap
           .insert(TwoDSlidingFitResultMap::value_type(pCluster, slidingFitResult))
           .second)
  ...
  const TwoDSlidingFitResult& CandidateVertexCreationAlgorithm::GetCachedSlidingFitResult(
  ...
  void CandidateVertexCreationAlgorithm::TidyUp() { m_slidingFitResultMap.clear(); }
  ...
  StatusCode CandidateVertexCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ChiSquaredCut", m_chiSquaredCut));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NMaxCrossingCandidates", m_nMaxCrossingCandidates));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ExtrapolationNSteps", m_extrapolationNSteps));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ExtrapolationStepSize", m_extrapolationStepSize));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ReducedCandidates", m_reducedCandidates));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SelectionCutFactorMax", m_selectionCutFactorMax));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxCrossingSeparation", maxCrossingSeparation));
