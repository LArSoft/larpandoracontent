 ...
 */
  ...
  private:
     ...
     */
    Classification ClassifyPfo(const unsigned int& nHits,
    ...
    void PrintPfoTable(const pandora::PfoVector& orderedPfoVector,
                       ...
                       const pandora::PfoList& ambiguousPfos) const;
    ...
    LArMCParticleHelper::PrimaryParameters
      m_parameters; ///< Parameters used to decide when an MCParticle is reconstructable
    unsigned int
      m_minHitsToConsiderTagging; ///< The minimum number of hits to consider a Pfo for tagging
