...
namespace lar_content {
  ...
  MCParticleMonitoringAlgorithm::MCParticleMonitoringAlgorithm()
    : m_useTrueNeutrinosOnly(false), m_minHitsForDisplay(1)
  {}
  ...
  {
    std::cout << "---MC-PARTICLE-MONITORING--------------------------------------------------------"
                 "---------------"
              << std::endl;
  ...
  void MCParticleMonitoringAlgorithm::PrintPrimaryMCParticles(
    const LArMCParticleHelper::MCContributionMap& mcContributionMap) const
  {
    ...
    for (const MCParticle* const pMCPrimary : mcPrimaryVector) {
      ...
      if (caloHitList.size() >= m_minHitsForDisplay) {
        std::cout << std::endl
                  << "--Primary " << index << ", MCPDG " << pMCPrimary->GetParticleId()
                  << ", Energy " << pMCPrimary->GetEnergy() << ", Dist. "
                  ...
                  << LArMonitoringHelper::CountHitsByType(TPC_VIEW_W, caloHitList) << ")"
                  << std::endl;
        ...
        LArMCParticleHelper::GetMCParticleToCaloHitMatches(
          &caloHitList, mcToPrimaryMCMap, caloHitToPrimaryMCMap, mcToTrueHitListMap);
    ...
    const int depth) const
  {
    const CaloHitList& caloHitList(
      mcToTrueHitListMap.count(pMCParticle) ? mcToTrueHitListMap.at(pMCParticle) : CaloHitList());
    ...
    if (caloHitList.size() >= m_minHitsForDisplay) {
      ...
      std::cout << "MCPDG " << pMCParticle->GetParticleId() << ", Energy "
                << pMCParticle->GetEnergy() << ", Dist. "
                << (pMCParticle->GetEndpoint() - pMCParticle->GetVertex()).GetMagnitude()
                << ", nMCHits " << caloHitList.size() << " ("
                << LArMonitoringHelper::CountHitsByType(TPC_VIEW_U, caloHitList) << ", "
                << LArMonitoringHelper::CountHitsByType(TPC_VIEW_V, caloHitList) << ", "
                << LArMonitoringHelper::CountHitsByType(TPC_VIEW_W, caloHitList) << ")"
  ...
  StatusCode MCParticleMonitoringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinHitsForDisplay", m_minHitsForDisplay));
