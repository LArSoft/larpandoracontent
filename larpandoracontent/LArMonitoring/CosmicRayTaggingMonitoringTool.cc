...
namespace lar_content {
  ...
  void CosmicRayTaggingMonitoringTool::FindAmbiguousPfos(const PfoList& parentCosmicRayPfos,
                                                         ...
                                                         const MasterAlgorithm* const pAlgorithm)
  {
    ...
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                            !=,
                            PandoraContentApi::GetList(*pAlgorithm, m_caloHitList2D, pCaloHitList));
    ...
    LArMCParticleHelper::GetPfoToReconstructable2DHitsMap(parentCosmicRayPfos,
                                                          mcParticlesToGoodHitsMaps,
                                                          pfoToReconstructable2DHitsMap,
                                                          m_parameters.m_foldBackHierarchy);
    ...
    LArMCParticleHelper::MCContributionMapVector targetsToGoodHitsMaps(
      {nuMCParticlesToGoodHitsMap, beamMCParticlesToGoodHitsMap});
    this->CalculatePfoMetrics(pfoToMCParticleHitSharingMap,
                              pfoToReconstructable2DHitsMap,
                              targetsToGoodHitsMaps,
                              pfoSignificanceMap,
                              pfoPurityMap,
                              pfoClassificationMap);

    // -------------------------------------------------------------------------------------------------------------------------------------

    // Print the monte-carlo information for this event
    MCParticleVector orderedMCParticleVector;
    LArMonitoringHelper::GetOrderedMCParticleVector(mcParticlesToGoodHitsMaps,
                                                    orderedMCParticleVector);
    ...
    LArMonitoringHelper::PrintMCParticleTable(beamMCParticlesToGoodHitsMap,
                                              orderedMCParticleVector);
    ...
    std::cout << "Columns with headers [n] are the number of shared hits between the Pfo and the "
                 "target MCParticle with ID n."
              << std::endl;

    PfoVector orderedPfoVector;
    LArMonitoringHelper::GetOrderedPfoVector(pfoToReconstructable2DHitsMap, orderedPfoVector);
    this->PrintPfoTable(orderedPfoVector,
                        pfoToReconstructable2DHitsMap,
                        pfoPurityMap,
                        pfoSignificanceMap,
                        pfoClassificationMap,
  ...
  void CosmicRayTaggingMonitoringTool::CalculatePfoMetrics(
    const LArMCParticleHelper::PfoToMCParticleHitSharingMap& hitSharingMap,
    ...
    PfoToFloatMap& pfoSignificanceMap,
    PfoToFloatMap& pfoPurityMap,
    PfoClassificationMap& pfoClassificationMap) const
  {
    ...
    for (const ParticleFlowObject* const pPfo : sortedPfos) {
      ...
      if (n2DHits != 0) {
        ...
        for (const LArMCParticleHelper::MCParticleCaloHitListPair& targetHitsShared :
             hitSharingMap.at(pPfo)) {
          ...
          for (const LArMCParticleHelper::MCContributionMap& mcContributionMap :
               targetsToGoodHitsMaps) {
      ...
      Classification classification(
        this->ClassifyPfo(n2DHits, significance, purity, this->IsMainMCParticleMuon(pPfo)));
      if (!pfoClassificationMap.insert(PfoClassificationMap::value_type(pPfo, classification))
             .second)
        throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
    }
  }
  ...
  bool CosmicRayTaggingMonitoringTool::IsMainMCParticleMuon(
    const ParticleFlowObject* const /*pPfo*/) const
  {
    bool isMuon(false);
    try {
      isMuon =
        false; // TODO Local treatment is being developed, specific to this tool (std::abs(LArMCParticleHelper::GetMainMCParticle(pPfo)->GetParticleId()) == MU_MINUS);
    }
  ...
  CosmicRayTaggingMonitoringTool::Classification CosmicRayTaggingMonitoringTool::ClassifyPfo(
    ...
    const float& purity,
    const bool isMuon) const
  {
    if (nHits < m_minHitsToConsiderTagging) return SPARSE;

    bool isPure(purity > m_minPurity);
    bool isImpure((1 - purity) > m_minImpurity);
    bool isSignificant(significance > m_minSignificance);

    if (!isPure && !isImpure) return MIXED;

    if (isPure && isSignificant) return TARGET;

    if (isPure && !isSignificant) return FRAGMENTED;

    if (!isPure && isSignificant) return ABSORBED;

    if (!isPure && !isSignificant && isMuon) return CR_MUON;
  ...
  void CosmicRayTaggingMonitoringTool::PrintPfoTable(
    ...
    const PfoList& ambiguousPfos) const
  {
    ...
    LArFormattingHelper::Table table({"ID",
                                      "PID",
                                      "",
                                      "nHits",
                                      "U",
    ...
    for (unsigned int id = 0; id < orderedPfoVector.size(); ++id) {
      ...
      LArMCParticleHelper::PfoContributionMap::const_iterator it =
        pfoToReconstructable2DHitsMap.find(pPfo);
      ...
      table.AddElement(this->GetClassificationName(classification),
                       LArFormattingHelper::INVERTED,
                       this->GetClassificationColor(classification));

      const bool isTagged(std::find(ambiguousPfos.begin(), ambiguousPfos.end(), pPfo) ==
                          ambiguousPfos.end());
      ...
      const LArFormattingHelper::Color tagColor(
        isGoodTag ?
          ...
          (isBadTag ? LArFormattingHelper::LIGHT_RED : LArFormattingHelper::LIGHT_YELLOW));
  ...
  LArFormattingHelper::Color CosmicRayTaggingMonitoringTool::GetClassificationColor(
  ...
  std::string CosmicRayTaggingMonitoringTool::GetClassificationName(
    const Classification& classification) const
  {
    switch (classification) {
  ...
  StatusCode CosmicRayTaggingMonitoringTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "CaloHitList2D", m_caloHitList2D));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SelectInputHits", m_parameters.m_selectInputHits));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MinHitSharingFraction",
                                                         m_parameters.m_minHitSharingFraction));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MinPurity", m_minPurity));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MinImpurity", m_minImpurity));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinSignificance", m_minSignificance));
