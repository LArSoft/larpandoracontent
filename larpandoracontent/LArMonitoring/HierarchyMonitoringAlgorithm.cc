...
namespace lar_content {
  ...
  StatusCode HierarchyMonitoringAlgorithm::Run()
  {
    ...
    if (m_match) {
      ...
      if (m_visualizeReco) this->VisualizeReco(recoHierarchy);
    }
  ...
  void HierarchyMonitoringAlgorithm::VisualizeMC(
    const LArHierarchyHelper::MCHierarchy& hierarchy) const
  {
    const std::map<int, const std::string> keys = {
      {13, "mu"}, {11, "e"}, {22, "gamma"}, {321, "kaon"}, {211, "pi"}, {2212, "p"}};
    const std::map<std::string, int> colors = {
      {"mu", 5}, {"e", 2}, {"gamma", 9}, {"kaon", 1}, {"pi", 3}, {"p", 4}, {"other", 14}};
    ...
    for (const LArHierarchyHelper::MCHierarchy::Node* pNode : nodes) {
      ...
      if (keys.find(pdg) != keys.end()) key = keys.at(pdg);
  ...
  void HierarchyMonitoringAlgorithm::VisualizeMCDistinct(
    const LArHierarchyHelper::MCHierarchy& hierarchy) const
  {
    const std::map<int, const std::string> keys = {
      {13, "mu"}, {11, "e"}, {22, "gamma"}, {321, "kaon"}, {211, "pi"}, {2212, "p"}};
  ...
  void HierarchyMonitoringAlgorithm::VisualizeMCProcess(
    const LArHierarchyHelper::MCHierarchy& hierarchy) const
  {
    const std::map<MCProcess, std::string> procToCategoryMap = {
      {MC_PROC_INCIDENT_NU, "invisible"},
      {MC_PROC_UNKNOWN, "invisible"},
      {MC_PROC_PRIMARY, "primary"},
      {MC_PROC_COMPT, "compt"},
      {MC_PROC_PHOT, "phot"},
      {MC_PROC_ANNIHIL, "annihil"},
      {MC_PROC_E_IONI, "ioni"},
      {MC_PROC_E_BREM, "brem"},
      {MC_PROC_CONV, "conv"},
      {MC_PROC_MU_IONI, "ioni"},
      {MC_PROC_MU_MINUS_CAPTURE_AT_REST, "capture"},
      {MC_PROC_NEUTRON_INELASTIC, "inelastic"},
      {MC_PROC_N_CAPTURE, "capture"},
      {MC_PROC_HAD_ELASTIC, "elastic"},
      {MC_PROC_DECAY, "decay"},
      {MC_PROC_COULOMB_SCAT, "coulomb"},
      {MC_PROC_MU_BREM, "brem"},
      {MC_PROC_MU_PAIR_PROD, "pair_prod"},
      {MC_PROC_PHOTON_INELASTIC, "inelastic"},
      {MC_PROC_HAD_IONI, "ioni"},
      {MC_PROC_PROTON_INELASTIC, "inelastic"},
      {MC_PROC_PI_PLUS_INELASTIC, "inelastic"},
      {MC_PROC_CHIPS_NUCLEAR_CAPTURE_AT_REST, "capture"},
      {MC_PROC_PI_MINUS_INELASTIC, "inelastic"},
      {MC_PROC_TRANSPORTATION, "transport"},
      {MC_PROC_RAYLEIGH, "rayleigh"},
      {MC_PROC_HAD_BREM, "brem"},
      {MC_PROC_HAD_PAIR_PROD, "pair_prod"},
      {MC_PROC_ION_IONI, "ioni"},
      {MC_PROC_NEUTRON_KILLER, "kill"},
      {MC_PROC_ION_INELASTIC, "inelastic"},
      {MC_PROC_HE3_INELASTIC, "inelastic"},
      {MC_PROC_ALPHA_INELASTIC, "inelastic"},
      {MC_PROC_ANTI_HE3_INELASTIC, "inelastic"},
      {MC_PROC_ANTI_ALPHA_INELASTIC, "inelastic"},
      {MC_PROC_HAD_FRITIOF_CAPTURE_AT_REST, "inelastic"},
      {MC_PROC_ANTI_DEUTERON_INELASTIC, "inelastic"},
      {MC_PROC_ANTI_NEUTRON_INELASTIC, "inelastic"},
      {MC_PROC_ANTI_PROTON_INELASTIC, "inelastic"},
      {MC_PROC_ANTI_TRITON_INELASTIC, "inelastic"},
      {MC_PROC_DEUTERON_INELASTIC, "inelastic"},
      {MC_PROC_ELECTRON_NUCLEAR, "nuclear"},
      {MC_PROC_PHOTON_NUCLEAR, "nuclear"},
      {MC_PROC_KAON_PLUS_INELASTIC, "inelastic"},
      {MC_PROC_KAON_MINUS_INELASTIC, "inelastic"},
      {MC_PROC_HAD_BERTINI_CAPTURE_AT_REST, "capture"},
      {MC_PROC_LAMBDA_INELASTIC, "inelastic"},
      {MC_PROC_MU_NUCLEAR, "nuclear"},
      {MC_PROC_TRITON_INELASTIC, "inelastic"},
      {MC_PROC_PRIMARY_BACKGROUND, "background"}};

    const std::map<std::string, int> categoryToColorMap = {{"invisible", 0},
                                                           {"primary", 1},
                                                           {"compt", 2},
                                                           {"phot", 3},
                                                           {"annihil", 4},
    ...
    for (const LArHierarchyHelper::MCHierarchy::Node* pNode : nodes) {
      ...
      CaloHitList uHits, vHits, wHits;
      this->FillHitLists(pNode->GetCaloHits(), uHits, vHits, wHits);
      std::string suffix{std::to_string(nodeIdx) + " (" + std::to_string(tier) + ") " +
                         std::to_string(pdg) + " " + category + " " + std::to_string(process)};
  ...
  void HierarchyMonitoringAlgorithm::VisualizeReco(
    const LArHierarchyHelper::RecoHierarchy& hierarchy) const
  {
    const int nColors{7};
  ...
  void HierarchyMonitoringAlgorithm::VisualizeMatches(
    const LArHierarchyHelper::MatchInfo& matchInfo) const
  {
    ...
    for (const LArHierarchyHelper::MCMatches& matches : matchInfo.GetMatches())
      if (mcIdxMap.find(matches.GetMC()) == mcIdxMap.end()) mcIdxMap[matches.GetMC()] = mcIdx++;
  ...
  void HierarchyMonitoringAlgorithm::VisualizeMatchedMC(
    const LArHierarchyHelper::MCMatches& matches,
    const int mcIdx) const
  {
    const std::map<int, const std::string> keys = {
      {13, "mu"}, {11, "e"}, {22, "gamma"}, {321, "kaon"}, {211, "pi"}, {2212, "p"}};
    ...
    for (const LArHierarchyHelper::RecoHierarchy::Node* pRecoNode : matches.GetRecoMatches()) {
      ...
      const std::string recoKey{
        characterisation == MU_MINUS ? "T" : characterisation == E_MINUS ? "S" : "?"};
      const std::string recoSuffix{std::to_string(mcIdx) + "->" + std::to_string(recoIdx) + "_" +
                                   recoKey};
      if (!recoUHits.empty()) {
  ...
  void HierarchyMonitoringAlgorithm::VisualizeUnmatchedReco(
    const LArHierarchyHelper::RecoHierarchy::Node* pNode) const
  {
    const int gray{14};
  ...
  void HierarchyMonitoringAlgorithm::Visualize(const CaloHitList& hits,
                                               const std::string& label,
                                               const int color) const
  {
    if (!hits.empty() && hits.size() > 1) {
      if (m_collectionOnly && hits.front()->GetHitType() != TPC_VIEW_W) return;
      PANDORA_MONITORING_API(
        VisualizeCaloHits(this->GetPandora(), &hits, label, static_cast<Color>(color)));
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void HierarchyMonitoringAlgorithm::FillHitLists(const CaloHitList& hits,
                                                  CaloHitList& uHits,
                                                  CaloHitList& vHits,
                                                  CaloHitList& wHits) const
  {
  ...
  StatusCode HierarchyMonitoringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "VisualizeMC", m_visualizeMC));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VisualizeReco", m_visualizeReco));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VisualizeDistinct", m_visualizeDistinct));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VisualizeProcess", m_visualizeProcess));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "PerformMatching", m_match));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "CollectionOnly", m_collectionOnly));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
    ...
    if (m_foldToPrimaries) m_foldDynamic = false;
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "TransparencyThresholdE", m_transparencyThresholdE));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "EnergyScaleThresholdE", m_energyScaleThresholdE));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ScalingFactor", m_scalingFactor));
