...
namespace lar_content {
  ...
  StatusCode VisualParticleMonitoringAlgorithm::Run()
  {
    ...
    if (m_visualizePfo) {
      ...
      if (m_showPfoByPid) { this->VisualizePfoByParticleId(*pPfoList); }
      else {
  ...
  void VisualParticleMonitoringAlgorithm::VisualizeIndependentMC(
    const LArMCParticleHelper::MCContributionMap& mcMap) const
  {
    const std::map<int, const std::string> keys = {
      {13, "mu"}, {11, "e"}, {22, "gamma"}, {321, "kaon"}, {211, "pi"}, {2212, "p"}};
    const std::map<int, Color> colors = {
      {0, RED}, {1, BLACK}, {2, BLUE}, {3, CYAN}, {4, MAGENTA}, {5, GREEN}, {6, ORANGE}, {7, GRAY}};
    MCParticleList linearisedMC;
    if (mcMap.empty()) return;
    ...
    for (const MCParticle* pMC : linearisedMC) {
      ...
      if (!uHits.empty())
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &uHits, "u_" + suffix, colors.at(colorIdx)));
      if (!vHits.empty())
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &vHits, "v_" + suffix, colors.at(colorIdx)));
      if (!wHits.empty())
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &wHits, "w_" + suffix, colors.at(colorIdx)));
  ...
  void VisualParticleMonitoringAlgorithm::VisualizeMCByPdgCode(
    const LArMCParticleHelper::MCContributionMap& mcMap) const
  {
    const std::map<int, const std::string> keys = {
      {13, "mu"}, {11, "e"}, {22, "gamma"}, {321, "kaon"}, {211, "pi"}, {2212, "p"}};
    const std::map<std::string, Color> colors = {{"mu", MAGENTA},
                                                 {"e", RED},
                                                 {"gamma", ORANGE},
                                                 {"kaon", BLACK},
                                                 {"pi", GREEN},
    ...
    for (const auto& [key, value] : keys) {
      ...
      if (!uHits[value].empty())
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &uHits[value], "u_" + value, colors.at(value)));
    }
    if (!uHits["other"].empty())
      PANDORA_MONITORING_API(
        VisualizeCaloHits(this->GetPandora(), &uHits["other"], "u_other", colors.at("other")));

    for (const auto& [key, value] : keys) {
      (void)key; // GCC 7 support, 8+ doesn't need this
      if (!vHits[value].empty())
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &vHits[value], "v_" + value, colors.at(value)));
    }
    if (!vHits["other"].empty())
      PANDORA_MONITORING_API(
        VisualizeCaloHits(this->GetPandora(), &uHits["other"], "v_other", colors.at("other")));

    for (const auto& [key, value] : keys) {
      (void)key; // GCC 7 support, 8+ doesn't need this
      if (!wHits[value].empty())
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &wHits[value], "w_" + value, colors.at(value)));
    }
    if (!wHits["other"].empty())
      PANDORA_MONITORING_API(
        VisualizeCaloHits(this->GetPandora(), &uHits["other"], "w_other", colors.at("other")));
  ...
  void VisualParticleMonitoringAlgorithm::VisualizeIndependentPfo(
            ...
            if (iter != mcMap.end()) {
              ...
              if (!uHitsMC.empty())
                PANDORA_MONITORING_API(VisualizeCaloHits(
                  this->GetPandora(), &uHitsMC, "u_" + mcSuffix, colors.at(colorIdx)));
              if (!vHitsMC.empty())
                PANDORA_MONITORING_API(VisualizeCaloHits(
                  this->GetPandora(), &vHitsMC, "v_" + mcSuffix, colors.at(colorIdx)));
              if (!wHitsMC.empty())
                PANDORA_MONITORING_API(VisualizeCaloHits(
                  this->GetPandora(), &wHitsMC, "w_" + mcSuffix, colors.at(colorIdx)));
            }
          }
        }
        catch (const StatusCodeException&) { // No matched MC, move on
        }
      }
      ++pfoIdx;
    }
  ...
  void VisualParticleMonitoringAlgorithm::VisualizePfoByParticleId(const PfoList& pfoList) const
  {
    ...
    for (const ParticleFlowObject* pPfo : linearisedPfo) {
      ...
      if (isTrack) {
        ...
        if (!uTrackHits.empty())
          PANDORA_MONITORING_API(
            VisualizeCaloHits(this->GetPandora(), &uTrackHits, "u_" + suffix, BLUE));
        if (!vTrackHits.empty())
          PANDORA_MONITORING_API(
            VisualizeCaloHits(this->GetPandora(), &vTrackHits, "v_" + suffix, BLUE));
        if (!wTrackHits.empty())
          PANDORA_MONITORING_API(
            VisualizeCaloHits(this->GetPandora(), &wTrackHits, "w_" + suffix, BLUE));
      }
      else {
        std::string suffix{std::to_string(pfoIdx) + "_S"};
        if (!uShowerHits.empty())
          PANDORA_MONITORING_API(
            VisualizeCaloHits(this->GetPandora(), &uShowerHits, "u_" + suffix, RED));
        if (!vShowerHits.empty())
          PANDORA_MONITORING_API(
            VisualizeCaloHits(this->GetPandora(), &vShowerHits, "v_" + suffix, RED));
        if (!wShowerHits.empty())
          PANDORA_MONITORING_API(
            VisualizeCaloHits(this->GetPandora(), &wShowerHits, "w_" + suffix, RED));
      }
      ++pfoIdx;
    }
    ...
    LArMCParticleHelper::MCContributionMap& mcMap) const
  {
    ...
    if (!m_isTestBeam) {
      LArMCParticleHelper::SelectReconstructableMCParticles(
        pMCList, pCaloHitList, parameters, LArMCParticleHelper::IsBeamNeutrinoFinalState, mcMap);
      LArMCParticleHelper::SelectReconstructableMCParticles(
        pMCList, pCaloHitList, parameters, LArMCParticleHelper::IsCosmicRay, mcMap);
    }
    else {
      LArMCParticleHelper::SelectReconstructableMCParticles(
        pMCList, pCaloHitList, parameters, LArMCParticleHelper::IsBeamParticle, mcMap);
      LArMCParticleHelper::SelectReconstructableMCParticles(
        pMCList, pCaloHitList, parameters, LArMCParticleHelper::IsCosmicRay, mcMap);
    }
  }
  ...
  StatusCode VisualParticleMonitoringAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VisualizePFO", m_visualizePfo));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "GroupMCByPDG", m_groupMCByPdg));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ShowPFOByPID", m_showPfoByPid));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ShowPFOMatchedMC", m_showPfoMatchedMC));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
