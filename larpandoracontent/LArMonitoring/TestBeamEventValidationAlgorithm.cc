...
namespace lar_content {
  ...
  void TestBeamEventValidationAlgorithm::FillValidationInfo(
    ...
    ValidationInfo& validationInfo) const
  {
    ...
    if (pPfoList) {
      ...
      for (const ParticleFlowObject* const pPfo : allConnectedPfos) {
        if (pPfo->GetParentPfoList().empty()) finalStatePfos.push_back(pPfo);
      }
  ...
  void TestBeamEventValidationAlgorithm::ProcessOutput(const ValidationInfo& validationInfo,
                                                       ...
                                                       const bool fillTree) const
  {
    ...
    int nCorrectTB(0), nTotalTB(0), nCorrectCR(0), nTotalCR(0), nFakeTB(0), nFakeCR(0), nSplitTB(0),
      nSplitCR(0), nLost(0);
    ...
    for (const MCParticle* const pMCPrimary : mcPrimaryVector) {
      ...
      targetSS << (!isTargetPrimary ? "(Non target) " : "") << "PrimaryId " << mcPrimaryIndex
               << ", TB " << isBeamParticle << ", CR " << isCosmicRay << ", MCPDG "
      ...
      for (const LArMCParticleHelper::PfoCaloHitListPair& pfoToSharedHits :
        ...
        if (0 == matchIndex++) {
          ...
          try {
            const Vertex* const pRecoVertex(
              isRecoTestBeam ? LArPfoHelper::GetTestBeamInteractionVertex(pfoToSharedHits.first) :
        ...
        targetSS << "-" << (!isGoodMatch ? "(Below threshold) " : "") << "MatchedPfoId " << pfoId
                 << ", TB " << isRecoTestBeam << ", CR " << (!isRecoTestBeam) << ", PDG "
      ...
      if (fillTree) {
        ...
        PANDORA_MONITORING_API(SetTreeVariable(
          this->GetPandora(), m_treeName.c_str(), "nPrimaryMatchedTBPfos", &nPrimaryMatchedTBPfos));
        PANDORA_MONITORING_API(SetTreeVariable(
        ...
        PANDORA_MONITORING_API(SetTreeVariable(
          this->GetPandora(), m_treeName.c_str(), "nTargetTBMatches", nTargetTBMatches));
        PANDORA_MONITORING_API(SetTreeVariable(
          this->GetPandora(), m_treeName.c_str(), "nTargetCRMatches", nTargetCRMatches));
        PANDORA_MONITORING_API(SetTreeVariable(
          this->GetPandora(), m_treeName.c_str(), "bestMatchPfoIsTB", &bestMatchPfoIsTB));
      }
      ...
      if (isBeamParticle || isCosmicRay) {
        ...
        outcomeSS << LArInteractionTypeHelper::ToString(interactionType) << " (Nuance "
                  << mcNuanceCode << ", TB " << isBeamParticle << ", CR " << isCosmicRay << ")"
                  << std::endl;
        ...
        if (fillTree) {
          ...
          PANDORA_MONITORING_API(
            SetTreeVariable(this->GetPandora(), m_treeName.c_str(), "isCorrectTB", isCorrectTB));
          PANDORA_MONITORING_API(
            SetTreeVariable(this->GetPandora(), m_treeName.c_str(), "isCorrectCR", isCorrectCR));
          PANDORA_MONITORING_API(
            SetTreeVariable(this->GetPandora(), m_treeName.c_str(), "isFakeTB", isFakeTB));
          PANDORA_MONITORING_API(
            SetTreeVariable(this->GetPandora(), m_treeName.c_str(), "isFakeCR", isFakeCR));
          PANDORA_MONITORING_API(
            SetTreeVariable(this->GetPandora(), m_treeName.c_str(), "isSplitTB", isSplitTB));
          PANDORA_MONITORING_API(
    ...
    if (useInterpretedMatching) {
      ...
      summarySS << "---SUMMARY---------------------------------------------------------------------"
                ...
                << std::endl;
      if (nTotalTB > 0)
        summarySS << "#CorrectTB: " << nCorrectTB << "/" << nTotalTB << ", Fraction: "
                  << (nTotalTB > 0 ? static_cast<float>(nCorrectTB) / static_cast<float>(nTotalTB) :
                                     0.f)
                  << std::endl;
      if (nTotalCR > 0)
