...
namespace lar_content {
  ...
  void CosmicRayTaggingTool::FindAmbiguousPfos(const PfoList& parentCosmicRayPfos,
                                               ...
                                               const MasterAlgorithm* const /*pAlgorithm*/)
  {
    ...
    this->TagCRMuons(
      candidates, pfoToInTimeMap, pfoToIsTopToBottomMap, neutrinoSliceSet, pfoToIsLikelyCRMuonMap);

    for (const ParticleFlowObject* const pPfo : parentCosmicRayPfos) {
      if (!pfoToIsLikelyCRMuonMap.at(pPfo)) ambiguousPfos.push_back(pPfo);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool CosmicRayTaggingTool::GetValid3DCluster(const ParticleFlowObject* const pPfo,
                                               const Cluster*& pCluster3D) const
  {
    pCluster3D = nullptr;
    ...
    if (clusters3D.empty() || (clusters3D.front()->GetNCaloHits() < m_minimumHits)) return false;
  ...
  void CosmicRayTaggingTool::GetPfoAssociations(const PfoList& parentCosmicRayPfos,
                                                PfoToPfoListMap& pfoAssociationMap) const
  {
    // ATTN If wire w pitches vary between TPCs, exception will be raised in initialisation of lar pseudolayer plugin
    const LArTPC* const pFirstLArTPC(
      this->GetPandora().GetGeometry()->GetLArTPCMap().begin()->second);
    ...
    for (const ParticleFlowObject* const pPfo : parentCosmicRayPfos) {
      ...
      if (!this->GetValid3DCluster(pPfo, pCluster) || !pCluster) continue;

      (void)pfoToSlidingFitsMap.insert(PfoToSlidingFitsMap::value_type(
        pPfo,
        std::make_pair(ThreeDSlidingFitResult(pCluster, 5, layerPitch),
                       ThreeDSlidingFitResult(pCluster, 100, layerPitch)))); // TODO Configurable
    }
    ...
    for (const ParticleFlowObject* const pPfo1 : parentCosmicRayPfos) {
      ...
      if (pfoToSlidingFitsMap.end() == iter1) continue;
      ...
      for (const ParticleFlowObject* const pPfo2 : parentCosmicRayPfos) {
        ...
        if (!(this->CheckAssociation(fitPos1.GetGlobalMinLayerPosition(),
                                     fitDir1.GetGlobalMinLayerDirection() * -1.f,
                                     fitPos2.GetGlobalMinLayerPosition(),
                                     fitDir2.GetGlobalMinLayerDirection() * -1.f) ||
              this->CheckAssociation(fitPos1.GetGlobalMinLayerPosition(),
                                     fitDir1.GetGlobalMinLayerDirection() * -1.f,
                                     fitPos2.GetGlobalMaxLayerPosition(),
                                     fitDir2.GetGlobalMaxLayerDirection()) ||
              this->CheckAssociation(fitPos1.GetGlobalMaxLayerPosition(),
                                     fitDir1.GetGlobalMaxLayerDirection(),
                                     fitPos2.GetGlobalMinLayerPosition(),
                                     fitDir2.GetGlobalMinLayerDirection() * -1.f) ||
              this->CheckAssociation(fitPos1.GetGlobalMaxLayerPosition(),
                                     fitDir1.GetGlobalMaxLayerDirection(),
  ...
  bool CosmicRayTaggingTool::CheckAssociation(const CartesianVector& endPoint1,
                                              ...
                                              const CartesianVector& endDir2) const
  {
    ...
    if (std::fabs(b - 1.f) < std::numeric_limits<float>::epsilon()) return false;
    ...
    const float maxVertexUncertainty(m_maxAssociationDist * std::sin(deltaTheta) +
    ...
    if ((lambda < -maxVertexUncertainty) || (mu < -maxVertexUncertainty) ||
        (lambda > m_maxAssociationDist + maxVertexUncertainty) ||
    ...
    if ((impactPosition.GetX() < m_face_Xa - maxVertexUncertainty) ||
        (impactPosition.GetX() > m_face_Xc + maxVertexUncertainty) ||
        (impactPosition.GetY() < m_face_Yb - maxVertexUncertainty) ||
        (impactPosition.GetY() > m_face_Yt + maxVertexUncertainty) ||
        (impactPosition.GetZ() < m_face_Zu - maxVertexUncertainty) ||
        (impactPosition.GetZ() > m_face_Zd + maxVertexUncertainty)) {
      return false;
    }

    // Compare distance of closest approach and max uncertainty in impact parameter
    const float maxImpactDist(std::sin(deltaTheta) * (std::fabs(mu) + std::fabs(lambda)) +
                              m_positionalUncertainty);
  ...
  void CosmicRayTaggingTool::SliceEvent(const PfoList& parentCosmicRayPfos,
                                        const PfoToPfoListMap& pfoAssociationMap,
                                        PfoToSliceIdMap& pfoToSliceIdMap) const
  {
    SliceList sliceList;
  ...
  void CosmicRayTaggingTool::FillSlice(const ParticleFlowObject* const pPfo,
                                       const PfoToPfoListMap& pfoAssociationMap,
                                       PfoList& slice) const
  {
    if (std::find(slice.begin(), slice.end(), pPfo) != slice.end()) return;
  ...
  void CosmicRayTaggingTool::GetCRCandidates(const PfoList& parentCosmicRayPfos,
                                             const PfoToSliceIdMap& pfoToSliceIdMap,
                                             CRCandidateList& candidates) const
  {
    for (const ParticleFlowObject* const pPfo : parentCosmicRayPfos) {
  ...
  void CosmicRayTaggingTool::CheckIfInTime(const CRCandidateList& candidates,
                                           PfoToBoolMap& pfoToInTimeMap) const
  {
    const LArTPCMap& larTPCMap(this->GetPandora().GetGeometry()->GetLArTPCMap());
    ...
    for (const CRCandidate& candidate : candidates) {
      ...
      if (candidate.m_canFit) {
        minX = ((candidate.m_endPoint1.GetX() < candidate.m_endPoint2.GetX()) ?
                  candidate.m_endPoint1.GetX() :
                  candidate.m_endPoint2.GetX());
        maxX = ((candidate.m_endPoint1.GetX() > candidate.m_endPoint2.GetX()) ?
                  candidate.m_endPoint1.GetX() :
                  candidate.m_endPoint2.GetX());
      }
      else {
        ...
        for (const Cluster* const pCluster : candidate.m_pPfo->GetClusterList()) {
          float clusterMinX(std::numeric_limits<float>::max()),
            clusterMaxX(-std::numeric_limits<float>::max());
      ...
      if (isInTime) {
        ...
        if (isInSingleVolume && (larTPCMap.end() != tpcIter)) {
          const float thisFaceXLow(tpcIter->second->GetCenterX() -
                                   0.5f * tpcIter->second->GetWidthX());
          const float thisFaceXHigh(tpcIter->second->GetCenterX() +
                                    0.5f * tpcIter->second->GetWidthX());
  ...
  void CosmicRayTaggingTool::CheckIfContained(const CRCandidateList& candidates,
                                              PfoToBoolMap& pfoToIsContainedMap) const
  {
    for (const CRCandidate& candidate : candidates) {
      const float upperY((candidate.m_endPoint1.GetY() > candidate.m_endPoint2.GetY()) ?
                           candidate.m_endPoint1.GetY() :
                           candidate.m_endPoint2.GetY());
      const float lowerY((candidate.m_endPoint1.GetY() < candidate.m_endPoint2.GetY()) ?
                           candidate.m_endPoint1.GetY() :
                           candidate.m_endPoint2.GetY());

      const float zAtUpperY((candidate.m_endPoint1.GetY() > candidate.m_endPoint2.GetY()) ?
                              candidate.m_endPoint1.GetZ() :
                              candidate.m_endPoint2.GetZ());
      const float zAtLowerY((candidate.m_endPoint1.GetY() < candidate.m_endPoint2.GetY()) ?
                              candidate.m_endPoint1.GetZ() :
                              candidate.m_endPoint2.GetZ());

      const bool isContained(
        (upperY < m_face_Yt - m_marginY) && (upperY > m_face_Yb + m_marginY) &&
      ...
      if (!pfoToIsContainedMap.insert(PfoToBoolMap::value_type(candidate.m_pPfo, isContained))
             .second)
        throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
    }
  }
  ...
  void CosmicRayTaggingTool::CheckIfTopToBottom(const CRCandidateList& candidates,
                                                PfoToBoolMap& pfoToIsTopToBottomMap) const
  {
    for (const CRCandidate& candidate : candidates) {
      const float upperY((candidate.m_endPoint1.GetY() > candidate.m_endPoint2.GetY()) ?
                           candidate.m_endPoint1.GetY() :
      ...
      const bool isTopToBottom((upperY > m_face_Yt - m_marginY) &&
                               (lowerY < m_face_Yb + m_marginY));

      if (!pfoToIsTopToBottomMap.insert(PfoToBoolMap::value_type(candidate.m_pPfo, isTopToBottom))
             .second)
        throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
    }
  }
  ...
  void CosmicRayTaggingTool::GetNeutrinoSlices(const CRCandidateList& candidates,
                                               ...
                                               UIntSet& neutrinoSliceSet) const
  {
    ...
    for (const CRCandidate& candidate : candidates) {
      if (neutrinoSliceSet.count(candidate.m_sliceId)) continue;

      const bool likelyNeutrino(
        candidate.m_canFit && sliceIdToIsInTimeMap.at(candidate.m_sliceId) &&
        (candidate.m_theta < m_maxNeutrinoCosTheta || pfoToIsContainedMap.at(candidate.m_pPfo)));

      if (likelyNeutrino) (void)neutrinoSliceSet.insert(candidate.m_sliceId);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void CosmicRayTaggingTool::TagCRMuons(const CRCandidateList& candidates,
                                        const PfoToBoolMap& pfoToInTimeMap,
                                        const PfoToBoolMap& pfoToIsTopToBottomMap,
                                        const UIntSet& neutrinoSliceSet,
                                        PfoToBoolMap& pfoToIsLikelyCRMuonMap) const
  {
    for (const CRCandidate& candidate : candidates) {
      const bool likelyCRMuon(
        ...
        (!pfoToInTimeMap.at(candidate.m_pPfo) ||
         (candidate.m_canFit && (pfoToIsTopToBottomMap.at(candidate.m_pPfo) ||
                                 ((candidate.m_theta > m_minCosmicCosTheta) &&
                                  (candidate.m_curvature < m_maxCosmicCurvature))))));

      if (!pfoToIsLikelyCRMuonMap.insert(PfoToBoolMap::value_type(candidate.m_pPfo, likelyCRMuon))
             .second)
        throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
    }
  }
  ...
  CosmicRayTaggingTool::CRCandidate::CRCandidate(const Pandora& pandora,
                                                 const ParticleFlowObject* const pPfo,
                                                 const unsigned int sliceId)
    : m_pPfo(pPfo)
    , m_sliceId(sliceId)
    , m_canFit(false)
    , m_endPoint1(std::numeric_limits<float>::max(),
                  std::numeric_limits<float>::max(),
                  std::numeric_limits<float>::max())
    , m_endPoint2(std::numeric_limits<float>::max(),
                  std::numeric_limits<float>::max(),
                  std::numeric_limits<float>::max())
    ...
    , m_theta(std::numeric_limits<float>::max())
  {
    ...
    if (!clusters3D.empty() && (clusters3D.front()->GetNCaloHits() > 15)) // TODO Configurable
    {
      ...
      const ThreeDSlidingFitResult slidingFitResult(
        clusters3D.front(), 5, pFirstLArTPC->GetWirePitchW()); // TODO Configurable
  ...
  void CosmicRayTaggingTool::CRCandidate::CalculateFitVariables(
    const ThreeDSlidingFitResult& slidingFitResult)
  {
    ...
    for (int i = slidingFitResult.GetMinLayer(); i < slidingFitResult.GetMaxLayer(); ++i) {
      ...
      if (STATUS_CODE_SUCCESS ==
          slidingFitResult.GetGlobalFitDirection(static_cast<float>(i) * layerPitch, direction))
    ...
    if (!directionList.empty() > 0) meanDirection *= 1.f / static_cast<float>(directionList.size());

    m_curvature = 0.f;
    for (const CartesianVector& direction : directionList)
      m_curvature += (direction - meanDirection).GetMagnitude();

    if (!directionList.empty() > 0) m_curvature *= 1.f / static_cast<float>(directionList.size());
  }
  ...
  StatusCode CosmicRayTaggingTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "CutMode", m_cutMode));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "AngularUncertainty", m_angularUncertainty));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "PositionalUncertainty", m_positionalUncertainty));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxAssociationDist", m_maxAssociationDist));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "HitThreshold", m_minimumHits));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "InTimeMargin", m_inTimeMargin));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MarginY", m_marginY));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MarginZ", m_marginZ));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxNeutrinoCosTheta", m_maxNeutrinoCosTheta));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinCosmicCosTheta", m_minCosmicCosTheta));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxCosmicCurvature", m_maxCosmicCurvature));
