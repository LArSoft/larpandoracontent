...
namespace lar_content {
  ...
  void StitchingCosmicRayMergingTool::Run(const MasterAlgorithm* const pAlgorithm,
                                          const PfoList* const pMultiPfoList,
                                          ...
                                          PfoToFloatMap& stitchedPfosToX0Map)
  {
    ...
    if (this->GetPandora().GetGeometry()->GetLArTPCMap().size() < 2) return;

    if (pfoToLArTPCMap.empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
    ...
    this->StitchPfos(
      pAlgorithm, pointingClusterMap, pfoOrderedMerges, pfoToLArTPCMap, stitchedPfosToX0Map);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void StitchingCosmicRayMergingTool::SelectPrimaryPfos(const PfoList* pInputPfoList,
                                                        const PfoToLArTPCMap& pfoToLArTPCMap,
                                                        PfoList& outputPfoList) const
  {
    for (const ParticleFlowObject* const pPfo : *pInputPfoList) {
      if (!LArPfoHelper::IsFinalState(pPfo) || !LArPfoHelper::IsTrack(pPfo)) continue;

      if (!pfoToLArTPCMap.count(pPfo)) continue;
  ...
  void StitchingCosmicRayMergingTool::BuildPointingClusterMaps(
    const PfoList& inputPfoList,
    const PfoToLArTPCMap& pfoToLArTPCMap,
    ThreeDPointingClusterMap& pointingClusterMap) const
  {
    for (const ParticleFlowObject* const pPfo : inputPfoList) {
      try {
        PfoToLArTPCMap::const_iterator tpcIter(pfoToLArTPCMap.find(pPfo));

        if (pfoToLArTPCMap.end() == tpcIter) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
        ...
        if (1 != clusterList.size()) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

        const ThreeDSlidingFitResult slidingFitResult(
          clusterList.front(), m_halfWindowLayers, slidingFitPitch);
        (void)pointingClusterMap.insert(
          ThreeDPointingClusterMap::value_type(pPfo, LArPointingCluster(slidingFitResult)));
      }
  ...
  void StitchingCosmicRayMergingTool::BuildTPCMaps(const PfoList& inputPfoList,
                                                   const PfoToLArTPCMap& pfoToLArTPCMap,
                                                   LArTPCToPfoMap& larTPCToPfoMap) const
  {
    for (const ParticleFlowObject* const pPfo : inputPfoList) {
      PfoToLArTPCMap::const_iterator iter(pfoToLArTPCMap.find(pPfo));

      if (pfoToLArTPCMap.end() != iter) larTPCToPfoMap[iter->second].push_back(pPfo);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void StitchingCosmicRayMergingTool::CreatePfoMatches(
    ...
    PfoAssociationMatrix& pfoAssociationMatrix) const
  {
    ...
    for (LArTPCVector::const_iterator tpcIter1 = larTPCVector.begin(),
                                      tpcIterEnd = larTPCVector.end();
         tpcIter1 != tpcIterEnd;
         ++tpcIter1) {
      ...
      for (LArTPCVector::const_iterator tpcIter2 = tpcIter1; tpcIter2 != tpcIterEnd; ++tpcIter2) {
        ...
        if (!LArStitchingHelper::CanTPCsBeStitched(*pLArTPC1, *pLArTPC2)) continue;

        for (const ParticleFlowObject* const pPfo1 : pfoList1) {
          for (const ParticleFlowObject* const pPfo2 : pfoList2)
            this->CreatePfoMatches(
              *pLArTPC1, *pLArTPC2, pPfo1, pPfo2, pointingClusterMap, pfoAssociationMatrix);
        }
      }
    }
  }
  ...
  void StitchingCosmicRayMergingTool::CreatePfoMatches(
    ...
    const ParticleFlowObject* const pPfo1,
    const ParticleFlowObject* const pPfo2,
    const ThreeDPointingClusterMap& pointingClusterMap,
    PfoAssociationMatrix& pfoAssociationMatrix) const
  {
    ...
    if (pointingClusterMap.end() == iter1 || pointingClusterMap.end() == iter2) return;
    ...
    if (pointingCluster1.GetLengthSquared() < m_minLengthSquared ||
        pointingCluster2.GetLengthSquared() < m_minLengthSquared)
      return;
    ...
    try {
      LArStitchingHelper::GetClosestVertices(
        larTPC1, larTPC2, pointingCluster1, pointingCluster2, pointingVertex1, pointingVertex2);
    }
    ...
    const float cosRelativeAngle(
      -pointingVertex1.GetDirection().GetDotProduct(pointingVertex2.GetDirection()));

    if (cosRelativeAngle < m_relaxCosRelativeAngle) return;
    ...
    const float intersectX(
      0.5 * (pointingVertex1.GetPosition().GetX() + pointingVertex2.GetPosition().GetX()));

    if (std::fabs(intersectX - boundaryCenterX) > maxLongitudinalDisplacementX) return;
    ...
    try {
      if (m_useXcoordinate) {
        ...
        LArPointingClusterHelper::GetImpactParametersInYZ(
          pointingVertex1, pointingVertex2, rL1, rT1);
        LArPointingClusterHelper::GetImpactParametersInYZ(
          pointingVertex2, pointingVertex1, rL2, rT2);
      }
    }
    catch (const pandora::StatusCodeException&) {
      return;
    ...
    const float minL(
      (!LArGeometryHelper::IsInGap(this->GetPandora(), pointingVertex1.GetPosition(), TPC_3D) ||
    ...
    const float dXdL1(m_useXcoordinate ? pX1 :
                                         (1.f - pX1 * pX1 > std::numeric_limits<float>::epsilon()) ?
                                         pX1 / std::sqrt(1.f - pX1 * pX1) :
                                         minL);
    const float dXdL2(m_useXcoordinate ? pX2 :
                                         (1.f - pX2 * pX2 > std::numeric_limits<float>::epsilon()) ?
                                         pX2 / std::sqrt(1.f - pX2 * pX2) :
                                         minL);
    const float maxL1(maxLongitudinalDisplacementX / dXdL1);
    const float maxL2(maxLongitudinalDisplacementX / dXdL2);

    if (rL1 < minL || rL1 > maxL1 || rL2 < minL || rL2 > maxL2) return;

    // Selection cuts on transverse impact parameters
    const bool minPass(std::min(rT1, rT2) < m_relaxTransverseDisplacement &&
                       cosRelativeAngle > m_relaxCosRelativeAngle);
    const bool maxPass(std::max(rT1, rT2) < m_maxTransverseDisplacement &&
                       cosRelativeAngle > m_minCosRelativeAngle);

    if (!minPass && !maxPass) return;

    // Store this association
    const PfoAssociation::VertexType vertexType1(
      pointingVertex1.IsInnerVertex() ? PfoAssociation::INNER : PfoAssociation::OUTER);
    const PfoAssociation::VertexType vertexType2(
      pointingVertex2.IsInnerVertex() ? PfoAssociation::INNER : PfoAssociation::OUTER);
    ...
    pfoAssociationMatrix[pPfo1].insert(PfoAssociationMap::value_type(
      pPfo2, PfoAssociation(vertexType1, vertexType2, particleLength2)));
    pfoAssociationMatrix[pPfo2].insert(PfoAssociationMap::value_type(
      pPfo1, PfoAssociation(vertexType2, vertexType1, particleLength1)));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void StitchingCosmicRayMergingTool::SelectPfoMatches(
    const PfoAssociationMatrix& pfoAssociationMatrix,
    PfoMergeMap& pfoMatches) const
  {
    ...
    for (const ParticleFlowObject* const pPfo1 : pfoVector1) {
      ...
      PfoAssociation bestAssociationInner(
        PfoAssociation::UNDEFINED, PfoAssociation::UNDEFINED, 0.f);

      const ParticleFlowObject* pBestPfoOuter(nullptr);
      PfoAssociation bestAssociationOuter(
        PfoAssociation::UNDEFINED, PfoAssociation::UNDEFINED, 0.f);
      ...
      if (pBestPfoInner)
        (void)bestAssociationMatrix[pPfo1].insert(
          PfoAssociationMap::value_type(pBestPfoInner, bestAssociationInner));

      if (pBestPfoOuter)
        (void)bestAssociationMatrix[pPfo1].insert(
          PfoAssociationMap::value_type(pBestPfoOuter, bestAssociationOuter));
    }
    ...
    for (const ParticleFlowObject* const pParentPfo : pfoVector3) {
      ...
      for (const ParticleFlowObject* const pDaughterPfo : pfoVector4) {
        ...
        if (bestAssociationMatrix.end() == iter5) continue;
        ...
        if (daughterAssociationMap.end() == iter6) continue;
  ...
  void StitchingCosmicRayMergingTool::SelectPfoMerges(const PfoMergeMap& pfoMatches,
                                                      PfoMergeMap& pfoMerges) const
  {
    PfoSet vetoSet;
    ...
    for (const ParticleFlowObject* const pInputPfo : inputPfoVector) {
      ...
      for (const ParticleFlowObject* const pSeedPfo : pfoList) {
        if (vetoSet.count(pSeedPfo)) continue;
        ...
        for (const ParticleFlowObject* const pAssociatedPfo : mergeList) {
          ...
          if (vetoSet.count(pAssociatedPfo) ||
              (selectedPfoList.end() !=
               std::find(selectedPfoList.begin(), selectedPfoList.end(), pAssociatedPfo)))
  ...
  void StitchingCosmicRayMergingTool::CollectAssociatedPfos(
    const ParticleFlowObject* const pSeedPfo,
    const ParticleFlowObject* const pCurrentPfo,
    const PfoMergeMap& pfoMergeMap,
    const PfoSet& vetoSet,
    PfoList& associatedList) const
  {
    if (vetoSet.count(pCurrentPfo)) return;

    PfoMergeMap::const_iterator iter1 = pfoMergeMap.find(pCurrentPfo);

    if (pfoMergeMap.end() == iter1) return;

    for (PfoList::const_iterator iter2 = iter1->second.begin(), iterEnd2 = iter1->second.end();
         iter2 != iterEnd2;
         ++iter2) {
      const ParticleFlowObject* const pAssociatedPfo = *iter2;

      if (pAssociatedPfo == pSeedPfo) continue;

      if (associatedList.end() !=
          std::find(associatedList.begin(), associatedList.end(), pAssociatedPfo))
        continue;
  ...
  void StitchingCosmicRayMergingTool::OrderPfoMerges(
    const PfoToLArTPCMap& pfoToLArTPCMap,
    const ThreeDPointingClusterMap& pointingClusterMap,
    const PfoMergeMap& inputPfoMerges,
    PfoMergeMap& outputPfoMerges) const
  {
    PfoVector inputPfoVector;
    ...
    for (const ParticleFlowObject* const pInputPfo : inputPfoVector) {
      ...
      for (PfoList::const_iterator iter1 = pfoList.begin(), iterEnd = pfoList.end();
           iter1 != iterEnd;
           ++iter1) {
        ...
        for (PfoList::const_iterator iter2 = iter1; iter2 != iterEnd; ++iter2) {
          ...
          try {
            ...
            LArStitchingHelper::GetClosestVertices(
              *pLArTPC1, *pLArTPC2, pointingCluster1, pointingCluster2, nearVertex1, nearVertex2);

            const LArPointingCluster::Vertex& farVertex1(nearVertex1.IsInnerVertex() ?
                                                           pointingCluster1.GetOuterVertex() :
                                                           pointingCluster1.GetInnerVertex());
            const LArPointingCluster::Vertex& farVertex2(nearVertex2.IsInnerVertex() ?
                                                           pointingCluster2.GetOuterVertex() :
                                                           pointingCluster2.GetInnerVertex());
      ...
      if (pVertexPfo)
        outputPfoMerges[pVertexPfo].insert(
          outputPfoMerges[pVertexPfo].begin(), pfoList.begin(), pfoList.end());
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void StitchingCosmicRayMergingTool::StitchPfos(const MasterAlgorithm* const pAlgorithm,
                                                 const ThreeDPointingClusterMap& pointingClusterMap,
    ...
    for (const ParticleFlowObject* const pPfoToEnlarge : pfoVectorToEnlarge) {
      ...
      if (!m_useXcoordinate || m_alwaysApplyT0Calculation) {
        try {
          ...
          if (!this->CalculateX0(
                pfoToLArTPCMap, pointingClusterMap, pfoVector, x0, pfoToPointingVertexMatrix))
            continue;
        }
        catch (const pandora::StatusCodeException&) {
          continue;
      ...
      for (PfoVector::const_iterator iterI = pfoVector.begin(); iterI != pfoVector.end(); ++iterI) {
        ...
        for (PfoVector::const_iterator iterJ = std::next(iterI); iterJ != pfoVector.end();
             ++iterJ) {
          const ParticleFlowObject* const pPfoJ(*iterJ);
          const LArTPC* const pLArTPCJ(pfoToLArTPCMap.at(pPfoJ));

          if (!LArStitchingHelper::CanTPCsBeStitched(*pLArTPCI, *pLArTPCJ)) continue;

          if (std::find(shiftedPfos.begin(), shiftedPfos.end(), pPfoI) == shiftedPfos.end()) {
            if (!m_useXcoordinate || m_alwaysApplyT0Calculation)
              this->ShiftPfo(
                pAlgorithm, pPfoI, pPfoJ, x0, pfoToLArTPCMap, pfoToPointingVertexMatrix);
          ...
          if (std::find(shiftedPfos.begin(), shiftedPfos.end(), pPfoJ) == shiftedPfos.end()) {
            if (!m_useXcoordinate || m_alwaysApplyT0Calculation)
              this->ShiftPfo(
                pAlgorithm, pPfoJ, pPfoI, x0, pfoToLArTPCMap, pfoToPointingVertexMatrix);
      ...
      for (const ParticleFlowObject* const pPfoToDelete : shiftedPfos) {
        if (pPfoToDelete == pPfoToEnlarge) continue;
  ...
  void StitchingCosmicRayMergingTool::ShiftPfo(
    const MasterAlgorithm* const pAlgorithm,
    const ParticleFlowObject* const pPfoToShift,
    const ParticleFlowObject* const pMatchedPfo,
    const float x0,
    const PfoToLArTPCMap& pfoToLArTPCMap,
    const PfoToPointingVertexMatrix& pfoToPointingVertexMatrix) const
  {
    // get stitching vertex for the pfo to be shifted
    const PfoToPointingVertexMatrix::const_iterator pfoToPointingVertexMatrixIter(
      pfoToPointingVertexMatrix.find(pPfoToShift));
    const LArPointingCluster::Vertex stitchingVertex(
      pfoToPointingVertexMatrixIter->second.at(pMatchedPfo));
    ...
    const float tpcBoundaryCenterX(
      LArStitchingHelper::GetTPCBoundaryCenterX(*pShiftLArTPC, *pMatchedLArTPC));
    ...
    const float positionShiftSign =
      stitchingVertex.GetPosition().GetX() < tpcBoundaryX ? 1.f : -1.f;
    ...
    for (const ParticleFlowObject* const pHierarchyPfo : downstreamPfoList)
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ParticleFlowObject::AlterMetadata(*pAlgorithm, pHierarchyPfo, metadata));
  ...
  bool StitchingCosmicRayMergingTool::CalculateX0(
    const PfoToLArTPCMap& pfoToLArTPCMap,
    const ThreeDPointingClusterMap& pointingClusterMap,
    const PfoVector& pfoVector,
    float& x0,
    PfoToPointingVertexMatrix& pfoToPointingVertexMatrix) const
  {
    float sumX(0.f), sumN(0.f);

    for (PfoVector::const_iterator iter1 = pfoVector.begin(), iterEnd = pfoVector.end();
         iter1 != iterEnd;
         ++iter1) {
      const ParticleFlowObject* const pPfo1(*iter1);
      PfoToLArTPCMap::const_iterator tpcIter1 = pfoToLArTPCMap.find(pPfo1);
      ...
      for (PfoVector::const_iterator iter2 = std::next(iter1); iter2 != iterEnd; ++iter2) {
        ...
        LArPointingCluster::Vertex pointingVertex1, pointingVertex2;
        try {
          LArStitchingHelper::GetClosestVertices(*pLArTPC1,
                                                 *pLArTPC2,
                                                 pointingCluster1,
                                                 pointingCluster2,
                                                 pointingVertex1,
          ...
          const PfoToPointingVertexMatrix::iterator pfoToPointingVertexMatrixIter1(
            pfoToPointingVertexMatrix.find(pPfo1));
          if (pfoToPointingVertexMatrixIter1 == pfoToPointingVertexMatrix.end()) {
            ...
            (void)pfoToPointingVertexMatrix.insert(
              PfoToPointingVertexMatrix::value_type(pPfo1, pfoToPointingVertexMap));
          }
          else {
            ...
            const PfoToPointingVertexMap::iterator pfoToPointingVertexMapIter(
              pfoToPointingVertexMap.find(pPfo2));
            if (pfoToPointingVertexMapIter == pfoToPointingVertexMap.end()) {
              (void)pfoToPointingVertexMap.insert(
                PfoToPointingVertexMap::value_type(pPfo2, pointingVertex1));
            }
            else {
              if ((pfoToPointingVertexMapIter->second.GetPosition() - pointingVertex1.GetPosition())
                    .GetMagnitude() > std::numeric_limits<float>::epsilon())
                throw StatusCodeException(STATUS_CODE_FAILURE);
            }
          }

          // Record pfo2 stitching vertex for pfo1<->pfo2 match, used to determine shifting direction in later step
          const PfoToPointingVertexMatrix::iterator pfoToPointingVertexMatrixIter2(
            pfoToPointingVertexMatrix.find(pPfo2));
          if (pfoToPointingVertexMatrixIter2 == pfoToPointingVertexMatrix.end()) {
            ...
            (void)pfoToPointingVertexMatrix.insert(
              PfoToPointingVertexMatrix::value_type(pPfo2, pfoToPointingVertexMap));
          }
          else {
            // ATTN: another match for a different TPC boundary may be present, add pfo1<->pfo2 match
            PfoToPointingVertexMap& pfoToPointingVertexMap(pfoToPointingVertexMatrixIter2->second);
            const PfoToPointingVertexMap::iterator pfoToPointingVertexMapIter(
              pfoToPointingVertexMap.find(pPfo1));
            if (pfoToPointingVertexMapIter == pfoToPointingVertexMap.end()) {
              (void)pfoToPointingVertexMap.insert(
                PfoToPointingVertexMap::value_type(pPfo1, pointingVertex2));
            }
            else {
              if ((pfoToPointingVertexMapIter->second.GetPosition() - pointingVertex2.GetPosition())
                    .GetMagnitude() > std::numeric_limits<float>::epsilon())
                throw StatusCodeException(STATUS_CODE_FAILURE);
            }
          }
          ...
          const float tpcBoundaryCenterX(
            LArStitchingHelper::GetTPCBoundaryCenterX(*pLArTPC1, *pLArTPC2));
          const bool isCPAStitch(pLArTPC1->GetCenterX() < tpcBoundaryCenterX ?
                                   !pLArTPC1->IsDriftInPositiveX() :
                                   !pLArTPC2->IsDriftInPositiveX());
          float thisX0(LArStitchingHelper::CalculateX0(
            *pLArTPC1, *pLArTPC2, pointingVertex1, pointingVertex2));
          ...
          if ((sumN > std::numeric_limits<float>::epsilon()) &&
              (sumX > std::numeric_limits<float>::epsilon())) {
            const float fractionalDiff(std::fabs((sumX - (thisX0 * sumN)) / sumX));

            if ((fractionalDiff > m_maxX0FractionalDeviation) &&
                (std::fabs(sumX / sumN) > m_boundaryToleranceWidth))
              return false;
          }
          ...
          if (STATUS_CODE_FAILURE == statusCodeException.GetStatusCode())
            std::cout << "StitchingCosmicRayMergingTool: Attempting to stitch a pfo with multiple "
                         "vertices for the same match"
                      << std::endl;
        }
      }
    }
    ...
    if ((sumN < std::numeric_limits<float>::epsilon()) ||
        (std::fabs(sumX) < std::numeric_limits<float>::epsilon()))
      throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  StitchingCosmicRayMergingTool::PfoAssociation::PfoAssociation(const VertexType parent,
  ...
  StitchingCosmicRayMergingTool::PfoAssociation::VertexType
  StitchingCosmicRayMergingTool::PfoAssociation::GetParent() const
  {
    return m_parent;
  }
  ...
  StitchingCosmicRayMergingTool::PfoAssociation::VertexType
  StitchingCosmicRayMergingTool::PfoAssociation::GetDaughter() const
  {
    return m_daughter;
  }
  ...
  float StitchingCosmicRayMergingTool::PfoAssociation::GetFigureOfMerit() const { return m_fom; }
  ...
  StatusCode StitchingCosmicRayMergingTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "ThreeDStitchingMode", m_useXcoordinate));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "HalfWindowLayers", m_halfWindowLayers));

    float minLength(std::sqrt(m_minLengthSquared));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MinPfoLength", minLength));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinCosRelativeAngle", m_minCosRelativeAngle));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "RelaxMinLongitudinalDisplacement",
                                                         m_relaxMinLongitudinalDisplacement));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "LooserMaxTransverseDisplacement",
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinNCaloHits3D", m_minNCaloHits3D));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "BoundaryToleranceWidth", m_boundaryToleranceWidth));
