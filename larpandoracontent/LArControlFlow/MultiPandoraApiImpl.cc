  ...
  const unsigned int volumeId) const
{
  ...
  for (const pandora::Pandora* const pPandora : instanceList) {
    try {
      if (volumeId == this->GetVolumeId(pPandora)) return pPandora;
    }
...
{
  if (!m_primaryToDaughtersMap
         .insert(PandoraInstanceMap::value_type(pPrimaryPandora, PandoraInstanceList()))
         .second)
    throw pandora::StatusCodeException(pandora::STATUS_CODE_ALREADY_PRESENT);
}
                                                     ...
                                                     const pandora::Pandora* const pDaughterPandora)
{
  ...
  if (!m_daughterToPrimaryMap
         .insert(PandoraRelationMap::value_type(pDaughterPandora, pPrimaryPandora))
         .second)
    throw pandora::StatusCodeException(pandora::STATUS_CODE_FAILURE);
}
...
void MultiPandoraApiImpl::DeletePandoraInstances(const pandora::Pandora* const pPrimaryPandora)
{
  ...
  try {
    ...
    std::cout << "MultiPandoraApiImpl::DeletePandoraInstances - unable to find daughter instances "
