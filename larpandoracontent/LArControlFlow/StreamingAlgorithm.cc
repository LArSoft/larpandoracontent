...
namespace lar_content {
  ...
  StatusCode StreamingAlgorithm::Run()
  {
    ...
    for (std::string listName : m_inputListNames) {
      ...
      if (code == STATUS_CODE_SUCCESS) {
        for (const auto& alg : m_streamAlgorithmMap.at(
               algStreamName)) { // ATTN - The algorithms replace the current list as they go
          PandoraContentApi::GetCurrentList(*this, pClusterList);
          if (!pClusterList->empty()) {
            PANDORA_RETURN_RESULT_IF(
              STATUS_CODE_SUCCESS, !=, PandoraContentApi::RunDaughterAlgorithm(*this, alg));
          }
        }
  ...
  StatusCode StreamingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "ListType", m_listType));
    std::transform(m_listType.begin(), m_listType.end(), m_listType.begin(), ::tolower);
    if (m_listType != "cluster") {
      std::cout << "StreamingAlgorithm::ReadSettings - Error: Only Cluster list type is supported "
                   "at this time"
                << std::endl;
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "InputListNames", m_inputListNames));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "OutputListName", m_outputListName));
    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "OutputListNames", m_outputListNames));
    if ((m_outputListName.empty() && m_outputListNames.empty()) ||
        (!m_outputListName.empty() && !m_outputListNames.empty())) {
      std::cout << "StreamingAlgorithm::ReadSettings - Error: You must provide either a single "
                   "output list name OR a list of output list names"
                << std::endl;
      return STATUS_CODE_NOT_FOUND;
    }
    if (!m_outputListNames.empty() && m_inputListNames.size() != m_outputListNames.size()) {
      std::cout << "StreamingAlgorithm::ReadSettings - Error: When providing a list of output "
                   "lists, there should be a one-to-one "
    ...
    for (std::string listName : m_inputListNames) {
      ...
      if (m_streamAlgorithmMap.find(algStreamName) != m_streamAlgorithmMap.end()) {
        std::cout << "StreamingAlgorithm::ReadSettings - Error: Duplicate stream name found"
                  << std::endl;
        return STATUS_CODE_INVALID_PARAMETER;
      }
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ProcessAlgorithmList(
          *this, xmlHandle, algStreamName, m_streamAlgorithmMap[algStreamName]));
      if (m_streamAlgorithmMap.at(algStreamName).empty()) {
        std::cout << "StreamingAlgorithm::ReadSettings - Error: Found no algorithms for \'"
                  << algStreamName << "\'" << std::endl;
        return STATUS_CODE_NOT_FOUND;
      }
    }
