...
namespace lar_content {
  ...
  SimpleNeutrinoIdTool::SimpleNeutrinoIdTool()
    : m_selectAllNeutrinos(true), m_selectOnlyFirstSliceNeutrinos(false)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  void SimpleNeutrinoIdTool::SelectOutputPfos(const Algorithm* const pAlgorithm,
                                              ...
                                              PfoList& selectedPfos)
  {
    ...
    for (unsigned int sliceIndex = 0, nSlices = nuSliceHypotheses.size(); sliceIndex < nSlices;
      ...
      const PfoList& sliceOutput(
        (m_selectAllNeutrinos || (m_selectOnlyFirstSliceNeutrinos && (0 == sliceIndex))) ?
          nuSliceHypotheses.at(sliceIndex) :
          crSliceHypotheses.at(sliceIndex));

      const float score(
        m_selectAllNeutrinos || (m_selectOnlyFirstSliceNeutrinos && (0 == sliceIndex)) ? 1.f :
                                                                                         -1.f);
  ...
  StatusCode SimpleNeutrinoIdTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "SelectAllNeutrinos", m_selectAllNeutrinos));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "SelectOnlyFirstSliceNeutrinos",
                                                         m_selectOnlyFirstSliceNeutrinos));

    if (m_selectAllNeutrinos == m_selectOnlyFirstSliceNeutrinos) {
      std::cout << "SimpleNeutrinoIdTool::ReadSettings - exactly one of SelectAllNeutrinos and "
                   "SelectOnlyFirstSliceNeutrinos must be true"
                << std::endl;
