  ...
  private:
    ...
    public:
      /**
         ...
         */
      CRCandidate(const pandora::Pandora& pandora,
     ...
     */
    bool GetValid3DCluster(const pandora::ParticleFlowObject* const pPfo,
                           const pandora::Cluster*& pCluster3D) const;

    typedef std::unordered_map<const pandora::ParticleFlowObject*, pandora::PfoList>
      PfoToPfoListMap;
     ...
     */
    void GetPfoAssociations(const pandora::PfoList& parentCosmicRayPfos,
     ...
     */
    bool CheckAssociation(const pandora::CartesianVector& endPoint1,
     ...
     */
    void SliceEvent(const pandora::PfoList& parentCosmicRayPfos,
     ...
     */
    void FillSlice(const pandora::ParticleFlowObject* const pPfo,
                   const PfoToPfoListMap& pfoAssociationMap,
                   pandora::PfoList& slice) const;
     ...
     */
    void GetCRCandidates(const pandora::PfoList& parentCosmicRayPfos,
     ...
     */
    void CheckIfContained(const CRCandidateList& candidates,
     ...
     */
    void CheckIfTopToBottom(const CRCandidateList& candidates,
     ...
     */
    void GetNeutrinoSlices(const CRCandidateList& candidates,
     ...
     */
    void TagCRMuons(const CRCandidateList& candidates,
                    const PfoToBoolMap& pfoToInTimeMap,
    ...
    typedef std::unordered_map<const pandora::ParticleFlowObject*, SlidingFitPair>
      PfoToSlidingFitsMap;
    ...
    float
      m_maxAssociationDist; ///< The maximum distance from endpoint to point of closest approach, typically a multiple of LAr radiation length

    unsigned int m_minimumHits; ///< The minimum number of hits for a Pfo to be considered

    float
      m_inTimeMargin; ///< The maximum distance outside of the physical detector volume that a Pfo may be to still be considered in time
    float
      m_inTimeMaxX0; ///< The maximum pfo x0 (determined from shifted vertex) to allow pfo to still be considered in time
    ...
    float
      m_maxNeutrinoCosTheta; ///< The maximum cos(theta) that a Pfo can have to be classified as a likely neutrino
    float
      m_minCosmicCosTheta; ///< The minimum cos(theta) that a Pfo can have to be classified as a likely CR muon
    float
      m_maxCosmicCurvature; ///< The maximum curvature that a Pfo can have to be classified as a likely CR muon
