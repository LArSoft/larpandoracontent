...
namespace lar_content {
  ...
  StatusCode PostProcessingAlgorithm::Run()
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::DropCurrentList<CaloHit>(*this));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::DropCurrentList<Track>(*this));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::DropCurrentList<MCParticle>(*this));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::DropCurrentList<Cluster>(*this));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::DropCurrentList<ParticleFlowObject>(*this));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::DropCurrentList<Vertex>(*this));

    if (!m_currentPfoListReplacement.empty()) {
      const std::string replacementListName(m_currentPfoListReplacement +
                                            TypeToString(m_listCounter));

      if (STATUS_CODE_SUCCESS !=
          PandoraContentApi::ReplaceCurrentList<Pfo>(*this, replacementListName)) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout
            << "PostProcessingAlgorithm: could not replace current pfo list with list named: "
            << replacementListName << std::endl;

        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, PandoraContentApi::DropCurrentList<Pfo>(*this));
      }
    }
  ...
  StatusCode PostProcessingAlgorithm::RenameList(const std::string& oldListName) const
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      PandoraContentApi::RenameList<T>(*this, oldListName, newListName));
    return STATUS_CODE_SUCCESS;
  }
  ...
  template pandora::StatusCode PostProcessingAlgorithm::RenameList<PfoList>(
    const std::string&) const;
  template pandora::StatusCode PostProcessingAlgorithm::RenameList<ClusterList>(
    const std::string&) const;
  template pandora::StatusCode PostProcessingAlgorithm::RenameList<VertexList>(
    const std::string&) const;
  template pandora::StatusCode PostProcessingAlgorithm::RenameList<CaloHitList>(
    const std::string&) const;
