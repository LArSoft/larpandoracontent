 ...
 */
  ...
  public:
    ...
    void Run(const MasterAlgorithm* const pAlgorithm,
  ...
  private:
     ...
     */
    void SelectPrimaryPfos(const pandora::PfoList* pInputPfoList,
                           const PfoToLArTPCMap& pfoToLArTPCMap,
                           pandora::PfoList& outputPfoList) const;

    typedef std::unordered_map<const pandora::ParticleFlowObject*, LArPointingCluster>
     ...
     */
    void BuildPointingClusterMaps(const pandora::PfoList& inputPfoList,
                                  const PfoToLArTPCMap& pfoToLArTPCMap,
     ...
     */
    void BuildTPCMaps(const pandora::PfoList& inputPfoList,
                      const PfoToLArTPCMap& pfoToLArTPCMap,
                      LArTPCToPfoMap& larTPCToPfoMap) const;

    typedef std::unordered_map<const pandora::ParticleFlowObject*, PfoAssociation>
      PfoAssociationMap;
    typedef std::unordered_map<const pandora::ParticleFlowObject*, PfoAssociationMap>
     ...
     */
    void CreatePfoMatches(const LArTPCToPfoMap& larTPCToPfoMap,
     ...
     *  @param  pfoAssociationMatrix the output matrix of associations between Pfos
     */
    void CreatePfoMatches(const pandora::LArTPC& larTPC1,
     ...
     */
    void SelectPfoMatches(const PfoAssociationMatrix& pfoAssociationMatrix,
                          PfoMergeMap& pfoSelectedMatches) const;
     ...
     */
    void CollectAssociatedPfos(const pandora::ParticleFlowObject* const pSeedPfo,
                               const pandora::ParticleFlowObject* const pCurrentPfo,
                               const PfoMergeMap& pfoMerges,
                               const pandora::PfoSet& vetoSet,
                               pandora::PfoList& associatedList) const;
     ...
     */
    void OrderPfoMerges(const PfoToLArTPCMap& pfoToLArTPCMap,
                        const ThreeDPointingClusterMap& pointingClusterMap,
     ...
     */
    void StitchPfos(const MasterAlgorithm* const pAlgorithm,
                    const ThreeDPointingClusterMap& pointingClusterMap,
                    const PfoMergeMap& pfoMerges,
                    PfoToLArTPCMap& pfoToLArTPCMap,
                    PfoToFloatMap& stitchedPfosToX0Map) const;

    typedef std::unordered_map<const pandora::ParticleFlowObject*, LArPointingCluster::Vertex>
      PfoToPointingVertexMap;
    typedef std::unordered_map<const pandora::ParticleFlowObject*, PfoToPointingVertexMap>
     ...
     */
    void ShiftPfo(const MasterAlgorithm* const pAlgorithm,
     ...
     */
    bool CalculateX0(const PfoToLArTPCMap& pfoToLArTPCMap,
                     const ThreeDPointingClusterMap& pointingClusterMap,
    ...
    float
      m_relaxMinLongitudinalDisplacement; ///< The minimum value of the longitudinal impact parameter for association if both verticies fall in the detector gap
    ...
    float
      m_maxX0FractionalDeviation; ///< The maximum allowed fractional difference of an X0 contribution for matches to be stitched
    float
      m_boundaryToleranceWidth; ///< The distance from the APA/CPA boundary inside which the deviation consideration is ignored
  };
