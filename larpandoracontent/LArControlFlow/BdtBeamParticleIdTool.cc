...
namespace lar_content {
  ...
  StatusCode BdtBeamParticleIdTool::Initialize()
  {
    ...
    try {
      m_sliceFeatureParameters.Initialize(
        parentMinX, parentMaxX, parentMinY, parentMaxY, parentMinZ, parentMaxZ);
    }
    catch (const StatusCodeException& statusCodeException) {
      std::cout
        << "BdtBeamParticleIdTool::Initialize - unable to initialize LArTPC geometry parameters"
        << std::endl;
  ...
  void BdtBeamParticleIdTool::SelectOutputPfos(const pandora::Algorithm* const pAlgorithm,
                                               ...
                                               PfoList& selectedPfos)
  {
    ...
    if (m_useTrainingMode) {
      ...
      this->GetBestMCSliceIndices(
        pAlgorithm, nuSliceHypotheses, crSliceHypotheses, bestSliceIndices);
      ...
      for (unsigned int sliceIndex = 0; sliceIndex < nSlices; ++sliceIndex) {
        ...
        try {
          ...
          std::cout << "BdtBeamParticleIdTool::SelectOutputPfos - unable to fill feature vector"
                    << std::endl;
          continue;
        }

        bool isGoodTrainingSlice(false);
        if (std::find(bestSliceIndices.begin(), bestSliceIndices.end(), sliceIndex) !=
            bestSliceIndices.end())
          isGoodTrainingSlice = true;

        LArMvaHelper::ProduceTrainingExample(
          m_trainingOutputFile, isGoodTrainingSlice, featureVector);
      }

      return;
    }

    this->SelectPfosByAdaBDTScore(
      pAlgorithm, nuSliceHypotheses, crSliceHypotheses, sliceFeaturesVector, selectedPfos);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void BdtBeamParticleIdTool::GetSliceFeatures(const SliceHypotheses& nuSliceHypotheses,
                                               const SliceHypotheses& crSliceHypotheses,
                                               SliceFeaturesVector& sliceFeaturesVector) const
  {
    for (unsigned int sliceIndex = 0, nSlices = nuSliceHypotheses.size(); sliceIndex < nSlices;
         ++sliceIndex)
      sliceFeaturesVector.push_back(SliceFeatures(nuSliceHypotheses.at(sliceIndex),
  ...
  void BdtBeamParticleIdTool::SelectAllPfos(const pandora::Algorithm* const pAlgorithm,
                                            const SliceHypotheses& hypotheses,
                                            PfoList& selectedPfos) const
  {
    for (const PfoList& pfos : hypotheses) {
      for (const ParticleFlowObject* const pPfo : pfos) {
        ...
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::ParticleFlowObject::AlterMetadata(*pAlgorithm, pPfo, metadata));
      }
  ...
  void BdtBeamParticleIdTool::GetBestMCSliceIndices(const pandora::Algorithm* const pAlgorithm,
                                                    const SliceHypotheses& nuSliceHypotheses,
                                                    const SliceHypotheses& crSliceHypotheses,
                                                    pandora::IntVector& bestSliceIndices) const
  {
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::GetList(*pAlgorithm, m_caloHitListName, pAllReconstructedCaloHitList));

    const MCParticleList* pMCParticleList(nullptr);
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::GetList(*pAlgorithm, m_mcParticleListName, pMCParticleList));
    ...
    LArMCParticleHelper::SelectCaloHits(pAllReconstructedCaloHitList,
                                        mcToPrimaryMCMap,
                                        reconstructableCaloHitList,
                                        parameters.m_selectInputHits,
                                        parameters.m_maxPhotonPropagation);

    MCParticleToIntMap mcParticleToReconstructableHitsMap;
    this->PopulateMCParticleToHitsMap(mcParticleToReconstructableHitsMap,
                                      reconstructableCaloHitList);

    const CaloHitSet reconstructableCaloHitSet(reconstructableCaloHitList.begin(),
                                               reconstructableCaloHitList.end());
    ...
    for (unsigned int sliceIndex = 0, nSlices = nuSliceHypotheses.size(); sliceIndex < nSlices;
      ...
      this->Collect2DHits(
        crSliceHypotheses.at(sliceIndex), reconstructedCaloHitList, reconstructableCaloHitSet);

      if (nuSliceHypotheses.at(sliceIndex).size() == 1) {
        const PfoList& nuFinalStates(
          nuSliceHypotheses.at(sliceIndex).front()->GetDaughterPfoList());
      ...
      if (mcParticleToHitsInSliceMap.empty()) continue;
      ...
      if (2001 != LArMCParticleHelper::GetNuanceCode(pBestMCParticle)) continue;

      const unsigned int nMCHits(mcParticleToReconstructableHitsMap.at(pBestMCParticle));
      const float purity(
        nRecoHits > 0 ? static_cast<float>(nSharedHits) / static_cast<float>(nRecoHits) : 0.f);
      const float completeness(
        nMCHits > 0 ? static_cast<float>(nSharedHits) / static_cast<float>(nMCHits) : 0.f);

      if (this->PassesQualityCuts(purity, completeness)) bestSliceIndices.push_back(sliceIndex);
    }
    return;
  }
  ...
  void BdtBeamParticleIdTool::PopulateMCParticleToHitsMap(
    MCParticleToIntMap& mcParticleToIntMap,
    const pandora::CaloHitList& caloHitList) const
  {
    for (const CaloHit* const pCaloHit : caloHitList) {
      try {
        const MCParticle* pParentMCParticle(
          LArMCParticleHelper::GetParentMCParticle(MCParticleHelper::GetMainMCParticle(pCaloHit)));
        MCParticleToIntMap::iterator iter(mcParticleToIntMap.find(pParentMCParticle));

        if (iter != mcParticleToIntMap.end()) { (*iter).second += 1; }
        else {
  ...
  void BdtBeamParticleIdTool::Collect2DHits(const PfoList& pfos,
                                            CaloHitList& reconstructedCaloHitList,
                                            const CaloHitSet& reconstructableCaloHitSet) const
  {
    ...
    for (const CaloHit* const pCaloHit : collectedHits) {
      ...
      if (!reconstructableCaloHitSet.count(pParentCaloHit)) continue;

      // Ensure no hits have been double counted
      if (std::find(reconstructedCaloHitList.begin(),
                    reconstructedCaloHitList.end(),
                    pParentCaloHit) == reconstructedCaloHitList.end())
  ...
  void BdtBeamParticleIdTool::SelectPfosByAdaBDTScore(
    ...
    PfoList& selectedPfos) const
  {
    ...
    for (unsigned int sliceIndex = 0, nSlices = nuSliceHypotheses.size(); sliceIndex < nSlices;
         ++sliceIndex) {
      const float nuAdaBDTScore(
        sliceFeaturesVector.at(sliceIndex).GetAdaBoostDecisionTreeScore(m_adaBoostDecisionTree));
    ...
    std::sort(sliceIndexAdaBDTScorePairs.begin(),
              sliceIndexAdaBDTScorePairs.end(),
    ...
    , m_d(-1. * static_cast<double>(normal.GetDotProduct(point)))
  {
    try {
      ...
      std::cout
        << "BdtBeamParticleIdTool::Plane::Plane - normal vector to plane has a magnitude of zero"
        << std::endl;
      throw statusCodeException;
    }
  }
  ...
  CartesianVector BdtBeamParticleIdTool::Plane::GetLineIntersection(
    ...
    const CartesianVector& direction) const
  {
    if (std::fabs(direction.GetDotProduct(m_unitNormal)) < std::numeric_limits<float>::epsilon())
      return CartesianVector(std::numeric_limits<float>::max(),
    ...
    const double t(-1. * (static_cast<double>(point.GetDotProduct(m_unitNormal)) + m_d) /
  ...
  void BdtBeamParticleIdTool::SliceFeatureParameters::Initialize(const float larTPCMinX,
  ...
  BdtBeamParticleIdTool::SliceFeatures::SliceFeatures(
    const PfoList& pfosNu,
    const PfoList& pfosCr,
    const SliceFeatureParameters& sliceFeatureParameters)
    : m_isAvailable(false), m_sliceFeatureParameters(sliceFeatureParameters)
  {
    try {
      double closestDistanceNu(std::numeric_limits<double>::max()),
        closestDistanceCr(std::numeric_limits<double>::max()),
        supplementaryAngleToBeamNu(std::numeric_limits<double>::max()),
        supplementaryAngleToBeamCr(std::numeric_limits<double>::max()),
        separationNu(std::numeric_limits<double>::max()),
        separationCr(std::numeric_limits<double>::max());
      ;
      ...
      if (!selectedCaloHitListNu.empty() && !selectedCaloHitListCr.empty()) {
        float maxYNu(-std::numeric_limits<float>::max()),
          maxYCr(-std::numeric_limits<float>::max());
        CartesianVector centroidNu(0.f, 0.f, 0.f), interceptOneNu(0.f, 0.f, 0.f),
          interceptTwoNu(0.f, 0.f, 0.f), centroidCr(0.f, 0.f, 0.f), interceptOneCr(0.f, 0.f, 0.f),
          interceptTwoCr(0.f, 0.f, 0.f);
        ...
        supplementaryAngleToBeamNu =
          majorAxisNu.GetOpeningAngle(m_sliceFeatureParameters.GetBeamDirection());

        this->GetLArTPCIntercepts(centroidNu, majorAxisNu, interceptOneNu, interceptTwoNu);
        const double separationOneNu(
          (interceptOneNu - m_sliceFeatureParameters.GetBeamLArTPCIntersection()).GetMagnitude());
        const double separationTwoNu(
          (interceptTwoNu - m_sliceFeatureParameters.GetBeamLArTPCIntersection()).GetMagnitude());
        ...
        supplementaryAngleToBeamCr =
          majorAxisCr.GetOpeningAngle(m_sliceFeatureParameters.GetBeamDirection());

        this->GetLArTPCIntercepts(centroidCr, majorAxisCr, interceptOneCr, interceptTwoCr);
        const double separationOneCr(
          (interceptOneCr - m_sliceFeatureParameters.GetBeamLArTPCIntersection()).GetMagnitude());
        const double separationTwoCr(
          (interceptTwoCr - m_sliceFeatureParameters.GetBeamLArTPCIntersection()).GetMagnitude());
  ...
  void BdtBeamParticleIdTool::SliceFeatures::GetLeadingCaloHits(
    const CaloHitList& inputCaloHitList,
    CaloHitList& outputCaloHitList,
    double& closestHitToFaceDistance) const
  {
    if (inputCaloHitList.empty()) {
      std::cout << "BdtBeamParticleIdTool::SliceFeatures::GetLeadingCaloHits - empty calo hit list"
                << std::endl;
    ...
    for (const CaloHit* const pCaloHit : inputCaloHitList)
      hitDistanceVector.emplace_back(
        pCaloHit,
        (pCaloHit->GetPositionVector() - m_sliceFeatureParameters.GetBeamLArTPCIntersection())
          .GetMagnitudeSquared());

    std::sort(hitDistanceVector.begin(),
              hitDistanceVector.end(),
              [](const HitDistancePair& lhs, const HitDistancePair& rhs) -> bool {
                return (lhs.second < rhs.second);
              });

    if (hitDistanceVector.front().second < 0.f) {
      std::cout << "BdtBeamParticleIdTool::SliceFeatures::GetLeadingCaloHits - unphysical "
    ...
    const unsigned int nSelectedCaloHits(
      nInputHits < m_sliceFeatureParameters.GetNSelectedHits() ?
        nInputHits :
        static_cast<unsigned int>(std::ceil(static_cast<float>(nInputHits) *
                                            m_sliceFeatureParameters.GetSelectedFraction() /
                                            100.f)));

    for (const HitDistancePair& hitDistancePair : hitDistanceVector) {
      outputCaloHitList.push_back(hitDistancePair.first);

      if (outputCaloHitList.size() >= nSelectedCaloHits) break;
    }
  }
  ...
  void BdtBeamParticleIdTool::SliceFeatures::GetLArTPCIntercepts(
    const CartesianVector& a0,
    const CartesianVector& lineDirection,
    CartesianVector& interceptOne,
    CartesianVector& interceptTwo) const
  {
    ...
    try {
      ...
      std::cout << "BdtBeamParticleIdTool::SliceFeatures::GetLArTPCIntercepts - normal vector to "
                   "plane has a magnitude of zero"
                << std::endl;
      throw statusCodeException;
    }
    ...
    if (intercepts.size() > 1) {
      ...
      for (unsigned int i = 0; i < intercepts.size(); i++) {
        for (unsigned int j = i + 1; j < intercepts.size(); j++) {
          ...
          const float separationSquared(
            (candidateInterceptOne - candidateInterceptTwo).GetMagnitudeSquared());
      ...
      if (!interceptsSet) {
        std::cout << "BdtBeamParticleIdTool::SliceFeatures::GetLArTPCIntercepts - unable to set "
                     "the intercepts between a line and the LArTPC"
                  << std::endl;
        throw StatusCodeException(STATUS_CODE_NOT_ALLOWED);
      }
    }
    else {
      std::cout << "BdtBeamParticleIdTool::SliceFeatures::GetLArTPCIntercepts - inconsistent "
                   "number of intercepts between a line and the LArTPC"
                << std::endl;
      throw StatusCodeException(STATUS_CODE_NOT_ALLOWED);
    }
  }
  ...
  bool BdtBeamParticleIdTool::SliceFeatures::IsContained(const CartesianVector& spacePoint,
  ...
  void BdtBeamParticleIdTool::SliceFeatures::FillFeatureVector(
    LArMvaHelper::MvaFeatureVector& featureVector) const
  {
    if (!m_isAvailable) throw StatusCodeException(STATUS_CODE_NOT_FOUND);

    if (!featureVector.empty()) {
      std::cout << "BdtBeamParticleIdTool::SliceFeatures::FillFeatureVector - feature vector "
                   "already populated"
                << std::endl;
      throw StatusCodeException(STATUS_CODE_NOT_ALLOWED);
    }
  ...
  float BdtBeamParticleIdTool::SliceFeatures::GetAdaBoostDecisionTreeScore(
    const AdaBoostDecisionTree& adaBoostDecisionTree) const
  {
    ...
    try {
      ...
      std::cout << "BdtBeamParticleIdTool::SliceFeatures::GetAdaBoostDecisionTreeScore - unable to "
                   "fill feature vector"
                << std::endl;
      return -1.f;
    }
  ...
  StatusCode BdtBeamParticleIdTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "UseTrainingMode", m_useTrainingMode));

    if (m_useTrainingMode) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "TrainingOutputFileName", m_trainingOutputFile));
      ...
      std::string bdtName;
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "BdtName", bdtName));

      std::string bdtFileName;
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "BdtFileName", bdtFileName));

      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                               !=,
                               XmlHelper::ReadValue(xmlHandle, "MinAdaBDTScore", m_minAdaBDTScore));

      const std::string fullBdtFileName(
        LArFileHelper::FindFileInPath(bdtFileName, m_filePathEnvironmentVariable));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MinimumPurity", m_minPurity));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinimumCompleteness", m_minCompleteness));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaximumNeutrinos", m_maxNeutrinos));
    ...
    if (3 == beamLArTPCIntersection.size()) {
      ...
      std::cout << "BdtBeamParticleIdTool::ReadSettings - invalid BeamTPCIntersection specified "
                << std::endl;
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "BeamDirection", beamDirection));

    if (3 == beamDirection.size()) {
      CartesianVector beamDirectionCartesianVector(
        beamDirection.at(0), beamDirection.at(1), beamDirection.at(2));
      m_sliceFeatureParameters.SetBeamDirection(beamDirectionCartesianVector);
    }
    else if (!beamDirection.empty()) {
      std::cout << "BdtBeamParticleIdTool::ReadSettings - invalid BeamDirection specified "
                << std::endl;
      return STATUS_CODE_INVALID_PARAMETER;
    }
    else {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SelectedFraction", selectedFraction));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NSelectedHits", nSelectedHits));

    if (nSelectedHits > 0) m_sliceFeatureParameters.SetNSelectedHits(nSelectedHits);
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ContainmentLimit", containmentLimit));

    if (containmentLimit < 0.f) {
      std::cout << "BdtBeamParticleIdTool::ReadSettings - invalid ContainmentLimit specified "
                << std::endl;
      return STATUS_CODE_INVALID_PARAMETER;
    }
