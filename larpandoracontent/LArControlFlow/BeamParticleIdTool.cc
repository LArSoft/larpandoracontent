...
namespace lar_content {
  ...
  void BeamParticleIdTool::SelectOutputPfos(const Algorithm* const pAlgorithm,
                                            const SliceHypotheses& beamSliceHypotheses,
                                            ...
                                            PfoList& selectedPfos)
  {
    ...
    if (m_selectAllBeamParticles || m_selectOnlyFirstSliceBeamParticles) {
      for (unsigned int sliceIndex = 0, nSlices = beamSliceHypotheses.size(); sliceIndex < nSlices;
           ++sliceIndex) {
        const PfoList& sliceOutput(
          (m_selectAllBeamParticles || (m_selectOnlyFirstSliceBeamParticles && (0 == sliceIndex))) ?
            beamSliceHypotheses.at(sliceIndex) :
            crSliceHypotheses.at(sliceIndex));

        const float score(m_selectAllBeamParticles ||
                              (m_selectOnlyFirstSliceBeamParticles && (0 == sliceIndex)) ?
                            1.f :
                            -1.f);
    ...
    for (unsigned int sliceIndex = 0, nSlices = beamSliceHypotheses.size(); sliceIndex < nSlices;
      ...
      try {
        ...
        if (!selectedCaloHitList.empty()) {
          ...
          if ((std::min(separationOne, separationTwo) < m_projectionIntersectionCut) &&
              (closestDistance < m_closestDistanceCut) &&
      ...
      const PfoList& sliceOutput(usebeamHypothesis ? beamSliceHypotheses.at(sliceIndex) :
                                                     crSliceHypotheses.at(sliceIndex));
      selectedPfos.insert(selectedPfos.end(), sliceOutput.begin(), sliceOutput.end());
  ...
  void BeamParticleIdTool::GetSelectedCaloHits(const CaloHitList& inputCaloHitList,
                                               CaloHitList& outputCaloHitList,
                                               float& closestHitToFaceDistance) const
  {
    if (inputCaloHitList.empty()) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
    ...
    for (const CaloHit* const pCaloHit : inputCaloHitList)
      hitDistanceVector.emplace_back(
        pCaloHit, (pCaloHit->GetPositionVector() - m_beamTPCIntersection).GetMagnitudeSquared());
    ...
    const unsigned int nSelectedCaloHits(
      nInputHits < m_nSelectedHits ?
        nInputHits :
        static_cast<unsigned int>(
          std::round(static_cast<float>(nInputHits) * m_selectedFraction / 100.f + 0.5f)));
  ...
  void BeamParticleIdTool::GetTPCIntercepts(const CartesianVector& a0,
                                            ...
                                            CartesianVector& interceptTwo) const
  {
    ...
    for (const Plane& plane : m_tpcPlanes) {
      ...
      if (this->IsContained(intercept)) intercepts.push_back(intercept);
    }
  ...
  CartesianVector BeamParticleIdTool::Plane::GetLineIntersection(const CartesianVector& a0,
                                                                 const CartesianVector& a) const
  {
  ...
  StatusCode BeamParticleIdTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "SelectAllBeamParticles", m_selectAllBeamParticles));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "SelectOnlyFirstSliceBeamParticles",
                                                         m_selectOnlyFirstSliceBeamParticles));

    if (m_selectAllBeamParticles && m_selectOnlyFirstSliceBeamParticles) {
      std::cout << "BeamParticleIdTool::ReadSettings - cannot use both SelectAllBeamParticles and "
                   "SelectOnlyFirstSliceBeamParticles simultaneously"
                << std::endl;
    ...
    if (3 == beamTPCIntersection.size()) {
      m_beamTPCIntersection.SetValues(
        beamTPCIntersection.at(0), beamTPCIntersection.at(1), beamTPCIntersection.at(2));
    }
    else if (!beamTPCIntersection.empty()) {
      std::cout << "BeamParticleIdTool::ReadSettings - invalid BeamTPCIntersection specified "
                << std::endl;
      return STATUS_CODE_INVALID_PARAMETER;
    }
    else {
    ...
    if (3 == beamDirection.size()) {
      ...
      std::cout << "BeamParticleIdTool::ReadSettings - invalid BeamDirection specified "
                << std::endl;
      return STATUS_CODE_INVALID_PARAMETER;
    }
    else {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ClosestDistanceCut", m_closestDistanceCut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "AngleToBeamCut", m_angleToBeamCut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SelectedFraction", m_selectedFraction));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NSelectedHits", m_nSelectedHits));
