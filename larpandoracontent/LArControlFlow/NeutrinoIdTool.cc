...
namespace lar_content {
  ...
  template <typename T>
  void NeutrinoIdTool<T>::SelectOutputPfos(const Algorithm* const pAlgorithm,
                                           ...
                                           PfoList& selectedPfos)
  {
    ...
    if (m_useTrainingMode) {
      ...
      if (!this->GetBestMCSliceIndex(
            pAlgorithm, nuSliceHypotheses, crSliceHypotheses, bestSliceIndex))
        return;
      ...
      for (unsigned int sliceIndex = 0; sliceIndex < nSlices; ++sliceIndex) {
        ...
        LArMvaHelper::ProduceTrainingExample(
          m_trainingOutputFile, sliceIndex == bestSliceIndex, featureVector);
      }

      return;
    }

    this->SelectPfosByProbability(
  ...
  template <typename T>
  void NeutrinoIdTool<T>::GetSliceFeatures(const NeutrinoIdTool<T>* const pTool,
                                           const SliceHypotheses& nuSliceHypotheses,
                                           const SliceHypotheses& crSliceHypotheses,
  ...
  template <typename T>
  bool NeutrinoIdTool<T>::GetBestMCSliceIndex(const Algorithm* const pAlgorithm,
                                              const SliceHypotheses& nuSliceHypotheses,
                                              const SliceHypotheses& crSliceHypotheses,
                                              unsigned int& bestSliceIndex) const
  {
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::GetCurrentList(*pAlgorithm, pAllReconstructedCaloHitList));

    const MCParticleList* pMCParticleList(nullptr);
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetCurrentList(*pAlgorithm, pMCParticleList));
    ...
    const float purity(nHitsInBestSlice > 0 ? static_cast<float>(nNuHitsInBestSlice) /
                                                static_cast<float>(nHitsInBestSlice) :
                                              0.f);
    const float completeness(nuNHitsTotal > 0 ? static_cast<float>(nNuHitsInBestSlice) /
                                                  static_cast<float>(nuNHitsTotal) :
                                                0.f);
  ...
  template <typename T>
  bool NeutrinoIdTool<T>::PassesQualityCuts(const Algorithm* const pAlgorithm,
                                            const float purity,
                                            const float completeness) const
  {
  ...
  template <typename T>
  void NeutrinoIdTool<T>::Collect2DHits(const PfoList& pfos,
                                        CaloHitList& reconstructedCaloHitList,
                                        const CaloHitSet& reconstructableCaloHitSet) const
  {
    ...
    for (const CaloHit* const pCaloHit : collectedHits) {
      ...
      if (!reconstructableCaloHitSet.count(pParentHit)) continue;

      // Ensure no hits have been double counted
      if (std::find(reconstructedCaloHitList.begin(), reconstructedCaloHitList.end(), pParentHit) ==
  ...
  unsigned int NeutrinoIdTool<T>::CountNeutrinoInducedHits(const CaloHitList& caloHitList) const
  {
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      try {
        if (LArMCParticleHelper::IsNeutrino(LArMCParticleHelper::GetParentMCParticle(
  ...
  int NeutrinoIdTool<T>::GetNuanceCode(const Algorithm* const pAlgorithm) const
  {
    ...
    if (trueNeutrinos.size() != 1) {
      std::cout << "NeutrinoIdTool::GetNuanceCode - Error: number of true neutrinos in event must "
                   "be exactly one"
                << std::endl;
  ...
  template <typename T>
  void NeutrinoIdTool<T>::SelectAllPfos(const pandora::Algorithm* const pAlgorithm,
  ...
  template <typename T>
  void NeutrinoIdTool<T>::SelectPfosByProbability(const pandora::Algorithm* const pAlgorithm,
                                                  const SliceHypotheses& nuSliceHypotheses,
                                                  const SliceHypotheses& crSliceHypotheses,
                                                  ...
                                                  PfoList& selectedPfos) const
  {
    ...
    std::sort(sliceIndexProbabilityPairs.begin(),
              sliceIndexProbabilityPairs.end(),
  ...
  template <typename T>
  NeutrinoIdTool<T>::SliceFeatures::SliceFeatures(const PfoList& nuPfos,
                                                  const PfoList& crPfos,
                                                  const NeutrinoIdTool<T>* const pTool)
    : m_isAvailable(false), m_pTool(pTool)
  {
    try {
      ...
      const CartesianVector nuWeightedDir(nuWeightedDirTotal *
                                          (1.f / static_cast<float>(nuNHitsUsedTotal)));

      CartesianPointVector pointsInSphere;
      this->GetPointsInSphere(nuAllSpacePoints, nuVertex, 10, pointsInSphere);

      CartesianVector centroid(std::numeric_limits<float>::max(),
                               std::numeric_limits<float>::max(),
                               std::numeric_limits<float>::max());
      LArPcaHelper::EigenValues eigenValues(std::numeric_limits<float>::max(),
                                            std::numeric_limits<float>::max(),
                                            std::numeric_limits<float>::max());
      ...
      const float crFracHitsInLongestTrack =
  ...
  template <typename T>
  void NeutrinoIdTool<T>::SliceFeatures::GetFeatureVector(
  ...
  template <typename T>
  const ParticleFlowObject* NeutrinoIdTool<T>::SliceFeatures::GetNeutrino(
    const PfoList& nuPfos) const
  {
    // ATTN we should only ever have one neutrino reconstructed per slice
    if (nuPfos.size() != 1) throw StatusCodeException(STATUS_CODE_OUT_OF_RANGE);
  ...
  template <typename T>
  void NeutrinoIdTool<T>::SliceFeatures::GetSpacePoints(const ParticleFlowObject* const pPfo,
                                                        CartesianPointVector& spacePoints) const
  {
    ClusterList clusters3D;
    LArPfoHelper::GetThreeDClusterList(pPfo, clusters3D);

    if (clusters3D.size() > 1) throw StatusCodeException(STATUS_CODE_OUT_OF_RANGE);
  ...
  template <typename T>
  CartesianVector NeutrinoIdTool<T>::SliceFeatures::GetDirectionFromVertex(
    const CartesianPointVector& spacePoints,
    const CartesianVector& vertex) const
  {
    return this->GetDirection(
      spacePoints, [&](const CartesianVector& pointA, const CartesianVector& pointB) {
  ...
  template <typename T>
  CartesianVector NeutrinoIdTool<T>::SliceFeatures::GetUpperDirection(
    const CartesianPointVector& spacePoints) const
  {
  ...
  template <typename T>
  CartesianVector NeutrinoIdTool<T>::SliceFeatures::GetLowerDirection(
    const CartesianPointVector& spacePoints) const
  {
    return this->GetDirection(spacePoints,
                              [&](const CartesianVector& pointA, const CartesianVector& pointB) {
  ...
  template <typename T>
  CartesianVector NeutrinoIdTool<T>::SliceFeatures::GetDirection(
    const CartesianPointVector& spacePoints,
    std::function<bool(const CartesianVector& pointA, const CartesianVector& pointB)>
      fShouldChooseA) const
  {
  ...
  template <typename T>
  void NeutrinoIdTool<T>::SliceFeatures::GetPointsInSphere(
    const CartesianPointVector& spacePoints,
    const CartesianVector& vertex,
    const float radius,
    CartesianPointVector& spacePointsInSphere) const
  {
  ...
  StatusCode NeutrinoIdTool<T>::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SelectNuanceCode", m_selectNuanceCode));

    if (m_selectNuanceCode) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "NuanceCode", m_nuance));
    }
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinimumNeutrinoProbability", m_minProbability));
    ...
    if (!m_useTrainingMode) {
      std::string mvaName;
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "MvaName", mvaName));

      std::string mvaFileName;
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "MvaFileName", mvaFileName));

      const std::string fullMvaFileName(
        LArFileHelper::FindFileInPath(mvaFileName, m_filePathEnvironmentVariable));
