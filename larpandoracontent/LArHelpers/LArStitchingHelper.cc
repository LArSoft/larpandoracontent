...
namespace lar_content {
  ...
  const LArTPC& LArStitchingHelper::FindClosestTPC(const Pandora& pandora,
                                                   const LArTPC& inputTPC,
                                                   const bool checkPositive)
  {
    if (!MultiPandoraApi::GetPandoraInstanceMap().count(&pandora)) {
      std::cout << "LArStitchingHelper::FindClosestTPC - functionality only available to "
                   "primary/master Pandora instance "
                << std::endl;
    ...
    for (const LArTPCMap::value_type& mapEntry : larTPCMap) {
      ...
      if (&inputTPC == &checkTPC) continue;

      if (checkPositive != (checkTPC.GetCenterX() > inputTPC.GetCenterX())) continue;
      ...
      if (deltaY > maxDisplacement || deltaZ > maxDisplacement) continue;
    ...
    if (!pClosestTPC) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  {
    if (&firstTPC == &secondTPC) return false;

    // ATTN: We assume that Pfos are crossing either an anode-anode boundary or a cathode-cathode boundary
    if (firstTPC.IsDriftInPositiveX() == secondTPC.IsDriftInPositiveX()) return false;
  ...
  bool LArStitchingHelper::AreTPCsAdjacent(const LArTPC& firstTPC, const LArTPC& secondTPC)
  {
    ...
    if (std::fabs(deltaX - widthX) > maxDisplacement || deltaY > maxDisplacement ||
  ...
  bool LArStitchingHelper::AreTPCsAdjacent(const Pandora& pandora,
                                           ...
                                           const LArTPC& secondTPC)
  {
    ...
    try {
      ...
      if ((&firstTPCCheck == &firstTPC) && (&secondTPCCheck == &secondTPC)) return true;
    }
  ...
  float LArStitchingHelper::GetTPCBoundaryCenterX(const LArTPC& firstTPC, const LArTPC& secondTPC)
  {
    ...
    if (firstTPC.GetCenterX() < secondTPC.GetCenterX()) {
      return 0.5 * ((firstTPC.GetCenterX() + 0.5 * firstTPC.GetWidthX()) +
                    (secondTPC.GetCenterX() - 0.5 * secondTPC.GetWidthX()));
    }
    else {
      return 0.5 * ((firstTPC.GetCenterX() - 0.5 * firstTPC.GetWidthX()) +
                    (secondTPC.GetCenterX() + 0.5 * secondTPC.GetWidthX()));
  ...
  float LArStitchingHelper::GetTPCBoundaryWidthX(const LArTPC& firstTPC, const LArTPC& secondTPC)
  {
    ...
    if (firstTPC.GetCenterX() < secondTPC.GetCenterX()) {
      return ((secondTPC.GetCenterX() - 0.5 * secondTPC.GetWidthX()) -
              (firstTPC.GetCenterX() + 0.5 * firstTPC.GetWidthX()));
    }
    else {
      return ((firstTPC.GetCenterX() - 0.5 * firstTPC.GetWidthX()) -
              (secondTPC.GetCenterX() + 0.5 * secondTPC.GetWidthX()));
    }
  }
  ...
  void LArStitchingHelper::GetClosestVertices(const LArTPC& larTPC1,
  ...
  {
    if (&larTPC1 == &larTPC2) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

    // Find the closest vertices based on relative X positions in tpc
    const float dxVolume(larTPC2.GetCenterX() - larTPC1.GetCenterX());
    const float dx1(pointingCluster1.GetOuterVertex().GetPosition().GetX() -
                    pointingCluster1.GetInnerVertex().GetPosition().GetX());
    const float dx2(pointingCluster2.GetOuterVertex().GetPosition().GetX() -
                    pointingCluster2.GetInnerVertex().GetPosition().GetX());

    if (std::fabs(dx1) < std::numeric_limits<float>::epsilon() ||
        std::fabs(dx2) < std::numeric_limits<float>::epsilon())
      throw StatusCodeException(STATUS_CODE_NOT_FOUND);

    const bool useInner1(
      (dxVolume > 0.f) ==
      (dx1 < 0.f)); // xVol1 - OUTER - INNER - | - xVol2  [xVol2-xVol1>0; xOuter1-xInner1<0]
    const bool useInner2(
      (dxVolume > 0.f) ==
      (dx2 > 0.f)); // xVol1 - | - INNER - OUTER - xVol2  [xVol2-xVol1>0; xOuter2-xInner2>0]

    // Confirm that these really are the closest pair of vertices by checking the other possible pairs
    const LArPointingCluster::Vertex& nearVertex1(useInner1 ? pointingCluster1.GetInnerVertex() :
                                                              pointingCluster1.GetOuterVertex());
    const LArPointingCluster::Vertex& nearVertex2(useInner2 ? pointingCluster2.GetInnerVertex() :
                                                              pointingCluster2.GetOuterVertex());

    const LArPointingCluster::Vertex& farVertex1(useInner1 ? pointingCluster1.GetOuterVertex() :
                                                             pointingCluster1.GetInnerVertex());
    const LArPointingCluster::Vertex& farVertex2(useInner2 ? pointingCluster2.GetOuterVertex() :
    ...
    const float drNearNearSquared(dxNearNear * dxNearNear + dyNearNear * dyNearNear +
                                  dzNearNear * dzNearNear);
    ...
    const float drNearFarSquared(dxNearFar * dxNearFar + dyNearFar * dyNearFar +
                                 dzNearFar * dzNearFar);
    ...
    const float drFarNearSquared(dxFarNear * dxFarNear + dyFarNear * dyFarNear +
                                 dzFarNear * dzFarNear);
  ...
  float LArStitchingHelper::CalculateX0(const LArTPC& firstTPC,
                                        const LArTPC& secondTPC,
                                        const LArPointingCluster::Vertex& firstVertex,
                                        const LArPointingCluster::Vertex& secondVertex)
  {
    if (&firstTPC == &secondTPC) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
    ...
    const CartesianVector firstPositionYZ(
      0.f, firstVertex.GetPosition().GetY(), firstVertex.GetPosition().GetZ());
    const CartesianVector firstDirectionYZ(
      0.f, firstVertex.GetDirection().GetY(), firstVertex.GetDirection().GetZ());

    const CartesianVector secondPositionYZ(
      0.f, secondVertex.GetPosition().GetY(), secondVertex.GetPosition().GetZ());
    const CartesianVector secondDirectionYZ(
      0.f, secondVertex.GetDirection().GetY(), secondVertex.GetDirection().GetZ());
    ...
    if (firstDirectionYZmag < std::numeric_limits<float>::epsilon() ||
        secondDirectionYZmag < std::numeric_limits<float>::epsilon())
      throw StatusCodeException(STATUS_CODE_NOT_FOUND);

    const float R1(
      firstDirectionYZ.GetUnitVector().GetDotProduct(firstPositionYZ - secondPositionYZ) /
      firstDirectionYZmag);
    const float X1(-1.f * firstDirectionX.GetUnitVector().GetDotProduct(
                            secondVertex.GetPosition() -
                            (firstVertex.GetPosition() - firstVertex.GetDirection() * R1)));

    const float R2(
      secondDirectionYZ.GetUnitVector().GetDotProduct(secondPositionYZ - firstPositionYZ) /
      secondDirectionYZmag);
    const float X2(-1.f * secondDirectionX.GetUnitVector().GetDotProduct(
                            firstVertex.GetPosition() -
                            (secondVertex.GetPosition() - secondVertex.GetDirection() * R2)));
  ...
  bool LArStitchingHelper::SortTPCs(const pandora::LArTPC* const pLhs,
  ...
  bool LArStitchingHelper::HasPfoBeenStitched(const ParticleFlowObject* const pPfo)
  {
    ...
    if (iter != properties.end()) return true;
  ...
  float LArStitchingHelper::GetPfoX0(const ParticleFlowObject* const pPfo)
  {
    ...
    if (!LArStitchingHelper::HasPfoBeenStitched(pPfo)) return 0.f;
