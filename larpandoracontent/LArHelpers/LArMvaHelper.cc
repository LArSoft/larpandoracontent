...
namespace lar_content {
  ...
  StatusCode LArMvaHelper::ProcessAlgorithmToolListToMap(const Algorithm& algorithm,
                                                         const TiXmlHandle& xmlHandle,
                                                         const std::string& listName,
                                                         StringVector& algorithmToolNameVector,
                                                         AlgorithmToolMap& algorithmToolMap)
  {
    // Fill a vector with names in the desired run order as well as the map

    if ("algorithm" != xmlHandle.ToNode()->ValueStr()) return STATUS_CODE_NOT_ALLOWED;

    const TiXmlHandle algorithmListHandle = TiXmlHandle(xmlHandle.FirstChild(listName).Element());

    for (TiXmlElement* pXmlElement = algorithmListHandle.FirstChild("tool").Element();
         nullptr != pXmlElement;
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::CreateAlgorithmTool(algorithm, pXmlElement, pAlgorithmTool));
