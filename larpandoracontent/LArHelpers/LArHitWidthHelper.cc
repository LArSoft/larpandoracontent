...
namespace lar_content {
  ...
  LArHitWidthHelper::ConstituentHit::ConstituentHit(const CartesianVector& positionVector,
                                                    const float hitWidth,
                                                    const Cluster* const pParentClusterAddress)
  ...
  bool LArHitWidthHelper::ConstituentHit::SortByDistanceToPoint::operator()(
    const ConstituentHit& lhs,
    const ConstituentHit& rhs)
  {
    ...
    return (m_referencePoint.GetDistanceSquared(lhsPosition) <
            m_referencePoint.GetDistanceSquared(rhsPosition));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------------------------------------------------------------

  LArHitWidthHelper::ClusterParameters::ClusterParameters(const Cluster* const pCluster,
    ...
    , m_constituentHitVector(LArHitWidthHelper::GetConstituentHits(pCluster,
  ...
  LArHitWidthHelper::ClusterParameters::ClusterParameters(
    ...
    const CartesianVector& lowerXExtrema,
    const CartesianVector& higherXExtrema)
    : m_pCluster(pCluster)
  ...
  bool LArHitWidthHelper::SortByHigherXExtrema::operator()(const Cluster* const pLhs,
                                                           const Cluster* const pRhs)
  {
    const LArHitWidthHelper::ClusterParameters& lhsClusterParameters(
      LArHitWidthHelper::GetClusterParameters(pLhs, m_clusterToParametersMap));
    const LArHitWidthHelper::ClusterParameters& rhsClusterParameters(
      LArHitWidthHelper::GetClusterParameters(pRhs, m_clusterToParametersMap));

    return (lhsClusterParameters.GetHigherXExtrema().GetX() <
            rhsClusterParameters.GetHigherXExtrema().GetX());
  }
  ...
  {
    if (clusterToParametersMap.empty()) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
  ...
  unsigned int LArHitWidthHelper::GetNProposedConstituentHits(const Cluster* const pCluster,
                                                              const float maxConstituentHitWidth,
                                                              const float hitWidthScalingFactor)
  {
    if (maxConstituentHitWidth < std::numeric_limits<float>::epsilon()) {
      std::cout << "LArHitWidthHelper::GetConstituentHits - Negative or equivalent to zero "
                   "constitent hit width not allowed"
                << std::endl;
    ...
    const bool isUniform)
  {
    ...
    for (const OrderedCaloHitList::value_type& mapEntry : orderedCaloHitList) {
      for (const CaloHit* const pCaloHit : *mapEntry.second) {
        ...
        const unsigned int numberOfConstituentHits = std::ceil(hitWidth / maxConstituentHitWidth);
        if (isUniform) {
          LArHitWidthHelper::SplitHitIntoConstituents(pCaloHit,
                                                      pCluster,
                                                      numberOfConstituentHits,
                                                      maxConstituentHitWidth,
                                                      constituentHitVector);
        }
        else {
          const float constituentHitWidth = hitWidth / numberOfConstituentHits;
          LArHitWidthHelper::SplitHitIntoConstituents(
            pCaloHit, pCluster, numberOfConstituentHits, constituentHitWidth, constituentHitVector);
        }
      }
    }
  ...
  void LArHitWidthHelper::SplitHitIntoConstituents(
    ...
    LArHitWidthHelper::ConstituentHitVector& constituentHitVector)
  {
    ...
    for (unsigned int i = 0; i < loopIterations; ++i) {
      ...
      constituentHitVector.push_back(
  ...
  CartesianVector LArHitWidthHelper::GetExtremalCoordinatesLowerX(
    const ConstituentHitVector& constituentHitVector)
  {
    CartesianVector lowerXCoordinate(0.f, 0.f, 0.f), higherXCoordinate(0.f, 0.f, 0.f);
    LArHitWidthHelper::GetExtremalCoordinatesX(
      constituentHitVector, lowerXCoordinate, higherXCoordinate);
  ...
  CartesianVector LArHitWidthHelper::GetExtremalCoordinatesHigherX(
    const ConstituentHitVector& constituentHitVector)
  {
    CartesianVector lowerXCoordinate(0.f, 0.f, 0.f), higherXCoordinate(0.f, 0.f, 0.f);
    LArHitWidthHelper::GetExtremalCoordinatesX(
  ...
  void LArHitWidthHelper::GetExtremalCoordinatesX(const ConstituentHitVector& constituentHitVector,
                                                  CartesianVector& lowerXCoordinate,
                                                  CartesianVector& higherXCoordinate)
  {
    const CartesianPointVector& constituentHitPositionVector(
      GetConstituentHitPositionVector(constituentHitVector));

    CartesianVector innerCoordinate(0.f, 0.f, 0.f), outerCoordinate(0.f, 0.f, 0.f);
    LArClusterHelper::GetExtremalCoordinates(
      constituentHitPositionVector, innerCoordinate, outerCoordinate);
    ...
    if ((deltaX > 0.f) ||
        ((std::fabs(deltaX) < std::numeric_limits<float>::epsilon()) && (deltaZ > 0.f))) {
  ...
  CartesianVector LArHitWidthHelper::GetClosestPointToLine2D(const CartesianVector& lineStart,
                                                             const CartesianVector& lineDirection,
                                                             const CaloHit* const pCaloHit)
  {
    const CartesianVector& hitPosition(pCaloHit->GetPositionVector());

    if (std::fabs(lineDirection.GetZ()) < std::numeric_limits<float>::epsilon()) return hitPosition;
    ...
    const float closestPointX(
      xOnLine < hitLowXEdge ? hitLowXEdge : xOnLine > hitHighXEdge ? hitHighXEdge : xOnLine);
  ...
  float LArHitWidthHelper::GetClosestDistanceToPoint2D(const CaloHit* const pCaloHit,
                                                       const CartesianVector& point2D)
  {
    const CartesianVector& hitPosition(pCaloHit->GetPositionVector());
    const float hitWidth(pCaloHit->GetCellSize1());
    const float hitLowXEdge(hitPosition.GetX() - (hitWidth * 0.5f));
    ...
    if ((hitLowXEdge < point2D.GetX()) && (hitHighXEdge > point2D.GetX())) return modDeltaZ;

    const float deltaX = hitLowXEdge > point2D.GetX() ? (point2D.GetX() - hitLowXEdge) :
                                                        (point2D.GetX() - hitHighXEdge);
