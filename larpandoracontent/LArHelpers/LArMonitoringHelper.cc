...
namespace lar_content {
  ...
  unsigned int LArMonitoringHelper::CountHitsByType(const HitType hitType,
                                                    const CaloHitList& caloHitList)
  {
    unsigned int nHitsOfSpecifiedType(0);

    for (const CaloHit* const pCaloHit : caloHitList) {
      if (hitType == pCaloHit->GetHitType()) ++nHitsOfSpecifiedType;
    }
  ...
  void LArMonitoringHelper::GetOrderedMCParticleVector(
    const LArMCParticleHelper::MCContributionMapVector& selectedMCParticleToGoodHitsMaps,
    MCParticleVector& orderedMCParticleVector)
  {
    for (const LArMCParticleHelper::MCContributionMap& mcParticleToGoodHitsMap :
         selectedMCParticleToGoodHitsMaps) {
      if (mcParticleToGoodHitsMap.empty()) continue;
      ...
      std::copy(mcParticleToGoodHitsMap.begin(),
                mcParticleToGoodHitsMap.end(),
                std::back_inserter(mcParticleToGoodHitsVect));

      // Sort by number of hits descending
      std::sort(mcParticleToGoodHitsVect.begin(),
                mcParticleToGoodHitsVect.end(),
                [](const LArMCParticleHelper::MCParticleCaloHitListPair& a,
                   const LArMCParticleHelper::MCParticleCaloHitListPair& b) -> bool {
                  // Neutrinos, then beam particles, then cosmic rays
                  const bool isANuFinalState(
                    LArMCParticleHelper::IsBeamNeutrinoFinalState(a.first)),
                    isBNuFinalState(LArMCParticleHelper::IsBeamNeutrinoFinalState(b.first));

                  if (isANuFinalState != isBNuFinalState) return isANuFinalState;
                  ...
                  if (isABeamParticle != isBBeamParticle) return isABeamParticle;
      ...
      for (const LArMCParticleHelper::MCParticleCaloHitListPair& mcParticleCaloHitPair :
           mcParticleToGoodHitsVect)
    ...
    if (std::distance(orderedMCParticleVector.begin(),
                      std::unique(orderedMCParticleVector.begin(),
                                  orderedMCParticleVector.end())) != nMCParticles)
      throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArMonitoringHelper::GetOrderedPfoVector(
    ...
    pandora::PfoVector& orderedPfoVector)
  {
    ...
    std::copy(pfoToReconstructable2DHitsMap.begin(),
              pfoToReconstructable2DHitsMap.end(),
              std::back_inserter(pfoToReconstructable2DHitsVect));

    // Sort by number of hits descending putting neutrino final states first
    std::sort(pfoToReconstructable2DHitsVect.begin(),
              pfoToReconstructable2DHitsVect.end(),
              [](const LArMCParticleHelper::PfoCaloHitListPair& a,
                ...
                const bool isANuFinalState(LArPfoHelper::IsNeutrinoFinalState(a.first)),
                  isBNuFinalState(LArPfoHelper::IsNeutrinoFinalState(b.first));
    ...
    for (const LArMCParticleHelper::PfoCaloHitListPair& pfoCaloHitPair :
         pfoToReconstructable2DHitsVect)
      orderedPfoVector.push_back(pfoCaloHitPair.first);

    // Check that all elements of the vector are unique
    const unsigned int nPfos(orderedPfoVector.size());
    if (std::distance(orderedPfoVector.begin(),
                      std::unique(orderedPfoVector.begin(), orderedPfoVector.end())) != nPfos)
      throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
  }
  ...
  void LArMonitoringHelper::PrintMCParticleTable(
    const LArMCParticleHelper::MCContributionMap& selectedMCParticleToGoodHitsMap,
    const MCParticleVector& orderedMCParticleVector)
  {
    ...
    LArFormattingHelper::Table table(
      {"ID", "NUANCE", "TYPE", "", "E", "dist", "", "nGoodHits", "U", "V", "W"});
    ...
    for (unsigned int id = 0; id < orderedMCParticleVector.size(); ++id) {
      ...
      LArMCParticleHelper::MCContributionMap::const_iterator it =
        selectedMCParticleToGoodHitsMap.find(pMCParticle);
  ...
  void LArMonitoringHelper::PrintPfoTable(
    const LArMCParticleHelper::PfoContributionMap& pfoToReconstructable2DHitsMap,
  ...
  void LArMonitoringHelper::PrintMatchingTable(
    const PfoVector& orderedPfoVector,
    const MCParticleVector& orderedMCParticleVector,
    ...
    const unsigned int nMatches)
  {
    ...
    for (unsigned int mcParticleId = 0; mcParticleId < orderedMCParticleVector.size();
         ++mcParticleId) {
      const MCParticle* const pMCParticle(orderedMCParticleVector.at(mcParticleId));
      LArMCParticleHelper::MCParticleToPfoHitSharingMap::const_iterator it =
        mcParticleToPfoHitSharingMap.find(pMCParticle);
      LArMCParticleHelper::PfoToSharedHitsVector pfoToSharedHitsVector;

      if (it != mcParticleToPfoHitSharingMap.end()) pfoToSharedHitsVector = it->second;
      ...
      const LArFormattingHelper::Color mcCol(
        LArMCParticleHelper::IsBeamNeutrinoFinalState(pMCParticle) ?
          ...
          (LArMCParticleHelper::IsBeamParticle(pMCParticle) ? LArFormattingHelper::LIGHT_BLUE :
                                                              LArFormattingHelper::LIGHT_RED));
        ...
        for (unsigned int pfoId = 0; pfoId < orderedPfoVector.size(); ++pfoId) {
          ...
          if (nPfosShown < nMatchesToShow) {
            ...
            const LArFormattingHelper::Color pfoCol(
              LArPfoHelper::IsNeutrinoFinalState(pfoNSharedHitsPair.first) ?
                LArFormattingHelper::LIGHT_GREEN :
                LArFormattingHelper::LIGHT_RED);
            table.AddElement(pfoId + 1, LArFormattingHelper::REGULAR, pfoCol);
            table.AddElement(
              pfoNSharedHitsPair.second.size(), LArFormattingHelper::REGULAR, pfoCol);
            nPfosShown++;
          }
          else {
