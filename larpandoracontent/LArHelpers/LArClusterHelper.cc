...
namespace lar_content {
  ...
  {
    if (0 == pCluster->GetNCaloHits()) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
  ...
  void LArClusterHelper::GetClustersUVW(const ClusterList& inputClusters,
                                        ClusterList& clusterListU,
                                        ClusterList& clusterListV,
                                        ClusterList& clusterListW)
  {
    for (ClusterList::const_iterator iter = inputClusters.begin(), iterEnd = inputClusters.end();
         iter != iterEnd;
         ++iter) {
  ...
  void LArClusterHelper::GetClustersByHitType(const ClusterList& inputClusters,
                                              ...
                                              ClusterList& clusterList)
  {
    for (ClusterList::const_iterator iter = inputClusters.begin(), iterEnd = inputClusters.end();
         iter != iterEnd;
         ++iter) {
      if (hitType == LArClusterHelper::GetClusterHitType(*iter)) clusterList.push_back(*iter);
    }
  }
  ...
  float LArClusterHelper::GetLengthSquared(const Cluster* const pCluster)
  {
    ...
    if (orderedCaloHitList.empty()) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
  ...
  float LArClusterHelper::GetLayerOccupancy(const Cluster* const pCluster)
  {
    ...
    const unsigned int nLayers(1 + pCluster->GetOuterPseudoLayer() -
                               pCluster->GetInnerPseudoLayer());

    if (nLayers > 0) return (static_cast<float>(nOccupiedLayers) / static_cast<float>(nLayers));
  ...
  float LArClusterHelper::GetLayerOccupancy(const Cluster* const pCluster1,
                                            const Cluster* const pCluster2)
  {
    const unsigned int nOccupiedLayers(pCluster1->GetOrderedCaloHitList().size() +
                                       pCluster2->GetOrderedCaloHitList().size());
    const unsigned int nLayers(
      1 + std::max(pCluster1->GetOuterPseudoLayer(), pCluster2->GetOuterPseudoLayer()) -
  ...
  float LArClusterHelper::GetClosestDistance(const ClusterList& clusterList1,
                                             const ClusterList& clusterList2)
  {
    ...
    for (ClusterList::const_iterator iter1 = clusterList1.begin(), iterEnd1 = clusterList1.end();
         iter1 != iterEnd1;
         ++iter1) {
  ...
  float LArClusterHelper::GetClosestDistance(const Cluster* const pCluster,
                                             const ClusterList& clusterList)
  {
    if (clusterList.empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);

    float closestDistance(std::numeric_limits<float>::max());

    for (ClusterList::const_iterator iter = clusterList.begin(), iterEnd = clusterList.end();
         iter != iterEnd;
         ++iter) {
  ...
  float LArClusterHelper::GetClosestDistance(const Cluster* const pCluster1,
                                             const Cluster* const pCluster2)
  {
  ...
  float LArClusterHelper::GetClosestDistance(const CartesianVector& position,
                                             const ClusterList& clusterList)
  {
  ...
  CartesianVector LArClusterHelper::GetClosestPosition(const CartesianVector& position,
                                                       const ClusterList& clusterList)
  {
    ...
    for (ClusterList::const_iterator iter = clusterList.begin(), iterEnd = clusterList.end();
         ...
         ++iter) {
      const Cluster* const pTestCluster = *iter;
      const CartesianVector thisPosition(
        LArClusterHelper::GetClosestPosition(position, pTestCluster));
    ...
    if (distanceFound) return closestPosition;
                                                       ...
                                                       const CaloHitList& caloHitList)
  {
    ...
    if (pClosestCaloHit) return pClosestCaloHit->GetPositionVector();
  ...
  void LArClusterHelper::GetClosestPositions(const Cluster* const pCluster1,
                                             const Cluster* const pCluster2,
                                             CartesianVector& outputPosition1,
                                             CartesianVector& outputPosition2)
  {
    ...
    for (OrderedCaloHitList::const_iterator iter1 = orderedCaloHitList1.begin(),
                                            iter1End = orderedCaloHitList1.end();
         iter1 != iter1End;
         ++iter1) {
      for (CaloHitList::const_iterator hitIter1 = iter1->second->begin(),
                                       hitIter1End = iter1->second->end();
           hitIter1 != hitIter1End;
           ++hitIter1) {
        const CartesianVector& positionVector1((*hitIter1)->GetPositionVector());

        // Loop over hits in cluster 2
        for (OrderedCaloHitList::const_iterator iter2 = orderedCaloHitList2.begin(),
                                                iter2End = orderedCaloHitList2.end();
             iter2 != iter2End;
             ++iter2) {
          for (CaloHitList::const_iterator hitIter2 = iter2->second->begin(),
                                           hitIter2End = iter2->second->end();
               hitIter2 != hitIter2End;
               ++hitIter2) {
  ...
  void LArClusterHelper::GetClusterBoundingBox(const Cluster* const pCluster,
                                               CartesianVector& minimumCoordinate,
                                               CartesianVector& maximumCoordinate)
  {
    const OrderedCaloHitList& orderedCaloHitList(pCluster->GetOrderedCaloHitList());
    ...
    for (OrderedCaloHitList::const_iterator ochIter = orderedCaloHitList.begin(),
                                            ochIterEnd = orderedCaloHitList.end();
         ochIter != ochIterEnd;
         ++ochIter) {
      for (CaloHitList::const_iterator hIter = ochIter->second->begin(),
                                       hIterEnd = ochIter->second->end();
           hIter != hIterEnd;
           ++hIter) {
  ...
  StatusCode LArClusterHelper::GetAverageZ(const Cluster* const pCluster,
                                           const float xmin,
                                           const float xmax,
                                           float& averageZ)
  {
    ...
    if (xmin > xmax) return STATUS_CODE_INVALID_PARAMETER;
    ...
    for (OrderedCaloHitList::const_iterator ochIter = orderedCaloHitList.begin(),
      ...
      for (CaloHitList::const_iterator hIter = ochIter->second->begin(),
        ...
        if (hit.GetX() < xmin || hit.GetX() > xmax) continue;
    ...
    if (count == 0) return STATUS_CODE_NOT_FOUND;
  ...
  void LArClusterHelper::GetExtremalCoordinates(const ClusterList& clusterList,
                                                CartesianVector& innerCoordinate,
                                                CartesianVector& outerCoordinate)
  {
    OrderedCaloHitList orderedCaloHitList;

    for (ClusterList::const_iterator cIter = clusterList.begin(), cIterEnd = clusterList.end();
         cIter != cIterEnd;
         ++cIter) {
      const Cluster* const pCluster = *cIter;
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, orderedCaloHitList.Add(pCluster->GetOrderedCaloHitList()));
    }

    return LArClusterHelper::GetExtremalCoordinates(
      orderedCaloHitList, innerCoordinate, outerCoordinate);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArClusterHelper::GetExtremalCoordinates(const Cluster* const pCluster,
                                                CartesianVector& innerCoordinate,
                                                CartesianVector& outerCoordinate)
  {
    return LArClusterHelper::GetExtremalCoordinates(
      pCluster->GetOrderedCaloHitList(), innerCoordinate, outerCoordinate);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArClusterHelper::GetExtremalCoordinates(const OrderedCaloHitList& orderedCaloHitList,
                                                CartesianVector& innerCoordinate,
                                                CartesianVector& outerCoordinate)
  {
    if (orderedCaloHitList.empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
    ...
    std::sort(coordinateVector.begin(),
              coordinateVector.end(),
              LArClusterHelper::SortCoordinatesByPosition);
    return LArClusterHelper::GetExtremalCoordinates(
      coordinateVector, innerCoordinate, outerCoordinate);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArClusterHelper::GetExtremalCoordinates(const CartesianPointVector& coordinateVector,
                                                CartesianVector& innerCoordinate,
                                                CartesianVector& outerCoordinate)
  {
    if (coordinateVector.empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
    ...
    for (CartesianPointVector::const_iterator pIter = coordinateVector.begin(),
                                              pIterEnd = coordinateVector.end();
         pIter != pIterEnd;
         ++pIter) {
    ...
    const bool useX((xSpan > std::numeric_limits<float>::epsilon()) &&
                    (xSpan + std::numeric_limits<float>::epsilon() > std::max(ySpan, zSpan)));
    const bool useY((ySpan > std::numeric_limits<float>::epsilon()) &&
                    (ySpan + std::numeric_limits<float>::epsilon() > std::max(zSpan, xSpan)));
    const bool useZ((zSpan > std::numeric_limits<float>::epsilon()) &&
                    (zSpan + std::numeric_limits<float>::epsilon() > std::max(xSpan, ySpan)));
    ...
    for (CartesianPointVector::const_iterator pIter = coordinateVector.begin(),
      ...
      if (useX) {
        if (((pos.GetX() - xMin) < std::numeric_limits<float>::epsilon()) ||
            ((pos.GetX() - xMax) > -std::numeric_limits<float>::epsilon()))
          candidateVector.push_back(pos);
      }

      if (useY) {
        if (((pos.GetY() - yMin) < std::numeric_limits<float>::epsilon()) ||
            ((pos.GetY() - yMax) > -std::numeric_limits<float>::epsilon()))
          candidateVector.push_back(pos);
      }

      if (useZ) {
        if (((pos.GetZ() - zMin) < std::numeric_limits<float>::epsilon()) ||
            ((pos.GetZ() - zMax) > -std::numeric_limits<float>::epsilon()))
          candidateVector.push_back(pos);
      }
    }
    ...
    for (CartesianPointVector::const_iterator iterI = candidateVector.begin(),
                                              iterEndI = candidateVector.end();
         iterI != iterEndI;
         ++iterI) {
      const CartesianVector& posI = *iterI;

      for (CartesianPointVector::const_iterator iterJ = iterI, iterEndJ = candidateVector.end();
           iterJ != iterEndJ;
           ++iterJ) {
    ...
    if ((deltaZ > 0.f) ||
        ((std::fabs(deltaZ) < std::numeric_limits<float>::epsilon()) && (deltaX > 0.f))) {
  ...
  void LArClusterHelper::GetCoordinateVector(const Cluster* const pCluster,
                                             CartesianPointVector& coordinateVector)
  {
  ...
  void LArClusterHelper::GetCaloHitListInBoundingBox(const pandora::Cluster* const pCluster,
                                                     const pandora::CartesianVector& lowerBound,
                                                     const pandora::CartesianVector& upperBound,
                                                     pandora::CaloHitList& caloHitList)
  {
    const bool useX(std::fabs(upperBound.GetX() - lowerBound.GetX()) >
                    std::numeric_limits<float>::epsilon());
    const bool useY(std::fabs(upperBound.GetY() - lowerBound.GetY()) >
                    std::numeric_limits<float>::epsilon());
    const bool useZ(std::fabs(upperBound.GetZ() - lowerBound.GetZ()) >
                    std::numeric_limits<float>::epsilon());
    if (!useX && !useY && !useZ) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  void LArClusterHelper::GetDaughterVolumeIDs(const Cluster* const pCluster,
                                              UIntSet& daughterVolumeIds)
  {
    ...
    for (OrderedCaloHitList::const_iterator ochIter = orderedCaloHitList.begin(),
      ...
      for (CaloHitList::const_iterator hIter = ochIter->second->begin(),
        ...
        if (pLArCaloHit) daughterVolumeIds.insert(pLArCaloHit->GetDaughterVolumeId());
      }
    }
  }
  //------------------------------------------------------------------------------------------------------------------------------------------
  ...
  bool LArClusterHelper::SortByNOccupiedLayers(const Cluster* const pLhs, const Cluster* const pRhs)
  {
    ...
    if (nOccupiedLayersLhs != nOccupiedLayersRhs) return (nOccupiedLayersLhs > nOccupiedLayersRhs);
  ...
  bool LArClusterHelper::SortByNHits(const Cluster* const pLhs, const Cluster* const pRhs)
  {
    ...
    if (nHitsLhs != nHitsRhs) return (nHitsLhs > nHitsRhs);
  ...
  bool LArClusterHelper::SortByLayerSpan(const Cluster* const pLhs, const Cluster* const pRhs)
  {
    ...
    if (layerSpanLhs != layerSpanRhs) return (layerSpanLhs > layerSpanRhs);
  ...
  bool LArClusterHelper::SortByInnerLayer(const Cluster* const pLhs, const Cluster* const pRhs)
  {
    ...
    if (innerLayerLhs != innerLayerRhs) return (innerLayerLhs < innerLayerRhs);
  ...
  {
    const CartesianVector deltaPositionIL(pRhs->GetCentroid(pRhs->GetInnerPseudoLayer()) -
                                          pLhs->GetCentroid(pLhs->GetInnerPseudoLayer()));
    ...
    const CartesianVector deltaPositionOL(pRhs->GetCentroid(pRhs->GetOuterPseudoLayer()) -
                                          pLhs->GetCentroid(pLhs->GetOuterPseudoLayer()));
  ...
  bool LArClusterHelper::SortHitsByPositionInX(const pandora::CaloHit* const pLhs,
  ...
  bool LArClusterHelper::SortCoordinatesByPosition(const CartesianVector& lhs,
