...
namespace lar_content {
  ...
  std::string LArFileHelper::FindFileInPath(const std::string& unqualifiedFileName,
                                            ...
                                            const std::string& delimiter)
  {
    ...
    if (pFilePathList) XmlHelper::TokenizeString(pFilePathList, filePaths, delimiter);
    ...
    for (const std::string& filePath : filePaths) {
      ...
      if (0 == stat(qualifiedFileNameAttempt.c_str(), &fileInfo)) return qualifiedFileNameAttempt;
    }

    std::cout << "Unable to find file  " << unqualifiedFileName
