...
namespace lar_content {
  ...
  {
    if (LArMCParticleHelper::GetHierarchyTier(pMCParticle) == 0) return MC_PROC_UNKNOWN;
    ...
    if (!pLArMCParticle) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  bool LArMuonLeadingHelper::IsMuonLeading(const MCParticle* const pMCParticle)
  {
    ...
    return ((LArMCParticleHelper::GetHierarchyTier(pMCParticle) == 1) &&
            (LArMCParticleHelper::IsCosmicRay(pParentMCParticle)));
  }
  ...
  {
    if (!pMCParticle) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    while (LArMCParticleHelper::GetHierarchyTier(pParentMCParticle) != 1) {
      ...
      if (parentList.size() != 1) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  void LArMuonLeadingHelper::GetMCToLeadingMap(const MCParticleList* const pMCParticleList,
                                               LArMCParticleHelper::MCRelationMap& mcToLeadingMap)
  {
    for (const MCParticle* const pMCParticle : *pMCParticleList) {
      ...
      if (!LArMCParticleHelper::IsCosmicRay(pParentMCParticle)) continue;

      // For the CRs: fold hits to themselves, for the DRs: fold hits to the leading MCParticle
      if (pMCParticle == pParentMCParticle) { mcToLeadingMap[pMCParticle] = pMCParticle; }
      else {
        const MCParticle* const pLeadingMCParticle(
          LArMuonLeadingHelper::GetLeadingParticle(pMCParticle));
  ...
  void LArMuonLeadingHelper::SelectReconstructableLeadingParticles(
    ...
    LArMCParticleHelper::MCContributionMap& selectedMCParticlesToHitsMap)
  {
    ...
    LArMuonLeadingHelper::SelectCaloHits(pCaloHitList,
                                         mcToLeadingMCMap,
                                         selectedCaloHitList,
    ...
    LArMCParticleHelper::GetMCParticleToCaloHitMatches(
      &selectedCaloHitList, mcToLeadingMCMap, trueHitToLeadingMCMap, leadingMCToTrueHitListMap);

    // Add in close post bremsstrahlung hits
    LArMuonLeadingHelper::AddInPostBremsstrahlungHits(leadingMCParticleToPostBremsstrahlungHitList,
                                                      parameters.m_maxBremsstrahlungSeparation,
                                                      leadingMCToTrueHitListMap);
    ...
    LArMCParticleHelper::SelectParticlesByHitCount(leadingMCVector,
                                                   leadingMCToTrueHitListMap,
                                                   mcToLeadingMCMap,
                                                   parameters,
  ...
  void LArMuonLeadingHelper::SelectCaloHits(
    ...
    LeadingMCParticleToPostBremsstrahlungHitList& leadingMCParticleToPostBremsstrahlungHitList)
  {
    ...
    for (const CaloHit* const pCaloHit : *pCaloHitList) {
      try {
        ...
        if (mcToTargetMCMap.find(pHitParticle) == mcToTargetMCMap.end()) continue;
        ...
        if (!LArMCParticleHelper::IsCosmicRay(mcToTargetMCMap.at(pHitParticle))) {
          if (std::find(recoMuonHitList.begin(), recoMuonHitList.end(), pCaloHit) !=
              recoMuonHitList.end())
            continue;
        }
        ...
        std::sort(mcParticleContributionVector.begin(),
                  mcParticleContributionVector.end(),
                  PointerLessThan<MCParticle>());
        ...
        std::sort(mcTargetContributionVector.begin(),
                  mcTargetContributionVector.end(),
                  PointerLessThan<MCParticle>());
        ...
        for (const MCParticle* const pTargetMCParticle : mcTargetContributionVector) {
          ...
          if (targetWeight > bestTargetWeight) { bestTargetWeight = targetWeight; }
        }

        if ((targetWeightSum < std::numeric_limits<float>::epsilon()) ||
            ((bestTargetWeight / targetWeightSum) < minHitSharingFraction))
          continue;

        // Remove and record post bremsstrahlung hits
        if (LArMuonLeadingHelper::RejectBremsstrahlungHits(
              pCaloHit, leadingMCParticleToPostBremsstrahlungHitList))
          continue;
    ...
    LeadingMCParticleToPostBremsstrahlungHitList& leadingMCParticleToPostBremsstrahlungHitList)
  {
    ...
    for (const MCParticle* const pAncestorMCParticle : ancestorMCParticleList) {
      ...
      if (pAncestorMCParticle->GetParticleId() == PHOTON) isPostBremsstrahlung = true;
    }
  ...
  void LArMuonLeadingHelper::AddInPostBremsstrahlungHits(
  ...
  void LArMuonLeadingHelper::AddInPostBremsstrahlungHits(
    ...
    const float maxBremsstrahlungSeparation,
    LArMCParticleHelper::MCContributionMap& leadingMCToTrueHitListMap,
    const HitType tpcView)
  {
    CaloHitList leadingViewHitList;
    for (const CaloHit* const pCaloHit : leadingMCToTrueHitListMap.at(pLeadingMCParticle)) {
      if (pCaloHit->GetHitType() == tpcView) leadingViewHitList.push_back(pCaloHit);
    }

    if (leadingViewHitList.empty()) return;

    CaloHitList postBremsstrahlungViewHitList;
    for (const CaloHit* const pCaloHit :
         leadingMCParticleToPostBremsstrahlungHitList.at(pLeadingMCParticle)) {
      if (pCaloHit->GetHitType() == tpcView) postBremsstrahlungViewHitList.push_back(pCaloHit);
    }

    if (postBremsstrahlungViewHitList.empty()) return;
    ...
    do {
      ...
      for (const CaloHit* const pPostBremsstrahlungHit : postBremsstrahlungViewHitList) {
        if (std::find(leadingViewHitList.begin(),
                      leadingViewHitList.end(),
                      pPostBremsstrahlungHit) != leadingViewHitList.end())
          continue;

        const float separationDistance(LArClusterHelper::GetClosestDistance(
          pPostBremsstrahlungHit->GetPositionVector(), leadingViewHitList));
  ...
  void LArMuonLeadingHelper::SelectLeadingMCParticles(const MCParticleList* pMCParticleList,
                                                      MCParticleVector& selectedParticles)
  {
    for (const MCParticle* const pMCParticle : *pMCParticleList) {
      ...
      if (pMCParticle == pParentMCParticle) { selectedParticles.push_back(pMCParticle); }
      else {
    ...
    std::sort(
      selectedParticles.begin(), selectedParticles.end(), LArMCParticleHelper::SortByMomentum);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArMuonLeadingHelper::GetPfoMatchContamination(const MCParticle* const pLeadingParticle,
                                                      const CaloHitList& matchedPfoHitList,
                                                      CaloHitList& parentTrackHits,
                                                      CaloHitList& otherTrackHits,
                                                      CaloHitList& otherShowerHits)
  {
    const MCParticle* const pParentCosmicRay(
      LArMCParticleHelper::GetParentMCParticle(pLeadingParticle));

    for (const CaloHit* const pCaloHit : matchedPfoHitList) {
      const MCParticle* const pHitParticle(MCParticleHelper::GetMainMCParticle(pCaloHit));

      if (LArMCParticleHelper::IsCosmicRay(pHitParticle)) {
        (pHitParticle == pParentCosmicRay) ? parentTrackHits.push_back(pCaloHit) :
                                             otherTrackHits.push_back(pCaloHit);
      }
      else {
        const MCParticle* const pHitLeadingParticle(
          LArMuonLeadingHelper::GetLeadingParticle(pHitParticle));

        if (pHitLeadingParticle != pLeadingParticle) otherShowerHits.push_back(pCaloHit);
      }
    }
  }
  ...
  void LArMuonLeadingHelper::GetMuonPfoContaminationContribution(
    const CaloHitList& cosmicRayPfoHitList,
    const CaloHitList& leadingMCHitList,
    CaloHitList& leadingHitsInParentCosmicRay)
  {
    for (const CaloHit* const pCaloHit : cosmicRayPfoHitList) {
      if (std::find(leadingMCHitList.begin(), leadingMCHitList.end(), pCaloHit) !=
          leadingMCHitList.end())
  ...
  float LArMuonLeadingHelper::GetClosestDistance(const Cluster* const pCluster,
                                                 const CartesianPointVector& cartesianPointVector)
  {
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      const float distance(
        LArMuonLeadingHelper::GetClosestDistance(pCaloHit, cartesianPointVector));
  ...
  float LArMuonLeadingHelper::GetClosestDistance(const CaloHit* const pCaloHit,
                                                 const CartesianPointVector& cartesianPointVector)
  {
    ...
    for (const CartesianVector& testPosition : cartesianPointVector) {
      ...
      if (separationSquared < shortestDistanceSquared) shortestDistanceSquared = separationSquared;
    }
  ...
  StatusCode LArMuonLeadingHelper::GetClosestPosition(
    ...
    CartesianVector& closestPosition)
  {
    ...
    for (const CartesianVector& testPosition : cartesianPointVector) {
      ...
      if (separationSquared > maxDistanceToReferencePoint) continue;
  ...
  void LArMuonLeadingHelper::GetClosestPositions(const CartesianPointVector& cartesianPointVector1,
