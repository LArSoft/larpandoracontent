...
namespace lar_content {
    ...
    const unsigned int nPermutations)
  {
    ...
    for (unsigned int iPermutation = 0; iPermutation < nPermutations; ++iPermutation) {
      ...
      if ((rRandomised - rNominal) > std::numeric_limits<float>::epsilon()) nExtreme++;
    }
    ...
    const float upperLimit)
  {
    ...
    if (0 > dof) throw pandora::StatusCodeException(pandora::STATUS_CODE_INVALID_PARAMETER);
    ...
    const float tTestStatistic(correlation * std::sqrt(dof) /
                               (std::sqrt(tTestStatisticDenominator)));
    const float tDistCoeff(std::tgamma(0.5f * (dof + 1.f)) / std::tgamma(0.5f * dof) /
                           (std::sqrt(dof * M_PI)));

    const float dx((upperLimit - tTestStatistic) / static_cast<float>(nIntegrationSteps));
    float integral(tDistCoeff *
                     std::pow(1.f + tTestStatistic * tTestStatistic / dof, -0.5f * (dof + 1.f)) +
    ...
    for (unsigned int iStep = 1; iStep < nIntegrationSteps; ++iStep) {
      integral += 2.f * tDistCoeff *
                  std::pow(1.f + (tTestStatistic + static_cast<float>(iStep) * dx) *
                                   (tTestStatistic + static_cast<float>(iStep) * dx) / dof,
                           -0.5f * (dof + 1.f));
    }
  ...
  float LArDiscreteProbabilityHelper::CalculateCorrelationCoefficient(const T& t1, const T& t2)
  {
    ...
    if (size1 != size2) throw pandora::StatusCodeException(pandora::STATUS_CODE_INVALID_PARAMETER);

    if (2 > size1) throw pandora::StatusCodeException(pandora::STATUS_CODE_INVALID_PARAMETER);
    ...
    if (variance1 < std::numeric_limits<float>::epsilon() ||
        variance2 < std::numeric_limits<float>::epsilon())
      throw pandora::StatusCodeException(pandora::STATUS_CODE_FAILURE);
  ...
  float LArDiscreteProbabilityHelper::CalculateMean(const T& t)
  {
    ...
    if (1 > size) throw pandora::StatusCodeException(pandora::STATUS_CODE_NOT_INITIALIZED);
  ...
  template float
  LArDiscreteProbabilityHelper::CalculateCorrelationCoefficientPValueFromStudentTDistribution(
    const DiscreteProbabilityVector&,
    const DiscreteProbabilityVector&,
    const unsigned int,
    const float);
  ...
  template float LArDiscreteProbabilityHelper::CalculateCorrelationCoefficient(
    const DiscreteProbabilityVector&,
    const DiscreteProbabilityVector&);
  template float LArDiscreteProbabilityHelper::CalculateCorrelationCoefficient(
    const pandora::FloatVector&,
