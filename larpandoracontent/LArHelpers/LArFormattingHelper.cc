...
namespace lar_content {
  ...
  {
    if (column >= m_columnTitles.size()) throw StatusCodeException(STATUS_CODE_OUT_OF_RANGE);
  ...
  {
    if (m_widths.empty() || m_elements.empty()) throw StatusCodeException(STATUS_CODE_OUT_OF_RANGE);

    const unsigned int currentElementIndex(m_elements.size() - 1);
    const unsigned int column(currentElementIndex % m_widths.size());
    m_widths[column] = std::max(static_cast<unsigned int>(m_elements[currentElementIndex].length()),
                                m_widths[column]);
  }
  ...
  {
    if (m_columnTitles.empty()) return;
  ...
  void LArFormattingHelper::Table::PrintColumnTitles() const
  {
    for (unsigned int i = 0, nColumns = m_columnTitles.size(); i < nColumns; ++i)
      this->PrintTableCell(
        m_columnTitles[i], LArFormattingHelper::GetFormatCharacter(LArFormattingHelper::BOLD), i);
  }
  ...
  {
    for (unsigned int i = 0, nColumns = m_columnTitles.size(); i < nColumns; ++i)
      this->PrintTableCell(std::string(m_widths[i], '-'),
                           LArFormattingHelper::GetFormatCharacter(LArFormattingHelper::REGULAR),
                           i);
  }
  ...
  void LArFormattingHelper::Table::PrintTableElements() const
  {
    ...
    const unsigned int nRows((m_elements.size() + m_columnTitles.size() - 1) /
                             m_columnTitles.size());
    const unsigned int nColumns(m_columnTitles.size());
    if (nRows * nColumns != m_elements.size()) {
      std::cout << "LArFormattingHelper::Table::PrintTableElements - Error: Number of table "
                   "elements added doesn't fill a whole row"
                << std::endl;
  ...
  void LArFormattingHelper::Table::PrintTableCell(const std::string& value,
                                                  ...
                                                  const unsigned int index) const
  {
    ...
    std::cout << "|" << format << separator << std::setw(m_widths[column]) << std::internal << value
              << separator << std::resetiosflags(std::ios::adjustfield);
    LArFormattingHelper::ResetStyle();

    if (column == m_columnTitles.size() - 1) std::cout << "|" << std::endl;
  }
