...
namespace lar_content {
  ...
  bool LArMCParticleHelper::DoesPrimaryMeetCriteria(
    const MCParticle* const pMCParticle,
    std::function<bool(const MCParticle* const)> fCriteria)
  {
    try {
      const MCParticle* const pPrimaryMCParticle =
        LArMCParticleHelper::GetPrimaryMCParticle(pMCParticle);
  ...
  bool LArMCParticleHelper::DoesLeadingMeetCriteria(
    const MCParticle* const pMCParticle,
    std::function<bool(const MCParticle* const)> fCriteria)
  {
    try {
      const MCParticle* const pLeadingMCParticle =
        LArMCParticleHelper::GetLeadingMCParticle(pMCParticle);
  ...
  {
    const MCParticle* const pParentMCParticle(
      LArMCParticleHelper::GetParentMCParticle(pMCParticle));
    return (LArMCParticleHelper::IsPrimary(pMCParticle) &&
            LArMCParticleHelper::IsNeutrino(pParentMCParticle));
  }
  ...
  bool LArMCParticleHelper::IsLeadingBeamParticle(const MCParticle* const pMCParticle)
  {
    ...
    const int parentNuance(
      LArMCParticleHelper::GetNuanceCode(LArMCParticleHelper::GetParentMCParticle(pMCParticle)));
    return (LArMCParticleHelper::IsLeading(pMCParticle) &&
            (parentNuance == 2001 || parentNuance == 2000));
  }
  ...
  bool LArMCParticleHelper::IsCosmicRay(const MCParticle* const pMCParticle)
  {
    ...
    return (LArMCParticleHelper::IsPrimary(pMCParticle) &&
            ((nuance == 3000) ||
             ((nuance == 0) && !LArMCParticleHelper::IsBeamNeutrinoFinalState(pMCParticle))));
  }
  ...
  unsigned int LArMCParticleHelper::GetNuanceCode(const MCParticle* const pMCParticle)
  {
    ...
    if (pLArMCParticle) return pLArMCParticle->GetNuanceCode();

    std::cout << "LArMCParticleHelper::GetNuanceCode - Error: Can't cast to LArMCParticle"
              << std::endl;
  ...
  {
    const int nuance(LArMCParticleHelper::GetNuanceCode(pMCParticle));
    if ((nuance == 0) || (nuance == 2000) || (nuance == 2001) || (nuance == 3000)) return false;

    const int absoluteParticleId(std::abs(pMCParticle->GetParticleId()));
    return ((NU_E == absoluteParticleId) || (NU_MU == absoluteParticleId) ||
            (NU_TAU == absoluteParticleId));
  }
  ...
  bool LArMCParticleHelper::IsVisible(const MCParticle* const pMCParticle)
  {
    ...
    if ((E_MINUS == absoluteParticleId) || (MU_MINUS == absoluteParticleId) ||
        ...
        (SIGMA_MINUS == absoluteParticleId) || (SIGMA_PLUS == absoluteParticleId) ||
        (HYPERON_MINUS == absoluteParticleId) || (PROTON == absoluteParticleId) ||
  ...
  void LArMCParticleHelper::GetTrueNeutrinos(const MCParticleList* const pMCParticleList,
                                             MCParticleVector& trueNeutrinos)
  {
    for (const MCParticle* const pMCParticle : *pMCParticleList) {
      if (LArMCParticleHelper::IsNeutrino(pMCParticle)) trueNeutrinos.push_back(pMCParticle);
    }
  ...
  void LArMCParticleHelper::GetTrueTestBeamParticles(const MCParticleList* const pMCParticleList,
                                                     MCParticleVector& trueTestBeamParticles)
  {
    for (const MCParticle* const pMCParticle : *pMCParticleList) {
    ...
    std::sort(trueTestBeamParticles.begin(),
              trueTestBeamParticles.end(),
  ...
  void LArMCParticleHelper::GetAllDescendentMCParticles(
    ...
    pandora::MCParticleList& descendentMCParticleList)
  {
    for (const MCParticle* pDaughterMCParticle : pMCParticle->GetDaughterList()) {
      if (std::find(descendentMCParticleList.begin(),
                    descendentMCParticleList.end(),
                    pDaughterMCParticle) == descendentMCParticleList.end()) {
        descendentMCParticleList.emplace_back(pDaughterMCParticle);
        LArMCParticleHelper::GetAllDescendentMCParticles(pDaughterMCParticle,
                                                         descendentMCParticleList);
      }
    }
  }
  ...
  void LArMCParticleHelper::GetAllDescendentMCParticles(const MCParticle* const pMCParticle,
                                                        MCParticleList& descendentTrackParticles,
                                                        MCParticleList& leadingShowerParticles,
                                                        MCParticleList& leadingNeutrons)
  {
    for (const MCParticle* pDaughterMCParticle : pMCParticle->GetDaughterList()) {
      if (std::find(descendentTrackParticles.begin(),
                    descendentTrackParticles.end(),
                    pDaughterMCParticle) == descendentTrackParticles.end()) {
        ...
        if (pdg == E_MINUS || pdg == PHOTON) {
          ...
          LArMCParticleHelper::GetAllDescendentMCParticles(
            pDaughterMCParticle, descendentTrackParticles, leadingShowerParticles, leadingNeutrons);
        }
      }
    }
  }
  ...
  void LArMCParticleHelper::GetAllAncestorMCParticles(
    const pandora::MCParticle* const pMCParticle,
    pandora::MCParticleList& ancestorMCParticleList)
  {
    ...
    if (parentMCParticleList.size() != 1) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

    const MCParticle* pParentMCParticle = *parentMCParticleList.begin();
    if (std::find(ancestorMCParticleList.begin(),
                  ancestorMCParticleList.end(),
                  pParentMCParticle) == ancestorMCParticleList.end()) {
  ...
  void LArMCParticleHelper::GetPrimaryMCParticleList(const MCParticleList* const pMCParticleList,
                                                     MCParticleVector& mcPrimaryVector)
  {
    for (const MCParticle* const pMCParticle : *pMCParticleList) {
      if (LArMCParticleHelper::IsPrimary(pMCParticle)) mcPrimaryVector.push_back(pMCParticle);
    }
  ...
  void LArMCParticleHelper::GetLeadingMCParticleList(const MCParticleList* const pMCParticleList,
                                                     MCParticleVector& mcLeadingVector)
  {
    for (const MCParticle* const pMCParticle : *pMCParticleList) {
      const bool isBeamParticle(
        LArMCParticleHelper::IsBeamParticle(LArMCParticleHelper::GetParentMCParticle(pMCParticle)));

      if ((isBeamParticle && LArMCParticleHelper::IsLeading(pMCParticle)) ||
          (!isBeamParticle && LArMCParticleHelper::IsPrimary(pMCParticle))) {
  ...
  const MCParticle* LArMCParticleHelper::GetPrimaryMCParticle(const MCParticle* const pMCParticle)
  {
    ...
    for (MCParticleVector::const_reverse_iterator iter = mcVector.rbegin(),
      ...
      if (LArMCParticleHelper::IsVisible(pNextParticle)) return pNextParticle;
    }
  ...
  const MCParticle* LArMCParticleHelper::GetLeadingMCParticle(const MCParticle* const pMCParticle,
                                                              const int hierarchyTierLimit)
  {
    // ATTN: If not beam particle return primary particle
    const bool isBeamParticle(
      LArMCParticleHelper::IsBeamParticle(LArMCParticleHelper::GetParentMCParticle(pMCParticle)));

    if (!isBeamParticle) return LArMCParticleHelper::GetPrimaryMCParticle(pMCParticle);
    ...
    for (MCParticleVector::const_reverse_iterator iter = mcVector.rbegin(),
      ...
      if (!LArMCParticleHelper::IsVisible(pNextParticle)) continue;

      if (hierarchyTier <= hierarchyTierLimit) pLeadingMCParticle = pNextParticle;

      hierarchyTier++;
    }

    if (!pLeadingMCParticle) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  void LArMCParticleHelper::GetMCPrimaryMap(const MCParticleList* const pMCParticleList,
  ...
  void LArMCParticleHelper::GetMCLeadingMap(const MCParticleList* const pMCParticleList,
  ...
  void LArMCParticleHelper::GetMCToSelfMap(const MCParticleList* const pMCParticleList,
  ...
  bool LArMCParticleHelper::SortByMomentum(const MCParticle* const pLhs,
                                           const MCParticle* const pRhs)
  {
  ...
  void LArMCParticleHelper::GetMCParticleToCaloHitMatches(const CaloHitList* const pCaloHitList,
    ...
    for (const CaloHit* const pCaloHit : *pCaloHitList) {
      try {
        ...
        if (!mcToTargetMCMap.empty()) {
          ...
          if (mcToTargetMCMap.end() == mcIter) continue;
  ...
  void LArMCParticleHelper::SelectReconstructableMCParticles(
    const MCParticleList* pMCParticleList,
    ...
    MCContributionMap& selectedMCParticlesToHitsMap)
  {
    ...
    parameters.m_foldBackHierarchy ?
    ...
    LArMCParticleHelper::SelectCaloHits(pCaloHitList,
                                        mcToTargetMCMap,
                                        selectedCaloHitList,
                                        parameters.m_selectInputHits,
                                        parameters.m_maxPhotonPropagation);
    ...
    LArMCParticleHelper::GetMCParticleToCaloHitMatches(
      &selectedCaloHitList, mcToTargetMCMap, trueHitToTargetMCMap, targetMCToTrueHitListMap);
    ...
    if (parameters.m_foldBackHierarchy) {
      ...
      std::copy(
        pMCParticleList->begin(), pMCParticleList->end(), std::back_inserter(targetMCVector));
    }

    // Select MCParticles matching criteria
    MCParticleVector candidateTargets;
    LArMCParticleHelper::SelectParticlesMatchingCriteria(
      targetMCVector, fCriteria, candidateTargets, parameters, false);

    // Ensure the MCParticles have enough "good" hits to be reconstructed
    LArMCParticleHelper::SelectParticlesByHitCount(candidateTargets,
                                                   targetMCToTrueHitListMap,
                                                   mcToTargetMCMap,
                                                   parameters,
  ...
  void LArMCParticleHelper::SelectReconstructableTestBeamHierarchyMCParticles(
    const MCParticleList* pMCParticleList,
    const CaloHitList* pCaloHitList,
    const PrimaryParameters& parameters,
    std::function<bool(const MCParticle* const)> fCriteria,
    MCContributionMap& selectedMCParticlesToHitsMap)
  {
    ...
    LArMCParticleHelper::MCRelationMap mcToTargetMCMap;
    parameters.m_foldBackHierarchy ?
      LArMCParticleHelper::GetMCLeadingMap(pMCParticleList, mcToTargetMCMap) :
      LArMCParticleHelper::GetMCToSelfMap(pMCParticleList, mcToTargetMCMap);
    ...
    MCParticleVector candidateTargets;
    LArMCParticleHelper::SelectParticlesMatchingCriteria(
      targetMCVector, fCriteria, candidateTargets, parameters, true);
    ...
    for (const MCParticle* const pMCParticle : candidateTargets) {
      if (!LArMCParticleHelper::IsBeamParticle(pMCParticle)) continue;

      const MCParticle* const pParentMCParticle(
        LArMCParticleHelper::GetParentMCParticle(pMCParticle));
      if (selectedMCParticlesToHitsMap.find(pParentMCParticle) ==
          selectedMCParticlesToHitsMap.end()) {
        CaloHitList caloHitList;
        selectedMCParticlesToHitsMap.insert(
          MCContributionMap::value_type(pParentMCParticle, caloHitList));
      }
    }
  }
  ...
  void LArMCParticleHelper::GetPfoToReconstructable2DHitsMap(
    ...
    const bool foldBackHierarchy)
  {
    LArMCParticleHelper::GetPfoToReconstructable2DHitsMap(
      pfoList,
      MCContributionMapVector({selectedMCParticleToHitsMap}),
      pfoToReconstructable2DHitsMap,
      foldBackHierarchy);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArMCParticleHelper::GetTestBeamHierarchyPfoToReconstructable2DHitsMap(
    const PfoList& pfoList,
    const MCContributionMap& selectedMCParticleToHitsMap,
    PfoContributionMap& pfoToReconstructable2DHitsMap,
    const bool foldBackHierarchy)
    ...
    const bool foldBackHierarchy)
  {
    for (const ParticleFlowObject* const pPfo : pfoList) {
      ...
      LArMCParticleHelper::CollectReconstructable2DHits(
        pPfo, selectedMCParticleToHitsMaps, pfoHitList, foldBackHierarchy);

      if (!pfoToReconstructable2DHitsMap.insert(PfoContributionMap::value_type(pPfo, pfoHitList))
             .second)
    ...
    const bool foldBackHierarchy)
  {
    for (const ParticleFlowObject* const pPfo : pfoList) {
      CaloHitList pfoHitList;
      LArMCParticleHelper::CollectReconstructableTestBeamHierarchy2DHits(
        pPfo, selectedMCParticleToHitsMaps, pfoHitList, foldBackHierarchy);
  ...
  void LArMCParticleHelper::GetPfoMCParticleHitSharingMaps(
    ...
    MCParticleToPfoHitSharingMap& mcParticleToPfoHitSharingMap)
  {
    ...
    for (const ParticleFlowObject* const pPfo : sortedPfos) {
      for (const MCContributionMap& mcParticleToHitsMap : selectedMCParticleToHitsMaps) {
        ...
        std::sort(
          sortedMCParticles.begin(), sortedMCParticles.end(), PointerLessThan<MCParticle>());
        ...
        for (const MCParticle* const pMCParticle : sortedMCParticles) {
          ...
          if (pfoToMCParticleHitSharingMap.find(pPfo) == pfoToMCParticleHitSharingMap.end())
            if (!pfoToMCParticleHitSharingMap
                   .insert(
                     PfoToMCParticleHitSharingMap::value_type(pPfo, MCParticleToSharedHitsVector()))
                   .second)
          ...
          if (mcParticleToPfoHitSharingMap.find(pMCParticle) == mcParticleToPfoHitSharingMap.end())
            if (!mcParticleToPfoHitSharingMap
                   .insert(
                     MCParticleToPfoHitSharingMap::value_type(pMCParticle, PfoToSharedHitsVector()))
                   .second)
              throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
          ...
          if (std::any_of(
                mcHitPairs.begin(), mcHitPairs.end(), [&](const MCParticleCaloHitListPair& pair) {
                  return (pair.first == pMCParticle);
                }))
            throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);

          if (std::any_of(pfoHitPairs.begin(),
                          pfoHitPairs.end(),
                          [&](const PfoCaloHitListPair& pair) { return (pair.first == pPfo); }))
            throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);

          // Add records to maps if there are any shared hits
          const CaloHitList sharedHits(LArMCParticleHelper::GetSharedHits(
            pfoToReconstructable2DHitsMap.at(pPfo), mcParticleToHitsMap.at(pMCParticle)));
  ...
  void LArMCParticleHelper::GetClusterToReconstructable2DHitsMap(
  ...
  {
    LArMCParticleHelper::GetClusterToReconstructable2DHitsMap(
      clusterList,
      MCContributionMapVector({selectedMCToHitsMap}),
    ...
    ClusterContributionMap& clusterToReconstructable2DHitsMap)
  {
    for (const Cluster* const pCluster : clusterList) {
      CaloHitList caloHitList;
      LArMCParticleHelper::CollectReconstructable2DHits(
        pCluster, selectedMCToHitsMaps, caloHitList);

      if (!clusterToReconstructable2DHitsMap
             .insert(ClusterContributionMap::value_type(pCluster, caloHitList))
             .second)
        throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
    }
  }
  ...
  bool LArMCParticleHelper::IsBremsstrahlung(const MCParticle* const pMCParticle)
  {
    ...
    if (!pLArMCParticle) return false;

    switch (pLArMCParticle->GetProcess()) {
    case MC_PROC_E_BREM:
    case MC_PROC_MU_BREM:
    case MC_PROC_HAD_BREM: return true;
  ...
  bool LArMCParticleHelper::IsCapture(const MCParticle* const pMCParticle)
  {
    ...
    case MC_PROC_HAD_BERTINI_CAPTURE_AT_REST: return true;
    default: return false;
    }
  ...
  bool LArMCParticleHelper::IsDecay(const MCParticle* const pMCParticle)
  {
    ...
    switch (pLArMCParticle->GetProcess()) {
    case MC_PROC_DECAY: return true;
    default: return false;
    }
  ...
  bool LArMCParticleHelper::IsElasticScatter(const MCParticle* const pMCParticle)
  {
    ...
    case MC_PROC_RAYLEIGH: return true;
    default: return false;
    }
  ...
  bool LArMCParticleHelper::IsInelasticScatter(const MCParticle* const pMCParticle)
  {
    ...
    case MC_PROC_TRITON_INELASTIC: return true;
    default: return false;
    }
  ...
  bool LArMCParticleHelper::IsIonisation(const MCParticle* const pMCParticle)
  {
    ...
    case MC_PROC_ION_IONI: return true;
    default: return false;
    }
  ...
  bool LArMCParticleHelper::IsNuclear(const MCParticle* const pMCParticle)
  {
    ...
    case MC_PROC_MU_NUCLEAR: return true;
    default: return false;
    }
  ...
  bool LArMCParticleHelper::IsPairProduction(const MCParticle* const pMCParticle)
  {
    ...
    case MC_PROC_HAD_PAIR_PROD: return true;
    default: return false;
    }
  ...
  CaloHitList LArMCParticleHelper::GetSharedHits(const CaloHitList& hitListA,
                                                 const CaloHitList& hitListB)
  {
  ...
  bool LArMCParticleHelper::AreTopologicallyContinuous(const MCParticle* const pMCParent,
                                                       const MCParticle* const pMCChild,
                                                       const float cosAngleTolerance)
  {
    CartesianVector childDirection{pMCChild->GetEndpoint() - pMCChild->GetVertex()};
    if (childDirection.GetMagnitude() < std::numeric_limits<float>::epsilon()) return true;
  ...
  void LArMCParticleHelper::CollectReconstructable2DHits(
    ...
    const MCContributionMapVector& selectedMCParticleToHitsMaps,
    CaloHitList& reconstructableCaloHitList2D,
    const bool foldBackHierarchy)
  {
    ...
    if (foldBackHierarchy) { LArPfoHelper::GetAllDownstreamPfos(pPfo, pfoList); }
    else {
      pfoList.push_back(pPfo);
    }

    LArMCParticleHelper::CollectReconstructable2DHits(
      pfoList, selectedMCParticleToHitsMaps, reconstructableCaloHitList2D);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArMCParticleHelper::CollectReconstructableTestBeamHierarchy2DHits(
    const ParticleFlowObject* const pPfo,
    const MCContributionMapVector& selectedMCParticleToHitsMaps,
    CaloHitList& reconstructableCaloHitList2D,
    const bool foldBackHierarchy)
    ...
    CaloHitList& reconstructableCaloHitList2D)
  {
    ...
    LArPfoHelper::GetIsolatedCaloHits(
      pfoList, TPC_VIEW_U, caloHitList2D); // TODO check isolated usage throughout
    ...
    for (const CaloHit* const pCaloHit : caloHitList2D) {
      ...
      for (const MCContributionMap& mcParticleToHitsMap : selectedMCParticleToHitsMaps) {
        ...
        for (const MCContributionMap::value_type& mapEntry : mcParticleToHitsMap) {
          if (std::find(mapEntry.second.begin(), mapEntry.second.end(), pCaloHit) !=
              mapEntry.second.end()) {
            isTargetHit = true;
            break;
          }
      ...
      if (isTargetHit) reconstructableCaloHitList2D.push_back(pCaloHit);
    }
  }
    ...
    pandora::CaloHitList& reconstructableCaloHitList2D)
  {
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      bool isTargetHit{false};
      for (
        const MCContributionMap& mcParticleToHitsMap :
        selectedMCToHitsMaps) { // ATTN This map is unordered, but this does not impact search for specific target hit
        for (const MCContributionMap::value_type& mapEntry : mcParticleToHitsMap) {
          if (std::find(mapEntry.second.begin(), mapEntry.second.end(), pCaloHit) !=
              mapEntry.second.end()) {
            isTargetHit = true;
  ...
  void LArMCParticleHelper::SelectCaloHits(
  ...
  {
    if (!selectInputHits) {
      selectedCaloHitList.insert(
        selectedCaloHitList.end(), pCaloHitList->begin(), pCaloHitList->end());
      return;
    }
    ...
    for (const CaloHit* const pCaloHit : *pCaloHitList) {
      try {
        ...
        LArMCParticleHelper::MCRelationMap::const_iterator mcIter =
          mcToTargetMCMap.find(pHitParticle);

        if (mcToTargetMCMap.end() == mcIter) continue;

        // ATTN With folding on or off, still require primary particle to review hierarchy details
        const MCParticle* const pPrimaryParticle =
          LArMCParticleHelper::GetPrimaryMCParticle(pHitParticle);

        if (PassMCParticleChecks(
              pPrimaryParticle, pPrimaryParticle, pHitParticle, maxPhotonPropagation))
  ...
  bool LArMCParticleHelper::IsDescendentOf(const MCParticle* const pMCParticle,
                                           const int pdg,
                                           const bool isChargeSensitive)
  {
    ...
    while (!pCurrentParticle->GetParentList().empty()) {
      ...
      const bool found{isChargeSensitive ? pParent->GetParticleId() == pdg :
  ...
  void LArMCParticleHelper::GetBreadthFirstHierarchyRepresentation(
    const MCParticle* const pMCParticle,
  ...
  void LArMCParticleHelper::SelectParticlesByHitCount(
    const MCParticleVector& candidateTargets,
    ...
    MCContributionMap& selectedMCParticlesToHitsMap)
  {
    ...
    for (const MCParticle* const pMCTarget : candidateTargets) {
      ...
      if (mcToTrueHitListMap.end() == trueHitsIter) continue;
      ...
      LArMCParticleHelper::SelectGoodCaloHits(&caloHitList,
                                              mcToTargetMCMap,
                                              goodCaloHitList,
                                              parameters.m_selectInputHits,
                                              parameters.m_minHitSharingFraction);

      if (goodCaloHitList.size() < parameters.m_minPrimaryGoodHits) continue;

      unsigned int nGoodViews(0);
      if (LArMonitoringHelper::CountHitsByType(TPC_VIEW_U, goodCaloHitList) >=
          parameters.m_minHitsForGoodView)
        ++nGoodViews;

      if (LArMonitoringHelper::CountHitsByType(TPC_VIEW_V, goodCaloHitList) >=
          parameters.m_minHitsForGoodView)
        ++nGoodViews;

      if (LArMonitoringHelper::CountHitsByType(TPC_VIEW_W, goodCaloHitList) >=
          parameters.m_minHitsForGoodView)
        ++nGoodViews;

      if (nGoodViews < parameters.m_minPrimaryGoodViews) continue;

      if (!selectedMCParticlesToHitsMap
             .insert(MCContributionMap::value_type(pMCTarget, caloHitList))
             .second)
        throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
    }
  }
  ...
  void LArMCParticleHelper::SelectGoodCaloHits(
    const CaloHitList* const pSelectedCaloHitList,
    const LArMCParticleHelper::MCRelationMap& mcToTargetMCMap,
    CaloHitList& selectedGoodCaloHitList,
    const bool selectInputHits,
    const float minHitSharingFraction)
  {
    if (!selectInputHits) {
      selectedGoodCaloHitList.insert(
        selectedGoodCaloHitList.end(), pSelectedCaloHitList->begin(), pSelectedCaloHitList->end());
      return;
    }
    ...
    for (const CaloHit* const pCaloHit : *pSelectedCaloHitList) {
      ...
      for (const MCParticle* const pMCParticle : mcParticleVector) {
        ...
        LArMCParticleHelper::MCRelationMap::const_iterator mcIter =
          mcToTargetMCMap.find(pMCParticle);

        if (mcToTargetMCMap.end() != mcIter) targetWeightMap[mcIter->second] += weight;
      }
      ...
      if (!pBestTargetParticle || (targetWeightSum < std::numeric_limits<float>::epsilon()) ||
          ((bestTargetWeight / targetWeightSum) < minHitSharingFraction))
        continue;
  ...
  void LArMCParticleHelper::SelectParticlesMatchingCriteria(
    const MCParticleVector& inputMCParticles,
    std::function<bool(const MCParticle* const)> fCriteria,
    ...
    const bool isTestBeam)
  {
    for (const MCParticle* const pMCParticle : inputMCParticles) {
      if (parameters.m_foldBackHierarchy) {
        if (!fCriteria(pMCParticle)) continue;
      }
      else {
        if (isTestBeam) {
          if (!LArMCParticleHelper::DoesLeadingMeetCriteria(pMCParticle, fCriteria)) continue;
        }
        else {
          if (!LArMCParticleHelper::DoesPrimaryMeetCriteria(pMCParticle, fCriteria)) continue;
        }
      }
  ...
  bool LArMCParticleHelper::PassMCParticleChecks(const MCParticle* const pOriginalPrimary,
                                                 const MCParticle* const pThisMCParticle,
                                                 const MCParticle* const pHitMCParticle,
                                                 const float maxPhotonPropagation)
  {
    if (NEUTRON == std::abs(pThisMCParticle->GetParticleId())) return false;

    if ((PHOTON == pThisMCParticle->GetParticleId()) &&
        (PHOTON != pOriginalPrimary->GetParticleId()) &&
        (E_MINUS != std::abs(pOriginalPrimary->GetParticleId()))) {
      if ((pThisMCParticle->GetEndpoint() - pThisMCParticle->GetVertex()).GetMagnitude() >
          maxPhotonPropagation)
        return false;
    }

    if (pThisMCParticle == pHitMCParticle) return true;

    for (const MCParticle* const pDaughterMCParticle : pThisMCParticle->GetDaughterList()) {
      if (PassMCParticleChecks(
            pOriginalPrimary, pDaughterMCParticle, pHitMCParticle, maxPhotonPropagation))
        return true;
    }
