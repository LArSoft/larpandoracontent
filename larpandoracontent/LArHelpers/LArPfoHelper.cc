...
namespace lar_content {
  ...
  void LArPfoHelper::GetCoordinateVector(const ParticleFlowObject* const pPfo,
  ...
  void LArPfoHelper::GetCaloHits(const PfoList& pfoList,
                                 const HitType& hitType,
  ...
  void LArPfoHelper::GetCaloHits(const ParticleFlowObject* const pPfo,
                                 const HitType& hitType,
                                 CaloHitList& caloHitList)
  {
    ClusterList clusterList;
  ...
  void LArPfoHelper::GetIsolatedCaloHits(const PfoList& pfoList,
                                         const HitType& hitType,
                                         CaloHitList& caloHitList)
  {
    for (const ParticleFlowObject* const pPfo : pfoList)
  ...
  void LArPfoHelper::GetIsolatedCaloHits(const ParticleFlowObject* const pPfo,
                                         const HitType& hitType,
                                         CaloHitList& caloHitList)
  {
    ClusterList clusterList;
  ...
  void LArPfoHelper::GetClusters(const PfoList& pfoList,
                                 const HitType& hitType,
                                 ClusterList& clusterList)
  {
    for (const ParticleFlowObject* const pPfo : pfoList)
  ...
  void LArPfoHelper::GetClusters(const ParticleFlowObject* const pPfo,
                                 const HitType& hitType,
                                 ClusterList& clusterList)
  {
    for (const Cluster* const pCluster : pPfo->GetClusterList()) {
      if (hitType != LArClusterHelper::GetClusterHitType(pCluster)) continue;
  ...
  void LArPfoHelper::GetTwoDClusterList(const ParticleFlowObject* const pPfo,
                                        ClusterList& clusterList)
  {
    for (const Cluster* const pCluster : pPfo->GetClusterList()) {
  ...
  void LArPfoHelper::GetThreeDClusterList(const ParticleFlowObject* const pPfo,
                                          ClusterList& clusterList)
  {
    for (const Cluster* const pCluster : pPfo->GetClusterList()) {
      if (TPC_3D != LArClusterHelper::GetClusterHitType(pCluster)) continue;
  ...
  void LArPfoHelper::GetAllConnectedPfos(const ParticleFlowObject* const pPfo,
                                         PfoList& outputPfoList)
  {
    if (outputPfoList.end() != std::find(outputPfoList.begin(), outputPfoList.end(), pPfo)) return;
  ...
  void LArPfoHelper::GetAllDownstreamPfos(const ParticleFlowObject* const pPfo,
                                          PfoList& outputPfoList)
  {
    if (outputPfoList.end() != std::find(outputPfoList.begin(), outputPfoList.end(), pPfo)) return;
  ...
  void LArPfoHelper::GetAllDownstreamPfos(const pandora::ParticleFlowObject* const pPfo,
                                          ...
                                          pandora::PfoList& outputLeadingShowerPfoList)
  {
    if (LArPfoHelper::IsTrack(pPfo)) {
      ...
      for (const ParticleFlowObject* pChild : pPfo->GetDaughterPfoList()) {
        if (std::find(outputTrackPfoList.begin(), outputTrackPfoList.end(), pChild) ==
            outputTrackPfoList.end()) {
          const int pdg{std::abs(pChild->GetParticleId())};
          if (pdg == E_MINUS) { outputLeadingShowerPfoList.emplace_back(pChild); }
          else {
            outputTrackPfoList.emplace_back(pChild);
            LArPfoHelper::GetAllDownstreamPfos(
              pChild, outputTrackPfoList, outputLeadingShowerPfoList);
          }
        }
      }
    }
  ...
  {
    if (!LArPfoHelper::IsTwoD(pPfo)) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  {
    if (!LArPfoHelper::IsThreeD(pPfo)) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  float LArPfoHelper::GetClosestDistance(const ParticleFlowObject* const pPfo,
                                         const Cluster* const pCluster)
  {
    ...
    for (const Cluster* const pPfoCluster : clusterList) {
      ...
      if (thisDistance < bestDistance) bestDistance = thisDistance;
    }
  ...
  float LArPfoHelper::GetThreeDSeparation(const ParticleFlowObject* const pPfo1,
  ...
  {
    for (const Cluster* const pCluster : pPfo->GetClusterList()) {
      if (TPC_3D != LArClusterHelper::GetClusterHitType(pCluster)) return true;
    }
  ...
  bool LArPfoHelper::IsTrack(const ParticleFlowObject* const pPfo)
  {
    ...
    return ((MU_MINUS == std::abs(pdg)) || (PI_PLUS == std::abs(pdg)) ||
            (PROTON == std::abs(pdg)) || (K_PLUS == std::abs(pdg)));
  }
  ...
  {
    if (pPfo->GetParentPfoList().empty() && !LArPfoHelper::IsNeutrino(pPfo)) return true;

    if (LArPfoHelper::IsNeutrinoFinalState(pPfo)) return true;
  ...
  {
    return ((pPfo->GetParentPfoList().size() == 1) &&
            (LArPfoHelper::IsNeutrino(*(pPfo->GetParentPfoList().begin()))));
  }
  ...
  bool LArPfoHelper::IsNeutrino(const ParticleFlowObject* const pPfo)
  {
    ...
    if ((NU_E == absoluteParticleId) || (NU_MU == absoluteParticleId) ||
  ...
  bool LArPfoHelper::IsTestBeam(const ParticleFlowObject* const pPfo)
  {
    ...
    if (iter != properties.end()) return ((iter->second > 0.f) ? true : false);
  ...
  void LArPfoHelper::GetRecoNeutrinos(const PfoList* const pPfoList, PfoList& recoNeutrinos)
  {
    ...
    for (const ParticleFlowObject* const pPfo : *pPfoList) {
      if (LArPfoHelper::IsNeutrino(pPfo)) recoNeutrinos.push_back(pPfo);
    }
  }
  ...
  const ParticleFlowObject* LArPfoHelper::GetParentNeutrino(const ParticleFlowObject* const pPfo)
  {
    ...
    if (!LArPfoHelper::IsNeutrino(pParentPfo)) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  {
    if (pPfo->GetVertexList().empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
    ...
    if (LArPfoHelper::IsTestBeam(pPfo) && pPfo->GetParentPfoList().empty()) {
      ...
      if (pPfo->GetVertexList().size() != 1) throw StatusCodeException(STATUS_CODE_FAILURE);

      pVertex = *(pPfo->GetVertexList().begin());
    }

    if (VERTEX_3D != pVertex->GetVertexType()) throw StatusCodeException(STATUS_CODE_FAILURE);
  ...
  {
    if (pPfo->GetVertexList().empty() || !pPfo->GetParentPfoList().empty() ||
        !LArPfoHelper::IsTestBeam(pPfo))
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

    const Vertex* pInteractionVertex(
      LArPfoHelper::GetVertexWithLabel(pPfo->GetVertexList(), VERTEX_INTERACTION));
  ...
  const Vertex* LArPfoHelper::GetVertexWithLabel(const VertexList& vertexList,
                                                 const VertexLabel vertexLabel)
  {
    ...
    for (const Vertex* pCandidateVertex : vertexList) {
      if (pCandidateVertex->GetVertexLabel() == vertexLabel) {
        if (!pTargetVertex) { pTargetVertex = pCandidateVertex; }
        else {
          throw StatusCodeException(STATUS_CODE_FAILURE);
        }
      }
    ...
    if (!pTargetVertex) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  void LArPfoHelper::GetSlidingFitTrajectory(const CartesianPointVector& pointVector,
                                             ...
                                             IntVector* const pIndexVector)
  {
    LArPfoHelper::SlidingFitTrajectoryImpl(
      &pointVector, vertexPosition, layerWindow, layerPitch, trackStateVector, pIndexVector);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArPfoHelper::GetSlidingFitTrajectory(const ParticleFlowObject* const pPfo,
                                             ...
                                             LArTrackStateVector& trackStateVector)
  {
    ...
    LArPfoHelper::SlidingFitTrajectoryImpl(
      &caloHitList, pVertex->GetPosition(), layerWindow, layerPitch, trackStateVector);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  LArShowerPCA LArPfoHelper::GetPrincipalComponents(const CartesianPointVector& pointVector,
                                                    const CartesianVector& vertexPosition)
  {
  ...
  LArShowerPCA LArPfoHelper::GetPrincipalComponents(const ParticleFlowObject* const pPfo,
                                                    const Vertex* const pVertex)
  {
  ...
  bool LArPfoHelper::SortByHitProjection(const LArTrackTrajectoryPoint& lhs,
  ...
  bool LArPfoHelper::SortByNHits(const ParticleFlowObject* const pLhs,
                                 const ParticleFlowObject* const pRhs)
  {
    unsigned int nTwoDHitsLhs(0), nThreeDHitsLhs(0);
    float energyLhs(0.f);
    for (ClusterList::const_iterator iter = pLhs->GetClusterList().begin(),
                                     iterEnd = pLhs->GetClusterList().end();
         iter != iterEnd;
         ++iter) {
    ...
    float energyRhs(0.f);
    for (ClusterList::const_iterator iter = pRhs->GetClusterList().begin(),
                                     iterEnd = pRhs->GetClusterList().end();
         iter != iterEnd;
         ++iter) {
    ...
    if (nTwoDHitsLhs != nTwoDHitsRhs) return (nTwoDHitsLhs > nTwoDHitsRhs);

    if (nThreeDHitsLhs != nThreeDHitsRhs) return (nThreeDHitsLhs > nThreeDHitsRhs);
  ...
  void LArPfoHelper::GetBreadthFirstHierarchyRepresentation(
    ...
    pandora::PfoList& pfoList)
  {
    ...
    while (!parents.empty()) {
      if (parents.size() > 1) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
      pRoot = parents.front();
  ...
  template <typename T>
  void LArPfoHelper::SlidingFitTrajectoryImpl(const T* const pT,
                                              const CartesianVector& vertexPosition,
                                              const unsigned int layerWindow,
                                              const float layerPitch,
                                              LArTrackStateVector& trackStateVector,
                                              IntVector* const pIndexVector)
  {
    ...
    if (pointVector.empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
    ...
    try {
      ...
      const float scaleFactor(
        (seedDirection.GetDotProduct(seedPosition - vertexPosition) > 0.f) ? +1.f : -1.f);
      ...
      for (const auto& nextPoint : *pT) {
        ...
        try {
          const float rL(slidingFitResult.GetLongitudinalDisplacement(
            LArObjectHelper::TypeAdaptor::GetPosition(nextPoint)));
          ...
          trackTrajectory.push_back(LArTrackTrajectoryPoint(
            projection * scaleFactor,
            LArTrackState(position,
                          direction * scaleFactor,
                          LArObjectHelper::TypeAdaptor::GetCaloHit(nextPoint)),
            index));
        }
      ...
      if (STATUS_CODE_FAILURE == statusCodeException2.GetStatusCode()) throw statusCodeException2;
    }
  ...
  template void LArPfoHelper::SlidingFitTrajectoryImpl(const CartesianPointVector* const,
                                                       const CartesianVector&,
                                                       const unsigned int,
                                                       const float,
                                                       LArTrackStateVector&,
                                                       IntVector* const);
  template void LArPfoHelper::SlidingFitTrajectoryImpl(const CaloHitList* const,
                                                       const CartesianVector&,
                                                       const unsigned int,
                                                       const float,
                                                       LArTrackStateVector&,
