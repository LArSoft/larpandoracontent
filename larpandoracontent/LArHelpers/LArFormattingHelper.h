 ...
 */
  ...
  public:
    ...
    private:
      ...
      /**
         ...
         *  @param  widths a vector of the number of characters to include in each column
         */
      void PrintTableCell(const std::string& value,
      ...
      const unsigned int
        m_precision; ///< The number of significant figures to use when displaying number types
      ...
      std::vector<unsigned int>
        m_widths; ///< The widths of each column (in units of number of characters)
  ...
  template <typename T>
  inline void LArFormattingHelper::Table::AddElement(const T& value,
                                                     const Style style,
                                                     const Color color)
  {
    ...
    m_format.push_back(LArFormattingHelper::GetFormatCharacter(style) +
                       LArFormattingHelper::GetFormatCharacter(color));
    m_stringstream.str("");
