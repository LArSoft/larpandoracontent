...
namespace lar_content {
  ...
  template <typename T>
  void LArPcaHelper::RunPca(const T& t,
                            ...
                            EigenVectors& outputEigenVectors)
  {
    ...
    for (const auto& point : t)
      weightedPointVector.push_back(
        std::make_pair(LArObjectHelper::TypeAdaptor::GetPosition(point), 1.));

    return LArPcaHelper::RunPca(
      weightedPointVector, centroid, outputEigenValues, outputEigenVectors);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArPcaHelper::RunPca(const WeightedPointVector& pointVector,
                            CartesianVector& centroid,
                            EigenValues& outputEigenValues,
                            EigenVectors& outputEigenVectors)
  {
    ...
    if (eigenMat.info() != Eigen::ComputationInfo::Success) {
      std::cout << "LArPcaHelper::RunPca - decomposition failure, nThreeDHits = "
                << pointVector.size() << std::endl;
    ...
    std::sort(eigenValColVector.begin(),
              eigenValColVector.end(),
              [](const EigenValColPair& left, const EigenValColPair& right) {
                return left.first > right.first;
              });

    // Get the eigen values
    outputEigenValues = CartesianVector(
      eigenValColVector.at(0).first, eigenValColVector.at(1).first, eigenValColVector.at(2).first);
    ...
    for (const EigenValColPair& pair : eigenValColVector)
      outputEigenVectors.emplace_back(
        eigenVecs(0, pair.second), eigenVecs(1, pair.second), eigenVecs(2, pair.second));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  template void LArPcaHelper::RunPca(const CartesianPointVector&,
                                     CartesianVector&,
                                     EigenValues&,
                                     EigenVectors&);
  template void LArPcaHelper::RunPca(const CaloHitList&,
                                     CartesianVector&,
                                     EigenValues&,
                                     EigenVectors&);
