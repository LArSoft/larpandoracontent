...
namespace lar_content {
  ...
  float LArGeometryHelper::MergeTwoPositions(const Pandora& pandora,
                                             const HitType view1,
                                             const HitType view2,
                                             const float position1,
                                             const float position2)
  {
    if (view1 == view2) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  CartesianVector LArGeometryHelper::MergeTwoDirections(const Pandora& pandora,
                                                        const HitType view1,
                                                        const HitType view2,
                                                        ...
                                                        const CartesianVector& direction2)
  {
    ...
    const float s1((std::fabs(direction1.GetX()) > std::numeric_limits<float>::epsilon()) ?
                     100.f * std::fabs(direction2.GetX()) :
                     1.f);
    const float s2((std::fabs(direction2.GetX()) > std::numeric_limits<float>::epsilon()) ?
                     100.f * std::fabs(direction1.GetX()) :
                     1.f);
    ...
    if (newView == TPC_VIEW_W)
      return CartesianVector(
               pX, 0.f, pandora.GetPlugins()->GetLArTransformationPlugin()->UVtoW(pU, pV))
        .GetUnitVector();

    if (newView == TPC_VIEW_U)
      return CartesianVector(
               pX, 0.f, pandora.GetPlugins()->GetLArTransformationPlugin()->VWtoU(pV, pW))
        .GetUnitVector();

    if (newView == TPC_VIEW_V)
      return CartesianVector(
               pX, 0.f, pandora.GetPlugins()->GetLArTransformationPlugin()->WUtoV(pW, pU))
        .GetUnitVector();
  ...
  void LArGeometryHelper::MergeTwoPositions(const Pandora& pandora,
                                            const HitType view1,
                                            const HitType view2,
                                            const CartesianVector& position1,
                                            const CartesianVector& position2,
                                            CartesianVector& position3,
                                            float& chiSquared)
  {
    if (view1 == view2) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    ...
    chiSquared = ((X3 - position1.GetX()) * (X3 - position1.GetX()) +
                  (X3 - position2.GetX()) * (X3 - position2.GetX())) /
                 (sigmaUVW * sigmaUVW);
  }
  ...
  void LArGeometryHelper::MergeTwoPositions(const Pandora& pandora,
                                            ...
                                            const CartesianVector& position1,
                                            const CartesianVector& position2,
                                            CartesianVector& outputU,
                                            CartesianVector& outputV,
                                            CartesianVector& outputW,
                                            float& chiSquared)
  {
    if (view1 == view2) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    ...
    LArGeometryHelper::MergeTwoPositions(
      pandora, view1, view2, position1, position2, position3, chiSquared);

    float aveU(0.f), aveV(0.f), aveW(0.f);
    const float Z1(position1.GetZ()), Z2(position2.GetZ()), Z3(position3.GetZ()),
      aveX(position3.GetX());
  ...
  void LArGeometryHelper::MergeThreePositions(const Pandora& pandora,
                                              const HitType view1,
                                              const HitType view2,
                                              const HitType view3,
                                              const CartesianVector& position1,
                                              const CartesianVector& position2,
                                              const CartesianVector& position3,
                                              CartesianVector& outputU,
                                              CartesianVector& outputV,
                                              CartesianVector& outputW,
                                              float& chiSquared)
  {
    ...
    if ((view1 == TPC_VIEW_U) && (view2 == TPC_VIEW_V)) {
      return LArGeometryHelper::MergeThreePositions(
        pandora, position1, position2, position3, outputU, outputV, outputW, chiSquared);
    }

    if ((view1 == TPC_VIEW_V) && (view2 == TPC_VIEW_W)) {
      return LArGeometryHelper::MergeThreePositions(
        pandora, position3, position1, position2, outputU, outputV, outputW, chiSquared);
    }

    if ((view1 == TPC_VIEW_W) && (view2 == TPC_VIEW_U)) {
      return LArGeometryHelper::MergeThreePositions(
        pandora, position2, position3, position1, outputU, outputV, outputW, chiSquared);
    }

    if ((view1 == TPC_VIEW_V) && (view2 == TPC_VIEW_U)) {
      return LArGeometryHelper::MergeThreePositions(
        pandora, position2, position1, position3, outputU, outputV, outputW, chiSquared);
    }

    if ((view1 == TPC_VIEW_W) && (view2 == TPC_VIEW_V)) {
      return LArGeometryHelper::MergeThreePositions(
        pandora, position3, position2, position1, outputU, outputV, outputW, chiSquared);
    }

    if ((view1 == TPC_VIEW_U) && (view2 == TPC_VIEW_W)) {
      return LArGeometryHelper::MergeThreePositions(
        pandora, position1, position3, position2, outputU, outputV, outputW, chiSquared);
    }
  ...
  void LArGeometryHelper::MergeThreePositions(const Pandora& pandora,
                                              ...
                                              const CartesianVector& positionW,
                                              CartesianVector& outputU,
                                              CartesianVector& outputV,
                                              CartesianVector& outputW,
                                              float& chiSquared)
  {
    const float YfromUV(pandora.GetPlugins()->GetLArTransformationPlugin()->UVtoY(
      positionU.GetZ(), positionV.GetZ()));
    const float YfromUW(pandora.GetPlugins()->GetLArTransformationPlugin()->UWtoY(
      positionU.GetZ(), positionW.GetZ()));
    const float YfromVW(pandora.GetPlugins()->GetLArTransformationPlugin()->VWtoY(
      positionV.GetZ(), positionW.GetZ()));

    const float ZfromUV(pandora.GetPlugins()->GetLArTransformationPlugin()->UVtoZ(
      positionU.GetZ(), positionV.GetZ()));
    const float ZfromUW(pandora.GetPlugins()->GetLArTransformationPlugin()->UWtoZ(
      positionU.GetZ(), positionW.GetZ()));
    const float ZfromVW(pandora.GetPlugins()->GetLArTransformationPlugin()->VWtoZ(
      positionV.GetZ(), positionW.GetZ()));

    // ATTN For detectors where w and z are equivalent, remain consistent with original treatment. TODO Use new treatment always.
    const bool useOldWZEquivalentTreatment(std::fabs(ZfromUW - ZfromVW) <
    ...
    const float aveZ(useOldWZEquivalentTreatment ? (positionW.GetZ() + 2.f * ZfromUV) / 3.f :
                                                   (ZfromUV + ZfromUW + ZfromVW) / 3.f);
  ...
  void LArGeometryHelper::MergeTwoPositions3D(const Pandora& pandora,
                                              const HitType view1,
                                              const HitType view2,
                                              const CartesianVector& position1,
                                              const CartesianVector& position2,
                                              CartesianVector& position3D,
                                              float& chiSquared)
  {
    CartesianVector positionU(0.f, 0.f, 0.f), positionV(0.f, 0.f, 0.f), positionW(0.f, 0.f, 0.f);
    LArGeometryHelper::MergeTwoPositions(
      pandora, view1, view2, position1, position2, positionU, positionV, positionW, chiSquared);

    position3D.SetValues(
      positionW.GetX(),
      pandora.GetPlugins()->GetLArTransformationPlugin()->UVtoY(positionU.GetZ(), positionV.GetZ()),
      pandora.GetPlugins()->GetLArTransformationPlugin()->UVtoZ(positionU.GetZ(),
                                                                positionV.GetZ()));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LArGeometryHelper::MergeThreePositions3D(const Pandora& pandora,
                                                const HitType view1,
                                                const HitType view2,
                                                const HitType view3,
                                                const CartesianVector& position1,
  ...
  {
    CartesianVector positionU(0.f, 0.f, 0.f), positionV(0.f, 0.f, 0.f), positionW(0.f, 0.f, 0.f);
    LArGeometryHelper::MergeThreePositions(pandora,
                                           view1,
                                           view2,
                                           view3,
                                           position1,
  ...
  CartesianVector LArGeometryHelper::ProjectPosition(const Pandora& pandora,
                                                     const CartesianVector& position3D,
                                                     const HitType view)
  {
    if (view == TPC_VIEW_U) {
      return CartesianVector(position3D.GetX(),
                             0.f,
                             pandora.GetPlugins()->GetLArTransformationPlugin()->YZtoU(
                               position3D.GetY(), position3D.GetZ()));
    }

    else if (view == TPC_VIEW_V) {
      return CartesianVector(position3D.GetX(),
                             0.f,
                             pandora.GetPlugins()->GetLArTransformationPlugin()->YZtoV(
                               position3D.GetY(), position3D.GetZ()));
    }
      ...
      return CartesianVector(position3D.GetX(),
                             0.f,
                             pandora.GetPlugins()->GetLArTransformationPlugin()->YZtoW(
                               position3D.GetY(), position3D.GetZ()));
    }
  ...
  CartesianVector LArGeometryHelper::ProjectDirection(const Pandora& pandora,
                                                      const CartesianVector& direction3D,
                                                      const HitType view)
  {
    if (view == TPC_VIEW_U) {
      return CartesianVector(direction3D.GetX(),
                             0.f,
                             pandora.GetPlugins()->GetLArTransformationPlugin()->YZtoU(
                               direction3D.GetY(), direction3D.GetZ()))
        .GetUnitVector();
    }
  ...
  float LArGeometryHelper::GetWirePitch(const Pandora& pandora,
                                        const HitType view,
                                        const float maxWirePitchDiscrepancy)
  {
    ...
    if (larTPCMap.empty()) {
      std::cout << "LArGeometryHelper::GetWirePitch - LArTPC description not registered with "
                   "Pandora as required "
                << std::endl;
    ...
    const float wirePitch(
      view == TPC_VIEW_U ?
        pFirstLArTPC->GetWirePitchU() :
    ...
    for (const LArTPCMap::value_type& mapEntry : larTPCMap) {
      ...
      const float alternateWirePitch(
        view == TPC_VIEW_U ?
          pLArTPC->GetWirePitchU() :
          (view == TPC_VIEW_V ? pLArTPC->GetWirePitchV() : pLArTPC->GetWirePitchW()));

      if (std::fabs(wirePitch - alternateWirePitch) > maxWirePitchDiscrepancy) {
        std::cout << "LArGeometryHelper::GetWirePitch - LArTPC configuration not supported"
                  << std::endl;
        throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
      }
    }
  ...
  void LArGeometryHelper::GetCommonDaughterVolumes(const Cluster* const pCluster1,
                                                   ...
                                                   UIntSet& intersect)
  {
    ...
    std::set_intersection(daughterVolumeIds1.begin(),
                          daughterVolumeIds1.end(),
                          daughterVolumeIds2.begin(),
                          daughterVolumeIds2.end(),
                          std::inserter(intersect, intersect.begin()));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool LArGeometryHelper::IsInGap(const Pandora& pandora,
                                  ...
                                  const float gapTolerance)
  {
    ...
    for (const DetectorGap* const pDetectorGap : pandora.GetGeometry()->GetDetectorGapList()) {
      if (pDetectorGap->IsInGap(testPoint2D, hitType, gapTolerance)) return true;
    }
  ...
  bool LArGeometryHelper::IsInGap3D(const Pandora& pandora,
                                    const CartesianVector& testPoint3D,
                                    const HitType hitType,
                                    const float gapTolerance)
  {
    const CartesianVector testPoint2D(
      LArGeometryHelper::ProjectPosition(pandora, testPoint3D, hitType));
    return LArGeometryHelper::IsInGap(pandora, testPoint2D, hitType, gapTolerance);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool LArGeometryHelper::IsXSamplingPointInGap(const Pandora& pandora,
                                                const float xSample,
                                                ...
                                                const float gapTolerance)
  {
    ...
    if ((xSample > lowXCoordinate.GetX()) && (xSample < highXCoordinate.GetX())) {
      ...
      if (STATUS_CODE_SUCCESS ==
          slidingFitResult.GetGlobalFitPositionAtX(xSample, slidingFitPosition))
        return (LArGeometryHelper::IsInGap(pandora, slidingFitPosition, hitType, gapTolerance));
    }

    const CartesianVector lowXDirection(minLayerIsAtLowX ?
                                          slidingFitResult.GetGlobalMinLayerDirection() :
                                          slidingFitResult.GetGlobalMaxLayerDirection());
    const CartesianVector highXDirection(minLayerIsAtLowX ?
                                           slidingFitResult.GetGlobalMaxLayerDirection() :
                                           slidingFitResult.GetGlobalMinLayerDirection());

    const bool sampleIsNearerToLowX(std::fabs(xSample - lowXCoordinate.GetX()) <
                                    std::fabs(xSample - highXCoordinate.GetX()));
  ...
  float LArGeometryHelper::CalculateGapDeltaZ(const Pandora& pandora,
                                              const float minZ,
                                              const float maxZ,
                                              const HitType hitType)
  {
    ...
    for (const DetectorGap* const pDetectorGap : pandora.GetGeometry()->GetDetectorGapList()) {
      ...
      if ((pLineGap->GetLineStartZ() > maxZ) || (pLineGap->GetLineEndZ() < minZ)) continue;
  ...
  float LArGeometryHelper::GetSigmaUVW(const Pandora& pandora, const float maxSigmaDiscrepancy)
  {
    ...
    if (larTPCMap.empty()) {
      std::cout << "LArGeometryHelper::GetSigmaUVW - LArTPC description not registered with "
                   "Pandora as required "
                << std::endl;
      throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);
    }
    ...
    for (const LArTPCMap::value_type& mapEntry : larTPCMap) {
      ...
      if (std::fabs(sigmaUVW - pLArTPC->GetSigmaUVW()) > maxSigmaDiscrepancy) {
        std::cout << "LArGeometryHelper::GetSigmaUVW - Plugin does not support provided LArTPC "
                     "configurations "
                  << std::endl;
        throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
      }
