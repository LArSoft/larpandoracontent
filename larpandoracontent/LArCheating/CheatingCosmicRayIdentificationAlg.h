 ...
 */
  ...
  private:
    ...
    std::string
      m_inputDaughterPfoListName; ///< The input daughter pfo list name (if not specified, will assume same as main input list)
    std::string
      m_outputDaughterPfoListName; ///< The output daughter pfo list name (if not specified, will assume same as main output list)
    float
      m_maxNeutrinoFraction; ///< The maximum true neutrino fraction in a particle to be labelled as a cosmic ray
  };
