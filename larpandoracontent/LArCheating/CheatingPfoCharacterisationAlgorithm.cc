...
namespace lar_content {
  ...
  bool CheatingPfoCharacterisationAlgorithm::IsClearTrack(
    const ParticleFlowObject* const pPfo) const
  {
    ...
    if (!pBestMCParticle) return false;

    return ((PHOTON != pBestMCParticle->GetParticleId()) &&
            (E_MINUS != std::abs(pBestMCParticle->GetParticleId())));
  }
