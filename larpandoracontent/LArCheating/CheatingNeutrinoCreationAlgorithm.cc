...
namespace lar_content {
  ...
  CheatingNeutrinoCreationAlgorithm::CheatingNeutrinoCreationAlgorithm()
    : m_collapseToPrimaryMCParticles(false), m_vertexTolerance(0.5f)
  {}
  ...
  StatusCode CheatingNeutrinoCreationAlgorithm::Run()
  {
    ...
    for (const MCParticle* const pMCNeutrino : mcNeutrinoVector) {
      ...
      if (!pNeutrinoPfo) return STATUS_CODE_FAILURE;
  ...
  void CheatingNeutrinoCreationAlgorithm::GetMCNeutrinoVector(
    MCParticleVector& mcNeutrinoVector) const
  {
    ...
    for (const MCParticle* const pMCNeutrino : allMCNeutrinoVector) {
      if (LArMCParticleHelper::IsNeutrino(pMCNeutrino) &&
          (MC_3D == pMCNeutrino->GetMCParticleType()))
  ...
  void CheatingNeutrinoCreationAlgorithm::CreateAndSaveNeutrinoPfo(
    const MCParticle* const pMCNeutrino,
    const ParticleFlowObject*& pNeutrinoPfo) const
  {
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::ParticleFlowObject::Create(*this, pfoParameters, pNeutrinoPfo));

    if (!pNeutrinoPfoList || pNeutrinoPfoList->empty())
      throw StatusCodeException(STATUS_CODE_FAILURE);

    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::SaveList<Pfo>(*this, m_neutrinoPfoListName));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::ReplaceCurrentList<Pfo>(*this, m_neutrinoPfoListName));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void CheatingNeutrinoCreationAlgorithm::AddNeutrinoVertex(
    const MCParticle* const pMCNeutrino,
    const ParticleFlowObject* const pNeutrinoPfo) const
  {
    ...
    if (!pNeutrinoVertex || (VERTEX_3D != pNeutrinoVertex->GetVertexType()) ||
        ((pNeutrinoVertex->GetPosition() - pMCNeutrino->GetEndpoint()).GetMagnitude() >
         m_vertexTolerance))
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToPfo(*this, pNeutrinoPfo, pNeutrinoVertex));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void CheatingNeutrinoCreationAlgorithm::GetMCParticleToDaughterPfoMap(
    MCParticleToPfoMap& mcParticleToPfoMap) const
  {
    ...
    for (const std::string& daughterPfoListName : m_daughterPfoListNames) {
      ...
      if (STATUS_CODE_SUCCESS !=
          PandoraContentApi::GetList(*this, daughterPfoListName, pDaughterPfoList)) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "CheatingNeutrinoCreationAlgorithm: pfo list " << daughterPfoListName
      ...
      for (const ParticleFlowObject* const pDaughterPfo : *pDaughterPfoList) {
        ...
        if (!mcParticleToPfoMap.insert(MCParticleToPfoMap::value_type(pMCParticle, pDaughterPfo))
               .second)
  ...
  void CheatingNeutrinoCreationAlgorithm::CreatePfoHierarchy(
  ...
  StatusCode CheatingNeutrinoCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "VertexListName", m_vertexListName));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexTolerance", m_vertexTolerance));
