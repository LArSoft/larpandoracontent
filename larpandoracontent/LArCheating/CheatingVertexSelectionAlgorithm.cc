...
namespace lar_content {
  ...
  void CheatingVertexSelectionAlgorithm::GetVertexScoreList(const VertexVector& vertexVector,
                                                            ...
                                                            VertexScoreList& vertexScoreList) const
  {
    ...
    if (pBestVertex) vertexScoreList.emplace_back(pBestVertex, 1.0f);
  }
  ...
  StatusCode CheatingVertexSelectionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    if (m_mcParticleListName.empty()) {
      std::cout << "CheatingVertexSelectionAlgorithm: MCParticleListName required for cheated "
                   "vertex selection"
                << std::endl;
