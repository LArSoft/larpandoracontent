...
namespace lar_content {
  ...
  StatusCode CheatingCosmicRayIdentificationAlg::Run()
  {
    ...
    if (!pPfoList) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "CheatingCosmicRayIdentificationAlg: pfo list " << m_inputPfoListName
    ...
    for (const ParticleFlowObject* const pPfo : *pPfoList) {
      ...
      if ((thisTotalWeight < std::numeric_limits<float>::epsilon()) ||
          ((thisNeutrinoWeight / thisTotalWeight) < m_maxNeutrinoFraction))
    ...
    if (!outputPfoList.empty()) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::SaveList(*this, m_inputPfoListName, m_outputPfoListName, outputPfoList));
      ...
      if (!outputDaughterPfoList.empty())
        PANDORA_RETURN_RESULT_IF(
          ...
          PandoraContentApi::SaveList(
            *this, m_inputDaughterPfoListName, m_outputDaughterPfoListName, outputDaughterPfoList));
    }
  ...
  StatusCode CheatingCosmicRayIdentificationAlg::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxNeutrinoFraction", m_maxNeutrinoFraction));
