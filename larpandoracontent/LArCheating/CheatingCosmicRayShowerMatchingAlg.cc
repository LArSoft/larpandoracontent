...
namespace lar_content {
  ...
  StatusCode CheatingCosmicRayShowerMatchingAlg::Run()
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetList(*this, m_inputPfoListName, pPfoList));
  ...
  void CheatingCosmicRayShowerMatchingAlg::GetCandidateClusters(
    ClusterList& candidateClusterList) const
  {
    for (const std::string& clusterListName : m_inputClusterListNames) {
      const ClusterList* pClusterList(nullptr);

      if (STATUS_CODE_SUCCESS != PandoraContentApi::GetList(*this, clusterListName, pClusterList)) {
        std::cout << "CheatingCosmicRayShowerMatchingAlg - Could not access cluster list with name "
                  << clusterListName << std::endl;
        continue;
      }

      candidateClusterList.insert(
        candidateClusterList.end(), pClusterList->begin(), pClusterList->end());
    }
  }
  ...
  void CheatingCosmicRayShowerMatchingAlg::CosmicRayShowerMatching(
    ...
    const Cluster* const pPfoCluster,
    const ClusterList& candidateClusterList) const
  {
    try {
      ...
      const MCParticle* const pPfoParentMCParticle(
        LArMCParticleHelper::GetParentMCParticle(pPfoMCParticle));

      for (const Cluster* const pCandidateCluster : candidateClusterList) {
        if (pfoClusterHitType != LArClusterHelper::GetClusterHitType(pCandidateCluster)) continue;

        if (!PandoraContentApi::IsAvailable(*this, pCandidateCluster)) continue;

        if (pPfoCluster == pCandidateCluster) continue;

        try {
          const MCParticle* const pMCParticle(
            MCParticleHelper::GetMainMCParticle(pCandidateCluster));
          ...
          if (!LArMCParticleHelper::IsNeutrino(pParentMCParticle) &&
              (pPfoParentMCParticle == pParentMCParticle))
            PANDORA_THROW_RESULT_IF(
              STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToPfo(*this, pPfo, pCandidateCluster));
        }
