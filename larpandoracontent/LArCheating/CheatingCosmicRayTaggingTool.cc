...
namespace lar_content {
  ...
  void CheatingCosmicRayTaggingTool::FindAmbiguousPfos(const PfoList& parentCosmicRayPfos,
                                                       ...
                                                       const MasterAlgorithm* const /*pAlgorithm*/)
  {
    ...
    for (const Pfo* const pParentCosmicRayPfo : parentCosmicRayPfos) {
      ...
      CheatingSliceIdBaseTool::GetTargetParticleWeight(
        &downstreamPfos, thisCosmicRayWeight, thisTotalWeight, LArMCParticleHelper::IsCosmicRay);

      if ((thisTotalWeight > 0.f) &&
          ((thisCosmicRayWeight / thisTotalWeight) < m_maxCosmicRayFraction))
  ...
  StatusCode CheatingCosmicRayTaggingTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxCosmicRayFraction", m_maxCosmicRayFraction));
