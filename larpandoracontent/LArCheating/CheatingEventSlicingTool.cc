...
namespace lar_content {
  ...
  void CheatingEventSlicingTool::RunSlicing(const Algorithm* const pAlgorithm,
                                            ...
                                            SliceList& sliceList)
  {
    ...
    for (const MCParticle* const pMCParticle : mcParticleVector) {
      ...
      if (!slice.m_caloHitListU.empty() || !slice.m_caloHitListV.empty() ||
          !slice.m_caloHitListW.empty())
    ...
    MCParticleToSliceMap& mcParticleToSliceMap) const
  {
    for (const auto& mapEntry : caloHitListNames) {
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::GetList(*pAlgorithm, mapEntry.second, pCaloHitList));
      ...
      for (const CaloHit* const pCaloHit : *pCaloHitList) {
        ...
        for (const MCParticle* const pMCParticle : mcParticleVector) {
          ...
          if (mcParticleToSliceMap.count(pParentMCParticle)) continue;

          if (!mcParticleToSliceMap
                 .insert(MCParticleToSliceMap::value_type(pParentMCParticle, Slice()))
                 .second)
  ...
  void CheatingEventSlicingTool::FillSlices(const Algorithm* const pAlgorithm,
                                            ...
                                            MCParticleToSliceMap& mcParticleToSliceMap) const
  {
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::GetList(*pAlgorithm, caloHitListNames.at(hitType), pCaloHitList));

    for (const CaloHit* const pCaloHit : *pCaloHitList) {
      try {
        const MCParticle* const pMainMCParticle(MCParticleHelper::GetMainMCParticle(pCaloHit));
        const MCParticle* const pParentMCParticle(
          LArMCParticleHelper::GetParentMCParticle(pMainMCParticle));

        MCParticleToSliceMap::iterator mapIter = mcParticleToSliceMap.find(pParentMCParticle);

        if (mcParticleToSliceMap.end() == mapIter) throw StatusCodeException(STATUS_CODE_FAILURE);

        Slice& slice(mapIter->second);
        CaloHitList& caloHitList((TPC_VIEW_U == hitType) ?
