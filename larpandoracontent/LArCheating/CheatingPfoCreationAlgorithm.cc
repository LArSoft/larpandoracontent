...
namespace lar_content {
    ...
    for (const std::string& clusterListName : m_inputClusterListNames) {
      ...
      if (STATUS_CODE_SUCCESS != PandoraContentApi::GetList(*this, clusterListName, pClusterList)) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "CheatingPfoCreationAlgorithm - Could not access cluster list with name "
  ...
  void CheatingPfoCreationAlgorithm::GetMCParticleToClusterListMap(
    ...
    MCParticleToClusterListMap& mcParticleToClusterListMap) const
  {
    for (const Cluster* const pCluster : *pClusterList) {
      try {
        ...
        if (m_collapseToPrimaryMCParticles) {
          LArMCParticleHelper::MCRelationMap::const_iterator primaryIter =
            mcPrimaryMap.find(pMCParticle);

          if (mcPrimaryMap.end() == primaryIter) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  void CheatingPfoCreationAlgorithm::CreatePfos(
    const MCParticleToClusterListMap& mcParticleToClusterListMap) const
  {
    if (mcParticleToClusterListMap.empty()) return;
    ...
    for (const MCParticle* const pMCParticle : mcParticleVector) {
      ...
      try {
        ...
        pfoParameters.m_clusterList.insert(
          pfoParameters.m_clusterList.end(), clusterList.begin(), clusterList.end());
        ...
        std::cout
          << "CheatingPfoCreationAlgorithm: Could not create PFO for MCParticle with pdg code "
          << pMCParticle->GetParticleId() << std::endl;
      }
    }
  ...
  unsigned int CheatingPfoCreationAlgorithm::GetNHitTypesAboveThreshold(
    const ClusterList& clusterList,
    const unsigned int nHitsThreshold) const
  {
    ...
    for (const HitTypeMap::value_type& mapEntry : hitTypeMap) {
      if (mapEntry.second > nHitsThreshold) ++nGoodViews;
    }
  ...
  StatusCode CheatingPfoCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "AddVertices", m_addVertices));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "ParticleIdList", particleIdVector));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinGoodHitTypes", m_minGoodHitTypes));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NHitsForGoodHitType", m_nHitsForGoodHitType));
