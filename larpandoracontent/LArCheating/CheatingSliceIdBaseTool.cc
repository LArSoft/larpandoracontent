...
namespace lar_content {
    ...
    for (const ParticleFlowObject* const pPfo : *pPfoList) {
      ...
      for (const CaloHit* const pCaloHit : caloHitList) {
        ...
        CheatingSliceIdBaseTool::GetTargetParticleWeight(
          pCaloHit, thisTargetParticleWeight, thisTotalWeight, fCriteria);
    ...
    std::function<bool(const MCParticle* const)> fCriteria)
  {
    ...
    if (hitMCParticleWeightMap.empty()) return;
    ...
    for (const MCParticle* const pMCParticle : mcParticleList) {
      ...
      if (fCriteria(pParentMCParticle)) targetParticleWeight += weight;
