...
namespace lar_content {
  ...
  CheatingSliceSelectionTool::CheatingSliceSelectionTool()
    : m_maxSlices{1}, m_threshold{-1.f}, m_cutVariable{"completeness"}
  {}

  //-----------------------------------------------------------------------------------------------------------------------------------------

  void CheatingSliceSelectionTool::SelectSlices(const pandora::Algorithm* const /*pAlgorithm*/,
                                                ...
                                                SliceVector& outputSliceVector)
  {
    ...
    for (const CaloHitList& sliceHits : inputSliceVector) {
      ...
      for (const CaloHit* const pCaloHit : localHitList) {
        ...
        if (hitMCParticleWeightMap.empty()) continue;
        ...
        for (const MCParticle* const pMCParticle : mcParticleList) {
          ...
          if (this->IsTarget(pParentMCParticle)) thisTargetParticleWeight += weight;
    ...
    for (unsigned int i = 0; i < targetWeights.size(); ++i) {
      ...
      const float completeness{totalTargetWeight > std::numeric_limits<float>::epsilon() ?
                                 targetWeights[i] / totalTargetWeight :
                                 0.f};
      const float purity{
        sliceWeight > std::numeric_limits<float>::epsilon() ? targetWeights[i] / sliceWeight : 0.f};
      // Already checked correctness of variable in ReadSettings
      if (m_cutVariable == "completeness") {
        if (completeness >= m_threshold) reducedSliceVarIndexMap.emplace(completeness, i);
      }
      else if (m_cutVariable == "purity") {
        if (purity >= m_threshold) reducedSliceVarIndexMap.emplace(purity, i);
      }
    }
    ...
    int i = 0;
    for (const auto [cutVariable, index] :
         reducedSliceVarIndexMap) { // ATTN: Map is sorted on cut variable from max to min
  ...
  StatusCode CheatingSliceSelectionTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MaxSlices", m_maxSlices));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "Threshold", m_threshold));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "CutVariable", m_cutVariable));
    std::transform(m_cutVariable.begin(),
                   m_cutVariable.end(),
                   m_cutVariable.begin(),
                   [](unsigned char c) { return std::tolower(c); });
    if (m_cutVariable != "completeness" && m_cutVariable != "purity") {
      std::cout << "CheatingSliceSelectionTool::ReadSettings: Unknown cut variable \'"
                << m_cutVariable << "\'" << std::endl;
