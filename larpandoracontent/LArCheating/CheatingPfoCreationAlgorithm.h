 ...
 */
  ...
  private:
    ...
    typedef std::unordered_map<const pandora::MCParticle*, pandora::ClusterList>
     ...
     */
    void GetMCParticleToClusterListMap(
     ...
     */
    unsigned int GetNHitTypesAboveThreshold(const pandora::ClusterList& clusterList,
    ...
    bool
      m_useOnlyAvailableClusters; ///< Whether to consider unavailable clusters when identifying cheated pfos
    ...
    unsigned int
      m_minGoodHitTypes; ///< The min number of good hit types in the clusters collected for a given mc particle
    unsigned int
      m_nHitsForGoodHitType; ///< The min number of hits of a particular hit type in order to declare the hit type is good
    ParticleIdList
      m_particleIdList; ///< The list of particle ids to consider for pfo creation; will consider all ids if empty
  };
