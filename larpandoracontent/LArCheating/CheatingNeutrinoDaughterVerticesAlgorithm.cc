...
namespace lar_content {
  ...
  StatusCode CheatingNeutrinoDaughterVerticesAlgorithm::Run()
  {
    ...
    if (!pPfoList) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "CheatingNeutrinoDaughterVerticesAlgorithm: pfo list unavailable."
                  << std::endl;
  ...
  void CheatingNeutrinoDaughterVerticesAlgorithm::GetMCPrimaryMap(
    LArMCParticleHelper::MCRelationMap& mcPrimaryMap) const
  {
  ...
  void CheatingNeutrinoDaughterVerticesAlgorithm::ProcessRecoNeutrinos(
    const PfoList& neutrinoPfos,
    const LArMCParticleHelper::MCRelationMap& mcPrimaryMap) const
  {
    for (const ParticleFlowObject* const pNeutrinoPfo : neutrinoPfos) {
      ...
      PfoList::iterator neutrinoIter(
        std::find(daughterPfos.begin(), daughterPfos.end(), pNeutrinoPfo));

      if (daughterPfos.end() != neutrinoIter) daughterPfos.erase(neutrinoIter);
    ...
    const LArMCParticleHelper::MCRelationMap& mcPrimaryMap) const
  {
    ...
    if (!pVertexList->empty()) {
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToPfo<Vertex>(*this, pDaughterPfo, pVertex));
    }
  }
