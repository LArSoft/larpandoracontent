...
namespace lar_content {
  ...
  void CheatingClusterCreationAlgorithm::GetMCParticleToHitListMap(
  ...
  void CheatingClusterCreationAlgorithm::SimpleMCParticleCollection(
    ...
    MCParticleToHitListMap& mcParticleToHitListMap) const
  {
    ...
    if (!this->SelectMCParticlesForClustering(pMCParticle)) return;
  ...
  bool CheatingClusterCreationAlgorithm::SelectMCParticlesForClustering(
    const MCParticle* const pMCParticle) const
  {
    if (m_particleIdList.empty()) return true;

    for (const int particleId : m_particleIdList) {
      if (pMCParticle->GetParticleId() == particleId) return true;
    }
  ...
  void CheatingClusterCreationAlgorithm::CreateClusters(
    const MCParticleToHitListMap& mcParticleToHitListMap) const
  {
    ...
    for (const MCParticle* const pMCParticle : mcParticleVector) {
      ...
      case E_MINUS:
      case MU_PLUS:
      case MU_MINUS: metadata.m_particleId = pMCParticle->GetParticleId(); break;
      default: break;
      }
  ...
  StatusCode CheatingClusterCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "ParticleIdList", m_particleIdList));
