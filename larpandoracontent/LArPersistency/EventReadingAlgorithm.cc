...
namespace lar_content {
  ...
  EventReadingAlgorithm::~EventReadingAlgorithm() { delete m_pEventFileReader; }
  ...
  StatusCode EventReadingAlgorithm::Initialize()
  {
    ...
    if (!m_eventFileName.empty()) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, this->ReplaceEventFileReader(m_eventFileName));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, m_pEventFileReader->GoToEvent(m_skipToEvent));
    }
  ...
  StatusCode EventReadingAlgorithm::Run()
  {
    if ((nullptr != m_pEventFileReader) && !m_eventFileName.empty()) {
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::RepeatEventPreparation(*this));
    }
  ...
  {
    if (m_eventFileNameVector.empty()) throw StopProcessingException("All event files processed");
  ...
  FileType EventReadingAlgorithm::GetFileType(const std::string& fileName) const
  {
    ...
    if (std::string(".xml") == fileExtension) { return XML; }
  ...
  StatusCode EventReadingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    if (this->ExternalParametersPresent()) {
      pExternalParameters =
        dynamic_cast<ExternalEventReadingParameters*>(this->GetExternalParameters());
    ...
    if (pExternalParameters && !pExternalParameters->m_eventFileNameList.empty()) {
      XmlHelper::TokenizeString(
        pExternalParameters->m_eventFileNameList, m_eventFileNameVector, ":");
    }
    else {
      PANDORA_RETURN_RESULT_IF_AND_IF(
        STATUS_CODE_SUCCESS,
    ...
    if (pExternalParameters && pExternalParameters->m_skipToEvent.IsInitialized()) {
      ...
      PANDORA_RETURN_RESULT_IF_AND_IF(
        ...
        STATUS_CODE_NOT_FOUND,
        !=,
        XmlHelper::ReadValue(xmlHandle, "SkipToEvent", m_skipToEvent));
    }
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "LArMCParticleVersion", m_larMCParticleVersion));
