...
namespace lar_content {
  ...
  StatusCode EventWritingAlgorithm::Initialize()
  {
    if (m_shouldWriteGeometry) {
      ...
      if (BINARY == m_geometryFileType) {
        m_pGeometryFileWriter =
          new BinaryFileWriter(this->GetPandora(), m_geometryFileName, fileMode);
      }
    ...
    if (m_shouldWriteEvents) {
      ...
      if (m_useLArMCParticles) m_pEventFileWriter->SetFactory(new LArMCParticleFactory);
    }
  ...
  StatusCode EventWritingAlgorithm::Run()
  {
    ...
    bool matchNeutrinoVertexPosition(!m_shouldFilterByNeutrinoVertex ||
                                     this->PassNeutrinoVertexFilter());

    if (matchNuanceCode && matchParticles && matchNeutrinoVertexPosition && m_pEventFileWriter &&
        m_shouldWriteEvents) {
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetCurrentList(*this, pTrackList));
      ...
      PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                               !=,
                               m_pEventFileWriter->WriteEvent(*pCaloHitList,
                                                              *pTrackList,
                                                              *pMCParticleList,
  ...
  bool EventWritingAlgorithm::PassNuanceCodeFilter() const
  {
    ...
    for (const MCParticle* const pMCNeutrino : mcNeutrinoList) {
      ...
      if (pLArMCNeutrino && (pLArMCNeutrino->GetNuanceCode() == m_filterNuanceCode)) return true;
    }
  ...
  bool EventWritingAlgorithm::PassMCParticleFilter() const
  {
    ...
    unsigned int nNonNeutrons(0), nMuons(0), nElectrons(0), nProtons(0), nPhotons(0),
      nChargedPions(0);
    ...
    if ((nNonNeutrons == m_nNonNeutrons) && (nMuons == m_nMuons) && (nElectrons == m_nElectrons) &&
        (nProtons == m_nProtons) && (nPhotons == m_nPhotons) &&
        (nChargedPions == m_nChargedPions)) {
      return true;
    }
  ...
  bool EventWritingAlgorithm::PassNeutrinoVertexFilter() const
  {
    ...
    for (const MCParticle* const pMCNeutrino : mcNeutrinoVector) {
      ...
      if ((neutrinoInteractionVertex.GetX() <
           (m_detectorHalfLengthX - m_coordinateOffsetX - m_selectedBorderX)) &&
          (neutrinoInteractionVertex.GetX() > (-m_coordinateOffsetX + m_selectedBorderX)) &&
          (neutrinoInteractionVertex.GetY() <
           (m_detectorHalfLengthY - m_coordinateOffsetY - m_selectedBorderY)) &&
          (neutrinoInteractionVertex.GetY() > (-m_coordinateOffsetY + m_selectedBorderY)) &&
          (neutrinoInteractionVertex.GetZ() <
           (m_detectorHalfLengthZ - m_coordinateOffsetZ - m_selectedBorderZ)) &&
  ...
  StatusCode EventWritingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldWriteGeometry", m_shouldWriteGeometry));

    if (m_shouldWriteGeometry) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "GeometryFileName", m_geometryFileName));

      std::string fileExtension(m_geometryFileName.substr(m_geometryFileName.find_last_of(".")));
      std::transform(fileExtension.begin(), fileExtension.end(), fileExtension.begin(), ::tolower);

      if (std::string(".xml") == fileExtension) { m_geometryFileType = XML; }
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ShouldWriteEvents", m_shouldWriteEvents));

    if (m_shouldWriteEvents) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "EventFileName", m_eventFileName));

      std::string fileExtension(m_eventFileName.substr(m_eventFileName.find_last_of(".")));
      std::transform(fileExtension.begin(), fileExtension.end(), fileExtension.begin(), ::tolower);

      if (std::string(".xml") == fileExtension) { m_eventFileType = XML; }
      else if (std::string(".pndr") == fileExtension) {
    ...
    if (m_shouldFilterByNuanceCode) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "FilterNuanceCode", m_filterNuanceCode));
    }
    ...
    if (m_shouldFilterByMCParticles) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "NeutrinoInducedOnly", m_neutrinoInducedOnly));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "MatchingMinPrimaryHits", m_matchingMinPrimaryHits));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "NNonNeutrons", m_nNonNeutrons));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "NMuons", m_nMuons));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "NElectrons", m_nElectrons));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "NProtons", m_nProtons));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "NPhotons", m_nPhotons));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "NChargedPions", m_nChargedPions));
    }
    ...
    if (m_shouldFilterByNeutrinoVertex) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "DetectorHalfLengthX", m_detectorHalfLengthX));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "DetectorHalfLengthY", m_detectorHalfLengthY));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "DetectorHalfLengthZ", m_detectorHalfLengthZ));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "CoordinateOffsetX", m_coordinateOffsetX));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "CoordinateOffsetY", m_coordinateOffsetY));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "CoordinateOffsetZ", m_coordinateOffsetZ));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "SelectedBorderX", m_selectedBorderX));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "SelectedBorderY", m_selectedBorderY));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "SelectedBorderZ", m_selectedBorderZ));
    }
