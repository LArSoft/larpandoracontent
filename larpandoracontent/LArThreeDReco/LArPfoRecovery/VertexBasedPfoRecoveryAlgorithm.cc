...
namespace lar_content {
  ...
  StatusCode VertexBasedPfoRecoveryAlgorithm::Run()
  {
    ...
    const Vertex* const pSelectedVertex(
      (pVertexList && (pVertexList->size() == 1) &&
       (VERTEX_3D == (*(pVertexList->begin()))->GetVertexType())) ?
        *(pVertexList->begin()) :
        NULL);

    if (!pSelectedVertex) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "VertexBasedPfoRecoveryAlgorithm: unable to find vertex in current list "
                  << std::endl;
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      this->GetAvailableClusters(m_inputClusterListNames, availableClusters));
    ...
    this->SelectVertexClusters(
      pSelectedVertex, slidingFitResultMap, availableClusters, selectedClusters);

    // Match the cluster end points
    ClusterSet vetoList;
    ParticleList particleList;
    this->MatchThreeViews(
      pSelectedVertex, slidingFitResultMap, selectedClusters, vetoList, particleList);
    this->MatchTwoViews(
      pSelectedVertex, slidingFitResultMap, selectedClusters, vetoList, particleList);
  ...
  StatusCode VertexBasedPfoRecoveryAlgorithm::GetAvailableClusters(
    const StringVector inputClusterListNames,
    ClusterVector& clusterVector) const
  {
    for (StringVector::const_iterator iter = inputClusterListNames.begin(),
                                      iterEnd = inputClusterListNames.end();
         iter != iterEnd;
         ++iter) {
      ...
      PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                     STATUS_CODE_NOT_INITIALIZED,
                                     !=,
                                     PandoraContentApi::GetList(*this, *iter, pClusterList));

      if (NULL == pClusterList) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "VertexBasedPfoRecoveryAlgorithm: could not find cluster list " << *iter
                    << std::endl;
        continue;
      }

      for (ClusterList::const_iterator cIter = pClusterList->begin(),
                                       cIterEnd = pClusterList->end();
           cIter != cIterEnd;
           ++cIter) {
        ...
        if (pCluster->GetNCaloHits() <= 1) continue;

        if (TPC_3D == LArClusterHelper::GetClusterHitType(pCluster)) continue;
  ...
  void VertexBasedPfoRecoveryAlgorithm::BuildSlidingFitResultMap(
      ...
      if (slidingFitResultMap.end() == slidingFitResultMap.find(*iter)) {
        try {
          const TwoDSlidingFitResult slidingFitResult(
            *iter, m_slidingFitHalfWindow, slidingFitPitch);
          const LArPointingCluster pointingCluster(slidingFitResult);

          if (pointingCluster.GetLengthSquared() < std::numeric_limits<float>::epsilon()) continue;
  ...
  void VertexBasedPfoRecoveryAlgorithm::SelectVertexClusters(
    const Vertex* const pVertex,
    const TwoDSlidingFitResultMap& slidingFitResultMap,
    const ClusterVector& inputClusters,
    ClusterVector& outputClusters) const
  {
    const CartesianVector vertexU(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), pVertex->GetPosition(), TPC_VIEW_U));
    const CartesianVector vertexV(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), pVertex->GetPosition(), TPC_VIEW_V));
    const CartesianVector vertexW(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), pVertex->GetPosition(), TPC_VIEW_W));

    for (ClusterVector::const_iterator cIter = inputClusters.begin(),
                                       cIterEnd = inputClusters.end();
         cIter != cIterEnd;
         ++cIter) {
      const Cluster* const pCluster = *cIter;
      ...
      const CartesianVector vertexPosition(
        (TPC_VIEW_U == hitType) ? vertexU : (TPC_VIEW_V == hitType) ? vertexV : vertexW);

      TwoDSlidingFitResultMap::const_iterator sIter = slidingFitResultMap.find(pCluster);
      if (slidingFitResultMap.end() == sIter) continue;

      const TwoDSlidingFitResult& slidingFitResult = sIter->second;
      const LArPointingCluster pointingCluster(slidingFitResult);

      for (unsigned int iVtx = 0; iVtx < 2; ++iVtx) {
        const LArPointingCluster::Vertex& pointingVertex(
          (0 == iVtx) ? pointingCluster.GetInnerVertex() : pointingCluster.GetOuterVertex());
  ...
  void VertexBasedPfoRecoveryAlgorithm::MatchThreeViews(
    const Vertex* const pVertex,
    const TwoDSlidingFitResultMap& slidingFitResultMap,
    const ClusterVector& inputClusters,
    ClusterSet& vetoList,
    ParticleList& particleList) const
  {
    while (true) {
      ...
      if (NULL == pCluster1 || NULL == pCluster2 || NULL == pCluster3) return;
  ...
  void VertexBasedPfoRecoveryAlgorithm::MatchTwoViews(
    const Vertex* const pVertex,
    const TwoDSlidingFitResultMap& slidingFitResultMap,
    const ClusterVector& inputClusters,
    ClusterSet& vetoList,
    ParticleList& particleList) const
  {
    while (true) {
      ...
      this->GetBestChi2(
        pVertex, slidingFitResultMap, clustersU, clustersV, pCluster1, pCluster2, chi2);
      this->GetBestChi2(
        pVertex, slidingFitResultMap, clustersV, clustersW, pCluster1, pCluster2, chi2);
      this->GetBestChi2(
        pVertex, slidingFitResultMap, clustersW, clustersU, pCluster1, pCluster2, chi2);

      if (NULL == pCluster1 || NULL == pCluster2) return;

      const HitType hitType1(LArClusterHelper::GetClusterHitType(pCluster1));
      const HitType hitType2(LArClusterHelper::GetClusterHitType(pCluster2));

      const Cluster* const pClusterU(
        (TPC_VIEW_U == hitType1) ? pCluster1 : (TPC_VIEW_U == hitType2) ? pCluster2 : NULL);
      const Cluster* const pClusterV(
        (TPC_VIEW_V == hitType1) ? pCluster1 : (TPC_VIEW_V == hitType2) ? pCluster2 : NULL);
      const Cluster* const pClusterW(
        (TPC_VIEW_W == hitType1) ? pCluster1 : (TPC_VIEW_W == hitType2) ? pCluster2 : NULL);
  ...
  void VertexBasedPfoRecoveryAlgorithm::GetBestChi2(
    const Vertex* const pVertex,
    const TwoDSlidingFitResultMap& slidingFitResultMap,
    const ClusterVector& clusters1,
    ...
    for (ClusterVector::const_iterator cIter1 = clusters1.begin(), cIterEnd1 = clusters1.end();
      ...
      for (ClusterVector::const_iterator cIter2 = clusters2.begin(), cIterEnd2 = clusters2.end();
        ...
        for (ClusterVector::const_iterator cIter3 = clusters3.begin(), cIterEnd3 = clusters3.end();
          ...
          const float thisChi2(
            this->GetChi2(pVertex, pointingCluster1, pointingCluster2, pointingCluster3));
  ...
  float VertexBasedPfoRecoveryAlgorithm::GetChi2(const Vertex* const pVertex,
                                                 ...
                                                 const LArPointingCluster& pointingCluster2) const
  {
    ...
    const CartesianVector vertex1(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), pVertex->GetPosition(), hitType1));
    const CartesianVector vertex2(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), pVertex->GetPosition(), hitType2));

    const LArPointingCluster::Vertex& pointingVertex1(
      this->GetOuterVertex(vertex1, pointingCluster1));
    const LArPointingCluster::Vertex& pointingVertex2(
      this->GetOuterVertex(vertex2, pointingCluster2));

    float chi2(0.f);
    CartesianVector mergedPosition(0.f, 0.f, 0.f);
    LArGeometryHelper::MergeTwoPositions3D(this->GetPandora(),
                                           hitType1,
                                           hitType2,
                                           pointingVertex1.GetPosition(),
                                           pointingVertex2.GetPosition(),
                                           mergedPosition,
                                           chi2);
  ...
  float VertexBasedPfoRecoveryAlgorithm::GetChi2(const Vertex* const pVertex,
                                                 ...
                                                 const LArPointingCluster& pointingCluster3) const
  {
    const HitType hitType1(LArClusterHelper::GetClusterHitType(pointingCluster1.GetCluster()));
    const HitType hitType2(LArClusterHelper::GetClusterHitType(pointingCluster2.GetCluster()));
    ...
    const CartesianVector vertex2(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), pVertex->GetPosition(), hitType2));
    const CartesianVector vertex3(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), pVertex->GetPosition(), hitType3));

    const LArPointingCluster::Vertex& pointingVertex1(
      this->GetOuterVertex(vertex1, pointingCluster1));
    const LArPointingCluster::Vertex& pointingVertex2(
      this->GetOuterVertex(vertex2, pointingCluster2));
    const LArPointingCluster::Vertex& pointingVertex3(
      this->GetOuterVertex(vertex3, pointingCluster3));

    float chi2(0.f);
    CartesianVector mergedPosition(0.f, 0.f, 0.f);
    LArGeometryHelper::MergeThreePositions3D(this->GetPandora(),
                                             hitType1,
                                             hitType2,
                                             hitType3,
                                             pointingVertex1.GetPosition(),
                                             pointingVertex2.GetPosition(),
                                             pointingVertex3.GetPosition(),
                                             mergedPosition,
                                             chi2);
  ...
  void VertexBasedPfoRecoveryAlgorithm::SelectAvailableClusters(const ClusterSet& vetoList,
                                                                ...
                                                                ClusterVector& outputVector) const
  {
    for (ClusterVector::const_iterator iter = inputVector.begin(), iterEnd = inputVector.end();
         iter != iterEnd;
         ++iter) {
      if (0 == vetoList.count(*iter)) outputVector.push_back(*iter);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void VertexBasedPfoRecoveryAlgorithm::SelectClusters(const HitType hitType,
                                                       const ClusterVector& inputVector,
                                                       ClusterVector& outputVector) const
  {
    for (ClusterVector::const_iterator iter = inputVector.begin(), iterEnd = inputVector.end();
         iter != iterEnd;
         ++iter) {
      if (hitType == LArClusterHelper::GetClusterHitType(*iter)) outputVector.push_back(*iter);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  const LArPointingCluster::Vertex& VertexBasedPfoRecoveryAlgorithm::GetInnerVertex(
    const CartesianVector& vertex,
    const LArPointingCluster& cluster) const
  {
    const float innerDistance(
      (vertex - cluster.GetInnerVertex().GetPosition()).GetMagnitudeSquared());
    const float outerDistance(
      (vertex - cluster.GetOuterVertex().GetPosition()).GetMagnitudeSquared());
  ...
  const LArPointingCluster::Vertex& VertexBasedPfoRecoveryAlgorithm::GetOuterVertex(
    const CartesianVector& vertex,
    const LArPointingCluster& cluster) const
  {
  ...
  void VertexBasedPfoRecoveryAlgorithm::BuildParticles(const ParticleList& particleList)
  {
    ...
    for (ParticleList::const_iterator iter = particleList.begin(), iterEnd = particleList.end();
         iter != iterEnd;
         ++iter) {
  ...
  VertexBasedPfoRecoveryAlgorithm::Particle::Particle(const Cluster* const pClusterU,
                                                      const Cluster* const pClusterV,
                                                      const Cluster* const pClusterW)
    : m_pClusterU(pClusterU), m_pClusterV(pClusterV), m_pClusterW(pClusterW)
  {
    ...
    const HitType hitTypeU(NULL == m_pClusterU ? TPC_VIEW_U :
                                                 LArClusterHelper::GetClusterHitType(m_pClusterU));
    const HitType hitTypeV(NULL == m_pClusterV ? TPC_VIEW_V :
                                                 LArClusterHelper::GetClusterHitType(m_pClusterV));
    const HitType hitTypeW(NULL == m_pClusterW ? TPC_VIEW_W :
                                                 LArClusterHelper::GetClusterHitType(m_pClusterW));
  ...
  StatusCode VertexBasedPfoRecoveryAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "SlidingFitHalfWindow", m_slidingFitHalfWindow));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "TwoViewChi2Cut", m_twoViewChi2Cut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ThreeViewChi2Cut", m_threeViewChi2Cut));
