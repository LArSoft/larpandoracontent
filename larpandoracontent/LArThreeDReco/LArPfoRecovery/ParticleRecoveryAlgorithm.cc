...
namespace lar_content {
  ...
  void ParticleRecoveryAlgorithm::GetInputClusters(ClusterList& inputClusterListU,
                                                   ClusterList& inputClusterListV,
                                                   ClusterList& inputClusterListW) const
  {
    for (StringVector::const_iterator iter = m_inputClusterListNames.begin(),
                                      iterEnd = m_inputClusterListNames.end();
         iter != iterEnd;
         ++iter) {
      ...
      if (!pClusterList || pClusterList->empty()) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "ParticleRecoveryAlgorithm: unable to find cluster list " << *iter
                    << std::endl;
      ...
      for (ClusterList::const_iterator cIter = pClusterList->begin(),
        ...
        ClusterList& clusterList((TPC_VIEW_U == hitType) ?
                                   inputClusterListU :
                                   (TPC_VIEW_V == hitType) ? inputClusterListV : inputClusterListW);
  ...
  void ParticleRecoveryAlgorithm::SelectInputClusters(const ClusterList& inputClusterList,
  ...
  void ParticleRecoveryAlgorithm::StandardClusterSelection(const ClusterList& inputClusterList,
                                                           ClusterList& selectedClusterList) const
  {
    for (ClusterList::const_iterator iter = inputClusterList.begin(),
      ...
      if ((xMax - xMin) < m_minClusterXSpan) continue;
  ...
  void ParticleRecoveryAlgorithm::VertexClusterSelection(const ClusterList& inputClusterList,
                                                         ClusterList& selectedClusterList) const
  {
    ...
    for (ClusterList::const_iterator iter = inputClusterList.begin(),
      ...
      try {
        ...
        for (CartesianPointVector::const_iterator vIter = vertexList.begin(),
                                                  vIterEnd = vertexList.end();
             vIter != vIterEnd;
             ++vIter) {
          if (LArPointingClusterHelper::IsNode(*vIter,
                                               pointingCluster.GetInnerVertex(),
                                               m_minVertexLongitudinalDistance,
                                               m_maxVertexTransverseDistance) ||
              LArPointingClusterHelper::IsNode(*vIter,
  ...
  void ParticleRecoveryAlgorithm::FindOverlaps(const ClusterList& clusterList1,
                                               const ClusterList& clusterList2,
                                               SimpleOverlapTensor& overlapTensor) const
  {
    for (ClusterList::const_iterator iter1 = clusterList1.begin(), iter1End = clusterList1.end();
         iter1 != iter1End;
         ++iter1) {
      for (ClusterList::const_iterator iter2 = clusterList2.begin(), iter2End = clusterList2.end();
           iter2 != iter2End;
           ++iter2) {
        if (this->IsOverlap(*iter1, *iter2)) overlapTensor.AddAssociation(*iter1, *iter2);
      }
    }
  }
  ...
  bool ParticleRecoveryAlgorithm::IsOverlap(const Cluster* const pCluster1,
                                            const Cluster* const pCluster2) const
  {
    if (LArClusterHelper::GetClusterHitType(pCluster1) ==
        LArClusterHelper::GetClusterHitType(pCluster2))
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    ...
    if ((xSpan1 < std::numeric_limits<float>::epsilon()) ||
        (xSpan2 < std::numeric_limits<float>::epsilon()))
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    ...
    if (m_checkGaps)
      this->CalculateEffectiveOverlapFractions(
        pCluster1, xMin1, xMax1, pCluster2, xMin2, xMax2, xOverlapFraction1, xOverlapFraction2);
  ...
  void ParticleRecoveryAlgorithm::CalculateEffectiveOverlapFractions(const Cluster* const pCluster1,
                                                                     const float xMin1,
                                                                     const float xMax1,
  ...
  {
    if (PandoraContentApi::GetGeometry(*this)->GetDetectorGapList().empty()) return;
    ...
    if ((effectiveXSpan1 > std::numeric_limits<float>::epsilon()) &&
        (effectiveXSpan2 > std::numeric_limits<float>::epsilon())) {
  ...
  void ParticleRecoveryAlgorithm::CalculateEffectiveSpan(const pandora::Cluster* const pCluster,
                                                         const float xMin,
                                                         const float xMax,
                                                         float& xMinEff,
                                                         float& xMaxEff) const
  {
    ...
    try {
      ...
      const TwoDSlidingFitResult slidingFitResult(
        pCluster, m_slidingFitHalfWindow, slidingFitPitch);
      ...
      for (int iSample = 1; iSample <= nSamplingPointsLeft; ++iSample) {
        const float xSample(
          std::max(xMin, xMinEff - static_cast<float>(iSample) * m_sampleStepSize));

        if (!LArGeometryHelper::IsXSamplingPointInGap(
              this->GetPandora(), xSample, slidingFitResult, m_sampleStepSize))
          break;
      ...
      for (int iSample = 1; iSample <= nSamplingPointsRight; ++iSample) {
        const float xSample(
          std::min(xMax, xMaxEff + static_cast<float>(iSample) * m_sampleStepSize));
  ...
  {
    ClusterVector sortedKeyClusters(overlapTensor.GetKeyClusters().begin(),
                                    overlapTensor.GetKeyClusters().end());
    std::sort(sortedKeyClusters.begin(), sortedKeyClusters.end(), LArClusterHelper::SortByNHits);

    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ClusterList clusterListU, clusterListV, clusterListW;

      overlapTensor.GetConnectedElements(
        pKeyCluster, true, clusterListU, clusterListV, clusterListW);
      const unsigned int nU(clusterListU.size()), nV(clusterListV.size()), nW(clusterListW.size());

      if ((0 == nU * nV) && (0 == nV * nW) && (0 == nW * nU)) continue;
      ...
      if ((1 == nU * nV * nW) && this->CheckConsistency(*(clusterListU.begin()),
                                                        *(clusterListV.begin()),
                                                        *(clusterListW.begin()))) {
        this->CreateTrackParticle(clusterListAll);
      }
      else if ((0 == nU * nV * nW) &&
               ((1 == nU && 1 == nV) || (1 == nV && 1 == nW) || (1 == nW && 1 == nU))) {
        this->CreateTrackParticle(clusterListAll);
      }
      else {
  ...
  bool ParticleRecoveryAlgorithm::CheckConsistency(const Cluster* const pClusterU,
                                                   ...
                                                   const Cluster* const pClusterW) const
  {
    ...
    float u(std::numeric_limits<float>::max()), v(std::numeric_limits<float>::max()),
    ...
    const float uv2w(
      LArGeometryHelper::MergeTwoPositions(this->GetPandora(), TPC_VIEW_U, TPC_VIEW_V, u, v));
    const float vw2u(
      LArGeometryHelper::MergeTwoPositions(this->GetPandora(), TPC_VIEW_V, TPC_VIEW_W, v, w));
    const float wu2v(
      LArGeometryHelper::MergeTwoPositions(this->GetPandora(), TPC_VIEW_W, TPC_VIEW_U, w, u));

    const float pseudoChi2(
      ((u - vw2u) * (u - vw2u) + (v - wu2v) * (v - wu2v) + (w - uv2w) * (w - uv2w)) / 3.f);

    if (pseudoChi2 > m_pseudoChi2Cut) return false;
  ...
  {
    if (clusterList.size() < 2) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  void ParticleRecoveryAlgorithm::SimpleOverlapTensor::AddAssociation(
  ...
  void ParticleRecoveryAlgorithm::SimpleOverlapTensor::GetConnectedElements(
  ...
  {
    if (ignoreUnavailable && !pCluster->IsAvailable()) return;
    ...
    ClusterList& clusterList((TPC_VIEW_U == hitType) ?
                               clusterListU :
                               (TPC_VIEW_V == hitType) ? clusterListV : clusterListW);
    const ClusterNavigationMap& navigationMap((TPC_VIEW_U == hitType) ?
                                                m_clusterNavigationMapUV :
                                                (TPC_VIEW_V == hitType) ? m_clusterNavigationMapVW :
                                                                          m_clusterNavigationMapWU);

    if (clusterList.end() != std::find(clusterList.begin(), clusterList.end(), pCluster)) return;

    clusterList.push_back(pCluster);

    ClusterNavigationMap::const_iterator iter = navigationMap.find(pCluster);

    if (navigationMap.end() == iter) return;

    for (ClusterList::const_iterator cIter = iter->second.begin(), cIterEnd = iter->second.end();
         cIter != cIterEnd;
         ++cIter)
      this->GetConnectedElements(
        *cIter, ignoreUnavailable, clusterListU, clusterListV, clusterListW);
  }
  ...
  StatusCode ParticleRecoveryAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "CheckGaps", m_checkGaps));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinClusterXSpan", m_minClusterXSpan));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexClusterMode", m_vertexClusterMode));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SampleStepSize", m_sampleStepSize));

    if (m_sampleStepSize < std::numeric_limits<float>::epsilon()) {
      std::cout << "ParticleRecoveryAlgorithm: Invalid value for SampleStepSize "
