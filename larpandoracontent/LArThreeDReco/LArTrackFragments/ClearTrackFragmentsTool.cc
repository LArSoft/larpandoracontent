...
namespace lar_content {
  ...
  bool ClearTrackFragmentsTool::Run(ThreeViewTrackFragmentsAlgorithm* const pAlgorithm,
  ...
  bool ClearTrackFragmentsTool::FindTrackFragments(
    ThreeViewTrackFragmentsAlgorithm* const pAlgorithm,
    const TensorType& overlapTensor) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      if (!this->GetAndCheckElementList(overlapTensor, pKeyCluster, elementList)) continue;

      IteratorList iteratorList;
      this->SelectClearElements(elementList, iteratorList);

      if (iteratorList.empty()) return false;
      ...
      const Cluster *pClusterU(iter->GetClusterU()), *pClusterV(iter->GetClusterV()),
        *pClusterW(iter->GetClusterW());

      if (!this->CheckOverlapResult(overlapResult)) continue;
      ...
      if (!pFragmentCluster) throw StatusCodeException(STATUS_CODE_FAILURE);
  ...
  bool ClearTrackFragmentsTool::GetAndCheckElementList(const TensorType& overlapTensor,
                                                       ...
                                                       TensorType::ElementList& elementList) const
  {
    ...
    for (TensorType::ElementList::const_iterator eIter = elementList.begin();
      ...
      if (!((TPC_VIEW_U == thisHitType) || (TPC_VIEW_V == thisHitType) ||
            (TPC_VIEW_W == thisHitType)))
        throw StatusCodeException(STATUS_CODE_FAILURE);

      if (thisHitType != fragmentHitType && HIT_CUSTOM != fragmentHitType) return false;
  ...
  bool ClearTrackFragmentsTool::CheckOverlapResult(
    const TensorType::OverlapResult& overlapResult) const
  {
  ...
  void ClearTrackFragmentsTool::SelectClearElements(const TensorType::ElementList& elementList,
                                                    IteratorList& iteratorList) const
  {
    for (TensorType::ElementList::const_iterator eIter1 = elementList.begin(),
                                                 eIterEnd1 = elementList.end();
         eIter1 != eIterEnd1;
         ++eIter1) {
      const CaloHitList& fragmentHits1(eIter1->GetOverlapResult().GetFragmentCaloHitList());
      const float nCaloHits1(static_cast<float>(eIter1->GetClusterU()->GetNCaloHits() +
                                                eIter1->GetClusterV()->GetNCaloHits() +
      ...
      for (TensorType::ElementList::const_iterator eIter2 = elementList.begin(),
                                                   eIterEnd2 = elementList.end();
           eIter2 != eIterEnd2;
           ++eIter2) {
        const CaloHitList& fragmentHits2(eIter2->GetOverlapResult().GetFragmentCaloHitList());
        const float nCaloHits2(static_cast<float>(eIter2->GetClusterU()->GetNCaloHits() +
                                                  eIter2->GetClusterV()->GetNCaloHits() +
        ...
        if (commonClusterU && commonClusterV && commonClusterW) continue;

        if (eIter1->GetOverlapResult().GetFragmentHitType() !=
            eIter2->GetOverlapResult().GetFragmentHitType())
          throw StatusCodeException(STATUS_CODE_FAILURE);

        bool isAmbiguousElement(commonClusterU || commonClusterV || commonClusterW);

        if (!isAmbiguousElement) {
          for (CaloHitList::const_iterator hIter2 = fragmentHits2.begin(),
                                           hIterEnd2 = fragmentHits2.end();
               hIter2 != hIterEnd2;
               ++hIter2) {
            if (fragmentHits1.end() !=
                std::find(fragmentHits1.begin(), fragmentHits1.end(), *hIter2)) {
      ...
      if (isClearElement) iteratorList.push_back(eIter1);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void ClearTrackFragmentsTool::ProcessTensorElement(
    ThreeViewTrackFragmentsAlgorithm* const pAlgorithm,
    const TensorType& overlapTensor,
    const TensorType::OverlapResult& overlapResult,
    const Cluster*& pFragmentCluster) const
  {
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::ReplaceCurrentList<Cluster>(*pAlgorithm, currentListName));
    ...
    const CaloHitSet caloHitSet(overlapResult.GetFragmentCaloHitList().begin(),
                                overlapResult.GetFragmentCaloHitList().end());
    ...
    for (const Cluster* const pCluster : fragmentClusterList) {
      if (deletedClusters.count(pCluster)) continue;

      if (!pCluster->IsAvailable()) throw StatusCodeException(STATUS_CODE_FAILURE);
      ...
      for (const CaloHit* const pCaloHit : clusterHitList) {
        if (caloHitSet.count(pCaloHit)) { daughterHits.push_back(pCaloHit); }
        else {
          separateHits.push_back(pCaloHit);
        }
      }

      if (daughterHits.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);

      this->Recluster(pAlgorithm,
                      pCluster,
                      daughterHits,
                      separateHits,
                      deletedClusters,
      ...
      ClusterList::iterator rebuildIter(
        std::find(clustersToRebuild.begin(), clustersToRebuild.end(), pCluster));
      if (deletedClusters.count(pCluster)) {
        if (clustersToRebuild.end() != rebuildIter) clustersToRebuild.erase(rebuildIter);
      }
  ...
  void ClearTrackFragmentsTool::Recluster(ThreeViewTrackFragmentsAlgorithm* const pAlgorithm,
                                          const Cluster* const pCluster,
                                          const CaloHitList& daughterHits,
                                          const CaloHitList& separateHits,
                                          ClusterSet& deletedClusters,
                                          ClusterSet& badClusters,
                                          const Cluster*& pFragmentCluster) const
  {
    if (separateHits.empty()) {
      if (!pFragmentCluster) { pFragmentCluster = pCluster; }
      else {
        ...
        if (!badClusters.insert(pCluster).second) throw StatusCodeException(STATUS_CODE_FAILURE);

        (void)deletedClusters.insert(pCluster);
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::MergeAndDeleteClusters(*pAlgorithm, pFragmentCluster, pCluster));
      }
    }
    else {
      for (const CaloHit* const pCaloHit : daughterHits)
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::RemoveFromCluster(*pAlgorithm, pCluster, pCaloHit));
      ...
      if (!pFragmentCluster) {
        ...
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::GetCurrentListName<Cluster>(*pAlgorithm, currentListName));
        PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                                !=,
                                PandoraContentApi::CreateTemporaryListAndSetCurrent<ClusterList>(
                                  *pAlgorithm, pTemporaryList, temporaryListName));
        ...
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::Cluster::Create(*pAlgorithm, hitParameters, pFragmentCluster));
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::SaveList<Cluster>(*pAlgorithm, temporaryListName, currentListName));
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::ReplaceCurrentList<Cluster>(*pAlgorithm, currentListName));
        ...
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::AddToCluster(*pAlgorithm, pFragmentCluster, &daughterHits));
      }
    }
  }
  ...
  void ClearTrackFragmentsTool::RebuildClusters(ThreeViewTrackFragmentsAlgorithm* const pAlgorithm,
                                                const ClusterList& modifiedClusters,
                                                ClusterList& newClusters) const
  {
    ClusterList rebuildList;

    for (const Cluster* const pCluster : modifiedClusters) {
      if (pCluster->IsAvailable()) rebuildList.push_back(pCluster);
    }

    if (!rebuildList.empty()) pAlgorithm->RebuildClusters(rebuildList, newClusters);
  }
  ...
  void ClearTrackFragmentsTool::GetAffectedKeyClusters(
    const TensorType& overlapTensor,
    const ClusterList& clustersToRemoveFromTensor,
    ClusterList& affectedKeyClusters) const
  {
    for (TensorType::const_iterator tIterU = overlapTensor.begin(), tIterUEnd = overlapTensor.end();
         tIterU != tIterUEnd;
         ++tIterU) {
      for (TensorType::OverlapMatrix::const_iterator tIterV = tIterU->second.begin(),
                                                     tIterVEnd = tIterU->second.end();
           tIterV != tIterVEnd;
           ++tIterV) {
        for (TensorType::OverlapList::const_iterator tIterW = tIterV->second.begin(),
                                                     tIterWEnd = tIterV->second.end();
             tIterW != tIterWEnd;
             ++tIterW) {
          ...
          for (ClusterList::const_iterator fIter = fragmentClusters.begin(),
                                           fIterEnd = fragmentClusters.end();
               fIter != fIterEnd;
               ++fIter) {
            if (clustersToRemoveFromTensor.end() == std::find(clustersToRemoveFromTensor.begin(),
                                                              clustersToRemoveFromTensor.end(),
                                                              *fIter))
              continue;

            if ((TPC_VIEW_U != fragmentHitType) &&
                (affectedKeyClusters.end() ==
                 std::find(affectedKeyClusters.begin(), affectedKeyClusters.end(), tIterU->first)))
              affectedKeyClusters.push_back(tIterU->first);

            if ((TPC_VIEW_V != fragmentHitType) &&
                (affectedKeyClusters.end() ==
                 std::find(affectedKeyClusters.begin(), affectedKeyClusters.end(), tIterV->first)))
              affectedKeyClusters.push_back(tIterV->first);

            if ((TPC_VIEW_W != fragmentHitType) &&
                (affectedKeyClusters.end() ==
                 std::find(affectedKeyClusters.begin(), affectedKeyClusters.end(), tIterW->first)))
