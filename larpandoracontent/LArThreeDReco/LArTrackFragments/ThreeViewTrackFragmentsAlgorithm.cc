...
namespace lar_content {
  ...
  void ThreeViewTrackFragmentsAlgorithm::UpdateForNewCluster(const Cluster* const pNewCluster)
  {
    ...
    ClusterList& clusterList((TPC_VIEW_U == hitType) ?
                               matchingControl.m_clusterListU :
    ...
    ClusterList clusterList1(
      this->GetSelectedClusterList((TPC_VIEW_U == hitType) ? TPC_VIEW_V : TPC_VIEW_U));
    ClusterList clusterList2(
      this->GetSelectedClusterList((TPC_VIEW_W == hitType) ? TPC_VIEW_V : TPC_VIEW_W));
    ...
    for (const Cluster* const pCluster1 : clusterList1) {
      if (TPC_VIEW_U == hitType) { this->CalculateOverlapResult(pNewCluster, pCluster1, nullptr); }
    ...
    for (const Cluster* const pCluster2 : clusterList2) {
      if (TPC_VIEW_U == hitType) { this->CalculateOverlapResult(pNewCluster, nullptr, pCluster2); }
      else if (TPC_VIEW_V == hitType) {
  ...
  void ThreeViewTrackFragmentsAlgorithm::RebuildClusters(const ClusterList& rebuildList,
                                                         ClusterList& newClusters) const
  {
    ...
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                            !=,
                            PandoraContentApi::InitializeReclustering(
                              *this, TrackList(), rebuildList, oldClusterListName));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::RunClusteringAlgorithm(
        *this, m_reclusteringAlgorithmName, pNewClusterList, newClusterListName));

    newClusters.insert(newClusters.end(), pNewClusterList->begin(), pNewClusterList->end());
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::EndReclustering(*this, newClusterListName));
  }
  ...
  void ThreeViewTrackFragmentsAlgorithm::CalculateOverlapResult(const Cluster* const pClusterU,
                                                                ...
                                                                const Cluster* const pClusterW)
  {
    ...
    if (HIT_CUSTOM == missingHitType) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    ...
    const Cluster *pMatchedClusterU(nullptr), *pMatchedClusterV(nullptr),
      *pMatchedClusterW(nullptr);

    const TwoDSlidingFitResult& fitResult1((TPC_VIEW_U == missingHitType) ?
                                             this->GetCachedSlidingFitResult(pClusterV) :
                                             this->GetCachedSlidingFitResult(pClusterU));

    const TwoDSlidingFitResult& fitResult2((TPC_VIEW_U == missingHitType) ?
                                             this->GetCachedSlidingFitResult(pClusterW) :
                                             (TPC_VIEW_V == missingHitType) ?
                                             this->GetCachedSlidingFitResult(pClusterW) :
                                             this->GetCachedSlidingFitResult(pClusterV));
    ...
    const StatusCode statusCode(this->CalculateOverlapResult(
      fitResult1, fitResult2, inputClusterList, pBestMatchedCluster, newOverlapResult));

    if ((STATUS_CODE_SUCCESS != statusCode) && (STATUS_CODE_NOT_FOUND != statusCode))
      throw StatusCodeException(statusCode);

    if (!newOverlapResult.IsInitialized()) return;
    ...
    if (STATUS_CODE_SUCCESS == statusCode) {
      ...
      try {
        oldOverlapResult =
          overlapTensor.GetOverlapResult(pMatchedClusterU, pMatchedClusterV, pMatchedClusterW);
      }
    ...
    if (!oldOverlapResult.IsInitialized()) {
      overlapTensor.SetOverlapResult(
        pMatchedClusterU, pMatchedClusterV, pMatchedClusterW, newOverlapResult);
    }
    else if (newOverlapResult.GetFragmentCaloHitList().size() >
             oldOverlapResult.GetFragmentCaloHitList().size()) {
      overlapTensor.ReplaceOverlapResult(
        pMatchedClusterU, pMatchedClusterV, pMatchedClusterW, newOverlapResult);
    }
    else if (newOverlapResult.GetFragmentCaloHitList().size() ==
             oldOverlapResult.GetFragmentCaloHitList().size()) {
  ...
  StatusCode ThreeViewTrackFragmentsAlgorithm::CalculateOverlapResult(
    ...
    FragmentOverlapResult& fragmentOverlapResult) const
  {
    ...
    if (!this->CheckOverlapResult(overlapResult)) return STATUS_CODE_NOT_FOUND;
    ...
    CartesianPointVector& projectedPositions) const
  {
    ...
    if (HIT_CUSTOM == hitType3) return STATUS_CODE_INVALID_PARAMETER;
    ...
    if ((xOverlap < m_minXOverlap) || (xSpan < std::numeric_limits<float>::epsilon()) ||
        ((xOverlap / xSpan) < m_minXOverlapFraction))
      return STATUS_CODE_NOT_FOUND;
    ...
    LArGeometryHelper::MergeTwoPositions3D(
      this->GetPandora(), hitType1, hitType2, vtxPosition1, vtxPosition2, vtxPosition3D, vtxChi2);

    float endChi2(0.f);
    CartesianVector endPosition3D(0.f, 0.f, 0.f);
    LArGeometryHelper::MergeTwoPositions3D(
      this->GetPandora(), hitType1, hitType2, endPosition1, endPosition2, endPosition3D, endChi2);

    const CartesianVector vtxProjection3(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), vtxPosition3D, hitType3));
    const CartesianVector endProjection3(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), endPosition3D, hitType3));
    ...
    if (nSamplingPoints < 1.f) return STATUS_CODE_NOT_FOUND;
    ...
    for (float iSample = 0.5f; iSample < nSamplingPoints; iSample += 1.f) {
      const CartesianVector linearPosition3D(vtxPosition3D + (endPosition3D - vtxPosition3D) *
                                                               (iSample / nSamplingPoints));
      const CartesianVector linearPosition1(
        LArGeometryHelper::ProjectPosition(this->GetPandora(), linearPosition3D, hitType1));
      const CartesianVector linearPosition2(
        LArGeometryHelper::ProjectPosition(this->GetPandora(), linearPosition3D, hitType2));
      ...
      LArGeometryHelper::MergeTwoPositions(
        this->GetPandora(), hitType1, hitType2, position1, position2, position3, chi2);
      ...
      if (foundLastPosition) {
        ...
        if (thisDisplacement > 2.f * samplingPitch) {
          ...
          for (float iExtra = 0.5f; iExtra < nExtraPoints; iExtra += 1.f) {
            const CartesianVector extraPosition(position3 + (lastPosition - position3) *
                                                              (iExtra / nExtraPoints));
    ...
    if (projectedPositions.empty()) return STATUS_CODE_NOT_FOUND;
  ...
  StatusCode ThreeViewTrackFragmentsAlgorithm::GetMatchedHits(
    const ClusterList& inputClusterList,
    const CartesianPointVector& projectedPositions,
    HitToClusterMap& hitToClusterMap,
    CaloHitList& matchedHits) const
  {
    CaloHitVector availableCaloHits;

    for (ClusterList::const_iterator iter = inputClusterList.begin(),
                                     iterEnd = inputClusterList.end();
         iter != iterEnd;
         ++iter) {
      ...
      for (CaloHitList::const_iterator hIter = caloHitList.begin(), hIterEnd = caloHitList.end();
           hIter != hIterEnd;
           ++hIter)
    ...
    std::sort(
      availableCaloHits.begin(), availableCaloHits.end(), LArClusterHelper::SortHitsByPosition);
    ...
    for (const CartesianVector& projectedPosition : projectedPositions) {
      ...
      for (const CaloHit* const pCaloHit : availableCaloHits) {
        const float distanceSquared(
          (pCaloHit->GetPositionVector() - projectedPosition).GetMagnitudeSquared());

        if ((distanceSquared < closestDistanceSquared) ||
            ((std::fabs(distanceSquared - closestDistanceSquared) <
  ...
  StatusCode ThreeViewTrackFragmentsAlgorithm::GetMatchedClusters(
    const CaloHitList& matchedHits,
    const HitToClusterMap& hitToClusterMap,
    ClusterList& matchedClusters,
    const Cluster*& pBestMatchedCluster) const
  {
    ClusterToMatchedHitsMap clusterToMatchedHitsMap;

    for (CaloHitList::const_iterator iter = matchedHits.begin(), iterEnd = matchedHits.end();
         iter != iterEnd;
         ++iter) {
      const CaloHit* const pCaloHit = *iter;
      HitToClusterMap::const_iterator cIter = hitToClusterMap.find(pCaloHit);

      if (hitToClusterMap.end() == cIter) throw StatusCodeException(STATUS_CODE_FAILURE);

      ++clusterToMatchedHitsMap[cIter->second];

      if (matchedClusters.end() ==
          std::find(matchedClusters.begin(), matchedClusters.end(), cIter->second))
        matchedClusters.push_back(cIter->second);
    }

    if (matchedClusters.empty()) return STATUS_CODE_NOT_FOUND;
    ...
    for (const Cluster* const pCluster : sortedClusters) {
      ...
      if ((nMatchedHits > bestClusterMatchedHits) ||
          ((nMatchedHits == bestClusterMatchedHits) &&
           (pCluster->GetHadronicEnergy() > tieBreakerBestEnergy))) {
    ...
    if (nullptr == pBestMatchedCluster) return STATUS_CODE_NOT_FOUND;
  ...
  void ThreeViewTrackFragmentsAlgorithm::GetFragmentOverlapResult(
    const CartesianPointVector& projectedPositions,
    const CaloHitList& matchedHits,
    ...
    FragmentOverlapResult& fragmentOverlapResult) const
  {
    ...
    std::sort(
      sortedMatchedHits.begin(), sortedMatchedHits.end(), LArClusterHelper::SortHitsByPosition);
    ...
    fragmentOverlapResult = FragmentOverlapResult(
      nMatchedSamplingPoints, projectedPositions.size(), chi2Sum, matchedHits, matchedClusters);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool ThreeViewTrackFragmentsAlgorithm::CheckMatchedClusters(
    const CartesianPointVector& projectedPositions,
    const ClusterList& matchedClusters) const
  {
    if (projectedPositions.empty() || matchedClusters.empty()) return false;
    ...
    if (clusterLengthSquared > 4.f * projectedLengthSquared) return false;
  ...
  bool ThreeViewTrackFragmentsAlgorithm::CheckOverlapResult(
    const FragmentOverlapResult& overlapResult) const
  {
    // ATTN This method is currently mirrored in ClearTrackFragments tool
    if (overlapResult.GetMatchedFraction() < m_minMatchedSamplingPointFraction) return false;

    if (overlapResult.GetFragmentCaloHitList().size() < m_minMatchedHits) return false;
  ...
  void ThreeViewTrackFragmentsAlgorithm::ExamineOverlapContainer()
  {
    ...
    for (TensorToolVector::const_iterator iter = m_algorithmToolVector.begin(),
                                          iterEnd = m_algorithmToolVector.end();
         iter != iterEnd;) {
      if ((*iter)->Run(this, this->GetMatchingControl().GetOverlapTensor())) {
        iter = m_algorithmToolVector.begin();

        if (++repeatCounter > m_nMaxTensorToolRepeats) break;
      }
      else {
        ++iter;
      }
  ...
  StatusCode ThreeViewTrackFragmentsAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                             !=,
                             XmlHelper::ProcessAlgorithm(
                               *this, xmlHandle, "ClusterRebuilding", m_reclusteringAlgorithmName));

    AlgorithmToolVector algorithmToolVector;
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ProcessAlgorithmToolList(*this, xmlHandle, "TrackTools", algorithmToolVector));

    for (AlgorithmToolVector::const_iterator iter = algorithmToolVector.begin(),
                                             iterEnd = algorithmToolVector.end();
         iter != iterEnd;
         ++iter) {
      FragmentTensorTool* const pFragmentTensorTool(dynamic_cast<FragmentTensorTool*>(*iter));

      if (!pFragmentTensorTool) return STATUS_CODE_INVALID_PARAMETER;
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "NMaxTensorToolRepeats", m_nMaxTensorToolRepeats));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MinXOverlap", m_minXOverlap));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinXOverlapFraction", m_minXOverlapFraction));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxPointDisplacement", maxPointDisplacement));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MinMatchedSamplingPointFraction",
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinMatchedHits", m_minMatchedHits));
