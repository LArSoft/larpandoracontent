...
namespace lar_content {
  ...
  bool MatchedEndPointsTool::Run(ThreeViewLongitudinalTracksAlgorithm* const pAlgorithm,
  ...
  void MatchedEndPointsTool::FindMatchedTracks(const TensorType& overlapTensor,
                                               ProtoParticleVector& protoParticleVector) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      for (TensorType::ElementList::const_iterator iter = elementList.begin();
           iter != elementList.end();
           ++iter) {
        if (usedClusters.count(iter->GetClusterU()) || usedClusters.count(iter->GetClusterV()) ||
            usedClusters.count(iter->GetClusterW()))
          continue;

        if (iter->GetOverlapResult().GetMatchedFraction() < m_minMatchedFraction) continue;

        if (std::max(iter->GetOverlapResult().GetInnerChi2(),
                     iter->GetOverlapResult().GetOuterChi2()) > m_maxEndPointChi2)
          continue;
  ...
  bool MatchedEndPointsTool::SortByChiSquared(const TensorType::Element& lhs,
  ...
  StatusCode MatchedEndPointsTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxEndPointChi2", m_maxEndPointChi2));
