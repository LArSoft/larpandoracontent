...
namespace lar_content {
  ...
  void ThreeViewLongitudinalTracksAlgorithm::CalculateOverlapResult(const Cluster* const pClusterU,
    ...
    LongitudinalOverlapResult& longitudinalOverlapResult)
  {
    ...
    for (unsigned int iPermutation = 0; iPermutation < 4; ++iPermutation) {
      ...
      const CartesianVector vtxU((isForwardU) ? slidingFitResultU.GetGlobalMinLayerPosition() :
                                                slidingFitResultU.GetGlobalMaxLayerPosition());
      const CartesianVector endU((!isForwardU) ? slidingFitResultU.GetGlobalMinLayerPosition() :
                                                 slidingFitResultU.GetGlobalMaxLayerPosition());

      const CartesianVector vtxV((isForwardV) ? slidingFitResultV.GetGlobalMinLayerPosition() :
                                                slidingFitResultV.GetGlobalMaxLayerPosition());
      const CartesianVector endV((!isForwardV) ? slidingFitResultV.GetGlobalMinLayerPosition() :
                                                 slidingFitResultV.GetGlobalMaxLayerPosition());

      const CartesianVector vtxW((isForwardW) ? slidingFitResultW.GetGlobalMinLayerPosition() :
                                                slidingFitResultW.GetGlobalMaxLayerPosition());
      const CartesianVector endW((!isForwardW) ? slidingFitResultW.GetGlobalMinLayerPosition() :
                                                 slidingFitResultW.GetGlobalMaxLayerPosition());
      ...
      CartesianVector vtxMergedU(0.f, 0.f, 0.f), vtxMergedV(0.f, 0.f, 0.f),
        vtxMergedW(0.f, 0.f, 0.f);
      CartesianVector endMergedU(0.f, 0.f, 0.f), endMergedV(0.f, 0.f, 0.f),
        endMergedW(0.f, 0.f, 0.f);

      float vtxChi2(std::numeric_limits<float>::max());
      float endChi2(std::numeric_limits<float>::max());

      LArGeometryHelper::MergeThreePositions(this->GetPandora(),
                                             TPC_VIEW_U,
                                             TPC_VIEW_V,
                                             TPC_VIEW_W,
                                             vtxU,
      ...
      if (vtxChi2 > m_vertexChi2Cut) continue;

      if (((vtxMergedU - vtxU).GetMagnitudeSquared() >
           std::min(halfLengthSquaredU, (vtxMergedU - endU).GetMagnitudeSquared())) ||
          ((vtxMergedV - vtxV).GetMagnitudeSquared() >
           std::min(halfLengthSquaredV, (vtxMergedV - endV).GetMagnitudeSquared())) ||
          ((vtxMergedW - vtxW).GetMagnitudeSquared() >
      ...
      if (endChi2 > m_vertexChi2Cut) continue;

      if (((endMergedU - endU).GetMagnitudeSquared() >
           std::min(halfLengthSquaredU, (endMergedU - vtxU).GetMagnitudeSquared())) ||
          ((endMergedV - endV).GetMagnitudeSquared() >
           std::min(halfLengthSquaredV, (endMergedV - vtxV).GetMagnitudeSquared())) ||
          ((endMergedW - endW).GetMagnitudeSquared() >
      ...
      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_U, TPC_VIEW_V, vtxU, vtxV, position3D, chi2);
      vtxList3D.push_back(position3D);

      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_V, TPC_VIEW_W, vtxV, vtxW, position3D, chi2);
      vtxList3D.push_back(position3D);

      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_W, TPC_VIEW_U, vtxW, vtxU, position3D, chi2);
      vtxList3D.push_back(position3D);

      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_U, TPC_VIEW_V, endU, endV, position3D, chi2);
      endList3D.push_back(position3D);

      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_V, TPC_VIEW_W, endV, endW, position3D, chi2);
      endList3D.push_back(position3D);

      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), TPC_VIEW_W, TPC_VIEW_U, endW, endU, position3D, chi2);
      endList3D.push_back(position3D);

      // Find best matched 3D trajactory
      for (CartesianPointVector::const_iterator iterI = vtxList3D.begin(),
                                                iterEndI = vtxList3D.end();
           iterI != iterEndI;
           ++iterI) {
        const CartesianVector& vtxMerged3D(*iterI);

        for (CartesianPointVector::const_iterator iterJ = endList3D.begin(),
                                                  iterEndJ = endList3D.end();
             iterJ != iterEndJ;
             ++iterJ) {
          const CartesianVector& endMerged3D(*iterJ);

          TrackOverlapResult overlapResult;
          this->CalculateOverlapResult(slidingFitResultU,
                                       slidingFitResultV,
                                       slidingFitResultW,
                                       vtxMerged3D,
                                       endMerged3D,
          ...
          if (overlapResult.IsInitialized() && (overlapResult.GetNMatchedSamplingPoints() > 0) &&
              (overlapResult > bestOverlapResult)) {
  ...
  void ThreeViewLongitudinalTracksAlgorithm::CalculateOverlapResult(
    ...
    const CartesianVector& endMerged3D,
    TrackOverlapResult& overlapResult) const
  {
    // Calculate start and end positions of linear trajectory
    const CartesianVector vtxMergedU(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), vtxMerged3D, TPC_VIEW_U));
    const CartesianVector vtxMergedV(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), vtxMerged3D, TPC_VIEW_V));
    const CartesianVector vtxMergedW(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), vtxMerged3D, TPC_VIEW_W));

    const CartesianVector endMergedU(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), endMerged3D, TPC_VIEW_U));
    const CartesianVector endMergedV(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), endMerged3D, TPC_VIEW_V));
    const CartesianVector endMergedW(
      LArGeometryHelper::ProjectPosition(this->GetPandora(), endMerged3D, TPC_VIEW_W));

    const unsigned int nSamplingPoints =
      static_cast<unsigned int>((endMerged3D - vtxMerged3D).GetMagnitude() / m_samplingPitch);

    if (0 == nSamplingPoints) return;
    ...
    for (unsigned int n = 0; n < nSamplingPoints; ++n) {
      ...
      LArGeometryHelper::MergeThreePositions(
        this->GetPandora(), posU, posV, posW, mergedU, mergedV, mergedW, deltaChi2);

      if (deltaChi2 < m_reducedChi2Cut) ++nMatchedSamplingPoints;
  ...
  StatusCode ThreeViewLongitudinalTracksAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    for (AlgorithmToolVector::const_iterator iter = algorithmToolVector.begin(),
         ...
         iter != iterEnd;
         ++iter) {
      LongitudinalTensorTool* const pLongitudinalTensorTool(
        dynamic_cast<LongitudinalTensorTool*>(*iter));

      if (!pLongitudinalTensorTool) return STATUS_CODE_INVALID_PARAMETER;
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexChi2Cut", m_vertexChi2Cut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ReducedChi2Cut", m_reducedChi2Cut));
