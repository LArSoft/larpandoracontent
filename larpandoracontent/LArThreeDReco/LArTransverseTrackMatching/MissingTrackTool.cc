...
namespace lar_content {
  ...
  bool MissingTrackTool::Run(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  void MissingTrackTool::FindMissingTracks(const TensorType& overlapTensor,
                                           ProtoParticleVector& protoParticleVector) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      for (TensorType::ElementList::const_iterator eIter = elementList.begin();
           ...
           ++eIter) {
        const bool includeU(eIter->GetClusterU()->IsAvailable() &&
                            !usedClusters.count(eIter->GetClusterU()));
        const bool includeV(eIter->GetClusterV()->IsAvailable() &&
                            !usedClusters.count(eIter->GetClusterV()));
        const bool includeW(eIter->GetClusterW()->IsAvailable() &&
        ...
        if (2 != nAvailable) continue;

        const TransverseOverlapResult& overlapResult(eIter->GetOverlapResult());

        if (overlapResult.GetNMatchedSamplingPoints() < m_minMatchedSamplingPoints) continue;

        if (overlapResult.GetMatchedFraction() < m_minMatchedFraction) continue;

        if (overlapResult.GetReducedChi2() > m_maxReducedChiSquared) continue;
        ...
        if (includeU &&
            (xOverlapSpan / overlapResult.GetXOverlap().GetXSpanU() < m_minXOverlapFraction))
          continue;

        if (includeV &&
            (xOverlapSpan / overlapResult.GetXOverlap().GetXSpanV() < m_minXOverlapFraction))
          continue;

        if (includeW &&
            (xOverlapSpan / overlapResult.GetXOverlap().GetXSpanW() < m_minXOverlapFraction))
          continue;
  ...
  StatusCode MissingTrackTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxReducedChiSquared", m_maxReducedChiSquared));
