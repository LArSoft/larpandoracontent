 ...
 */
  ...
  private:
     ...
     *  @param  splitPositionMap to receive the split position map
     */
    bool PassesChecks(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
     ...
     */
    bool CheckSplitPosition(const pandora::CartesianVector& splitPosition,
                            const float splitX,
    ...
    float
      m_minXOverlapFraction; ///< The min x overlap fraction (between long clusters and short cluster vs. shared overlap)
    unsigned int
      m_minMatchedSamplingPointRatio; ///< The min ratio between 1st and 2nd highest msps for simple ambiguity resolution

    float
      m_maxShortDeltaXFraction; ///< Max x distance between ends of two short clusters (measured as fraction of long cluster x length)
    float
      m_maxAbsoluteShortDeltaX; ///< Max x distance between ends of two short clusters (measured as an absolute distance)
    float
      m_minLongDeltaXFraction; ///< Min x distance between ends of short and long clusters (measured as fraction of long cluster x length)
    float
      m_minAbsoluteLongDeltaX; ///< Min x distance between ends of short and long clusters (measured as an absolute distance)
    float
      m_minSplitToVertexProjection; ///< Min projected distance between split position and either inner or outer vertex of long cluster
    float
      m_maxSplitVsFitPositionDistance; ///< Max allowed distance between split position and sliding linear fit position at the split x coordinate
  };
