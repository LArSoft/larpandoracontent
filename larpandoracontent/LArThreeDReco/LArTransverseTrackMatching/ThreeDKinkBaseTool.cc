...
namespace lar_content {
  ...
  bool ThreeDKinkBaseTool::PassesElementCuts(TensorType::ElementList::const_iterator eIter,
  ...
  float ThreeDKinkBaseTool::GetXSamplingPoint(const CartesianVector& splitPosition1,
                                              ...
                                              const TwoDSlidingFitResult& fitResult3) const
  {
    ...
    const float commonX(isForwardInX ? std::max(xMin1, std::max(xMin2, xMin3)) :
                                       std::min(xMax1, std::min(xMax2, xMax3)));
  ...
  bool ThreeDKinkBaseTool::IsALowestInX(const LArPointingCluster& pointingClusterA,
                                        const LArPointingCluster& pointingClusterB)
  {
    if ((pointingClusterA.GetInnerVertex().GetPosition().GetX() <
         pointingClusterB.GetInnerVertex().GetPosition().GetX()) &&
        (pointingClusterA.GetInnerVertex().GetPosition().GetX() <
         pointingClusterB.GetOuterVertex().GetPosition().GetX())) {
      return true;
    }

    if ((pointingClusterA.GetOuterVertex().GetPosition().GetX() <
         pointingClusterB.GetInnerVertex().GetPosition().GetX()) &&
  ...
  bool ThreeDKinkBaseTool::Run(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  void ThreeDKinkBaseTool::GetModifications(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  bool ThreeDKinkBaseTool::ApplyChanges(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  void ThreeDKinkBaseTool::SelectTensorElements(TensorType::ElementList::const_iterator eIter,
  ...
  StatusCode ThreeDKinkBaseTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MajorityRulesMode", m_majorityRulesMode));
