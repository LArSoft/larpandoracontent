...
namespace lar_content {
        ...
        try {
          ...
          if (m_splitMode) {
            const TwoDSlidingFitResult& fitResult1(
              pAlgorithm->GetCachedSlidingFitResult(particle.m_pCommonCluster1));
            const TwoDSlidingFitResult& fitResult2(
              pAlgorithm->GetCachedSlidingFitResult(particle.m_pCommonCluster2));

            CartesianVector splitPosition1(0.f, 0.f, 0.f), splitPosition2(0.f, 0.f, 0.f);
            if ((STATUS_CODE_SUCCESS !=
                 fitResult1.GetGlobalFitPositionAtX(splitPosition.GetX(), splitPosition1)) ||
                (STATUS_CODE_SUCCESS !=
                 fitResult2.GetGlobalFitPositionAtX(splitPosition.GetX(), splitPosition2))) {
              continue;
            }
  ...
  bool UndershootTracksTool::IsThreeDKink(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
                                          const Particle& particle,
                                          const CartesianVector& splitPosition,
                                          const bool isALowestInX) const
  {
    try {
      const TwoDSlidingFitResult& fitResultCommon1(
        pAlgorithm->GetCachedSlidingFitResult(particle.m_pCommonCluster1));
      const TwoDSlidingFitResult& fitResultCommon2(
        pAlgorithm->GetCachedSlidingFitResult(particle.m_pCommonCluster2));
      ...
      const float minusX(this->GetXSamplingPoint(
        splitPosition, false, lowXFitResult, fitResultCommon1, fitResultCommon2));
      const float plusX(this->GetXSamplingPoint(
        splitPosition, true, highXFitResult, fitResultCommon1, fitResultCommon2));
  ...
  UndershootTracksTool::Particle::Particle(const TensorType::Element& elementA,
                                           const TensorType::Element& elementB)
  {
    ...
    m_pCommonCluster1 = (elementA.GetClusterU() == elementB.GetClusterU()) ?
                          ...
                          elementA.GetClusterW();
    m_pCommonCluster2 =
      ((m_pClusterA != elementA.GetClusterU()) && (m_pCommonCluster1 != elementA.GetClusterU())) ?
        elementA.GetClusterU() :
