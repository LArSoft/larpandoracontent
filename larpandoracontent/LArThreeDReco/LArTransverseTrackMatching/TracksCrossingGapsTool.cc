...
namespace lar_content {
  ...
  bool TracksCrossingGapsTool::Run(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
                                   TensorType& overlapTensor)
  {
    ...
    if (PandoraContentApi::GetGeometry(*pAlgorithm)->GetDetectorGapList().empty()) return false;
  ...
  void TracksCrossingGapsTool::FindTracks(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
                                              ...
                                              IteratorList& iteratorList) const
  {
    for (TensorType::ElementList::const_iterator eIter = elementList.begin();
      ...
      if (xOverlap.GetXOverlapSpan() < std::numeric_limits<float>::epsilon()) continue;
      ...
      this->CalculateEffectiveOverlapFractions(
        pAlgorithm, *eIter, xOverlapFractionU, xOverlapFractionV, xOverlapFractionW);

      if ((xOverlap.GetXSpanU() > std::numeric_limits<float>::epsilon()) &&
          (xOverlapFractionU > m_minXOverlapFraction) &&
          (xOverlap.GetXSpanV() > std::numeric_limits<float>::epsilon()) &&
          (xOverlapFractionV > m_minXOverlapFraction) &&
          (xOverlap.GetXSpanW() > std::numeric_limits<float>::epsilon()) &&
          (xOverlapFractionW > m_minXOverlapFraction)) {
  ...
  void TracksCrossingGapsTool::CalculateEffectiveOverlapFractions(
    ...
    float& xOverlapFractionW) const
  {
    float xMinEffU(element.GetOverlapResult().GetXOverlap().GetUMinX()),
      xMaxEffU(element.GetOverlapResult().GetXOverlap().GetUMaxX());
    float xMinEffV(element.GetOverlapResult().GetXOverlap().GetVMinX()),
      xMaxEffV(element.GetOverlapResult().GetXOverlap().GetVMaxX());
    float xMinEffW(element.GetOverlapResult().GetXOverlap().GetWMinX()),
      xMaxEffW(element.GetOverlapResult().GetXOverlap().GetWMaxX());
    this->CalculateEffectiveOverlapSpan(
      pAlgorithm, element, xMinEffU, xMaxEffU, xMinEffV, xMaxEffV, xMinEffW, xMaxEffW);

    const float effectiveXSpanU(xMaxEffU - xMinEffU), effectiveXSpanV(xMaxEffV - xMinEffV),
      effectiveXSpanW(xMaxEffW - xMinEffW);
    ...
    xOverlapFractionU =
      effectiveXSpanU > 0.f ? std::min(1.f, (effectiveXOverlapSpan / effectiveXSpanU)) : 0.f;
    xOverlapFractionV =
      effectiveXSpanV > 0.f ? std::min(1.f, (effectiveXOverlapSpan / effectiveXSpanV)) : 0.f;
    xOverlapFractionW =
      effectiveXSpanW > 0.f ? std::min(1.f, (effectiveXOverlapSpan / effectiveXSpanW)) : 0.f;
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void TracksCrossingGapsTool::CalculateEffectiveOverlapSpan(
    ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
    const TensorType::Element& element,
    float& xMinEffU,
    float& xMaxEffU,
    ...
    float& xMaxEffW) const
  {
    ...
    for (int iSample = 1; iSample <= nSamplingPointsLeft; ++iSample) {
      ...
      const float xSample(
        std::max(xMinAll, minCommonX - static_cast<float>(iSample) * m_sampleStepSize));

      if (!this->PassesGapChecks(pAlgorithm, element, xSample, gapInU, gapInV, gapInW)) break;
    ...
    for (int iSample = 1; iSample <= nSamplingPointsRight; ++iSample) {
      bool gapInU(false), gapInV(false), gapInW(false);
      const float xSample(
        std::min(xMaxAll, maxCommonX + static_cast<float>(iSample) * m_sampleStepSize));
  ...
  bool TracksCrossingGapsTool::PassesGapChecks(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
                                               const TensorType::Element& element,
                                               const float xSample,
                                               bool& gapInU,
                                               bool& gapInV,
                                               bool& gapInW) const
  {
    const TwoDSlidingFitResult& slidingFitResultU(
      pAlgorithm->GetCachedSlidingFitResult(element.GetClusterU()));
    const TwoDSlidingFitResult& slidingFitResultV(
      pAlgorithm->GetCachedSlidingFitResult(element.GetClusterV()));
    const TwoDSlidingFitResult& slidingFitResultW(
      pAlgorithm->GetCachedSlidingFitResult(element.GetClusterW()));
    ...
    CartesianVector fitUPosition(0.f, 0.f, 0.f), fitVPosition(0.f, 0.f, 0.f),
      fitWPosition(0.f, 0.f, 0.f);
    ...
    if ((STATUS_CODE_SUCCESS == statusCodeU) && (STATUS_CODE_SUCCESS == statusCodeV) &&
        (STATUS_CODE_SUCCESS == statusCodeW))
      return false;
    ...
    try {
      ...
      if ((STATUS_CODE_SUCCESS != statusCodeU) &&
          (!this->IsEndOfCluster(xSample, slidingFitResultU)))
        return this->CheckXPositionInGap(
          xSample, slidingFitResultU, slidingFitResultV, slidingFitResultW, gapInU, gapInV, gapInW);

      if ((STATUS_CODE_SUCCESS != statusCodeV) &&
          (!this->IsEndOfCluster(xSample, slidingFitResultV)))
        return this->CheckXPositionInGap(
          xSample, slidingFitResultV, slidingFitResultU, slidingFitResultW, gapInV, gapInU, gapInW);

      if ((STATUS_CODE_SUCCESS != statusCodeW) &&
          (!this->IsEndOfCluster(xSample, slidingFitResultW)))
        return this->CheckXPositionInGap(
          xSample, slidingFitResultW, slidingFitResultU, slidingFitResultV, gapInW, gapInU, gapInV);
    }
  ...
  bool TracksCrossingGapsTool::CheckXPositionInGap(const float xSample,
                                                   ...
                                                   bool& gapIn3) const
  {
    ...
    if ((STATUS_CODE_SUCCESS == slidingFitResult2.GetGlobalFitPositionAtX(xSample, fitPosition2)) &&
      ...
      const float zSample(LArGeometryHelper::MergeTwoPositions(
        this->GetPandora(), hitType2, hitType3, fitPosition2.GetZ(), fitPosition3.GetZ()));
      const CartesianVector samplingPoint(xSample, 0.f, zSample);
      return LArGeometryHelper::IsInGap(
        this->GetPandora(), CartesianVector(xSample, 0.f, zSample), hitType1, m_maxGapTolerance);
    }
    ...
    gapIn1 = LArGeometryHelper::IsXSamplingPointInGap(
      this->GetPandora(), xSample, slidingFitResult1, m_sampleStepSize);

    if (!gapIn1) return false;
    ...
    if ((STATUS_CODE_SUCCESS != slidingFitResult2.GetGlobalFitPositionAtX(xSample, fitPosition2)) &&
      ...
      if (!endIn2)
        gapIn2 = LArGeometryHelper::IsXSamplingPointInGap(
          this->GetPandora(), xSample, slidingFitResult2, m_sampleStepSize);

      if (!endIn3)
        gapIn3 = LArGeometryHelper::IsXSamplingPointInGap(
          this->GetPandora(), xSample, slidingFitResult3, m_sampleStepSize);
  ...
  bool TracksCrossingGapsTool::IsEndOfCluster(const float xSample,
                                              const TwoDSlidingFitResult& slidingFitResult) const
  {
    return ((std::fabs(slidingFitResult.GetGlobalMinLayerPosition().GetX() - xSample) <
             slidingFitResult.GetLayerPitch()) ||
            (std::fabs(slidingFitResult.GetGlobalMaxLayerPosition().GetX() - xSample) <
             slidingFitResult.GetLayerPitch()));
  }
  ...
  StatusCode TracksCrossingGapsTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxGapTolerance", m_maxGapTolerance));
    ...
    if (m_sampleStepSize < std::numeric_limits<float>::epsilon()) {
      std::cout << "TracksCrossingGapsTool: Invalid value for SampleStepSize " << m_sampleStepSize
                << std::endl;
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxAngleRatio", m_maxAngleRatio));
