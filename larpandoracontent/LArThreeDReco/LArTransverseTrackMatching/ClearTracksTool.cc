...
namespace lar_content {
  ...
  bool ClearTracksTool::Run(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  void ClearTracksTool::CreateThreeDParticles(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
                                              ...
                                              bool& particlesMade) const
  {
    ...
    for (TensorType::ElementList::const_iterator iter = elementList.begin(),
      ...
      if ((xOverlap.GetXSpanV() < std::numeric_limits<float>::epsilon()) ||
          (xOverlap.GetXOverlapSpan() / xOverlap.GetXSpanV() < m_minXOverlapFraction))
        continue;

      if ((xOverlap.GetXSpanW() < std::numeric_limits<float>::epsilon()) ||
