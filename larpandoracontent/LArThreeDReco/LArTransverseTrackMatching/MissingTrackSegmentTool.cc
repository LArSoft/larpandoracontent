...
namespace lar_content {
  ...
  bool MissingTrackSegmentTool::Run(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  void MissingTrackSegmentTool::FindTracks(ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
                                           ...
                                           ClusterMergeMap& clusterMergeMap) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      for (IteratorList::const_iterator iIter = iteratorList.begin(), iIterEnd = iteratorList.end();
           iIter != iIterEnd;
           ++iIter) {
        if (LongTracksTool::HasLongDirectConnections(iIter, iteratorList)) continue;

        if (!LongTracksTool::IsLongerThanDirectConnections(
              iIter, elementList, m_minMatchedSamplingPointRatio, usedClusters))
          continue;
  ...
  void MissingTrackSegmentTool::SelectElements(const TensorType::ElementList& elementList,
                                               ...
                                               IteratorList& iteratorList) const
  {
    for (TensorType::ElementList::const_iterator eIter = elementList.begin();
      ...
      if (eIter->GetOverlapResult().GetMatchedFraction() < m_minMatchedFraction) continue;
      ...
      const float shortSpan(
        std::min(xOverlap.GetXSpanU(), std::min(xOverlap.GetXSpanV(), xOverlap.GetXSpanW())));
      const float longSpan1(
        std::max(xOverlap.GetXSpanU(), std::max(xOverlap.GetXSpanV(), xOverlap.GetXSpanW())));
      const float longSpan2(
        ((xOverlap.GetXSpanU() > shortSpan) && (xOverlap.GetXSpanU() < longSpan1)) ?
          xOverlap.GetXSpanU() :
          ((xOverlap.GetXSpanV() > shortSpan) && (xOverlap.GetXSpanV() < longSpan1)) ?
          xOverlap.GetXSpanV() :
          xOverlap.GetXSpanW());

      if ((xOverlap.GetXOverlapSpan() < std::numeric_limits<float>::epsilon()) ||
          (longSpan1 < std::numeric_limits<float>::epsilon()))
        continue;

      if (((shortSpan / xOverlap.GetXOverlapSpan()) < m_minInitialXOverlapFraction) ||
          ((longSpan2 / longSpan1) < m_minInitialXOverlapFraction))
        continue;
  ...
  bool MissingTrackSegmentTool::PassesParticleChecks(
    ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
    ...
    ClusterList& candidateClusters) const
  {
    ...
    for (ClusterList::const_iterator iter = clusterList.begin(), iterEnd = clusterList.end();
      ...
      if (pCluster == particle.m_pShortCluster) continue;

      if (pCluster->GetNCaloHits() < m_minCaloHitsInCandidateCluster) continue;
  ...
  void MissingTrackSegmentTool::GetSlidingFitResultMap(
    ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
    ...
    TwoDSlidingFitResultMap& slidingFitResultMap) const
  {
    ...
    for (ClusterList::const_iterator iter = candidateClusterList.begin(),
                                     iterEnd = candidateClusterList.end();
         iter != iterEnd;
         ++iter) {
      const Cluster* const pCluster(*iter);
      ...
      try {
        const TwoDSlidingFitResult& slidingFitResult(
          pAlgorithm->GetCachedSlidingFitResult(pCluster));
        (void)slidingFitResultMap.insert(
          TwoDSlidingFitResultMap::value_type(pCluster, slidingFitResult));
        continue;
      }
      catch (StatusCodeException&) {
      }
      ...
      try {
        const TwoDSlidingFitResult slidingFitResult(
          pCluster, pAlgorithm->GetSlidingFitWindow(), slidingFitPitch);
        (void)slidingFitResultMap.insert(
          TwoDSlidingFitResultMap::value_type(pCluster, slidingFitResult));
        continue;
      }
  ...
  void MissingTrackSegmentTool::GetSegmentOverlapMap(
    ThreeViewTransverseTracksAlgorithm* const pAlgorithm,
    const Particle& particle,
    ...
    SegmentOverlapMap& segmentOverlapMap) const
  {
    const TwoDSlidingFitResult& fitResult1(
      pAlgorithm->GetCachedSlidingFitResult(particle.m_pCluster1));
    const TwoDSlidingFitResult& fitResult2(
      pAlgorithm->GetCachedSlidingFitResult(particle.m_pCluster2));

    const float nPoints1(
      std::fabs(static_cast<float>(fitResult1.GetMaxLayer() - fitResult1.GetMinLayer())));
    const float nPoints2(
      std::fabs(static_cast<float>(fitResult2.GetMaxLayer() - fitResult2.GetMinLayer())));
    ...
    for (unsigned n = 0; n <= nPoints; ++n) {
      const float x(particle.m_longMinX + (particle.m_longMaxX - particle.m_longMinX) *
                                            static_cast<float>(n) / static_cast<float>(nPoints));

      if ((x > particle.m_shortMinX) && (x < particle.m_shortMaxX)) continue;
      ...
      const float prediction(LArGeometryHelper::MergeTwoPositions(this->GetPandora(),
                                                                  particle.m_hitType1,
                                                                  particle.m_hitType2,
                                                                  fitVector1.GetZ(),
                                                                  fitVector2.GetZ()));
  ...
  bool MissingTrackSegmentTool::MakeDecisions(const Particle& particle,
                                              const TwoDSlidingFitResultMap& slidingFitResultMap,
                                              ...
                                              ClusterMergeMap& clusterMergeMap) const
  {
    ...
    for (const Cluster* const pCluster : sortedClusters) {
      ...
      if (!this->PassesSamplingCuts(segmentOverlap)) continue;
      ...
      if (!usedClusters.insert(pCluster).second || !pCluster->IsAvailable()) continue;

      if (!this->IsPossibleMerge(pCluster, particle, segmentOverlap, slidingFitResultMap)) continue;

      possibleMerges.push_back(pCluster);
    }

    if (std::fabs(particle.m_longMaxX - particle.m_longMinX) <
        std::numeric_limits<float>::epsilon())
      throw StatusCodeException(STATUS_CODE_FAILURE);

    if (!matchesACluster || possibleMerges.empty()) return false;

    if (((shortMaxX - shortMinX) / (particle.m_longMaxX - particle.m_longMinX)) <
  ...
  {
    if (0 == segmentOverlap.m_nSamplingPoints) return false;

    if ((segmentOverlap.m_nSamplingPoints < m_makePfoMinSamplingPoints) ||
        (segmentOverlap.m_nMatchedSamplingPoints < m_makePfoMinMatchedSamplingPoints))
      return false;

    if ((static_cast<float>(segmentOverlap.m_nMatchedSamplingPoints) /
         static_cast<float>(segmentOverlap.m_nSamplingPoints)) < m_makePfoMinMatchedFraction)
      return false;
  ...
  bool MissingTrackSegmentTool::IsPossibleMerge(
    const Cluster* const pCluster,
    const Particle& particle,
    const SegmentOverlap& segmentOverlap,
    const TwoDSlidingFitResultMap& slidingFitResultMap) const
  {
    if ((segmentOverlap.m_pseudoChi2Sum / static_cast<float>(segmentOverlap.m_nSamplingPoints)) >
        m_mergeMaxChi2PerSamplingPoint)
      return false;

    TwoDSlidingFitResultMap::const_iterator fitIter = slidingFitResultMap.find(pCluster);

    if (slidingFitResultMap.end() == fitIter) throw StatusCodeException(STATUS_CODE_FAILURE);

    float mergeMinX(std::numeric_limits<float>::max()),
      mergeMaxX(-std::numeric_limits<float>::max());
    fitIter->second.GetMinAndMaxX(mergeMinX, mergeMaxX);

    // cluster should not be wider than the longest span
    if ((mergeMinX < particle.m_longMinX - m_mergeXContainmentTolerance) ||
        (mergeMaxX > particle.m_longMaxX + m_mergeXContainmentTolerance))
      return false;

    // cluster should not overlap with the shortest span
    if ((mergeMinX < particle.m_shortMaxX - m_mergeXContainmentTolerance) &&
        (mergeMaxX > particle.m_shortMinX + m_mergeXContainmentTolerance))
      return false;
  ...
  MissingTrackSegmentTool::Particle::Particle(const TensorType::Element& element)
  {
    ...
    m_shortHitType = ((xOverlap.GetXSpanU() < xOverlap.GetXSpanV()) &&
                      (xOverlap.GetXSpanU() < xOverlap.GetXSpanW())) ?
                       TPC_VIEW_U :
                       ((xOverlap.GetXSpanV() < xOverlap.GetXSpanU()) &&
                        (xOverlap.GetXSpanV() < xOverlap.GetXSpanW())) ?
                       TPC_VIEW_V :
                       ((xOverlap.GetXSpanW() < xOverlap.GetXSpanU()) &&
                        (xOverlap.GetXSpanW() < xOverlap.GetXSpanV())) ?
                       TPC_VIEW_W :
                       HIT_CUSTOM;

    if (HIT_CUSTOM == m_shortHitType) throw StatusCodeException(STATUS_CODE_FAILURE);

    m_pShortCluster =
      (TPC_VIEW_U == m_shortHitType) ?
        element.GetClusterU() :
    ...
    m_shortMinX = (TPC_VIEW_U == m_shortHitType) ?
                    xOverlap.GetUMinX() :
                    (TPC_VIEW_V == m_shortHitType) ? xOverlap.GetVMinX() : xOverlap.GetWMinX();
    m_shortMaxX = (TPC_VIEW_U == m_shortHitType) ?
                    xOverlap.GetUMaxX() :
                    (TPC_VIEW_V == m_shortHitType) ? xOverlap.GetVMaxX() : xOverlap.GetWMaxX();
    m_longMinX = (TPC_VIEW_U == m_shortHitType) ?
                   std::min(xOverlap.GetVMinX(), xOverlap.GetWMinX()) :
                   (TPC_VIEW_V == m_shortHitType) ?
                   std::min(xOverlap.GetUMinX(), xOverlap.GetWMinX()) :
                   std::min(xOverlap.GetUMinX(), xOverlap.GetVMinX());
    m_longMaxX = (TPC_VIEW_U == m_shortHitType) ?
  ...
  StatusCode MissingTrackSegmentTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MakePfoMinMatchedSamplingPoints",
