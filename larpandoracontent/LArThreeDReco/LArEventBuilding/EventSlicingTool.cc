...
namespace lar_content {
  ...
  void EventSlicingTool::RunSlicing(const Algorithm* const pAlgorithm,
                                    ...
                                    SliceList& sliceList)
  {
    ...
    if (clusterSliceList.size() < 2) {
      ...
      this->CopyPfoHitsToSlices(
        clusterToSliceIndexMap, clusterToPfoMap, sliceList, assignedClusters);
  ...
  void EventSlicingTool::CopyAllHitsToSingleSlice(const Algorithm* const pAlgorithm,
                                                  const HitTypeToNameMap& caloHitListNames,
                                                  SliceList& sliceList) const
  {
    if (!sliceList.empty()) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  void EventSlicingTool::GetThreeDClusters(const Algorithm* const pAlgorithm,
                                           const std::string& pfoListName,
                                           ClusterList& clusters3D,
                                           ClusterToPfoMap& clusterToPfoMap) const
  {
    const PfoList* pPfoList(nullptr);
    PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                   STATUS_CODE_NOT_INITIALIZED,
                                   !=,
                                   PandoraContentApi::GetList(*pAlgorithm, pfoListName, pPfoList));
    ...
    for (const ParticleFlowObject* const pPfo : *pPfoList) {
      ...
      for (const Cluster* const pCluster3D : pfoClusters3D) {
        if (pCluster3D->GetNCaloHits() < m_minHitsPer3DCluster) continue;
  ...
  void EventSlicingTool::GetClusterSliceList(const ClusterList& trackClusters3D,
                                             const ClusterList& showerClusters3D,
                                             ClusterSliceList& clusterSliceList) const
  {
    ...
    for (const Cluster* const pCluster3D : trackClusters3D) {
      try {
        trackFitResults.insert(ThreeDSlidingFitResultMap::value_type(
          pCluster3D, ThreeDSlidingFitResult(pCluster3D, m_halfWindowLayers, layerPitch)));
      }
      catch (StatusCodeException&) {
        std::cout << "EventSlicingTool: ThreeDSlidingFitResult failure for track cluster."
                  << std::endl;
      }
    }
    ...
    for (const Cluster* const pCluster3D : showerClusters3D) {
      try {
        showerConeFitResults.insert(ThreeDSlidingConeFitResultMap::value_type(
          pCluster3D, ThreeDSlidingConeFitResult(pCluster3D, m_halfWindowLayers, layerPitch)));
      }
      catch (StatusCodeException&) {
        std::cout << "EventSlicingTool: ThreeDSlidingConeFitResult failure for shower cluster."
                  << std::endl;
      }
    }

    ClusterVector sortedClusters3D(trackClusters3D.begin(), trackClusters3D.end());
    sortedClusters3D.insert(
      sortedClusters3D.end(), showerClusters3D.begin(), showerClusters3D.end());
    ...
    for (const Cluster* const pCluster3D : sortedClusters3D) {
      if (usedClusters.count(pCluster3D)) continue;

      if (pCluster3D->GetNCaloHits() < m_min3DHitsToSeedNewSlice) continue;
      ...
      this->CollectAssociatedClusters(pCluster3D,
                                      sortedClusters3D,
                                      trackFitResults,
                                      showerConeFitResults,
                                      clusterSlice,
                                      usedClusters);
    }
  }
  ...
  void EventSlicingTool::CollectAssociatedClusters(
    const Cluster* const pClusterInSlice,
    const ClusterVector& candidateClusters,
    const ThreeDSlidingFitResultMap& trackFitResults,
    const ThreeDSlidingConeFitResultMap& showerConeFitResults,
    ClusterVector& clusterSlice,
    ClusterSet& usedClusters) const
  {
    ClusterVector addedClusters;

    for (const Cluster* const pCandidateCluster : candidateClusters) {
      if (usedClusters.count(pCandidateCluster) || (pClusterInSlice == pCandidateCluster)) continue;

      if ((m_usePointingAssociation &&
           this->PassPointing(pClusterInSlice, pCandidateCluster, trackFitResults)) ||
    ...
    for (const Cluster* const pAddedCluster : addedClusters)
      this->CollectAssociatedClusters(pAddedCluster,
                                      candidateClusters,
                                      trackFitResults,
                                      showerConeFitResults,
                                      clusterSlice,
  ...
  bool EventSlicingTool::PassPointing(const Cluster* const pClusterInSlice,
                                      const Cluster* const pCandidateCluster,
                                      const ThreeDSlidingFitResultMap& trackFitResults) const
  {
    ...
    ThreeDSlidingFitResultMap::const_iterator candidateIter =
      trackFitResults.find(pCandidateCluster);
  ...
  bool EventSlicingTool::PassProximity(const Cluster* const pClusterInSlice,
                                       const Cluster* const pCandidateCluster) const
  {
      ...
      for (const CaloHit* const pCaloHit1 : *(orderedList1.second)) {
        ...
        for (const auto& orderedList2 : pCandidateCluster->GetOrderedCaloHitList()) {
          for (const CaloHit* const pCaloHit2 : *(orderedList2.second)) {
            if ((positionVector1 - pCaloHit2->GetPositionVector()).GetMagnitudeSquared() <
  ...
  bool EventSlicingTool::PassShowerCone(
    const Cluster* const pConeCluster,
    const Cluster* const pNearbyCluster,
    const ThreeDSlidingConeFitResultMap& showerConeFitResults) const
  {
    ...
    if (showerConeFitResults.end() == fitIter) return false;
    ...
    try {
      ...
      const ThreeDSlidingFitResult& slidingFitResult3D(
        slidingConeFitResult3D.GetSlidingFitResult());
      slidingConeFitResult3D.GetSimpleConeList(
        m_nConeFitLayers, m_nConeFits, CONE_BOTH_DIRECTIONS, simpleConeList);
      clusterLength = (slidingFitResult3D.GetGlobalMaxLayerPosition() -
                       slidingFitResult3D.GetGlobalMinLayerPosition())
                        .GetMagnitude();
    }
    ...
    for (const SimpleCone& simpleCone : simpleConeList) {
      ...
      if (simpleCone.GetBoundedHitFraction(pNearbyCluster, coneLength, m_coneTanHalfAngle1) <
          m_coneBoundedFraction1)
        continue;

      if (simpleCone.GetBoundedHitFraction(pNearbyCluster, coneLength, m_coneTanHalfAngle2) <
  ...
  bool EventSlicingTool::CheckClosestApproach(const LArPointingCluster& cluster1,
                                              const LArPointingCluster& cluster2) const
  {
  ...
  bool EventSlicingTool::CheckClosestApproach(const LArPointingCluster::Vertex& vertex1,
                                              const LArPointingCluster::Vertex& vertex2) const
  {
    CartesianVector intersectionPoint(0.f, 0.f, 0.f);
    float displacement1(std::numeric_limits<float>::max()),
      displacement2(std::numeric_limits<float>::max());

    try {
      LArPointingClusterHelper::GetIntersection(
        vertex1, vertex2, intersectionPoint, displacement1, displacement2);
    }
    catch (const StatusCodeException&) {
      return false;
    }
    ...
    return ((closestApproach < m_maxClosestApproach) &&
            (std::fabs(displacement1) < m_maxInterceptDistance) &&
            (std::fabs(displacement2) < m_maxInterceptDistance));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool EventSlicingTool::IsNode(const LArPointingCluster& cluster1,
                                const LArPointingCluster& cluster2) const
  {
    return (LArPointingClusterHelper::IsNode(cluster1.GetInnerVertex().GetPosition(),
                                             cluster2.GetInnerVertex(),
                                             m_minVertexLongitudinalDistance,
                                             m_maxVertexTransverseDistance) ||
            LArPointingClusterHelper::IsNode(cluster1.GetOuterVertex().GetPosition(),
                                             cluster2.GetInnerVertex(),
                                             m_minVertexLongitudinalDistance,
                                             m_maxVertexTransverseDistance) ||
            LArPointingClusterHelper::IsNode(cluster1.GetInnerVertex().GetPosition(),
                                             cluster2.GetOuterVertex(),
                                             m_minVertexLongitudinalDistance,
                                             m_maxVertexTransverseDistance) ||
            LArPointingClusterHelper::IsNode(cluster1.GetOuterVertex().GetPosition(),
                                             cluster2.GetOuterVertex(),
                                             m_minVertexLongitudinalDistance,
                                             m_maxVertexTransverseDistance) ||
            LArPointingClusterHelper::IsNode(cluster2.GetInnerVertex().GetPosition(),
                                             cluster1.GetInnerVertex(),
                                             m_minVertexLongitudinalDistance,
                                             m_maxVertexTransverseDistance) ||
            LArPointingClusterHelper::IsNode(cluster2.GetOuterVertex().GetPosition(),
                                             cluster1.GetInnerVertex(),
                                             m_minVertexLongitudinalDistance,
                                             m_maxVertexTransverseDistance) ||
            LArPointingClusterHelper::IsNode(cluster2.GetInnerVertex().GetPosition(),
                                             cluster1.GetOuterVertex(),
                                             m_minVertexLongitudinalDistance,
                                             m_maxVertexTransverseDistance) ||
            LArPointingClusterHelper::IsNode(cluster2.GetOuterVertex().GetPosition(),
                                             cluster1.GetOuterVertex(),
  ...
  bool EventSlicingTool::IsEmission(const LArPointingCluster& cluster1,
                                    const LArPointingCluster& cluster2) const
  {
    return (LArPointingClusterHelper::IsEmission(cluster1.GetInnerVertex().GetPosition(),
                                                 cluster2.GetInnerVertex(),
                                                 m_minVertexLongitudinalDistance,
                                                 m_maxVertexLongitudinalDistance,
                                                 m_maxVertexTransverseDistance,
                                                 m_vertexAngularAllowance) ||
            LArPointingClusterHelper::IsEmission(cluster1.GetOuterVertex().GetPosition(),
                                                 cluster2.GetInnerVertex(),
                                                 m_minVertexLongitudinalDistance,
                                                 m_maxVertexLongitudinalDistance,
                                                 m_maxVertexTransverseDistance,
                                                 m_vertexAngularAllowance) ||
            LArPointingClusterHelper::IsEmission(cluster1.GetInnerVertex().GetPosition(),
                                                 cluster2.GetOuterVertex(),
                                                 m_minVertexLongitudinalDistance,
                                                 m_maxVertexLongitudinalDistance,
                                                 m_maxVertexTransverseDistance,
            ...
            LArPointingClusterHelper::IsEmission(cluster1.GetOuterVertex().GetPosition(),
                                                 ...
                                                 m_maxVertexTransverseDistance,
                                                 m_vertexAngularAllowance) ||
            LArPointingClusterHelper::IsEmission(cluster2.GetInnerVertex().GetPosition(),
                                                 cluster1.GetInnerVertex(),
                                                 m_minVertexLongitudinalDistance,
                                                 m_maxVertexLongitudinalDistance,
                                                 m_maxVertexTransverseDistance,
                                                 m_vertexAngularAllowance) ||
            LArPointingClusterHelper::IsEmission(cluster2.GetOuterVertex().GetPosition(),
                                                 cluster1.GetInnerVertex(),
                                                 m_minVertexLongitudinalDistance,
                                                 m_maxVertexLongitudinalDistance,
                                                 m_maxVertexTransverseDistance,
  ...
  void EventSlicingTool::CreateSlices(const ClusterSliceList& clusterSliceList,
                                      SliceList& sliceList,
                                      ClusterToSliceIndexMap& clusterToSliceIndexMap) const
  {
    ...
    for (const ClusterVector& clusterList : clusterSliceList) {
      for (const Cluster* const pCluster3D : clusterList) {
        ...
        if (!clusterToSliceIndexMap.insert(ClusterToSliceIndexMap::value_type(pCluster3D, index))
               .second)
          throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
      }
  ...
  void EventSlicingTool::CopyPfoHitsToSlices(const ClusterToSliceIndexMap& clusterToSliceIndexMap,
                                             const ClusterToPfoMap& clusterToPfoMap,
                                             SliceList& sliceList,
                                             ClusterSet& assignedClusters) const
  {
    ...
    for (const Cluster* const pCluster3D : clusterList) {
      ...
      for (const Cluster* const pCluster2D : clusters2D) {
        ...
        CaloHitList& targetList((TPC_VIEW_U == hitType) ?
                                  slice.m_caloHitListU :
        ...
        targetList.insert(targetList.end(),
                          pCluster2D->GetIsolatedCaloHitList().begin(),
                          pCluster2D->GetIsolatedCaloHitList().end());
  ...
  void EventSlicingTool::GetRemainingClusters(const Algorithm* const pAlgorithm,
                                              const HitTypeToNameMap& clusterListNames,
                                              const ClusterSet& assignedClusters,
                                              ClusterList& remainingClusters) const
  {
    this->GetRemainingClusters(
      pAlgorithm, clusterListNames.at(TPC_VIEW_U), assignedClusters, remainingClusters);
    this->GetRemainingClusters(
      pAlgorithm, clusterListNames.at(TPC_VIEW_V), assignedClusters, remainingClusters);
    this->GetRemainingClusters(
      pAlgorithm, clusterListNames.at(TPC_VIEW_W), assignedClusters, remainingClusters);
  }
                                              ...
                                              ClusterList& remainingClusters) const
  {
    ...
    if (!pClusterList || pClusterList->empty()) {
      if (PandoraContentApi::GetSettings(*pAlgorithm)->ShouldDisplayAlgorithmInfo())
        std::cout << "EventSlicingTool: unable to find cluster list " << clusterListName
                  << std::endl;
    ...
    for (const Cluster* const pCluster2D : *pClusterList) {
      ...
      if (assignedClusters.count(pCluster2D)) continue;

      if (remainingClusters.end() !=
          std::find(remainingClusters.begin(), remainingClusters.end(), pCluster2D))
        throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
  ...
  void EventSlicingTool::AssignRemainingHitsToSlices(
    const ClusterList& remainingClusters,
    ...
    SliceList& sliceList) const
  {
    ...
    try {
      ...
      if (m_use3DProjectionsInHitPickUp)
        this->GetKDTreeEntries3D(
          clusterToSliceIndexMap, pointsU, pointsV, pointsW, pointToSliceIndexMap);
      ...
      std::sort(sortedRemainingClusters.begin(),
                sortedRemainingClusters.end(),
      ...
      for (const Cluster* const pCluster2D : sortedRemainingClusters) {
        ...
        PointKDTree2D& kdTree(
          (TPC_VIEW_U == hitType) ? kdTreeU : (TPC_VIEW_V == hitType) ? kdTreeV : kdTreeW);
        const PointKDNode2D* pBestResultPoint(this->MatchClusterToSlice(pCluster2D, kdTree));

        if (!pBestResultPoint) continue;
  ...
  void EventSlicingTool::GetKDTreeEntries2D(const SliceList& sliceList,
                                            PointList& pointsU,
                                            PointList& pointsV,
                                            PointList& pointsW,
  ...
  void EventSlicingTool::GetKDTreeEntries3D(const ClusterToSliceIndexMap& clusterToSliceIndexMap,
                                            PointList& pointsU,
                                            PointList& pointsV,
                                            PointList& pointsW,
                                            PointToSliceIndexMap& pointToSliceIndexMap) const
  {
    ...
    for (const Cluster* const pCluster3D : clusterList) {
      ...
      for (const CaloHit* const pCaloHit3D : caloHitList) {
        if (TPC_3D != pCaloHit3D->GetHitType()) throw StatusCodeException(STATUS_CODE_FAILURE);

        const CartesianVector& position3D(pCaloHit3D->GetPositionVector());

        const CartesianVector* const pProjectionU(new CartesianVector(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), position3D, TPC_VIEW_U)));
        const CartesianVector* const pProjectionV(new CartesianVector(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), position3D, TPC_VIEW_V)));
        const CartesianVector* const pProjectionW(new CartesianVector(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), position3D, TPC_VIEW_W)));
  ...
  const EventSlicingTool::PointKDNode2D* EventSlicingTool::MatchClusterToSlice(
    const Cluster* const pCluster2D,
    PointKDTree2D& kdTree) const
  {
    PointList clusterPointList;
    const PointKDNode2D* pBestResultPoint(nullptr);

    try {
      clusterPointList.push_back(
      ...
      clusterPointList.push_back(
        new CartesianVector((pCluster2D->GetCentroid(pCluster2D->GetInnerPseudoLayer()) +
                             pCluster2D->GetCentroid(pCluster2D->GetOuterPseudoLayer())) *
                            0.5f));
      ...
      for (const CartesianVector* const pClusterPoint : clusterPointList) {
        ...
        const PointKDNode2D targetPoint(
          pClusterPoint, pClusterPoint->GetX(), pClusterPoint->GetZ());
  ...
  bool EventSlicingTool::SortPoints(const CartesianVector* const pLhs,
                                    const CartesianVector* const pRhs)
  {
  ...
  StatusCode EventSlicingTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "TrackPfoListName", m_trackPfoListName));

    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ShowerPfoListName", m_showerPfoListName));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinHitsPer3DCluster", m_minHitsPer3DCluster));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SlidingFitHalfWindow", m_halfWindowLayers));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "UsePointingAssociation", m_usePointingAssociation));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MinVertexLongitudinalDistance",
                                                         m_minVertexLongitudinalDistance));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexAngularAllowance", m_vertexAngularAllowance));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxClosestApproach", m_maxClosestApproach));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxInterceptDistance", m_maxInterceptDistance));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxHitSeparation", maxHitSeparation));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NConeFitLayers", m_nConeFitLayers));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "NConeFits", m_nConeFits));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConeLengthMultiplier", m_coneLengthMultiplier));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxConeLength", m_maxConeLength));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConeTanHalfAngle1", m_coneTanHalfAngle1));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConeBoundedFraction1", m_coneBoundedFraction1));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConeTanHalfAngle2", m_coneTanHalfAngle2));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConeBoundedFraction2", m_coneBoundedFraction2));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "Use3DProjectionsInHitPickUp",
