...
namespace lar_content {
  ...
  {
    if (m_forceSingleEmptyNeutrino) return this->ForceSingleEmptyNeutrino();
    ...
    if (!pVertexList || pVertexList->empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "NeutrinoCreationAlgorithm: unable to find vertex list " << m_vertexListName
                  << std::endl;
    ...
    for (VertexList::const_iterator vIter = pVertexList->begin(), vIterEnd = pVertexList->end();
         vIter != vIterEnd;
         ++vIter) {
  ...
  void NeutrinoCreationAlgorithm::FillDefaultNeutrinoParameters(
  ...
  StatusCode NeutrinoCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputVertexListName", m_vertexListName));
