...
namespace lar_content {
  ...
  NeutrinoDaughterVerticesAlgorithm::NeutrinoDaughterVerticesAlgorithm()
    : m_useParentShowerVertex(false), m_halfWindowLayers(20)
  {}
  ...
  StatusCode NeutrinoDaughterVerticesAlgorithm::Run()
  {
    ...
    PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                   ...
                                   !=,
                                   PandoraContentApi::GetList(*this, m_neutrinoListName, pPfoList));

    if (!pPfoList || pPfoList->empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "NeutrinoDaughterVerticesAlgorithm: unable to find pfo list "
  ...
  void NeutrinoDaughterVerticesAlgorithm::GetDaughterPfos(const PfoList* const pPfoList,
  ...
  void NeutrinoDaughterVerticesAlgorithm::BuildPointingClusterMap(
    const PfoVector& pfoList,
    LArPointingClusterMap& pointingClusterMap) const
  {
    const float slidingFitPitch(LArGeometryHelper::GetWireZPitch(this->GetPandora()));

    for (PfoVector::const_iterator pIter = pfoList.begin(), pIterEnd = pfoList.end();
         pIter != pIterEnd;
         ++pIter) {
  ...
  void NeutrinoDaughterVerticesAlgorithm::BuildDaughterParticles(
    ...
    const PfoVector& pfoVector) const
  {
    for (PfoVector::const_iterator iter = pfoVector.begin(), iterEnd = pfoVector.end();
      ...
      if (LArPfoHelper::IsTrack(pPfo)) { this->BuildDaughterTrack(pointingClusterMap, pPfo); }
      else {
  ...
  void NeutrinoDaughterVerticesAlgorithm::BuildDaughterTrack(
    const LArPointingClusterMap& pointingClusterMap,
    const ParticleFlowObject* const pDaughterPfo) const
  {
    ...
    if (parentList.empty() && pParentPfo->GetVertexList().empty()) return;
    ...
    for (ClusterList::const_iterator dIter = daughterList.begin(), dIterEnd = daughterList.end();
         dIter != dIterEnd;
         ++dIter) {
      ...
      for (ClusterList::const_iterator pIter = parentList.begin(), pIterEnd = parentList.end();
           pIter != pIterEnd;
           ++pIter) {
        const Cluster* const pParentCluster = *pIter;
        minDistance = std::min(minDistance,
                               (LArClusterHelper::GetClosestDistance(minPosition, pParentCluster)));
        maxDistance = std::min(maxDistance,
                               (LArClusterHelper::GetClosestDistance(maxPosition, pParentCluster)));
      }
  ...
  void NeutrinoDaughterVerticesAlgorithm::BuildDaughterShower(
    const ParticleFlowObject* const pDaughterPfo) const
  {
    if (pDaughterPfo->GetParentPfoList().size() != 1)
      throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    for (ClusterList::const_iterator dIter = daughterList.begin(), dIterEnd = daughterList.end();
      ...
      for (ClusterList::const_iterator pIter = parentList.begin(), pIterEnd = parentList.end();
        ...
        CartesianVector closestDaughterPosition(0.f, 0.f, 0.f),
          closestParentPosition(0.f, 0.f, 0.f);
        LArClusterHelper::GetClosestPositions(
          pDaughterCluster, pParentCluster, closestDaughterPosition, closestParentPosition);

        const float closestDistanceSquared(
          (closestDaughterPosition - closestParentPosition).GetMagnitudeSquared());
  ...
  StatusCode NeutrinoDaughterVerticesAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NeutrinoPfoListName", m_neutrinoListName));
