...
namespace lar_content {
  ...
  void VertexAssociatedPfosTool::Run(const NeutrinoHierarchyAlgorithm* const pAlgorithm,
                                     ...
                                     PfoInfoMap& pfoInfoMap)
  {
    ...
    for (const Pfo* const pPfo : sortedPfos) {
      ...
      if (pPfoInfo->IsNeutrinoVertexAssociated() || pPfoInfo->GetParentPfo()) continue;

      const LArPointingCluster pointingCluster(*(pPfoInfo->GetSlidingFitResult3D()));
      const bool useInner(
        (pointingCluster.GetInnerVertex().GetPosition() - neutrinoVertex).GetMagnitudeSquared() <
      ...
      if (LArPointingClusterHelper::IsNode(neutrinoVertex,
                                           daughterVertex,
                                           m_minVertexLongitudinalDistance,
                                           m_maxVertexTransverseDistance) ||
          LArPointingClusterHelper::IsEmission(neutrinoVertex,
                                               daughterVertex,
                                               m_minVertexLongitudinalDistance,
