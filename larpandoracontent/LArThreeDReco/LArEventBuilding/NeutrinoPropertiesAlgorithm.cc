...
namespace lar_content {
  ...
  StatusCode NeutrinoPropertiesAlgorithm::Run()
  {
    ...
    if (!pPfoList || pPfoList->empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "NeutrinoPropertiesAlgorithm: unable to find pfo list "
    ...
    const ParticleFlowObject* const pNeutrinoPfo((1 == pPfoList->size()) ? *(pPfoList->begin()) :
                                                                           nullptr);

    if (!pNeutrinoPfo || !LArPfoHelper::IsNeutrino(pNeutrinoPfo)) return STATUS_CODE_FAILURE;
    ...
    if (pNeutrinoPfo->GetDaughterPfoList().empty()) {
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::Delete(*this, pNeutrinoPfo, m_neutrinoPfoListName));
    }
    else {
  ...
  void NeutrinoPropertiesAlgorithm::SetNeutrinoId(
    const ParticleFlowObject* const pNeutrinoPfo) const
  {
    ...
    PfoVector daughterPfoVector(pNeutrinoPfo->GetDaughterPfoList().begin(),
                                pNeutrinoPfo->GetDaughterPfoList().end());
    ...
    if (!pPrimaryDaughter) throw StatusCodeException(STATUS_CODE_FAILURE);

    PandoraContentApi::ParticleFlowObject::Metadata metadata;

    if (E_MINUS == std::abs(pPrimaryDaughter->GetParticleId())) { metadata.m_particleId = NU_E; }
    ...
    if (metadata.m_particleId.IsInitialized()) {
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ParticleFlowObject::AlterMetadata(*this, pNeutrinoPfo, metadata));
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  unsigned int NeutrinoPropertiesAlgorithm::GetNTwoDHitsInPfoChain(
    const ParticleFlowObject* const pPfo) const
  {
    ...
    for (const Cluster* const pCluster : pPfo->GetClusterList()) {
      ...
      if ((TPC_VIEW_U == hitType) || (TPC_VIEW_V == hitType) || (TPC_VIEW_W == hitType)) {
        ...
        if (m_includeIsolatedHits) nTwoDHits += pCluster->GetNIsolatedCaloHits();
      }
    }
  ...
  StatusCode NeutrinoPropertiesAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "IncludeIsolatedHits", m_includeIsolatedHits));
