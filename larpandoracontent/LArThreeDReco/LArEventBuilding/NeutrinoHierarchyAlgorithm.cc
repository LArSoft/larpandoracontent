...
namespace lar_content {
  ...
  NeutrinoHierarchyAlgorithm::NeutrinoHierarchyAlgorithm()
    : m_halfWindowLayers(20), m_displayPfoInfoMap(false)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  void NeutrinoHierarchyAlgorithm::SeparatePfos(const PfoInfoMap& pfoInfoMap,
                                                PfoVector& assignedPfos,
                                                PfoVector& unassignedPfos) const
  {
  ...
  StatusCode NeutrinoHierarchyAlgorithm::Run()
  {
    ...
    try {
      ...
      std::cout << "NeutrinoHierarchyAlgorithm: unable to process input neutrino pfo, "
                << statusCodeException.ToString() << std::endl;
    }

    if (m_displayPfoInfoMap) this->DisplayPfoInfoMap(pNeutrinoPfo, pfoInfoMap);
  ...
  void NeutrinoHierarchyAlgorithm::GetNeutrinoPfo(const ParticleFlowObject*& pNeutrinoPfo) const
  {
    ...
    if (!pPfoList || pPfoList->empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "NeutrinoHierarchyAlgorithm: unable to find pfo list " << m_neutrinoPfoListName
                  << std::endl;
  ...
  void NeutrinoHierarchyAlgorithm::GetCandidateDaughterPfoList(
    PfoList& candidateDaughterPfoList) const
  {
    for (const std::string& daughterPfoListName : m_daughterPfoListNames) {
      ...
      if (STATUS_CODE_SUCCESS ==
          PandoraContentApi::GetList(*this, daughterPfoListName, pCandidatePfoList)) {
        candidateDaughterPfoList.insert(
          candidateDaughterPfoList.end(), pCandidatePfoList->begin(), pCandidatePfoList->end());
      }
      else if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo()) {
        std::cout << "NeutrinoHierarchyAlgorithm: unable to find pfo list " << daughterPfoListName
                  << std::endl;
      }
    }

    if (candidateDaughterPfoList.empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void NeutrinoHierarchyAlgorithm::GetInitialPfoInfoMap(const PfoList& pfoList,
                                                        PfoInfoMap& pfoInfoMap) const
  {
    ...
    for (const ParticleFlowObject* const pPfo : pfoList) {
      ...
      try {
        ...
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "NeutrinoHierarchyAlgorithm: Unable to calculate pfo information "
                    << std::endl;
  ...
  void NeutrinoHierarchyAlgorithm::ProcessPfoInfoMap(const ParticleFlowObject* const pNeutrinoPfo,
                                                     const PfoList& candidateDaughterPfoList,
                                                     PfoInfoMap& pfoInfoMap,
                                                     const unsigned int callDepth) const
  {
    PfoVector candidateDaughterPfoVector(candidateDaughterPfoList.begin(),
                                         candidateDaughterPfoList.end());
    std::sort(candidateDaughterPfoVector.begin(),
              candidateDaughterPfoVector.end(),
              LArPfoHelper::SortByNHits);
    ...
    for (const ParticleFlowObject* const pDaughterPfo : candidateDaughterPfoVector) {
      ...
      if ((pfoInfoMap.end() != iter) && (iter->second->IsNeutrinoVertexAssociated()))
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::SetPfoParentDaughterRelationship(*this, pNeutrinoPfo, pDaughterPfo));
    }
    ...
    if ((0 == callDepth) && (0 == pNeutrinoPfo->GetNDaughterPfos())) {
      ...
      return this->ProcessPfoInfoMap(
        pNeutrinoPfo, candidateDaughterPfoList, pfoInfoMap, callDepth + 1);
    }
    ...
    for (const Pfo* const pPfo : sortedPfos) {
      ...
      PfoVector daughterPfos(pPfoInfo->GetDaughterPfoList().begin(),
                             pPfoInfo->GetDaughterPfoList().end());
      ...
      for (const ParticleFlowObject* const pDaughterPfo : daughterPfos)
        PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                                !=,
                                PandoraContentApi::SetPfoParentDaughterRelationship(
    ...
    for (const ParticleFlowObject* const pRemainingPfo : candidateDaughterPfoVector) {
      if (!pRemainingPfo->GetParentPfoList().empty()) continue;

      // TODO Most appropriate decision - add as daughter of either i) nearest pfo, or ii) the neutrino (current approach)
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::SetPfoParentDaughterRelationship(*this, pNeutrinoPfo, pRemainingPfo));
    }
  }
  ...
  void NeutrinoHierarchyAlgorithm::AdjustVertexAndPfoInfo(
    const ParticleFlowObject* const pNeutrinoPfo,
    const PfoList& candidateDaughterPfoList,
    PfoInfoMap& pfoInfoMap) const
  {
    PfoVector candidateDaughterPfoVector(candidateDaughterPfoList.begin(),
    ...
    if (candidateDaughterPfoVector.empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
    ...
    if (daughterClusterList3D.empty()) return;

    const Vertex* pOldNeutrinoVertex(LArPfoHelper::GetVertex(pNeutrinoPfo));
    const CartesianVector newVertexPosition(LArClusterHelper::GetClosestPosition(
      pOldNeutrinoVertex->GetPosition(), daughterClusterList3D.front()));
    ...
    if (neutrinoVertexListName.empty())
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::GetCurrentListName<Vertex>(*this, neutrinoVertexListName));
    ...
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                            !=,
                            PandoraContentApi::CreateTemporaryListAndSetCurrent(
                              *this, pTemporaryVertexList, temporaryVertexListName));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::Vertex::Create(*this, parameters, pNewNeutrinoVertex));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::SaveList<Vertex>(*this, neutrinoVertexListName));

    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::RemoveFromPfo(*this, pNeutrinoPfo, pOldNeutrinoVertex));
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                            !=,
                            PandoraContentApi::AddToPfo(*this, pNeutrinoPfo, pNewNeutrinoVertex));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::Delete<Vertex>(*this, pOldNeutrinoVertex, neutrinoVertexListName));
  ...
  void NeutrinoHierarchyAlgorithm::DisplayPfoInfoMap(const ParticleFlowObject* const pNeutrinoPfo,
                                                     const PfoInfoMap& pfoInfoMap) const
  {
    bool display(false);
    PANDORA_MONITORING_API(
      SetEveDisplayParameters(this->GetPandora(), false, DETECTOR_VIEW_XZ, -1.f, -1.f, 1.f));
    std::cout << "-Neutrino Pfo, nDaughters " << pNeutrinoPfo->GetDaughterPfoList().size()
    ...
    for (const Pfo* const pPfo : sortedPfos) {
      ...
      std::cout << "Pfo " << pPfoInfo->GetThisPfo() << ", vtxAssoc "
                << pPfoInfo->IsNeutrinoVertexAssociated() << ", parent " << pPfoInfo->GetParentPfo()
      ...
      if (pPfoInfo->IsNeutrinoVertexAssociated()) {
        ...
        PANDORA_MONITORING_API(VisualizeParticleFlowObjects(
          this->GetPandora(), &tempPfoList, "VertexPfo", RED, true, false));
      }
    }

    if (display) {
      PANDORA_MONITORING_API(VisualizeVertices(
        this->GetPandora(), &(pNeutrinoPfo->GetVertexList()), "NeutrinoVertex", ORANGE));
    ...
    for (const Pfo* const pPfo : sortedPfos) {
      ...
      if (!pPfoInfo->GetDaughterPfoList().empty()) {
        ...
        const PfoList tempPfoList(1, pPfoInfo->GetThisPfo());
        PANDORA_MONITORING_API(VisualizeParticleFlowObjects(
          this->GetPandora(), &tempPfoList, "ParentPfo", RED, true, false));
        PANDORA_MONITORING_API(VisualizeParticleFlowObjects(this->GetPandora(),
                                                            &(pPfoInfo->GetDaughterPfoList()),
                                                            "DaughterPfos",
                                                            BLUE,
                                                            true,
                                                            false));
  ...
  NeutrinoHierarchyAlgorithm::PfoInfo::PfoInfo(const pandora::ParticleFlowObject* const pPfo,
    ...
    , m_pParentPfo(nullptr)
  {
    ...
    if (1 != clusterList3D.size()) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  {
    if (!rhs.m_pSlidingFitResult3D) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

    m_pSlidingFitResult3D = new ThreeDSlidingFitResult(
      m_pCluster3D,
      rhs.m_pSlidingFitResult3D->GetFirstFitResult().GetLayerFitHalfWindow(),
      rhs.m_pSlidingFitResult3D->GetFirstFitResult().GetLayerPitch());
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  NeutrinoHierarchyAlgorithm::PfoInfo& NeutrinoHierarchyAlgorithm::PfoInfo::operator=(
    const PfoInfo& rhs)
  {
    if (this != &rhs) {
      if (!rhs.m_pSlidingFitResult3D) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  NeutrinoHierarchyAlgorithm::PfoInfo::~PfoInfo() { delete m_pSlidingFitResult3D; }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void NeutrinoHierarchyAlgorithm::PfoInfo::SetNeutrinoVertexAssociation(
    const bool isNeutrinoVertexAssociated)
  {
  ...
  void NeutrinoHierarchyAlgorithm::PfoInfo::SetInnerLayerAssociation(
    const bool isInnerLayerAssociated)
  {
  ...
  void NeutrinoHierarchyAlgorithm::PfoInfo::SetParentPfo(
    const pandora::ParticleFlowObject* const pParentPfo)
  {
    if (m_pParentPfo) throw StatusCodeException(STATUS_CODE_ALREADY_PRESENT);
  ...
  void NeutrinoHierarchyAlgorithm::PfoInfo::RemoveParentPfo() { m_pParentPfo = nullptr; }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void NeutrinoHierarchyAlgorithm::PfoInfo::AddDaughterPfo(
    const pandora::ParticleFlowObject* const pDaughterPfo)
  {
    if (m_daughterPfoList.end() !=
        std::find(m_daughterPfoList.begin(), m_daughterPfoList.end(), pDaughterPfo))
  ...
  void NeutrinoHierarchyAlgorithm::PfoInfo::RemoveDaughterPfo(
    const pandora::ParticleFlowObject* const pDaughterPfo)
  {
    PfoList::iterator eraseIter =
      std::find(m_daughterPfoList.begin(), m_daughterPfoList.end(), pDaughterPfo);

    if (m_daughterPfoList.end() == eraseIter) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  StatusCode NeutrinoHierarchyAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                             ...
                             XmlHelper::ProcessAlgorithmToolList(
                               *this, xmlHandle, "PfoRelationTools", algorithmToolVector));
    ...
    for (AlgorithmToolVector::const_iterator iter = algorithmToolVector.begin(),
      ...
      if (!pPfoRelationTool) return STATUS_CODE_INVALID_PARAMETER;
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "DaughterPfoListNames", m_daughterPfoListNames));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NeutrinoVertexListName", m_neutrinoVertexListName));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "DisplayPfoInfoMap", m_displayPfoInfoMap));
