...
namespace lar_content {
  ...
  void BranchAssociatedPfosTool::Run(const NeutrinoHierarchyAlgorithm* const pAlgorithm,
                                     ...
                                     PfoInfoMap& pfoInfoMap)
  {
    ...
    while (associationsMade) {
      ...
      for (const ParticleFlowObject* const pParentPfo : assignedPfos) {
        ...
        const float parentLength3D((parentFitResult.GetGlobalMinLayerPosition() -
                                    parentFitResult.GetGlobalMaxLayerPosition())
                                     .GetMagnitude());
        const CartesianVector& parentVertexPosition(pParentPfoInfo->IsInnerLayerAssociated() ?
                                                      parentFitResult.GetGlobalMinLayerPosition() :
                                                      parentFitResult.GetGlobalMaxLayerPosition());
        ...
        for (const ParticleFlowObject* const pPfo : unassignedPfos) {
          ...
          const float dNeutrinoVertex(std::min(
            (pointingCluster.GetInnerVertex().GetPosition() - pNeutrinoVertex->GetPosition())
              .GetMagnitude(),
            (pointingCluster.GetOuterVertex().GetPosition() - pNeutrinoVertex->GetPosition())
              .GetMagnitude()));

          if (dNeutrinoVertex < m_minNeutrinoVertexDistance) continue;

          const float dParentVertex(std::min(
            (pointingCluster.GetInnerVertex().GetPosition() - parentVertexPosition).GetMagnitude(),
            (pointingCluster.GetOuterVertex().GetPosition() - parentVertexPosition)
              .GetMagnitude()));

          if (parentIsTrack && (dParentVertex < m_trackBranchAdditionFraction * parentLength3D))
            continue;

          const float dInnerVertex(LArClusterHelper::GetClosestDistance(
            pointingCluster.GetInnerVertex().GetPosition(), pParentCluster3D));
          const float dOuterVertex(LArClusterHelper::GetClosestDistance(
            pointingCluster.GetOuterVertex().GetPosition(), pParentCluster3D));

          if ((dInnerVertex < m_maxParentClusterDistance) ||
              (dOuterVertex < m_maxParentClusterDistance)) {
