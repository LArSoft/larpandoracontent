...
namespace lar_content {
  ...
  void EndAssociatedPfosTool::Run(const NeutrinoHierarchyAlgorithm* const pAlgorithm,
                                  ...
                                  PfoInfoMap& pfoInfoMap)
  {
    ...
    while (associationsMade) {
      ...
      for (const ParticleFlowObject* const pParentPfo : assignedPfos) {
        ...
        const LArPointingCluster::Vertex& parentEndpoint(pParentPfoInfo->IsInnerLayerAssociated() ?
                                                           parentPointingCluster.GetOuterVertex() :
                                                           parentPointingCluster.GetInnerVertex());
        const float neutrinoVertexDistance(
          (parentEndpoint.GetPosition() - pNeutrinoVertex->GetPosition()).GetMagnitude());

        if (neutrinoVertexDistance < m_minNeutrinoVertexDistance) continue;

        for (const ParticleFlowObject* const pPfo : unassignedPfos) {
          if (recentlyAssigned.count(pPfo)) continue;

          PfoInfo* const pPfoInfo(pfoInfoMap.at(pPfo));

          const LArPointingCluster pointingCluster(*(pPfoInfo->GetSlidingFitResult3D()));
          const bool useInner(
            (pointingCluster.GetInnerVertex().GetPosition() - parentEndpoint.GetPosition())
              .GetMagnitudeSquared() <
            (pointingCluster.GetOuterVertex().GetPosition() - parentEndpoint.GetPosition())
              .GetMagnitudeSquared());

          const LArPointingCluster::Vertex& daughterVertex(
            useInner ? pointingCluster.GetInnerVertex() : pointingCluster.GetOuterVertex());

          if (LArPointingClusterHelper::IsNode(parentEndpoint.GetPosition(),
                                               daughterVertex,
                                               m_minVertexLongitudinalDistance,
                                               m_maxVertexTransverseDistance) ||
              LArPointingClusterHelper::IsNode(daughterVertex.GetPosition(),
                                               parentEndpoint,
                                               m_minVertexLongitudinalDistance,
                                               m_maxVertexTransverseDistance) ||
              LArPointingClusterHelper::IsEmission(parentEndpoint.GetPosition(),
                                                   daughterVertex,
                                                   m_minVertexLongitudinalDistance,
                                                   m_maxVertexLongitudinalDistance,
                                                   m_maxVertexTransverseDistance,
                                                   m_vertexAngularAllowance) ||
              LArPointingClusterHelper::IsEmission(daughterVertex.GetPosition(),
                                                   parentEndpoint,
                                                   m_minVertexLongitudinalDistance,
                                                   m_maxVertexLongitudinalDistance,
                                                   m_maxVertexTransverseDistance,
                                                   m_vertexAngularAllowance) ||
              this->IsCloseToParentEndpoint(parentEndpoint.GetPosition(),
                                            pParentPfoInfo->GetCluster3D(),
                                            pPfoInfo->GetCluster3D())) {
  ...
  bool EndAssociatedPfosTool::IsCloseToParentEndpoint(const CartesianVector& parentEndpoint,
                                                      ...
                                                      const Cluster* const pDaughterCluster3D) const
  {
    try {
      ...
      LArClusterHelper::GetClosestPositions(
        pParentCluster3D, pDaughterCluster3D, parentPosition3D, daughterPosition3D);
