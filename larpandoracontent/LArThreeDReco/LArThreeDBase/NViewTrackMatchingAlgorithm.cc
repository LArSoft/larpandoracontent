...
namespace lar_content {
  ...
  template <typename T>
  const TwoDSlidingFitResult& NViewTrackMatchingAlgorithm<T>::GetCachedSlidingFitResult(
  ...
  bool NViewTrackMatchingAlgorithm<T>::MakeClusterSplits(const SplitPositionMap& splitPositionMap)
  {
    ...
    for (const Cluster* pCurrentCluster : splitClusters) {
      ...
      std::sort(splitPositions.begin(),
                splitPositions.end(),
                NViewTrackMatchingAlgorithm<T>::SortSplitPositions);
  ...
  template <typename T>
  bool NViewTrackMatchingAlgorithm<T>::MakeClusterSplit(const CartesianVector& splitPosition,
                                                        ...
                                                        const Cluster*& pHighXCluster) const
  {
    ...
    try {
      ...
      const bool innerIsLowX(pointingCluster.GetInnerVertex().GetPosition().GetX() <
                             pointingCluster.GetOuterVertex().GetPosition().GetX());
      lowXEnd = (innerIsLowX ? pointingCluster.GetInnerVertex().GetPosition() :
                               pointingCluster.GetOuterVertex().GetPosition());
      highXEnd = (innerIsLowX ? pointingCluster.GetOuterVertex().GetPosition() :
                                pointingCluster.GetInnerVertex().GetPosition());
    }
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      const CartesianVector unitVector(
        (pCaloHit->GetPositionVector() - splitPosition).GetUnitVector());
      ...
      const Cluster*& pClusterToModify((dotProductLowX > dotProductHighX) ? pLowXCluster :
                                                                            pHighXCluster);
  ...
  template <typename T>
  bool NViewTrackMatchingAlgorithm<T>::SortSplitPositions(const pandora::CartesianVector& lhs,
  ...
  template <typename T>
  void NViewTrackMatchingAlgorithm<T>::SelectInputClusters(
  ...
  template <typename T>
  void NViewTrackMatchingAlgorithm<T>::SetPfoParticleId(
  ...
  template class NViewTrackMatchingAlgorithm<
    TwoViewMatchingControl<TwoViewTransverseOverlapResult>>;
