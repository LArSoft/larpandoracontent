...
namespace lar_content {
  ...
  void TwoViewMatchingControl<T>::UpdateForNewCluster(const Cluster* const pNewCluster)
  {
    ...
    for (const Cluster* const pCluster2 : clusterVector2) {
      if (1 == iter->second) { m_pAlgorithm->CalculateOverlapResult(pNewCluster, pCluster2); }
      else {
  ...
  {
    ClusterList::iterator iter1 =
      std::find(m_clusterList1.begin(), m_clusterList1.end(), pDeletedCluster);
    ClusterList::iterator iter2 =
      std::find(m_clusterList2.begin(), m_clusterList2.end(), pDeletedCluster);

    if (m_clusterList1.end() != iter1) m_clusterList1.erase(iter1);

    if (m_clusterList2.end() != iter2) m_clusterList2.erase(iter2);
  ...
  {
    HitTypeToIndexMap::const_iterator iter = m_hitTypeToIndexMap.find(hitType);
    if (m_hitTypeToIndexMap.end() == iter) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

    if ((1 != iter->second) && (2 != iter->second)) throw StatusCodeException(STATUS_CODE_FAILURE);
  ...
  template <typename T>
  const pandora::ClusterList& TwoViewMatchingControl<T>::GetInputClusterList(
    const HitType hitType) const
  {
    HitTypeToIndexMap::const_iterator iter = m_hitTypeToIndexMap.find(hitType);
    if (m_hitTypeToIndexMap.end() == iter) throw StatusCodeException(STATUS_CODE_NOT_INITIALIZED);

    if ((1 == iter->second) && m_pInputClusterList1) return (*m_pInputClusterList1);

    if ((2 == iter->second) && m_pInputClusterList2) return (*m_pInputClusterList2);
  ...
  template <typename T>
  const pandora::ClusterList& TwoViewMatchingControl<T>::GetSelectedClusterList(
    const HitType hitType) const
  {
    HitTypeToIndexMap::const_iterator iter = m_hitTypeToIndexMap.find(hitType);
    if (m_hitTypeToIndexMap.end() == iter) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  void TwoViewMatchingControl<T>::SelectAllInputClusters()
  {
    ...
    if (!m_pInputClusterList1 || !m_pInputClusterList2) {
      if (PandoraContentApi::GetSettings(*m_pAlgorithm)->ShouldDisplayAlgorithmInfo())
        std::cout << "TwoViewMatchingControl: one or more input cluster lists unavailable."
                  << std::endl;
    ...
    if (!m_pInputClusterList1->empty())
      m_hitTypeToIndexMap.insert(HitTypeToIndexMap::value_type(
        LArClusterHelper::GetClusterHitType(m_pInputClusterList1->front()), 1));

    if (!m_pInputClusterList2->empty())
      m_hitTypeToIndexMap.insert(HitTypeToIndexMap::value_type(
        LArClusterHelper::GetClusterHitType(m_pInputClusterList2->front()), 2));
  ...
  StatusCode TwoViewMatchingControl<T>::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputClusterListName1", m_inputClusterListName1));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputClusterListName2", m_inputClusterListName2));
