...
namespace lar_content {
  ...
  void ThreeViewMatchingControl<T>::UpdateForNewCluster(const Cluster* const pNewCluster)
  {
    ...
    ClusterList& clusterList((TPC_VIEW_U == hitType) ?
                               m_clusterListU :
                               (TPC_VIEW_V == hitType) ? m_clusterListV : m_clusterListW);
  ...
  {
    ClusterList::iterator iterU =
      std::find(m_clusterListU.begin(), m_clusterListU.end(), pDeletedCluster);
    ClusterList::iterator iterV =
      std::find(m_clusterListV.begin(), m_clusterListV.end(), pDeletedCluster);
    ClusterList::iterator iterW =
      std::find(m_clusterListW.begin(), m_clusterListW.end(), pDeletedCluster);

    if (m_clusterListU.end() != iterU) m_clusterListU.erase(iterU);

    if (m_clusterListV.end() != iterV) m_clusterListV.erase(iterV);

    if (m_clusterListW.end() != iterW) m_clusterListW.erase(iterW);
  ...
  {
    if (TPC_VIEW_U == hitType) return m_inputClusterListNameU;

    if (TPC_VIEW_V == hitType) return m_inputClusterListNameV;

    if (TPC_VIEW_W == hitType) return m_inputClusterListNameW;
  ...
  template <typename T>
  const pandora::ClusterList& ThreeViewMatchingControl<T>::GetInputClusterList(
    const HitType hitType) const
  {
    if ((TPC_VIEW_U == hitType) && m_pInputClusterListU) return (*m_pInputClusterListU);

    if ((TPC_VIEW_V == hitType) && m_pInputClusterListV) return (*m_pInputClusterListV);

    if ((TPC_VIEW_W == hitType) && m_pInputClusterListW) return (*m_pInputClusterListW);
  ...
  template <typename T>
  const pandora::ClusterList& ThreeViewMatchingControl<T>::GetSelectedClusterList(
    const HitType hitType) const
  {
    if (TPC_VIEW_U == hitType) return m_clusterListU;

    if (TPC_VIEW_V == hitType) return m_clusterListV;

    if (TPC_VIEW_W == hitType) return m_clusterListW;
  ...
  void ThreeViewMatchingControl<T>::SelectAllInputClusters()
  {
    ...
    if (!m_pInputClusterListU || !m_pInputClusterListV || !m_pInputClusterListW) {
      if (PandoraContentApi::GetSettings(*m_pAlgorithm)->ShouldDisplayAlgorithmInfo())
        std::cout << "ThreeViewMatchingControl: one or more input cluster lists unavailable."
                  << std::endl;
