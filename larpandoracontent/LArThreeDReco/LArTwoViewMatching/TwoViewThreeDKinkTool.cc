...
namespace lar_content {
  ...
  bool TwoViewThreeDKinkTool::Run(TwoViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  bool TwoViewThreeDKinkTool::PassesElementCuts(MatrixType::ElementList::const_iterator eIter,
                                                const ClusterSet& usedClusters) const
  {
    if (usedClusters.count(eIter->GetCluster1()) || usedClusters.count(eIter->GetCluster2()))
      return false;

    if (eIter->GetOverlapResult().GetTwoViewXOverlap().GetXOverlapFraction0() -
          m_minXOverlapFraction <
        std::numeric_limits<float>::epsilon())
      return false;

    if (eIter->GetOverlapResult().GetTwoViewXOverlap().GetXOverlapFraction1() -
          m_minXOverlapFraction <
        std::numeric_limits<float>::epsilon())
      return false;
    ...
    if (eIter->GetOverlapResult().GetLocallyMatchedFraction() - m_minLocallyMatchedFraction <
        std::numeric_limits<float>::epsilon())
      return false;
  ...
  float TwoViewThreeDKinkTool::GetXSamplingPoint(const CartesianVector& splitPosition1,
                                                 ...
                                                 const TwoDSlidingFitResult& fitResult2) const
  {
    ...
    const int lowLayer(
      std::max(fitResult1.GetMinLayer(),
               std::min(fitResult1.GetMaxLayer(), splitLayer - m_nLayersForKinkSearch)));
    const int highLayer(
      std::max(fitResult1.GetMinLayer(),
               std::min(fitResult1.GetMaxLayer(), splitLayer + m_nLayersForKinkSearch)));
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, fitResult1.GetGlobalFitPosition(fitResult1.GetL(lowLayer), minus));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, fitResult1.GetGlobalFitPosition(fitResult1.GetL(highLayer), plus));

    if (minus.GetX() > plus.GetX()) std::swap(minus, plus);

    const float layerStepX(isForwardInX ? plus.GetX() : minus.GetX());

    // Final x position selection
    const float chosenX(isForwardInX ? std::max(layerStepX, commonX) :
                                       std::min(layerStepX, commonX));
    const float finalX(isForwardInX ? chosenX + m_additionalXStepForKinkSearch :
                                      chosenX - m_additionalXStepForKinkSearch);
    return finalX;
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool TwoViewThreeDKinkTool::IsALowestInX(const LArPointingCluster& pointingClusterA,
                                           const LArPointingCluster& pointingClusterB)
  {
    const float xMinA{std::min(pointingClusterA.GetInnerVertex().GetPosition().GetX(),
                               pointingClusterA.GetOuterVertex().GetPosition().GetX())};
    const float xMinB{std::min(pointingClusterB.GetInnerVertex().GetPosition().GetX(),
                               pointingClusterB.GetOuterVertex().GetPosition().GetX())};
    return xMinA < xMinB;
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void TwoViewThreeDKinkTool::GetModifications(TwoViewTransverseTracksAlgorithm* const pAlgorithm,
                                               ...
                                               ModificationList& modificationList) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      if (!pKeyCluster->IsAvailable()) continue;
      ...
      for (MatrixType::ElementList::const_iterator eIter = elementList.begin();
           eIter != elementList.end();
           ++eIter) {
        if (!this->PassesElementCuts(eIter, usedClusters)) continue;

        IteratorList iteratorList;
        this->SelectMatrixElements(eIter, elementList, usedClusters, iteratorList);

        if (iteratorList.size() < 2) continue;

        ModificationList localModificationList;
        this->GetIteratorListModifications(pAlgorithm, iteratorList, localModificationList);

        if (localModificationList.empty()) continue;

        for (ModificationList::const_iterator mIter = localModificationList.begin(),
                                              mIterEnd = localModificationList.end();
             mIter != mIterEnd;
             ++mIter) {
          usedClusters.insert(mIter->m_affectedClusters.begin(), mIter->m_affectedClusters.end());
        }

        modificationList.insert(
          modificationList.end(), localModificationList.begin(), localModificationList.end());
      }
    }
  }
  ...
  bool TwoViewThreeDKinkTool::ApplyChanges(TwoViewTransverseTracksAlgorithm* const pAlgorithm,
                                           const ModificationList& modificationList) const
  {
    ...
    for (const Modification& modification : modificationList) {
      ...
      for (const Cluster* const pParentCluster : parentClusters) {
        ...
        targetClusterList.insert(
          targetClusterList.end(), daughterClusters.begin(), daughterClusters.end());
      }
      ...
      for (const Cluster* const pSplitCluster : splitClusters) {
        const CartesianPointVector& splitPositions(
          modification.m_splitPositionMap.at(pSplitCluster));

        CartesianPointVector& cartesianPointVector(consolidatedSplitMap[pSplitCluster]);
        cartesianPointVector.insert(
          cartesianPointVector.end(), splitPositions.begin(), splitPositions.end());
      }
    }
                                                   ...
                                                   IteratorList& iteratorList) const
  {
    ...
    for (MatrixType::ElementList::const_iterator eIter2 = elementList.begin();
         eIter2 != elementList.end();
         ++eIter2) {
      if (eIter == eIter2) continue;

      if (!this->PassesElementCuts(eIter2, usedClusters)) continue;

      for (IteratorList::const_iterator iIter = iteratorList.begin(); iIter != iteratorList.end();
           ++iIter) {
        if ((*iIter) == eIter2) continue;

        unsigned int nMatchedClusters(0);

        if ((*iIter)->GetCluster1() == eIter2->GetCluster1()) ++nMatchedClusters;

        if ((*iIter)->GetCluster2() == eIter2->GetCluster2()) ++nMatchedClusters;

        if (nMatchedClusters > 1) continue;
  ...
  void TwoViewThreeDKinkTool::GetIteratorListModifications(
    TwoViewTransverseTracksAlgorithm* const pAlgorithm,
    const IteratorList& iteratorList,
    ModificationList& modificationList) const
  {
    for (IteratorList::const_iterator iIter1 = iteratorList.begin(), iIter1End = iteratorList.end();
         iIter1 != iIter1End;
         ++iIter1) {
      for (IteratorList::const_iterator iIter2 = std::next(iIter1); iIter2 != iIter1End; ++iIter2) {
        try {
          const unsigned int nMatchedReUpsampledSamplingPoints1(
            (*iIter1)->GetOverlapResult().GetNMatchedReUpsampledSamplingPoints());
          const unsigned int nMatchedReUpsampledSamplingPoints2(
            (*iIter2)->GetOverlapResult().GetNMatchedReUpsampledSamplingPoints());
          IteratorList::const_iterator iIterA(
            (nMatchedReUpsampledSamplingPoints1 >= nMatchedReUpsampledSamplingPoints2) ? iIter1 :
                                                                                         iIter2);
          IteratorList::const_iterator iIterB(
            (nMatchedReUpsampledSamplingPoints1 >= nMatchedReUpsampledSamplingPoints2) ? iIter2 :
                                                                                         iIter1);

          Particle particle(*(*iIterA), *(*iIterB));
          const LArPointingCluster pointingClusterA(
          ...
          LArPointingClusterHelper::GetClosestVertices(
            pointingClusterA, pointingClusterB, vertexA, vertexB);

          float transverseAB(std::numeric_limits<float>::max()),
            transverseBA(std::numeric_limits<float>::max());
          float longitudinalAB(-std::numeric_limits<float>::max()),
            longitudinalBA(-std::numeric_limits<float>::max());

          LArPointingClusterHelper::GetImpactParameters(
            vertexA, vertexB, longitudinalAB, transverseAB);
          LArPointingClusterHelper::GetImpactParameters(
            vertexB, vertexA, longitudinalBA, transverseBA);

          if (std::min(longitudinalAB, longitudinalBA) < m_minLongitudinalImpactParameter) continue;

          if (std::min(transverseAB, transverseBA) > m_maxTransverseImpactParameter) continue;

          const float cosTheta(-vertexA.GetDirection().GetCosOpeningAngle(vertexB.GetDirection()));

          if (cosTheta < m_minImpactParameterCosTheta) continue;

          const bool isALowestInX(this->IsALowestInX(pointingClusterA, pointingClusterB));
          const CartesianVector splitPosition((vertexA.GetPosition() + vertexB.GetPosition()) *
                                              0.5f);
          const bool isThreeDKink(
            this->IsThreeDKink(pAlgorithm, particle, splitPosition, isALowestInX));

          // Construct the modification object
          Modification modification;

          if (isThreeDKink) {
            const TwoDSlidingFitResult& fitResultCommon(
              pAlgorithm->GetCachedSlidingFitResult(particle.m_pCommonCluster));

            CartesianVector splitPositionCommon(0.f, 0.f, 0.f);
            if (STATUS_CODE_SUCCESS != fitResultCommon.GetGlobalFitPositionAtX(
                                         splitPosition.GetX(), splitPositionCommon)) {
              continue;
            }

            modification.m_splitPositionMap[particle.m_pCommonCluster].push_back(
              splitPositionCommon);
          }
          else {
            const bool vertexAIsLowX(vertexA.GetPosition().GetX() < vertexB.GetPosition().GetX());
            const Cluster* const pLowXCluster(vertexAIsLowX ? particle.m_pClusterA :
                                                              particle.m_pClusterB);
            const Cluster* const pHighXCluster(vertexAIsLowX ? particle.m_pClusterB :
                                                               particle.m_pClusterA);
  ...
  bool TwoViewThreeDKinkTool::IsThreeDKink(TwoViewTransverseTracksAlgorithm* const pAlgorithm,
                                           ...
                                           const bool isALowestInX) const
  {
    try {
      const TwoDSlidingFitResult& fitResultCommon(
        pAlgorithm->GetCachedSlidingFitResult(particle.m_pCommonCluster));
      const TwoDSlidingFitResult& lowXFitResult(
        isALowestInX ? pAlgorithm->GetCachedSlidingFitResult(particle.m_pClusterA) :
                       pAlgorithm->GetCachedSlidingFitResult(particle.m_pClusterB));
      const TwoDSlidingFitResult& highXFitResult(
        isALowestInX ? pAlgorithm->GetCachedSlidingFitResult(particle.m_pClusterB) :
                       pAlgorithm->GetCachedSlidingFitResult(particle.m_pClusterA));

      const float minusX(
        this->GetXSamplingPoint(splitPosition, false, lowXFitResult, fitResultCommon));
      const float plusX(
        this->GetXSamplingPoint(splitPosition, true, highXFitResult, fitResultCommon));
      ...
      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), hitType1, hitType2, minus1, minus2, minus, chi2Dummy);
      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), hitType1, hitType2, split1, split2, split, chi2Dummy);
      LArGeometryHelper::MergeTwoPositions3D(
        this->GetPandora(), hitType1, hitType2, plus1, plus2, plus, chi2Dummy);
      ...
      if (dotProduct < m_cosThetaCutForKinkSearch) return true;
    }
  ...
  TwoViewThreeDKinkTool::Particle::Particle(const MatrixType::Element& elementA,
                                            const MatrixType::Element& elementB)
  {
    m_pClusterA = (elementA.GetCluster1() != elementB.GetCluster1()) ? elementA.GetCluster1() :
                                                                       elementA.GetCluster2();
    m_pClusterB = (elementA.GetCluster1() != elementB.GetCluster1()) ? elementB.GetCluster1() :
                                                                       elementB.GetCluster2();
    m_pCommonCluster = (elementA.GetCluster1() == elementB.GetCluster1()) ? elementA.GetCluster1() :
                                                                            elementA.GetCluster2();
  ...
  StatusCode TwoViewThreeDKinkTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinMatchingScore", m_minMatchingScore));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MinLongitudinalImpactParameter",
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NLayersForKinkSearch", m_nLayersForKinkSearch));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "AdditionalXStepForKinkSearch",
