...
namespace lar_content {
  ...
  bool TwoViewLongTracksTool::HasLongDirectConnections(IteratorList::const_iterator iIter,
                                                       const IteratorList& iteratorList)
  {
    for (IteratorList::const_iterator iIter2 = iteratorList.begin(), iIter2End = iteratorList.end();
      ...
      if (((*iIter)->GetCluster1() == (*iIter2)->GetCluster1()) ||
          ((*iIter)->GetCluster2() == (*iIter2)->GetCluster2()))
        return true;
    }
  ...
  bool TwoViewLongTracksTool::IsLongerThanDirectConnections(
  ...
  {
    const unsigned int nMatchedReUpsampledSamplingPoints(
      (*iIter)->GetOverlapResult().GetNMatchedReUpsampledSamplingPoints());
    ...
    for (MatrixType::ElementList::const_iterator eIter = elementList.begin();
      ...
      if (((*iIter)->GetCluster1() != eIter->GetCluster1()) &&
          ((*iIter)->GetCluster2() != eIter->GetCluster2()))
        continue;

      if (nMatchedReUpsampledSamplingPoints <
          minMatchedSamplingPointRatio *
            eIter->GetOverlapResult().GetNMatchedReUpsampledSamplingPoints())
        return false;
    }
  ...
  bool TwoViewLongTracksTool::Run(TwoViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  void TwoViewLongTracksTool::FindLongTracks(const MatrixType& overlapMatrix,
                                             ProtoParticleVector& protoParticleVector) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      for (IteratorList::const_iterator iIter = iteratorList.begin(), iIterEnd = iteratorList.end();
           iIter != iIterEnd;
           ++iIter) {
        if (TwoViewLongTracksTool::HasLongDirectConnections(iIter, iteratorList)) continue;

        if (!TwoViewLongTracksTool::IsLongerThanDirectConnections(
  ...
  void TwoViewLongTracksTool::SelectLongElements(const MatrixType::ElementList& elementList,
                                                 const pandora::ClusterSet& usedClusters,
                                                 IteratorList& iteratorList) const
  {
    for (MatrixType::ElementList::const_iterator eIter = elementList.begin();
      ...
      if (eIter->GetOverlapResult().GetLocallyMatchedFraction() < m_minMatchedFraction) continue;

      if (eIter->GetOverlapResult().GetMatchingScore() < m_minMatchingScore) continue;

      if (eIter->GetOverlapResult().GetNMatchedReUpsampledSamplingPoints() <
