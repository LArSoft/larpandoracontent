...
namespace lar_content {
  ...
  bool TwoViewSimpleTracksTool::Run(TwoViewTransverseTracksAlgorithm* const pAlgorithm,
  ...
  void TwoViewSimpleTracksTool::FindBestTrack(const MatrixType& overlapMatrix,
                                              ProtoParticleVector& protoParticleVector) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      for (MatrixType::ElementList::const_reverse_iterator iIter = elementList.rbegin();
           iIter != elementList.rend();
           ++iIter) {
        if (this->PassesElementCuts(iIter)) {
          if ((nullptr == iIter->GetCluster1()) || (nullptr == iIter->GetCluster2())) continue;
  ...
  bool TwoViewSimpleTracksTool::PassesElementCuts(
    MatrixType::ElementList::const_reverse_iterator eIter) const
  {
    if (!eIter->GetOverlapResult().IsInitialized()) return false;

    if (eIter->GetOverlapResult().GetLocallyMatchedFraction() < m_minMatchedFraction) return false;

    if (eIter->GetOverlapResult().GetMatchingScore() < m_minMatchingScore) return false;
