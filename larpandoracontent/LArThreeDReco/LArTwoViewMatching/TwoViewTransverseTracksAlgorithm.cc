...
namespace lar_content {
  ...
  void TwoViewTransverseTracksAlgorithm::CalculateOverlapResult(const Cluster* const pCluster1,
    ...
    TwoViewTransverseOverlapResult& overlapResult)
  {
    ...
    if (daughterVolumeIntersection.empty()) return STATUS_CODE_NOT_FOUND;

    if (this->GetPrimaryAxisDotDriftAxis(pCluster1) > m_maxDotProduct ||
        this->GetPrimaryAxisDotDriftAxis(pCluster2) > m_maxDotProduct)
    ...
    if (twoViewXOverlap.GetXSpan0() < std::numeric_limits<float>::epsilon() ||
        twoViewXOverlap.GetXSpan1() < std::numeric_limits<float>::epsilon())
    ...
    const CartesianVector boundingBoxMin1(xOverlapMin, 0.f, zMin1),
      boundingBoxMax1(xOverlapMax, 0.f, zMax1);
    const CartesianVector boundingBoxMin2(xOverlapMin, 0.f, zMin2),
      boundingBoxMax2(xOverlapMax, 0.f, zMax2);

    pandora::CaloHitList overlapHits1, overlapHits2;
    LArClusterHelper::GetCaloHitListInBoundingBox(
      pCluster1, boundingBoxMin1, boundingBoxMax1, overlapHits1);
    LArClusterHelper::GetCaloHitListInBoundingBox(
      pCluster2, boundingBoxMin2, boundingBoxMax2, overlapHits2);

    if (m_minSamples > std::min(overlapHits1.size(), overlapHits2.size()))
      return STATUS_CODE_NOT_FOUND;

    if (1 > m_downsampleFactor) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
    const unsigned int nSamples(
      std::max(m_minSamples,
               static_cast<unsigned int>(std::min(overlapHits1.size(), overlapHits2.size())) /
                 m_downsampleFactor));
    ...
    for (unsigned int iSample = 0; iSample < nSamples; ++iSample) {
      resamplingPointsX.emplace_back(
        (xOverlapMin + (xOverlapMax - xOverlapMin) * static_cast<float>(iSample + 1) /
                         static_cast<float>(nSamples + 1)));
    }

    const DiscreteProbabilityVector resampledDiscreteProbabilityVector1(discreteProbabilityVector1,
                                                                        resamplingPointsX);
    const DiscreteProbabilityVector resampledDiscreteProbabilityVector2(discreteProbabilityVector2,
                                                                        resamplingPointsX);
    ...
    const float pvalue(
      LArDiscreteProbabilityHelper::CalculateCorrelationCoefficientPValueFromPermutationTest(
        resampledDiscreteProbabilityVector1,
        resampledDiscreteProbabilityVector2,
        m_randomNumberGenerator,
        m_nPermutations));

    const float matchingScore(1.f - pvalue);
    if (matchingScore < m_minOverallMatchingScore) return STATUS_CODE_NOT_FOUND;

    const unsigned int nLocallyMatchedSamplingPoints(
      this->CalculateNumberOfLocallyMatchingSamplingPoints(resampledDiscreteProbabilityVector1,
                                                           resampledDiscreteProbabilityVector2,
                                                           m_randomNumberGenerator));
    const int nComparisons(static_cast<int>(resampledDiscreteProbabilityVector1.GetSize()) -
                           (static_cast<int>(m_minSamples) - 1));
    if (1 > nComparisons) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

    const float locallyMatchedFraction(static_cast<float>(nLocallyMatchedSamplingPoints) /
                                       static_cast<float>(nComparisons));
    if (locallyMatchedFraction < m_minOverallLocallyMatchedFraction) return STATUS_CODE_NOT_FOUND;

    overlapResult = TwoViewTransverseOverlapResult(matchingScore,
                                                   m_downsampleFactor,
                                                   nComparisons,
                                                   nLocallyMatchedSamplingPoints,
                                                   correlation,
                                                   twoViewXOverlap);
    ...
    std::mt19937& randomNumberGenerator)
  {
    ...
    if (m_minSamples > discreteProbabilityVector1.GetSize()) throw STATUS_CODE_INVALID_PARAMETER;
    ...
    for (unsigned int iValue = 0; iValue < discreteProbabilityVector1.GetSize(); ++iValue) {
      ...
      if (localValues1.size() == m_minSamples) {
        ...
        try {
          ...
          std::cout << "TwoViewTransverseTracksAlgorithm: failed to calculate correlation "
                       "coefficient p-value for these numbers"
                    << std::endl;
          ;
          std::cout << "----view 0: ";
        ...
        if ((1.f - localPValue) - m_localMatchingScoreThreshold >
  ...
  float TwoViewTransverseTracksAlgorithm::GetPrimaryAxisDotDriftAxis(
  ...
  void TwoViewTransverseTracksAlgorithm::ExamineOverlapContainer()
  {
    ...
    for (MatrixToolVector::const_iterator iter = m_algorithmToolVector.begin(),
  ...
  StatusCode TwoViewTransverseTracksAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    for (AlgorithmToolVector::const_iterator iter = algorithmToolVector.begin(),
      ...
      if (!pTransverseMatrixTool) return STATUS_CODE_INVALID_PARAMETER;
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "DownsampleFactor", m_downsampleFactor));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "MinSamples", m_minSamples));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NPermutations", m_nPermutations));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "LocalMatchingScoreThreshold",
                                                         m_localMatchingScoreThreshold));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxDotProduct", m_maxDotProduct));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MinOverallLocallyMatchedFraction",
                                                         m_minOverallLocallyMatchedFraction));
