 ...
 */
  class TwoViewTransverseTracksAlgorithm
    : public NViewTrackMatchingAlgorithm<TwoViewMatchingControl<TwoViewTransverseOverlapResult>> {
  public:
    typedef NViewTrackMatchingAlgorithm<TwoViewMatchingControl<TwoViewTransverseOverlapResult>>
      BaseAlgorithm;
  ...
  private:
     ...
     */
    unsigned int CalculateNumberOfLocallyMatchingSamplingPoints(
    ...
    unsigned int
      m_downsampleFactor; ///< The downsampling (hit merging) applied to hits in the overlap region
    ...
    float
      m_localMatchingScoreThreshold; ///< The minimum score to classify a local region as matching
    float
      m_maxDotProduct; ///M The maximum allowed cluster primary qxis Dot drift axis to fill the overlap result
    float
      m_minOverallMatchingScore; ///< The minimum required global matching score to fill the overlap result
    float
      m_minOverallLocallyMatchedFraction; ///< The minimum required lcoally matched fraction to fill the overlap result
 ...
 */
  ...
  public:
     ...
     */
    virtual bool Run(TwoViewTransverseTracksAlgorithm* const pAlgorithm,
                     MatrixType& overlapMatrix) = 0;
  };
