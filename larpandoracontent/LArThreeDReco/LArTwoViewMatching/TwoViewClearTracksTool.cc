...
namespace lar_content {
  ...
  TwoViewClearTracksTool::TwoViewClearTracksTool()
    : m_minXOverlapFraction(0.1f), m_minMatchingScore(0.95f), m_minLocallyMatchedFraction(0.3f)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool TwoViewClearTracksTool::Run(TwoViewTransverseTracksAlgorithm* const pAlgorithm,
    ...
    bool& particlesMade) const
  {
    ...
    for (MatrixType::ElementList::const_iterator iter = elementList.begin(),
                                                 iterEnd = elementList.end();
         iter != iterEnd;
         ++iter) {
      if (iter->GetOverlapResult().GetTwoViewXOverlap().GetXOverlapFraction0() -
            m_minXOverlapFraction <
          -1.f * std::numeric_limits<float>::epsilon())
        continue;
      if (iter->GetOverlapResult().GetTwoViewXOverlap().GetXOverlapFraction1() -
            m_minXOverlapFraction <
          -1.f * std::numeric_limits<float>::epsilon())
        continue;

      if (iter->GetOverlapResult().GetMatchingScore() - m_minMatchingScore <
          std::numeric_limits<float>::epsilon())
        continue;

      if (iter->GetOverlapResult().GetLocallyMatchedFraction() - m_minLocallyMatchedFraction <
          std::numeric_limits<float>::epsilon())
        continue;
