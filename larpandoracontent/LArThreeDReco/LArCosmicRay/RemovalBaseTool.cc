...
namespace lar_content {
  ...
  bool RemovalBaseTool::PassElementChecks(const TensorType::Element& element,
  ...
  bool RemovalBaseTool::IsMuonEndpoint(const TensorType::Element& element,
                                       const bool ignoreHitType,
                                       const HitType hitTypeToIgnore) const
  {
    for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W}) {
      if (ignoreHitType && (hitType == hitTypeToIgnore)) continue;
      ...
      if ((xMinDR < xMinCR) || (xMaxDR > xMaxCR)) return true;
      ...
      if ((zMinDR < zMinCR) || (zMaxDR > zMaxCR)) return true;
    }
  ...
  bool RemovalBaseTool::IsBestElement(const TensorType::Element& element,
                                      ...
                                      const ClusterSet& modifiedClusters) const
  {
    ...
    const unsigned int hitSum(element.GetClusterU()->GetNCaloHits() +
                              element.GetClusterV()->GetNCaloHits() +
                              element.GetClusterW()->GetNCaloHits());

    for (const TensorType::Element& testElement : elementList) {
      if (modifiedClusters.count(testElement.GetClusterU()) ||
          modifiedClusters.count(testElement.GetClusterV()) ||
          modifiedClusters.count(testElement.GetClusterW()))
        continue;

      if (testElement.GetCluster(hitType) != element.GetCluster(hitType)) continue;

      if ((testElement.GetClusterU() == element.GetClusterU()) &&
          (testElement.GetClusterV() == element.GetClusterV()) &&
          (testElement.GetClusterW() == element.GetClusterW()))
        continue;

      const unsigned int testHitSum(testElement.GetClusterU()->GetNCaloHits() +
                                    testElement.GetClusterV()->GetNCaloHits() +
                                    testElement.GetClusterW()->GetNCaloHits());
  ...
  bool RemovalBaseTool::IsCloseToLine(const CartesianVector& hitPosition,
  ...
  bool RemovalBaseTool::IsInLineSegment(const CartesianVector& lowerBoundary,
  ...
  void RemovalBaseTool::FindExtrapolatedHits(const Cluster* const pCluster,
                                             const CartesianVector& lowerBoundary,
                                             const CartesianVector& upperBoundary,
                                             CaloHitList& collectedHits) const
  {
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      ...
      if (!this->IsCloseToLine(
            pCaloHit->GetPositionVector(), lowerBoundary, upperBoundary, m_distanceToLine))
        continue;
  ...
  StatusCode RemovalBaseTool::ProjectDeltaRayPositions(
