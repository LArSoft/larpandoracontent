...
namespace lar_content {
  ...
  void DeltaRayMatchingContainers::FillContainers(const PfoList& inputPfoList,
                                                  const ClusterList& inputClusterList1,
                                                  const ClusterList& inputClusterList2,
  ...
  void DeltaRayMatchingContainers::AddToClusterMap(const Cluster* const pCluster)
  {
    ...
    HitToClusterMap& hitToClusterMap(
      (hitType == TPC_VIEW_U) ? m_hitToClusterMapU :
                                (hitType == TPC_VIEW_V) ? m_hitToClusterMapV : m_hitToClusterMapW);
  ...
  {
    if (inputClusterList.empty()) return;
  ...
  void DeltaRayMatchingContainers::BuildKDTree(const HitType hitType)
  {
    const HitToClusterMap& hitToClusterMap(
      (hitType == TPC_VIEW_U) ? m_hitToClusterMapU :
                                (hitType == TPC_VIEW_V) ? m_hitToClusterMapV : m_hitToClusterMapW);
    HitKDTree2D& kdTree((hitType == TPC_VIEW_U) ? m_kdTreeU :
                                                  (hitType == TPC_VIEW_V) ? m_kdTreeV : m_kdTreeW);
  ...
  void DeltaRayMatchingContainers::AddToClusterProximityMap(const Cluster* const pCluster)
  {
    ...
    HitKDTree2D& kdTree((hitType == TPC_VIEW_U) ? m_kdTreeU :
                                                  (hitType == TPC_VIEW_V) ? m_kdTreeV : m_kdTreeW);
    ClusterProximityMap& clusterProximityMap((hitType == TPC_VIEW_U) ?
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      ...
      for (const auto& hit : found) {
        ...
        if (pNearbyCluster == pCluster) continue;

        ClusterList& nearbyClusterList(clusterProximityMap[pCluster]);

        if (std::find(nearbyClusterList.begin(), nearbyClusterList.end(), pNearbyCluster) ==
            nearbyClusterList.end())
          nearbyClusterList.push_back(pNearbyCluster);

        ClusterList& invertedNearbyClusterList(clusterProximityMap[pNearbyCluster]);

        if (std::find(invertedNearbyClusterList.begin(),
                      invertedNearbyClusterList.end(),
                      pCluster) == invertedNearbyClusterList.end())
  ...
  void DeltaRayMatchingContainers::AddClustersToPfoMaps(const ParticleFlowObject* const pPfo)
  {
    for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W}) {
      ...
      ClusterToPfoMap& clusterToPfoMap((hitType == TPC_VIEW_U) ?
                                         m_clusterToPfoMapU :
                                         (hitType == TPC_VIEW_V) ? m_clusterToPfoMapV :
                                                                   m_clusterToPfoMapW);

      for (const Cluster* const pCluster : pfoClusters) {
        if (clusterToPfoMap.find(pCluster) != clusterToPfoMap.end()) continue;
  ...
  void DeltaRayMatchingContainers::AddClustersToContainers(const ClusterVector& newClusterVector,
                                                           const PfoVector& pfoVector)
  {
    ...
    for (unsigned int i = 0; i < newClusterVector.size(); i++) {
      ...
      if (pMuonPfo) this->AddClustersToPfoMaps(pMuonPfo);
    }
  }
  ...
  void DeltaRayMatchingContainers::RemoveClusterFromContainers(const Cluster* const pDeletedCluster)
  {
    ...
    ClusterToPfoMap& clusterToPfoMap(
      (hitType == TPC_VIEW_U) ? m_clusterToPfoMapU :
                                (hitType == TPC_VIEW_V) ? m_clusterToPfoMapV : m_clusterToPfoMapW);
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      ...
      if (iter == hitToClusterMap.end()) throw StatusCodeException(STATUS_CODE_FAILURE);

      hitToClusterMap.erase(iter);
    }

    const ClusterProximityMap::const_iterator clusterProximityIter(
      clusterProximityMap.find(pDeletedCluster));
    ...
    if (clusterProximityIter != clusterProximityMap.end()) {
      ...
      for (const Cluster* const pNearbyCluster : nearbyClusterList) {
        ...
        ClusterList::iterator invertedIter(
          std::find(invertedCloseClusters.begin(), invertedCloseClusters.end(), pDeletedCluster));
    ...
    const DeltaRayMatchingContainers::ClusterToPfoMap::const_iterator clusterToPfoIter(
      clusterToPfoMap.find(pDeletedCluster));

    if (clusterToPfoIter != clusterToPfoMap.end()) clusterToPfoMap.erase(clusterToPfoIter);
  }
