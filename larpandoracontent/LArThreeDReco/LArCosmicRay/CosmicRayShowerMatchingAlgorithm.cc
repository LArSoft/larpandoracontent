...
namespace lar_content {
  ...
  void CosmicRayShowerMatchingAlgorithm::SelectCleanClusters(const ClusterVector& inputVector,
  ...
  bool CosmicRayShowerMatchingAlgorithm::MatchClusters(const Cluster* const pCluster1,
    ...
    const Cluster* const pCluster3) const
  {
    ...
    float p1(std::numeric_limits<float>::max()), p2(std::numeric_limits<float>::max()),
      p3(std::numeric_limits<float>::max());
    ...
    const float q3(
      LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitType1, hitType2, p1, p2));
    const float q1(
      LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitType2, hitType3, p2, p3));
    const float q2(
      LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitType3, hitType1, p3, p1));

    const float pseudoChi2(((q1 - p1) * (q1 - p1) + (q2 - p2) * (q2 - p2) + (q3 - p3) * (q3 - p3)) /
                           3.f);
  ...
  void CosmicRayShowerMatchingAlgorithm::SetPfoParameters(
