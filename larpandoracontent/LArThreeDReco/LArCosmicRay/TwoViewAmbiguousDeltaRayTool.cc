...
namespace lar_content {
  ...
  bool TwoViewAmbiguousDeltaRayTool::Run(TwoViewDeltaRayMatchingAlgorithm* const pAlgorithm,
  ...
  bool TwoViewAmbiguousDeltaRayTool::PickOutGoodMatches(
    const MatrixType::ElementList& elementList) const
  {
    ...
    for (auto iter = elementList.begin(); iter != elementList.end(); ++iter) {
      ...
      const Cluster *const pCluster1(element.GetCluster1()), *const pCluster2(
                                                               element.GetCluster2());
