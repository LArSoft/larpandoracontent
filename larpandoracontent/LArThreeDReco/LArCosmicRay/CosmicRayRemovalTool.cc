...
namespace lar_content {
  ...
  bool CosmicRayRemovalTool::Run(ThreeViewDeltaRayMatchingAlgorithm* const pAlgorithm,
  ...
  bool CosmicRayRemovalTool::RemoveCosmicRayHits(const TensorType::ElementList& elementList) const
  {
    ...
    for (const TensorType::Element& element : elementList) {
      for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W}) {
        ...
        if ((modifiedClusters.count(element.GetClusterU())) ||
            (modifiedClusters.count(element.GetClusterV())) ||
        ...
        if (!this->IsBestElement(element, hitType, elementList, modifiedClusters)) continue;
                                               ...
                                               const HitType hitType) const
  {
    ...
    if (this->IsMuonEndpoint(element, true, hitType)) return false;
  ...
  bool CosmicRayRemovalTool::IsContaminated(const TensorType::Element& element,
                                            const HitType hitType) const
  {
  ...
  void CosmicRayRemovalTool::CollectHitsFromDeltaRay(const CartesianVector& positionOnMuon,
  ...
  void CosmicRayRemovalTool::SplitDeltaRayCluster(const TensorType::Element& element,
                                                  const HitType hitType,
  ...
  void CosmicRayRemovalTool::ReclusterRemnant(const HitType hitType,
