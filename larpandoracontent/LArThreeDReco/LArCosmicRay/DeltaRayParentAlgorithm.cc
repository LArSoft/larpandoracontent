...
namespace lar_content {
  ...
  StatusCode DeltaRayParentAlgorithm::Run()
  {
    ...
    if (!pMuonPfoList || pMuonPfoList->empty()) return STATUS_CODE_SUCCESS;
    ...
    if (!pDeltaRayPfoList || pDeltaRayPfoList->empty()) return STATUS_CODE_SUCCESS;
  ...
  void DeltaRayParentAlgorithm::InitialisePfoLengthMap(const PfoList* const muonPfoList,
                                                       const PfoList* const deltaRayPfoList,
  ...
  void DeltaRayParentAlgorithm::FindParentPfo(const PfoLengthMap& pfoLengthMap,
                                              const ParticleFlowObject* const pPfo,
                                              const ParticleFlowObject*& pParentPfo) const
  {
    const PfoLengthMap::const_iterator currentIter(pfoLengthMap.find(pPfo));

    if (currentIter == pfoLengthMap.end()) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    for (const ParticleFlowObject* const pTestParent : allPfoVector) {
      if (pTestParent == pPfo) continue;

      const PfoLengthMap::const_iterator testIter(pfoLengthMap.find(pTestParent));

      if (testIter == pfoLengthMap.end()) throw StatusCodeException(STATUS_CODE_FAILURE);

      if (testIter->second < lengthSquared) continue;

      float distance(std::numeric_limits<float>::max());

      if (this->GetTwoDSeparation(pPfo, pTestParent, distance) == STATUS_CODE_NOT_FOUND) continue;
  ...
  StatusCode DeltaRayParentAlgorithm::GetTwoDSeparation(const ParticleFlowObject* const pPfo1,
                                                        ...
                                                        float& separation) const
  {
    ...
    if (numViews < std::numeric_limits<float>::epsilon()) return STATUS_CODE_NOT_FOUND;
  ...
  void DeltaRayParentAlgorithm::AssignToParentPfo(const PfoList* const pMuonPfoList,
                                                  const PfoList* const pDeltaRayPfoList,
                                                  const ParticleFlowObject* const pPfo,
                                                  ...
                                                  PfoLengthMap& pfoLengthMap) const
  {
    if (std::find(pMuonPfoList->begin(), pMuonPfoList->end(), pParentPfo) != pMuonPfoList->end())
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::SetPfoParentDaughterRelationship(*this, pParentPfo, pPfo));

    if (std::find(pDeltaRayPfoList->begin(), pDeltaRayPfoList->end(), pParentPfo) !=
        pDeltaRayPfoList->end()) {
      ClusterList pfoClusters;
      LArPfoHelper::GetTwoDClusterList(pPfo, pfoClusters);

      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::Delete(*this, pPfo, m_deltaRayPfoListName));

      for (const Cluster* const pCluster : pfoClusters)
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToPfo(*this, pParentPfo, pCluster));
  ...
  void DeltaRayParentAlgorithm::UpdatePfoLengthMap(const PfoList& pfosToRemove,
                                                   const ParticleFlowObject* const pPfoToAdd,
                                                   PfoLengthMap& pfoLengthMap) const
  {
    for (const ParticleFlowObject* const pPfoToRemove : pfosToRemove) {
      ...
      if (iter == pfoLengthMap.end()) throw StatusCodeException(STATUS_CODE_FAILURE);
