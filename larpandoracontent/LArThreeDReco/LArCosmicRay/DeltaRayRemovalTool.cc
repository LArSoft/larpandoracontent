...
namespace lar_content {
  ...
  bool DeltaRayRemovalTool::Run(ThreeViewDeltaRayMatchingAlgorithm* const pAlgorithm,
  ...
  bool DeltaRayRemovalTool::RemoveDeltaRayHits(const TensorType::ElementList& elementList) const
  {
    ...
    for (const TensorType::Element& element : elementList) {
      for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W}) {
        ...
        const ParticleFlowObject* const pMuonPfo(
        ...
        if (m_pParentAlgorithm->CollectHitsFromMuon(nullptr,
                                                    nullptr,
                                                    pDeltaRayCluster,
                                                    pMuonPfo,
                                                    m_minDistanceFromMuon,
                                                    m_maxDistanceToCollected,
                                                    deltaRayHits) != STATUS_CODE_SUCCESS)
          continue;
  ...
  bool DeltaRayRemovalTool::PassElementChecks(const TensorType::Element& element,
                                              const HitType hitType) const
  {
    // ATTN: Avoid endpoints, topological michel reconstruction is very ambiguous
    if (this->IsMuonEndpoint(element, false)) return false;
  ...
  bool DeltaRayRemovalTool::IsContaminated(const TensorType::Element& element,
                                           const HitType hitType) const
  {
    ...
    const bool isTransverse(
      (muonDirection.GetOpeningAngle(xAxis) < m_minDeviationFromTransverse) ||
      ((M_PI - muonDirection.GetOpeningAngle(xAxis)) < m_minDeviationFromTransverse));

    if (isTransverse) return false;
    ...
    if ((minusMuonHits.size() < m_significantHitThreshold) &&
        (plusMuonHits.size() < m_significantHitThreshold))
      return true;

    if ((minusMuonHits.size() < m_significantHitThreshold) &&
        (minusDeltaRayHits.size() >= m_significantHitThreshold) &&
        (plusDeltaRayHits.size() < m_significantHitThreshold) &&
        (plusMuonHits.size() >= m_significantHitThreshold)) {
      return true;
    }

    if ((plusMuonHits.size() < m_significantHitThreshold) &&
        (plusDeltaRayHits.size() >= m_significantHitThreshold) &&
        (minusDeltaRayHits.size() < m_significantHitThreshold) &&
        (minusMuonHits.size() >= m_significantHitThreshold)) {
      return true;
    }
  ...
  void DeltaRayRemovalTool::SplitMuonCluster(const TensorType::Element& element,
                                             ...
                                             const CaloHitList& deltaRayHits) const
  {
    ...
    m_pParentAlgorithm->SplitMuonCluster(m_pParentAlgorithm->GetClusterListName(hitType),
                                         pMuonCluster,
                                         deltaRayHits,
                                         pDeltaRayCluster);
  ...
  StatusCode DeltaRayRemovalTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ContaminationWindow", m_contaminationWindow));
