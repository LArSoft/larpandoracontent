...
namespace lar_content {
  ...
  bool AmbiguousDeltaRayTool::Run(ThreeViewDeltaRayMatchingAlgorithm* const pAlgorithm,
  ...
  void AmbiguousDeltaRayTool::ExamineConnectedElements(TensorType& overlapTensor) const
  {
    ...
    if (!protoParticleVector.empty()) m_pParentAlgorithm->CreatePfos(protoParticleVector);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void AmbiguousDeltaRayTool::PickOutGoodMatches(const TensorType::ElementList& elementList,
                                                 ClusterSet& usedClusters,
                                                 ProtoParticleVector& protoParticleVector) const
  {
    ...
    do {
      ...
      for (const TensorType::Element& element : elementList) {
        const Cluster *const pClusterU(element.GetClusterU()),
          *const pClusterV(element.GetClusterV()), *const pClusterW(element.GetClusterW());

        if (usedClusters.count(pClusterU) || usedClusters.count(pClusterV) ||
            usedClusters.count(pClusterW))
          continue;

        const float chiSquared(element.GetOverlapResult().GetReducedChi2());

        if (chiSquared > m_maxGoodMatchReducedChiSquared) continue;

        const unsigned int hitSum(pClusterU->GetNCaloHits() + pClusterV->GetNCaloHits() +
                                  pClusterW->GetNCaloHits());

        if ((hitSum > highestHitCount) ||
            ((hitSum == highestHitCount) && (chiSquared < bestChiSquared))) {
