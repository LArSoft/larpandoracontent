...
namespace lar_content {
  ...
  template <typename T>
  void NViewDeltaRayMatchingAlgorithm<T>::SelectInputClusters(
    const ClusterList* const pInputClusterList,
    ClusterList& selectedClusterList) const
  {
  ...
  {
    if (preparedClusterList.empty()) return;
    ...
    PANDORA_THROW_RESULT_IF_AND_IF(
      ...
      !=,
      PandoraContentApi::GetList(*this, m_muonPfoListName, pMuonPfoList));

    if ((!pMuonPfoList) || pMuonPfoList->empty()) return;
  ...
  void NViewDeltaRayMatchingAlgorithm<T>::FillStrayClusterList(const HitType hitType)
  {
    ...
    ClusterList& strayClusterList((hitType == TPC_VIEW_U) ?
                                    m_strayClusterListU :
  ...
  template <typename T>
  StatusCode NViewDeltaRayMatchingAlgorithm<T>::GetMuonCluster(const PfoList& commonMuonPfoList,
                                                               const HitType hitType,
                                                               const Cluster*& pMuonCluster) const
  {
    if (commonMuonPfoList.size() != 1) return STATUS_CODE_NOT_FOUND;

    ClusterList muonClusterList;
    LArPfoHelper::GetClusters(commonMuonPfoList.front(), hitType, muonClusterList);

    if (muonClusterList.size() != 1) return STATUS_CODE_NOT_FOUND;
  ...
  template <typename T>
  void NViewDeltaRayMatchingAlgorithm<T>::GetNearbyMuonPfos(const Cluster* const pCluster,
                                                            ClusterList& consideredClusters,
                                                            PfoList& nearbyMuonPfos) const
  {
    const HitType hitType(LArClusterHelper::GetClusterHitType(pCluster));
    const DeltaRayMatchingContainers::ClusterToPfoMap& clusterToPfoMap(
      m_deltaRayMatchingContainers.GetClusterToPfoMap(hitType));
    const DeltaRayMatchingContainers::ClusterProximityMap& clusterProximityMap(
      m_deltaRayMatchingContainers.GetClusterProximityMap(hitType));

    consideredClusters.push_back(pCluster);

    const DeltaRayMatchingContainers::ClusterProximityMap::const_iterator clusterProximityIter(
      clusterProximityMap.find(pCluster));

    if (clusterProximityIter == clusterProximityMap.end()) return;

    for (const Cluster* const pNearbyCluster : clusterProximityIter->second) {
      if (std::find(consideredClusters.begin(), consideredClusters.end(), pNearbyCluster) !=
          consideredClusters.end())
        continue;

      const DeltaRayMatchingContainers::ClusterToPfoMap::const_iterator pfoIter(
        clusterToPfoMap.find(pNearbyCluster));

      if (pfoIter != clusterToPfoMap.end()) {
        if (std::find(nearbyMuonPfos.begin(), nearbyMuonPfos.end(), pfoIter->second) ==
            nearbyMuonPfos.end())
  ...
  StatusCode NViewDeltaRayMatchingAlgorithm<T>::PerformThreeViewMatching(
    const Cluster* const pCluster1,
    const Cluster* const pCluster2,
    const Cluster* const pCluster3,
    float& reducedChiSquared) const
  {
    ...
    if (this->PerformThreeViewMatching(pCluster1,
                                       pCluster2,
                                       pCluster3,
                                       chiSquaredSum,
                                       nSamplingPoints,
    ...
    if (nSamplingPoints == 0) return STATUS_CODE_NOT_FOUND;
    ...
    XOverlap& xOverlapObject) const
  {
    ...
    if (xCentreOverlap < std::numeric_limits<float>::epsilon()) return STATUS_CODE_NOT_FOUND;
    ...
    for (unsigned int n = 0; n < nPoints; ++n) {
      const float x(xMin +
                    (xMax - xMin) * (static_cast<float>(n) + 0.5f) / static_cast<float>(nPoints));
      ...
      try {
        ...
        const float zprojU(
          LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitTypeV, hitTypeW, zV, zW));
        const float zprojV(
          LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitTypeW, hitTypeU, zW, zU));
        const float zprojW(
          LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitTypeU, hitTypeV, zU, zV));

        ++nSamplingPoints;

        const float deltaSquared(((zU - zprojU) * (zU - zprojU) + (zV - zprojV) * (zV - zprojV) +
                                  (zW - zprojW) * (zW - zprojW)) /
                                 3.f);
        ...
        if (pseudoChi2 < m_pseudoChi2Cut) ++nMatchedSamplingPoints;
      }
    ...
    const float matchedFraction(static_cast<float>(nMatchedSamplingPoints) /
  ...
  template <typename T>
  StatusCode NViewDeltaRayMatchingAlgorithm<T>::PerformThreeViewMatching(
    const CaloHitList& clusterU,
    const CaloHitList& clusterV,
    const CaloHitList& clusterW,
    float& chiSquaredSum,
    unsigned int& nSamplingPoints,
    unsigned int& nMatchedSamplingPoints,
    XOverlap& xOverlapObject) const
  ...
  template <typename T>
  void NViewDeltaRayMatchingAlgorithm<T>::GetClusterSpanX(const CaloHitList& caloHitList,
                                                          float& xMin,
                                                          float& xMax) const
  {
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      ...
      if (xCoordinate < xMin) xMin = xCoordinate;

      if (xCoordinate > xMax) xMax = xCoordinate;
    }
  }
  ...
  template <typename T>
  StatusCode NViewDeltaRayMatchingAlgorithm<T>::GetClusterSpanZ(const CaloHitList& caloHitList,
                                                                const float xMin,
                                                                const float xMax,
                                                                float& zMin,
                                                                float& zMax) const
  {
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      ...
      if ((xCoordinate < xMin) || (xCoordinate > xMax)) continue;

      found = true;

      if (zCoordinate < zMin) zMin = zCoordinate;

      if (zCoordinate > zMax) zMax = zCoordinate;
    }

    if (!found) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  ...
  StatusCode NViewDeltaRayMatchingAlgorithm<T>::ProjectMuonPositions(
    const HitType& thirdViewHitType,
    const ParticleFlowObject* const pParentMuon,
    CartesianPointVector& projectedPositions) const
  {
    ...
    return (this->GetProjectedPositions(
      muonClusterList1.front(), muonClusterList2.front(), projectedPositions));
  }
    ...
    CartesianPointVector& projectedPositions) const
  {
    ...
    if (xOverlap < std::numeric_limits<float>::epsilon()) return STATUS_CODE_NOT_FOUND;

    const HitType hitType1(LArClusterHelper::GetClusterHitType(pCluster1));
    const HitType hitType2(LArClusterHelper::GetClusterHitType(pCluster2));

    if (hitType1 == hitType2) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    for (unsigned int n = 0; n < nPoints; ++n) {
      ...
      try {
        ...
        LArGeometryHelper::MergeTwoPositions(this->GetPandora(),
                                             hitType1,
                                             hitType2,
                                             CartesianVector(x, 0.f, z1),
                                             CartesianVector(x, 0.f, z2),
                                             projection,
                                             chi2);
    ...
    if (projectedPositions.size() < m_minProjectedPositions) return STATUS_CODE_NOT_FOUND;
  ...
  template <typename T>
  StatusCode NViewDeltaRayMatchingAlgorithm<T>::CollectHitsFromMuon(
    const Cluster* const pCluster1,
    const Cluster* const pCluster2,
    const Cluster* const pThirdViewCluster,
    const ParticleFlowObject* const pParentMuon,
    ...
    CaloHitList& collectedHits) const
  {
    ...
    if (!pThirdViewCluster) {
      if (this->GetProjectedPositions(pCluster1, pCluster2, deltaRayProjectedPositions) !=
          STATUS_CODE_SUCCESS)
        return STATUS_CODE_NOT_FOUND;

      for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W}) {
        if ((LArClusterHelper::GetClusterHitType(pCluster1) != hitType) &&
            (LArClusterHelper::GetClusterHitType(pCluster2) != hitType)) {
    ...
    if (this->ParameteriseMuon(pParentMuon,
                               deltaRayProjectedPositions,
                               thirdViewHitType,
                               positionOnMuon,
                               muonDirection) != STATUS_CODE_SUCCESS)
      return STATUS_CODE_NOT_FOUND;

    this->CollectHitsFromMuon(positionOnMuon,
                              muonDirection,
                              pMuonCluster,
                              deltaRayProjectedPositions,
                              minDistanceFromMuon,
    ...
    if (collectedHits.empty()) return STATUS_CODE_NOT_FOUND;

    // Catch if delta ray has travelled along muon
    if ((static_cast<float>(collectedHits.size()) / pMuonCluster->GetNCaloHits()) >
  ...
  template <typename T>
  void NViewDeltaRayMatchingAlgorithm<T>::CollectHitsFromMuon(
    const CartesianVector& positionOnMuon,
    const CartesianVector& muonDirection,
    const Cluster* const pMuonCluster,
    ...
    CaloHitList& collectedHits) const
  {
    ...
    while (hitsAdded) {
      ...
      for (const CaloHit* const pCaloHit : cosmicRayHitList) {
        ...
        const float distanceToCollectedHits(std::min(
          LArMuonLeadingHelper::GetClosestDistance(pCaloHit, deltaRayProjectedPositions),
          collectedHits.empty() ?
            std::numeric_limits<float>::max() :
            LArClusterHelper::GetClosestDistance(pCaloHit->GetPositionVector(), collectedHits)));
        const float distanceToMuonHits(
          muonDirection.GetCrossProduct(pCaloHit->GetPositionVector() - positionOnMuon)
            .GetMagnitude());

        if ((std::fabs(distanceToMuonHits - distanceToCollectedHits) <
             std::numeric_limits<float>::epsilon()) ||
            (distanceToMuonHits < minDistanceFromMuon) ||
            (distanceToCollectedHits > distanceToMuonHits) ||
  ...
  template <typename T>
  StatusCode NViewDeltaRayMatchingAlgorithm<T>::ParameteriseMuon(
    const ParticleFlowObject* const pParentMuon,
    const Cluster* const pDeltaRayCluster,
    CartesianVector& positionOnMuon,
    CartesianVector& muonDirection) const
  {
    ...
    return this->ParameteriseMuon(pParentMuon,
                                  deltaRayProjectedPositions,
                                  LArClusterHelper::GetClusterHitType(pDeltaRayCluster),
                                  positionOnMuon,
                                  muonDirection);
  }
    ...
    CartesianVector& muonDirection) const
  {
    ...
    if (this->ProjectMuonPositions(hitType, pParentMuon, muonProjectedPositions) !=
        STATUS_CODE_SUCCESS)
      return STATUS_CODE_NOT_FOUND;
    ...
    LArMuonLeadingHelper::GetClosestPositions(
      deltaRayProjectedPositions, pMuonCluster, deltaRayVertex, muonVertex);

    const StatusCode status(LArMuonLeadingHelper::GetClosestPosition(muonVertex,
                                                                     muonProjectedPositions,
                                                                     pMuonCluster,
                                                                     m_maxDistanceToCluster,
                                                                     m_maxDistanceToReferencePoint,
                                                                     positionOnMuon));

    if (status != STATUS_CODE_SUCCESS) return STATUS_CODE_NOT_FOUND;
  ...
  template <typename T>
  void NViewDeltaRayMatchingAlgorithm<T>::SplitMuonCluster(const std::string& clusterListName,
                                                           const Cluster* const pMuonCluster,
                                                           const CaloHitList& collectedHits,
                                                           const Cluster*& pDeltaRayCluster) const
  {
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                            !=,
                            PandoraContentApi::ReplaceCurrentList<Cluster>(*this, clusterListName));
    ...
    for (const CaloHit* const pCaloHit : muonCaloHitList) {
      if (std::find(collectedHits.begin(), collectedHits.end(), pCaloHit) != collectedHits.end()) {
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::RemoveFromCluster(*this, pMuonCluster, pCaloHit));
        ...
        if (!pDeltaRayCluster) {
          ...
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::GetCurrentListName<Cluster>(*this, currentListName));
          PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                                  !=,
                                  PandoraContentApi::CreateTemporaryListAndSetCurrent<ClusterList>(
          ...
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::Cluster::Create(*this, parameters, pDeltaRayCluster));
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::SaveList<Cluster>(*this, temporaryListName, currentListName));
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::ReplaceCurrentList<Cluster>(*this, currentListName));
        }
        else {
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::AddToCluster(*this, pDeltaRayCluster, pCaloHit));
        }
      }
    }
  }
  ...
  {
    std::sort(protoParticleVector.begin(),
              protoParticleVector.end(),
    ...
    for (ProtoParticle protoParticle : protoParticleVector) {
      ...
      for (const Cluster* const pCluster : protoParticle.m_clusterList) {
        ...
        if (!collectedClusters.empty()) this->AddInStrayClusters(pCluster, collectedClusters);
      }
    }
    ...
    ClusterList& collectedClusters)
  {
    ...
    const DeltaRayMatchingContainers::ClusterProximityMap& clusterProximityMap(
      m_deltaRayMatchingContainers.GetClusterProximityMap(hitType));
    const DeltaRayMatchingContainers::ClusterProximityMap::const_iterator clusterProximityIter(
      clusterProximityMap.find(pClusterToEnlarge));

    if (clusterProximityIter == clusterProximityMap.end()) return;

    for (const Cluster* const pNearbyCluster : clusterProximityIter->second) {
      if (std::find(strayClusterList.begin(), strayClusterList.end(), pNearbyCluster) ==
          strayClusterList.end())
        continue;

      float xMin(-std::numeric_limits<float>::max()), xMax(+std::numeric_limits<float>::max());
      pNearbyCluster->GetClusterSpanX(xMin, xMax);

      if (!(((xMin > rangeMinX) && (xMin < rangeMaxX)) ||
            ((xMax > rangeMinX) && (xMax < rangeMaxX))))
        continue;

      if (LArClusterHelper::GetClosestDistance(pClusterToEnlarge, pNearbyCluster) >
          m_strayClusterSeparation)
        continue;

      if (std::find(collectedClusters.begin(), collectedClusters.end(), pNearbyCluster) ==
          collectedClusters.end())
  ...
  template <typename T>
  void NViewDeltaRayMatchingAlgorithm<T>::AddInStrayClusters(const Cluster* const pClusterToEnlarge,
                                                             const ClusterList& collectedClusters)
  {
    ...
    for (const Cluster* const pCollectedCluster : collectedClusters) {
      ...
      std::string clusterListName(
        this->GetClusterListName(LArClusterHelper::GetClusterHitType(pClusterToEnlarge)));
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
  ...
  void NViewDeltaRayMatchingAlgorithm<T>::UpdateUponDeletion(const Cluster* const pDeletedCluster)
  {
    ...
    ClusterList& strayClusterList((hitType == TPC_VIEW_U) ?
                                    ...
                                    (hitType == TPC_VIEW_V) ? m_strayClusterListV :
                                                              m_strayClusterListW);
    const ClusterList::const_iterator strayClusterIter(
      std::find(strayClusterList.begin(), strayClusterList.end(), pDeletedCluster));

    if (strayClusterIter != strayClusterList.end()) strayClusterList.erase(strayClusterIter);
    ...
    if (!isMuonCluster) NViewMatchingAlgorithm<T>::UpdateUponDeletion(pDeletedCluster);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  template <typename T>
  void NViewDeltaRayMatchingAlgorithm<T>::UpdateForNewClusters(
    const ClusterVector& newClusterVector,
    const PfoVector& pfoVector)
  {
    ...
    for (unsigned int i = 0; i < newClusterVector.size(); i++) {
      ...
      if (!pMuonPfo) NViewMatchingAlgorithm<T>::UpdateForNewCluster(pNewCluster);
    }
  }
  ...
  StatusCode NViewDeltaRayMatchingAlgorithm<T>::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                             !=,
                             XmlHelper::ReadValue(xmlHandle, "MuonPfoListName", m_muonPfoListName));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(
        xmlHandle, "SearchRegion1D", m_deltaRayMatchingContainers.m_searchRegion1D));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "PseudoChi2Cut", m_pseudoChi2Cut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "OverlapWindow", m_xOverlapWindow));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinMatchedPoints", m_minMatchedPoints));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinProjectedPositions", m_minProjectedPositions));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxDistanceToCluster", m_maxDistanceToCluster));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MaxDistanceToReferencePoint",
                                                         m_maxDistanceToReferencePoint));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "StrayClusterSeparation", m_strayClusterSeparation));
  ...
  template class NViewDeltaRayMatchingAlgorithm<
    TwoViewMatchingControl<TwoViewDeltaRayOverlapResult>>;
