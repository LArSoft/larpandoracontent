...
namespace lar_content {
  ...
  StatusCode CosmicRayBaseMatchingAlgorithm::GetAvailableClusters(
    ...
    ClusterVector& clusterVector) const
  {
    ...
    if (!pClusterList || pClusterList->empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "CosmicRayBaseMatchingAlgorithm: unable to find cluster list "
  ...
  void CosmicRayBaseMatchingAlgorithm::MatchClusters(const ClusterVector& clusterVector1,
                                                     ...
                                                     ClusterAssociationMap& matchedClusters12) const
  {
    ...
    if (clusterVector1.empty() || clusterVector2.empty()) return;
      ...
      for (const Cluster* const pCluster2 : clusterVector2) {
        if (this->MatchClusters(pCluster1, pCluster2)) {
          ...
          LArGeometryHelper::GetCommonDaughterVolumes(
            pCluster1, pCluster2, daughterVolumeIntersection);
  ...
  void CosmicRayBaseMatchingAlgorithm::MatchThreeViews(
    ...
    ParticleList& matchedParticles) const
  {
    if (matchedClusters12.empty() || matchedClusters23.empty() || matchedClusters31.empty()) return;
    ...
    for (const Cluster* const pCluster1 : clusterList1) {
      ...
      for (const Cluster* const pCluster2 : clusterList2) {
        ...
        if (matchedClusters23.end() == iter23) continue;
        ...
        for (const Cluster* const pCluster3 : clusterList3) {
          ...
          if (matchedClusters31.end() == iter31) continue;

          if (iter31->second.end() ==
              std::find(iter31->second.begin(), iter31->second.end(), pCluster1))
            continue;
          ...
          if (!this->CheckMatchedClusters3D(pCluster1, pCluster2, pCluster3)) continue;
  ...
  void CosmicRayBaseMatchingAlgorithm::MatchTwoViews(const ClusterAssociationMap& matchedClusters12,
                                                     ParticleList& matchedParticles) const
  {
    if (matchedClusters12.empty()) return;
  ...
  void CosmicRayBaseMatchingAlgorithm::ResolveAmbiguities(const ParticleList& candidateParticles,
                                                          ParticleList& matchedParticles) const
  {
    for (const Particle& particle1 : candidateParticles) {
      ...
      if (isGoodMatch) matchedParticles.push_back(particle1);
    }
  }
  ...
  CosmicRayBaseMatchingAlgorithm::Particle::Particle(const Cluster* const pClusterU,
                                                     const Cluster* const pClusterV,
                                                     const Cluster* const pClusterW)
