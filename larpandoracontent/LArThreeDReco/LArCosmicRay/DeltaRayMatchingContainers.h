 ...
 */
  ...
  public:
     ...
     */
    void FillContainers(const pandora::PfoList& inputPfoList,
     ...
     */
    void AddClustersToContainers(const pandora::ClusterVector& newClusterVector,
  ...
  private:
    ...
    HitToClusterMap
      m_hitToClusterMapU; ///< The mapping of hits to the clusters to which they belong (in the U view)
    HitToClusterMap
      m_hitToClusterMapV; ///< The mapping of hits to the clusters to which they belong (in the V view)
    HitToClusterMap
      m_hitToClusterMapW; ///< The mapping of hits to the clusters to which they belong (in the W view)
    ...
    ClusterProximityMap
      m_clusterProximityMapU; ///< The mapping of clusters to their neighbouring clusters (in the U view)
    ClusterProximityMap
      m_clusterProximityMapV; ///< The mapping of clusters to their neighbouring clusters (in the V view)
    ClusterProximityMap
      m_clusterProximityMapW; ///< The mapping of clusters to their neighbouring clusters (in the W view)
    ClusterToPfoMap
      m_clusterToPfoMapU; ///< The mapping of cosmic ray U clusters to the cosmic ray pfos to which they belong
    ClusterToPfoMap
      m_clusterToPfoMapV; ///< The mapping of cosmic ray V clusters to the cosmic ray pfos to which they belong
    ClusterToPfoMap
      m_clusterToPfoMapW; ///< The mapping of cosmic ray W clusters to the cosmic ray pfos to which they belong
  };

  //------------------------------------------------------------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------------------------------------------------------------

  inline const DeltaRayMatchingContainers::ClusterProximityMap&
  DeltaRayMatchingContainers::GetClusterProximityMap(const pandora::HitType hitType) const
  {
    return ((hitType == pandora::TPC_VIEW_U) ?
              m_clusterProximityMapU :
              (hitType == pandora::TPC_VIEW_V) ? m_clusterProximityMapV : m_clusterProximityMapW);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  inline const DeltaRayMatchingContainers::ClusterToPfoMap&
  DeltaRayMatchingContainers::GetClusterToPfoMap(const pandora::HitType hitType) const
  {
    return ((hitType == pandora::TPC_VIEW_U) ?
              m_clusterToPfoMapU :
              (hitType == pandora::TPC_VIEW_V) ? m_clusterToPfoMapV : m_clusterToPfoMapW);
  }
