...
namespace lar_content {
  ...
  StatusCode DeltaRayMatchingAlgorithm::Run()
  {
    ...
    if (pfoVector.empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "DeltaRayMatchingAlgorithm: pfo list " << m_parentPfoListName
  ...
  void DeltaRayMatchingAlgorithm::InitializeNearbyClusterMap(const std::string& clusterListName,
                                                             ClusterToClustersMap& nearbyClusters)
  {
    ...
    if ((NULL == pClusterList) || pClusterList->empty()) return;
    ...
    for (const Cluster* const pCluster : *pClusterList) {
      ...
      for (const CaloHit* const pCaloHit : daughterHits) {
        KDTreeBox searchRegionHits =
          build_2d_kd_search_region(pCaloHit, m_searchRegion1D, m_searchRegion1D);
        ...
        for (const auto& hit : found) {
          ...
          if (nearbyClusterList.end() ==
              std::find(nearbyClusterList.begin(), nearbyClusterList.end(), pNearbyCluster))
  ...
  void DeltaRayMatchingAlgorithm::GetAllPfos(const std::string& inputPfoListName,
                                             PfoVector& pfoVector) const
  {
    const PfoList* pPfoList = NULL;
    PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                   STATUS_CODE_NOT_INITIALIZED,
                                   !=,
                                   PandoraContentApi::GetList(*this, inputPfoListName, pPfoList));

    if (NULL == pPfoList) return;
  ...
  void DeltaRayMatchingAlgorithm::GetTrackPfos(const std::string& inputPfoListName,
                                               PfoVector& pfoVector) const
  {
    PfoVector inputVector;
    this->GetAllPfos(inputPfoListName, inputVector);

    for (PfoVector::const_iterator iter = inputVector.begin(), iterEnd = inputVector.end();
         iter != iterEnd;
         ++iter) {
      const ParticleFlowObject* const pPfo = *iter;

      if (!LArPfoHelper::IsTrack(pPfo)) continue;
  ...
  void DeltaRayMatchingAlgorithm::GetClusters(const std::string& inputClusterListName,
                                              pandora::ClusterVector& clusterVector) const
  {
    ...
    if (NULL == pClusterList) return;

    for (ClusterList::const_iterator cIter = pClusterList->begin(), cIterEnd = pClusterList->end();
         cIter != cIterEnd;
         ++cIter) {
  ...
  void DeltaRayMatchingAlgorithm::ThreeViewMatching(ClusterLengthMap& clusterLengthMap) const
  {
    ...
    this->ThreeViewMatching(
      clustersU, clustersV, clustersW, clusterLengthMap, pfoLengthMap, initialParticleList);
  ...
  void DeltaRayMatchingAlgorithm::TwoViewMatching(ClusterLengthMap& clusterLengthMap) const
  {
    ...
    this->TwoViewMatching(
      clustersU, clustersV, clusterLengthMap, pfoLengthMap, initialParticleList);
    this->TwoViewMatching(
      clustersV, clustersW, clusterLengthMap, pfoLengthMap, initialParticleList);
    this->TwoViewMatching(
      clustersW, clustersU, clusterLengthMap, pfoLengthMap, initialParticleList);
    this->SelectParticles(initialParticleList, clusterLengthMap, finalParticleList);
    this->CreateParticles(finalParticleList);
  }
  ...
  void DeltaRayMatchingAlgorithm::ThreeViewMatching(const ClusterVector& clusters1,
                                                    const ClusterVector& clusters2,
                                                    const ClusterVector& clusters3,
                                                    ClusterLengthMap& clusterLengthMap,
                                                    PfoLengthMap& pfoLengthMap,
                                                    ParticleList& particleList) const
  {
    if (clusters1.empty() || clusters2.empty() || clusters3.empty()) return;

    for (const Cluster* const pCluster1 : clusters1) {
      if (!pCluster1->IsAvailable()) continue;

      for (const Cluster* const pCluster2 : clusters2) {
        if (!pCluster2->IsAvailable()) continue;

        for (const Cluster* const pCluster3 : clusters3) {
          if (!pCluster3->IsAvailable()) continue;

          if (!this->AreClustersMatched(pCluster1, pCluster2, pCluster3)) continue;

          const ParticleFlowObject* pBestPfo = NULL;
          this->FindBestParentPfo(
            pCluster1, pCluster2, pCluster3, clusterLengthMap, pfoLengthMap, pBestPfo);
  ...
  void DeltaRayMatchingAlgorithm::TwoViewMatching(const ClusterVector& clusters1,
                                                  const ClusterVector& clusters2,
                                                  ClusterLengthMap& clusterLengthMap,
                                                  PfoLengthMap& pfoLengthMap,
                                                  ParticleList& particleList) const
  {
    if (clusters1.empty() || clusters2.empty()) return;
    ...
    for (const Cluster* const pCluster1 : clusters1) {
      ...
      for (const Cluster* const pCluster2 : clusters2) {
        ...
        if (!this->AreClustersMatched(pCluster1, pCluster2, NULL)) continue;

        const ParticleFlowObject* pBestPfo = NULL;
        this->FindBestParentPfo(
          pCluster1, pCluster2, NULL, clusterLengthMap, pfoLengthMap, pBestPfo);

        if (NULL == pBestPfo) continue;
  ...
  void DeltaRayMatchingAlgorithm::OneViewMatching(const ClusterVector& clusters,
                                                  ClusterLengthMap& clusterLengthMap,
                                                  PfoLengthMap& pfoLengthMap,
                                                  ParticleList& particleList) const
  {
    ...
    for (const Cluster* const pCluster : clusters) {
      if (!pCluster->IsAvailable()) continue;
  ...
  void DeltaRayMatchingAlgorithm::SelectParticles(const ParticleList& initialParticles,
                                                  ClusterLengthMap& clusterLengthMap,
                                                  ParticleList& finalParticles) const
  {
    for (const Particle& particle1 : initialParticles) {
      ...
      for (const Particle& particle2 : initialParticles) {
        ...
        if (commonU && commonV && commonW) continue;

        if (ambiguousU || ambiguousV || ambiguousW) {
          if (particle2.GetNViews() > particle1.GetNViews()) { isGoodParticle = false; }
          else if (particle2.GetNViews() == particle1.GetNViews() &&
                   NULL != particle2.GetParentPfo()) {
            if ((NULL == particle1.GetParentPfo()) ||
                (particle2.GetNCaloHits() > particle1.GetNCaloHits()) ||
                (particle2.GetNCaloHits() == particle1.GetNCaloHits() &&
                 this->GetLength(particle2, clusterLengthMap) >=
                   this->GetLength(particle1, clusterLengthMap))) {
              isGoodParticle = false;
            }
          }

          if (!isGoodParticle) break;
        }
      }

      if (isGoodParticle && NULL != particle1.GetParentPfo()) finalParticles.push_back(particle1);
    }
  }
  ...
  void DeltaRayMatchingAlgorithm::CreateParticles(const ParticleList& particleList) const
  {
    ...
    for (const Particle& particle : particleList) {
      ...
      else if (daughterList.end() !=
               std::find(daughterList.begin(), daughterList.end(), pParentPfo)) {
  ...
  bool DeltaRayMatchingAlgorithm::AreClustersMatched(const Cluster* const pCluster1,
                                                     ...
                                                     const Cluster* const pCluster3) const
  {
    ...
    if (nullptr != pCluster1) pCluster1->GetClusterSpanX(xMin1, xMax1);

    if (nullptr != pCluster2) pCluster2->GetClusterSpanX(xMin2, xMax2);

    if (nullptr != pCluster3) pCluster3->GetClusterSpanX(xMin3, xMax3);
    ...
    if (xOverlap < std::numeric_limits<float>::epsilon()) return false;

    if (nullptr == pCluster1 || nullptr == pCluster2 || nullptr == pCluster3) return true;
    ...
    for (unsigned int n = 0; n < nSamplingPoints; ++n) {
      const float x(xMin + (xMax - xMin) * (static_cast<float>(n) + 0.5f) /
      ...
      try {
        ...
        const float zproj1(
          LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitType2, hitType3, z2, z3));
        const float zproj2(
          LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitType3, hitType1, z3, z1));
        const float zproj3(
          LArGeometryHelper::MergeTwoPositions(this->GetPandora(), hitType1, hitType2, z1, z2));

        const float deltaSquared(((z1 - zproj1) * (z1 - zproj1) + (z2 - zproj2) * (z2 - zproj2) +
                                  (z3 - zproj3) * (z3 - zproj3)) /
                                 3.f);
        const float sigmaSquared(dz1 * dz1 + dz2 * dz2 + dz3 * dz3 + dz4 * dz4);
        const float pseudoChi2(deltaSquared / sigmaSquared);

        if (pseudoChi2 < m_pseudoChi2Cut) return true;
      }
      catch (StatusCodeException& statusCodeException) {
        if (STATUS_CODE_NOT_FOUND != statusCodeException.GetStatusCode()) throw statusCodeException;
      }
    }
  ...
  void DeltaRayMatchingAlgorithm::FindBestParentPfo(const Cluster* const pCluster1,
                                                    const Cluster* const pCluster2,
                                                    ...
                                                    const ParticleFlowObject*& pBestPfo) const
  {
    ...
    if (pfoVector.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    float bestDistanceSquared(static_cast<float>(numViews) * m_distanceForMatching *
                              m_distanceForMatching);

    for (const ParticleFlowObject* const pPfo : pfoVector) {
      if (lengthSquared > this->GetLengthFromCache(pPfo, pfoLengthMap)) continue;

      try {
        float distanceSquared(0.f);

        if (NULL != pCluster1) distanceSquared += this->GetDistanceSquaredToPfo(pCluster1, pPfo);

        if (NULL != pCluster2) distanceSquared += this->GetDistanceSquaredToPfo(pCluster2, pPfo);

        if (NULL != pCluster3) distanceSquared += this->GetDistanceSquaredToPfo(pCluster3, pPfo);
  ...
  float DeltaRayMatchingAlgorithm::GetLengthFromCache(const Cluster* const pCluster,
                                                      ClusterLengthMap& clusterLengthMap) const
  {
    ClusterLengthMap::const_iterator iter = clusterLengthMap.find(pCluster);

    if (clusterLengthMap.end() != iter) return iter->second;
  ...
  float DeltaRayMatchingAlgorithm::GetLengthFromCache(const ParticleFlowObject* const pPfo,
                                                      PfoLengthMap& pfoLengthMap) const
  {
    PfoLengthMap::const_iterator iter = pfoLengthMap.find(pPfo);

    if (pfoLengthMap.end() != iter) return iter->second;
  ...
  float DeltaRayMatchingAlgorithm::GetLength(const Particle& particle,
                                             ClusterLengthMap& clusterLengthMap) const
  {
    float lengthSquared(0.f);
  ...
  float DeltaRayMatchingAlgorithm::GetDistanceSquaredToPfo(
    ...
    const ParticleFlowObject* const pPfo) const
  {
    ...
    const ClusterToClustersMap& nearbyClusters(
      (TPC_VIEW_U == hitType) ? m_nearbyClustersU :
                                (TPC_VIEW_V == hitType) ? m_nearbyClustersV : m_nearbyClustersW);

    if (!nearbyClusters.count(pCluster)) return std::numeric_limits<float>::max();
    ...
    for (const Cluster* const pPfoCluster : pfoClusterList) {
      ...
      if ((clusterList.end() != std::find(clusterList.begin(), clusterList.end(), pPfoCluster)) &&
          (comparisonList.end() ==
           std::find(comparisonList.begin(), comparisonList.end(), pPfoCluster))) {
        comparisonList.push_back(pPfoCluster);
      }
    }

    if (comparisonList.empty()) return std::numeric_limits<float>::max();
  ...
  void DeltaRayMatchingAlgorithm::CreateDaughterPfo(
    const ClusterList& clusterList,
    const ParticleFlowObject* const pParentPfo) const
  {
    const PfoList* pPfoList = NULL;
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::ParticleFlowObject::Create(*this, pfoParameters, pDaughterPfo));

    if (!pPfoList->empty()) {
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::SaveList<Pfo>(*this, m_daughterPfoListName));
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::SetPfoParentDaughterRelationship(*this, pParentPfo, pDaughterPfo));
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void DeltaRayMatchingAlgorithm::AddToDaughterPfo(const ClusterList& clusterList,
                                                   const ParticleFlowObject* const pParentPfo) const
  {
    for (const Cluster* const pDaughterCluster : clusterList) {
      ...
      const std::string clusterListName((TPC_VIEW_U == hitType) ?
      ...
      if (pfoClusters.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
  ...
  DeltaRayMatchingAlgorithm::Particle::Particle(const Cluster* const pCluster1,
                                                const Cluster* const pCluster2,
                                                const Cluster* const pCluster3,
                                                const ParticleFlowObject* const pPfo)
    : m_pClusterU(NULL), m_pClusterV(NULL), m_pClusterW(NULL), m_pParentPfo(NULL)
  {
    const HitType hitType1(NULL != pCluster1 ? LArClusterHelper::GetClusterHitType(pCluster1) :
                                               HIT_CUSTOM);
    const HitType hitType2(NULL != pCluster2 ? LArClusterHelper::GetClusterHitType(pCluster2) :
                                               HIT_CUSTOM);
    const HitType hitType3(NULL != pCluster3 ? LArClusterHelper::GetClusterHitType(pCluster3) :
                                               HIT_CUSTOM);

    m_pClusterU =
      ((TPC_VIEW_U == hitType1) ?
         pCluster1 :
         (TPC_VIEW_U == hitType2) ? pCluster2 : (TPC_VIEW_U == hitType3) ? pCluster3 : NULL);
    m_pClusterV =
      ((TPC_VIEW_V == hitType1) ?
         pCluster1 :
         (TPC_VIEW_V == hitType2) ? pCluster2 : (TPC_VIEW_V == hitType3) ? pCluster3 : NULL);
    m_pClusterW =
      ((TPC_VIEW_W == hitType1) ?
         pCluster1 :
         (TPC_VIEW_W == hitType2) ? pCluster2 : (TPC_VIEW_W == hitType3) ? pCluster3 : NULL);
    m_pParentPfo = pPfo;
  ...
  unsigned int DeltaRayMatchingAlgorithm::Particle::GetNViews() const
  {
    ...
    if (NULL != m_pClusterU) numViews += 1;

    if (NULL != m_pClusterV) numViews += 1;

    if (NULL != m_pClusterW) numViews += 1;
  ...
  unsigned int DeltaRayMatchingAlgorithm::Particle::GetNCaloHits() const
  {
    ...
    if (NULL != m_pClusterU) numCaloHits += m_pClusterU->GetNCaloHits();

    if (NULL != m_pClusterV) numCaloHits += m_pClusterV->GetNCaloHits();

    if (NULL != m_pClusterW) numCaloHits += m_pClusterW->GetNCaloHits();
  ...
  StatusCode DeltaRayMatchingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ParentPfoListName", m_parentPfoListName));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "DaughterPfoListName", m_daughterPfoListName));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinCaloHitsPerCluster", m_minCaloHitsPerCluster));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "DistanceForMatching", m_distanceForMatching));
