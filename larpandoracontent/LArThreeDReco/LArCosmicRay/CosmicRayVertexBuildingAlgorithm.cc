...
namespace lar_content {
  ...
  StatusCode CosmicRayVertexBuildingAlgorithm::Run()
  {
    ...
    PANDORA_THROW_RESULT_IF_AND_IF(
      ...
      !=,
      PandoraContentApi::GetList(*this, m_parentPfoListName, pPfoList));
  ...
  void CosmicRayVertexBuildingAlgorithm::GetCosmicPfos(const PfoList* const pPfoList,
                                                       PfoVector& pfoVector) const
  {
    PfoList outputList;

    for (PfoList::const_iterator pIter = pPfoList->begin(), pIterEnd = pPfoList->end();
         pIter != pIterEnd;
         ++pIter) {
    ...
    for (PfoList::const_iterator pIter = outputList.begin(), pIterEnd = outputList.end();
         pIter != pIterEnd;
         ++pIter) {
  ...
  void CosmicRayVertexBuildingAlgorithm::BuildPointingClusterMap(
    const PfoVector& pfoVector,
    LArPointingClusterMap& pointingClusterMap) const
  {
    const float slidingFitPitch(LArGeometryHelper::GetWireZPitch(this->GetPandora()));

    for (PfoVector::const_iterator pIter = pfoVector.begin(), pIterEnd = pfoVector.end();
         pIter != pIterEnd;
         ++pIter) {
      ...
      for (ClusterList::const_iterator cIter = clusterList.begin(), cIterEnd = clusterList.end();
        ...
        try {
          ...
          if (!pointingClusterMap
                 .insert(LArPointingClusterMap::value_type(pCluster, pointingCluster))
                 .second)
  ...
  void CosmicRayVertexBuildingAlgorithm::BuildCosmicRayParticles(
    const LArPointingClusterMap& pointingClusterMap,
    const PfoVector& pfoVector) const
  {
    for (PfoVector::const_iterator iter = pfoVector.begin(), iterEnd = pfoVector.end();
         iter != iterEnd;
         ++iter) {
  ...
  void CosmicRayVertexBuildingAlgorithm::BuildCosmicRayParent(
    const LArPointingClusterMap& pointingClusterMap,
    const ParticleFlowObject* const pPfo) const
  {
    ...
    for (ClusterList::const_iterator cIter1 = clusterList.begin(), cIterEnd1 = clusterList.end();
         cIter1 != cIterEnd1;
         ++cIter1) {
      ...
      try {
        ...
        if ((maxPosition - minPosition).GetMagnitudeSquared() <
        ...
        if (!foundVtx || (minVerticalCoordinate >
                          std::max(maxVerticalCoordinate,
        ...
        if (!foundVtx || (maxVerticalCoordinate >
                          std::max(minVerticalCoordinate,
                                   m_isDualPhase ? vtxPosition.GetX() : vtxPosition.GetY()))) {
          foundVtx = true;
        ...
        if (!foundEnd || (minVerticalCoordinate <
                          std::min(maxVerticalCoordinate,
        ...
        if (!foundEnd || (maxVerticalCoordinate <
                          std::min(minVerticalCoordinate,
                                   m_isDualPhase ? endPosition.GetX() : endPosition.GetY()))) {
          foundEnd = true;
  ...
  void CosmicRayVertexBuildingAlgorithm::BuildCosmicRayDaughter(
    const ParticleFlowObject* const pDaughterPfo) const
  {
    ...
    if (daughterList.empty() || parentList.empty()) return;

    bool found(false);
    float closestMaxVerticalCoordinate(-std::numeric_limits<float>::max()),
      maxVerticalCoordinate(-std::numeric_limits<float>::max());
        ...
        for (const CaloHit* const pDaughterCaloHit : daughterCaloHitList) {
          ...
          for (const CaloHit* const pParentCaloHit : parentCaloHitList) {
            ...
            const float separationSquared(
              (daughterPosition - parentPosition).GetMagnitudeSquared());
            const float verticalCoordinate(m_isDualPhase ? daughterPosition.GetX() :
            ...
            if (separationSquared <
                (m_maxVertexDisplacementFromTrack * m_maxVertexDisplacementFromTrack)) {
    ...
    const CartesianVector& vertexPosition(
      m_useParentShowerVertex ? LArClusterHelper::GetClosestPosition(daughterVertex, parentList) :
                                daughterVertex);
  ...
  void CosmicRayVertexBuildingAlgorithm::SetParticleParameters(
    ...
    const ParticleFlowObject* const pPfo) const
  {
    if (!pPfo->GetVertexList().empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    if (!pVertexList->empty()) {
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::SaveList<Vertex>(*this, m_vertexListName));
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToPfo<Vertex>(*this, pPfo, pVertex));
    }
  }
  ...
  StatusCode CosmicRayVertexBuildingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputPfoListName", m_parentPfoListName));

    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "OutputVertexListName", m_vertexListName));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "IsDualPhase", m_isDualPhase));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MaxVertexDisplacementFromTrack",
