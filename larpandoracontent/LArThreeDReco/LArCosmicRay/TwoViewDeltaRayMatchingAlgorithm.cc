...
namespace lar_content {
  ...
  TwoViewDeltaRayMatchingAlgorithm::HitTypeVector
  TwoViewDeltaRayMatchingAlgorithm::GetHitTypeVector()
  {
    ...
    for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W}) {
      ...
      if ((hitTypeIndex != 1) && (hitTypeIndex != 2)) continue;
  ...
  const Cluster* TwoViewDeltaRayMatchingAlgorithm::GetCluster(const MatrixType::Element& element,
  ...
  bool TwoViewDeltaRayMatchingAlgorithm::DoesClusterPassTensorThreshold(
  ...
  void TwoViewDeltaRayMatchingAlgorithm::CalculateOverlapResult(const Cluster* const pCluster1,
                                                                ...
                                                                const Cluster* const)
  {
    ...
    PANDORA_THROW_RESULT_IF_AND_IF(
      ...
      !=,
      this->CalculateOverlapResult(pCluster1, pCluster2, overlapResult));

    if (overlapResult.IsInitialized())
      this->GetMatchingControl().GetOverlapMatrix().SetOverlapResult(
        pCluster1, pCluster2, overlapResult);
  }
    ...
    TwoViewDeltaRayOverlapResult& overlapResult) const
  {
    ...
    if (commonMuonPfoList.empty()) return STATUS_CODE_NOT_FOUND;
    ...
    if (status != STATUS_CODE_SUCCESS) return status;
    ...
    if (matchedClusterList.empty()) return STATUS_CODE_NOT_FOUND;

    float reducedChiSquared(std::numeric_limits<float>::max());
    const Cluster* const pBestMatchedCluster = this->GetBestMatchedCluster(
      pCluster1, pCluster2, commonMuonPfoList, matchedClusterList, reducedChiSquared);
    ...
    if (pBestMatchedCluster && (pBestMatchedCluster->IsAvailable())) {
      ...
      if (hitSum13 > hitSum12) return STATUS_CODE_NOT_FOUND;

      const unsigned int hitSum23(pCluster2->GetNCaloHits() + pBestMatchedCluster->GetNCaloHits());

      if (hitSum23 > hitSum12) return STATUS_CODE_NOT_FOUND;
    }

    TwoViewXOverlap xOverlapObject(xMin1, xMax1, xMin2, xMax2);
    overlapResult = TwoViewDeltaRayOverlapResult(xOverlapObject,
                                                 commonMuonPfoList,
                                                 pBestMatchedCluster,
                                                 matchedClusterList,
                                                 reducedChiSquared);
  ...
  void TwoViewDeltaRayMatchingAlgorithm::FindCommonMuonParents(const Cluster* const pCluster1,
                                                               const Cluster* const pCluster2,
                                                               PfoList& commonMuonPfoList) const
  {
    ...
    for (const ParticleFlowObject* const pNearbyMuon1 : nearbyMuonPfos1) {
      for (const ParticleFlowObject* const pNearbyMuon2 : nearbyMuonPfos2) {
        if (pNearbyMuon1 == pNearbyMuon2) commonMuonPfoList.push_back(pNearbyMuon1);
      }
    }
  }
  ...
  void TwoViewDeltaRayMatchingAlgorithm::CollectThirdViewClusters(
    const Cluster* const pCluster1,
    const Cluster* const pCluster2,
    ...
    ClusterList& matchedClusters) const
  {
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::GetList(*this, m_inputClusterListName, pInputClusterList));

    if (!pInputClusterList || pInputClusterList->empty()) return;

    for (const Cluster* const pCluster : *pInputClusterList) {
      const float separation(
        LArMuonLeadingHelper::GetClosestDistance(pCluster, projectedPositions));

      if (separation > m_maxDistanceFromPrediction) continue;

      float reducedChiSquared(0.f);
      if (this->PerformThreeViewMatching(pCluster1, pCluster2, pCluster, reducedChiSquared) ==
          STATUS_CODE_NOT_FOUND)
        continue;

      if (reducedChiSquared > m_maxGoodMatchReducedChiSquared) continue;
  ...
  const Cluster* TwoViewDeltaRayMatchingAlgorithm::GetBestMatchedCluster(
    const Cluster* const pCluster1,
    const Cluster* const pCluster2,
    ...
    float& reducedChiSquared) const
  {
    ...
    if (matchedClusters.empty()) return pBestMatchedCluster;
    ...
    for (const Cluster* const pMatchedCluster : matchedClusters) {
      if (!pMatchedCluster->IsAvailable()) {
        if (std::find(muonClusterList.begin(), muonClusterList.end(), pMatchedCluster) ==
            muonClusterList.end())
          continue;
      }
    ...
    if (!pBestMatchedCluster) return pBestMatchedCluster;

    if (this->PerformThreeViewMatching(
          pCluster1, pCluster2, pBestMatchedCluster, reducedChiSquared) == STATUS_CODE_NOT_FOUND)
  ...
  bool TwoViewDeltaRayMatchingAlgorithm::CreatePfo(const MatrixType::Element& protoParticleElement)
  {
    ...
    const Cluster* const pBestMatchedCluster(
      protoParticleElement.GetOverlapResult().GetBestMatchedCluster());

    if (pBestMatchedCluster) this->FormThirdViewCluster(protoParticleElement, protoParticle);
  ...
  void TwoViewDeltaRayMatchingAlgorithm::FormThirdViewCluster(const MatrixType::Element& element,
                                                              ProtoParticle& protoParticle)
  {
    ...
    for (const ParticleFlowObject* const pMuonPfo : commonMuonPfoList) {
      ...
      if (std::find(muonClusterList.begin(), muonClusterList.end(), pBestMatchedCluster) !=
          muonClusterList.end())
    ...
    if (pMatchedMuonPfo) {
      ...
      if (this->CollectHitsFromMuon(element.GetCluster1(),
                                    element.GetCluster2(),
                                    nullptr,
                                    pMatchedMuonPfo,
                                    m_minDistanceFromMuon,
                                    m_maxDistanceToCollected,
                                    deltaRayHitList) == STATUS_CODE_SUCCESS) {
        this->SplitMuonCluster(this->GetThirdViewClusterListName(),
                               pBestMatchedCluster,
                               deltaRayHitList,
                               pThirdViewCluster);
  ...
  void TwoViewDeltaRayMatchingAlgorithm::MergeThirdView(const MatrixType::Element& element,
                                                        const Cluster* const pSeedCluster)
  {
    ...
    if (std::find(matchedClusters.begin(), matchedClusters.end(), pSeedCluster) !=
        matchedClusters.end())
    ...
    while (checkedClusters.size() != matchedClusters.size()) {
      ...
      for (const Cluster* const pMatchedCluster : matchedClusters) {
        if (checkedClusters.count(pMatchedCluster)) continue;
      ...
      if (!pClusterToDelete) return;

      checkedClusters.insert(pClusterToDelete);

      if (!pClusterToDelete->IsAvailable()) continue;
      ...
      float reducedChiSquared(std::numeric_limits<float>::max());
      const StatusCode status(this->PerformThreeViewMatching(
        caloHitList1, caloHitList2, caloHitList3, reducedChiSquared));

      if (status == STATUS_CODE_NOT_FOUND) continue;
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ReplaceCurrentList<Cluster>(*this, this->GetThirdViewClusterListName()));

      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::MergeAndDeleteClusters(*this, pSeedCluster, pClusterToDelete));
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void TwoViewDeltaRayMatchingAlgorithm::UpdateForThirdViewClusterModification(
    const Cluster* const pModifiedCluster,
    const bool isMuon)
  {
    ...
    for (auto [pCluster1, overlapList] : theMatrix) {
      for (auto [pCluster2, overlapResult] : overlapList) {
        ...
        auto matchedClustersIter(
          std::find(matchedClusters.begin(), matchedClusters.end(), pModifiedCluster));

        if (matchedClustersIter == matchedClusters.end()) continue;

        float tempReducedChiSquared(std::numeric_limits<float>::max());

        if (isMuon)
          this->PerformThreeViewMatching(
            pCluster1, pCluster2, pModifiedCluster, tempReducedChiSquared);
        ...
        float reducedChiSquared(std::numeric_limits<float>::max());
        const Cluster* const pBestMatchedCluster =
          this->GetBestMatchedCluster(pCluster1,
                                      pCluster2,
                                      overlapResult.GetCommonMuonPfoList(),
                                      matchedClusters,
                                      reducedChiSquared);

        TwoViewDeltaRayOverlapResult newOverlapResult(overlapResult.GetXOverlap(),
                                                      overlapResult.GetCommonMuonPfoList(),
                                                      pBestMatchedCluster,
                                                      matchedClusters,
                                                      reducedChiSquared);
  ...
  void TwoViewDeltaRayMatchingAlgorithm::ExamineOverlapContainer()
  {
    ...
    for (auto toolIter = m_algorithmToolVector.begin(); toolIter != m_algorithmToolVector.end();) {
      ...
      if (repeatCounter > m_nMaxMatrixToolRepeats) break;
    }
  }
  ...
  StatusCode TwoViewDeltaRayMatchingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ProcessAlgorithmToolList(*this, xmlHandle, "DeltaRayTools", algorithmToolVector));

    for (auto algorithmTool : algorithmToolVector) {
      DeltaRayMatrixTool* const pDeltaRayMatrixTool(
        dynamic_cast<DeltaRayMatrixTool*>(algorithmTool));

      if (!pDeltaRayMatrixTool) return STATUS_CODE_INVALID_PARAMETER;
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NMaxMatrixToolRepeats", m_nMaxMatrixToolRepeats));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinDistanceFromMuon", m_minDistanceFromMuon));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxDistanceToCollected", m_maxDistanceToCollected));
