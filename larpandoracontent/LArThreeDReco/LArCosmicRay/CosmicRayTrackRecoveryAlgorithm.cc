...
namespace lar_content {
  ...
  StatusCode CosmicRayTrackRecoveryAlgorithm::Run()
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      this->GetAvailableClusters(m_inputClusterListNameU, availableClustersU));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      this->GetAvailableClusters(m_inputClusterListNameV, availableClustersV));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      this->GetAvailableClusters(m_inputClusterListNameW, availableClustersW));
    ...
    this->MatchThreeViews(cleanClustersU,
                          cleanClustersV,
                          cleanClustersW,
                          matchedClustersUV,
                          matchedClustersVW,
    ...
    this->MatchTwoViews(cleanClustersU,
                        cleanClustersV,
                        cleanClustersW,
                        matchedClustersUV,
                        matchedClustersVW,
    ...
    this->MatchOneView(cleanClustersU,
                       cleanClustersV,
                       cleanClustersW,
                       matchedClustersUV,
                       matchedClustersVW,
  ...
  StatusCode CosmicRayTrackRecoveryAlgorithm::GetAvailableClusters(
    ...
    ClusterVector& clusterVector) const
  {
    ...
    if (!pClusterList || pClusterList->empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "CosmicRayTrackRecoveryAlgorithm: unable to find cluster list "
                  << inputClusterListName << std::endl;

      return STATUS_CODE_SUCCESS;
    }

    for (const Cluster* const pCluster : *pClusterList) {
      if (PandoraContentApi::IsAvailable(*this, pCluster)) clusterVector.push_back(pCluster);
    }
  ...
  void CosmicRayTrackRecoveryAlgorithm::SelectCleanClusters(const ClusterVector& inputVector,
                                                            ClusterVector& outputVector) const
  {
    for (ClusterVector::const_iterator iter = inputVector.begin(), iterEnd = inputVector.end();
      ...
      if (pCluster->GetNCaloHits() < m_clusterMinHits) continue;
      ...
      if (std::fabs(deltaCoordinate.GetZ()) < m_clusterMinSpanZ ||
          std::fabs(deltaCoordinate.GetX()) < m_clusterMinOverlapX)
        continue;
  ...
  void CosmicRayTrackRecoveryAlgorithm::BuildSlidingFitResultMap(
    ...
    TwoDSlidingFitResultMap& slidingFitResultMap) const
  {
    ...
    for (ClusterVector::const_iterator iter = clusterVector.begin(), iterEnd = clusterVector.end();
         iter != iterEnd;
         ++iter) {
      if (slidingFitResultMap.end() == slidingFitResultMap.find(*iter)) {
        try {
          ...
          if (!slidingFitResultMap
                 .insert(TwoDSlidingFitResultMap::value_type(*iter, slidingFitResult))
                 .second)
  ...
  void CosmicRayTrackRecoveryAlgorithm::MatchViews(
    const ClusterVector& clusterVector1,
    const ClusterVector& clusterVector2,
    const TwoDSlidingFitResultMap& slidingFitResultMap,
    ClusterAssociationMap& clusterAssociationMap) const
  {
    for (ClusterVector::const_iterator iter1 = clusterVector1.begin(),
                                       iterEnd1 = clusterVector1.end();
         iter1 != iterEnd1;
         ++iter1)
      this->MatchClusters(*iter1, clusterVector2, slidingFitResultMap, clusterAssociationMap);

    for (ClusterVector::const_iterator iter2 = clusterVector2.begin(),
                                       iterEnd2 = clusterVector2.end();
         iter2 != iterEnd2;
         ++iter2)
  ...
  void CosmicRayTrackRecoveryAlgorithm::MatchClusters(
    const Cluster* const pSeedCluster,
    const ClusterVector& targetClusters,
    const TwoDSlidingFitResultMap& slidingFitResultMap,
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    if (slidingFitResultMap.end() == fsIter) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    for (ClusterVector::const_iterator tIter = targetClusters.begin(),
                                       tIterEnd = targetClusters.end();
         tIter != tIterEnd;
         ++tIter) {
      const Cluster* const pTargetCluster = *tIter;

      UIntSet daughterVolumeIntersection;
      LArGeometryHelper::GetCommonDaughterVolumes(
        pSeedCluster, pTargetCluster, daughterVolumeIntersection);

      if (daughterVolumeIntersection.empty()) continue;

      if (LArClusterHelper::GetClusterHitType(pSeedCluster) ==
          LArClusterHelper::GetClusterHitType(pTargetCluster))
        throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

      TwoDSlidingFitResultMap::const_iterator ftIter = slidingFitResultMap.find(*tIter);

      if (slidingFitResultMap.end() == ftIter) throw StatusCodeException(STATUS_CODE_FAILURE);
      ...
      if (xSpan2 > 1.5f * xSpan1) continue;
      ...
      if (xOverlap < m_clusterMinOverlapX) continue;
    ...
    if (pBestCluster) {
      ...
      TwoDSlidingFitResultMap::const_iterator iterInner =
        slidingFitResultMap.find(pBestClusterInner);
      TwoDSlidingFitResultMap::const_iterator iterOuter =
        slidingFitResultMap.find(pBestClusterOuter);
      ...
      try {
        LArPointingClusterHelper::GetClosestVertices(
          pointingClusterInner, pointingClusterOuter, pointingVertexInner, pointingVertexOuter);
      }
      catch (StatusCodeException&) {
        return;
      }

      const LArPointingCluster::Vertex pointingEndInner(pointingVertexInner.IsInnerVertex() ?
                                                          pointingClusterInner.GetOuterVertex() :
                                                          pointingClusterInner.GetInnerVertex());
      const LArPointingCluster::Vertex pointingEndOuter(pointingVertexOuter.IsInnerVertex() ?
                                                          pointingClusterOuter.GetOuterVertex() :
                                                          pointingClusterOuter.GetInnerVertex());

      const float rSpan(
        (pointingEndInner.GetPosition() - pointingEndOuter.GetPosition()).GetMagnitude());

      if (LArPointingClusterHelper::IsEmission(pointingVertexInner.GetPosition(),
                                               pointingVertexOuter,
                                               -1.f,
                                               0.75f * rSpan,
                                               5.f,
                                               10.f) &&
          LArPointingClusterHelper::IsEmission(pointingVertexOuter.GetPosition(),
                                               pointingVertexInner,
                                               -1.f,
                                               0.75f * rSpan,
                                               5.f,
        ...
        if (bestInnerList.end() ==
            std::find(bestInnerList.begin(), bestInnerList.end(), pSeedCluster))
          bestInnerList.push_back(pSeedCluster);

        ClusterList& bestOuterList(clusterAssociationMap[pBestClusterOuter]);

        if (bestOuterList.end() ==
            std::find(bestOuterList.begin(), bestOuterList.end(), pSeedCluster))
  ...
  void CosmicRayTrackRecoveryAlgorithm::MatchThreeViews(
    const ClusterVector& clusterVectorU,
    const ClusterVector& clusterVectorV,
    const ClusterVector& clusterVectorW,
    const ClusterAssociationMap& matchedClustersUV,
    const ClusterAssociationMap& matchedClustersVW,
    const ClusterAssociationMap& matchedClustersWU,
    ParticleList& particleList) const
  {
    ...
    for (ClusterVector::const_iterator iter1 = clusterVector1.begin(),
      ...
      if (vetoList.count(pCluster1)) continue;

      const ClusterAssociationMap::const_iterator iter311 = matchedClusters31.find(pCluster1);
      const ClusterList matchedClusters31_pCluster1(
        iter311 != matchedClusters31.end() ? iter311->second : ClusterList());

      const ClusterAssociationMap::const_iterator iter121 = matchedClusters12.find(pCluster1);
      const ClusterList matchedClusters12_pCluster1(
        iter121 != matchedClusters12.end() ? iter121->second : ClusterList());

      for (ClusterVector::const_iterator iter2 = clusterVector2.begin(),
                                         iterEndV = clusterVector2.end();
           iter2 != iterEndV;
           ++iter2) {
        const Cluster* const pCluster2 = *iter2;

        if (vetoList.count(pCluster2)) continue;

        const ClusterAssociationMap::const_iterator iter122 = matchedClusters12.find(pCluster2);
        const ClusterList matchedClusters12_pCluster2(
          iter122 != matchedClusters12.end() ? iter122->second : ClusterList());

        const ClusterAssociationMap::const_iterator iter232 = matchedClusters23.find(pCluster2);
        const ClusterList matchedClusters23_pCluster2(
          iter232 != matchedClusters23.end() ? iter232->second : ClusterList());

        for (ClusterVector::const_iterator iter3 = clusterVector3.begin(),
                                           iterEnd3 = clusterVector3.end();
             iter3 != iterEnd3;
             ++iter3) {
          const Cluster* const pCluster3 = *iter3;

          if (vetoList.count(pCluster3)) continue;

          const ClusterAssociationMap::const_iterator iter233 = matchedClusters23.find(pCluster3);
          const ClusterList matchedClusters23_pCluster3(
            iter233 != matchedClusters23.end() ? iter233->second : ClusterList());

          const ClusterAssociationMap::const_iterator iter313 = matchedClusters31.find(pCluster3);
          const ClusterList matchedClusters31_pCluster3(
            iter313 != matchedClusters31.end() ? iter313->second : ClusterList());

          const bool match12(
            (matchedClusters12_pCluster1.size() + matchedClusters12_pCluster2.size() > 0) &&
            ((matchedClusters12_pCluster1.size() == 1 &&
              std::find(matchedClusters12_pCluster1.begin(),
            ...
            ((matchedClusters12_pCluster2.size() == 1 &&
              std::find(matchedClusters12_pCluster2.begin(),
          ...
          const bool match23(
            ...
            ((matchedClusters23_pCluster2.size() == 1 &&
              std::find(matchedClusters23_pCluster2.begin(),
            ...
            ((matchedClusters23_pCluster3.size() == 1 &&
              std::find(matchedClusters23_pCluster3.begin(),
          ...
          const bool match31(
            ...
            ((matchedClusters31_pCluster3.size() == 1 &&
              std::find(matchedClusters31_pCluster3.begin(),
            ...
            ((matchedClusters31_pCluster1.size() == 1 &&
              std::find(matchedClusters31_pCluster1.begin(),
  ...
  void CosmicRayTrackRecoveryAlgorithm::MatchTwoViews(
    const ClusterVector& clusterVectorU,
    const ClusterVector& clusterVectorV,
    const ClusterVector& clusterVectorW,
    const ClusterAssociationMap& matchedClustersUV,
    ...
    ParticleList& particleList) const
  {
    ...
    for (unsigned int iView = 0; iView < 3; ++iView) {
      const ClusterVector& clusterVector1(
        (0 == iView) ? clusterVectorU : (1 == iView) ? clusterVectorV : clusterVectorW);
      const ClusterVector& clusterVector2(
        (0 == iView) ? clusterVectorV : (1 == iView) ? clusterVectorW : clusterVectorU);
      const ClusterVector& clusterVector3(
        (0 == iView) ? clusterVectorW : (1 == iView) ? clusterVectorU : clusterVectorV);

      const ClusterAssociationMap& matchedClusters12(
        ((0 == iView) ? matchedClustersUV : (1 == iView) ? matchedClustersVW : matchedClustersWU));
      const ClusterAssociationMap& matchedClusters23(
        ((0 == iView) ? matchedClustersVW : (1 == iView) ? matchedClustersWU : matchedClustersUV));
      const ClusterAssociationMap& matchedClusters31(
        ((0 == iView) ? matchedClustersWU : (1 == iView) ? matchedClustersUV : matchedClustersVW));
      ...
      for (ClusterVector::const_iterator iter1 = clusterVector1.begin(),
        ...
        for (ClusterVector::const_iterator iter2 = clusterVector2.begin(),
          ...
          for (ClusterVector::const_iterator iter3 = clusterVector3.begin(),
            ...
            const bool match3(
              (matchedClusters31_pCluster3.size() + matchedClusters23_pCluster3.size() > 0) &&
              ((matchedClusters31_pCluster3.size() == 1 &&
                std::find(matchedClusters31_pCluster3.begin(),
                          matchedClusters31_pCluster3.end(),
                          pCluster1) != matchedClusters31_pCluster3.end()) ||
            ...
            if (match3) newParticle.m_clusterList.push_back(pCluster3);
          }
  ...
  void CosmicRayTrackRecoveryAlgorithm::MatchOneView(const ClusterVector& clusterVectorU,
                                                     const ClusterVector& clusterVectorV,
                                                     const ClusterVector& clusterVectorW,
                                                     const ClusterAssociationMap& matchedClustersUV,
                                                     const ClusterAssociationMap& matchedClustersVW,
                                                     ...
                                                     ParticleList& particleList) const
  {
    ...
    for (unsigned int iView = 0; iView < 3; ++iView) {
      ...
      for (ClusterVector::const_iterator iter1 = clusterVector1.begin(),
        ...
        if (matchedClusters12_pCluster1.size() + matchedClusters31_pCluster1.size() > 0) continue;
        ...
        if (newParticle.m_clusterList.size() > 1) newParticleList.push_back(newParticle);
      }
    }
  ...
  void CosmicRayTrackRecoveryAlgorithm::BuildVetoList(const ParticleList& particleList,
                                                      ClusterSet& vetoList) const
  {
    for (ParticleList::const_iterator pIter = particleList.begin(), pIterEnd = particleList.end();
         pIter != pIterEnd;
         ++pIter) {
  ...
  void CosmicRayTrackRecoveryAlgorithm::RemoveAmbiguities(const ParticleList& inputParticleList,
                                                          ParticleList& outputParticleList) const
  {
    for (ParticleList::const_iterator pIter1 = inputParticleList.begin(),
                                      pIterEnd1 = inputParticleList.end();
         pIter1 != pIterEnd1;
         ++pIter1) {
      ...
      try {
        ...
        for (ParticleList::const_iterator pIter2 = pIter1, pIterEnd2 = inputParticleList.end();
             pIter2 != pIterEnd2;
             ++pIter2) {
          ...
          for (ClusterList::const_iterator cIter1 = clusterList1.begin(),
                                           cIterEnd1 = clusterList1.end();
               cIter1 != cIterEnd1;
               ++cIter1) {
        ...
        if (!isUnique) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

        outputParticleList.push_back(particle1);
      }
      catch (StatusCodeException&) {
        std::cout << " Warning in CosmicRayTrackRecoveryAlgorithm: found duplicate particles in "
                     "candidate list "
                  << std::endl;
      }
    }
  ...
  void CosmicRayTrackRecoveryAlgorithm::BuildParticles(const ParticleList& particleList)
  {
    ...
    for (const Particle& particle : particleList) {
      if (!PandoraContentApi::IsAvailable(*this, &particle.m_clusterList)) continue;

      ClusterList clusterList;
      this->MergeClusters(particle.m_clusterList, clusterList);

      if (clusterList.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    if (!pPfoList->empty())
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::SaveList<Pfo>(*this, m_outputPfoListName));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void CosmicRayTrackRecoveryAlgorithm::MergeClusters(const ClusterList& inputClusterList,
                                                      ClusterList& outputClusterList) const
  {
    ...
    for (unsigned int iView = 0; iView < 3; ++iView) {
      const ClusterList clusterList((0 == iView) ? clusterListU :
                                                   (1 == iView) ? clusterListV : clusterListW);
      ...
      for (const Cluster* const pAssociatedCluster : clusterList) {
        if (pAssociatedCluster == pSeedCluster) continue;

        if (!PandoraContentApi::IsAvailable(*this, pAssociatedCluster)) continue;
  ...
  StatusCode CosmicRayTrackRecoveryAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "ClusterMinLength", m_clusterMinLength));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ClusterMinSpanZ", m_clusterMinSpanZ));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ClusterMinOverlapX", m_clusterMinOverlapX));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ClusterMaxDeltaX", m_clusterMaxDeltaX));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ClusterMinHits", m_clusterMinHits));

    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputClusterListNameU", m_inputClusterListNameU));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputClusterListNameV", m_inputClusterListNameV));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputClusterListNameW", m_inputClusterListNameW));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "OutputPfoListName", m_outputPfoListName));
