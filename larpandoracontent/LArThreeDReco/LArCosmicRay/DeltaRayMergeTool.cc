...
namespace lar_content {
  ...
  bool DeltaRayMergeTool::Run(ThreeViewDeltaRayMatchingAlgorithm* const pAlgorithm,
        ...
        for (const HitType hitType1 : {TPC_VIEW_U, TPC_VIEW_V}) {
          if ((element1.GetCluster(hitType1) == element2.GetCluster(hitType1))) {
            for (const HitType hitType2 : {TPC_VIEW_V, TPC_VIEW_W}) {
              if (hitType1 == hitType2) continue;

              if ((element1.GetCluster(hitType2) == element2.GetCluster(hitType2))) {
                const HitType mergeHitType(hitType1 == TPC_VIEW_U ?
                                             (hitType2 == TPC_VIEW_V ? TPC_VIEW_W : TPC_VIEW_V) :
                                             TPC_VIEW_U);
                const Cluster *pClusterToEnlarge(element1.GetCluster(mergeHitType)),
  ...
  bool DeltaRayMergeTool::AreAssociated(const TensorType::Element& element1,
                                        const TensorType::Element& element2,
                                        const HitType& mergeHitType) const
  {
    ...
    if (commonMuonPfoList.empty()) return false;

    const Cluster *const pCluster1(element1.GetCluster(mergeHitType)), *const pCluster2(
                                                                         element2.GetCluster(
                                                                           mergeHitType));
    ...
    for (const ParticleFlowObject* const pConnectedMuon1 : connectedMuonPfoList1) {
      for (const ParticleFlowObject* const pConnectedMuon2 : connectedMuonPfoList2) {
        if ((pConnectedMuon1 == pConnectedMuon2) &&
            (this->IsHiddenByTrack(pConnectedMuon1, pCluster1, pCluster2)))
          return true;
      }
    }
  ...
  void DeltaRayMergeTool::CombineCommonMuonPfoLists(const PfoList& commonMuonPfoList1,
                                                    const PfoList& commonMuonPfoList2,
                                                    PfoList& commonMuonPfoList) const
  {
    for (const ParticleFlowObject* const pCommonMuonPfo1 : commonMuonPfoList1) {
      for (const ParticleFlowObject* const pCommonMuonPfo2 : commonMuonPfoList2) {
        if (pCommonMuonPfo1 == pCommonMuonPfo2) commonMuonPfoList.push_back(pCommonMuonPfo1);
      }
    }
  }
  ...
  void DeltaRayMergeTool::GetConnectedMuons(const Cluster* const pDeltaRayCluster,
                                            const PfoList& commonMuonPfoList,
                                            PfoList& connectedMuonPfoList) const
  {
  ...
  bool DeltaRayMergeTool::IsConnected(const Cluster* const pCluster,
                                      const Pfo* const pCommonMuonPfo) const
  {
    ...
    if (muonClusterList.size() != 1) return false;
  ...
  bool DeltaRayMergeTool::IsBrokenCluster(const Cluster* const pClusterToEnlarge,
                                          const Cluster* const pClusterToDelete) const
  {
    const float clusterSeparation(
      LArClusterHelper::GetClosestDistance(pClusterToEnlarge, pClusterToDelete));
  ...
  bool DeltaRayMergeTool::IsHiddenByTrack(const ParticleFlowObject* const pMuonPfo,
                                          ...
                                          const Cluster* const pCluster2) const
  {
    ...
    for (const CaloHit* const pCaloHit : vertices1) {
      const float separation(
        LArClusterHelper::GetClosestDistance(pCaloHit->GetPositionVector(), vertices2));

      if (separation < m_maxVertexSeparation) return true;
    }
  ...
  void DeltaRayMergeTool::FindVertices(const Pfo* const pCommonMuonPfo,
                                       const Cluster* const pCluster,
                                       CaloHitList& vertexList) const
  {
    HitType hitType(LArClusterHelper::GetClusterHitType(pCluster));
    ...
    if (muonClusterList.size() != 1) return;

    CaloHitList caloHitList;
    muonClusterList.front()->GetOrderedCaloHitList().FillCaloHitList(caloHitList);

    for (const CaloHit* const pCaloHit : caloHitList) {
      if (LArClusterHelper::GetClosestDistance(pCaloHit->GetPositionVector(), pCluster) <
      ...
      for (auto iter2 = std::next(iter1); iter2 != elementList.end(); ++iter2) {
        ...
        for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W}) {
          if (element1.GetCluster(hitType) == element2.GetCluster(hitType)) {
            const HitType mergeHitType1(
              hitType == TPC_VIEW_U ? TPC_VIEW_V : hitType == TPC_VIEW_V ? TPC_VIEW_W : TPC_VIEW_U);
            const HitType mergeHitType2(mergeHitType1 == TPC_VIEW_U ?
                                          TPC_VIEW_V :
                                          mergeHitType1 == TPC_VIEW_V ? TPC_VIEW_W : TPC_VIEW_U);

            const Cluster *pClusterToEnlarge1 = element1.GetCluster(mergeHitType1),
                          *pClusterToDelete1 = element2.GetCluster(mergeHitType1);
            const Cluster *pClusterToEnlarge2 = element1.GetCluster(mergeHitType2),
                          *pClusterToDelete2 = element2.GetCluster(mergeHitType2);

            if ((pClusterToEnlarge1 == pClusterToDelete1) ||
                (pClusterToEnlarge2 == pClusterToDelete2))
              continue;

            if (!this->AreAssociated(element1, element2, mergeHitType1)) continue;

            if (!this->AreAssociated(element1, element2, mergeHitType2)) continue;
            ...
            StatusCode status(m_pParentAlgorithm->PerformThreeViewMatching(
            ...
            if (reducedChiSquared < m_maxGoodMatchReducedChiSquared) {
              ...
              PANDORA_THROW_RESULT_IF(
                ...
                !=,
                PandoraContentApi::ReplaceCurrentList<Cluster>(
                  *m_pParentAlgorithm, m_pParentAlgorithm->GetClusterListName(mergeHitType1)));

              PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                                      !=,
                                      PandoraContentApi::MergeAndDeleteClusters(*m_pParentAlgorithm,
                                                                                pClusterToEnlarge1,
                                                                                pClusterToDelete1));
              ...
              PANDORA_THROW_RESULT_IF(
                ...
                !=,
                PandoraContentApi::ReplaceCurrentList<Cluster>(
                  *m_pParentAlgorithm, m_pParentAlgorithm->GetClusterListName(mergeHitType2)));
  ...
  StatusCode DeltaRayMergeTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxVertexSeparation", m_maxVertexSeparation));
