 ...
 */
  ...
  private:
     ...
     */
    virtual bool PassElementChecks(const TensorType::Element& element,
     ...
     */
    void SplitMuonCluster(const TensorType::Element& element,
                          const pandora::HitType hitType,
                          const pandora::CaloHitList& deltaRayHits) const;

    unsigned int
      m_slidingFitWindow; ///< The sliding fit window used in cosmic ray parameterisations
    float
      m_minDeviationFromTransverse; ///< The minimum deviation from transverse required to avoid mistakes
    float
      m_contaminationWindow; ///< The distance in which to search for delta ray contamination in the cosmic ray track
    unsigned int
      m_significantHitThreshold; ///< The threshold number of hits which define significant contimination
    float
