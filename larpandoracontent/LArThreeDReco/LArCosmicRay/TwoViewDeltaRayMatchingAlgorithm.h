 ...
 */
  class TwoViewDeltaRayMatchingAlgorithm
    : public NViewDeltaRayMatchingAlgorithm<TwoViewMatchingControl<TwoViewDeltaRayOverlapResult>> {
  public:
    typedef NViewDeltaRayMatchingAlgorithm<TwoViewMatchingControl<TwoViewDeltaRayOverlapResult>>
      BaseAlgorithm;
     ...
     */
    const pandora::Cluster* GetCluster(const MatrixType::Element& element,
     ...
     */
    void UpdateForThirdViewClusterModification(const pandora::Cluster* const pModifiedCluster,
                                               const bool isMuon);

  private:
    void CalculateOverlapResult(const pandora::Cluster* const pCluster1,
     ...
     */
    pandora::StatusCode CalculateOverlapResult(const pandora::Cluster* const pCluster1,
                                               const pandora::Cluster* const pCluster2,
     ...
     */
    void FindCommonMuonParents(const pandora::Cluster* const pCluster1,
                               const pandora::Cluster* const pCluster2,
                               pandora::PfoList& commonMuonPfoList) const;
     ...
     */
    void CollectThirdViewClusters(const pandora::Cluster* const pCluster1,
                                  const pandora::Cluster* const pCluster2,
     ...
     */
    const pandora::Cluster* GetBestMatchedCluster(const pandora::Cluster* const pCluster1,
                                                  const pandora::Cluster* const pCluster2,
     ...
     */
    void MergeThirdView(const MatrixType::Element& element,
    ...
    std::string
      m_inputClusterListName; ///< The name of the cluster list in the view in which to project into
    std::string
      m_reclusteringAlgorithmName; ///< The name of the clustering algorithm to be used to recluster created delta ray remnants
    ...
    unsigned int
      m_minClusterCaloHits; ///< The threshold number of hits for a cluster to be considered
    float
      m_maxDistanceFromPrediction; ///< The maximum distance of a matched cluster from the third view projection points
    float
      m_maxGoodMatchReducedChiSquared; ///< The maximum reduced chi squared value of a good 1:1:1 match
    float
      m_minDistanceFromMuon; ///< The minimum distance of a hit from the cosmic ray track required for removal
    float
      m_maxDistanceToCollected; ///< The maximim distance of a hit from the projected delta ray hits required for removal
  };
 ...
 */
  ...
  public:
     ...
     */
    virtual bool Run(TwoViewDeltaRayMatchingAlgorithm* const pAlgorithm,
                     MatrixType& matrixTensor) = 0;

    TwoViewDeltaRayMatchingAlgorithm*
      m_pParentAlgorithm; ///< Address of the parent matching algorithm
  };
