...
namespace lar_content {
  ...
  ThreeViewDeltaRayMatchingAlgorithm::ThreeViewDeltaRayMatchingAlgorithm()
    : m_minClusterCaloHits(5), m_nMaxTensorToolRepeats(10)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool ThreeViewDeltaRayMatchingAlgorithm::DoesClusterPassTensorThreshold(
  ...
  void ThreeViewDeltaRayMatchingAlgorithm::CalculateOverlapResult(const Cluster* const pClusterU,
  ...
  StatusCode ThreeViewDeltaRayMatchingAlgorithm::CalculateOverlapResult(
    ...
    DeltaRayOverlapResult& overlapResult) const
  {
    ...
    StatusCode statusCode(this->PerformThreeViewMatching(pClusterU,
                                                         pClusterV,
                                                         pClusterW,
                                                         chiSquaredSum,
                                                         nSamplingPoints,
    ...
    if (statusCode != STATUS_CODE_SUCCESS) return statusCode;
    ...
    overlapResult = DeltaRayOverlapResult(
      nMatchedSamplingPoints, nSamplingPoints, chiSquaredSum, xOverlapObject, commonMuonPfoList);
  ...
  void ThreeViewDeltaRayMatchingAlgorithm::FindCommonMuonParents(const Cluster* const pClusterU,
                                                                 const Cluster* const pClusterV,
                                                                 const Cluster* const pClusterW,
                                                                 PfoList& commonMuonPfoList) const
  {
    ...
    for (const ParticleFlowObject* const pNearbyMuonU : nearbyMuonPfosU) {
      for (const ParticleFlowObject* const pNearbyMuonV : nearbyMuonPfosV) {
        if (pNearbyMuonV != pNearbyMuonU) continue;

        for (const ParticleFlowObject* const pNearbyMuonW : nearbyMuonPfosW) {
          if (pNearbyMuonW == pNearbyMuonV) commonMuonPfoList.emplace_back(pNearbyMuonU);
        }
      }
    }
  }
  ...
  StatusCode ThreeViewDeltaRayMatchingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    for (auto algorithmTool : algorithmToolVector) {
      DeltaRayTensorTool* const pDeltaRayTensorTool(
        dynamic_cast<DeltaRayTensorTool*>(algorithmTool));

      if (!pDeltaRayTensorTool) return STATUS_CODE_INVALID_PARAMETER;
