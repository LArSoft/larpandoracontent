...
namespace lar_content {
  ...
  bool TwoViewCosmicRayRemovalTool::Run(TwoViewDeltaRayMatchingAlgorithm* const pAlgorithm,
                                        MatrixType& overlapMatrix)
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      if (usedKeyClusters.count(pKeyCluster)) continue;
  ...
  bool TwoViewCosmicRayRemovalTool::RemoveCosmicRayHits(
    const MatrixType::ElementList& elementList) const
  {
    ...
    for (const MatrixType::Element& element : elementList) {
      for (const HitType hitType : m_pParentAlgorithm->GetHitTypeVector()) {
        ...
        if (checkedClusters.count(pDeltaRayCluster)) continue;

        // ATTN: The underlying matrix will update during this loop, do not proceed if element has been modified
        if ((modifiedClusters.count(element.GetCluster1())) ||
            (modifiedClusters.count(element.GetCluster2())))
          continue;

        if (!this->PassElementChecks(element, hitType)) continue;

        if (!this->IsContaminated(element, hitType)) continue;

        if (!this->IsBestElement(element, hitType, elementList)) continue;
        ...
        if (deltaRayHits.empty()) continue;

        // ATTN: If seed cannot be grown, abort
        CaloHitList deltaRayRemnantHits;
        if (this->GrowSeed(element, hitType, deltaRayHits, deltaRayRemnantHits) !=
            STATUS_CODE_SUCCESS)
          continue;

        if (deltaRayHits.size() == pDeltaRayCluster->GetNCaloHits()) continue;
  ...
  bool TwoViewCosmicRayRemovalTool::PassElementChecks(const MatrixType::Element& element,
                                                      const HitType hitType) const
  {
    // ATTN: Avoid endpoints, topological michel reconstruction is very ambiguous
    if (this->IsMuonEndpoint(element, hitType)) return false;

    const Cluster *pMuonCluster(nullptr), *const pDeltaRayCluster(
                                            m_pParentAlgorithm->GetCluster(element, hitType));

    if (m_pParentAlgorithm->GetMuonCluster(element.GetOverlapResult().GetCommonMuonPfoList(),
                                           hitType,
                                           pMuonCluster) != STATUS_CODE_SUCCESS)
      return false;
  ...
  bool TwoViewCosmicRayRemovalTool::IsMuonEndpoint(const MatrixType::Element& element,
                                                   const HitType hitType) const
  {
    for (const HitType& otherHitType : m_pParentAlgorithm->GetHitTypeVector()) {
      ...
      if ((xMinDR < xMinCR) || (xMaxDR > xMaxCR)) return false;
      ...
      if ((zMinDR < zMinCR) || (zMaxDR > zMaxCR)) return false;
    }
  ...
  bool TwoViewCosmicRayRemovalTool::IsContaminated(const MatrixType::Element& element,
                                                   const HitType hitType) const
  {
    const Cluster *pMuonCluster(nullptr), *const pDeltaRayCluster(
                                            m_pParentAlgorithm->GetCluster(element, hitType));
    ...
    slidingFitResult.GetGlobalDirection(
      slidingFitResult.GetLayerFitResultMap().begin()->second.GetGradient(), muonDirection);

    CartesianVector deltaRayVertex(0.f, 0.f, 0.f), muonVertex(0.f, 0.f, 0.f);
    LArClusterHelper::GetClosestPositions(
      pDeltaRayCluster, pMuonCluster, deltaRayVertex, muonVertex);
    ...
    for (const CaloHit* const pCaloHit : deltaRayHitList) {
      ...
      if (separation > furthestSeparation) {
        if (this->IsCloseToLine(
              position, muonVertex, muonVertex + muonDirection, m_distanceToLine)) {
    ...
    if (furthestSeparation < m_minContaminationLength) return false;
  ...
  bool TwoViewCosmicRayRemovalTool::IsCloseToLine(const CartesianVector& hitPosition,
                                                  ...
                                                  const float distanceToLine) const
  {
    ...
    if (transverseDistanceFromLine > distanceToLine) return false;
  ...
  bool TwoViewCosmicRayRemovalTool::IsInLineSegment(const CartesianVector& lowerBoundary,
                                                  ...
                                                  const MatrixType::ElementList& elementList) const
  {
    ...
    const unsigned int hitSum(element.GetCluster1()->GetNCaloHits() +
                              element.GetCluster2()->GetNCaloHits());

    for (const MatrixType::Element& testElement : elementList) {
      if (m_pParentAlgorithm->GetCluster(testElement, hitType) !=
          m_pParentAlgorithm->GetCluster(element, hitType))
        continue;

      if ((testElement.GetCluster1() == element.GetCluster1()) &&
          (testElement.GetCluster2() == element.GetCluster2()))
        continue;

      const float testChiSquared(testElement.GetOverlapResult().GetReducedChiSquared());
      const unsigned int testHitSum(testElement.GetCluster1()->GetNCaloHits() +
                                    testElement.GetCluster2()->GetNCaloHits());

      if ((testHitSum < hitSum) || ((testHitSum == hitSum) && (testChiSquared > chiSquared)))
        continue;

      if (this->PassElementChecks(testElement, hitType)) return false;
    }
                                               ...
                                               CaloHitList& collectedHits) const
  {
    ...
    if (m_pParentAlgorithm->ParameteriseMuon(
    ...
    if (this->ProjectDeltaRayPositions(element, hitType, deltaRayProjectedPositions) !=
        STATUS_CODE_SUCCESS)
      return;
      ...
      for (const CartesianVector& projectedPosition : deltaRayProjectedPositions) {
        ...
        if (distanceToProjectionSquared < m_maxDistanceToHit * m_maxDistanceToHit) {
          ...
          if ((!collectedHits.empty()) &&
              (LArClusterHelper::GetClosestDistance(position, collectedHits) > m_maxDistanceToHit))
            continue;

          const float distanceToMuonHitsSquared(
            muonDirection.GetCrossProduct(position - positionOnMuon).GetMagnitudeSquared());

          if (distanceToMuonHitsSquared < m_maxDistanceToHit * m_maxDistanceToHit) continue;
  ...
  StatusCode TwoViewCosmicRayRemovalTool::GrowSeed(const MatrixType::Element& element,
                                                   const HitType hitType,
                                                   CaloHitList& deltaRayHits,
                                                   CaloHitList& remnantHits) const
  {
    const Cluster *const pDeltaRayCluster(m_pParentAlgorithm->GetCluster(element, hitType)),
      *pMuonCluster(nullptr);
    ...
    this->CollectHitsFromDeltaRay(positionOnMuon,
                                  muonDirection,
                                  pDeltaRayCluster,
                                  m_maxDistanceToHit,
                                  true,
  ...
  void TwoViewCosmicRayRemovalTool::CollectHitsFromDeltaRay(const CartesianVector& positionOnMuon,
                                                            ...
                                                            const CaloHitList& protectedHits,
                                                            CaloHitList& collectedHits) const
  {
    CaloHitList deltaRayHitList;
    pDeltaRayCluster->GetOrderedCaloHitList().FillCaloHitList(deltaRayHitList);
    ...
    do {
      ...
      for (const CaloHit* const pCaloHit : deltaRayHitList) {
        ...
        const float distanceToCollectedHits(
          collectedHits.empty() ?
        ...
        if ((distanceToMuonHits < minDistanceFromMuon) ||
            (demandCloserToCollected && (distanceToCollectedHits > distanceToMuonHits)))
          continue;
  ...
  void TwoViewCosmicRayRemovalTool::SplitDeltaRayCluster(const MatrixType::Element& element,
                                                         const HitType hitType,
                                                         CaloHitList& collectedHits,
                                                         CaloHitList& deltaRayRemnantHits) const
  {
    const Cluster *const pDeltaRayCluster(m_pParentAlgorithm->GetCluster(element, hitType)),
      *pMuonCluster(nullptr);
    ...
    PANDORA_THROW_RESULT_IF(
      ...
      PandoraContentApi::ReplaceCurrentList<Cluster>(
        *m_pParentAlgorithm, m_pParentAlgorithm->GetClusterListName(hitType)));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
    ...
    for (const CaloHit* const pCaloHit : deltaRayHitList) {
      const bool isDeltaRay(std::find(collectedHits.begin(), collectedHits.end(), pCaloHit) !=
                            collectedHits.end());
      const bool isDeltaRayRemnant(
        !isDeltaRay &&
        (std::find(deltaRayRemnantHits.begin(), deltaRayRemnantHits.end(), pCaloHit) !=
         deltaRayRemnantHits.end()));
      const Cluster*& pCluster(isDeltaRay ? pDeltaRay :
                                            isDeltaRayRemnant ? pDeltaRayRemnant : pMuonCluster);
      ...
      if (!pCluster) {
        ...
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::Cluster::Create(*m_pParentAlgorithm, parameters, pCluster));
      }
      else {
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::AddToCluster(*m_pParentAlgorithm, pCluster, pCaloHit));
      }
    }
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::EndFragmentation(*m_pParentAlgorithm, fragmentListName, originalListName));
  ...
  void TwoViewCosmicRayRemovalTool::ReclusterRemnant(const HitType hitType,
                                                     const Cluster* const pMuonCluster,
                                                     const Cluster* const pDeltaRayRemnant,
                                                     ClusterVector& clusterVector,
                                                     PfoVector& pfoVector) const
  {
    std::string caloHitListName(hitType == TPC_VIEW_U ?
                                  "CaloHitListU" :
                                  hitType == TPC_VIEW_V ? "CaloHitListV" : "CaloHitListW");

    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::ReplaceCurrentList<CaloHit>(*m_pParentAlgorithm, caloHitListName));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::ReplaceCurrentList<Cluster>(
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::Delete<Cluster>(*m_pParentAlgorithm, pDeltaRayRemnant));
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::RunClusteringAlgorithm(*m_pParentAlgorithm,
                                                m_pParentAlgorithm->GetClusteringAlgName(),
                                                pClusterList,
                                                newClusterListName));
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::SaveList<Cluster>(
        *m_pParentAlgorithm, newClusterListName, m_pParentAlgorithm->GetClusterListName(hitType)));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::ReplaceCurrentList<Cluster>(
      ...
      if (pRemnant->GetNCaloHits() < m_minRemnantClusterSize) {
        if (LArClusterHelper::GetClosestDistance(pRemnant, pMuonCluster) < m_maxDistanceToTrack) {
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::MergeAndDeleteClusters(*m_pParentAlgorithm, pMuonCluster, pRemnant));
          continue;
        }
      }
    ...
    CartesianPointVector& projectedPositions) const
  {
    ...
    if (!pCluster1 || !pCluster2) return STATUS_CODE_NOT_FOUND;
  ...
  StatusCode TwoViewCosmicRayRemovalTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinSeparation", m_minSeparation));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinContaminationLength", m_minContaminationLength));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxDistanceToHit", m_maxDistanceToHit));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinRemnantClusterSize", m_minRemnantClusterSize));
