...
namespace lar_content {
  ...
  OneViewDeltaRayMatchingAlgorithm::OneViewDeltaRayMatchingAlgorithm()
    : m_overlapExtension(1.f), m_minClusterHits(3)
  {}
  ...
  StatusCode OneViewDeltaRayMatchingAlgorithm::Run()
  {
    ...
    m_deltaRayMatchingContainers.FillContainers(allPfoList,
                                                this->GetInputClusterList(TPC_VIEW_U),
                                                this->GetInputClusterList(TPC_VIEW_V),
                                                this->GetInputClusterList(TPC_VIEW_W));
  ...
  {
    const std::string inputClusterListName((hitType == TPC_VIEW_U) ?
    ...
    PANDORA_THROW_RESULT_IF_AND_IF(
      ...
      !=,
      PandoraContentApi::GetList(*this, inputClusterListName, pInputClusterList));

    if (!pInputClusterList) return ClusterList();
  ...
  const PfoList OneViewDeltaRayMatchingAlgorithm::GetMuonPfoList()
  {
    ...
    if (!pMuonPfoList) return PfoList();
  ...
  const PfoList OneViewDeltaRayMatchingAlgorithm::GetDeltaRayPfoList()
  {
    ...
    PANDORA_THROW_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_INITIALIZED,
      !=,
      PandoraContentApi::GetList(*this, m_deltaRayPfoListName, pDeltaRayPfoList));

    if (!pDeltaRayPfoList) return PfoList();
  ...
  void OneViewDeltaRayMatchingAlgorithm::PerformOneViewMatching(const HitType hitType)
  {
    ...
    for (auto& entry : clusterProximityMap) {
      if (entry.first->IsAvailable()) availableClusterList.push_back(entry.first);
    }

    std::sort(
      availableClusterList.begin(), availableClusterList.end(), LArClusterHelper::SortByNHits);

    ClusterSet modifiedClusters;

    for (const Cluster* const pAvailableCluster : availableClusterList) {
      if (modifiedClusters.count(pAvailableCluster)) continue;

      const DeltaRayMatchingContainers::ClusterProximityMap::const_iterator iter(
        clusterProximityMap.find(pAvailableCluster));

      // ATTN: Map will update during loop
      if (iter == clusterProximityMap.end()) continue;
      ...
      do {
        ...
        for (const Cluster* const pNearbyCluster : nearbyClusters) {
          if (!this->IsMuonPfo(pNearbyCluster)) continue;

          if (std::find(nearbyMuonPfoVector.begin(),
                        nearbyMuonPfoVector.end(),
                        clusterToPfoMap.at(pNearbyCluster)) != nearbyMuonPfoVector.end())
            continue;

          found = true;

          const float separation(
            LArClusterHelper::GetClosestDistance(pNearbyCluster, pAvailableCluster));
        ...
        if (pClosestMuonPfo) nearbyMuonPfoVector.push_back(pClosestMuonPfo);
      } while (found);

      if (nearbyMuonPfoVector.empty()) continue;

      if (this->AddIntoExistingDeltaRay(pAvailableCluster, nearbyMuonPfoVector)) continue;
  ...
  bool OneViewDeltaRayMatchingAlgorithm::IsMuonPfo(const Cluster* const pCluster)
  {
    ...
    const DeltaRayMatchingContainers::ClusterToPfoMap& clusterToPfoMap(
      m_deltaRayMatchingContainers.GetClusterToPfoMap(hitType));
    const DeltaRayMatchingContainers::ClusterToPfoMap::const_iterator iter(
      clusterToPfoMap.find(pCluster));

    if (iter == clusterToPfoMap.end()) return false;
  ...
  bool OneViewDeltaRayMatchingAlgorithm::AddIntoExistingDeltaRay(
    const Cluster* const pAvailableCluster,
    const PfoVector& nearbyMuonPfoVector)
  {
    const HitType hitType(LArClusterHelper::GetClusterHitType(pAvailableCluster));
    const HitType projectedHitType1(
      (hitType == TPC_VIEW_U) ? TPC_VIEW_V : (hitType == TPC_VIEW_V) ? TPC_VIEW_W : TPC_VIEW_U);
    const HitType projectedHitType2((projectedHitType1 == TPC_VIEW_U) ?
                                      TPC_VIEW_V :
                                      (projectedHitType1 == TPC_VIEW_V) ? TPC_VIEW_W : TPC_VIEW_U);
    const DeltaRayMatchingContainers::ClusterToPfoMap& clusterToPfoMap1(
      m_deltaRayMatchingContainers.GetClusterToPfoMap(projectedHitType1));
    const DeltaRayMatchingContainers::ClusterToPfoMap& clusterToPfoMap2(
      m_deltaRayMatchingContainers.GetClusterToPfoMap(projectedHitType2));

    for (const ParticleFlowObject* const pNearbyMuonPfo : nearbyMuonPfoVector) {
      const Cluster* const pProjectedCluster1(this->GetBestProjectedCluster(
        {pAvailableCluster}, pNearbyMuonPfo, projectedHitType1, false));
      const Cluster* const pProjectedCluster2(this->GetBestProjectedCluster(
        {pAvailableCluster}, pNearbyMuonPfo, projectedHitType2, false));

      if ((!pProjectedCluster1) || (!pProjectedCluster2)) continue;
      ...
      if (pPfo1 == pPfo2) {
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToPfo(*this, pPfo1, pAvailableCluster));
  ...
  const Cluster* OneViewDeltaRayMatchingAlgorithm::GetBestProjectedCluster(
    const ClusterList& deltaRayClusterGroup,
    const ParticleFlowObject* const pNearbyMuonPfo,
    ...
    const bool findAvailable)
  {
    ...
    if (muonClusterList.size() != 1) return nullptr;

    const DeltaRayMatchingContainers::ClusterProximityMap& clusterProximityMap(
      m_deltaRayMatchingContainers.GetClusterProximityMap(hitType));
    auto muonProximityIter(clusterProximityMap.find(muonClusterList.front()));

    if (muonProximityIter == clusterProximityMap.end()) return nullptr;
    ...
    for (const Cluster* const pNearbyCluster : muonProximityIter->second) {
      if (findAvailable && !pNearbyCluster->IsAvailable()) continue;

      if (!findAvailable && !this->IsDeltaRayPfo(pNearbyCluster)) continue;
  ...
  bool OneViewDeltaRayMatchingAlgorithm::IsDeltaRayPfo(const Cluster* const pCluster)
  {
    ...
    return (std::find(deltaRayPfoList.begin(), deltaRayPfoList.end(), pDeltaRayPfo) !=
            deltaRayPfoList.end());
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void OneViewDeltaRayMatchingAlgorithm::GetClusterSpanX(const ClusterList& clusterList,
                                                         float& spanMinX,
                                                         float& spanMaxX)
  {
    ...
    for (const Cluster* const pCluster : clusterList) {
      ...
      if (minX < spanMinX) spanMinX = minX;

      if (maxX > spanMaxX) spanMaxX = maxX;
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void OneViewDeltaRayMatchingAlgorithm::CreateDeltaRay(const Cluster* const pAvailableCluster,
                                                        const PfoVector& nearbyMuonPfoVector,
                                                        ClusterSet& modifiedClusters)
  {
    ...
    for (const ParticleFlowObject* const pNearbyMuonPfo : nearbyMuonPfoVector) {
      const Cluster* const pProjectedCluster1(
        this->GetBestProjectedCluster(clusterGroup, pNearbyMuonPfo, projectedHitType1, true));
      const Cluster* const pProjectedCluster2(
        this->GetBestProjectedCluster(clusterGroup, pNearbyMuonPfo, projectedHitType2, true));

      if ((!pProjectedCluster1) && (!pProjectedCluster2)) continue;

      ClusterList consideredClusters1;
      if (pProjectedCluster1)
        this->GetNearbyAvailableClusters(
          pProjectedCluster1, consideredClusters1, projectedClusters1);

      ClusterList consideredClusters2;
      if (pProjectedCluster2)
        this->GetNearbyAvailableClusters(
          pProjectedCluster2, consideredClusters2, projectedClusters2);
    }
  ...
  void OneViewDeltaRayMatchingAlgorithm::GetNearbyAvailableClusters(const Cluster* const pCluster,
                                                                    ClusterList& consideredClusters,
                                                                    ClusterList& foundClusters)
  {
    const HitType hitType(LArClusterHelper::GetClusterHitType(pCluster));
    const DeltaRayMatchingContainers::ClusterProximityMap& clusterProximityMap(
      m_deltaRayMatchingContainers.GetClusterProximityMap(hitType));
  ...
  const Cluster* OneViewDeltaRayMatchingAlgorithm::MergeClusterGroup(
    const ClusterList& clusterGroup)
  {
    if (clusterGroup.empty()) return nullptr;

    const Cluster* const pClusterToEnlarge(clusterGroup.front());

    if (clusterGroup.size() == 1) return pClusterToEnlarge;
    ...
    for (const Cluster* const pClusterToDelete : clusterGroup) {
      ...
      if (pClusterToDelete != pClusterToEnlarge) {
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::ReplaceCurrentList<Cluster>(*this, inputClusterListName));
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
  ...
  void OneViewDeltaRayMatchingAlgorithm::CreatePfo(const Cluster* const pCluster1,
                                                   ...
                                                   const Cluster* const pCluster3)
  {
    ...
    if (pCluster1) pfoParameters.m_clusterList.push_back(pCluster1);

    if (pCluster2) pfoParameters.m_clusterList.push_back(pCluster2);

    if (pCluster3) pfoParameters.m_clusterList.push_back(pCluster3);

    if (pfoParameters.m_clusterList.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);
    ...
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                            !=,
                            PandoraContentApi::ReplaceCurrentList<Pfo>(*this, m_outputPfoListName));
  ...
  void OneViewDeltaRayMatchingAlgorithm::PerformRecovery(const HitType hitType)
  {
    ...
    for (const Cluster* const pCluster : inputClusterList) {
      ...
      if (pCluster->GetNCaloHits() < m_minClusterHits) continue;
  ...
  StatusCode OneViewDeltaRayMatchingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "DeltaRayPfoListName", m_deltaRayPfoListName));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "OverlapExtension", m_overlapExtension));
