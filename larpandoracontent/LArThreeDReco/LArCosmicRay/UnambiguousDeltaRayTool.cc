...
namespace lar_content {
  ...
  UnambiguousDeltaRayTool::UnambiguousDeltaRayTool()
    : m_maxSeparation(2.f), m_minNConnectedClusters(1)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool UnambiguousDeltaRayTool::Run(ThreeViewDeltaRayMatchingAlgorithm* const pAlgorithm,
  ...
  bool UnambiguousDeltaRayTool::ExamineUnambiguousElements(TensorType::ElementList& elementList)
  {
    ...
    for (TensorType::Element& element : elementList) {
      if (!this->IsConnected(element)) continue;
    ...
    for (const ParticleFlowObject* const pMuonPfo : commonMuonPfoList) {
      ...
      for (const HitType hitType : {TPC_VIEW_U, TPC_VIEW_V, TPC_VIEW_W}) {
        ...
        const float separation(
          LArClusterHelper::GetClosestDistance(element.GetCluster(hitType), muonClusterList));

        if (separation < m_maxSeparation) ++connectedClusterCount;
      }

      if (connectedClusterCount > m_minNConnectedClusters) return true;
    }
  ...
  StatusCode UnambiguousDeltaRayTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinSeparation", m_maxSeparation));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinNConnectedClusters", m_minNConnectedClusters));
