 ...
 */
  ...
  private:
     ...
     */
    void CombineCommonMuonPfoLists(const pandora::PfoList& commonMuonPfoList1,
     ...
     */
    bool AreAssociated(const TensorType::Element& element1,
                       const TensorType::Element& element2,
                       const pandora::HitType& mergeHitType) const;
     ...
     */
    void GetConnectedMuons(const pandora::Cluster* const pDeltaRayCluster,
     ...
     */
    bool IsConnected(const pandora::Cluster* const pCluster,
                     const pandora::Pfo* const pCommonMuonPfo) const;
     ...
     */
    bool IsBrokenCluster(const pandora::Cluster* const pClusterToEnlarge,
                         const pandora::Cluster* const pClusterToDelete) const;
     ...
     */
    bool IsHiddenByTrack(const pandora::ParticleFlowObject* const pMuonPfo,
     ...
     */
    void FindVertices(const pandora::Pfo* const pCommonMuonPfo,
                      const pandora::Cluster* const pCluster,
                      pandora::CaloHitList& vertexList) const;
    ...
    float
      m_maxDRSeparationFromTrack; ///< The maximum distance of a connected delta ray from a cosmic ray track
    float
      m_maxClusterSeparation; ///< The maximum separation of two broken clusters that should be merged
    float
      m_maxVertexSeparation; ///< The maximum separation of the connection points of two delta ray clusters that are hidden by a CR track and should be merged
    float
      m_maxGoodMatchReducedChiSquared; ///< The threshold reduced chi squared value for a potential two view merge to go ahead
  };
