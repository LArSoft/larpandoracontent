...
namespace lar_content {
  ...
  void CosmicRayTrackMatchingAlgorithm::SelectCleanClusters(const ClusterVector& inputVector,
                                                            ClusterVector& outputVector) const
  {
    ...
    for (const Cluster* const pCluster : clusterVector) {
      ...
      for (const Cluster* const pClusterCheck : clusterVector) {
        if (pCluster == pClusterCheck) continue;
      ...
      if (!isDeltaRay) outputVector.push_back(pCluster);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool CosmicRayTrackMatchingAlgorithm::MatchClusters(const Cluster* const pCluster1,
                                                      const Cluster* const pCluster2) const
  {
    ...
    if (xVertex < m_vtxXOverlap) return true;
    ...
    if (xSpan < std::numeric_limits<float>::epsilon()) return false;

    if (xOverlap > m_minXOverlap && xOverlap / xSpan > m_minXOverlapFraction) return true;
  ...
  bool CosmicRayTrackMatchingAlgorithm::CheckMatchedClusters3D(const Cluster* const pCluster1,
                                                               ...
                                                               const Cluster* const pCluster3) const
  {
    ...
    for (unsigned int n = 0; n < 4; ++n) {
      ...
      if (std::fabs(vtx1.GetX() - vtx2.GetX()) <
            std::max(m_vtxXOverlap, std::fabs(vtx1.GetX() - end2.GetX())) &&
          std::fabs(end1.GetX() - end2.GetX()) <
            std::max(m_vtxXOverlap, std::fabs(end1.GetX() - vtx2.GetX())) &&
          std::fabs(vtx2.GetX() - vtx3.GetX()) <
            std::max(m_vtxXOverlap, std::fabs(vtx2.GetX() - end3.GetX())) &&
          std::fabs(end2.GetX() - end3.GetX()) <
            std::max(m_vtxXOverlap, std::fabs(end2.GetX() - vtx3.GetX())) &&
          std::fabs(vtx3.GetX() - vtx1.GetX()) <
            std::max(m_vtxXOverlap, std::fabs(vtx3.GetX() - end1.GetX())) &&
          std::fabs(end3.GetX() - end1.GetX()) <
            std::max(m_vtxXOverlap, std::fabs(end3.GetX() - vtx1.GetX()))) {
        float chi2(0.f);
        ...
        LArGeometryHelper::MergeTwoPositions(
          this->GetPandora(), hitType1, hitType2, vtx1, vtx2, projVtx3, chi2);
        LArGeometryHelper::MergeTwoPositions(
          this->GetPandora(), hitType1, hitType2, end1, end2, projEnd3, chi2);
        LArGeometryHelper::MergeTwoPositions(
          this->GetPandora(), hitType2, hitType3, vtx2, vtx3, projVtx1, chi2);
        LArGeometryHelper::MergeTwoPositions(
          this->GetPandora(), hitType2, hitType3, end2, end3, projEnd1, chi2);
        LArGeometryHelper::MergeTwoPositions(
          this->GetPandora(), hitType3, hitType1, vtx3, vtx1, projVtx2, chi2);
        LArGeometryHelper::MergeTwoPositions(
          this->GetPandora(), hitType3, hitType1, end3, end1, projEnd2, chi2);

        const bool matchedVtx1(LArClusterHelper::GetClosestDistance(projVtx1, pCluster1) <
                               m_maxDisplacement);
        const bool matchedVtx2(LArClusterHelper::GetClosestDistance(projVtx2, pCluster2) <
                               m_maxDisplacement);
        const bool matchedVtx3(LArClusterHelper::GetClosestDistance(projVtx3, pCluster3) <
                               m_maxDisplacement);

        const bool matchedEnd1(LArClusterHelper::GetClosestDistance(projEnd1, pCluster1) <
                               m_maxDisplacement);
        const bool matchedEnd2(LArClusterHelper::GetClosestDistance(projEnd2, pCluster2) <
                               m_maxDisplacement);
        const bool matchedEnd3(LArClusterHelper::GetClosestDistance(projEnd3, pCluster3) <
                               m_maxDisplacement);
  ...
  void CosmicRayTrackMatchingAlgorithm::SetPfoParameters(
  ...
  StatusCode CosmicRayTrackMatchingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "VtxXOverlap", m_vtxXOverlap));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxDisplacement", m_maxDisplacement));
