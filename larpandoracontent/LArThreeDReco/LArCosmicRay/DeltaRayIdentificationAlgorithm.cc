...
namespace lar_content {
  ...
  StatusCode DeltaRayIdentificationAlgorithm::Run()
  {
    ...
    if (parentPfos.empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "DeltaRayIdentificationAlgorithm: pfo list " << m_parentPfoListName
    ...
    if (!newDaughterPfoList.empty()) {
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::SaveList(
          *this, m_parentPfoListName, m_daughterPfoListName, newDaughterPfoList));
    }
  ...
  void DeltaRayIdentificationAlgorithm::GetPfos(const std::string& inputPfoListName,
                                                PfoVector& outputPfoVector) const
  {
  ...
  void DeltaRayIdentificationAlgorithm::BuildAssociationMap(
    const PfoVector& parentPfos,
    const PfoVector& daughterPfos,
    PfoAssociationMap& pfoAssociationMap) const
  {
    ...
    for (PfoVector::const_iterator iter1 = parentPfos.begin(), iterEnd1 = parentPfos.end();
         iter1 != iterEnd1;
         ++iter1) {
      ...
      for (PfoVector::const_iterator iter2 = allPfos.begin(), iterEnd2 = allPfos.end();
           iter2 != iterEnd2;
           ++iter2) {
        const ParticleFlowObject* const pThisParentPfo = *iter2;
        float thisDisplacement(std::numeric_limits<float>::max());

        if (pDaughterPfo == pThisParentPfo) continue;

        if (!this->IsAssociated(pDaughterPfo, pThisParentPfo, thisDisplacement)) continue;
      ...
      if (pBestParentPfo->GetParentPfoList().empty()) {
        ...
        if (daughterPfoList.count(pBestParentPfo)) throw StatusCodeException(STATUS_CODE_FAILURE);
      ...
      // Case 2: candidate parent comes from secondary list
        ...
        if (parentPfoList.count(pBestParentPfo)) throw StatusCodeException(STATUS_CODE_FAILURE);
        ...
        pfoAssociationMap.insert(
          PfoAssociationMap::value_type(pDaughterPfo, pReplacementParentPfo));
      }
    }
  }
  ...
  bool DeltaRayIdentificationAlgorithm::IsAssociated(const ParticleFlowObject* const pDaughterPfo,
                                                     const ParticleFlowObject* const pParentPfo,
                                                     float& displacement) const
  {
    displacement = std::numeric_limits<float>::max();

    if (pDaughterPfo == pParentPfo) return false;
    ...
    if (daughterLengthSquared > m_maxDaughterLengthSquared ||
        parentLengthSquared < m_minParentLengthSquared ||
    ...
    const float displacementCut((daughterLengthSquared > transitionLengthSquared) ?
                                  ...
                                  m_distanceForMatching *
                                    (2.f - daughterLengthSquared / transitionLengthSquared));
    ...
    if (displacement > displacementCut) return false;
  ...
  float DeltaRayIdentificationAlgorithm::GetTwoDSeparation(
  ...
  void DeltaRayIdentificationAlgorithm::GetTwoDVertices(const ParticleFlowObject* const pPfo,
                                                        const HitType& hitType,
                                                        CartesianPointVector& vertexVector) const
  {
  ...
  float DeltaRayIdentificationAlgorithm::GetClosestDistance(
    const CartesianPointVector& vertexVector,
    const ClusterList& clusterList) const
  {
    ...
    for (CartesianPointVector::const_iterator iter1 = vertexVector.begin(),
                                              iterEnd1 = vertexVector.end();
         iter1 != iterEnd1;
         ++iter1) {
      const CartesianVector& thisVertex = *iter1;

      for (ClusterList::const_iterator iter2 = clusterList.begin(), iterEnd2 = clusterList.end();
           iter2 != iterEnd2;
           ++iter2) {
        const Cluster* const pCluster = *iter2;
        const float thisDisplacement(LArClusterHelper::GetClosestDistance(thisVertex, pCluster));

        if (thisDisplacement < bestDisplacement) bestDisplacement = thisDisplacement;
      }
    }
  ...
  void DeltaRayIdentificationAlgorithm::BuildParentDaughterLinks(
    const PfoAssociationMap& pfoAssociationMap,
    PfoList& daughterPfoList) const
  {
    PfoList pfoList;
    ...
    for (const ParticleFlowObject* const pDaughterPfo : pfoList) {
      ...
      if (!pParentPfo) throw StatusCodeException(STATUS_CODE_FAILURE);

      if (!LArPfoHelper::IsTrack(pParentPfo)) continue;
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ParticleFlowObject::AlterMetadata(*this, pDaughterPfo, metadata));
  ...
  const ParticleFlowObject* DeltaRayIdentificationAlgorithm::GetParent(
    const PfoAssociationMap& pfoAssociationMap,
    const ParticleFlowObject* const pPfo) const
  {
    ...
    while (1) {
      ...
      if (pfoAssociationMap.end() == iter) break;
  ...
  StatusCode DeltaRayIdentificationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinParentLength", minParentLength));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxDaughterLength", maxDaughterLength));
