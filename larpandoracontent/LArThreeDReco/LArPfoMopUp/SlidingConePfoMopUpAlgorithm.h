 ...
 */
  ...
  private:
    ...
    public:
      /**
         ...
         */
      ClusterMerge(const pandora::Cluster* const pParentCluster,
    ...
    private:
      const pandora::Cluster*
        m_pParentCluster;       ///< The address of the candidate parent (shower) cluster
     ...
     */
    void GetThreeDClusters(pandora::ClusterVector& clusters3D,
     ...
     */
    void GetClusterMergeMap(const pandora::Vertex* const pVertex,
     ...
     *  @return boolean
     */
    bool IsVertexAssociated(const pandora::Cluster* const pCluster,
     ...
     */
    bool MakePfoMerges(const ClusterToPfoMap& clusterToPfoMap,
                       const ClusterMergeMap& clusterMergeMap) const;

    pandora::StatusCode ReadSettings(const pandora::TiXmlHandle xmlHandle);

    typedef std::unordered_map<const pandora::Cluster*, const pandora::Cluster*>
    ...
    unsigned int
      m_maxHitsToConsider3DTrack; ///< The maximum number of hits in a 3d track cluster to warrant inclusion in algorithm
    unsigned int
      m_minHitsToConsider3DShower; ///< The minimum number of hits in a 3d shower cluster to attempt cone fits
  ...
  inline const pandora::Cluster* SlidingConePfoMopUpAlgorithm::ClusterMerge::GetParentCluster()
    const
  {
