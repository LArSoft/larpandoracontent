...
namespace lar_content {
  ...
  StatusCode SlidingConePfoMopUpAlgorithm::Run()
  {
    ...
    if (m_useVertex && !pVertex) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "SlidingConePfoMopUpAlgorithm - interaction vertex not available for use."
                  << std::endl;
    ...
    while (nIterations++ < m_maxIterations) {
      ...
      if (!this->MakePfoMerges(clusterToPfoMap, clusterMergeMap)) break;
    }
  ...
  void SlidingConePfoMopUpAlgorithm::GetInteractionVertex(const Vertex*& pVertex) const
  {
    ...
    pVertex = ((pVertexList && (pVertexList->size() == 1) &&
  ...
  void SlidingConePfoMopUpAlgorithm::GetThreeDClusters(ClusterVector& clusters3D,
      ...
      for (const Pfo* const pPfo : *pPfoList) {
        ...
        for (const Cluster* const pCluster3D : pfoClusters3D) {
          if (LArPfoHelper::IsTrack(pPfo) &&
              (pCluster3D->GetNCaloHits() > m_maxHitsToConsider3DTrack))
            continue;
  ...
  void SlidingConePfoMopUpAlgorithm::GetClusterMergeMap(const Vertex* const pVertex,
                                                        const ClusterVector& clusters3D,
                                                        ...
                                                        ClusterMergeMap& clusterMergeMap) const
  {
    ...
    for (const Cluster* const pShowerCluster : clusters3D) {
      if ((pShowerCluster->GetNCaloHits() < m_minHitsToConsider3DShower) ||
          !LArPfoHelper::IsShower(clusterToPfoMap.at(pShowerCluster)))
        continue;
      ...
      try {
        const ThreeDSlidingConeFitResult slidingConeFitResult3D(
          pShowerCluster, m_halfWindowLayers, layerPitch);

        const CartesianVector& minLayerPosition(
          slidingConeFitResult3D.GetSlidingFitResult().GetGlobalMinLayerPosition());
        const CartesianVector& maxLayerPosition(
          slidingConeFitResult3D.GetSlidingFitResult().GetGlobalMaxLayerPosition());
        coneLength =
          std::min(m_coneLengthMultiplier * (maxLayerPosition - minLayerPosition).GetMagnitude(),
                   m_maxConeLength);

        const float vertexToMinLayer(
          !pVertex ? 0.f : (pVertex->GetPosition() - minLayerPosition).GetMagnitude());
        const float vertexToMaxLayer(
          !pVertex ? 0.f : (pVertex->GetPosition() - maxLayerPosition).GetMagnitude());
        const ConeSelection coneSelection(!pVertex ? CONE_BOTH_DIRECTIONS :
                                                     (vertexToMaxLayer > vertexToMinLayer) ?
                                                     CONE_FORWARD_ONLY :
                                                     CONE_BACKWARD_ONLY);

        slidingConeFitResult3D.GetSimpleConeList(
          m_nConeFitLayers, m_nConeFits, coneSelection, simpleConeList);
        isShowerVertexAssociated =
          this->IsVertexAssociated(pShowerCluster,
                                   pVertex,
      ...
      for (const Cluster* const pNearbyCluster : clusters3D) {
        if (pNearbyCluster == pShowerCluster) continue;

        ClusterMerge bestClusterMerge(nullptr, 0.f, 0.f);

        for (const SimpleCone& simpleCone : simpleConeList) {
          const float boundedFraction1(
            simpleCone.GetBoundedHitFraction(pNearbyCluster, coneLength, m_coneTanHalfAngle1));
          const float boundedFraction2(
            simpleCone.GetBoundedHitFraction(pNearbyCluster, coneLength, m_coneTanHalfAngle2));
          const ClusterMerge clusterMerge(pShowerCluster, boundedFraction1, boundedFraction2);

          if (clusterMerge < bestClusterMerge) bestClusterMerge = clusterMerge;
        }

        if (isShowerVertexAssociated &&
            this->IsVertexAssociated(pNearbyCluster, pVertex, vertexAssociationMap))
          continue;

        if (bestClusterMerge.GetParentCluster() &&
            (bestClusterMerge.GetBoundedFraction1() > m_coneBoundedFraction1) &&
    ...
    const ThreeDSlidingFitResult* const pSlidingFitResult) const
  {
    ...
    if (vertexAssociationMap.end() != iter) return iter->second;

    const bool isVertexAssociated(
      this->IsVertexAssociated(pCluster, pVertex->GetPosition(), pSlidingFitResult));
    (void)vertexAssociationMap.insert(
      VertexAssociationMap::value_type(pCluster, isVertexAssociated));
    ...
    const ThreeDSlidingFitResult* const pSlidingFitResult) const
  {
    try {
      ...
      const LArPointingCluster pointingCluster(
        pSlidingFitResult ? LArPointingCluster(*pSlidingFitResult) :
                            LArPointingCluster(pCluster, m_halfWindowLayers, layerPitch));

      const bool useInner(
        (pointingCluster.GetInnerVertex().GetPosition() - vertexPosition).GetMagnitudeSquared() <
        (pointingCluster.GetOuterVertex().GetPosition() - vertexPosition).GetMagnitudeSquared());

      const LArPointingCluster::Vertex& daughterVertex(useInner ? pointingCluster.GetInnerVertex() :
                                                                  pointingCluster.GetOuterVertex());
      return LArPointingClusterHelper::IsNode(vertexPosition,
                                              daughterVertex,
  ...
  bool SlidingConePfoMopUpAlgorithm::MakePfoMerges(const ClusterToPfoMap& clusterToPfoMap,
                                                   const ClusterMergeMap& clusterMergeMap) const
  {
    ...
    for (ClusterVector::const_reverse_iterator rIter = daughterClusters.rbegin(),
                                               rIterEnd = daughterClusters.rend();
         rIter != rIterEnd;
         ++rIter) {
      ...
      if (pDaughterCluster == pParentCluster) continue;
      ...
      for (ClusterReplacementMap::value_type& mapEntry : clusterReplacementMap) {
        if (pDaughterCluster == mapEntry.second) mapEntry.second = pParentCluster;
      }
    }
  ...
  {
    if (!this->GetParentCluster() && !rhs.GetParentCluster()) return false;

    if (this->GetParentCluster() && !rhs.GetParentCluster()) return true;

    if (!this->GetParentCluster() && rhs.GetParentCluster()) return false;

    if (std::fabs(this->GetBoundedFraction1() - rhs.GetBoundedFraction1()) >
        std::numeric_limits<float>::epsilon())
      return (this->GetBoundedFraction1() > rhs.GetBoundedFraction1());

    if (std::fabs(this->GetBoundedFraction2() - rhs.GetBoundedFraction2()) >
        std::numeric_limits<float>::epsilon())
  ...
  StatusCode SlidingConePfoMopUpAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "InputPfoListNames", m_inputPfoListNames));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "UseVertex", m_useVertex));
    ...
    m_daughterListNames.insert(
      m_daughterListNames.end(), m_inputPfoListNames.begin(), m_inputPfoListNames.end());
