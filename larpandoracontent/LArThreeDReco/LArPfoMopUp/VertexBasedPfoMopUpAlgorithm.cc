...
namespace lar_content {
  ...
  bool VertexBasedPfoMopUpAlgorithm::IsVertexAssociated(
    const CartesianVector& vertex2D,
    const LArPointingCluster& pointingCluster) const
  {
    return (LArPointingClusterHelper::IsNode(vertex2D,
  ...
  VertexBasedPfoMopUpAlgorithm::PfoAssociation VertexBasedPfoMopUpAlgorithm::GetPfoAssociation(
    const Pfo* const pVertexPfo,
    const Pfo* const pDaughterPfo,
    HitTypeToAssociationMap& hitTypeToAssociationMap) const
  {
    if ((pVertexPfo->GetClusterList().size() != pDaughterPfo->GetClusterList().size()) ||
        (3 != pVertexPfo->GetClusterList().size()))
      throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

    return PfoAssociation(pVertexPfo,
                          pDaughterPfo,
                          hitTypeToAssociationMap.at(TPC_VIEW_U),
                          hitTypeToAssociationMap.at(TPC_VIEW_V),
                          hitTypeToAssociationMap.at(TPC_VIEW_W));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void VertexBasedPfoMopUpAlgorithm::GetInputPfos(const Vertex* const pVertex,
                                                  PfoList& vertexPfos,
                                                  PfoList& nonVertexPfos) const
  {
    ...
    for (const std::string& listName : listNames) {
      ...
      if (STATUS_CODE_SUCCESS != PandoraContentApi::GetList(*this, listName, pPfoList)) continue;

      for (const Pfo* const pPfo : *pPfoList) {
        PfoList& pfoTargetList(this->IsVertexAssociated(pPfo, pVertex) ? vertexPfos :
                                                                         nonVertexPfos);
  ...
  bool VertexBasedPfoMopUpAlgorithm::IsVertexAssociated(const Pfo* const pPfo,
                                                        const Vertex* const pVertex) const
  {
    ...
    for (const Cluster* const pCluster : pPfo->GetClusterList()) {
      ...
      try {
        ...
        if (this->IsVertexAssociated(vertex2D, pointingCluster)) hitTypeSet.insert(hitType);
      }
    ...
    PfoAssociationList& pfoAssociationList) const
  {
    for (const Pfo* const pVertexPfo : vertexPfos) {
      for (const Pfo* const pDaughterPfo : nonVertexPfos) {
        try {
          const PfoAssociation pfoAssociation(
            this->GetPfoAssociation(pVertex, pVertexPfo, pDaughterPfo));
    ...
    const Pfo* const pDaughterPfo) const
  {
    ...
    for (const Cluster* const pVertexCluster : pVertexPfo->GetClusterList()) {
      ...
      for (const Cluster* const pDaughterCluster : pDaughterPfo->GetClusterList()) {
        ...
        if (vertexHitType != daughterHitType) continue;

        const ClusterAssociation clusterAssociation(
          this->GetClusterAssociation(pVertex, pVertexCluster, pDaughterCluster));
  ...
  VertexBasedPfoMopUpAlgorithm::ClusterAssociation
  VertexBasedPfoMopUpAlgorithm::GetClusterAssociation(const Vertex* const pVertex,
                                                      const Cluster* const pVertexCluster,
                                                      const Cluster* const pDaughterCluster) const
  {
    const HitType vertexHitType(LArClusterHelper::GetClusterHitType(pVertexCluster));
    const CartesianVector vertexPosition2D(LArGeometryHelper::ProjectPosition(
      this->GetPandora(), pVertex->GetPosition(), vertexHitType));

    const ConeParameters coneParameters(
      pVertexCluster, vertexPosition2D, m_coneAngleCentile, m_maxConeCosHalfAngle);
    const float boundedFraction(
      coneParameters.GetBoundedFraction(pDaughterCluster, m_maxConeLengthMultiplier));

    const LArVertexHelper::ClusterDirection vertexClusterDirection(
      LArVertexHelper::GetClusterDirectionInZ(
        this->GetPandora(), pVertex, pVertexCluster, m_directionTanAngle, m_directionApexShift));
    const LArVertexHelper::ClusterDirection daughterClusterDirection(
      LArVertexHelper::GetClusterDirectionInZ(
        this->GetPandora(), pVertex, pDaughterCluster, m_directionTanAngle, m_directionApexShift));
    const bool isConsistentDirection(vertexClusterDirection == daughterClusterDirection);

    return ClusterAssociation(
      pVertexCluster, pDaughterCluster, boundedFraction, isConsistentDirection);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool VertexBasedPfoMopUpAlgorithm::ProcessPfoAssociations(
    const PfoAssociationList& pfoAssociationList) const
  {
    ...
    for (const PfoAssociation& pfoAssociation : pfoAssociationList) {
      ...
      if (pTrackPfoList) {
        if ((pTrackPfoList->end() != std::find(pTrackPfoList->begin(),
                                               pTrackPfoList->end(),
                                               pfoAssociation.GetVertexPfo())) &&
            (pTrackPfoList->end() != std::find(pTrackPfoList->begin(),
                                               pTrackPfoList->end(),
                                               pfoAssociation.GetDaughterPfo()))) {
          continue;
        }
  ...
  void VertexBasedPfoMopUpAlgorithm::MergePfos(const PfoAssociation& pfoAssociation) const
  {
    ...
    if (!pTrackPfoList && !pShowerPfoList) throw StatusCodeException(STATUS_CODE_FAILURE);

    const Pfo* const pVertexPfo(pfoAssociation.GetVertexPfo());
    const bool isvertexTrack(pTrackPfoList &&
                             (pTrackPfoList->end() !=
                              std::find(pTrackPfoList->begin(), pTrackPfoList->end(), pVertexPfo)));
    const Pfo* pDaughterPfo(pfoAssociation.GetDaughterPfo());
    const bool isDaughterShower(
      pShowerPfoList && (pShowerPfoList->end() !=
                         std::find(pShowerPfoList->begin(), pShowerPfoList->end(), pDaughterPfo)));
    ...
    if (isvertexTrack && isDaughterShower) {
      ...
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ParticleFlowObject::AlterMetadata(*this, pVertexPfo, metadata));

      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::SaveList(*this, m_trackPfoListName, m_showerPfoListName, vertexPfoList));
    }
  }
  ...
  VertexBasedPfoMopUpAlgorithm::PfoAssociation::PfoAssociation(
    const Pfo* const pVertexPfo,
    const Pfo* const pDaughterPfo,
  ...
  {
    return ((this->GetClusterAssociationU().GetBoundedFraction() +
             this->GetClusterAssociationV().GetBoundedFraction() +
  ...
  float VertexBasedPfoMopUpAlgorithm::PfoAssociation::GetMaxBoundedFraction() const
  {
    return std::max(this->GetClusterAssociationU().GetBoundedFraction(),
                    std::max(this->GetClusterAssociationV().GetBoundedFraction(),
                             this->GetClusterAssociationW().GetBoundedFraction()));
  }
  ...
  float VertexBasedPfoMopUpAlgorithm::PfoAssociation::GetMinBoundedFraction() const
  {
    return std::min(this->GetClusterAssociationU().GetBoundedFraction(),
                    std::min(this->GetClusterAssociationV().GetBoundedFraction(),
                             this->GetClusterAssociationW().GetBoundedFraction()));
  }
  ...
  unsigned int VertexBasedPfoMopUpAlgorithm::PfoAssociation::GetNConsistentDirections() const
  {
    ...
    if (this->GetClusterAssociationU().IsConsistentDirection()) ++nConsistentDirections;

    if (this->GetClusterAssociationV().IsConsistentDirection()) ++nConsistentDirections;

    if (this->GetClusterAssociationW().IsConsistentDirection()) ++nConsistentDirections;
  ...
  {
    if (std::fabs(this->GetMeanBoundedFraction() - rhs.GetMeanBoundedFraction()) >
  ...
  float VertexBasedPfoMopUpAlgorithm::ConeParameters::GetBoundedFraction(
    const Cluster* const pDaughterCluster,
    const float coneLengthMultiplier) const
  {
    ...
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
      ...
      for (CaloHitList::const_iterator hIter = iter->second->begin(),
        ...
        if (m_direction.GetCosOpeningAngle(positionVector - m_apex) < m_coneCosHalfAngle) continue;

        if (m_direction.GetDotProduct(positionVector - m_apex) >
            coneLengthMultiplier * m_coneLength)
          continue;
    ...
    return (static_cast<float>(nMatchedHits) /
  ...
  CartesianVector VertexBasedPfoMopUpAlgorithm::ConeParameters::GetDirectionEstimate() const
  {
    ...
    if (sumDxDx < std::numeric_limits<float>::epsilon()) return CartesianVector(0.f, 0.f, 1.f);
    ...
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
      ...
      for (CaloHitList::const_iterator hitIter = iter->second->begin(),
        ...
        const float projectedLength(
          m_direction.GetDotProduct((*hitIter)->GetPositionVector() - m_apex));
  ...
  float VertexBasedPfoMopUpAlgorithm::ConeParameters::GetCosHalfAngleEstimate(
    const float coneAngleCentile) const
  {
    ...
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
      ...
      for (CaloHitList::const_iterator hitIter = iter->second->begin(),
           ...
           hitIter != hitIterEnd;
           ++hitIter)
        halfAngleValues.push_back(
          m_direction.GetOpeningAngle((*hitIter)->GetPositionVector() - m_apex));
    }

    std::sort(halfAngleValues.begin(), halfAngleValues.end());

    if (halfAngleValues.empty()) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  StatusCode VertexBasedPfoMopUpAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConeAngleCentile", m_coneAngleCentile));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxConeCosHalfAngle", m_maxConeCosHalfAngle));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "DirectionTanAngle", m_directionTanAngle));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "DirectionApexShift", m_directionApexShift));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MeanBoundedFractionCut", m_meanBoundedFractionCut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxBoundedFractionCut", m_maxBoundedFractionCut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinBoundedFractionCut", m_minBoundedFractionCut));
