...
namespace lar_content {
  ...
  bool ConnectedRemnantsTool::Run(ThreeViewRemnantsAlgorithm* const pAlgorithm,
  ...
  void ConnectedRemnantsTool::FindConnectedShowers(const TensorType& overlapTensor,
                                                   ProtoParticleVector& protoParticleVector,
                                                   ClusterMergeMap& clusterMergeMap) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      this->GetClusters(
        connectedElements, usedClusters, clusterVectorU, clusterVectorV, clusterVectorW);

      if (clusterVectorU.empty() || clusterVectorV.empty() || clusterVectorW.empty()) continue;
      ...
      if (!(this->IsConnected(clusterVectorU) && this->IsConnected(clusterVectorV) &&
            this->IsConnected(clusterVectorW)))
        continue;
  ...
  void ConnectedRemnantsTool::GetClusters(const TensorType::ElementList& elementList,
                                          const ClusterSet& usedClusters,
                                          ClusterVector& clusterVectorU,
                                          ClusterVector& clusterVectorV,
                                          ClusterVector& clusterVectorW) const
  {
    for (const TensorType::Element& element : elementList) {
      if (usedClusters.count(element.GetClusterU()) || usedClusters.count(element.GetClusterV()) ||
          usedClusters.count(element.GetClusterW()))
        continue;
  ...
  void ConnectedRemnantsTool::FillMergeMap(const Cluster* const pFirstCluster,
                                           const ClusterVector& clusterVector,
                                           ClusterMergeMap& clusterMergeMap) const
  {
    if (clusterVector.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);

    for (const Cluster* const pSecondCluster : clusterVector) {
      if (pFirstCluster == pSecondCluster) continue;
