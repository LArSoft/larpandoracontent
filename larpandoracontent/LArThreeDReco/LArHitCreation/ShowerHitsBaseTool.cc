...
namespace lar_content {
  ...
  void ShowerHitsBaseTool::Run(ThreeDHitCreationAlgorithm* const pAlgorithm,
                               ...
                               ProtoHitVector& protoHitVector)
  {
    ...
    try {
      if (!LArPfoHelper::IsShower(pPfo)) return;
  ...
  void ShowerHitsBaseTool::GetShowerHits3D(const CaloHitVector& inputTwoDHits,
                                           ...
                                           ProtoHitVector& protoHitVector) const
  {
    for (const CaloHit* const pCaloHit2D : inputTwoDHits) {
      try {
        ...
        this->FilterCaloHits(
          pCaloHit2D->GetPositionVector().GetX(), m_xTolerance, caloHitVector1, filteredHits1);
        this->FilterCaloHits(
          pCaloHit2D->GetPositionVector().GetX(), m_xTolerance, caloHitVector2, filteredHits2);
  ...
  void ShowerHitsBaseTool::FilterCaloHits(const float x,
                                          const float xTolerance,
                                          const CaloHitVector& inputCaloHitVector,
                                          CaloHitVector& outputCaloHitVector) const
  {
    for (const CaloHit* const pCaloHit : inputCaloHitVector) {
      ...
      if (std::fabs(deltaX) < xTolerance) outputCaloHitVector.push_back(pCaloHit);
    }
  }
  ...
  StatusCode ShowerHitsBaseTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "XTolerance", m_xTolerance));
