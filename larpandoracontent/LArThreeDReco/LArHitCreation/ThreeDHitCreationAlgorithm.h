 ...
 */
  ...
  public:
    ...
    public:
      /**
         *  @brief  Constructor
         */
      TrajectorySample(const pandora::CartesianVector& position,
     ...
     */
    void FilterCaloHitsByType(const pandora::CaloHitVector& inputCaloHitVector,
                              const pandora::HitType hitType,
  ...
  private:
     ...
     */
    void SeparateTwoDHits(const pandora::ParticleFlowObject* const pPfo,
     ...
     */
    void ExtractResults(const ProtoHitVector& protoHitVector,
                        double& chi2,
                        pandora::CartesianPointVector& pointVector) const;
     ...
     */
    double GetChi2WrtFit(const ThreeDSlidingFitResult& slidingFitResult,
     ...
     */
    void RefineHitPositions(const ThreeDSlidingFitResult& slidingFitResult,
     ...
     */
    void CreateThreeDHits(const ProtoHitVector& protoHitVector,
     ...
     */
    void AddThreeDHitsToPfo(const pandora::ParticleFlowObject* const pPfo,
    ...
    bool
      m_iterateTrackHits; ///< Whether to enable iterative improvement of 3D hits for track trajectories
    ...
    double
      m_sigma3DFitMultiplier; ///< Multiplicative factor: sigmaUVW (same as sigmaHit and sigma2DFit) to sigma3DFit
    double
      m_iterationMaxChi2Ratio; ///< Max ratio between current and previous chi2 values to cease iterations
  };
  ...
  inline const pandora::CartesianVector& ThreeDHitCreationAlgorithm::TrajectorySample::GetPosition()
    const
  {
    return m_position;
  }
  ...
  inline double ThreeDHitCreationAlgorithm::TrajectorySample::GetSigma() const { return m_sigma; }

  //------------------------------------------------------------------------------------------------------------------------------------------
  //------------------------------------------------------------------------------------------------------------------------------------------

  inline ThreeDHitCreationAlgorithm::ProtoHit::ProtoHit(
    const pandora::CaloHit* const pParentCaloHit2D)
  ...
  inline void ThreeDHitCreationAlgorithm::ProtoHit::SetPosition3D(
  ...
  inline void ThreeDHitCreationAlgorithm::ProtoHit::AddTrajectorySample(
    const TrajectorySample& trajectorySample)
  {
