...
namespace lar_content {
  ...
  void ThreeViewShowerHitsTool::GetShowerHit3D(const CaloHitVector& caloHitVector1,
                                               ...
                                               ProtoHit& protoHit) const
  {
    ...
    for (const CaloHit* const pCaloHit1 : caloHitVector1) {
      ...
      const float prediction(LArGeometryHelper::MergeTwoPositions(
        this->GetPandora(), hitType2D, hitType1, position2D, position1.GetZ()));

      for (const CaloHit* const pCaloHit2 : caloHitVector2) {
        const CartesianVector& position2(pCaloHit2->GetPositionVector());

        if (std::fabs(position2.GetZ() - prediction) > m_zTolerance) continue;
  ...
  StatusCode ThreeViewShowerHitsTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "ZTolerance", m_zTolerance));
