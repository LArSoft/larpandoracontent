...
namespace lar_content {
  ...
  void MultiValuedTransverseTrackHitsTool::GetTransverseTrackHit3D(
    ...
    ProtoHit& protoHit) const
  {
    ...
    if (matchedSlidingFitMap.end() != iter1) {
      ...
      PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                     ...
                                     !=,
                                     fitResult1.GetGlobalFitPositionListAtX(
                                       pCaloHit2D->GetPositionVector().GetX(), fitPositionList1));
    }
    ...
    if (matchedSlidingFitMap.end() != iter2) {
      ...
      PANDORA_THROW_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                     STATUS_CODE_NOT_FOUND,
                                     !=,
                                     fitResult2.GetGlobalFitPositionListAtX(
                                       pCaloHit2D->GetPositionVector().GetX(), fitPositionList2));
    }
    ...
    if (nViews <= 2) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
