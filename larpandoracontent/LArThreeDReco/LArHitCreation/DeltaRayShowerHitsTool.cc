...
namespace lar_content {
  ...
  void DeltaRayShowerHitsTool::Run(ThreeDHitCreationAlgorithm* const pAlgorithm,
                                   ...
                                   ProtoHitVector& protoHitVector)
  {
    ...
    try {
      if (!LArPfoHelper::IsShower(pPfo) || (1 != pPfo->GetParentPfoList().size())) return;
      ...
      if (parentHitList3D.empty()) return;

      CaloHitVector parentHitVector3D(parentHitList3D.begin(), parentHitList3D.end());
      std::sort(
        parentHitVector3D.begin(), parentHitVector3D.end(), LArClusterHelper::SortHitsByPosition);
  ...
  void DeltaRayShowerHitsTool::CreateDeltaRayShowerHits3D(const CaloHitVector& inputTwoDHits,
    ...
    for (const CaloHit* const pCaloHit2D : inputTwoDHits) {
      try {
        ...
        for (const CaloHit* const pCaloHit3D : parentHits3D) {
          ...
          const CartesianVector thisPosition2D(
            LArGeometryHelper::ProjectPosition(this->GetPandora(), thisPosition3D, hitType));
          const float thisDistanceSquared(
            (pCaloHit2D->GetPositionVector() - thisPosition2D).GetMagnitudeSquared());
        ...
        if (!foundClosestPosition) continue;

        const CartesianVector position1(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), closestPosition3D, hitType1));
        const CartesianVector position2(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), closestPosition3D, hitType2));

        ProtoHit protoHit(pCaloHit2D);
        this->GetBestPosition3D(hitType1, hitType2, position1, position2, protoHit);

        if (protoHit.IsPositionSet()) protoHitVector.push_back(protoHit);
      }
      catch (StatusCodeException&) {
      }
    }
