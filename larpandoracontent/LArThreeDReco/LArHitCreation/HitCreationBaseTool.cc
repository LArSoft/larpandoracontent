...
namespace lar_content {
  ...
  void HitCreationBaseTool::GetBestPosition3D(const HitType hitType1,
                                              const HitType hitType2,
                                              const CartesianPointVector& fitPositionList1,
                                              const CartesianPointVector& fitPositionList2,
                                              ProtoHit& protoHit) const
  {
    if (fitPositionList1.empty() && fitPositionList2.empty()) {
      ...
      if (fitPositionList2.size() != 1) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

      this->GetBestPosition3D(hitType2, fitPositionList2.front(), protoHit);
    }
    else if (fitPositionList2.empty()) {
      if (fitPositionList1.size() != 1) throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);
  ...
  void HitCreationBaseTool::GetBestPosition3D(const HitType hitType1,
                                              const HitType hitType2,
                                              const CartesianVector& fitPosition1,
                                              const CartesianVector& fitPosition2,
                                              ProtoHit& protoHit) const
  {
    ...
    this->GetPandora().GetPlugins()->GetLArTransformationPlugin()->GetMinChiSquaredYZ(
      u, v, w, sigmaU, sigmaV, sigmaW, bestY, bestZ, chi2);
    position3D.SetValues(
      pCaloHit2D->GetPositionVector().GetX(), static_cast<float>(bestY), static_cast<float>(bestZ));
  ...
  void HitCreationBaseTool::GetBestPosition3D(const HitType hitType,
                                              const CartesianVector& fitPosition,
                                              ProtoHit& protoHit) const
  {
    // TODO Input better uncertainties into this method (sigmaHit, sigmaFit, sigmaX)
    const CaloHit* const pCaloHit2D(protoHit.GetParentCaloHit2D());
    ...
    LArGeometryHelper::MergeTwoPositions3D(this->GetPandora(),
                                           pCaloHit2D->GetHitType(),
                                           hitType,
                                           pCaloHit2D->GetPositionVector(),
                                           fitPosition,
                                           position3D,
                                           chi2);
  ...
  StatusCode HitCreationBaseTool::ReadSettings(const pandora::TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "SigmaX", sigmaX));
