...
namespace lar_content {
  ...
  void TransverseTrackHitsBaseTool::GetTrackHits3D(const CaloHitVector& inputTwoDHits,
  ...
  void TransverseTrackHitsBaseTool::AddTransverseChi2(
    const MatchedSlidingFitMap& matchedSlidingFitMap,
    ProtoHit& protoHit) const
  {
    ...
    for (const MatchedSlidingFitMap::value_type& mapEntry : matchedSlidingFitMap) {
      if (mapEntry.first == inputHitType) continue;

      const CartesianVector inputPosition2D(
        LArGeometryHelper::ProjectPosition(this->GetPandora(), inputPosition3D, mapEntry.first));
  ...
  double TransverseTrackHitsBaseTool::GetTransverseChi2(const CartesianVector& position2D,
                                                        const TwoDSlidingFitResult& fitResult) const
  {
    ...
    const float minLayerDistanceSquared(
      (position2D - fitResult.GetGlobalMinLayerPosition()).GetMagnitudeSquared());
    const float maxLayerDistanceSquared(
      (position2D - fitResult.GetGlobalMaxLayerPosition()).GetMagnitudeSquared());
