...
namespace lar_content {
  ...
  void LongitudinalTrackHitsBaseTool::GetTrackHits3D(const CaloHitVector& inputTwoDHits,
  ...
  void LongitudinalTrackHitsBaseTool::GetVertexAndEndPositions(
    const MatchedSlidingFitMap& inputSlidingFitMap,
    MatchedSlidingFitMap& outputSlidingFitMap,
    CartesianVector& bestVtx3D,
    CartesianVector& bestEnd3D) const
  {
    ...
    for (unsigned int iPermutation = 0; iPermutation < 4; ++iPermutation) {
      ...
      if (foundU) {
        ...
        vtxU = (isForwardU ? slidingFitResultU.GetGlobalMinLayerPosition() :
                             slidingFitResultU.GetGlobalMaxLayerPosition());
        endU = (isForwardU ? slidingFitResultU.GetGlobalMaxLayerPosition() :
                             slidingFitResultU.GetGlobalMinLayerPosition());
      }

      if (foundV) {
        const TwoDSlidingFitResult& slidingFitResultV = iterV->second;
        vtxV = (isForwardV ? slidingFitResultV.GetGlobalMinLayerPosition() :
                             slidingFitResultV.GetGlobalMaxLayerPosition());
        endV = (isForwardV ? slidingFitResultV.GetGlobalMaxLayerPosition() :
                             slidingFitResultV.GetGlobalMinLayerPosition());
      }

      if (foundW) {
        const TwoDSlidingFitResult& slidingFitResultW = iterW->second;
        vtxW = (isForwardW ? slidingFitResultW.GetGlobalMinLayerPosition() :
                             slidingFitResultW.GetGlobalMaxLayerPosition());
        endW = (isForwardW ? slidingFitResultW.GetGlobalMaxLayerPosition() :
                             slidingFitResultW.GetGlobalMinLayerPosition());
      }
      ...
      if (foundU) {
        const CartesianVector projVtxU(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), vtx3D, TPC_VIEW_U));
        const CartesianVector projEndU(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), end3D, TPC_VIEW_U));

        if ((endU - vtxU).GetMagnitudeSquared() > m_minTrackLengthSquared &&
            (projVtxU - vtxU).GetMagnitudeSquared() <
              std::min(m_vtxDisplacementCutSquared, (projVtxU - endU).GetMagnitudeSquared()) &&
            (projEndU - endU).GetMagnitudeSquared() <
              std::min(m_vtxDisplacementCutSquared, (projEndU - vtxU).GetMagnitudeSquared())) {
          matchedU = true;
          ++matchedViews;
        }
      }
      ...
      if (foundV) {
        const CartesianVector projVtxV(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), vtx3D, TPC_VIEW_V));
        const CartesianVector projEndV(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), end3D, TPC_VIEW_V));

        if ((endV - vtxV).GetMagnitudeSquared() > m_minTrackLengthSquared &&
            (projVtxV - vtxV).GetMagnitudeSquared() <
              std::min(m_vtxDisplacementCutSquared, (projVtxV - endV).GetMagnitudeSquared()) &&
            (projEndV - endV).GetMagnitudeSquared() <
              std::min(m_vtxDisplacementCutSquared, (projEndV - vtxV).GetMagnitudeSquared())) {
          matchedV = true;
          ++matchedViews;
        }
      }
      ...
      if (foundW) {
        const CartesianVector projVtxW(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), vtx3D, TPC_VIEW_W));
        const CartesianVector projEndW(
          LArGeometryHelper::ProjectPosition(this->GetPandora(), end3D, TPC_VIEW_W));

        if ((endW - vtxW).GetMagnitudeSquared() > m_minTrackLengthSquared &&
            (projVtxW - vtxW).GetMagnitudeSquared() <
              std::min(m_vtxDisplacementCutSquared, (projVtxW - endW).GetMagnitudeSquared()) &&
            (projEndW - endW).GetMagnitudeSquared() <
              std::min(m_vtxDisplacementCutSquared, (projEndW - vtxW).GetMagnitudeSquared())) {
          matchedW = true;
          ++matchedViews;
        }
      }

      if (matchedViews < 2) continue;
    ...
    if (outputSlidingFitMap.empty()) throw StatusCodeException(STATUS_CODE_NOT_FOUND);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void LongitudinalTrackHitsBaseTool::UpdateBestPosition(const HitType hitType1,
                                                         const HitType hitType2,
                                                         const CartesianVector& vtx1,
                                                         const CartesianVector& vtx2,
                                                         CartesianVector& bestVtx,
                                                         float& bestChi2) const
  {
    ...
    LArGeometryHelper::MergeTwoPositions3D(
      this->GetPandora(), hitType1, hitType2, vtx1, vtx2, mergedVtx, mergedChi2);
  ...
  StatusCode LongitudinalTrackHitsBaseTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexDisplacementCut", vtxDisplacementCut));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinTrackLength", minTrackLength));
