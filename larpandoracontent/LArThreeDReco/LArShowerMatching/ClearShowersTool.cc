...
namespace lar_content {
  ...
  bool ClearShowersTool::HasLargeDirectConnections(IteratorList::const_iterator iIter,
                                                   const IteratorList& iteratorList)
  {
    for (IteratorList::const_iterator iIter2 = iteratorList.begin(), iIter2End = iteratorList.end();
         iIter2 != iIter2End;
         ++iIter2) {
      if (iIter == iIter2) continue;

      if (((*iIter)->GetClusterU() == (*iIter2)->GetClusterU()) ||
          ((*iIter)->GetClusterV() == (*iIter2)->GetClusterV()) ||
  ...
  bool ClearShowersTool::IsLargerThanDirectConnections(
    IteratorList::const_iterator iIter,
    ...
    const ClusterSet& usedClusters)
  {
    const unsigned int nMatchedSamplingPoints(
      (*iIter)->GetOverlapResult().GetNMatchedSamplingPoints());
    ...
    for (TensorType::ElementList::const_iterator eIter = elementList.begin();
         ...
         ++eIter) {
      if ((*iIter) == eIter) continue;

      if (usedClusters.count(eIter->GetClusterU()) || usedClusters.count(eIter->GetClusterV()) ||
          usedClusters.count(eIter->GetClusterW()))
        continue;

      if (((*iIter)->GetClusterU() != eIter->GetClusterU()) &&
          ((*iIter)->GetClusterV() != eIter->GetClusterV()) &&
          ((*iIter)->GetClusterW() != eIter->GetClusterW()))
        continue;

      if (nMatchedSamplingPoints <
          minMatchedSamplingPointRatio * eIter->GetOverlapResult().GetNMatchedSamplingPoints())
        return false;

      if (xOverlapSpan <
          minXOverlapSpanRatio * eIter->GetOverlapResult().GetXOverlap().GetXOverlapSpan())
        return false;
    }
  ...
  void ClearShowersTool::FindClearShowers(const TensorType& overlapTensor,
                                          ProtoParticleVector& protoParticleVector) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      for (IteratorList::const_iterator iIter = iteratorList.begin(), iIterEnd = iteratorList.end();
           iIter != iIterEnd;
           ++iIter) {
        if (ClearShowersTool::HasLargeDirectConnections(iIter, iteratorList)) continue;

        if (!ClearShowersTool::IsLargerThanDirectConnections(iIter,
                                                             elementList,
        ...
        if (m_visualize) {
          ClusterList clusterList{
            (*iIter)->GetClusterU(), (*iIter)->GetClusterV(), (*iIter)->GetClusterW()};
          PANDORA_MONITORING_API(
            VisualizeClusters(this->GetPandora(), &clusterList, "Selected", BLUE));
  ...
  void ClearShowersTool::SelectLargeShowerElements(const TensorType::ElementList& elementList,
                                                   ...
                                                   IteratorList& iteratorList) const
  {
    for (TensorType::ElementList::const_iterator eIter = elementList.begin();
      ...
      if (m_visualize) {
        ...
        PANDORA_MONITORING_API(
          VisualizeClusters(this->GetPandora(), &clusterList, "Considered", RED));
        PANDORA_MONITORING_API(ViewEvent(this->GetPandora()));
      }
      ...
      if ((xOverlap.GetXSpanU() > std::numeric_limits<float>::epsilon()) &&
          (xOverlap.GetXOverlapSpan() / xOverlap.GetXSpanU() > m_minXOverlapFraction) &&
          (xOverlap.GetXSpanV() > std::numeric_limits<float>::epsilon()) &&
          (xOverlap.GetXOverlapSpan() / xOverlap.GetXSpanV() > m_minXOverlapFraction) &&
          (xOverlap.GetXSpanW() > std::numeric_limits<float>::epsilon()) &&
          (xOverlap.GetXOverlapSpan() / xOverlap.GetXSpanW() > m_minXOverlapFraction)) {
        iteratorList.push_back(eIter);
        if (m_visualize) {
          ClusterList clusterList{eIter->GetClusterU(), eIter->GetClusterV(), eIter->GetClusterW()};
          PANDORA_MONITORING_API(
            VisualizeClusters(this->GetPandora(), &clusterList, "Large", GREEN));
          PANDORA_MONITORING_API(ViewEvent(this->GetPandora()));
        }
      }
    }
  ...
  StatusCode ClearShowersTool::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinXOverlapSpanRatio", m_minXOverlapSpanRatio));
