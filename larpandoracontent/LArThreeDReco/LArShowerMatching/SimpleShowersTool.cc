...
namespace lar_content {
  ...
  SimpleShowersTool::SimpleShowersTool()
    : m_minMatchedFraction(0.2f), m_minMatchedSamplingPoints(40), m_minXOverlapFraction(0.5f)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  bool SimpleShowersTool::Run(ThreeViewShowersAlgorithm* const pAlgorithm,
  ...
  void SimpleShowersTool::FindBestShower(const TensorType& overlapTensor,
                                         ProtoParticleVector& protoParticleVector) const
  {
    ...
    for (const Cluster* const pKeyCluster : sortedKeyClusters) {
      ...
      if (elementList.empty()) continue;

      TensorType::Element bestElement(elementList.back());

      if (!bestElement.GetOverlapResult().IsInitialized()) continue;

      if ((NULL == bestElement.GetClusterU()) || (NULL == bestElement.GetClusterV()) ||
          (NULL == bestElement.GetClusterW()))
        continue;
