...
namespace lar_content {
  ...
  StatusCode StreamSelectionAlgorithm::Run()
  {
    ...
    if (m_inputListName.empty()) {
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::GetCurrentListName<Cluster>(*this, originalClusterListName));
    }
    else {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetList(*this, m_inputListName, pClusterList));
    ...
    for (std::string listName : m_listNames)
      PandoraContentApi::SaveList<ClusterList>(
        *this, originalClusterListName, listName, m_clusterListMap.at(listName));
  ...
  StatusCode StreamSelectionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputListName", m_inputListName));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "ListNames", m_listNames));
