...
namespace lar_content {
  ...
  StatusCode ClusterMergingAlgorithm::Run()
  {
    ...
    if (m_inputClusterListName.empty()) {
      ...
      PANDORA_RETURN_RESULT_IF_AND_IF(
        ...
        !=,
        PandoraContentApi::GetList(*this, m_inputClusterListName, pClusterList));
    }

    if (!pClusterList || pClusterList->empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "ClusterMergingAlgorithm: unable to find cluster list "
    ...
    while (true) {
      ...
      if (clusterMergeMap.empty()) break;
  ...
  void ClusterMergingAlgorithm::MergeClusters(ClusterVector& clusterVector,
                                              ClusterMergeMap& clusterMergeMap) const
  {
    ...
    for (const Cluster* const pSeedCluster : clusterVector) {
      ...
      this->CollectAssociatedClusters(
        pSeedCluster, pSeedCluster, clusterMergeMap, clusterVetoList, mergeList);
      ...
      for (const Cluster* const pAssociatedCluster : mergeList) {
        ...
        if (!pAssociatedCluster->IsAvailable()) throw StatusCodeException(STATUS_CODE_FAILURE);
        ...
        if (m_inputClusterListName.empty()) {
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::MergeAndDeleteClusters(*this, pSeedCluster, pAssociatedCluster));
        }
        else {
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
  ...
  void ClusterMergingAlgorithm::CollectAssociatedClusters(const Cluster* const pSeedCluster,
                                                          const ClusterMergeMap& clusterMergeMap,
                                                          ClusterList& associatedClusterList) const
  {
    ClusterSet clusterVetoList;
    this->CollectAssociatedClusters(
      pSeedCluster, pSeedCluster, clusterMergeMap, clusterVetoList, associatedClusterList);
  }
  ...
  void ClusterMergingAlgorithm::CollectAssociatedClusters(const Cluster* const pSeedCluster,
                                                          ...
                                                          const ClusterMergeMap& clusterMergeMap,
                                                          const ClusterSet& clusterVetoList,
                                                          ClusterList& associatedClusterList) const
  {
    if (clusterVetoList.count(pCurrentCluster)) return;

    ClusterMergeMap::const_iterator iter1 = clusterMergeMap.find(pCurrentCluster);

    if (iter1 == clusterMergeMap.end()) return;
    ...
    for (const Cluster* const pAssociatedCluster : associatedClusters) {
      ...
      if (associatedClusterList.end() !=
          std::find(associatedClusterList.begin(), associatedClusterList.end(), pAssociatedCluster))
        continue;

      associatedClusterList.push_back(pAssociatedCluster);
      this->CollectAssociatedClusters(
        pSeedCluster, pAssociatedCluster, clusterMergeMap, clusterVetoList, associatedClusterList);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void ClusterMergingAlgorithm::GetSortedListOfCleanClusters(const ClusterVector& inputClusters,
                                                             ClusterVector& outputClusters) const
  {
    ClusterVector pfoClusters, availableClusters;

    for (ClusterVector::const_iterator iter = inputClusters.begin(), iterEnd = inputClusters.end();
         iter != iterEnd;
         ++iter) {
      const Cluster* const pCluster = *iter;

      if (!pCluster->IsAvailable()) { pfoClusters.push_back(pCluster); }
      else {
