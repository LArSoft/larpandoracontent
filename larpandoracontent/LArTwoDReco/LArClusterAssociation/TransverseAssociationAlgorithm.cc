...
namespace lar_content {
  ...
  void TransverseAssociationAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
  ...
  void TransverseAssociationAlgorithm::PopulateClusterAssociationMap(
    const ClusterVector& allClusters,
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    try {
      ...
      if (nearbyClusters.empty()) return;
      ...
      ClusterVector shortClusters, transverseMediumClusters, longitudinalMediumClusters,
        longClusters;
      this->SortInputClusters(allClusters,
                              shortClusters,
                              transverseMediumClusters,
                              longitudinalMediumClusters,
                              longClusters);

      ClusterVector transverseClusters(shortClusters.begin(), shortClusters.end());
      transverseClusters.insert(
        transverseClusters.end(), transverseMediumClusters.begin(), transverseMediumClusters.end());

      ClusterVector establishedClusters(transverseMediumClusters.begin(),
                                        transverseMediumClusters.end());
      establishedClusters.insert(establishedClusters.end(),
                                 longitudinalMediumClusters.begin(),
                                 longitudinalMediumClusters.end());
      establishedClusters.insert(
        establishedClusters.end(), longClusters.begin(), longClusters.end());
      ...
      this->FillReducedAssociationMap(
        nearbyClusters, shortClusters, establishedClusters, firstAssociationMap);
      ...
      this->FillTransverseClusterList(
        nearbyClusters, transverseClusters, firstAssociationMap, transverseClusterList);
      ...
      this->FillReducedAssociationMap(
        nearbyClusters, transverseMediumClusters, longClusters, secondAssociationMap);
      this->FillReducedAssociationMap(
        nearbyClusters, transverseClusters, allClusters, secondAssociationMap);
      ...
      this->FillTransverseAssociationMap(
        nearbyClusters, transverseClusterList, secondAssociationMap, transverseAssociationMap);
      ...
      std::cout << "TransverseAssociationAlgorithm: exception " << statusCodeException.ToString()
                << std::endl;
    }

    for (TransverseClusterList::const_iterator iter = transverseClusterList.begin(),
                                               iterEnd = transverseClusterList.end();
         iter != iterEnd;
         ++iter) {
      delete *iter;
    }
  ...
  void TransverseAssociationAlgorithm::GetNearbyClusterMap(
    const ClusterVector& allClusters,
    ClusterToClustersMap& nearbyClusters) const
  {
    ...
    for (const Cluster* const pCluster : allClusters) {
      ...
      for (const CaloHit* const pCaloHit : daughterHits) {
        KDTreeBox searchRegionHits(
          build_2d_kd_search_region(pCaloHit, m_searchRegionX, m_searchRegionZ));
  ...
  void TransverseAssociationAlgorithm::SortInputClusters(const ClusterVector& inputVector,
                                                         ClusterVector& shortVector,
                                                         ClusterVector& transverseMediumVector,
                                                         ClusterVector& longitudinalMediumVector,
                                                         ClusterVector& longVector) const
  {
    for (ClusterVector::const_iterator iter = inputVector.begin(), iterEnd = inputVector.end();
         iter != iterEnd;
         ++iter) {
      ...
      const float clusterLengthSquared(clusterLengthT * clusterLengthT +
                                       clusterLengthL * clusterLengthL);
    ...
    std::sort(
      transverseMediumVector.begin(), transverseMediumVector.end(), LArClusterHelper::SortByNHits);
    std::sort(longitudinalMediumVector.begin(),
              longitudinalMediumVector.end(),
              LArClusterHelper::SortByNHits);
  ...
  void TransverseAssociationAlgorithm::FillReducedAssociationMap(
    ...
    const ClusterVector& firstVector,
    const ClusterVector& secondVector,
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    this->FillAssociationMap(
      nearbyClusters, firstVector, firstVector, firstAssociationMap, firstAssociationMapSwapped);
    this->FillAssociationMap(
      nearbyClusters, firstVector, secondVector, secondAssociationMap, secondAssociationMapSwapped);
    this->FillReducedAssociationMap(firstAssociationMap,
                                    secondAssociationMap,
                                    secondAssociationMapSwapped,
                                    clusterAssociationMap);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void TransverseAssociationAlgorithm::FillAssociationMap(
    const ClusterToClustersMap& nearbyClusters,
    const ClusterVector& firstVector,
    const ClusterVector& secondVector,
    ClusterAssociationMap& firstAssociationMap,
  ...
  {
    for (ClusterVector::const_iterator iterI = firstVector.begin(), iterEndI = firstVector.end();
         iterI != iterEndI;
         ++iterI) {
      const Cluster* const pClusterI = *iterI;

      for (ClusterVector::const_iterator iterJ = secondVector.begin(),
                                         iterEndJ = secondVector.end();
           iterJ != iterEndJ;
           ++iterJ) {
  ...
  void TransverseAssociationAlgorithm::FillTransverseClusterList(
    const ClusterToClustersMap& nearbyClusters,
    const ClusterVector& inputVector,
    ...
    TransverseClusterList& transverseClusterList) const
  {
    for (ClusterVector::const_iterator iter = inputVector.begin(), iterEnd = inputVector.end();
      ...
      this->GetAssociatedClusters(
        nearbyClusters, pCluster, inputAssociationMap, associatedClusters);
  ...
  void TransverseAssociationAlgorithm::FillTransverseAssociationMap(
    const ClusterToClustersMap& nearbyClusters,
    const TransverseClusterList& transverseClusterList,
    const ClusterAssociationMap& transverseAssociationMap,
    ClusterAssociationMap& clusterAssociationMap) const
  {
    for (TransverseClusterList::const_iterator iter1 = transverseClusterList.begin(),
                                               iterEnd1 = transverseClusterList.end();
         iter1 != iterEnd1;
         ++iter1) {
      ...
      ClusterAssociationMap::const_iterator iterInner =
        transverseAssociationMap.find(pInnerCluster);
      if (transverseAssociationMap.end() == iterInner) continue;

      for (TransverseClusterList::const_iterator iter2 = transverseClusterList.begin(),
                                                 iterEnd2 = transverseClusterList.end();
           iter2 != iterEnd2;
           ++iter2) {
        ...
        ClusterAssociationMap::const_iterator iterOuter =
          transverseAssociationMap.find(pOuterCluster);
        if (transverseAssociationMap.end() == iterOuter) continue;

        if (pInnerCluster == pOuterCluster) continue;
        ...
        if (!this->IsTransverseAssociated(
              pInnerTransverseCluster, pOuterTransverseCluster, nearbyClusters))
          continue;
  ...
  void TransverseAssociationAlgorithm::GetAssociatedClusters(
    const ClusterToClustersMap& nearbyClusters,
    const Cluster* const pClusterI,
    const ClusterAssociationMap& associationMap,
    ClusterVector& associatedVector) const
  {
    ClusterAssociationMap::const_iterator iterI = associationMap.find(pClusterI);
    if (associationMap.end() == iterI) return;

    for (ClusterSet::const_iterator iterJ = iterI->second.m_forwardAssociations.begin(),
                                    iterEndJ = iterI->second.m_forwardAssociations.end();
         iterJ != iterEndJ;
         ++iterJ) {
      const Cluster* const pClusterJ = *iterJ;
    ...
    for (ClusterSet::const_iterator iterJ = iterI->second.m_backwardAssociations.begin(),
                                    iterEndJ = iterI->second.m_backwardAssociations.end();
         iterJ != iterEndJ;
         ++iterJ) {
      const Cluster* const pClusterJ = *iterJ;
  ...
  bool TransverseAssociationAlgorithm::IsAssociated(
    const bool isForward,
    const Cluster* const pFirstCluster,
    const Cluster* const pSecondCluster,
    const ClusterToClustersMap& nearbyClusters) const
  {
    if ((0 == nearbyClusters.at(pFirstCluster).count(pSecondCluster)) ||
        (0 == nearbyClusters.at(pSecondCluster).count(pFirstCluster)))
      return false;
    ...
    if ((firstCoordinate.GetZ() >
         std::max(secondInner.GetZ(), secondOuter.GetZ()) + m_maxLongitudinalOverlap) ||
        (firstCoordinate.GetZ() <
         std::min(secondInner.GetZ(), secondOuter.GetZ()) - m_maxLongitudinalOverlap))
      return false;
    ...
    const CartesianVector firstProjection(
      LArClusterHelper::GetClosestPosition(firstCoordinate, pSecondCluster));
  ...
  bool TransverseAssociationAlgorithm::IsTransverseAssociated(
    const Cluster* const pInnerCluster,
    const Cluster* const pOuterCluster,
    const ClusterToClustersMap& nearbyClusters) const
  {
    if ((0 == nearbyClusters.at(pInnerCluster).count(pOuterCluster)) ||
        (0 == nearbyClusters.at(pOuterCluster).count(pInnerCluster)))
      return false;
    ...
    if ((std::fabs(innerCentroid.GetZ() - outerInner.GetZ()) >
         std::fabs(innerCentroid.GetX() - outerInner.GetX()) * std::fabs(m_clusterTanAngle)) &&
        (std::fabs(innerCentroid.GetZ() - outerOuter.GetZ()) >
         std::fabs(innerCentroid.GetX() - outerOuter.GetX()) * std::fabs(m_clusterTanAngle)))
      return false;

    if ((std::fabs(outerCentroid.GetZ() - innerInner.GetZ()) >
         std::fabs(outerCentroid.GetX() - innerInner.GetX()) * std::fabs(m_clusterTanAngle)) &&
        (std::fabs(outerCentroid.GetZ() - innerOuter.GetZ()) >
         std::fabs(outerCentroid.GetX() - innerOuter.GetX()) * std::fabs(m_clusterTanAngle)))
      return false;

    const CartesianVector innerProjection(
      LArClusterHelper::GetClosestPosition(outerInner, pInnerCluster));
    const CartesianVector outerProjection(
      LArClusterHelper::GetClosestPosition(innerOuter, pOuterCluster));
  ...
  bool TransverseAssociationAlgorithm::IsTransverseAssociated(
    const LArTransverseCluster* const pInnerTransverseCluster,
    const LArTransverseCluster* const pOuterTransverseCluster,
    const ClusterToClustersMap& nearbyClusters) const
  {
    if (pInnerTransverseCluster->GetDirection().GetDotProduct(
          pOuterTransverseCluster->GetDirection()) < m_transverseClusterMinCosTheta)
      return false;

    if (!this->IsTransverseAssociated(pInnerTransverseCluster,
                                      pOuterTransverseCluster->GetInnerVertex()))
      return false;

    if (!this->IsTransverseAssociated(pOuterTransverseCluster,
                                      pInnerTransverseCluster->GetOuterVertex()))
      return false;

    if (!this->IsTransverseAssociated(pInnerTransverseCluster->GetSeedCluster(),
                                      pOuterTransverseCluster->GetSeedCluster(),
                                      nearbyClusters))
      return false;

    if (this->IsOverlapping(pInnerTransverseCluster->GetSeedCluster(),
                            pOuterTransverseCluster->GetSeedCluster()))
      return false;
  ...
  bool TransverseAssociationAlgorithm::IsTransverseAssociated(
    const LArTransverseCluster* const pTransverseCluster,
    const CartesianVector& testVertex) const
  {
    ...
    if (direction.GetCrossProduct(testVertex - innerVertex).GetMagnitudeSquared() >
        m_transverseClusterMaxDisplacement * m_transverseClusterMaxDisplacement)
      return false;
  ...
  bool TransverseAssociationAlgorithm::IsOverlapping(const Cluster* const pInnerCluster,
                                                     const Cluster* const pOuterCluster) const
  {
    ...
    return (std::max(innerOverlapSquared, outerOverlapSquared) >
            m_maxProjectedOverlap * m_maxProjectedOverlap);
  }
  ...
  float TransverseAssociationAlgorithm::GetTransverseSpan(
    const Cluster* const pCentralCluster,
    const ClusterVector& associatedClusters) const
  {
    ...
    for (ClusterVector::const_iterator iter = associatedClusters.begin(),
                                       iterEnd = associatedClusters.end();
         iter != iterEnd;
         ++iter) {
      ...
      if (localMinX < overallMinX) overallMinX = localMinX;

      if (localMaxX > overallMaxX) overallMaxX = localMaxX;
    }
  ...
  bool TransverseAssociationAlgorithm::IsExtremalCluster(const bool isForward,
                                                         const Cluster* const pCurrentCluster,
                                                         const Cluster* const pTestCluster) const
  {
  ...
  void TransverseAssociationAlgorithm::GetExtremalCoordinatesX(const Cluster* const pCluster,
                                                               float& minX,
                                                               float& maxX) const
  {
  ...
  void TransverseAssociationAlgorithm::GetExtremalCoordinatesZ(const Cluster* const pCluster,
                                                               float& minZ,
                                                               float& maxZ) const
  {
  ...
  void TransverseAssociationAlgorithm::GetExtremalCoordinatesXZ(const Cluster* const pCluster,
                                                                const bool useX,
                                                                float& minXZ,
                                                                float& maxXZ) const
  {
    ...
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
      ...
      for (CaloHitList::const_iterator hitIter = iter->second->begin(),
           ...
           hitIter != hitIterEnd;
           ++hitIter) {
        const float caloHitXZ(useX ? (*hitIter)->GetPositionVector().GetX() :
                                     (*hitIter)->GetPositionVector().GetZ());

        if (caloHitXZ < minXZ) minXZ = caloHitXZ;

        if (caloHitXZ > maxXZ) maxXZ = caloHitXZ;
      }
    }

    if (maxXZ < minXZ) throw pandora::StatusCodeException(STATUS_CODE_FAILURE);
  }
  ...
  void TransverseAssociationAlgorithm::GetExtremalCoordinatesX(
    ...
    CartesianVector& innerCoordinate,
    CartesianVector& outerCoordinate) const
  {
    ...
    innerCoordinate =
      (firstCoordinate.GetX() < secondCoordinate.GetX() ? firstCoordinate : secondCoordinate);
    outerCoordinate =
      (firstCoordinate.GetX() > secondCoordinate.GetX() ? firstCoordinate : secondCoordinate);
  }
  ...
  void TransverseAssociationAlgorithm::FillReducedAssociationMap(
    const ClusterAssociationMap& inputAssociationMap,
    ClusterAssociationMap& outputAssociationMap) const
  {
    return this->FillReducedAssociationMap(
      inputAssociationMap, inputAssociationMap, inputAssociationMap, outputAssociationMap);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void TransverseAssociationAlgorithm::FillReducedAssociationMap(
    const ClusterAssociationMap& firstAssociationMap,
    ...
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    for (const Cluster* const pCluster : sortedClusters) {
      ...
      ClusterVector sortedOuterClusters(firstAssociation.m_forwardAssociations.begin(),
                                        firstAssociation.m_forwardAssociations.end());
      std::sort(
        sortedOuterClusters.begin(), sortedOuterClusters.end(), LArClusterHelper::SortByNHits);

      ClusterVector sortedInnerClusters(firstAssociation.m_backwardAssociations.begin(),
                                        firstAssociation.m_backwardAssociations.end());
      std::sort(
        sortedInnerClusters.begin(), sortedInnerClusters.end(), LArClusterHelper::SortByNHits);
      ...
      if (secondAssociationMap.end() != iterSecond) {
        sortedMiddleClustersF.insert(sortedMiddleClustersF.end(),
                                     iterSecond->second.m_forwardAssociations.begin(),
                                     iterSecond->second.m_forwardAssociations.end());
        sortedMiddleClustersB.insert(sortedMiddleClustersB.end(),
                                     iterSecond->second.m_backwardAssociations.begin(),
                                     iterSecond->second.m_backwardAssociations.end());
        std::sort(sortedMiddleClustersF.begin(),
                  sortedMiddleClustersF.end(),
                  LArClusterHelper::SortByNHits);
        std::sort(sortedMiddleClustersB.begin(),
                  sortedMiddleClustersB.end(),
                  LArClusterHelper::SortByNHits);
      }
      ...
      for (const Cluster* const pOuterCluster : sortedOuterClusters) {
        ...
        for (const Cluster* const pMiddleCluster : sortedMiddleClustersF) {
          ClusterAssociationMap::const_iterator iterSecondCheck =
            secondAssociationMapSwapped.find(pMiddleCluster);
          if (secondAssociationMapSwapped.end() == iterSecondCheck) continue;
    ...
    ClusterAssociationMap& outputAssociationMap) const
  {
    ...
    for (const Cluster* const pCluster : sortedClusters) {
      ...
      ClusterVector sortedForwardClusters(inputAssociation.m_forwardAssociations.begin(),
                                          inputAssociation.m_forwardAssociations.end());
      std::sort(
        sortedForwardClusters.begin(), sortedForwardClusters.end(), LArClusterHelper::SortByNHits);

      ClusterVector sortedBackwardClusters(inputAssociation.m_backwardAssociations.begin(),
                                           inputAssociation.m_backwardAssociations.end());
      std::sort(sortedBackwardClusters.begin(),
                sortedBackwardClusters.end(),
                LArClusterHelper::SortByNHits);
      ...
      for (const Cluster* const pForwardCluster : sortedForwardClusters) {
        ...
        if (inputAssociation.m_backwardAssociations.count(pForwardCluster)) --nCounter;
        ...
        if (inputAssociationMap.end() != iterCheck) {
          if (iterCheck->second.m_forwardAssociations.count(pCluster)) --nCounter;

          if (iterCheck->second.m_backwardAssociations.count(pCluster)) ++nCounter;
        }
      ...
      for (const Cluster* const pBackwardCluster : sortedBackwardClusters) {
        ...
        if (inputAssociation.m_forwardAssociations.count(pBackwardCluster)) ++nCounter;

        ClusterAssociationMap::const_iterator iterCheck =
          inputAssociationMap.find(pBackwardCluster);
        if (inputAssociationMap.end() != iterCheck) {
          if (iterCheck->second.m_backwardAssociations.count(pCluster)) ++nCounter;
  ...
  TransverseAssociationAlgorithm::LArTransverseCluster::LArTransverseCluster(
    const Cluster* const pSeedCluster,
    const ClusterVector& associatedClusters)
    ...
    , m_direction(0.f, 0.f, 0.f)
  {
    ...
    for (ClusterList::const_iterator iterI = clusterList.begin(), iterEndI = clusterList.end();
         iterI != iterEndI;
         ++iterI) {
      for (OrderedCaloHitList::const_iterator iterJ = (*iterI)->GetOrderedCaloHitList().begin(),
           ...
           iterJ != iterEndJ;
           ++iterJ) {
        for (CaloHitList::const_iterator iterK = iterJ->second->begin(),
                                         iterEndK = iterJ->second->end();
             iterK != iterEndK;
             ++iterK) {
    ...
    if (Sw > 0.f) {
      ...
      if (Sw * Swxx - Swx * Swx > 0.) {
        ...
        m_innerVertex.SetValues(
          static_cast<float>(minX), 0.f, static_cast<float>(averageZ + m * (minX - averageX)));
        m_outerVertex.SetValues(
          static_cast<float>(maxX), 0.f, static_cast<float>(averageZ + m * (maxX - averageX)));
  ...
  StatusCode TransverseAssociationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "FirstLengthCut", m_firstLengthCut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SecondLengthCut", m_secondLengthCut));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ClusterWindow", m_clusterWindow));

    const StatusCode angleStatusCode(
      XmlHelper::ReadValue(xmlHandle, "clusterAngle", m_clusterAngle));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxTransverseOverlap", m_maxTransverseOverlap));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxProjectedOverlap", m_maxProjectedOverlap));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxLongitudinalOverlap", m_maxLongitudinalOverlap));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "TransverseClusterMinCosTheta",
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "TransverseClusterMaxDisplacement",
