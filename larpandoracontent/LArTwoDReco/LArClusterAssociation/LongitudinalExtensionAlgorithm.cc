...
namespace lar_content {
  ...
  void LongitudinalExtensionAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
                                                              ClusterVector& clusterVector) const
  {
    for (ClusterList::const_iterator iter = pClusterList->begin(), iterEnd = pClusterList->end();
      ...
      if (LArClusterHelper::GetLayerOccupancy(pCluster) < m_clusterMinLayerOccupancy) continue;
    ...
    ClusterAssociationMatrix& clusterAssociationMatrix) const
  {
    ...
    for (LArPointingClusterList::const_iterator iterI = pointingClusterList.begin(),
                                                iterEndI = pointingClusterList.end();
         iterI != iterEndI;
         ++iterI) {
      const LArPointingCluster& clusterI = *iterI;

      for (LArPointingClusterList::const_iterator iterJ = iterI,
                                                  iterEndJ = pointingClusterList.end();
           iterJ != iterEndJ;
           ++iterJ) {
  ...
  void LongitudinalExtensionAlgorithm::FillClusterAssociationMatrix(
    const LArPointingCluster& clusterI,
    ...
    ClusterAssociationMatrix& clusterAssociationMatrix) const
  {
    ...
    try {
      LArPointingClusterHelper::GetClosestVertices(
        clusterI, clusterJ, targetVertexI, targetVertexJ);
    }
    catch (StatusCodeException&) {
      return;
    }
    ...
    if (distanceSquared >
        m_emissionMaxLongitudinalDisplacement * m_emissionMaxLongitudinalDisplacement)
      return;

    // Check that vertices have a reasonable linear fit
    if (targetVertexI.GetRms() > 1.f || targetVertexJ.GetRms() > 1.f) return;
    ...
    if (distanceSquared < 2.f * m_nodeMaxDisplacement * m_nodeMaxDisplacement) {
      ...
      if (distanceSquared < m_nodeMaxDisplacement * m_nodeMaxDisplacement) {
        ...
        if (cosTheta > m_nodeMaxCosRelativeAngle) { associationType = ClusterAssociation::STRONG; }
      }
    }
    ...
    if (associationType < ClusterAssociation::STRONG) {
      ...
      const float cosThetaI(
        (vertexPositionI - vertexPositionJ).GetUnitVector().GetDotProduct(vertexDirectionI));
      const float cosThetaJ(
        (vertexPositionJ - vertexPositionI).GetUnitVector().GetDotProduct(vertexDirectionJ));

      float rT1(0.f), rL1(0.f), rT2(0.f), rL2(0.f);
      LArPointingClusterHelper::GetImpactParameters(
        vertexPositionI, vertexDirectionI, vertexPositionJ, rL1, rT1);
      LArPointingClusterHelper::GetImpactParameters(
        vertexPositionJ, vertexDirectionJ, vertexPositionI, rL2, rT2);

      if ((rL1 > -2.5f &&
           rL1 < std::min(0.66f * clusterLengthJ, m_emissionMaxLongitudinalDisplacement)) &&
          (rL2 > -2.5f &&
           rL2 < std::min(0.66f * clusterLengthI, m_emissionMaxLongitudinalDisplacement)) &&
          (rT1 < m_emissionMaxTransverseDisplacement) &&
          (rT2 < m_emissionMaxTransverseDisplacement) &&
          (targetVertexI.GetRms() < 0.5f && targetVertexJ.GetRms() < 0.5f) &&
          (cosTheta > m_emissionMaxCosRelativeAngle) && (std::fabs(cosThetaI) > 0.25f) &&
          (std::fabs(cosThetaJ) > 0.25f)) {
    ...
    if (ClusterAssociation::NONE != associationType) {
      ...
      (void)clusterAssociationMatrix[pClusterI].insert(ClusterAssociationMap::value_type(
        pClusterJ, ClusterAssociation(vertexTypeI, vertexTypeJ, associationType, clusterLengthJ)));
      (void)clusterAssociationMatrix[pClusterJ].insert(ClusterAssociationMap::value_type(
        pClusterI, ClusterAssociation(vertexTypeJ, vertexTypeI, associationType, clusterLengthI)));
      return;
    }
  }
  ...
  void LongitudinalExtensionAlgorithm::FillClusterMergeMap(
    const ClusterAssociationMatrix& inputAssociationMatrix,
    ClusterMergeMap& clusterMergeMap) const
  {
    ...
    std::sort(
      sortedInputClusters.begin(), sortedInputClusters.end(), LArClusterHelper::SortByNHits);
    ...
    for (const Cluster* const pCluster1 : sortedInputClusters) {
      ...
      for (const Cluster* const pCluster2 : sortedInputClusters) {
        ...
        if (associationMap1.end() == iter12) continue;

        ClusterAssociationMap::const_iterator iter21 = associationMap2.find(pCluster1);
        if (associationMap2.end() == iter21) continue;
        ...
        std::sort(sortedAssociationClusters.begin(),
                  sortedAssociationClusters.end(),
        ...
        for (const Cluster* const pCluster3 : sortedAssociationClusters) {
          ...
          if (associationMap2.end() == iter23) continue;
        ...
        if (isAssociated) {
          (void)clusterAssociationMatrix[pCluster1].insert(
            ClusterAssociationMap::value_type(pCluster2, association12));
          (void)clusterAssociationMatrix[pCluster2].insert(
            ClusterAssociationMap::value_type(pCluster1, association21));
        }
      }
    }
    ...
    for (const Cluster* const pParentCluster : sortedClusters) {
      const ClusterAssociationMap& clusterAssociationMap(
        clusterAssociationMatrix.at(pParentCluster));

      const Cluster* pBestClusterInner(nullptr);
      ClusterAssociation bestAssociationInner(ClusterAssociation::UNDEFINED,
                                              ClusterAssociation::UNDEFINED,
                                              ClusterAssociation::NONE,
                                              0.f);

      const Cluster* pBestClusterOuter(nullptr);
      ClusterAssociation bestAssociationOuter(ClusterAssociation::UNDEFINED,
                                              ClusterAssociation::UNDEFINED,
                                              ClusterAssociation::NONE,
                                              0.f);
      ...
      if (pBestClusterInner)
        (void)intermediateAssociationMatrix[pParentCluster].insert(
          ClusterAssociationMap::value_type(pBestClusterInner, bestAssociationInner));

      if (pBestClusterOuter)
        (void)intermediateAssociationMatrix[pParentCluster].insert(
          ClusterAssociationMap::value_type(pBestClusterOuter, bestAssociationOuter));
    }
    ...
    std::sort(intermediateSortedClusters.begin(),
              intermediateSortedClusters.end(),
              LArClusterHelper::SortByNHits);

    for (const Cluster* const pParentCluster : intermediateSortedClusters) {
      const ClusterAssociationMap& parentAssociationMap(
        intermediateAssociationMatrix.at(pParentCluster));
      ...
      for (const Cluster* const pDaughterCluster : sortedAssociationClusters) {
        const ClusterAssociation& parentToDaughterAssociation(
          parentAssociationMap.at(pDaughterCluster));

        ClusterAssociationMatrix::const_iterator iter5 =
          intermediateAssociationMatrix.find(pDaughterCluster);

        if (intermediateAssociationMatrix.end() == iter5) continue;
  ...
  StatusCode LongitudinalExtensionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "NodeMaxDisplacement", m_nodeMaxDisplacement));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "EmissionMaxLongitudinalDisplacement",
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "EmissionMaxTransverseDisplacement",
