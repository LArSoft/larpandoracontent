...
namespace lar_content {
  ...
  void LongitudinalAssociationAlgorithm::GetListOfCleanClusters(
  ...
  void LongitudinalAssociationAlgorithm::PopulateClusterAssociationMap(
    ...
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    for (ClusterVector::const_iterator iterI = clusterVector.begin(),
      ...
      for (ClusterVector::const_iterator iterJ = iterI, iterJEnd = clusterVector.end();
        ...
        if (!this->AreClustersAssociated(pInnerCluster, pOuterCluster)) continue;
  ...
  bool LongitudinalAssociationAlgorithm::IsExtremalCluster(const bool isForward,
  ...
  bool LongitudinalAssociationAlgorithm::AreClustersAssociated(
    ...
    const Cluster* const pOuterCluster) const
  {
    ...
    if ((pInnerCluster->GetOuterPseudoLayer() > pOuterCluster->GetInnerPseudoLayer() + 1) ||
        (pOuterCluster->GetInnerPseudoLayer() >
         pInnerCluster->GetOuterPseudoLayer() + m_maxGapLayers)) {
      return false;
    }

    if ((2 * pInnerCluster->GetOuterPseudoLayer() <
         pOuterCluster->GetInnerPseudoLayer() + pInnerCluster->GetInnerPseudoLayer()) ||
        (pInnerCluster->GetOuterPseudoLayer() + pOuterCluster->GetOuterPseudoLayer() <
         2 * pOuterCluster->GetInnerPseudoLayer())) {
      return false;
    }

    const CartesianVector innerEndCentroid(
      pInnerCluster->GetCentroid(pInnerCluster->GetOuterPseudoLayer()));
    const CartesianVector outerStartCentroid(
      pOuterCluster->GetCentroid(pOuterCluster->GetInnerPseudoLayer()));
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, ClusterFitHelper::FitEnd(pInnerCluster, m_fitLayers, innerEndFit));
    PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                            !=,
                            ClusterFitHelper::FitStart(pOuterCluster, m_fitLayers, outerStartFit));

    if (this->AreClustersAssociated(
          innerEndCentroid, outerStartCentroid, innerEndFit, outerStartFit))
      return true;
  ...
  bool LongitudinalAssociationAlgorithm::AreClustersAssociated(
    const CartesianVector& innerClusterEnd,
    const CartesianVector& outerClusterStart,
    const ClusterFitResult& innerFit,
    const ClusterFitResult& outerFit) const
  {
    if (!innerFit.IsFitSuccessful() || !outerFit.IsFitSuccessful()) return false;
    ...
    const CartesianVector innerEndFit1(
      innerFit.GetIntercept() +
      innerFit.GetDirection() *
        (innerFit.GetDirection().GetDotProduct(innerClusterEnd - innerFit.GetIntercept())));
    const CartesianVector innerEndFit2(
      outerFit.GetIntercept() +
      outerFit.GetDirection() *
        (outerFit.GetDirection().GetDotProduct(innerClusterEnd - outerFit.GetIntercept())));

    const CartesianVector outerStartFit1(
      outerFit.GetIntercept() +
      outerFit.GetDirection() *
        (outerFit.GetDirection().GetDotProduct(outerClusterStart - outerFit.GetIntercept())));
    const CartesianVector outerStartFit2(
      innerFit.GetIntercept() +
      innerFit.GetDirection() *
        (innerFit.GetDirection().GetDotProduct(outerClusterStart - innerFit.GetIntercept())));
  ...
  StatusCode LongitudinalAssociationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "FitLayers", m_fitLayers));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxGapDistance", maxGapDistance));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "HitSizeZ", m_hitSizeZ));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "HitSizeX", m_hitSizeX));
