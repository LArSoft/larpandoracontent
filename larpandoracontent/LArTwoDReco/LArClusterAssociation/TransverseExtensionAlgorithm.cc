...
namespace lar_content {
  ...
  void TransverseExtensionAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
  ...
  void TransverseExtensionAlgorithm::FillClusterAssociationMatrix(
    ...
    ClusterAssociationMatrix& clusterAssociationMatrix) const
  {
    ...
    for (LArPointingClusterList::const_iterator iter1 = pointingClusterList.begin(),
                                                iterEnd1 = pointingClusterList.end();
         iter1 != iterEnd1;
         ++iter1) {
      ...
      for (ClusterVector::const_iterator iter2 = clusterVector.begin(),
        ...
        if (parentCluster.GetCluster() == pDaughterCluster) continue;

        this->FillClusterAssociationMatrix(
          parentCluster, pDaughterCluster, clusterAssociationMatrix);
      }
    }
  }
    ...
    ClusterAssociationMatrix& clusterAssociationMatrix) const
  {
    ...
    if (pParentCluster == pDaughterCluster) return;

    for (unsigned int useInner = 0; useInner < 2; ++useInner) {
      const LArPointingCluster::Vertex& pointingVertex(
        useInner == 1 ? pointingCluster.GetInnerVertex() : pointingCluster.GetOuterVertex());
      const ClusterAssociation::VertexType vertexType(useInner == 1 ? ClusterAssociation::INNER :
                                                                      ClusterAssociation::OUTER);

      if (pointingVertex.GetRms() > 0.5f) continue;

      const float projectedDisplacement(
        LArClusterHelper::GetClosestDistance(pointingVertex.GetPosition(), pDaughterCluster));

      if (projectedDisplacement > m_maxLongitudinalDisplacement) continue;
      ...
      LArPointingClusterHelper::GetImpactParameters(
        pointingVertex, firstCoordinate, firstL, firstT);
      LArPointingClusterHelper::GetImpactParameters(
        pointingVertex, secondCoordinate, secondL, secondT);
  ...
  void TransverseExtensionAlgorithm::FillClusterMergeMap(
      ...
      for (const Cluster* const pDaughterCluster : sortedLocalDaughterClusters) {
        ...
        if (clusterAssociation.GetAssociation() == ClusterAssociation::WEAK) {
          if (clusterAssociation.GetParent() == ClusterAssociation::INNER &&
              clusterAssociation.GetFigureOfMerit() < maxDisplacementInner)
            maxDisplacementInner = clusterAssociation.GetFigureOfMerit();

          if (clusterAssociation.GetParent() == ClusterAssociation::OUTER &&
              clusterAssociation.GetFigureOfMerit() < maxDisplacementOuter)
    ...
    for (const Cluster* const pDaughterCluster : sortedDaughterClusters) {
      ...
      for (const Cluster* const pCandidateParentCluster : sortedLocalParentClusters) {
        const ClusterAssociation& clusterAssociation(
          parentToAssociationMap.at(pCandidateParentCluster));
      ...
      if (pParentCluster) {
        ...
        if (daughterList.end() ==
            std::find(daughterList.begin(), daughterList.end(), pParentCluster))
