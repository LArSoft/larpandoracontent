...
namespace lar_content {
  ...
  ClusterAssociationAlgorithm::ClusterAssociationAlgorithm()
    : m_mergeMade(false), m_resolveAmbiguousAssociations(true)
  {}
    ...
    while (m_mergeMade) {
      ...
      while (m_mergeMade) {
        ...
        for (const Cluster* const pCluster : clusterVector) {
          ...
          if (pClusterList->end() ==
              std::find(pClusterList->begin(), pClusterList->end(), pCluster))
            continue;
      ...
      if (!m_resolveAmbiguousAssociations) continue;
      ...
      for (const Cluster* const pCluster : clusterVector) {
        ...
        if (clusterAssociationMap.end() == mapIterFwd) continue;

        if (mapIterFwd->second.m_backwardAssociations.empty() &&
            !mapIterFwd->second.m_forwardAssociations.empty())
        ...
        if (clusterAssociationMap.end() == mapIterBwd) continue;

        if (mapIterBwd->second.m_forwardAssociations.empty() &&
            !mapIterBwd->second.m_backwardAssociations.empty())
  ...
  void ClusterAssociationAlgorithm::UnambiguousPropagation(
    ...
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    if (clusterAssociationMap.end() == iterEnlarge) return;

    ClusterSet& clusterSetEnlarge(isForward ? iterEnlarge->second.m_forwardAssociations :
                                              iterEnlarge->second.m_backwardAssociations);

    if (clusterSetEnlarge.size() != 1) return;
    ...
    if (clusterAssociationMap.end() == iterDelete) return;

    ClusterSet& clusterSetDelete(isForward ? iterDelete->second.m_backwardAssociations :
                                             iterDelete->second.m_forwardAssociations);

    if (clusterSetDelete.size() != 1) return;

    this->UpdateForUnambiguousMerge(
      pClusterToEnlarge, pClusterToDelete, isForward, clusterAssociationMap);
  ...
  void ClusterAssociationAlgorithm::AmbiguousPropagation(
    const Cluster* const pCluster,
    const bool isForward,
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    if (clusterAssociationMap.end() == cIter) throw StatusCodeException(STATUS_CODE_FAILURE);

    const Cluster* pExtremalCluster = pCluster;

    ClusterSet firstClusterSet;
    this->NavigateAlongAssociations(
      clusterAssociationMap, pCluster, isForward, pExtremalCluster, firstClusterSet);

    ClusterSet secondClusterSet;
    this->NavigateAlongAssociations(
      clusterAssociationMap, pExtremalCluster, !isForward, pExtremalCluster, secondClusterSet);
    ...
    if (pCluster == pExtremalCluster) {
      for (const Cluster* const pFirstCluster : firstClusterSet) {
        if ((pCluster != pFirstCluster) && (secondClusterSet.count(pFirstCluster)) &&
            (daughterClusterVector.end() == std::find(daughterClusterVector.begin(),
                                                      daughterClusterVector.end(),
                                                      pFirstCluster))) {
    ...
    std::sort(
      daughterClusterVector.begin(), daughterClusterVector.end(), LArClusterHelper::SortByNHits);

    for (ClusterVector::iterator dIter = daughterClusterVector.begin(),
                                 dIterEnd = daughterClusterVector.end();
         dIter != dIterEnd;
         ++dIter) {
      ...
      PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                              !=,
                              PandoraContentApi::MergeAndDeleteClusters(*this, pCluster, *dIter));
      m_mergeMade = true;
      *dIter = NULL;
    }
  ...
  void ClusterAssociationAlgorithm::UpdateForUnambiguousMerge(
    ...
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    ClusterSet& clusterSetToMove(isForwardMerge ? iterDelete->second.m_forwardAssociations :
                                                  iterDelete->second.m_backwardAssociations);
    ClusterSet& clusterSetToReplace(isForwardMerge ? iterEnlarge->second.m_forwardAssociations :
                                                     iterEnlarge->second.m_backwardAssociations);
    clusterSetToReplace = clusterSetToMove;
    clusterAssociationMap.erase(iterDelete);

    for (ClusterAssociationMap::iterator iter = clusterAssociationMap.begin(),
                                         iterEnd = clusterAssociationMap.end();
         iter != iterEnd;
         ++iter) {
  ...
  void ClusterAssociationAlgorithm::UpdateForAmbiguousMerge(
    const Cluster* const pCluster,
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ClusterAssociationMap::iterator cIter = clusterAssociationMap.find(pCluster);

    if (clusterAssociationMap.end() == cIter) throw StatusCodeException(STATUS_CODE_NOT_FOUND);

    for (ClusterAssociationMap::iterator mIter = clusterAssociationMap.begin(),
                                         mIterEnd = clusterAssociationMap.end();
         mIter != mIterEnd;
         ++mIter) {
      ...
      if (forwardClusters.end() != fIter) forwardClusters.erase(fIter);

      if (backwardClusters.end() != bIter) backwardClusters.erase(bIter);
    }
  ...
  void ClusterAssociationAlgorithm::NavigateAlongAssociations(
    const ClusterAssociationMap& clusterAssociationMap,
    const Cluster* const pCluster,
    const bool isForward,
    const Cluster*& pExtremalCluster,
    ClusterSet& clusterSet) const
  {
    ...
    if ((pCluster != pExtremalCluster) &&
        this->IsExtremalCluster(isForward, pExtremalCluster, pCluster))
      pExtremalCluster = pCluster;

    const ClusterSet& associatedClusterSet(isForward ?
                                             iterAssociation->second.m_forwardAssociations :
                                             iterAssociation->second.m_backwardAssociations);

    for (ClusterSet::const_iterator iter = associatedClusterSet.begin(),
                                    iterEnd = associatedClusterSet.end();
         iter != iterEnd;
         ++iter) {
      this->NavigateAlongAssociations(
        clusterAssociationMap, *iter, isForward, pExtremalCluster, clusterSet);
    }
  }
