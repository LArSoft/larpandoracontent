...
namespace lar_content {
  ...
  StatusCode ClusterGrowingAlgorithm::Run()
  {
    ...
    if (!pClusterList || pClusterList->empty()) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "ClusterGrowingAlgorithm: unable to find cluster list "
  ...
  void ClusterGrowingAlgorithm::GetListOfNonSeedClusters(const ClusterVector& inputClusters,
                                                         const ClusterVector& seedClusters,
  ...
  void ClusterGrowingAlgorithm::PopulateClusterMergeMap(const ClusterVector& seedClusters,
                                                        const ClusterVector& nonSeedClusters,
                                                        ClusterMergeMap& clusterMergeMap) const
  {
    for (ClusterVector::const_iterator nIter = nonSeedClusters.begin(),
                                       nIterEnd = nonSeedClusters.end();
         nIter != nIterEnd;
         ++nIter) {
      ...
      for (ClusterVector::const_iterator sIter = seedClusters.begin(),
                                         sIterEnd = seedClusters.end();
           sIter != sIterEnd;
           ++sIter) {
        const Cluster* const pThisSeedCluster = *sIter;
        const float thisDistance(
          LArClusterHelper::GetClosestDistance(pNonSeedCluster, pThisSeedCluster));
      ...
      if (pBestSeedCluster) clusterMergeMap[pBestSeedCluster].push_back(pNonSeedCluster);
    }
  }
      ...
      for (const Cluster* const pAssociatedCluster : clusterList) {
        if (m_inputClusterListName.empty()) {
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
            !=,
            PandoraContentApi::MergeAndDeleteClusters(*this, pParentCluster, pAssociatedCluster));
        }
        else {
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS,
