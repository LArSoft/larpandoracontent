...
namespace lar_content {
  ...
  void CrossGapsExtensionAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
  ...
  void CrossGapsExtensionAlgorithm::FillClusterAssociationMatrix(
    ...
    ClusterAssociationMatrix& clusterAssociationMatrix) const
  {
    ...
    this->BuildPointingClusterList(
      clusterVector, innerPointingClusterList, outerPointingClusterList);
    ...
    for (const LArPointingCluster& pointingClusterInner : innerPointingClusterList) {
      ...
      for (const LArPointingCluster& pointingClusterOuter : outerPointingClusterList) {
        const LArPointingCluster::Vertex& pointingVertexOuter(
        ...
        if (!this->IsAcrossGap(
              zOuter,
              zInner,
              LArClusterHelper::GetClusterHitType(pointingClusterInner.GetCluster())))
          continue;

        if (!this->IsAssociated(pointingVertexInner, pointingVertexOuter)) continue;
        ...
        (void)clusterAssociationMatrix[pClusterInner].insert(
          ClusterAssociationMap::value_type(pClusterOuter,
                                            ClusterAssociation(ClusterAssociation::INNER,
                                                               ClusterAssociation::OUTER,
                                                               ClusterAssociation::STRONG,
                                                               lengthSquaredOuter)));
        (void)clusterAssociationMatrix[pClusterOuter].insert(
          ClusterAssociationMap::value_type(pClusterInner,
                                            ClusterAssociation(ClusterAssociation::OUTER,
                                                               ClusterAssociation::INNER,
                                                               ClusterAssociation::STRONG,
                                                               lengthSquaredInner)));
      }
    }
    ...
    LArPointingClusterList& outputPointingClusterList) const
  {
    for (const LArPointingCluster& pointingCluster : inputPointingClusterList) {
      const LArPointingCluster::Vertex& pointingVertex(useInner ? pointingCluster.GetInnerVertex() :
                                                                  pointingCluster.GetOuterVertex());
      const HitType hitType(LArClusterHelper::GetClusterHitType(pointingCluster.GetCluster()));

      if (LArGeometryHelper::IsInGap(
            this->GetPandora(), pointingVertex.GetPosition(), hitType, m_maxGapTolerance))
  ...
  bool CrossGapsExtensionAlgorithm::IsAssociated(
    const LArPointingCluster::Vertex& pointingVertex1,
    const LArPointingCluster::Vertex& pointingVertex2) const
  {
    const float maxLongitudinalDisplacement(
      (pointingVertex2.GetPosition() - pointingVertex1.GetPosition()).GetMagnitude());

    const bool isAssociated1(LArPointingClusterHelper::IsEmission(pointingVertex1.GetPosition(),
                                                                  pointingVertex2,
                                                                  -1.f,
                                                                  maxLongitudinalDisplacement + 1.f,
                                                                  m_maxTransverseDisplacement,
                                                                  m_maxRelativeAngle));
    const bool isAssociated2(LArPointingClusterHelper::IsEmission(pointingVertex2.GetPosition(),
                                                                  pointingVertex1,
                                                                  -1.f,
                                                                  maxLongitudinalDisplacement + 1.f,
                                                                  m_maxTransverseDisplacement,
  ...
  bool CrossGapsExtensionAlgorithm::IsAcrossGap(const float minZ,
                                                const float maxZ,
                                                const HitType hitType) const
  {
    if (maxZ - minZ < std::numeric_limits<float>::epsilon()) return false;

    const float gapDeltaZ(
      LArGeometryHelper::CalculateGapDeltaZ(this->GetPandora(), minZ, maxZ, hitType));

    if (gapDeltaZ / (maxZ - minZ) < m_minGapFraction) return false;
  ...
  void CrossGapsExtensionAlgorithm::FillClusterMergeMap(
  ...
  StatusCode CrossGapsExtensionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinGapFraction", m_minGapFraction));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxCosRelativeAngle", maxCosRelativeAngle));
