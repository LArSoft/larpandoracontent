...
namespace lar_content {
  ...
  void HitWidthClusterMergingAlgorithm::GetListOfCleanClusters(
    ...
    ClusterVector& clusterVector) const
  {
    ...
    if (!m_clusterToParametersMap.empty()) m_clusterToParametersMap.clear();

    for (const Cluster* const pCluster : *pClusterList) {
      // the original cluster weight, with no hit scaling or hit padding
      if (LArHitWidthHelper::GetOriginalTotalClusterWeight(pCluster) < m_minClusterWeight) continue;

      const unsigned int numberOfProposedConstituentHits(
        LArHitWidthHelper::GetNProposedConstituentHits(
          pCluster, m_maxConstituentHitWidth, m_hitWidthScalingFactor));

      if (numberOfProposedConstituentHits == 0) continue;

      // clusterSparseness [0 -> 1] where a higher value indicates sparseness
      const float clusterSparseness(1.f - (static_cast<float>(pCluster->GetNCaloHits()) /
                                           static_cast<float>(numberOfProposedConstituentHits)));

      if (clusterSparseness < m_minClusterSparseness) continue;

      m_clusterToParametersMap.insert(
        std::pair<const Cluster*, LArHitWidthHelper::ClusterParameters>(
          pCluster,
          LArHitWidthHelper::ClusterParameters(
            pCluster, m_maxConstituentHitWidth, false, m_hitWidthScalingFactor)));

      clusterVector.push_back(pCluster);
    }

    std::sort(clusterVector.begin(),
              clusterVector.end(),
              LArHitWidthHelper::SortByHigherXExtrema(m_clusterToParametersMap));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void HitWidthClusterMergingAlgorithm::PopulateClusterAssociationMap(
    ...
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    for (ClusterVector::const_iterator iterCurrentCluster = clusterVector.begin();
         iterCurrentCluster != clusterVector.end();
         ++iterCurrentCluster) {
      ...
      for (ClusterVector::const_iterator iterTestCluster = iterCurrentCluster;
           iterTestCluster != clusterVector.end();
           ++iterTestCluster) {
        if (iterCurrentCluster == iterTestCluster) continue;
        ...
        if (!this->AreClustersAssociated(currentClusterParameters, testClusterParameters)) continue;
  ...
  bool HitWidthClusterMergingAlgorithm::IsExtremalCluster(const bool isForward,
                                                          ...
                                                          const Cluster* const pTestCluster) const
  {
    ...
    const LArHitWidthHelper::ConstituentHitVector currentConstituentHitVector(
      LArHitWidthHelper::GetConstituentHits(
        pCurrentCluster, m_maxConstituentHitWidth, m_hitWidthScalingFactor, false));
    const LArHitWidthHelper::ConstituentHitVector testConstituentHitVector(
      LArHitWidthHelper::GetConstituentHits(
        pTestCluster, m_maxConstituentHitWidth, m_hitWidthScalingFactor, false));
    const CartesianVector currentHigherXExtrema(
      LArHitWidthHelper::GetExtremalCoordinatesHigherX(currentConstituentHitVector));
    const CartesianVector testHigherXExtrema(
      LArHitWidthHelper::GetExtremalCoordinatesHigherX(testConstituentHitVector));
  ...
  bool HitWidthClusterMergingAlgorithm::AreClustersAssociated(
    const LArHitWidthHelper::ClusterParameters& currentFitParameters,
    const LArHitWidthHelper::ClusterParameters& testFitParameters) const
  {
    // check cluster extrema are not too far away in x
    if (testFitParameters.GetLowerXExtrema().GetX() >
        (currentFitParameters.GetHigherXExtrema().GetX() + m_maxXMergeDistance))
      return false;

    // check cluster extrema are not too far away in z
    if (testFitParameters.GetLowerXExtrema().GetZ() >
          (currentFitParameters.GetHigherXExtrema().GetZ() + m_maxZMergeDistance) ||
        testFitParameters.GetLowerXExtrema().GetZ() <
          (currentFitParameters.GetHigherXExtrema().GetZ() - m_maxZMergeDistance)) {
      return false;
    }
    ...
    if (testFitParameters.GetLowerXExtrema().GetX() <
        currentFitParameters.GetHigherXExtrema().GetX()) {
      this->FindClosestPointToPosition(currentFitParameters.GetHigherXExtrema(),
                                       testFitParameters.GetConstituentHitVector(),
                                       testMergePoint);

      // check closeness in z is maintained
      if (testMergePoint.GetZ() >
            (currentFitParameters.GetHigherXExtrema().GetZ() + m_maxZMergeDistance) ||
          testMergePoint.GetZ() <
            (currentFitParameters.GetHigherXExtrema().GetZ() - m_maxZMergeDistance)) {
        return false;
    ...
    try {
      this->GetClusterDirection(currentFitParameters.GetConstituentHitVector(),
                                currentClusterDirection,
                                currentFitParameters.GetHigherXExtrema(),
                                m_fittingWeight);
      this->GetClusterDirection(testFitParameters.GetConstituentHitVector(),
                                testClusterDirection,
                                testMergePoint,
                                m_fittingWeight);
    }
    ...
    if (currentClusterDirection.GetCosOpeningAngle(testClusterDirection) <
        m_minMergeCosOpeningAngle)
      return false;

    // check that the new direction is consistent with the old clusters
    LArHitWidthHelper::ConstituentHitVector newConstituentHitVector(
      currentFitParameters.GetConstituentHitVector());
    newConstituentHitVector.insert(newConstituentHitVector.end(),
                                   testFitParameters.GetConstituentHitVector().begin(),
                                   testFitParameters.GetConstituentHitVector().end());

    const CartesianVector midpoint((currentFitParameters.GetHigherXExtrema() + testMergePoint) *
                                   0.5);
    CartesianVector newClusterDirection(0.f, 0.f, 0.f);

    try {
      this->GetClusterDirection(
        newConstituentHitVector, newClusterDirection, midpoint, m_fittingWeight);
    }
    catch (const StatusCodeException&) {
      return false;
    }

    if (newClusterDirection.GetCosOpeningAngle(currentClusterDirection) <
          m_minDirectionDeviationCosAngle ||
        newClusterDirection.GetCosOpeningAngle(testClusterDirection) <
          m_minDirectionDeviationCosAngle) {
      return false;
    }
  ...
  void HitWidthClusterMergingAlgorithm::FindClosestPointToPosition(
  ...
  void HitWidthClusterMergingAlgorithm::GetClusterDirection(
    const LArHitWidthHelper::ConstituentHitVector& constituentHitVector,
    CartesianVector& direction,
    const CartesianVector& fitReferencePoint,
    const float fittingWeight) const
  {
    ...
    this->GetConstituentHitSubsetVector(
      constituentHitVector, fitReferencePoint, fittingWeight, constituentHitSubsetVector);
    ...
    this->GetFittingCoordinates(
      axisDirection, constituentHitSubsetVector.begin()->GetPositionVector(), firstHitL, firstHitT);
    ...
    for (const LArHitWidthHelper::ConstituentHit& constituentHit : constituentHitSubsetVector) {
      ...
      if (std::fabs(firstHitL - rL) > std::numeric_limits<float>::epsilon()) isLConstant = false;

      if (std::fabs(firstHitT - rT) > std::numeric_limits<float>::epsilon()) isTConstant = false;
    ...
    if (weightSum < std::numeric_limits<float>::epsilon()) {
      std::cout << "HitWidthClusterMergingAlgorithm::GetWeightedGradient - hit weight in fit is "
    ...
    // ATTN direction convention is to point in direction of increasing x
    if (direction.GetX() < 0.f) direction *= -1.f;
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void HitWidthClusterMergingAlgorithm::GetConstituentHitSubsetVector(
    const LArHitWidthHelper::ConstituentHitVector& constituentHitVector,
    const CartesianVector& fitReferencePoint,
    ...
    LArHitWidthHelper::ConstituentHitVector& constituentHitSubsetVector) const
  {
    ...
    std::sort(sortedConstituentHitVector.begin(),
              sortedConstituentHitVector.end(),
    ...
    for (const LArHitWidthHelper::ConstituentHit& constituentHit : sortedConstituentHitVector) {
      ...
      if (weightCount > fittingWeight) break;
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void HitWidthClusterMergingAlgorithm::GetFittingAxes(
    const LArHitWidthHelper::ConstituentHitVector& constituentHitSubsetVector,
    CartesianVector& axisDirection,
    CartesianVector& orthoDirection) const
  {
    CartesianPointVector constituentHitSubsetPositionVector(
      LArHitWidthHelper::GetConstituentHitPositionVector(constituentHitSubsetVector));
    ...
    if (axisDirection.GetZ() < 0.f) axisDirection *= -1.f;
    ...
    float& rT) const
  {
    ...
    const float openingAngle(axisDirection.GetOpeningAngle(xAxis)), c(std::cos(openingAngle)),
  ...
  void HitWidthClusterMergingAlgorithm::GetGlobalDirection(const CartesianVector& axisDirection,
                                                           const float gradient,
                                                           CartesianVector& globalDirection) const
  {
    const CartesianVector xAxis(1.f, 0.f, 0.f);
    const float openingAngle(axisDirection.GetOpeningAngle(xAxis)), c(std::cos(openingAngle)),
      s(std::sin(openingAngle));
  ...
  void HitWidthClusterMergingAlgorithm::RemoveShortcutAssociations(
    const ClusterVector& clusterVector,
    ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    for (const Cluster* const pCluster : clusterVector) {
      const ClusterAssociationMap::const_iterator primaryMapIter =
        clusterAssociationMap.find(pCluster);

      if (primaryMapIter == clusterAssociationMap.end()) continue;

      // put ClusterSet into ClusterVector
      ClusterVector primaryForwardAssociations(primaryMapIter->second.m_forwardAssociations.begin(),
                                               primaryMapIter->second.m_forwardAssociations.end());
      std::sort(primaryForwardAssociations.begin(),
                primaryForwardAssociations.end(),
      ...
      for (const Cluster* const pConsideredCluster : primaryForwardAssociations) {
        for (const Cluster* const pPrimaryCluster : primaryForwardAssociations) {
          if (pConsideredCluster == pPrimaryCluster) continue;

          const ClusterAssociationMap::const_iterator secondaryMapIter =
            clusterAssociationMap.find(pPrimaryCluster);

          // if primary cluster has no associations (this shouldn't ever be the case)
          if (secondaryMapIter == clusterAssociationMap.end()) continue;

          const ClusterSet& secondaryForwardAssociations(
            secondaryMapIter->second.m_forwardAssociations);

          if (secondaryForwardAssociations.find(pConsideredCluster) !=
              secondaryForwardAssociations.end()) {
            ClusterSet& tempPrimaryForwardAssociations(
              tempMap.find(pCluster)->second.m_forwardAssociations);
            const ClusterSet::const_iterator forwardAssociationToRemove(
              tempPrimaryForwardAssociations.find(pConsideredCluster));

            // if association has already been removed
            if (forwardAssociationToRemove == tempPrimaryForwardAssociations.end()) continue;

            ClusterSet& tempPrimaryBackwardAssociations(
              tempMap.find(pConsideredCluster)->second.m_backwardAssociations);
            const ClusterSet::const_iterator backwardAssociationToRemove(
              tempPrimaryBackwardAssociations.find(pCluster));

            // if association has already been removed
            if (backwardAssociationToRemove == tempPrimaryBackwardAssociations.end()) continue;
  ...
  StatusCode HitWidthClusterMergingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "FittingWeight", m_fittingWeight));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinClusterWeight", m_minClusterWeight));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxXMergeDistance", m_maxXMergeDistance));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxZMergeDistance", m_maxZMergeDistance));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxConstituentHitWidth", m_maxConstituentHitWidth));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "HitWidthScalingFactor", m_hitWidthScalingFactor));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinClusterSparseness", m_minClusterSparseness));
