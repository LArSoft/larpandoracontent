...
namespace lar_content {
  ...
  KinkSplittingAlgorithm::KinkSplittingAlgorithm()
    : m_maxScatterRms(0.2f), m_maxScatterCosTheta(0.905f), m_maxSlidingCosTheta(0.985f)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  StatusCode KinkSplittingAlgorithm::FindBestSplitPosition(
    ...
    CartesianVector& splitPosition) const
  {
    ...
    const int minLayer(layerFitResultMap.begin()->first),
      maxLayer(layerFitResultMap.rbegin()->first);

    const int nLayersHalfWindow(slidingFitResult.GetLayerFitHalfWindow());
    const int nLayersSpanned(1 + maxLayer - minLayer);

    if (nLayersSpanned <= 2 * nLayersHalfWindow) return STATUS_CODE_NOT_FOUND;
    ...
    for (LayerFitResultMap::const_iterator iter = layerFitResultMap.begin(),
      ...
      CartesianVector centralPosition(0.f, 0.f, 0.f), firstDirection(0.f, 0.f, 0.f),
        secondDirection(0.f, 0.f, 0.f);
  ...
  StatusCode KinkSplittingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxSlidingCosTeta", m_maxSlidingCosTheta));
