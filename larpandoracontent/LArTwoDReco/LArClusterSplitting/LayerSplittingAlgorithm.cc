...
namespace lar_content {
  ...
  StatusCode LayerSplittingAlgorithm::DivideCaloHits(const Cluster* const pCluster,
                                                     CaloHitList& firstHitList,
  ...
  StatusCode LayerSplittingAlgorithm::FindBestSplitLayer(const Cluster* const pCluster,
                                                         unsigned int& splitLayer) const
  {
    const OrderedCaloHitList& orderedCaloHitList(pCluster->GetOrderedCaloHitList());

    if (orderedCaloHitList.size() < m_minClusterLayers) return STATUS_CODE_NOT_FOUND;
    ...
    for (unsigned int iLayer = pCluster->GetInnerPseudoLayer() + 4;
         iLayer + 4 <= pCluster->GetOuterPseudoLayer();
         ++iLayer) {
      if (orderedCaloHitList.find(iLayer) == orderedCaloHitList.end()) continue;

      unsigned int innerLayer((pCluster->GetInnerPseudoLayer() + m_layerWindow > iLayer) ?
                                pCluster->GetInnerPseudoLayer() :
                                iLayer - m_layerWindow);
      unsigned int outerLayer((iLayer + m_layerWindow > pCluster->GetOuterPseudoLayer()) ?
                                pCluster->GetOuterPseudoLayer() :
                                iLayer + m_layerWindow);

      for (; innerLayer >= pCluster->GetInnerPseudoLayer(); --innerLayer) {
        if (orderedCaloHitList.find(innerLayer) != orderedCaloHitList.end()) break;
      }

      for (; outerLayer <= pCluster->GetOuterPseudoLayer(); ++outerLayer) {
        if (orderedCaloHitList.find(outerLayer) != orderedCaloHitList.end()) break;
      }
      ...
      if (cosTheta > 0.f) {
        ...
        if (cosTheta > m_maxScatterCosTheta) {
          rmsCut *=
            ((m_maxSlidingCosTheta > cosTheta) ?
               (m_maxSlidingCosTheta - cosTheta) / (m_maxSlidingCosTheta - m_maxScatterCosTheta) :
               0.f);
        }
      }
  ...
  float LayerSplittingAlgorithm::CalculateRms(const Cluster* const pCluster,
                                              ...
                                              const unsigned int& secondLayer) const
  {
    ...
    for (unsigned int iLayer = innerLayer + 1; iLayer + 1 < outerLayer; ++iLayer) {
      ...
      const CartesianVector predictedPosition(
        innerPosition +
        predictedDirection * predictedDirection.GetDotProduct(hitPosition - innerPosition));

      totalChi2 += (predictedPosition - hitPosition).GetMagnitudeSquared();
      totalLayers += 1.f;
    }

    if (totalLayers > 0.f) return std::sqrt(totalChi2 / totalLayers);
                                                     ...
                                                     CaloHitList& secondHitList) const
  {
    ...
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin();
      ...
      for (CaloHitList::const_iterator hitIter = iter->second->begin(),
        ...
        if (thisLayer < splitLayer) { firstHitList.push_back(pCaloHit); }
        else {
    ...
    if (firstHitList.empty() || secondHitList.empty()) return STATUS_CODE_NOT_FOUND;
  ...
  StatusCode LayerSplittingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "LayerWindow", m_layerWindow));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxScatterRms", m_maxScatterRms));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxScatterCosTheta", m_maxScatterCosTheta));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxSlidingCosTheta", m_maxSlidingCosTheta));
