...
namespace lar_content {
  ...
  StatusCode TwoDSlidingFitSplittingAndSwitchingAlgorithm::Run()
  {
    ...
    for (ClusterVector::iterator iter1 = clusterVector.begin(), iterEnd1 = clusterVector.end();
         iter1 != iterEnd1;
         ++iter1) {
      if (NULL == *iter1) continue;
      ...
      for (ClusterVector::iterator iter2 = iter1, iterEnd2 = iterEnd1; iter2 != iterEnd2; ++iter2) {
        ...
        if (slidingFitResult1.GetCluster() == slidingFitResult2.GetCluster()) continue;
        ...
        if (STATUS_CODE_SUCCESS !=
            this->FindBestSplitPosition(
              slidingFitResult1, slidingFitResult2, splitPosition, firstDirection, secondDirection))
          continue;
        ...
        if (STATUS_CODE_SUCCESS !=
            this->ReplaceClusters(
              pCluster1, pCluster2, splitPosition, firstDirection, secondDirection))
          continue;
  ...
  StatusCode TwoDSlidingFitSplittingAndSwitchingAlgorithm::PreparationStep(
  ...
  void TwoDSlidingFitSplittingAndSwitchingAlgorithm::GetListOfCleanClusters(
  ...
  void TwoDSlidingFitSplittingAndSwitchingAlgorithm::SplitCluster(
    const Cluster* const pCluster,
    const CartesianVector& splitPosition,
    const CartesianVector& splitDirection,
    CaloHitList& firstCaloHitList,
    CaloHitList& secondCaloHitList) const
  {
  ...
  StatusCode TwoDSlidingFitSplittingAndSwitchingAlgorithm::ReplaceClusters(
    const Cluster* const pCluster1,
    const Cluster* const pCluster2,
    const CartesianVector& splitPosition,
    const CartesianVector& firstDirection,
    const CartesianVector& secondDirection) const
  {
    ...
    this->SplitCluster(pCluster1,
                       splitPosition,
                       firstDirection,
                       firstParameters.m_caloHitList,
                       secondParameters.m_caloHitList);
    this->SplitCluster(pCluster2,
                       splitPosition,
                       secondDirection,
                       secondParameters.m_caloHitList,
                       firstParameters.m_caloHitList);
    ...
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::Cluster::Create(*this, firstParameters, pFirstCluster));
    PANDORA_THROW_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::Cluster::Create(*this, secondParameters, pSecondCluster));
