...
namespace lar_content {
  ...
  TwoDSlidingFitMultiSplitAlgorithm::TwoDSlidingFitMultiSplitAlgorithm()
    : m_slidingFitHalfWindow(15), m_inputClusterList("")
  {}
  ...
  StatusCode TwoDSlidingFitMultiSplitAlgorithm::Run()
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::GetCurrentListName<Cluster>(*this, originalListName));

    if (!m_inputClusterList.empty()) {
      const StatusCode statusCode(
        PandoraContentApi::ReplaceCurrentList<Cluster>(*this, m_inputClusterList));

      if (STATUS_CODE_NOT_FOUND == statusCode) {
        std::cout << "TwoDSlidingFitMultiSplitAlgorithm: cluster list not found "
    ...
    // Perform splits
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, this->SplitClusters(slidingFitResultMap, clusterSplittingMap));

    if (!m_inputClusterList.empty())
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ReplaceCurrentList<Cluster>(*this, originalListName));
    ...
    const ClusterPositionMap& clusterSplittingMap) const
  {
    ...
    for (const Cluster* const pCluster : clusterList) {
      ...
      if (splitPositionVector.empty()) continue;
  ...
  StatusCode TwoDSlidingFitMultiSplitAlgorithm::SplitCluster(
    ...
    const CartesianPointVector& splitPositionVector) const
  {
    ...
    for (CartesianPointVector::const_iterator pIter = splitPositionVector.begin(),
                                              pIterEnd = splitPositionVector.end();
         pIter != pIterEnd;
         ++pIter) {
    ...
    for (FloatVector::const_iterator fIter = displacementVector.begin(),
                                     fIterEnd = displacementVector.end();
         fIter != fIterEnd;
         ++fIter) {
      ...
      if (foundPreviousL) {
        ...
        for (CaloHitList::const_iterator hIter = oldCaloHitList.begin(),
                                         hIterEnd = oldCaloHitList.end();
             hIter != hIterEnd;
             ++hIter) {
          ...
          if (rL >= prevL && rL < nextL) newCaloHitList.push_back(pCaloHit);
        }

        if (newCaloHitList.empty()) continue;
        ...
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::Cluster::Create(*this, newParameters, pNewCluster));
      }
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::EndFragmentation(*this, clusterListToSave, clusterListToDelete));
  ...
  StatusCode TwoDSlidingFitMultiSplitAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "InputClusterListName", m_inputClusterList));
