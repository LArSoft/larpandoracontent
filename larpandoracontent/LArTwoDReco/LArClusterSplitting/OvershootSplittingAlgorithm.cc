...
namespace lar_content {
  ...
  void OvershootSplittingAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
  ...
  void OvershootSplittingAlgorithm::FindBestSplitPositions(
    ...
    ClusterPositionMap& clusterSplittingMap) const
  {
    ...
    this->BuildSortedIntersectionMap(
      slidingFitResultMap, clusterIntersectionMap, sortedIntersectionMap);
  ...
  void OvershootSplittingAlgorithm::BuildIntersectionMap(
    const TwoDSlidingFitResultMap& slidingFitResultMap,
    ...
    for (const Cluster* const pCluster1 : clusterList) {
      ...
      for (const Cluster* const pCluster2 : clusterList) {
        ...
        try {
          ...
          const float innerDisplacement(
            LArClusterHelper::GetClosestDistance(innerPosition, pCluster1));
          const float outerDisplacement(
            LArClusterHelper::GetClosestDistance(outerPosition, pCluster1));
          const bool useInner((innerDisplacement < outerDisplacement) ? true : false);

          const LArPointingCluster::Vertex& clusterVertex =
            (useInner ? pointingCluster.GetInnerVertex() : pointingCluster.GetOuterVertex());
          ...
          try {
            LArPointingClusterHelper::GetIntersection(
              clusterVertex, pCluster1, intersectPosition2, rL2, rT2);
          }
          catch (const StatusCodeException&) {
            continue;
          }

          if (rL2 < -m_maxIntersectDisplacement || rL2 > m_maxClusterSeparation) continue;
          ...
          const StatusCode statusCodePosition(
          ...
          const StatusCode statusCodeDirection(
          ...
          try {
            LArPointingClusterHelper::GetIntersection(projectedPosition1,
                                                      projectedDirection1,
                                                      projectedPosition2,
                                                      projectedDirection2,
                                                      intersectPosition1,
          ...
          const float closestDisplacement1(
            LArClusterHelper::GetClosestDistance(intersectPosition1, pCluster1));
          const float closestDisplacement2(
            LArClusterHelper::GetClosestDistance(intersectPosition1, pCluster2));
          ...
          const float minDisplacementSquared(
            (minPosition - intersectPosition1).GetMagnitudeSquared());
          const float maxDisplacementSquared(
            (maxPosition - intersectPosition1).GetMagnitudeSquared());

          if (std::min(minDisplacementSquared, maxDisplacementSquared) <
                (m_minVertexDisplacement * m_minVertexDisplacement) ||
  ...
  void OvershootSplittingAlgorithm::BuildSortedIntersectionMap(
    const TwoDSlidingFitResultMap& slidingFitResultMap,
    ...
    ClusterPositionMap& sortedIntersectionMap) const
  {
    ...
    for (const Cluster* const pCluster : clusterList) {
      ...
      if (inputPositionVector.empty()) continue;

      TwoDSlidingFitResultMap::const_iterator sIter = slidingFitResultMap.find(pCluster);
      if (slidingFitResultMap.end() == sIter) throw StatusCodeException(STATUS_CODE_FAILURE);

      const TwoDSlidingFitResult& slidingFitResult = sIter->second;

      MyTrajectoryPointList trajectoryPointList;
      for (CartesianPointVector::const_iterator pIter = inputPositionVector.begin(),
                                                pIterEnd = inputPositionVector.end();
           pIter != pIterEnd;
           ++pIter) {
      ...
      std::sort(trajectoryPointList.begin(),
                trajectoryPointList.end(),
                OvershootSplittingAlgorithm::SortByHitProjection);

      if (trajectoryPointList.empty()) throw StatusCodeException(STATUS_CODE_FAILURE);

      for (MyTrajectoryPointList::const_iterator qIter = trajectoryPointList.begin(),
                                                 qIterEnd = trajectoryPointList.end();
           qIter != qIterEnd;
           ++qIter) {
  ...
  void OvershootSplittingAlgorithm::PopulateSplitPositionMap(
    const ClusterPositionMap& clusterIntersectionMap,
    ClusterPositionMap& clusterSplittingMap) const
  {
    ClusterList clusterList;
    ...
    for (const Cluster* const pCluster : clusterList) {
      ...
      for (CartesianPointVector::const_iterator pIter = inputPositionVector.begin(),
        ...
        if (foundPrevPosition) {
          ...
          if (displacementSquared < m_maxIntersectDisplacement * m_maxIntersectDisplacement)
            candidatePositionList.push_back(
      ...
      if (candidatePositionList.empty()) continue;

      std::sort(candidatePositionList.begin(),
                candidatePositionList.end(),
                OvershootSplittingAlgorithm::SortByHitProjection);
      ...
      for (MyTrajectoryPointList::const_iterator pIter = candidatePositionList.begin(),
                                                 pIterEnd = candidatePositionList.end();
           pIter != pIterEnd;
           ++pIter) {
        const CartesianVector& nextCandidate = pIter->second;

        if (foundPrevCandidate) {
          if ((nextCandidate - prevCandidate).GetMagnitudeSquared() <
              m_minSplitDisplacement * m_minSplitDisplacement)
            continue;
        }
  ...
  bool OvershootSplittingAlgorithm::SortByHitProjection(const MyTrajectoryPoint& lhs,
  ...
  StatusCode OvershootSplittingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinVertexDisplacement", m_minVertexDisplacement));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinSplitDisplacement", m_minSplitDisplacement));
