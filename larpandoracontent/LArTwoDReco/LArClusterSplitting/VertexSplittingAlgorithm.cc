...
namespace lar_content {
  ...
  VertexSplittingAlgorithm::VertexSplittingAlgorithm()
    : m_splitDisplacementSquared(4.f * 4.f), m_vertexDisplacementSquared(1.f * 1.f)
  {
  ...
  StatusCode VertexSplittingAlgorithm::FindBestSplitPosition(
    ...
    CartesianVector& splitPosition) const
  {
    ...
    if (pVertexList->empty()) return STATUS_CODE_NOT_INITIALIZED;

    if (pVertexList->size() != 1) return STATUS_CODE_OUT_OF_RANGE;
    ...
    if (VERTEX_3D != pSelectedVertex->GetVertexType()) return STATUS_CODE_INVALID_PARAMETER;

    const CartesianVector theVertex2D(LArGeometryHelper::ProjectPosition(
      this->GetPandora(), pSelectedVertex->GetPosition(), hitType));
    ...
    const float vertexDisplacementSquared(
      std::min((splitPosition - innerVertex2D).GetMagnitudeSquared(),
               (splitPosition - outerVertex2D).GetMagnitudeSquared()));
  ...
  StatusCode VertexSplittingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "SplitDisplacement", splitDisplacement));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexDisplacement", vertexDisplacement));
