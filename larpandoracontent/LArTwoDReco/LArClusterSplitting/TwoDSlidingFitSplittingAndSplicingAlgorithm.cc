...
namespace lar_content {
  ...
  StatusCode TwoDSlidingFitSplittingAndSplicingAlgorithm::Run()
  {
    ...
    while (++nIterations < 100) // Protect against flip-flopping between two answers
    {
      ...
      this->BuildSlidingFitResultMap(
        clusterVector, m_shortHalfWindowLayers, branchSlidingFitResultMap);

      // Calculate sliding fit results for replacement clusters (use a hard linear fit for these)
      this->BuildSlidingFitResultMap(
        clusterVector, m_longHalfWindowLayers, replacementSlidingFitResultMap);

      // Compile a list of possible splits
      ClusterExtensionList splitList;

      if (m_runCosmicMode) {
        this->BuildClusterExtensionList(
          clusterVector, branchSlidingFitResultMap, replacementSlidingFitResultMap, splitList);
      }
      else {
        ClusterExtensionList intermediateList;
        this->BuildClusterExtensionList(clusterVector,
                                        branchSlidingFitResultMap,
                                        replacementSlidingFitResultMap,
                                        intermediateList);
        this->PruneClusterExtensionList(
          intermediateList, branchSlidingFitResultMap, replacementSlidingFitResultMap, splitList);
      }

      // Run splitting and extension
      if (STATUS_CODE_SUCCESS != this->RunSplitAndExtension(splitList,
                                                            branchSlidingFitResultMap,
                                                            replacementSlidingFitResultMap))
        break;
    }
  ...
  void TwoDSlidingFitSplittingAndSplicingAlgorithm::GetListOfCleanClusters(
  ...
  void TwoDSlidingFitSplittingAndSplicingAlgorithm::BuildClusterExtensionList(
    const ClusterVector& clusterVector,
    const TwoDSlidingFitResultMap& branchSlidingFitResultMap,
    const TwoDSlidingFitResultMap& replacementSlidingFitResultMap,
    ClusterExtensionList& clusterExtensionList) const
  {
    // Loop over each possible pair of clusters
    for (ClusterVector::const_iterator iterI = clusterVector.begin(),
                                       iterEndI = clusterVector.end();
         iterI != iterEndI;
         ++iterI) {
      const Cluster* const pClusterI = *iterI;

      for (ClusterVector::const_iterator iterJ = iterI, iterEndJ = clusterVector.end();
           iterJ != iterEndJ;
           ++iterJ) {
        ...
        TwoDSlidingFitResultMap::const_iterator iterBranchI =
          branchSlidingFitResultMap.find(*iterI);
        TwoDSlidingFitResultMap::const_iterator iterBranchJ =
          branchSlidingFitResultMap.find(*iterJ);

        TwoDSlidingFitResultMap::const_iterator iterReplacementI =
          replacementSlidingFitResultMap.find(*iterI);
        TwoDSlidingFitResultMap::const_iterator iterReplacementJ =
          replacementSlidingFitResultMap.find(*iterJ);

        if (branchSlidingFitResultMap.end() == iterBranchI ||
            branchSlidingFitResultMap.end() == iterBranchJ ||
            replacementSlidingFitResultMap.end() == iterReplacementI ||
            replacementSlidingFitResultMap.end() == iterReplacementJ) {
        ...
        try {
          this->FindBestSplitPosition(branchSlidingFitI,
                                      replacementSlidingFitJ,
                                      replacementStartPositionJ,
                                      branchSplitPositionI,
                                      branchSplitDirectionI);
          branchChisqI =
            this->CalculateBranchChi2(pClusterI, branchSplitPositionI, branchSplitDirectionI);
        }
        ...
        try {
          this->FindBestSplitPosition(branchSlidingFitJ,
                                      replacementSlidingFitI,
                                      replacementStartPositionI,
                                      branchSplitPositionJ,
                                      branchSplitDirectionJ);
          branchChisqJ =
            this->CalculateBranchChi2(pClusterJ, branchSplitPositionJ, branchSplitDirectionJ);
        }
        catch (StatusCodeException&) {
        }
        ...
        if (branchChisqI > 0.f && branchChisqJ > 0.f) {
          const CartesianVector relativeDirection(
            (branchSplitPositionJ - branchSplitPositionI).GetUnitVector());

          if (branchSplitDirectionI.GetDotProduct(relativeDirection) > 0.f &&
              branchSplitDirectionJ.GetDotProduct(relativeDirection) < 0.f) {
            try {
              const float newBranchChisqI(
                this->CalculateBranchChi2(pClusterI, branchSplitPositionI, relativeDirection));
              const float newBranchChisqJ(this->CalculateBranchChi2(
                pClusterJ, branchSplitPositionJ, relativeDirection * -1.f));
        ...
        if (branchChisqI > branchChisqJ) {
          clusterExtensionList.push_back(ClusterExtension(pClusterI,
                                                          pClusterJ,
                                                          replacementStartPositionJ,
                                                          branchSplitPositionI,
                                                          branchSplitDirectionI));
          ...
          clusterExtensionList.push_back(ClusterExtension(pClusterJ,
                                                          pClusterI,
                                                          replacementStartPositionI,
                                                          branchSplitPositionJ,
                                                          branchSplitDirectionJ));
  ...
  void TwoDSlidingFitSplittingAndSplicingAlgorithm::PruneClusterExtensionList(
    const ClusterExtensionList& inputList,
    const TwoDSlidingFitResultMap& branchMap,
    const TwoDSlidingFitResultMap& replacementMap,
    ClusterExtensionList& outputList) const
  {
    ...
    for (const ClusterExtension& thisSplit : inputList) {
      ...
      for (const Cluster* const pBranchCluster : branchList) {
        ...
        if (slidingFit.GetCluster() == thisSplit.GetReplacementCluster() ||
            slidingFit.GetCluster() == thisSplit.GetBranchCluster())
          continue;

        const float minDistanceSquared(
          (replacementVertex - slidingFit.GetGlobalMinLayerPosition()).GetMagnitudeSquared());
        const float maxDistanceSquared(
          (replacementVertex - slidingFit.GetGlobalMaxLayerPosition()).GetMagnitudeSquared());

        if (std::min(minDistanceSquared, maxDistanceSquared) <
      ...
      for (const Cluster* const pReplacementCluster : replacementList) {
        ...
        const float minDistanceSquared(
          (branchVertex - slidingFit.GetGlobalMinLayerPosition()).GetMagnitudeSquared());
        const float maxDistanceSquared(
          (branchVertex - slidingFit.GetGlobalMaxLayerPosition()).GetMagnitudeSquared());
      ...
      if (branchVeto || replacementVeto) continue;
    ...
    const CartesianVector& splitDirection) const
  {
    ...
    this->SplitBranchCluster(
      pCluster, splitPosition, splitDirection, principalCaloHitList, branchCaloHitList);

    float totalChi2(0.f);
    float totalHits(0.f);

    for (CaloHitList::const_iterator iter = branchCaloHitList.begin(),
                                     iterEnd = branchCaloHitList.end();
         iter != iterEnd;
         ++iter) {
      const CaloHit* const pCaloHit = *iter;

      const CartesianVector hitPosition(pCaloHit->GetPositionVector());
      const CartesianVector projectedPosition(
        splitPosition + splitDirection * splitDirection.GetDotProduct(hitPosition - splitPosition));

      totalChi2 += (hitPosition - projectedPosition).GetMagnitudeSquared();
      totalHits += 1.f;
    }

    if (totalHits > 0.f) return std::sqrt(totalChi2 / totalHits);
  ...
  void TwoDSlidingFitSplittingAndSplicingAlgorithm::SplitBranchCluster(
    const Cluster* const pCluster,
    const CartesianVector& splitPosition,
    const CartesianVector& splitDirection,
    CaloHitList& principalCaloHitList,
    CaloHitList& branchCaloHitList) const
  {
    ...
    for (CaloHitList::const_iterator iter = caloHitsToDistribute.begin(),
                                     iterEnd = caloHitsToDistribute.end();
         iter != iterEnd;
         ++iter) {
      const CaloHit* const pCaloHit = *iter;
    ...
    if (branchCaloHitList.empty()) throw StatusCodeException(STATUS_CODE_NOT_ALLOWED);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  StatusCode TwoDSlidingFitSplittingAndSplicingAlgorithm::RunSplitAndExtension(
    const ClusterExtensionList& splitList,
    TwoDSlidingFitResultMap& branchResultMap,
    TwoDSlidingFitResultMap& replacementResultMap) const
  {
    bool foundSplit(false);

    for (ClusterExtensionList::const_iterator iter = splitList.begin(), iterEnd = splitList.end();
         iter != iterEnd;
         ++iter) {
      ...
      TwoDSlidingFitResultMap::iterator iterReplacement1 =
        replacementResultMap.find(pBranchCluster);
      TwoDSlidingFitResultMap::iterator iterReplacement2 =
        replacementResultMap.find(pReplacementCluster);

      if (branchResultMap.end() == iterBranch1 || branchResultMap.end() == iterBranch2 ||
          replacementResultMap.end() == iterReplacement1 ||
          replacementResultMap.end() == iterReplacement2)
        continue;
      ...
      PANDORA_RETURN_RESULT_IF(
        ...
        !=,
        this->ReplaceBranch(
          pBranchCluster, pReplacementCluster, branchSplitPosition, branchSplitDirection));
    ...
    if (foundSplit) return STATUS_CODE_SUCCESS;
  ...
  StatusCode TwoDSlidingFitSplittingAndSplicingAlgorithm::ReplaceBranch(
    const Cluster* const pBranchCluster,
    const Cluster* const pReplacementCluster,
    const CartesianVector& branchSplitPosition,
    const CartesianVector& branchSplitDirection) const
  {
    ...
    this->SplitBranchCluster(pBranchCluster,
                             branchSplitPosition,
                             branchSplitDirection,
                             principalParameters.m_caloHitList,
                             residualParameters.m_caloHitList);

    const Cluster *pPrincipalCluster(NULL), *pResidualCluster(NULL);
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::Cluster::Create(*this, principalParameters, pPrincipalCluster));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::Cluster::Create(*this, residualParameters, pResidualCluster));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::EndFragmentation(*this, clusterListToSaveName, clusterListToDeleteName));
  ...
  StatusCode TwoDSlidingFitSplittingAndSplicingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "ShortHalfWindow", m_shortHalfWindowLayers));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "LongHalfWindow", m_longHalfWindowLayers));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VetoDisplacement", m_vetoDisplacement));

    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "CosmicMode", m_runCosmicMode));
