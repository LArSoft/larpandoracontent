...
namespace lar_content {
  ...
  void DeltaRaySplittingAlgorithm::FindBestSplitPosition(
    ...
    CartesianVector& branchSplitDirection) const
  {
    ...
    for (unsigned int principalForward = 0; principalForward < 2; ++principalForward) {
      ...
      for (unsigned int branchForward = 0; branchForward < 2; ++branchForward) {
        ...
        const float vertexProjection(
          +branchDirection.GetDotProduct(principalVertex - branchVertex));
        ...
        if (vertexProjection < 0.f && cosRelativeAngle > m_minCosRelativeAngle) continue;

        if (cosRelativeAngle < 0.f) continue;
        ...
        while (!foundSplit) {
          ...
          if (principalDirection.GetDotProduct(linearProjection - principalVertex) <
              -m_maxLongitudinalDisplacement)
            break;

          if ((linearProjection - branchVertex).GetMagnitudeSquared() >
              (linearProjection - branchEnd).GetMagnitudeSquared())
            break;
          ...
          CartesianVector truncatedDirection(1 == branchForward ? forwardDirection :
          ...
          LArPointingClusterHelper::GetImpactParameters(
            truncatedPosition, truncatedDirection, principalVertex, rL1, rT1);
          LArPointingClusterHelper::GetImpactParameters(
            principalVertex, principalDirection, truncatedPosition, rL2, rT2);
  ...
  StatusCode DeltaRaySplittingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "StepSize", m_stepSize));
