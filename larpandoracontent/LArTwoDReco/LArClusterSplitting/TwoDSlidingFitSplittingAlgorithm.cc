...
namespace lar_content {
  ...
  TwoDSlidingFitSplittingAlgorithm::TwoDSlidingFitSplittingAlgorithm()
    : m_slidingFitHalfWindow(20), m_minClusterLength(10.f)
  {}

  //------------------------------------------------------------------------------------------------------------------------------------------

  StatusCode TwoDSlidingFitSplittingAlgorithm::DivideCaloHits(const Cluster* const pCluster,
  ...
  StatusCode TwoDSlidingFitSplittingAlgorithm::DivideCaloHits(
    const TwoDSlidingFitResult& slidingFitResult,
    ...
    CaloHitList& secondCaloHitList) const
  {
    ...
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin();
      ...
      for (CaloHitList::const_iterator hitIter = iter->second->begin(),
        ...
        if (thisL < rL) { firstCaloHitList.push_back(pCaloHit); }
        else {
    ...
    if (firstCaloHitList.empty() || secondCaloHitList.empty()) return STATUS_CODE_NOT_FOUND;
