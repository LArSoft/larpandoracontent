...
namespace lar_content {
  ...
  {
    if (m_inputClusterListNames.empty()) return this->RunUsingCurrentList();
    ...
    for (const std::string& clusterListName : m_inputClusterListNames) {
      const StatusCode listChangeStatusCode(
      ...
      if (STATUS_CODE_NOT_FOUND == listChangeStatusCode) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "ClusterSplittingAlgorithm: cluster list not found " << clusterListName
                    << std::endl;

        continue;
      }

      if (STATUS_CODE_SUCCESS != listChangeStatusCode) return listChangeStatusCode;
  ...
  StatusCode ClusterSplittingAlgorithm::RunUsingCurrentList() const
  {
    ...
    for (ClusterList::iterator iter = internalClusterList.begin();
         iter != internalClusterList.end();
         ++iter) {
      const Cluster* const pCluster = *iter;
      ClusterList clusterSplittingList;

      if (STATUS_CODE_SUCCESS != this->SplitCluster(pCluster, clusterSplittingList)) continue;
  ...
  StatusCode ClusterSplittingAlgorithm::SplitCluster(const Cluster* const pCluster,
                                                     ClusterList& clusterSplittingList) const
  {
    // Split cluster into two CaloHit lists
    PandoraContentApi::Cluster::Parameters firstParameters, secondParameters;

    if (STATUS_CODE_SUCCESS != this->DivideCaloHits(pCluster,
