...
namespace lar_content {
  ...
  void BranchSplittingAlgorithm::FindBestSplitPosition(
    const TwoDSlidingFitResult& branchSlidingFit,
    const TwoDSlidingFitResult& principalSlidingFit,
    CartesianVector& principalStartPosition,
    CartesianVector& branchSplitPosition,
    CartesianVector& branchSplitDirection) const
  {
    // Conventions:
    ...
    for (unsigned int principalForward = 0; principalForward < 2; ++principalForward) {
      const CartesianVector principalVertexPosition(
        1 == principalForward ? principalSlidingFit.GetGlobalMinLayerPosition() :
                                principalSlidingFit.GetGlobalMaxLayerPosition());
      const CartesianVector principalEndPosition(1 != principalForward ?
                                                   principalSlidingFit.GetGlobalMinLayerPosition() :
                                                   principalSlidingFit.GetGlobalMaxLayerPosition());
      const CartesianVector principalVertexDirection(
        1 == principalForward ? principalSlidingFit.GetGlobalMinLayerDirection() :
      ...
      for (unsigned int branchForward = 0; branchForward < 2; ++branchForward) {
        const CartesianVector branchVertexPosition(1 == branchForward ?
                                                     branchSlidingFit.GetGlobalMinLayerPosition() :
                                                     branchSlidingFit.GetGlobalMaxLayerPosition());
        const CartesianVector branchEndPosition(1 != branchForward ?
                                                  branchSlidingFit.GetGlobalMinLayerPosition() :
                                                  branchSlidingFit.GetGlobalMaxLayerPosition());
        const CartesianVector branchEndDirection(
          1 != branchForward ? branchSlidingFit.GetGlobalMinLayerDirection() :
                               branchSlidingFit.GetGlobalMaxLayerDirection() * -1.f);

        if (principalVertexDirection.GetDotProduct(branchEndDirection) < 0.5f) continue;

        if ((principalEndPosition - branchEndPosition).GetMagnitudeSquared() <
            (principalVertexPosition - branchVertexPosition).GetMagnitudeSquared())
          continue;
        ...
        try {
          if (!projectedPositionFound && !projectedPositionFail) {
            projectedBranchPosition =
              LArPointingClusterHelper::GetProjectedPosition(principalVertexPosition,
                                                             principalVertexDirection,
                                                             branchSlidingFit.GetCluster(),
                                                             m_projectionAngularAllowance);
        ...
        if (!projectedPositionFound || projectedPositionFail) continue;

        const float projectedDistanceSquared(
          (projectedBranchPosition - principalVertexPosition).GetMagnitudeSquared());

        if (projectedDistanceSquared >
            m_maxLongitudinalDisplacement * m_maxLongitudinalDisplacement)
          continue;

        const float commonDistanceSquared(
          (projectedBranchPosition - branchEndPosition).GetMagnitudeSquared());

        if (projectedDistanceSquared > commonDistanceSquared) continue;

        const float replacementDistanceSquared(
          (projectedBranchPosition - principalEndPosition).GetMagnitudeSquared());

        if (replacementDistanceSquared < m_minLongitudinalExtension * m_minLongitudinalExtension)
          continue;

        const float branchDistanceSquared(
          (projectedBranchPosition - branchVertexPosition).GetMagnitudeSquared());

        if (branchDistanceSquared > 4.f * replacementDistanceSquared) continue;
        ...
        CartesianVector projectedBranchDirection(1 == branchForward ? forwardDirection :
        ...
        try {
          ...
          if (cosTheta < currentCosTheta) continue;
        }
        catch (StatusCodeException&) {
        }
        ...
        LArPointingClusterHelper::GetImpactParameters(
          projectedBranchPosition, projectedBranchDirection, principalVertexPosition, rL1, rT1);
        LArPointingClusterHelper::GetImpactParameters(
          principalVertexPosition, principalVertexDirection, projectedBranchPosition, rL2, rT2);

        if ((cosTheta > m_minCosRelativeAngle) && (rT1 < m_maxTransverseDisplacement) &&
            (rT2 < m_maxTransverseDisplacement)) {
          foundSplit = true;
