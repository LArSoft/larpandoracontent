...
namespace lar_content {
  ...
  StatusCode TwoDSlidingFitConsolidationAlgorithm::Run()
  {
    ...
    this->GetReclusteredHits(
      slidingFitResultList, showerClusters, clustersToExpand, clustersToContract);

    // Consolidate and re-build clusters
    ClusterSet unavailableClusters;
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                             !=,
                             this->RemoveHitsFromClusters(clustersToContract, unavailableClusters));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, this->AddHitsToClusters(clustersToExpand, unavailableClusters));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, this->RebuildClusters(clustersToContract, unavailableClusters));
  ...
  void TwoDSlidingFitConsolidationAlgorithm::BuildSlidingLinearFits(
    const ClusterVector& trackClusters,
    TwoDSlidingFitResultList& slidingFitResultList) const
  {
    const float slidingFitPitch(LArGeometryHelper::GetWireZPitch(this->GetPandora()));

    for (ClusterVector::const_iterator iter = trackClusters.begin(), iterEnd = trackClusters.end();
         iter != iterEnd;
         ++iter) {
      try {
  ...
  StatusCode TwoDSlidingFitConsolidationAlgorithm::RemoveHitsFromClusters(
    const ClusterToHitMap& clustersToContract,
    ClusterSet& unavailableClusters) const
  {
    ...
    for (const Cluster* const pCluster : clusterList) {
      ...
      if (caloHitListToRemove.empty()) continue;

      if (unavailableClusters.count(pCluster)) continue;

      CaloHitList caloHitList, caloHitListToKeep;
      pCluster->GetOrderedCaloHitList().FillCaloHitList(caloHitList);

      for (const CaloHit* const pCaloHit : caloHitList) {
        if (caloHitListToRemove.end() ==
            std::find(caloHitListToRemove.begin(), caloHitListToRemove.end(), pCaloHit))
      ...
      if (caloHitListToKeep.empty()) {
        ...
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, PandoraContentApi::Delete<Cluster>(*this, pCluster));
        continue;
      }

      for (const CaloHit* const pCaloHit : caloHitListToRemove) {
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, PandoraContentApi::RemoveFromCluster(*this, pCluster, pCaloHit));
      }
    }
  ...
  StatusCode TwoDSlidingFitConsolidationAlgorithm::AddHitsToClusters(
    const ClusterToHitMap& clustersToExpand,
    ClusterSet& unavailableClusters) const
  {
    ClusterList clusterList;

    for (const auto& mapEntry : clustersToExpand) {
      if (!unavailableClusters.count(mapEntry.first)) clusterList.push_back(mapEntry.first);
    }
    ...
    for (const Cluster* const pCluster : clusterList) {
      ...
      for (const CaloHit* const pCaloHit : caloHitList) {
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToCluster(*this, pCluster, pCaloHit));
      }
    }
  ...
  StatusCode TwoDSlidingFitConsolidationAlgorithm::RebuildClusters(
    const ClusterToHitMap& clustersToRebuild,
    const ClusterSet& unavailableClusters) const
  {
    if (clustersToRebuild.empty()) return STATUS_CODE_SUCCESS;

    ClusterVector sortedClusters;
    for (const auto& mapEntry : clustersToRebuild) {
      if (!unavailableClusters.count(mapEntry.first)) sortedClusters.push_back(mapEntry.first);
    }
    ...
    for (const Cluster* const pCluster : sortedClusters) {
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::GetCurrentListName<Cluster>(*this, currentClusterListName));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::Delete<Cluster>(*this, pClusterToDelete));
      ...
      PANDORA_RETURN_RESULT_IF(
        ...
        PandoraContentApi::RunClusteringAlgorithm(
          *this, m_reclusteringAlgorithmName, pClusterList, newClusterListName));

      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::SaveList<Cluster>(*this, newClusterListName, currentClusterListName));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ReplaceCurrentList<Cluster>(*this, currentClusterListName));
    }
  ...
  StatusCode TwoDSlidingFitConsolidationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinTrackLength", m_minTrackLength));
