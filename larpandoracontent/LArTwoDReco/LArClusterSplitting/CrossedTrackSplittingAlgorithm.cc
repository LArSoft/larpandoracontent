...
namespace lar_content {
  ...
  StatusCode CrossedTrackSplittingAlgorithm::FindBestSplitPosition(
    const TwoDSlidingFitResult& slidingFitResult1,
    const TwoDSlidingFitResult& slidingFitResult2,
    CartesianVector& splitPosition,
    CartesianVector& firstDirection,
    CartesianVector& secondDirection) const
  {
    ...
    if (!m_nearbyClusters.count(slidingFitResult1.GetCluster()) ||
        ...
        !m_nearbyClusters.at(slidingFitResult1.GetCluster())
           .count(slidingFitResult2.GetCluster()) ||
        !m_nearbyClusters.at(slidingFitResult2.GetCluster())
           .count(slidingFitResult1.GetCluster())) {
    ...
    if (LArClusterHelper::GetClosestDistance(minPosition1, slidingFitResult2.GetCluster()) <
          2.f * m_maxClusterSeparation ||
        LArClusterHelper::GetClosestDistance(maxPosition1, slidingFitResult2.GetCluster()) <
          2.f * m_maxClusterSeparation ||
        LArClusterHelper::GetClosestDistance(minPosition2, slidingFitResult1.GetCluster()) <
          2.f * m_maxClusterSeparation ||
        LArClusterHelper::GetClosestDistance(maxPosition2, slidingFitResult1.GetCluster()) <
          2.f * m_maxClusterSeparation) {
      return STATUS_CODE_NOT_FOUND;
    }

    if (LArClusterHelper::GetClosestDistance(
          slidingFitResult1.GetCluster(), slidingFitResult2.GetCluster()) > m_maxClusterSeparation)
      return STATUS_CODE_NOT_FOUND;

    CartesianPointVector candidateVector;
    this->FindCandidateSplitPositions(
      slidingFitResult1.GetCluster(), slidingFitResult2.GetCluster(), candidateVector);

    if (candidateVector.empty()) return STATUS_CODE_NOT_FOUND;
    ...
    for (CartesianPointVector::const_iterator iter = candidateVector.begin(),
                                              iterEnd = candidateVector.end();
         iter != iterEnd;
         ++iter) {
      ...
      for (unsigned int iForward = 0; iForward < 2; ++iForward) {
        ...
        if (mu1 < 0.f || mu2 < 0.f || mu1 > (b1 - a1).GetMagnitude() ||
            mu2 > (b2 - a2).GetMagnitude())
          continue;
    ...
    CartesianPointVector& candidateVector) const
  {
    ...
    CaloHitVector caloHitVector1(caloHitList1.begin(), caloHitList1.end()),
      caloHitVector2(caloHitList2.begin(), caloHitList2.end());
