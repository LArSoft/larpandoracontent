...
namespace lar_content {
  ...
  StatusCode TrackClusterCreationAlgorithm::Run()
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      this->FilterCaloHits(pCaloHitList, selectedCaloHitList, rejectedCaloHitList));

    HitAssociationMap forwardHitAssociationMap, backwardHitAssociationMap;
    this->MakePrimaryAssociations(
      selectedCaloHitList, forwardHitAssociationMap, backwardHitAssociationMap);
    this->MakeSecondaryAssociations(
      selectedCaloHitList, forwardHitAssociationMap, backwardHitAssociationMap);

    HitJoinMap hitJoinMap;
    HitToClusterMap hitToClusterMap;
    this->IdentifyJoins(
      selectedCaloHitList, forwardHitAssociationMap, backwardHitAssociationMap, hitJoinMap);
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      this->AddFilteredCaloHits(selectedCaloHitList, rejectedCaloHitList, hitToClusterMap));
  ...
  StatusCode TrackClusterCreationAlgorithm::FilterCaloHits(
    ...
    OrderedCaloHitList& selectedCaloHitList,
    OrderedCaloHitList& rejectedCaloHitList) const
  {
    CaloHitList availableHitList;

    for (const CaloHit* const pCaloHit : *pCaloHitList) {
      if (PandoraContentApi::IsAvailable(*this, pCaloHit)) availableHitList.push_back(pCaloHit);
    }

    if (availableHitList.empty()) return STATUS_CODE_SUCCESS;

    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, selectedCaloHitList.Add(availableHitList));

    for (OrderedCaloHitList::const_iterator iter = selectedCaloHitList.begin(),
                                            iterEnd = selectedCaloHitList.end();
         iter != iterEnd;
         ++iter) {
      ...
      for (const CaloHit* const pCaloHitI : caloHits) {
        ...
        for (const CaloHit* const pCaloHitJ : caloHits) {
          if (pCaloHitI == pCaloHitJ) continue;

          if ((pCaloHitI->GetMipEquivalentEnergy() < pCaloHitJ->GetMipEquivalentEnergy()) &&
              ((pCaloHitI->GetPositionVector() - pCaloHitJ->GetPositionVector())
                 .GetMagnitudeSquared() < m_minCaloHitSeparationSquared)) {
            useCaloHit = false;
            break;
          }
        }
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, selectedCaloHitList.Remove(rejectedCaloHitList));
    return STATUS_CODE_SUCCESS;
  }
  ...
  StatusCode TrackClusterCreationAlgorithm::AddFilteredCaloHits(
    const OrderedCaloHitList& selectedCaloHitList,
    const OrderedCaloHitList& rejectedCaloHitList,
    HitToClusterMap& hitToClusterMap) const
  {
    for (OrderedCaloHitList::const_iterator iter = rejectedCaloHitList.begin(),
                                            iterEnd = rejectedCaloHitList.end();
         iter != iterEnd;
         ++iter) {
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        selectedCaloHitList.GetCaloHitsInPseudoLayer(iter->first, pCaloHitList));

      CaloHitSet unavailableHits;

      CaloHitVector inputAvailableHits(iter->second->begin(), iter->second->end());
      std::sort(
        inputAvailableHits.begin(), inputAvailableHits.end(), LArClusterHelper::SortHitsByPosition);
      ...
      while (carryOn) {
        ...
        for (const CaloHit* const pCaloHitI : inputAvailableHits) {
          if (unavailableHits.count(pCaloHitI)) continue;

          if (hitToClusterMap.end() != hitToClusterMap.find(pCaloHitI)) continue;
          ...
          for (const CaloHit* const pCaloHitJ : clusteredHits) {
            if (pCaloHitI->GetMipEquivalentEnergy() > pCaloHitJ->GetMipEquivalentEnergy()) continue;
          ...
          if (!pClosestHit) continue;

          HitToClusterMap::const_iterator mapIter = hitToClusterMap.find(pClosestHit);

          if (hitToClusterMap.end() == mapIter) throw StatusCodeException(STATUS_CODE_FAILURE);

          const Cluster* const pCluster = mapIter->second;
          PANDORA_THROW_RESULT_IF(
            STATUS_CODE_SUCCESS, !=, PandoraContentApi::AddToCluster(*this, pCluster, pCaloHitI));
  ...
  void TrackClusterCreationAlgorithm::MakePrimaryAssociations(
  ...
  {
    for (OrderedCaloHitList::const_iterator iterI = orderedCaloHitList.begin(),
                                            iterIEnd = orderedCaloHitList.end();
         iterI != iterIEnd;
         ++iterI) {
      ...
      for (OrderedCaloHitList::const_iterator iterJ = iterI, iterJEnd = orderedCaloHitList.end();
           (nLayersConsidered++ <= m_maxGapLayers + 1) && (iterJ != iterJEnd);
           ++iterJ) {
        ...
        for (const CaloHit* const pCaloHitI : caloHitsI) {
          for (const CaloHit* const pCaloHitJ : caloHitsJ)
            this->CreatePrimaryAssociation(
              pCaloHitI, pCaloHitJ, forwardHitAssociationMap, backwardHitAssociationMap);
        }
      }
    }
  }
  ...
  void TrackClusterCreationAlgorithm::MakeSecondaryAssociations(
    const OrderedCaloHitList& orderedCaloHitList,
    HitAssociationMap& forwardHitAssociationMap,
    HitAssociationMap& backwardHitAssociationMap) const
  {
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
      ...
      for (const CaloHit* const pCaloHit : caloHits) {
        ...
        const CaloHit* const pForwardHit(
          (forwardHitAssociationMap.end() == fwdIter) ? NULL : fwdIter->second.GetPrimaryTarget());

        HitAssociationMap::const_iterator fwdCheckIter =
          backwardHitAssociationMap.find(pForwardHit);
        const CaloHit* const pForwardHitCheck((backwardHitAssociationMap.end() == fwdCheckIter) ?
                                                NULL :
                                                fwdCheckIter->second.GetPrimaryTarget());

        if ((NULL != pForwardHit) && (pForwardHitCheck != pCaloHit))
          this->CreateSecondaryAssociation(
            pCaloHit, pForwardHit, forwardHitAssociationMap, backwardHitAssociationMap);

        HitAssociationMap::const_iterator bwdIter = backwardHitAssociationMap.find(pCaloHit);
        const CaloHit* const pBackwardHit(
          (backwardHitAssociationMap.end() == bwdIter) ? NULL : bwdIter->second.GetPrimaryTarget());

        HitAssociationMap::const_iterator bwdCheckIter =
          forwardHitAssociationMap.find(pBackwardHit);
        const CaloHit* const pBackwardHitCheck((forwardHitAssociationMap.end() == bwdCheckIter) ?
                                                 NULL :
                                                 bwdCheckIter->second.GetPrimaryTarget());

        if ((NULL != pBackwardHit) && (pBackwardHitCheck != pCaloHit))
          this->CreateSecondaryAssociation(
            pBackwardHit, pCaloHit, forwardHitAssociationMap, backwardHitAssociationMap);
      }
    }
  }
    ...
    HitJoinMap& hitJoinMap) const
  {
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
      ...
      for (const CaloHit* const pCaloHit : caloHits) {
        const CaloHit* const pForwardJoinHit =
          this->GetJoinHit(pCaloHit, forwardHitAssociationMap, backwardHitAssociationMap);
        const CaloHit* const pBackwardJoinHit =
          this->GetJoinHit(pForwardJoinHit, backwardHitAssociationMap, forwardHitAssociationMap);

        if ((NULL == pForwardJoinHit) || (NULL == pBackwardJoinHit) ||
            (pBackwardJoinHit != pCaloHit))
          continue;
  ...
  void TrackClusterCreationAlgorithm::CreateClusters(const OrderedCaloHitList& orderedCaloHitList,
                                                     ...
                                                     HitToClusterMap& hitToClusterMap) const
  {
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
      ...
      for (const CaloHit* const pCaloHit : caloHits) {
        ...
        if (hitJoinMap.end() == joinIter) continue;
        ...
        PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                                !=,
                                PandoraContentApi::AddToCluster(*this, pCluster, joinIter->second));
  ...
  void TrackClusterCreationAlgorithm::CreatePrimaryAssociation(
    const CaloHit* const pCaloHitI,
    const CaloHit* const pCaloHitJ,
    HitAssociationMap& forwardHitAssociationMap,
    HitAssociationMap& backwardHitAssociationMap) const
  {
    const float distanceSquared(
      (pCaloHitJ->GetPositionVector() - pCaloHitI->GetPositionVector()).GetMagnitudeSquared());

    if (distanceSquared > m_maxCaloHitSeparationSquared) return;
    ...
    if (forwardHitAssociationMap.end() == forwardIter) {
      forwardHitAssociationMap.insert(
        HitAssociationMap::value_type(pCaloHitI, HitAssociation(pCaloHitJ, distanceSquared)));
    }
    ...
    if (backwardHitAssociationMap.end() == backwardIter) {
      backwardHitAssociationMap.insert(
        HitAssociationMap::value_type(pCaloHitJ, HitAssociation(pCaloHitI, distanceSquared)));
    }
  ...
  void TrackClusterCreationAlgorithm::CreateSecondaryAssociation(
    const CaloHit* const pCaloHitI,
    const CaloHit* const pCaloHitJ,
    HitAssociationMap& forwardHitAssociationMap,
    HitAssociationMap& backwardHitAssociationMap) const
  {
    ...
    if ((forwardHitAssociationMap.end() == forwardIter) ||
        (backwardHitAssociationMap.end() == backwardIter))
      return;

    HitAssociation& forwardAssociation(forwardIter->second);
    HitAssociation& backwardAssociation(backwardIter->second);

    if ((forwardAssociation.GetPrimaryTarget() != pCaloHitJ) &&
        (backwardAssociation.GetPrimaryTarget() == pCaloHitI)) {
      if ((backwardAssociation.GetPrimaryDistanceSquared() <
           forwardAssociation.GetSecondaryDistanceSquared()) &&
          (backwardAssociation.GetPrimaryDistanceSquared() < m_closeSeparationSquared)) {
        forwardAssociation.SetSecondaryTarget(pCaloHitJ,
                                              backwardAssociation.GetPrimaryDistanceSquared());
      }
    }

    if ((backwardAssociation.GetPrimaryTarget() != pCaloHitI) &&
        (forwardAssociation.GetPrimaryTarget() == pCaloHitJ)) {
      if ((forwardAssociation.GetPrimaryDistanceSquared() <
           backwardAssociation.GetSecondaryDistanceSquared()) &&
          (forwardAssociation.GetPrimaryDistanceSquared() < m_closeSeparationSquared)) {
        backwardAssociation.SetSecondaryTarget(pCaloHitI,
                                               forwardAssociation.GetPrimaryDistanceSquared());
      }
    }
  }
    ...
    const HitAssociationMap& hitAssociationMapJ) const
  {
    ...
    if (hitAssociationMapI.end() == iterI) return NULL;
    ...
    if (NULL == pSecondaryTarget) return pPrimaryTarget;

    unsigned int primaryNSteps(0), secondaryNSteps(0);
    const CaloHit* const pPrimaryTrace = this->TraceHitAssociation(
      pPrimaryTarget, hitAssociationMapI, hitAssociationMapJ, primaryNSteps);
    const CaloHit* const pSecondaryTrace = this->TraceHitAssociation(
      pSecondaryTarget, hitAssociationMapI, hitAssociationMapJ, secondaryNSteps);

    if ((pPrimaryTrace == pSecondaryTrace) || (secondaryNSteps < 5)) return pPrimaryTarget;
  ...
  const CaloHit* TrackClusterCreationAlgorithm::TraceHitAssociation(
    const CaloHit* const pCaloHit,
    const HitAssociationMap& hitAssociationMapI,
    ...
    unsigned int& nSteps) const
  {
    ...
    while (true) {
      ...
      if (hitAssociationMapI.end() == iterI) break;

      pLastHit = iterI->second.GetPrimaryTarget();
      HitAssociationMap::const_iterator iterJ = hitAssociationMapJ.find(pLastHit);

      if (hitAssociationMapJ.end() == iterJ) break;

      if (iterJ->second.GetPrimaryTarget() != pThisHit) break;
    }
  ...
  StatusCode TrackClusterCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MergeBackFilteredHits", m_mergeBackFilteredHits));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxGapLayers", m_maxGapLayers));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxCaloHitSeparation", maxCaloHitSeparation));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinCaloHitSeparation", minCaloHitSeparation));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "CloseSeparation", closeSeparation));
