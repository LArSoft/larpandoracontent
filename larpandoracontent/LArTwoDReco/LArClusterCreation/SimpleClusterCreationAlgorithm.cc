...
namespace lar_content {
  ...
  StatusCode SimpleClusterCreationAlgorithm::Run()
  {
    ...
    if (caloHitList.empty()) return STATUS_CODE_SUCCESS;
  ...
  void SimpleClusterCreationAlgorithm::SelectCaloHits(const CaloHitList* const pInputList,
                                                      CaloHitList& outputList) const
  {
    for (const CaloHit* const pCaloHit : *pInputList) {
      if (PandoraContentApi::IsAvailable(*this, pCaloHit)) outputList.push_back(pCaloHit);
    }
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void SimpleClusterCreationAlgorithm::BuildAssociationMap(
    ...
    HitAssociationMap& hitAssociationMap) const
  {
    for (const CaloHit* const pCaloHitI : caloHitList) {
      for (const CaloHit* const pCaloHitJ : caloHitList) {
        ...
        if ((pCaloHitI->GetPositionVector() - pCaloHitJ->GetPositionVector())
              .GetMagnitudeSquared() < m_clusteringWindowSquared) {
  ...
  void SimpleClusterCreationAlgorithm::CreateClusters(
    const CaloHitList& caloHitList,
    const HitAssociationMap& hitAssociationMap) const
  {
    ...
    for (const CaloHit* const pSeedCaloHit : caloHitVector) {
      if (vetoList.count(pSeedCaloHit)) continue;

      CaloHitList mergeList;
      this->CollectAssociatedHits(
        pSeedCaloHit, pSeedCaloHit, hitAssociationMap, vetoList, mergeList);

      const Cluster* pCluster = NULL;
      PandoraContentApi::Cluster::Parameters parameters;
      parameters.m_caloHitList.push_back(pSeedCaloHit);
      PANDORA_THROW_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::Cluster::Create(*this, parameters, pCluster));
      ...
      for (const CaloHit* const pAssociatedCaloHit : mergeList) {
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::AddToCluster(*this, pCluster, pAssociatedCaloHit));
  ...
  void SimpleClusterCreationAlgorithm::CollectAssociatedHits(
    const CaloHit* const pSeedCaloHit,
    const CaloHit* const pCurrentCaloHit,
    const HitAssociationMap& hitAssociationMap,
    const CaloHitSet& vetoList,
    CaloHitList& mergeList) const
  {
    if (vetoList.count(pCurrentCaloHit)) return;

    HitAssociationMap::const_iterator iter1 = hitAssociationMap.find(pCurrentCaloHit);
    if (iter1 == hitAssociationMap.end()) return;

    CaloHitVector caloHitVector(iter1->second.begin(), iter1->second.end());
    std::sort(caloHitVector.begin(), caloHitVector.end(), LArClusterHelper::SortHitsByPosition);

    for (const CaloHit* const pAssociatedCaloHit : caloHitVector) {
      if (pAssociatedCaloHit == pSeedCaloHit) continue;
      ...
      this->CollectAssociatedHits(
        pSeedCaloHit, pAssociatedCaloHit, hitAssociationMap, vetoList, mergeList);
    }
  }
  //------------------------------------------------------------------------------------------------------------------------------------------
  ...
  StatusCode SimpleClusterCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "ClusteringWindow", clusteringWindow));
