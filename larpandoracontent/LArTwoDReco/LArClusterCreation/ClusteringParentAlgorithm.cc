...
namespace lar_content {
  ...
  ClusteringParentAlgorithm::ClusteringParentAlgorithm()
    : m_replaceCurrentCaloHitList(false), m_replaceCurrentClusterList(true)
  {}
  ...
  StatusCode ClusteringParentAlgorithm::Run()
  {
    ...
    if (!m_inputCaloHitListName.empty()) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::GetCurrentListName<CaloHit>(*this, originalCaloHitListName));
      const StatusCode statusCode(
        PandoraContentApi::ReplaceCurrentList<CaloHit>(*this, m_inputCaloHitListName));

      if (STATUS_CODE_NOT_FOUND == statusCode) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "ClusteringParentAlgorithm: calohit list not found "
                    << m_inputCaloHitListName << std::endl;
    ...
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                             ...
                             PandoraContentApi::RunClusteringAlgorithm(
                               *this, m_clusteringAlgorithmName, pClusterList, newClusterListName));
    ...
    if (!pClusterList->empty() && !m_associationAlgorithmName.empty())
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::RunDaughterAlgorithm(*this, m_associationAlgorithmName));

    // Save the new cluster list
    if (!pClusterList->empty()) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::SaveList<Cluster>(*this, m_clusterListName));
    ...
    if (!m_replaceCurrentCaloHitList && !m_inputCaloHitListName.empty()) {
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::ReplaceCurrentList<CaloHit>(*this, originalCaloHitListName));
    }
  ...
  StatusCode ClusteringParentAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ProcessAlgorithm(*this, xmlHandle, "ClusterFormation", m_clusteringAlgorithmName));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      XmlHelper::ProcessAlgorithm(
        *this, xmlHandle, "ClusterAssociation", m_associationAlgorithmName));
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ReplaceCurrentCaloHitList", m_replaceCurrentCaloHitList));
