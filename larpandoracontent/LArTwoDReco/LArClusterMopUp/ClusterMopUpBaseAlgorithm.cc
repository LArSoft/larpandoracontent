...
namespace lar_content {
  ...
  void ClusterMopUpBaseAlgorithm::GetPfoClusterLists(ClusterList& clusterListU,
                                                     ClusterList& clusterListV,
                                                     ClusterList& clusterListW) const
  {
    for (StringVector::const_iterator sIter = m_pfoListNames.begin(),
                                      sIterEnd = m_pfoListNames.end();
         sIter != sIterEnd;
         ++sIter) {
      const PfoList* pPfoList = NULL;
      if (STATUS_CODE_SUCCESS != PandoraContentApi::GetList(*this, *sIter, pPfoList)) continue;
      ...
      for (PfoList::const_iterator pIter = pPfoList->begin(), pIterEnd = pPfoList->end();
        ...
        if (m_excludePfosContainingTracks && LArPfoHelper::IsTrack(pPfo)) continue;

        this->GetClusterLists(
          pPfo->GetClusterList(), false, clusterListU, clusterListV, clusterListW);
      }
    }
  }
  ...
  void ClusterMopUpBaseAlgorithm::GetDaughterClusterLists(ClusterList& clusterListU,
                                                          ClusterList& clusterListV,
                                                          ClusterList& clusterListW) const
  {
  ...
  void ClusterMopUpBaseAlgorithm::GetClusterLists(const ClusterList& inputClusterList,
                                                  const bool availabilityFlag,
                                                  ClusterList& clusterListU,
                                                  ClusterList& clusterListV,
                                                  ClusterList& clusterListW) const
  {
    for (ClusterList::const_iterator cIter = inputClusterList.begin(),
                                     cIterEnd = inputClusterList.end();
         cIter != cIterEnd;
         ++cIter) {
      const Cluster* const pCluster(*cIter);

      if (availabilityFlag != pCluster->IsAvailable()) continue;

      const HitType hitType(LArClusterHelper::GetClusterHitType(pCluster));

      if ((TPC_VIEW_U != hitType) && (TPC_VIEW_V != hitType) && (TPC_VIEW_W != hitType)) continue;

      ClusterList& target((TPC_VIEW_U == hitType) ?
                            clusterListU :
  ...
  void ClusterMopUpBaseAlgorithm::MakeClusterMerges(
    const ClusterAssociationMap& clusterAssociationMap) const
  {
    ...
    for (const Cluster* const pRemnantCluster : sortedRemnantClusters) {
      ...
      PANDORA_THROW_RESULT_IF(
        ...
        PandoraContentApi::MergeAndDeleteClusters(*this,
                                                  pBestPfoCluster,
                                                  pRemnantCluster,
                                                  this->GetListName(pBestPfoCluster),
                                                  this->GetListName(pRemnantCluster)));
    }
  }
