...
namespace lar_content {
  ...
  void IsolatedClusterMopUpAlgorithm::ClusterMopUp(const ClusterList& pfoClusters,
    ...
    for (const CaloHit* pCaloHit : sortedCaloHitList) {
      if (m_addHitsAsIsolated) {
        PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                                !=,
                                PandoraContentApi::AddIsolatedToCluster(
        ...
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          PandoraContentApi::AddToCluster(*this, caloHitToClusterMap.at(pCaloHit), pCaloHit));
      }
    }
  }
  ...
  void IsolatedClusterMopUpAlgorithm::DissolveClustersToHits(const ClusterList& clusterList,
                                                             CaloHitList& caloHitList) const
  {
    for (const Cluster* const pRemnantCluster : clusterList) {
      if (pRemnantCluster->GetNCaloHits() < m_maxCaloHitsInCluster) {
        ...
        PANDORA_THROW_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, PandoraContentApi::Delete(*this, pRemnantCluster, listNameR));
      }
    }
  }
  ...
  void IsolatedClusterMopUpAlgorithm::GetCaloHitToClusterMap(
    ...
    CaloHitToClusterMap& caloHitToClusterMap) const
  {
    ...
    for (const CaloHit* const pCaloHit : caloHitList) {
      ...
      const HitKDNode2D targetHit(
        pCaloHit, pCaloHit->GetPositionVector().GetX(), pCaloHit->GetPositionVector().GetZ());
      kdTree.findNearestNeighbour(targetHit, pResultHit, resultDistance);

      if (pResultHit && (resultDistance < m_maxHitClusterDistance))
        (void)caloHitToClusterMap.insert(
          CaloHitToClusterMap::value_type(pCaloHit, hitToParentClusterMap.at(pResultHit->data)));
    }
  }
  ...
  StatusCode IsolatedClusterMopUpAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxCaloHitsInCluster", m_maxCaloHitsInCluster));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxHitClusterDistance", m_maxHitClusterDistance));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "AddHitsAsIsolated", m_addHitsAsIsolated));
