...
namespace lar_content {
  ...
  void NearbyClusterMopUpAlgorithm::ClusterMopUp(const ClusterList& pfoClusters,
                                                 const ClusterList& remnantClusters) const
  {
    ...
    for (const Cluster* const pClusterP : sortedPfoClusters) {
      ...
      const float innerPV(
        (vertexPosition2D - pClusterP->GetCentroid(pClusterP->GetInnerPseudoLayer()))
          .GetMagnitude());
      const float outerPV(
        (vertexPosition2D - pClusterP->GetCentroid(pClusterP->GetOuterPseudoLayer()))
          .GetMagnitude());

      for (const Cluster* const pClusterR : sortedRemnantClusters) {
        if (pClusterR->GetNCaloHits() < m_minHitsInCluster) continue;

        const float innerRV(
          (vertexPosition2D - pClusterR->GetCentroid(pClusterR->GetInnerPseudoLayer()))
            .GetMagnitude());
        const float outerRV(
          (vertexPosition2D - pClusterR->GetCentroid(pClusterR->GetOuterPseudoLayer()))
            .GetMagnitude());
        ...
        const float innerRP(LArClusterHelper::GetClosestDistance(
          pClusterR->GetCentroid(pClusterR->GetInnerPseudoLayer()), pClusterP));
        const float outerRP(LArClusterHelper::GetClosestDistance(
          pClusterR->GetCentroid(pClusterR->GetOuterPseudoLayer()), pClusterP));

        const float minSeparation(std::min(innerRP, outerRP));

        if (minSeparation > m_minClusterSeparation) continue;
        ...
        const float figureOfMerit((minSeparation < m_touchingDistance) ? 1.f / m_touchingDistance :
                                                                         1.f / minSeparation);

        if (!associationDetails.insert(AssociationDetails::value_type(pClusterP, figureOfMerit))
               .second)
  ...
  StatusCode NearbyClusterMopUpAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "VertexProximity", m_vertexProximity));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinClusterSeparation", m_minClusterSeparation));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "TouchingDistance", m_touchingDistance));
