...
namespace lar_content {
  ...
  void BoundedClusterMopUpAlgorithm::ClusterMopUp(const ClusterList& pfoClusters,
                                                  const ClusterList& remnantClusters) const
  {
    ...
    for (const Cluster* const pPfoCluster : sortedPfoClusters) {
      ...
      if (clusterHitList.size() <= 3) continue;
      try {
        const TwoDSlidingShowerFitResult fitResult(
        ...
        for (const Cluster* const pRemnantCluster : sortedRemnantClusters) {
          const float boundedFraction(
            this->GetBoundedFraction(pRemnantCluster, xSampling, showerPositionMap));
        ...
        if (e.GetStatusCode() != STATUS_CODE_NOT_INITIALIZED) throw e;
      }
    }
    ...
    ShowerPositionMap& showerPositionMap) const
  {
    for (int n = 0; n <= xSampling.m_nPoints; ++n) {
      const float x(xSampling.m_minX + (xSampling.m_maxX - xSampling.m_minX) *
                                         static_cast<float>(n) /
                                         static_cast<float>(xSampling.m_nPoints));
      ...
      try {
        ...
        showerPositionMap.insert(ShowerPositionMap::value_type(
          xBin, ShowerExtent(x, edgePositions.front(), edgePositions.back())));
      }
    ...
    const ShowerPositionMap& showerPositionMap) const
  {
    if (((xSampling.m_maxX - xSampling.m_minX) < std::numeric_limits<float>::epsilon()) ||
        (0 >= xSampling.m_nPoints) || (0 == pCluster->GetNCaloHits())) {
    ...
    for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
      ...
      for (CaloHitList::const_iterator hIter = iter->second->begin(),
        ...
        try {
          ...
          if ((showerPositionMap.end() != positionIter) &&
