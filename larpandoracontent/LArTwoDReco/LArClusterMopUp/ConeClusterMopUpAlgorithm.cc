...
namespace lar_content {
  ...
  void ConeClusterMopUpAlgorithm::ClusterMopUp(const ClusterList& pfoClusters,
                                               const ClusterList& remnantClusters) const
  {
    ...
    std::sort(
      sortedRemnantClusters.begin(), sortedRemnantClusters.end(), LArClusterHelper::SortByNHits);

    for (const Cluster* const pPfoCluster : sortedPfoClusters) {
      try {
        const TwoDSlidingShowerFitResult showerFitResult(
          pPfoCluster, m_slidingFitWindow, slidingFitPitch, m_showerEdgeMultiplier);

        const LayerFitResultMap& layerFitResultMapS(
          showerFitResult.GetShowerFitResult().GetLayerFitResultMap());
        const LayerFitResultMap& layerFitResultMapP(
          showerFitResult.GetPositiveEdgeFitResult().GetLayerFitResultMap());
        const LayerFitResultMap& layerFitResultMapN(
          showerFitResult.GetNegativeEdgeFitResult().GetLayerFitResultMap());

        if (layerFitResultMapS.size() < 2) continue;

        // Cone direction
        CartesianVector minLayerPositionOnAxis(0.f, 0.f, 0.f),
          maxLayerPositionOnAxis(0.f, 0.f, 0.f);
        showerFitResult.GetShowerFitResult().GetGlobalPosition(
          layerFitResultMapS.begin()->second.GetL(), 0.f, minLayerPositionOnAxis);
        showerFitResult.GetShowerFitResult().GetGlobalPosition(
          layerFitResultMapS.rbegin()->second.GetL(), 0.f, maxLayerPositionOnAxis);
        ...
        for (LayerFitResultMap::const_iterator iterS = layerFitResultMapS.begin();
             iterS != layerFitResultMapS.end();
             ++iterS) {
        ...
        if (coordinateListP.empty() || coordinateListN.empty()) continue;

        std::sort(coordinateListP.begin(),
                  coordinateListP.end(),
                  ConeClusterMopUpAlgorithm::SortCoordinates);
        std::sort(coordinateListN.begin(),
                  coordinateListN.end(),
                  ConeClusterMopUpAlgorithm::SortCoordinates);

        const Coordinate maxP(coordinateListP.at(m_coneAngleCentile * coordinateListP.size()));
        const Coordinate minP(vertexAtMinL ?
                                Coordinate(layerFitResultMapP.begin()->second.GetL(),
                                           layerFitResultMapP.begin()->second.GetFitT()) :
                                Coordinate(layerFitResultMapP.rbegin()->second.GetL(),
                                           layerFitResultMapP.rbegin()->second.GetFitT()));

        const Coordinate maxN(
          coordinateListN.at((1.f - m_coneAngleCentile) * coordinateListN.size()));
        const Coordinate minN(vertexAtMinL ?
                                Coordinate(layerFitResultMapN.begin()->second.GetL(),
                                           layerFitResultMapN.begin()->second.GetFitT()) :
                                Coordinate(layerFitResultMapN.rbegin()->second.GetL(),
                                           layerFitResultMapN.rbegin()->second.GetFitT()));

        const float minL(layerFitResultMapS.begin()->second.GetL());
        const float maxL(minL + m_maxConeLengthMultiplier *
                                  std::fabs(layerFitResultMapS.rbegin()->second.GetL() - minL));
        ...
        for (const Cluster* const pRemnantCluster : sortedRemnantClusters) {
          ...
          for (OrderedCaloHitList::const_iterator iter = orderedCaloHitList.begin(),
            ...
            for (CaloHitList::const_iterator hIter = iter->second->begin(),
                 ...
                 ++hIter) {
              float rL(0.f), rT(0.f);
              showerFitResult.GetShowerFitResult().GetLocalPosition(
                (*hIter)->GetPositionVector(), rL, rT);

              if ((rL < minL) || (rL > maxL)) continue;

              const float rTP(minP.second + (rL - minP.first) * ((maxP.second - minP.second) /
                                                                 (maxP.first - minP.first)));
              const float rTN(minN.second + (rL - minN.first) * ((maxN.second - minN.second) /
                                                                 (maxN.first - minN.first)));

              if ((rT > rTP) || (rT < rTN)) continue;
          ...
          const float boundedFraction(
            (nHits > 0) ? static_cast<float>(nMatchedHits) / static_cast<float>(nHits) : 0.f);

          if (boundedFraction < m_minBoundedFraction) continue;

          AssociationDetails& associationDetails(clusterAssociationMap[pRemnantCluster]);

          if (!associationDetails
                 .insert(AssociationDetails::value_type(pPfoCluster, boundedFraction))
                 .second)
  ...
  StatusCode ConeClusterMopUpAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ShowerEdgeMultiplier", m_showerEdgeMultiplier));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinBoundedFraction", m_minBoundedFraction));
