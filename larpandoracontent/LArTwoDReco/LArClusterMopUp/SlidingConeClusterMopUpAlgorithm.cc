...
namespace lar_content {
  ...
  StatusCode SlidingConeClusterMopUpAlgorithm::Run()
  {
    ...
    if (m_useVertex && !pVertex) {
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
        std::cout << "SlidingConeClusterMopUpAlgorithm - interaction vertex not available for use."
                  << std::endl;
  ...
  void SlidingConeClusterMopUpAlgorithm::GetThreeDClusters(ClusterVector& clusters3D,
      ...
      for (const Pfo* const pPfo : *pPfoList) {
        ...
        for (const Cluster* const pCluster3D : pfoClusters3D) {
          ...
          if (LArPfoHelper::IsShower(pPfo) &&
              (pCluster3D->GetNCaloHits() < m_minHitsToConsider3DShower))
            continue;
  ...
  void SlidingConeClusterMopUpAlgorithm::GetAvailableTwoDClusters(
    ClusterVector& availableClusters2D) const
  {
    for (const std::string& clusterListName : m_daughterListNames) {
      ...
      for (const Cluster* const pCluster : *pClusterList) {
        ...
        if (pCluster->GetNCaloHits() > m_maxHitsToConsider2DCluster) continue;
    ...
    std::sort(
      availableClusters2D.begin(), availableClusters2D.end(), LArClusterHelper::SortByNHits);
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void SlidingConeClusterMopUpAlgorithm::GetClusterMergeMap(
    const Vertex* const pVertex,
    const ClusterVector& clusters3D,
    const ClusterVector& availableClusters2D,
    ...
    for (const Cluster* const pShowerCluster : clusters3D) {
      ...
      try {
        ...
        slidingConeFitResult3D.GetSimpleConeList(
          m_nConeFitLayers, m_nConeFits, coneSelection, simpleConeList3D);
      }
      ...
      for (const Cluster* const pNearbyCluster2D : availableClusters2D) {
        ...
        for (const SimpleCone& simpleCone3D : simpleConeList3D) {
          const CartesianVector coneBaseCentre3D(simpleCone3D.GetConeApex() +
                                                 simpleCone3D.GetConeDirection() * coneLength3D);
          const CartesianVector coneApex2D(LArGeometryHelper::ProjectPosition(
            this->GetPandora(), simpleCone3D.GetConeApex(), hitType));
          const CartesianVector coneBaseCentre2D(
            LArGeometryHelper::ProjectPosition(this->GetPandora(), coneBaseCentre3D, hitType));

          const CartesianVector apexToBase2D(coneBaseCentre2D - coneApex2D);
          const SimpleCone simpleCone2D(coneApex2D,
                                        apexToBase2D.GetUnitVector(),
                                        apexToBase2D.GetMagnitude(),
                                        m_coneTanHalfAngle);
          const ClusterMerge clusterMerge(pShowerCluster,
                                          simpleCone2D.GetBoundedHitFraction(pNearbyCluster2D),
                                          simpleCone2D.GetMeanRT(pNearbyCluster2D));

          if (clusterMerge < bestClusterMerge) bestClusterMerge = clusterMerge;
        }

        if (bestClusterMerge.GetParentCluster() &&
            (bestClusterMerge.GetBoundedFraction() > m_coneBoundedFraction))
  ...
  void SlidingConeClusterMopUpAlgorithm::MakeClusterMerges(
    ...
    const ClusterMergeMap& clusterMergeMap) const
  {
    ...
    for (ClusterVector::const_reverse_iterator rIter = daughterClusters.rbegin(),
      ...
      const Cluster* const pParentCluster3D(
        clusterMergeMap.at(pDaughterCluster).at(0).GetParentCluster());

      const Pfo* const pParentPfo(clusterToPfoMap.at(pParentCluster3D));
      const Cluster* const pParentCluster(
        this->GetParentCluster(pParentPfo->GetClusterList(), daughterHitType));
      ...
      if (pParentCluster) {
        PANDORA_THROW_RESULT_IF(
          ...
          PandoraContentApi::MergeAndDeleteClusters(*this,
                                                    pParentCluster,
                                                    pDaughterCluster,
                                                    this->GetListName(pParentCluster),
                                                    this->GetListName(pDaughterCluster)));
      }
      else {
        PANDORA_THROW_RESULT_IF(STATUS_CODE_SUCCESS,
                                !=,
                                PandoraContentApi::AddToPfo(*this, pParentPfo, pDaughterCluster));
      }
    }
  }
  ...
  bool SlidingConeClusterMopUpAlgorithm::ClusterMerge::operator<(const ClusterMerge& rhs) const
  {
    ...
    if (std::fabs(this->GetBoundedFraction() - rhs.GetBoundedFraction()) >
  ...
  StatusCode SlidingConeClusterMopUpAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConeTanHalfAngle", m_coneTanHalfAngle));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "ConeBoundedFraction", m_coneBoundedFraction));
