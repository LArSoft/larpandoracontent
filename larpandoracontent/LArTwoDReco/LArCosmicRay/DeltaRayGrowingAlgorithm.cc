...
namespace lar_content {
  ...
  void DeltaRayGrowingAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
  ...
  void DeltaRayGrowingAlgorithm::GetListOfSeedClusters(const ClusterVector& inputClusters,
                                                       ClusterVector& seedClusters) const
  {
    if (inputClusters.empty()) return;
    ...
    for (const Cluster* const pCluster : parentClusters) {
      if (LArClusterHelper::GetLengthSquared(pCluster) <
          m_maxSeedClusterLength * m_maxSeedClusterLength)
    ...
    for (const Cluster* const pCluster : inputClusters) {
      if (pCluster->GetNCaloHits() < m_minSeedClusterCaloHits) continue;

      const float parentDistance(parentClusters.empty() ?
                                   std::numeric_limits<float>::max() :
                                   LArClusterHelper::GetClosestDistance(pCluster, parentClusters));

      if (parentDistance > m_maxSeedClusterDisplacement) continue;

      const float daughterDistance(daughterClusters.empty() ? std::numeric_limits<float>::max() :
                                                              LArClusterHelper::GetClosestDistance(
                                                                pCluster, daughterClusters));

      if (daughterDistance < m_maxSeedClusterDisplacement) continue;
  ...
  void DeltaRayGrowingAlgorithm::GetPfos(const std::string inputPfoListName,
  ...
  StatusCode DeltaRayGrowingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinSeedClusterCaloHits", m_minSeedClusterCaloHits));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxSeedClusterLength", m_maxSeedClusterLength));
