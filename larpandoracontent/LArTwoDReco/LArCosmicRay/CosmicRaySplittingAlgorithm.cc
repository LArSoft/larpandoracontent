...
namespace lar_content {
  ...
  CosmicRaySplittingAlgorithm::CosmicRaySplittingAlgorithm()
    ...
    , m_maxLongitudinalDisplacementSquared(m_maxLongitudinalDisplacement *
  ...
  StatusCode CosmicRaySplittingAlgorithm::Run()
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetCurrentList(*this, pClusterList));
    ...
    for (ClusterVector::const_iterator bIter = clusterVector.begin(),
                                       bIterEnd1 = clusterVector.end();
         bIter != bIterEnd1;
         ++bIter) {
      if (splitClusters.count(*bIter) > 0) continue;

      TwoDSlidingFitResultMap::const_iterator bFitIter = slidingFitResultMap.find(*bIter);

      if (slidingFitResultMap.end() == bFitIter) continue;
      ...
      if (STATUS_CODE_SUCCESS !=
          this->FindBestSplitPosition(
            branchSlidingFitResult, splitPosition, splitDirection1, splitDirection2))
        continue;
      ...
      for (ClusterVector::const_iterator rIter = clusterVector.begin(),
                                         rIterEnd = clusterVector.end();
           rIter != rIterEnd;
           ++rIter) {
        if (splitClusters.count(*rIter) > 0) continue;

        TwoDSlidingFitResultMap::const_iterator rFitIter = slidingFitResultMap.find(*rIter);

        if (slidingFitResultMap.end() == rFitIter) continue;
        ...
        if (STATUS_CODE_SUCCESS != this->ConfirmSplitPosition(branchSlidingFitResult,
                                                              replacementSlidingFitResult,
                                                              splitPosition,
                                                              splitDirection1,
                                                              splitDirection2,
                                                              lengthSquared1,
      ...
      if (NULL == pReplacementCluster1 && NULL == pReplacementCluster2) continue;

      const Cluster* const pBranchCluster = branchSlidingFitResult.GetCluster();

      // Crossed tracks
      if (pReplacementCluster1 && pReplacementCluster2) {
        if (!(this->IdentifyCrossedTracks(
              pBranchCluster, pReplacementCluster1, pReplacementCluster2, splitPosition)))
          continue;

        PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS,
                                 !=,
                                 this->PerformDoubleSplit(pBranchCluster,
                                                          pReplacementCluster1,
                                                          pReplacementCluster2,
                                                          splitPosition,
                                                          splitDirection1,
                                                          splitDirection2));
      }
      // Single scatter
        ...
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          this->PerformSingleSplit(
            pBranchCluster, pReplacementCluster1, splitPosition, splitDirection1, splitDirection2));
      }
        ...
        PANDORA_RETURN_RESULT_IF(
          ...
          !=,
          this->PerformSingleSplit(
            pBranchCluster, pReplacementCluster2, splitPosition, splitDirection2, splitDirection1));
      }

      // Choose not to re-use clusters (for now)
      if (pReplacementCluster1) splitClusters.insert(pReplacementCluster1);

      if (pReplacementCluster2) splitClusters.insert(pReplacementCluster2);
  ...
  void CosmicRaySplittingAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
                                                           ClusterVector& clusterVector) const
  {
    for (ClusterList::const_iterator iter = pClusterList->begin(), iterEnd = pClusterList->end();
         iter != iterEnd;
         ++iter) {
  ...
  void CosmicRaySplittingAlgorithm::BuildSlidingFitResultMap(
  ...
  StatusCode CosmicRaySplittingAlgorithm::FindBestSplitPosition(
    const TwoDSlidingFitResult& branchSlidingFitResult,
    CartesianVector& splitPosition,
    CartesianVector& splitDirection1,
    CartesianVector& splitDirection2) const
  {
    ...
    for (unsigned int n = 0; n < nSamplingPoints; ++n) {
      ...
      CartesianVector centralPosition(0.f, 0.f, 0.f), forwardDirection(0.f, 0.f, 0.f),
        backwardDirection(0.f, 0.f, 0.f);
      ...
      if ((STATUS_CODE_SUCCESS !=
          ...
          (STATUS_CODE_SUCCESS != branchSlidingFitResult.GetGlobalFitDirection(
                                    rL - halfWindowLength, backwardDirection))) {
        continue;
      }
      ...
      if (cosTheta < splitCosTheta) {
        ...
        if ((STATUS_CODE_SUCCESS !=
            ...
            (STATUS_CODE_SUCCESS != branchSlidingFitResult.GetGlobalFitPosition(
                                      rL - halfWindowLength, backwardPosition))) {
          continue;
        }
        ...
        splitDirection1 = (forwardDirection.GetDotProduct(forwardVectorOutwards) > 0.f) ?
                            forwardDirection :
                            forwardDirection * -1.f;
        splitDirection2 = (backwardDirection.GetDotProduct(backwardVectorOutwards) > 0.f) ?
                            backwardDirection :
    ...
    if (!foundSplit) return STATUS_CODE_NOT_FOUND;
  ...
  StatusCode CosmicRaySplittingAlgorithm::ConfirmSplitPosition(
    const TwoDSlidingFitResult& branchSlidingFitResult,
    ...
    float& bestLengthSquared2) const
  {
    ...
    for (unsigned int iFwd = 0; iFwd < 2; ++iFwd) {
      const CartesianVector pAxis((0 == iFwd) ? (maxPosition - minPosition) :
      ...
      const CartesianVector vtxDirection(
        (0 == iFwd) ? (pAxis.GetDotProduct(minDirection) > 0 ? minDirection : minDirection * -1.f) :
      ...
      const float vtxDisplacement(
        LArClusterHelper::GetClosestDistance(vtxPosition, branchSlidingFitResult.GetCluster()));
      const float endDisplacement(
        LArClusterHelper::GetClosestDistance(endPosition, branchSlidingFitResult.GetCluster()));
      ...
      if (vtxDisplacement > endDisplacement) continue;

      if (std::min(lengthSquared, std::min(lengthSquared1, lengthSquared2)) >
      ...
      LArPointingClusterHelper::GetImpactParameters(
        vtxPosition, vtxDirection, splitPosition, impactL, impactT);
      ...
      if (vtxDirection.GetDotProduct(branchDirection1) >
          std::max(m_minCosMergingAngle, cosSplittingAngle)) {
      ...
      if (vtxDirection.GetDotProduct(branchDirection2) >
          std::max(m_minCosMergingAngle, cosSplittingAngle)) {
    ...
    if (!foundMatch) return STATUS_CODE_NOT_FOUND;
  ...
  StatusCode CosmicRaySplittingAlgorithm::PerformSingleSplit(
    const Cluster* const pBranchCluster,
    const Cluster* const pReplacementCluster,
    const CartesianVector& splitPosition,
    const CartesianVector& forwardDirection,
    const CartesianVector& backwardDirection) const
  {
    CaloHitList caloHitListToMove;
    this->GetCaloHitListToMove(pBranchCluster,
                               pReplacementCluster,
                               splitPosition,
                               forwardDirection,
                               backwardDirection,
  ...
  StatusCode CosmicRaySplittingAlgorithm::PerformDoubleSplit(
    const Cluster* const pBranchCluster,
    const Cluster* const pReplacementCluster1,
    const Cluster* const pReplacementCluster2,
    const CartesianVector& splitPosition,
    const CartesianVector& splitDirection1,
    const CartesianVector& splitDirection2) const
  {
    CaloHitList caloHitListToMove1, caloHitListToMove2;
    this->GetCaloHitListsToMove(pBranchCluster,
                                splitPosition,
                                splitDirection1,
                                splitDirection2,
                                caloHitListToMove1,
    ...
    if (caloHitListToKeep1.empty()) return STATUS_CODE_FAILURE;

    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      this->SplitCluster(pBranchCluster, pReplacementCluster1, caloHitListToMove1));
  ...
  void CosmicRaySplittingAlgorithm::GetCaloHitListToMove(const Cluster* const pBranchCluster,
                                                         const Cluster* const pReplacementCluster,
                                                         const CartesianVector& splitPosition,
                                                         const CartesianVector& forwardDirection,
  ...
  {
    const CartesianVector forwardPosition(
      LArClusterHelper::GetClosestPosition(splitPosition, pBranchCluster));
    const CartesianVector vtxPosition(
      LArClusterHelper::GetClosestPosition(splitPosition, pReplacementCluster));
    ...
    for (CaloHitList::const_iterator iter = caloHitListToSort.begin(),
                                     iterEnd = caloHitListToSort.end();
         iter != iterEnd;
         ++iter) {
      ...
      else if (forwardDirection.GetDotProduct(pCaloHit->GetPositionVector() - vtxPosition) >
               -1.25f) {
    ...
    for (CaloHitList::const_iterator iter = caloHitListToCheck.begin(),
                                     iterEnd = caloHitListToCheck.end();
         iter != iterEnd;
         ++iter) {
      const CaloHit* const pCaloHit = *iter;

      if (vtxDirection.GetCrossProduct(pCaloHit->GetPositionVector() - forwardPosition)
            .GetMagnitude() >
          backwardDirection.GetCrossProduct(pCaloHit->GetPositionVector() - forwardPosition)
            .GetMagnitude())
        continue;

      const float lengthSquared(
        (pCaloHit->GetPositionVector() - vtxPosition).GetMagnitudeSquared());

      if (lengthSquared < closestLengthSquared) closestLengthSquared = lengthSquared;
    }
    ...
    for (CaloHitList::const_iterator iter = caloHitListToCheck.begin(),
      ...
      if ((pCaloHit->GetPositionVector() - vtxPosition).GetMagnitudeSquared() >=
  ...
  void CosmicRaySplittingAlgorithm::GetCaloHitListsToMove(const Cluster* const pBranchCluster,
                                                          const CartesianVector& splitPosition,
                                                          const CartesianVector& splitDirection1,
                                                          const CartesianVector& splitDirection2,
                                                          CaloHitList& caloHitListToMove1,
                                                          CaloHitList& caloHitListToMove2) const
  {
    ...
    for (CaloHitList::const_iterator iter = caloHitListToSort.begin(),
      ...
      const float displacement1(
        splitDirection1.GetDotProduct(pCaloHit->GetPositionVector() - splitPosition));
      const float displacement2(
        splitDirection2.GetDotProduct(pCaloHit->GetPositionVector() - splitPosition));

      if (displacement1 > displacement2) { caloHitListToMove1.push_back(pCaloHit); }
      else {
  ...
  bool CosmicRaySplittingAlgorithm::IdentifyCrossedTracks(
    const Cluster* const pBranchCluster,
    const Cluster* const pReplacementCluster1,
    const Cluster* const pReplacementCluster2,
    const pandora::CartesianVector& splitPosition) const
  {
    ...
    const CartesianVector replacementVertex1(
      LArClusterHelper::GetClosestPosition(splitPosition, pReplacementCluster1));
    const CartesianVector replacementVertex2(
      LArClusterHelper::GetClosestPosition(splitPosition, pReplacementCluster2));

    if ((replacementVertex2 - replacementVertex1).GetMagnitudeSquared() >
        (branchVertex2 - branchVertex1).GetMagnitudeSquared())
      return false;
  ...
  StatusCode CosmicRaySplittingAlgorithm::GetCaloHitListToKeep(const Cluster* const pBranchCluster,
                                                               const CaloHitList& caloHitListToMove,
                                                               CaloHitList& caloHitListToKeep) const
  {
    if (caloHitListToMove.empty()) return STATUS_CODE_FAILURE;

    CaloHitList caloHitList;
    pBranchCluster->GetOrderedCaloHitList().FillCaloHitList(caloHitList);

    for (CaloHitList::const_iterator iter = caloHitList.begin(), iterEnd = caloHitList.end();
         iter != iterEnd;
         ++iter) {
      const CaloHit* const pCaloHit = *iter;

      if (caloHitListToMove.end() ==
          std::find(caloHitListToMove.begin(), caloHitListToMove.end(), pCaloHit))
  ...
  StatusCode CosmicRaySplittingAlgorithm::SplitCluster(const Cluster* const pBranchCluster,
                                                       const Cluster* const pReplacementCluster,
                                                       const CaloHitList& caloHitListToMove) const
  {
    if (caloHitListToMove.empty()) return STATUS_CODE_FAILURE;

    for (CaloHitList::const_iterator iter = caloHitListToMove.begin(),
                                     iterEnd = caloHitListToMove.end();
         iter != iterEnd;
         ++iter) {
      const CaloHit* const pCaloHit = *iter;
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::RemoveFromCluster(*this, pBranchCluster, pCaloHit));
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        PandoraContentApi::AddToCluster(*this, pReplacementCluster, pCaloHit));
    }
  ...
  StatusCode CosmicRaySplittingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "SamplingPitch", m_samplingPitch));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxCosSplittingAngle", m_maxCosSplittingAngle));

    PANDORA_RETURN_RESULT_IF_AND_IF(
      STATUS_CODE_SUCCESS,
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinCosMergingAngle", m_minCosMergingAngle));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    ...
                                    XmlHelper::ReadValue(xmlHandle,
                                                         "MaxLongitudinalDisplacement",
                                                         m_maxLongitudinalDisplacement));
    m_maxLongitudinalDisplacementSquared =
      m_maxLongitudinalDisplacement * m_maxLongitudinalDisplacement;
