...
namespace lar_content {
  ...
  void CosmicRayExtensionAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
    ...
    ClusterAssociationMatrix& clusterAssociationMatrix) const
  {
    ...
    try {
      LArPointingClusterHelper::GetClosestVertices(
        clusterI, clusterJ, clusterVertexI, clusterVertexJ);
    }
    catch (StatusCodeException&) {
      return;
    }
    ...
    const CartesianVector endI(clusterVertexI.IsInnerVertex() ?
                                 clusterI.GetOuterVertex().GetPosition() :
                                 clusterI.GetInnerVertex().GetPosition());
    const CartesianVector endJ(clusterVertexJ.IsInnerVertex() ?
                                 clusterJ.GetOuterVertex().GetPosition() :
                                 clusterJ.GetInnerVertex().GetPosition());
    ...
    if (distanceSquaredIJ > m_maxLongitudinalDisplacement * m_maxLongitudinalDisplacement) return;
    ...
    if (cosTheta < cosThetaCut) return;
    ...
    if (rms > rmsCut) return;

    // Record the association
    const ClusterAssociation::VertexType vertexTypeI(
      clusterVertexI.IsInnerVertex() ? ClusterAssociation::INNER : ClusterAssociation::OUTER);
    const ClusterAssociation::VertexType vertexTypeJ(
      clusterVertexJ.IsInnerVertex() ? ClusterAssociation::INNER : ClusterAssociation::OUTER);
    ...
    (void)clusterAssociationMatrix[pClusterI].insert(ClusterAssociationMap::value_type(
      pClusterJ, ClusterAssociation(vertexTypeI, vertexTypeJ, associationType, lengthSquaredJ)));

    (void)clusterAssociationMatrix[pClusterJ].insert(ClusterAssociationMap::value_type(
      pClusterI, ClusterAssociation(vertexTypeJ, vertexTypeI, associationType, lengthSquaredI)));
  }

  //------------------------------------------------------------------------------------------------------------------------------------------

  void CosmicRayExtensionAlgorithm::FillClusterMergeMap(
  ...
  float CosmicRayExtensionAlgorithm::CalculateRms(const Cluster* const pCluster,
                                                  ...
                                                  const CartesianVector& direction) const
  {
    ...
    for (CaloHitList::const_iterator iter = caloHitList.begin(), iterEnd = caloHitList.end();
      ...
      const CartesianVector predictedPosition(
        position + direction * direction.GetDotProduct(hitPosition - position));
  ...
  StatusCode CosmicRayExtensionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MinSeedClusterLength", m_minSeedClusterLength));
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxAverageRms", m_maxAverageRms));
