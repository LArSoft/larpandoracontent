...
namespace lar_content {
  ...
  void DeltaRayExtensionAlgorithm::GetListOfCleanClusters(const ClusterList* const pClusterList,
  ...
  void DeltaRayExtensionAlgorithm::FillClusterAssociationMatrix(
    ...
    ClusterAssociationMatrix& clusterAssociationMatrix) const
  {
    ...
    for (const Cluster* const pParentCluster : clusterVector) {
      for (const Cluster* const pDaughterCluster : clusterVector) {
        ...
        this->FillClusterAssociationMatrix(pParentCluster,
                                           pDaughterCluster,
                                           innerCoordinateMap,
                                           outerCoordinateMap,
  ...
  void DeltaRayExtensionAlgorithm::GetExtremalCoordinatesFromCache(
    ...
    CartesianVector& outerCoordinate) const
  {
    ...
    if ((innerCoordinateMap.end() == innerIter) || (outerCoordinateMap.end() == outerIter)) {
      ...
      (void)innerCoordinateMap.insert(
        ClusterToCoordinateMap::value_type(pCluster, innerCoordinate));
      (void)outerCoordinateMap.insert(
        ClusterToCoordinateMap::value_type(pCluster, outerCoordinate));
    }
    else {
    ...
    ClusterAssociationMatrix& clusterAssociationMatrix) const
  {
    ...
    if (!PandoraContentApi::IsAvailable(*this, pDaughterCluster)) return;
    ...
    this->GetExtremalCoordinatesFromCache(
      pParentCluster, innerCoordinateMap, outerCoordinateMap, innerCoordinateP, outerCoordinateP);

    CartesianVector innerCoordinateD(0.f, 0.f, 0.f), outerCoordinateD(0.f, 0.f, 0.f);
    this->GetExtremalCoordinatesFromCache(
      pDaughterCluster, innerCoordinateMap, outerCoordinateMap, innerCoordinateD, outerCoordinateD);
    ...
    for (unsigned int useInnerD = 0; useInnerD < 2; ++useInnerD) {
      ...
      if (daughterLengthSquared > m_maxClusterLength * m_maxClusterLength) continue;

      const CartesianVector projectedVertex(
        LArClusterHelper::GetClosestPosition(daughterVertex, pParentCluster));

      const float daughterVertexDistanceSquared(
        (projectedVertex - daughterVertex).GetMagnitudeSquared());
      ...
      if (daughterVertexDistanceSquared >
          std::min(m_maxLongitudinalDisplacement * m_maxLongitudinalDisplacement,
                   std::min(daughterEndDistanceSquared, daughterLengthSquared)))
        continue;

      const ClusterAssociation::VertexType daughterVertexType(
        useInnerD == 1 ? ClusterAssociation::INNER : ClusterAssociation::OUTER);
      ...
      for (unsigned int useInnerP = 0; useInnerP < 2; ++useInnerP) {
        ...
        const float parentVertexDistanceSquared(
          (projectedVertex - parentVertex).GetMagnitudeSquared());
        ...
        if (parentVertexDistanceSquared < parentEndDistanceSquared)
          parentVertexType =
            (useInnerP == 1 ? ClusterAssociation::INNER : ClusterAssociation::OUTER);

        // Parent cluster must be available and below a length cut for a strong association
        if (!PandoraContentApi::IsAvailable(*this, pParentCluster) ||
            parentLengthSquared > m_maxClusterLength * m_maxClusterLength)
          continue;

        // Require an end-to-end join between parent and daughter cluster
        if (parentVertexDistanceSquared >
            std::min(m_maxTransverseDisplacement * m_maxTransverseDisplacement,
        ...
        const float forwardDistance(
          daughterDirection.GetDotProduct((daughterVertex - projectedVertex)));
        const float sidewaysDistance(
          daughterDirection.GetCrossProduct((daughterVertex - projectedVertex)).GetMagnitude());

        if (forwardDistance < 0.f || forwardDistance > m_maxLongitudinalDisplacement ||
            sidewaysDistance > m_maxTransverseDisplacement)
          continue;

        if (-parentDirection.GetDotProduct(daughterDirection) < 0.25f) continue;
      ...
      if (parentVertexType > ClusterAssociation::UNDEFINED) {
        (void)clusterAssociationMatrix[pParentCluster].insert(ClusterAssociationMap::value_type(
          pDaughterCluster,
          ClusterAssociation(
            parentVertexType, daughterVertexType, associationType, figureOfMerit)));
      }
    }
  }
  ...
  void DeltaRayExtensionAlgorithm::FillClusterMergeMap(
    const ClusterAssociationMatrix& parentToDaughterMatrix,
    ClusterMergeMap& clusterMergeMap) const
  {
    ...
    std::sort(
      sortedParentClusters.begin(), sortedParentClusters.end(), LArClusterHelper::SortByNHits);

    for (const Cluster* const pParentCluster : sortedParentClusters) {
      const ClusterAssociationMap& daughterToAssociationMap(
        parentToDaughterMatrix.at(pParentCluster));
      ...
      std::sort(sortedLocalDaughterClusters.begin(),
                sortedLocalDaughterClusters.end(),
      ...
      for (const Cluster* const pDaughterCluster : sortedLocalDaughterClusters) {
        ...
        (void)daughterToParentMatrix[pDaughterCluster].insert(
          ClusterAssociationMap::value_type(pParentCluster, clusterAssociation));
      }
    }
    ...
    std::sort(
      sortedDaughterClusters.begin(), sortedDaughterClusters.end(), LArClusterHelper::SortByNHits);
    ...
    for (const Cluster* const pDaughterCluster : sortedDaughterClusters) {
      const ClusterAssociationMap& parentToAssociationMap(
        daughterToParentMatrix.at(pDaughterCluster));
      ...
      std::sort(sortedLocalParentClusters.begin(),
                sortedLocalParentClusters.end(),
                LArClusterHelper::SortByNHits);
      ...
      if (pBestInner) {
        ...
        (void)reducedParentToDaughterMatrix[pBestInner].insert(
          ClusterAssociationMap::value_type(pDaughterCluster, bestAssociationInner));
      }
      ...
      if (pBestOuter) {
        ...
        (void)reducedParentToDaughterMatrix[pBestOuter].insert(
          ClusterAssociationMap::value_type(pDaughterCluster, bestAssociationOuter));
      }
    }
    ...
    std::sort(sortedReducedParentClusters.begin(),
              sortedReducedParentClusters.end(),
              LArClusterHelper::SortByNHits);

    for (const Cluster* const pParentCluster : sortedReducedParentClusters) {
      const ClusterAssociationMap& daughterToAssociationMap(
        reducedParentToDaughterMatrix.at(pParentCluster));
      ...
      if (pBestInner) {
        ...
        if (bestInnerList.end() ==
            std::find(bestInnerList.begin(), bestInnerList.end(), pParentCluster))
      ...
      if (pBestOuter) {
        ...
        if (bestOuterList.end() ==
            std::find(bestOuterList.begin(), bestOuterList.end(), pParentCluster))
  ...
  StatusCode DeltaRayExtensionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(
      ...
      STATUS_CODE_NOT_FOUND,
      !=,
      XmlHelper::ReadValue(xmlHandle, "MaxClusterLength", m_maxClusterLength));
