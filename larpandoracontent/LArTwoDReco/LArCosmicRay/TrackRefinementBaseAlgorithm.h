 ...
 */
  ...
  protected:
    ...
    typedef std::unordered_map<const pandora::Cluster*, pandora::CaloHitList>
    ...
    public:
      /**
         ...
         */
      SortByDistanceAlongLine(const pandora::CartesianVector& startPoint,
     ...
     */
    template <typename T>
    void InitialiseContainers(const pandora::ClusterList* pClusterList,
     ...
     */
    bool GetClusterMergingCoordinates(const TwoDSlidingFitResult& clusterMicroFitResult,
    ...
    void GetHitsInBoundingBox(
      ...
      const float distanceToLine = -1.f) const;
     ...
     */
    bool IsInBoundingBox(const float minX,
                         const float maxX,
                         const float minZ,
                         const float maxZ,
                         const pandora::CartesianVector& hitPosition) const;
     ...
     */
    bool AreExtrapolatedHitsGood(const ClusterToCaloHitListMap& clusterToCaloHitListMap,
     ...
     */
    virtual bool AreExtrapolatedHitsNearBoundaries(
      const pandora::CaloHitVector& extrapolatedHitVector,
      ClusterAssociation& clusterAssociation) const = 0;
     ...
     *  @return  whether the check passes
     */
    bool IsNearBoundary(const pandora::CaloHit* const pCaloHit,
     ...
     */
    bool IsTrackContinuous(const ClusterAssociation& clusterAssociation,
     ...
     */
    void GetTrackSegmentBoundaries(const ClusterAssociation& clusterAssociation,
                                   pandora::CartesianPointVector& trackSegmentBoundaries) const;
     ...
     */
    void RepositionIfInGap(const pandora::CartesianVector& mergeDirection,
                           pandora::CartesianVector& trackPoint) const;
     ...
     */
    float DistanceInGap(const pandora::CartesianVector& upstreamPoint,
                        const pandora::CartesianVector& downstreamPoint,
                        const pandora::CartesianVector& connectingLine,
                        pandora::DetectorGapList& consideredGaps) const;
     ...
     *  @param  remnantClusterList the input list to store the remnant clusters
     */
    void AddHitsToMainTrack(const pandora::Cluster* const pMainTrackCluster,
                            ...
                            pandora::ClusterList& remnantClusterList) const;
     ...
     */
    void ProcessRemnantClusters(const pandora::ClusterList& remnantClusterList,
     ...
     */
    bool AddToNearestCluster(const pandora::Cluster* const pClusterToMerge,
                             const pandora::Cluster* const pMainTrackCluster,
     ...
     */
    void FragmentRemnantCluster(const pandora::Cluster* const pRemnantCluster,
                                pandora::ClusterList& fragmentedClusterList) const;
     ...
     */
    template <typename T>
    void UpdateContainers(const pandora::ClusterList& clustersToAdd,
     ...
     *  @param  slidingFitResultMapPair the {micro, macro} pair of [cluster -> TwoDSlidingFitResult] maps
     */
    void RemoveClusterFromContainers(const pandora::Cluster* const pClustertoRemove,
                                     pandora::ClusterVector& clusterVector,
                                     SlidingFitResultMapPair& slidingFitResultMapPair) const;

    float m_minClusterLength; ///< The minimum length of a considered cluster
    unsigned int
      m_microSlidingFitWindow; ///< The sliding fit window used in the fits contained within the microSlidingFitResultMap
    unsigned int
      m_macroSlidingFitWindow; ///< The sliding fit window used in the fits contained within the macroSlidingFitResultMap
    float
      m_stableRegionClusterFraction; ///< The threshold fraction of fit contributing layers which defines the stable region
    float
      m_mergePointMinCosAngleDeviation; ///< The threshold cos opening angle between the cluster local gradient and the associated cluster global gradient used to determine merge points
    float
      m_minHitFractionForHitRemoval; ///< The threshold fraction of hits to be removed from the cluster for hit removal to proceed
    float
      m_maxDistanceFromMainTrack; ///< The threshold distance for a hit to be added to the main track
    float
      m_maxHitDistanceFromCluster; ///< The threshold separation between a hit and cluster for the hit to be merged into the cluster
    float
      m_maxHitSeparationForConnectedCluster; ///< The maximum separation between two adjacent (in z) hits in a connected cluster
    unsigned int
      m_maxTrackGaps; ///< The maximum number of graps allowed in the extrapolated hit vector
