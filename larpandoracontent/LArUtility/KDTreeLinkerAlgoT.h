 ...
 */
  ...
  private:
    ...
    KDTreeNodeT<DATA, DIM>*
      nodePool_;       ///< Node pool allows us to do just 1 call to new for each tree building
  ...
  template <typename DATA, unsigned DIM>
  inline void KDTreeLinkerAlgo<DATA, DIM>::build(std::vector<KDTreeNodeInfoT<DATA, DIM>>& eltList,
  ...
  template <typename DATA, unsigned DIM>
  inline void KDTreeLinkerAlgo<DATA, DIM>::search(const KDTreeBoxT<DIM>& trackBox,
  ...
  template <typename DATA, unsigned DIM>
  inline void KDTreeLinkerAlgo<DATA, DIM>::recSearch(const KDTreeNodeT<DATA, DIM>* current,
                                                     const KDTreeBoxT<DIM>& trackBox)
  {
    ...
    if ((current->left == nullptr) && (current->right == nullptr)) {
      ...
      if (isFullyContained) { this->addSubtree(current->left); }
      ...
      if (isFullyContained) { this->addSubtree(current->right); }
      else if (hasIntersection) {
  ...
  template <typename DATA, unsigned DIM>
  inline void KDTreeLinkerAlgo<DATA, DIM>::recNearestNeighbour(
    unsigned int depth,
    const KDTreeNodeT<DATA, DIM>* current,
    const KDTreeNodeInfoT<DATA, DIM>& point,
    const KDTreeNodeT<DATA, DIM>*& best_match,
  ...
  template <typename DATA, unsigned DIM>
  inline float KDTreeLinkerAlgo<DATA, DIM>::dist2(const KDTreeNodeInfoT<DATA, DIM>& a,
                                                  const KDTreeNodeInfoT<DATA, DIM>& b) const
  {
    double d = 0.;
  ...
  template <typename DATA, unsigned DIM>
  inline KDTreeNodeT<DATA, DIM>*
  KDTreeLinkerAlgo<DATA, DIM>::recBuild(int low, int high, int depth, const KDTreeBoxT<DIM>& region)
  {
