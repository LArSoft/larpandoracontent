    ...
    for (const std::string& listName : m_pfoListNames) {
      try {
        ...
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "ListPruningAlgorithm: pfo list " << listName << " unavailable."
                    << std::endl;
      }
    }
    ...
    for (const std::string& listName : m_clusterListNames) {
      try {
        ...
        for (const Cluster* const pCluster : clusterList) {
          if (!m_warnIfObjectsUnavailable && !pCluster->IsAvailable()) continue;

          if (STATUS_CODE_SUCCESS != PandoraContentApi::Delete(*this, pCluster, listName) &&
              m_warnIfObjectsUnavailable)
            std::cout << "ListPruningAlgorithm: Could not delete Cluster." << std::endl;
        }
      }
      catch (StatusCodeException&) {
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "ListPruningAlgorithm: cluster list " << listName << " unavailable."
                    << std::endl;
      }
    }
    ...
    for (const std::string& listName : m_vertexListNames) {
      try {
        ...
        for (const Vertex* const pVertex : vertexList) {
          if (!m_warnIfObjectsUnavailable && !pVertex->IsAvailable()) continue;

          if (STATUS_CODE_SUCCESS != PandoraContentApi::Delete(*this, pVertex, listName) &&
              m_warnIfObjectsUnavailable)
        ...
        if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo())
          std::cout << "ListPruningAlgorithm: vertex list " << listName << " unavailable."
                    << std::endl;
      }
    }
