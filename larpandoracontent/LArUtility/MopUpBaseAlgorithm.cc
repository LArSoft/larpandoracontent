...
namespace lar_content {
  ...
  const std::string MopUpBaseAlgorithm::GetListName(const T* const pT) const
  {
    ...
    if (pCurrentList &&
        (pCurrentList->end() != std::find(pCurrentList->begin(), pCurrentList->end(), pT)))
    ...
    for (const std::string& listName : m_daughterListNames) {
      ...
      if (pList && (pList->end() != std::find(pList->begin(), pList->end(), pT))) return listName;
    }
  ...
  StatusCode MopUpBaseAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    if (m_daughterListNames.empty()) {
      std::cout << "MopUpBaseAlgorithm::ReadSettings - Must provide names of daughter object lists "
                   "for use in mop up."
                << std::endl;
