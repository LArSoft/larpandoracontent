  ...
  StatusCode ListChangingAlgorithm::Run()
  {
    if (!m_caloHitListName.empty()) {
      const StatusCode statusCode(
        PandoraContentApi::ReplaceCurrentList<CaloHit>(*this, m_caloHitListName));
      ...
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo() &&
        ...
        std::cout << "ListChangingAlgorithm: calohit list not found " << m_caloHitListName
                  << std::endl;
    }
    ...
    if (!m_clusterListName.empty()) {
      ...
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo() &&
          (STATUS_CODE_NOT_FOUND == statusCode))
        std::cout << "ListChangingAlgorithm: cluster list not found " << m_clusterListName
                  << std::endl;
    }
    ...
    if (!m_vertexListName.empty()) {
      ...
      if (PandoraContentApi::GetSettings(*this)->ShouldDisplayAlgorithmInfo() &&
          (STATUS_CODE_NOT_FOUND == statusCode))
        std::cout << "ListChangingAlgorithm: vertex list not found " << m_vertexListName
                  << std::endl;
    }
