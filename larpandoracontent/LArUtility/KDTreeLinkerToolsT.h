 ...
 */
  template <typename T>
  KDTreeBox fill_and_bound_2d_kd_tree(const MANAGED_CONTAINER<const T*>& points,
 ...
 */
  template <typename T>
  KDTreeCube fill_and_bound_3d_kd_tree(const MANAGED_CONTAINER<const T*>& points,
 ...
 *  @return KDTreeBox
 */
  KDTreeBox build_2d_kd_search_region(const pandora::CaloHit* const point,
                                      const float x_span,
                                      const float z_span);
 ...
 *  @return KDTreeBox
 */
  KDTreeBox build_2d_kd_search_region(const pandora::CartesianVector& pos,
                                      const float x_span,
                                      const float z_span);
 ...
 *  @return KDTreeCube
 */
  KDTreeCube build_3d_kd_search_region(const pandora::CaloHit* const point,
                                       const float x_span,
                                       const float y_span,
                                       const float z_span);
 ...
 *  @return KDTreeCube
 */
  KDTreeCube build_3d_kd_search_region(const pandora::CartesianVector& pos,
                                       const float x_span,
                                       const float y_span,
                                       const float z_span);
  ...
  template <>
  inline const pandora::CartesianVector& kdtree_type_adaptor<const pandora::CaloHit>::position(
    const pandora::CaloHit* const t)
  {
  ...
  kdtree_type_adaptor<const pandora::CartesianVector>::position(
    const pandora::CartesianVector* const t)
  {
    return *t;
  }
