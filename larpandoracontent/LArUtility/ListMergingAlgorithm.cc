...
namespace lar_content {
  ...
  StatusCode ListMergingAlgorithm::Run()
  {
    ...
    for (unsigned int iIndex = 0, iIndexEnd = m_sourceClusterListNames.size(); iIndex < iIndexEnd;
         ++iIndex) {
      ...
      const StatusCode statusCode(
        PandoraContentApi::SaveList<Cluster>(*this, sourceListName, targetListName));
      ...
      if (STATUS_CODE_SUCCESS != statusCode) {
        if (STATUS_CODE_NOT_FOUND == statusCode) {
          ...
          if (this->GetPandora().GetSettings()->ShouldDisplayAlgorithmInfo())
            std::cout << "ListMergingAlgorithm: no clusters to move, source: " << sourceListName
                      << ", target: " << targetListName << std::endl;
        }
        else {
          std::cout << "ListMergingAlgorithm: error in cluster merging, source: " << sourceListName
                    << ", target: " << targetListName << std::endl;
          return statusCode;
        }
      }
    ...
    // Pfo list merging
    if (m_sourcePfoListNames.size() != m_targetPfoListNames.size()) return STATUS_CODE_FAILURE;

    for (unsigned int iIndex = 0, iIndexEnd = m_sourcePfoListNames.size(); iIndex < iIndexEnd;
         ++iIndex) {
      ...
      const StatusCode statusCode(
        PandoraContentApi::SaveList<Pfo>(*this, sourceListName, targetListName));

      if (STATUS_CODE_SUCCESS != statusCode) {
        if (STATUS_CODE_NOT_FOUND == statusCode) {
          if (this->GetPandora().GetSettings()->ShouldDisplayAlgorithmInfo())
            std::cout << "ListMergingAlgorithm: pfo list not found, source: " << sourceListName
                      << ", target: " << targetListName << std::endl;
        }
        else if (STATUS_CODE_NOT_INITIALIZED == statusCode) {
          if (this->GetPandora().GetSettings()->ShouldDisplayAlgorithmInfo())
            std::cout << "ListMergingAlgorithm: no pfos to move, source: " << sourceListName
                      << ", target: " << targetListName << std::endl;
        }
        else {
          std::cout << "ListMergingAlgorithm: error in pfo merging, source: " << sourceListName
                    << ", target: " << targetListName << std::endl;
          return statusCode;
        }
      }
  ...
  StatusCode ListMergingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    if ((m_sourceClusterListNames.size() != m_targetClusterListNames.size()) ||
        (m_sourcePfoListNames.size() != m_targetPfoListNames.size())) {
