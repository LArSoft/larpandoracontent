...
namespace lar_dl_content {
    ...
    if (m_writeTree) {
      try {
        PANDORA_MONITORING_API(
          SaveTree(this->GetPandora(), m_rootTreeName, m_rootFileName, "RECREATE"));
      }
  ...
  StatusCode DlVertexingAlgorithm::PrepareTrainingSample()
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, this->CompleteMCHierarchy(mcToHitsMap, hierarchy));
    std::normal_distribution<> gauss(0.f, 15.f);

    for (const std::string& listname : m_caloHitListNames) {
      const CaloHitList* pCaloHitList(nullptr);
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetList(*this, listname, pCaloHitList));
      if (pCaloHitList->empty()) continue;
      ...
      for (const MCParticle* mc : hierarchy) {
        if (LArMCParticleHelper::IsNeutrino(mc)) vertices.push_back(mc->GetVertex());
      }
      ...
      // Vertices
      const LArTransformationPlugin* transform{
        this->GetPandora().GetPlugins()->GetLArTransformationPlugin()};
      ...
      for (const CaloHit* pCaloHit : *pCaloHitList) {
        ...
        if (m_pass > 1 && (x < xMin || x > xMax || z < zMin || z > zMax)) continue;
      ...
      if (nHits > 10) LArMvaHelper::ProduceTrainingExample(trainingFilename, true, featureVector);
    }
  ...
  StatusCode DlVertexingAlgorithm::Infer()
  {
    ...
    for (const std::string listName : m_caloHitListNames) {
      ...
      this->GetCanvasParameters(
        output, pixelVector, colOffset, rowOffset, canvasWidth, canvasHeight);
      ...
      for (const auto [row, col] : pixelVector) {
        ...
        if (cls > 0 && cls < m_nClasses) {
          const int inner{
            static_cast<int>(std::round(std::ceil(scaleFactor * m_thresholds[cls - 1])))};
          ...
          this->DrawRing(canvas,
                         ...
                         inner,
                         outer,
                         1.f / (outer * outer - inner * inner));
        }
      }

      CartesianPointVector positionVector;
      this->MakeWirePlaneCoordinatesFromCanvas(
        *pCaloHitList, canvas, canvasWidth, canvasHeight, colOffset, rowOffset, positionVector);
      if (isU)
      ...
      if (m_visualise) {
        PANDORA_MONITORING_API(
          SetEveDisplayParameters(this->GetPandora(), true, DETECTOR_VIEW_XZ, -1.f, 1.f, 1.f));
        try {
          ...
          if (isU) {
            ...
            PANDORA_MONITORING_API(
              AddMarkerToVisualization(this->GetPandora(), &trueVertex, "U(true)", BLUE, 3));
          }
          else if (isV) {
            const CartesianVector trueVertex(x, 0.f, v);
            PANDORA_MONITORING_API(
              AddMarkerToVisualization(this->GetPandora(), &trueVertex, "V(true)", BLUE, 3));
          }
          else if (isW) {
            const CartesianVector trueVertex(x, 0.f, w);
            PANDORA_MONITORING_API(
              AddMarkerToVisualization(this->GetPandora(), &trueVertex, "W(true)", BLUE, 3));
          }
        }
        catch (StatusCodeException& e) {
    ...
    int nEmptyLists{0};
    if (vertexCandidatesU.empty()) ++nEmptyLists;
    if (vertexCandidatesV.empty()) ++nEmptyLists;
    if (vertexCandidatesW.empty()) ++nEmptyLists;
    std::vector<VertexTuple> vertexTuples;
    CartesianPointVector candidates3D;
    if (nEmptyLists == 0) {
      vertexTuples.emplace_back(VertexTuple(this->GetPandora(),
                                            vertexCandidatesU.front(),
                                            vertexCandidatesV.front(),
                                            vertexCandidatesW.front()));
    }
    else if (nEmptyLists == 1) {
      if (vertexCandidatesU.empty()) { // V and W available
        vertexTuples.emplace_back(VertexTuple(this->GetPandora(),
                                              vertexCandidatesV.front(),
                                              vertexCandidatesW.front(),
                                              TPC_VIEW_V,
      ...
      else if (vertexCandidatesV.empty()) { // U and W available
        vertexTuples.emplace_back(VertexTuple(this->GetPandora(),
                                              vertexCandidatesU.front(),
                                              vertexCandidatesW.front(),
                                              TPC_VIEW_U,
    ...
    else { // Not enough views to reconstruct a 3D vertex
      std::cout << "Insufficient 2D vertices to reconstruct a 3D vertex" << std::endl;
      return STATUS_CODE_NOT_FOUND;
    }

    if (m_visualise) {
      PANDORA_MONITORING_API(AddMarkerToVisualization(
        this->GetPandora(), &vertexTuples.front().GetPosition(), "candidate", GREEN, 1));
    }
    ...
    if (!vertexTuples.empty()) {
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, this->MakeCandidateVertexList(vertexCandidates));
    }
    else {
      std::cout << "Insufficient 2D vertices to reconstruct a 3D vertex" << std::endl;
      return STATUS_CODE_NOT_FOUND;
    ...
    if (m_visualise) { PANDORA_MONITORING_API(ViewEvent(this->GetPandora())); }
  ...
  StatusCode DlVertexingAlgorithm::MakeNetworkInputFromHits(const pandora::CaloHitList& caloHits,
                                                            ...
                                                            PixelVector& pixelVector) const
  {
    ...
    for (const CaloHit* pCaloHit : caloHits) {
      ...
      const float adc{pCaloHit->GetInputEnergy() < m_maxHitAdc ? pCaloHit->GetInputEnergy() :
                                                                 m_maxHitAdc};
      ...
      if (accessor[0][0][pixelZ][pixelX] > maxValue) maxValue = accessor[0][0][pixelZ][pixelX];
    }
    if (maxValue > 0) {
      for (int row = 0; row < m_height; ++row) {
        for (int col = 0; col < m_width; ++col) {
          ...
          if (value > 0) pixelVector.emplace_back(std::make_pair(row, col));
        }
      }
    }
  ...
  StatusCode DlVertexingAlgorithm::MakeWirePlaneCoordinatesFromCanvas(
    const CaloHitList& caloHits,
    float** canvas,
  ...
  void DlVertexingAlgorithm::GetCanvasParameters(const LArDLHelper::TorchOutput& networkOutput,
                                                 const PixelVector& pixelVector,
                                                 int& colOffset,
                                                 int& rowOffset,
                                                 int& width,
                                                 int& height) const
  {
    ...
    for (const auto [row, col] : pixelVector) {
      ...
      if (threshold > 0. && threshold < 1.) {
        ...
        if ((row - distance) < rowOffsetMin) rowOffsetMin = row - distance;
        if ((row + distance) > rowOffsetMax) rowOffsetMax = row + distance;
        if ((col - distance) < colOffsetMin) colOffsetMin = col - distance;
        if ((col + distance) > colOffsetMax) colOffsetMax = col + distance;
      }
    }
  ...
  void DlVertexingAlgorithm::DrawRing(float** canvas,
                                      const int row,
                                      const int col,
                                      const int inner,
                                      const int outer,
                                      const float weight) const
  {
    ...
    while (c2 >= r2) {
      ...
      if (
        c1 <=
        r1) { // We've completed the arc of the inner ring already, so just move radially out from here (see Update comment below)
        cp1 = r2;
      }
      ...
      for (int c = cp1; c <= cp2; ++c) {
        ...
        if (rp2 != c) canvas[row + c][col + rp2] += weight;
        if (rp2 != 0 && cp2 != 0) {
          canvas[row - rp2][col - c] += weight;
          if (rp2 != c) canvas[row - c][col - rp2] += weight;
        }
      ...
      if (c1 > r1) this->Update(inner2, c1, r1);
  ...
  StatusCode DlVertexingAlgorithm::GetMCToHitsMap(
    LArMCParticleHelper::MCContributionMap& mcToHitsMap) const
  {
    const CaloHitList* pCaloHitList2D(nullptr);
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::GetList(*this, "CaloHitList2D", pCaloHitList2D));
  ...
  StatusCode DlVertexingAlgorithm::CompleteMCHierarchy(
    const LArMCParticleHelper::MCContributionMap& mcToHitsMap,
    MCParticleList& mcHierarchy) const
  {
    try {
    ...
    auto pivot =
      std::find_if(mcHierarchy.begin(), mcHierarchy.end(), [](const MCParticle* mc) -> bool {
  ...
  void DlVertexingAlgorithm::GetHitRegion(const CaloHitList& caloHitList,
                                          float& xMin,
                                          float& xMax,
                                          float& zMin,
                                          float& zMax) const
  {
    ...
    if (m_pass > 1) {
      ...
      const LArTransformationPlugin* transform{
        this->GetPandora().GetPlugins()->GetLArTransformationPlugin()};
      switch (view) {
      case TPC_VIEW_U: zCentroid = transform->YZtoU(centroid.GetY(), centroid.GetZ()); break;
      case TPC_VIEW_V: zCentroid = transform->YZtoV(centroid.GetY(), centroid.GetZ()); break;
      case TPC_VIEW_W: zCentroid = transform->YZtoW(centroid.GetY(), centroid.GetZ()); break;
      default: throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER); break;
      }
  ...
  StatusCode DlVertexingAlgorithm::MakeCandidateVertexList(const CartesianPointVector& positions)
  {
    ...
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::CreateTemporaryListAndSetCurrent(*this, pVertexList, temporaryListName));
    ...
    for (const CartesianVector position : positions) {
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, PandoraContentApi::Vertex::Create(*this, parameters, pVertex));
    }
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, PandoraContentApi::SaveList<Vertex>(*this, m_outputVertexListName));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      PandoraContentApi::ReplaceCurrentList<Vertex>(*this, m_outputVertexListName));
    ...
    if (STATUS_CODE_SUCCESS == PandoraContentApi::GetCurrentList(*this, pMCParticleList) &&
      ...
      if (!primaries.empty()) {
        ...
        if (parents.size() == 1) {
          ...
          if (LArMCParticleHelper::IsNeutrino(trueNeutrino)) return primaries.front()->GetVertex();
        }
      }
    }
  ...
  void DlVertexingAlgorithm::PopulateRootTree(
          ...
          if (!primaries.empty()) {
            ...
            if (parents.size() == 1) {
              ...
              if (LArMCParticleHelper::IsNeutrino(trueNeutrino)) {
                ...
                if (LArVertexHelper::IsInFiducialVolume(
                      this->GetPandora(), trueVertex, "dune_fd_hd")) {
                  ...
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "event", m_event));
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "pass", m_pass));
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "dr_u", dr_u));
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "dr_v", dr_v));
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "dr_w", dr_w));
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "dr", dr));
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "dx", dx));
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "dy", dy));
                  PANDORA_MONITORING_API(
                    SetTreeVariable(this->GetPandora(), m_rootTreeName, "dz", dz));
  ...
  StatusCode DlVertexingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "Pass", m_pass));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "ImageHeight", m_height));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "ImageWidth", m_width));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS,
      !=,
      XmlHelper::ReadVectorOfValues(xmlHandle, "DistanceThresholds", m_thresholds));
    ...
    if (m_trainingMode) {
      ...
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "ModelFileNameU", modelName));
      modelName = LArFileHelper::FindFileInPath(modelName, "FW_SEARCH_PATH");
      LArDLHelper::LoadModel(modelName, m_modelU);
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "ModelFileNameV", modelName));
      modelName = LArFileHelper::FindFileInPath(modelName, "FW_SEARCH_PATH");
      LArDLHelper::LoadModel(modelName, m_modelV);
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "ModelFileNameW", modelName));
      modelName = LArFileHelper::FindFileInPath(modelName, "FW_SEARCH_PATH");
      LArDLHelper::LoadModel(modelName, m_modelW);
      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "MaxHitAdc", m_maxHitAdc));
      PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                      STATUS_CODE_NOT_FOUND,
                                      !=,
      ...
      if (m_writeTree) {
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "RootTreeName", m_rootTreeName));
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "RootFileName", m_rootFileName));
      }

      PANDORA_RETURN_RESULT_IF(
        STATUS_CODE_SUCCESS,
        !=,
        XmlHelper::ReadValue(xmlHandle, "OutputVertexListName", m_outputVertexListName));
      if (m_pass > 1) {
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS,
          !=,
          XmlHelper::ReadValue(xmlHandle, "InputVertexListName", m_inputVertexListName));
      }
    }
  ...
  DlVertexingAlgorithm::VertexTuple::VertexTuple(const Pandora& pandora,
    ...
    : m_pos{0.f, 0.f, 0.f}, m_chi2{0.f}
  {
    ...
    std::cout << "Merging 2, position (" << m_pos.GetX() << ", " << m_pos.GetY() << ", "
              << m_pos.GetZ() << ") with chi2 " << m_chi2 << std::endl;
  }

  //-----------------------------------------------------------------------------------------------------------------------------------------

  const CartesianVector& DlVertexingAlgorithm::VertexTuple::GetPosition() const { return m_pos; }

  //-----------------------------------------------------------------------------------------------------------------------------------------

  float DlVertexingAlgorithm::VertexTuple::GetChi2() const { return m_chi2; }
  ...
  std::string DlVertexingAlgorithm::VertexTuple::ToString() const
  {
    ...
    return "3D pos: (" + std::to_string(x) + ", " + std::to_string(y) + ", " + std::to_string(z) +
