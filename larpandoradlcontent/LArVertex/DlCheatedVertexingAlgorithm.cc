/**
 *  @file   larpandoradlcontent/LArVertex/DlCheatedVertexingAlgorithm.cc
 *
 *  @brief  Implementation of the cheated deep learning vertexing algorithm.
 *
 *  $Log: $
 */
...
DlCheatedVertexingAlgorithm::DlCheatedVertexingAlgorithm() :
...
StatusCode DlCheatedVertexingAlgorithm::Run()
{
    if (m_trainingMode)
...
StatusCode DlCheatedVertexingAlgorithm::PrepareTrainingSample()
{
...
StatusCode DlCheatedVertexingAlgorithm::Infer()
{
    if (m_pass == 1)
        ++m_event;

    // Cheating! Drop all CR hits from slice.
    std::cout << "Starting to infer, cheating first!" << std::endl;
    LArMCParticleHelper::MCContributionMap mcToHitsMap;
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, this->GetMCToHitsMap(mcToHitsMap));
    MCParticleList hierarchy;
    PANDORA_RETURN_RESULT_IF(STATUS_CODE_SUCCESS, !=, this->CompleteMCHierarchy(mcToHitsMap, hierarchy));

    std::map<const CaloHit*, int> hitHasMC;
    for (const MCParticle *mc : hierarchy)
    {
        for (const auto hit : mcToHitsMap[mc])
            hitHasMC.insert({hit, mc->GetParticleId()});
    }
    std::cout << "Populated map for cheating..." << std::endl;
    std::cout << "There is " << hitHasMC.size() << " entries to use!" << std::endl;
    //=========================================================================
    ...
    for (const std::string &listname : m_caloHitListNames)
    {
        ...
        // Perform filtering using cheated map.
        std::vector<const CaloHit*> filteredHitsVector;
        std::cout << "Starting to filter..." << std::endl;
        std::copy_if(pCaloHitList->begin(), pCaloHitList->end(), std::back_inserter(filteredHitsVector), [&](const CaloHit *hit){ return hitHasMC.count(hit) > 0; });
        CaloHitList filteredHits(filteredHitsVector.begin(), filteredHitsVector.end());
        std::cout << "There was " << pCaloHitList->size() << " hits pre-filtering, then only " << filteredHits.size() << "!" << std::endl;

        this->GetHitRegion(filteredHits, viewDriftMin, viewDriftMax, wireMin[view], wireMax[view]);
        driftMin = std::min(viewDriftMin, driftMin);
        driftMax = std::max(viewDriftMax, driftMax);
    }
    ...
    for (const std::string &listName : m_caloHitListNames)
    {
        ...
        this->MakeNetworkInputFromHits(filteredHits, view, driftMin, driftMax, wireMin[view], wireMax[view], input, pixelVector);
        ...
        if (m_visualise)
        {
            ...
            {
                std::cerr << "DlCheatedVertexingAlgorithm: Warning. Couldn't find true vertex." << std::endl;
            }
...
StatusCode DlCheatedVertexingAlgorithm::MakeNetworkInputFromHits(const CaloHitList &caloHits, const HitType view, const float xMin,
...
StatusCode DlCheatedVertexingAlgorithm::MakeWirePlaneCoordinatesFromCanvas(float **canvas, const int canvasWidth, const int canvasHeight,
...
void DlCheatedVertexingAlgorithm::GetCanvasParameters(const LArDLHelper::TorchOutput &networkOutput, const PixelVector &pixelVector,
...
void DlCheatedVertexingAlgorithm::DrawRing(float **canvas, const int row, const int col, const int inner, const int outer, const float weight) const
{
...
void DlCheatedVertexingAlgorithm::Update(const int radius2, int &col, int &row) const
{
...
StatusCode DlCheatedVertexingAlgorithm::GetMCToHitsMap(LArMCParticleHelper::MCContributionMap &mcToHitsMap) const
{
...
StatusCode DlCheatedVertexingAlgorithm::CompleteMCHierarchy(const LArMCParticleHelper::MCContributionMap &mcToHitsMap, MCParticleList &mcHierarchy) const
{
    try
    {
...
void DlCheatedVertexingAlgorithm::GetHitRegion(const CaloHitList &caloHitList, float &xMin, float &xMax, float &zMin, float &zMax) const
{
...
StatusCode DlCheatedVertexingAlgorithm::MakeCandidateVertexList(const CartesianPointVector &positions)
{
    const VertexList *pVertexList{nullptr};
...
void DlCheatedVertexingAlgorithm::GetTrueVertexPosition(float &x, float &y, float &z) const
{
...
void DlCheatedVertexingAlgorithm::GetTrueVertexPosition(float &x, float &u, float &v, float &w) const
{
    const CartesianVector &trueVertex{this->GetTrueVertex()};
    const LArTransformationPlugin *transform{this->GetPandora().GetPlugins()->GetLArTransformationPlugin()};
    x = trueVertex.GetX();
...
const CartesianVector &DlCheatedVertexingAlgorithm::GetTrueVertex() const
{
    const MCParticleList *pMCParticleList{nullptr};
...
#ifdef MONITORING
void DlCheatedVertexingAlgorithm::PopulateRootTree(const std::vector<VertexTuple> &vertexTuples, const pandora::CartesianPointVector &vertexCandidatesU,
...
StatusCode DlCheatedVertexingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
...
DlCheatedVertexingAlgorithm::VertexTuple::VertexTuple(
...
const CartesianVector &DlCheatedVertexingAlgorithm::VertexTuple::GetPosition() const
{
    return m_pos;
}
...
float DlCheatedVertexingAlgorithm::VertexTuple::GetChi2() const
{
    return m_chi2;
}
...
std::string DlCheatedVertexingAlgorithm::VertexTuple::ToString() const
{
