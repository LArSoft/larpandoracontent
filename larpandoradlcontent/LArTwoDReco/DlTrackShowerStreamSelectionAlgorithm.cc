...
namespace lar_dl_content {
  ...
  StatusCode DlTrackShowerStreamSelectionAlgorithm::AllocateToStreams(const Cluster* const pCluster)
  {
    ...
    try {
      ...
      if (N > 0) {
        float mean{std::accumulate(std::begin(trackLikelihoods), std::end(trackLikelihoods), 0.f) /
                   N};
        if (mean >= 0.5f)
  ...
  StatusCode DlTrackShowerStreamSelectionAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, StreamSelectionAlgorithm::ReadSettings(xmlHandle));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "TrackListName", m_trackListName));
    PANDORA_RETURN_RESULT_IF(
      STATUS_CODE_SUCCESS, !=, XmlHelper::ReadValue(xmlHandle, "ShowerListName", m_showerListName));
