...
namespace lar_dl_content {
      ...
      for (const CaloHit* pCaloHit : *pCaloHitList) {
        ...
        try {
          ...
          if (targetMCParticleToHitsMap.find(pMCParticle) == targetMCParticleToHitsMap.end())
            continue;
          if (LArMCParticleHelper::IsDescendentOf(pMCParticle, 2112)) continue;
          inputEnergy = pCaloHit->GetInputEnergy();
          if (inputEnergy < 0.f) continue;
  ...
  StatusCode DlHitTrackShowerIdAlgorithm::Infer()
  {
    ...
    for (const std::string listName : m_caloHitListNames) {
      ...
      LArDLHelper::TorchModel& model{
        view == TPC_VIEW_U ? m_modelU : (view == TPC_VIEW_V ? m_modelV : m_modelW)};
      ...
      for (int i = 0; i < nTiles; ++i) {
        for (const CaloHit* pCaloHit : *pCaloHitList) {
          ...
          if (tile == i) {
            ...
            const int pixelZ = (m_imageHeight - 1) -
                               static_cast<int>(std::floor(localZ * m_imageHeight / m_tileSize));
        ...
        float chargeMin{std::numeric_limits<float>::max()},
          chargeMax{-std::numeric_limits<float>::max()};
        for (int r = 0; r < m_imageHeight; ++r) {
          for (int c = 0; c < m_imageWidth; ++c) {
            if (weights[r][c] > chargeMax) chargeMax = weights[r][c];
            if (weights[r][c] < chargeMin) chargeMin = weights[r][c];
          }
        }
        float chargeRange{chargeMax - chargeMin};
        if (chargeRange <= 0.f) chargeRange = 1.f;
        ...
        for (const CaloHit* pCaloHit : *pCaloHitList) {
          ...
          if (tile == i) {
            ...
            caloHitToPixelMap.insert(
              std::make_pair(pCaloHit, std::make_tuple(tileZ, tileX, pixelZ, pixelX)));
          }
        }
        // Reset weights
        ...
        for (const CaloHit* pCaloHit : *pCaloHitList) {
          ...
          const int tile = sparseMap.at(tileZ * nTilesX + tileX);
          if (tile == i) { // Make sure we're looking at a hit in the correct tile
      ...
      if (m_visualize) {
        ...
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &trackHits, trackListName, BLUE));
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &showerHits, showerListName, RED));
        PANDORA_MONITORING_API(
          VisualizeCaloHits(this->GetPandora(), &otherHits, otherListName, BLACK));
      }
    }

    if (m_visualize) { PANDORA_MONITORING_API(ViewEvent(this->GetPandora())); }
  ...
  void DlHitTrackShowerIdAlgorithm::GetHitRegion(const CaloHitList& caloHitList,
                                                 float& xMin,
                                                 float& xMax,
                                                 float& zMin,
                                                 float& zMax)
  ...
  void DlHitTrackShowerIdAlgorithm::GetSparseTileMap(const CaloHitList& caloHitList,
                                                     const float xMin,
                                                     const float zMin,
                                                     const int nTilesX,
                                                     PixelToTileMap& sparseMap)
  ...
  StatusCode DlHitTrackShowerIdAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
  {
    ...
    if (m_useTrainingMode) {
      ...
      PANDORA_RETURN_RESULT_IF_AND_IF(
        ...
        STATUS_CODE_NOT_FOUND,
        !=,
        XmlHelper::ReadValue(xmlHandle, "ModelFileNameU", m_modelFileNameU));
      if (!m_modelFileNameU.empty()) {
        m_modelFileNameU = LArFileHelper::FindFileInPath(m_modelFileNameU, "FW_SEARCH_PATH");
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, LArDLHelper::LoadModel(m_modelFileNameU, m_modelU));
        modelLoaded = true;
      }
      PANDORA_RETURN_RESULT_IF_AND_IF(
        STATUS_CODE_SUCCESS,
        STATUS_CODE_NOT_FOUND,
        !=,
        XmlHelper::ReadValue(xmlHandle, "ModelFileNameV", m_modelFileNameV));
      if (!m_modelFileNameV.empty()) {
        m_modelFileNameV = LArFileHelper::FindFileInPath(m_modelFileNameV, "FW_SEARCH_PATH");
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, LArDLHelper::LoadModel(m_modelFileNameV, m_modelV));
        modelLoaded = true;
      }
      PANDORA_RETURN_RESULT_IF_AND_IF(
        STATUS_CODE_SUCCESS,
        STATUS_CODE_NOT_FOUND,
        !=,
        XmlHelper::ReadValue(xmlHandle, "ModelFileNameW", m_modelFileNameW));
      if (!m_modelFileNameW.empty()) {
        m_modelFileNameW = LArFileHelper::FindFileInPath(m_modelFileNameW, "FW_SEARCH_PATH");
        PANDORA_RETURN_RESULT_IF(
          STATUS_CODE_SUCCESS, !=, LArDLHelper::LoadModel(m_modelFileNameW, m_modelW));
        modelLoaded = true;
      }
      if (!modelLoaded) {
        std::cout << "Error: Inference requested, but no model files were successfully loaded"
                  << std::endl;
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "ImageHeight", m_imageHeight));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "ImageWidth", m_imageWidth));
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS,
                                    STATUS_CODE_NOT_FOUND,
                                    !=,
                                    XmlHelper::ReadValue(xmlHandle, "TileSize", m_tileSize));
